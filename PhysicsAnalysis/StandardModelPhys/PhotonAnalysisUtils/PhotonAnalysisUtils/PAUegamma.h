...
///////////////////////////////////////////////////////////////////
// PAUegamma.h, (c) ATLAS Detector software 2007
///////////////////////////////////////////////////////////////////
//
// This file defines the class PAU::egamma, which is a "wrapper" for photon and electron objects
// All PAU code handles photons through PAU::egamma class, so that any additional UserData attributes
// can be stored here
//



#ifndef PHOTONANALYSISUTILS_PAUEGAMMA_H
#define PHOTONANALYSISUTILS_PAUEGAMMA_H
...
#include "PhotonAnalysisUtils/PAUegammaAttr.h"
...
namespace PAU {
  ...
  class egamma : public IAthenaBarCode, public egammaAttr, public egammaTruth   {
  private:
    ...
    const EMConvert*           m_convert_GSF;
    const EMConvert*           m_convert_DNA;
    //
    void set4mom() { 
      if ( m_ph ) {
  ...
  public:
    ...
    // default constructor
    egamma() : IAthenaBarCode(), m_ph(0)  , m_el(0)  , m_vx(0)  , m_trk(0), m_conv_flag(0), m_4mom(), m_4momclus(), m_shower(0), m_convert_GSF(0), m_convert_DNA(0) {} ;
    
    // constructors from Analysis::Photon
    
    egamma(const Analysis::Photon* ph, int flag=0)
      : IAthenaBarCode(), egammaAttr(), egammaTruth(), 
	m_ph(ph) , m_el(0) , m_vx(ph->conversion()) , m_trk(ph->trackParticle()) , m_conv_flag(flag) , m_shower(0), m_convert_GSF(0), m_convert_DNA(0)
    {set4mom();} ;
    egamma(const Analysis::Photon* ph,const Trk::VxCandidate* vx, int flag)
      : IAthenaBarCode(), egammaAttr(), egammaTruth(), 
	m_ph(ph) , m_el(0) , m_vx(vx) , m_trk(ph->trackParticle()) , m_conv_flag(flag) , m_shower(0), m_convert_GSF(0), m_convert_DNA(0)
    {set4mom();} ;
    
    // constructors from Analysis::Electron
    
    egamma(const Analysis::Electron* el, int flag=0)
      : IAthenaBarCode(), egammaAttr(), egammaTruth(), 
	m_ph(0) , m_el(el) , m_vx(el->conversion()) , m_trk(el->trackParticle()) , m_conv_flag(flag) , m_shower(0), m_convert_GSF(0), m_convert_DNA(0)
    {set4mom(); std::cout<<"DEPRECATED! create PAU::egamma from Electron"<<std::endl;} ;
    egamma(const Analysis::Electron* el,const Trk::VxCandidate* vx, int flag)
      : IAthenaBarCode(), egammaAttr(), egammaTruth(), 
	m_ph(0) , m_el(el) , m_vx(vx) , m_trk(el->trackParticle()) , m_conv_flag(flag) , m_shower(0), m_convert_GSF(0), m_convert_DNA(0)
    {set4mom(); std::cout<<"DEPRECATED! create PAU::egamma from Electron"<<std::endl;} ;
    
    egamma(const Analysis::Electron* el, int flag=-1, bool set_four_mom=true)
      : IAthenaBarCode(), egammaAttr(), egammaTruth(), 
	m_ph(0) , m_el(el) , m_vx(0), m_trk(el->trackParticle()) , m_conv_flag(flag) , m_shower(0), m_convert_GSF(0), m_convert_DNA(0)
    {if(set_four_mom) set4mom();} ;
    
    // copy constructor
    egamma(const egamma& g) : IAthenaBarCode(), egammaAttr(g), egammaTruth(g), 
			      m_ph(g.m_ph) , m_el(g.m_el) , m_vx(g.m_vx) , m_trk(g.m_trk) , m_conv_flag(g.m_conv_flag) , m_shower(g.m_shower), m_convert_GSF(g.m_convert_GSF), m_convert_DNA(g.m_convert_DNA) {} ;
    ...
    inline void setConvertGSF(const EMConvert* the_convert) { m_convert_GSF = the_convert ; }

    inline const EMConvert* convertGSF(const std::string& egDetailContainerName="") const {
      const ::egamma* eg = 0;
      if(m_ph) eg = m_ph;
      if(m_el) eg = m_el;
      if(eg == 0) return 0;
      ...
      if ( egDetailContainerName == "" ) {
	return m_convert_GSF;
      } else {
	for(int i=0; i<eg->nDetails(); i++){
	  if(eg->detailName(i) != egDetailContainerName) continue;
	  const egDetail *detail = eg->detail(i);
	  
	  const EMConvert *convert=0;
	  convert = dynamic_cast<const EMConvert*>(detail);
	  if(convert){
	    if(convert->linkIndex()==0 && convert->vertex_track1_author()==4)//GSF, best match
	      return convert;
	  }
	}
      }	  
    ...
    inline void setConvertDNA(const EMConvert* the_convert) { m_convert_DNA = the_convert ; }

    inline const EMConvert* convertDNA(const std::string& egDetailContainerName="") const {
      const ::egamma* eg = 0;
      if(m_ph) eg = m_ph;
      if(m_el) eg = m_el;
      if(eg == 0) return 0;
	...
	for(int i=0; i<eg->nDetails(); i++){
	  ...
	  convert = dynamic_cast<const EMConvert*>(detail);
	  if(convert){
	    if(convert->linkIndex()==0 && convert->vertex_track1_author()==3)//DNA, best match
	      return convert;
	  }
	}
      }	  
...
#endif // PHOTONANALYSISUTILS_PAUEGAMMA_H
