...
//
// - How to initialize the SystematicSet?
//   - One way is to have everything initialized in the constructors, rather
//     than an insert method. This way the hash and/or joined string name can
//     be computed just once, at initialization. The user will then need to
//     pass an array or vector of systematic names to the constructor.
//   - Another way is to allow the user to modify non-const SystematicSets
//     but handle the caching in the registry in accessor methods that return
//     the SystematicSet via a const reference.
//   - For this draft of the implementation, the 2nd option above is used.
