...
void Database::importSystTH1(const TH1* hist, EfficiencyType type, const std::string& sysname)
{
    ...
    {
        if ((float)eff->nominal != (float)hist->GetBinContent(x, y, z)){ syst_central_equal_nom_central = false;}
        ...
        if ( (float) hist->GetBinError(x, y, z) != (float) stat_up ) { syst_errors_equal_nom_errors = false;}
        ++eff;
    }
