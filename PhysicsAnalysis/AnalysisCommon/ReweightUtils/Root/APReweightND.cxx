...
APReweightND::APReweightND() : APReweightBase() {
  _isQuiet = false;
}
...
APReweightND::APReweightND(THnSparse* denominator_in, THnSparse* numerator_in, bool isTrig) : APReweightBase() {
  ...
  if( _isTrig ) {
    ...
    while( 1 ) {
      if( _numerator_hist->GetBinContent(coords) > _denominator_hist->GetBinContent(coords) ) {
        ...
        _numerator_hist->SetBinContent(coords, 0);
      }
