...
class _BTaggingFlags:
    ...
    # Which configuration scheme to use by default.
    _ConfigurationScheme = ['ConfigurationScheme']

    # WOUTER: This is a temporary measure. It allows the JetRec people to test setting up b-tagging from their end without our job options setting it also up (and therefore ending up with a double setup). Do not switch to True unless you know what you are doing.
    DoNotSetupBTagging = False
    ...
    def __init__(self):
        ...
        for attr in self._tags:
            ...
            if attr == 'MV2c10Flip':
                setattr(self, attr, False)
            if attr == 'MV2c10muFlip':
                setattr(self, attr, False)
            if attr == 'MV2c10rnnFlip':
                setattr(self, attr, False)
            if attr == 'MV2c20Flip':
                setattr(self, attr, False)
            if attr == 'MV2c100Flip':
        ...
        for attr in self._HighPriorityTaggers:
            setattr(self, attr, ['IP3D', 'SV1', 'BasicJetFitter', 'JetFitterTag', 'JetFitterNN', 'MV2c10', 'MV2c10mu', 'MV2c100', 'MV2c10rnn', 'MV2cl100', 'IP3DFlip', 'IP3DPos', 'IP3DNeg', 'IP3DSpc', 'IP3DSpcPos', 'IP3DSpcNeg', 'SV1Flip',
                                 'JetFitterTagFlip', 'JetFitterNNFlip', 'MV1Flip', 'MV1cFlip', 'MV2c10Flip', 'MV2c10muFlip', 'MV2c10rnnFlip', 'MV2c100Flip', 'MV2cl100Flip', 'JetVertexCharge', 'DL1', 'DL1Flip', 'DL1mu', 'DL1muFlip', 'DL1rnn', 'DL1rnnFlip', 'SoftMu'])
        for attr in self._MediumPriorityTaggers:
            setattr(self, attr, ['SV0', 'IP2D', 'IP2DFlip', 'IP2DPos', 'IP2DNeg', 'IP2DSpc',
                                 'IP2DSpcPos', 'IP2DSpcNeg', 'SoftEl', 'SoftMuChi2', 'MV2', 'MV2Flip'])
        for attr in self._LowPriorityTaggers:
            setattr(self, attr, ['IP1D', 'SV2', 'SV2Flip', 'TrackCounting',
    ...
    def Print(self):
        ...
        for attr in self._TriggerTaggers:
            print(" ".join([tag for tag in getattr(self, attr)
                            if (tag in self._tags and getattr(self, tag))]))
        print
