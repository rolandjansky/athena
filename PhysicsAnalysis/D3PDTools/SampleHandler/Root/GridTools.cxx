...
namespace SH
{
  ...
  std::vector<RucioListDidsEntry> rucioListDids (const std::string& dataset)
  {
    ...
    std::string output = sh::exec_read ("source $ATLAS_LOCAL_ROOT_BASE/user/atlasLocalSetup.sh && lsetup --force 'rucio -w' && echo " + separator + " && rucio list-dids " + sh::quote (dataset));
    auto split = output.rfind (separator + "\n");
    if (split == std::string::npos)
      RCU_THROW_MSG ("couldn't find separator in: " + output);
  ...
  rucioListFileReplicas (const std::string& dataset)
  {
    ...
    std::string command = "source $ATLAS_LOCAL_ROOT_BASE/user/atlasLocalSetup.sh && lsetup --force 'rucio -w' && echo " + separator + " && rucio list-file-replicas " + sh::quote (dataset);

    ANA_MSG_INFO ("querying rucio for dataset " << dataset);
    std::string output = sh::exec_read ("source $ATLAS_LOCAL_ROOT_BASE/user/atlasLocalSetup.sh && lsetup --force 'rucio -w' && echo " + separator + " && rucio list-file-replicas " + sh::quote (dataset));
    auto split = output.rfind (separator + "\n");
    if (split == std::string::npos)
      RCU_THROW_MSG ("couldn't find separator in: " + output);
  ...
  rucioGetMetadata (const std::set<std::string>& datasets)
  {
    ...
    std::string command = "source $ATLAS_LOCAL_ROOT_BASE/user/atlasLocalSetup.sh && lsetup --force 'rucio -w' && echo " + separator + " && rucio get-metadata";
                                     ...
                                     const std::string& dataset)
  {
    ...
    const std::string separator = "------- SampleHandler Split -------";
    std::string command = "source $ATLAS_LOCAL_ROOT_BASE/user/atlasLocalSetup.sh && lsetup --force 'rucio -w' && echo " + separator + " && cd " + sh::quote (location) + " && rucio download " + sh::quote (dataset) + " 2>&1";
