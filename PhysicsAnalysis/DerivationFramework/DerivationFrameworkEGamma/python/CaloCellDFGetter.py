...
#==========================================================================================
#This is basically a copy  of Calorimeter/CaloRec/python/CaloCellAODGetter.py
#Slightly modified as  to be usable in derivation.
#
...
class CaloCellDFGetter () :
    ...
    def configure(self):
        ...
        if  not hasattr(DerivationFrameworkJob,"CaloCellMakerFromCluster_"+self.outputCellKey()):
            mlog.info("Make Cell Collection  %s ",  self.outputCellKey())
            DerivationFrameworkJob+=self.theCaloCellMaker 
        else :
            mlog.info("CaloCellMakerFromCluster_%s exists already not adding it again",self.outputCellKey())        
        if not hasattr(DerivationFrameworkJob,"CaloClusterCellLinksUpdater_"+self.outputCellKey()):        
            mlog.info("Update _links for %s to point at %s", str(self.inputClusterKeys()), self.outputCellKey())
            DerivationFrameworkJob +=self.theClusterUpdater
        else :
