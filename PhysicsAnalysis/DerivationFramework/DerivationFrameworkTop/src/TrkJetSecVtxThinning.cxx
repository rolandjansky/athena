...
namespace DerivationFramework {
  TrkJetSecVtxThinning::TrkJetSecVtxThinning(const std::string& type,
    ...
    declareProperty("SecVtxContainerKey", m_verticesSGKey,
                    "StoreGate key for the SecVtx vertices container");
    declareProperty("ApplyAnd", m_and,
                    "Use IThinningSvc::Operator::And instead of "
                    "IThinningSvc::Operator::Or");
  }

  StatusCode TrkJetSecVtxThinning::initialize() {
    ...
    if (m_verticesSGKey == "") {
      ATH_MSG_FATAL("No BTagging_*_SecVtx collection provided for thinning.");
      return StatusCode::FAILURE;
    }
  ...
  StatusCode TrkJetSecVtxThinning::finalize() {
  ...
  StatusCode TrkJetSecVtxThinning::doThinning() const {

    // Retrieve the vertex containers
    const xAOD::VertexContainer* SecVtxVertices = nullptr;

    // Set up a mask with the same entries as the vertex container
    std::vector<bool> mask;
    ATH_CHECK(evtStore()->retrieve(SecVtxVertices, m_verticesSGKey));
    mask.assign(SecVtxVertices->size(), false);
    m_ntot += SecVtxVertices->size();
    ...
    if (m_event_parser) {
      ...
      if (!passEvent) {
        ATH_CHECK(m_thinningSvc->filter(*SecVtxVertices, mask, m_opType));
        return StatusCode::SUCCESS;
      }
    }
    ...
    // perform SV1 vertex thinning
    ATH_CHECK(thinSV1Vertices(mask, jetsToCheck));
  ...
  StatusCode TrkJetSecVtxThinning::thinSV1Vertices(
    std::vector<bool>& mask,
    std::vector<const xAOD::Jet*>& jetsToCheck) const {

    using VtxLnk = ElementLink<xAOD::VertexContainer>;
    ...
    for (const xAOD::Jet* jet : jetsToCheck) {
      ...
      // Loop over the SV1 vertices
      std::vector<VtxLnk> SV1Vertices;
      const bool hasSV1Vertices = btag->variable<std::vector<VtxLnk>>(
        "SV1", "vertices", SV1Vertices);
      if (!hasSV1Vertices) {
        continue;
      }
      for (const VtxLnk& vertexLink : SV1Vertices) {
