...
# art-description: DAOD building HION5 mc16
# art-type: grid
...
Reco_tf.py --inputAODFile /cvmfs/atlas-nightlies.cern.ch/repo/data/data-art/DerivationFrameworkART/AOD.18264389._000145.pool.root.1 --outputDAODFile art.pool.root --reductionConf HION5 --maxEvents -1 --preExec 'rec.doApplyAODFix.set_Value_and_Lock(True); rec.doTrigger.set_Value_and_Lock(False); from BTagging.BTaggingFlags import BTaggingFlags; BTaggingFlags.CalibrationTag = "BTagCalibRUN12Onl-08-49"; from HIRecExample.HIRecExampleFlags import jobproperties; jobproperties.HIRecExampleFlags.doHIAODFix.set_Value_and_Lock(True); from HIJetRec.HIJetRecFlags import HIJetFlags; HIJetFlags.SeedSuffix.set_Value_and_Lock("DFSeed"); from AthenaCommon.AlgSequence import AlgSequence; topSequence = AlgSequence(); topSequence += CfgMgr.xAODMaker__DynVarFixerAlg( "BTaggingFixer", Containers=["BTagging_AntiKt4HIAux."] )'

echo "art-result: $? reco"

DAODMerge_tf.py --inputDAOD_HION5File DAOD_HION5.art.pool.root --outputDAOD_HION5_MRGFile art_merged.pool.root --preExec 'rec.doHeavyIon.set_Value_and_Lock(False); rec.doTrigger.set_Value_and_Lock(False)' 
