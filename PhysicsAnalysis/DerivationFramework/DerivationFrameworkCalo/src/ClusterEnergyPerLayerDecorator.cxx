...
StatusCode DerivationFramework::ClusterEnergyPerLayerDecorator::initialize()
{
  ...
  // how to add a statement to theck that the tool indeed has the size matching the neta and nphi properties?
  // ATH_MSG_DEBUG("CaloFillRectangularCluster print size ...
...
StatusCode DerivationFramework::ClusterEnergyPerLayerDecorator::addBranches() const
{
  ...
  if(!m_SGKey_photons.key().empty()){
    ...
    for ( unsigned int i=0; i<m_layers.size(); i++ ) {
      decorations.emplace_back(SG::WriteDecorHandle<xAOD::EgammaContainer, float>(m_SGKey_photons_decorations[i], ctx));
    }
  ...
  if(!m_SGKey_electrons.key().empty()){
    ...
    std::vector< SG::WriteDecorHandle<xAOD::EgammaContainer, float> > decorations;
    for ( unsigned int i=0; i<m_layers.size(); i++ ) {
      decorations.emplace_back(SG::WriteDecorHandle<xAOD::EgammaContainer, float>(m_SGKey_electrons_decorations[i], ctx));
    }
  ...
  const CaloCellContainer* cellCont) const
{
  ...
  result.clear();
  if (not egamma or not egamma->caloCluster()) return result;
  ...
  if (not egamma->author(xAOD::EgammaParameters::AuthorCaloTopo35) )
  {
    ...
    const CaloFillRectangularCluster* tool = 
      dynamic_cast<const CaloFillRectangularCluster* >( &(*m_caloFillRectangularTool ));
