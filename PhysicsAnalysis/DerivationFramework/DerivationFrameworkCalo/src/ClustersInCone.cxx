...
bool
DerivationFramework::ClustersInCone::GetExtrapEtaPhi(const xAOD::Muon* mu,
                                                     float& eta,
                                                     float& phi)
{
  ...
  MsgStream& msg)
{
  ...
  if (!GetExtrapEtaPhi(particle, particleEta, particlePhi)) {
    msg << MSG::WARNING
        << "Failed to get the eta-phi of the muon calo extention " << endmsg;
  }
  unsigned int i(0);
