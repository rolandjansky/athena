...
Cascade3Plus1::Cascade3Plus1(const std::string& t, const std::string& n, const IInterface* p)  : AthAlgTool(t,n,p),
    ...
    m_beamSpotSvc("BeamCondSvc",n), m_cascadeOutputsKeys{ "CascadeVtx1", "CascadeVtx2" }
{
...
StatusCode Cascade3Plus1::addBranches() const
{
    ...
    for(auto &c : Candidates4) {
        ...
        if(c.cascVertex!=nullptr) {
            c.cascVertex->getSVOwnership(true);
        }
    }
...
//           TotalProb_decor(*mainVertex) = TMath::Prob(x->fitChi2() , x->nDoF());
...
//  for(int i =0;i < tracksDs.size();i++) ATH_MSG_DEBUG("Num " << i << " track " << tracksDs[i] << " mass " << massesDs[i]);
    ...
    // Ds vertex
    auto vID = m_iVertexFitter->startVertex(tracksDs, massesDs, m_3TrackMassConstraint ? m_3tracksMass : 0.0);
    std::vector<Trk::VertexID> cnstV;
    if (m_2TrackMassConstraint && !m_iVertexFitter->addMassConstraint(vID, tracksDs, cnstV, m_2tracksMass).isSuccess() ) {
        ATH_MSG_WARNING("addMassConstraint failed");
    }
    vrtList.push_back(vID);
    // Bs vertex
    m_iVertexFitter->nextVertex(tracksBs,massesBs,vrtList);
    // Do the work
    auto x = std::unique_ptr<Trk::VxCascadeInfo> (m_iVertexFitter->fitCascade());
...
std::unique_ptr<xAOD::Vertex> Cascade3Plus1::StandardFit(const std::vector<const xAOD::TrackParticle*> &inputTracks, const xAOD::TrackParticleContainer* importedTrackCollection) const {
    ...
    m_iVertexFitter->setDefault();
    auto theResult = std::unique_ptr<xAOD::Vertex>( m_iVertexFitter->fit(inputTracks, startingPoint));
