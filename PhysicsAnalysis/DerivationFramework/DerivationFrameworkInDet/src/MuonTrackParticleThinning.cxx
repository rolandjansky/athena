...
namespace DerivationFramework {
   ...
   MuonTrackParticleThinning( const std::string& type,
      ...
      declareProperty( "MuonKey", m_muonSGKey,
                       "StoreGate key of the muon container to use" );
   ...
   StatusCode MuonTrackParticleThinning::initialize() {
      ...
      ATH_MSG_INFO( "Inner detector track particles associated with objects in "
                    << m_muonSGKey << " will be retained in this format with "
                                      "the rest being thinned away" );

      // Set up the text-parsing machinery for selectiong the muons directly
   ...
   StatusCode MuonTrackParticleThinning::finalize() {
   ...
   StatusCode MuonTrackParticleThinning::doThinning() const {
      ...
      const xAOD::MuonContainer* importedMuons = nullptr;
      ATH_CHECK( evtStore()->retrieve( importedMuons, m_muonSGKey ) );
      const size_t nMuons = importedMuons->size();

      // Select which muons to use:
      std::vector< const xAOD::Muon* > muToCheck;
      if( m_parser ) {
         ...
         for( size_t i = 0; i < nMuons; ++i ) {
            if( entries.at( i ) == 1 ) {
               muToCheck.push_back( importedMuons->at( i ) );
            }
         }
      } else {
         // If no selection was specified, use all the muon objects:
         for( const xAOD::Muon* mu : *importedMuons ) {
            muToCheck.push_back( mu );
         }
      }

      // Helper object for selecting track particles in a cone around the muons
      static DerivationFramework::TracksInCone tInC;

      // Set elements in the mask to true if there is a corresponding
      // ElementLink from a reconstructed muon object
      for( const xAOD::Muon* mu : muToCheck ) {
         // Check that the link is what we expect:
         const auto& tpLink = mu->inDetTrackParticleLink();
         if( ! tpLink.isValid() ) {
            continue;
         }
         if( tpLink.dataID() != m_inDetSGKey ) {
            ATH_MSG_DEBUG( "Track particle associated with muon does not come "
         ...
         // Select track particles around the muon if it was requested:
         if( m_coneSize > 0.0 ) {
            tInC.select( mu, m_coneSize, importedTrackParticles, mask );
         }
      }
