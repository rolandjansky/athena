...
//std::cout<<"Tbar:"<<std::endl;
	...
	for(const xAOD::TruthParticle* particle : *truthParticles){
		...
		for(size_t q=0; q<particle->nChildren(); q++){
		 if(particle->child(q) && particle->child(q)->pdgId()==-5) bbar = CalcTopPartonHistory::findAfterFSR(particle->child(q))->p4();
		 if(particle->child(q) && particle->child(q)->pdgId()==-24){
			Wm = particle->child(q)->p4(); Wonshell = true;
		 ...
		 if(!Wonshell){
		 	if(particle->child(q) && particle->child(q)->pdgId()!=6 && particle->child(q)->pdgId()!=22 && particle->child(q)->pdgId()>0){
	...
	for(const xAOD::TruthParticle* particle : *truthParticles){
		if(particle->barcode() != 3) continue;
		for(size_t q=0; q<particle->nChildren(); q++){
		 if(particle->child(q) && particle->child(q)->pdgId()==5) b = CalcTopPartonHistory::findAfterFSR(particle->child(q))->p4();	
		 if(particle->child(q) && particle->child(q)->pdgId()==24){
			Wp = particle->child(q)->p4(); Wonshell = true;
			for(size_t Wc=0;Wc<particle->child(q)->nChildren();Wc++){
				if(particle->child(q)->child(Wc)->pdgId()>0){
				 WmDecay1 = CalcTopPartonHistory::findAfterFSR(particle->child(q)->child(Wc))->p4();
				 WmDecay1_pdgId = particle->child(q)->child(Wc)->pdgId();
	...
	if(ph_source==0){
		if(init_type==2) {
			category = 1;
			if(branchtype_t==15 && branchtype_tbar==15){category = 9;} // ISR_qq_DL
		}	//ISR_qq
		else if(init_type==1) {
			category = 2;
			if(branchtype_t==15 && branchtype_tbar==15){category = 10;} // ISR_gg_DL
		}	//ISR_gg/top
	}
		...
		else if((branchtype_t==14 && branchtype_tbar==50) || (branchtype_t==50 && branchtype_tbar==14)){category = 5;}//	W Radiation leptonic
		else if((branchtype_t==54 && branchtype_tbar==10) || (branchtype_t==10 && branchtype_tbar==54)){category = 6;}//	W Radiation hadronic
		else if((branchtype_t==18 && branchtype_tbar==50) || (branchtype_t==50 && branchtype_tbar==18)){category = 7;}//	b Radiation leptonic
		else if((branchtype_t==58 && branchtype_tbar==10) || (branchtype_t==10 && branchtype_tbar==58)){category = 8;}//	b Radiation hadronic
		else if((branchtype_t==12 && branchtype_tbar==10) || (branchtype_t==10 && branchtype_tbar==12)){category = 11;}//	Top Radiation dileptonic
