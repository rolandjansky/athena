...
namespace top {
  FwdElectronMC15::FwdElectronMC15(double ptcut, double minEtacut, double maxEtacut,
    ...
    if (!m_isMC) top::check(m_webBunchCrossingTool.retrieve(), "Failed to retrieve CP::WebBunchCrossingTool");
  ...
  bool FwdElectronMC15::passSelection(const xAOD::Electron& el, int bcid, int runNumber) const {
    if (!passSelectionCommon(el, bcid, runNumber)) return false;

    if (!m_fwdElectronSelectionTool->accept(&el)) return false;
  ...
  bool FwdElectronMC15::passSelectionLoose(const xAOD::Electron& el, int bcid, int runNumber) const {
    ...
    if (!m_fwdElectronLooseSelectionTool->accept(&el)) return false;
  ...
  bool FwdElectronMC15::passSelectionCommon(const xAOD::Electron& el, int bcid, int runNumber) const {
    ...
    if (fabs(el.eta()) < m_minEtacut) return false;

    if (fabs(el.eta()) > m_maxEtacut) return false;

    if (!m_isMC) {
      //fwd electrons in data in first 6 bunch crossings shouldn't be used in 2015+16 data, see ANALYSISTO-760

      if (fabs(el.eta()) > 2.5 && fabs(el.eta()) < 3.2 && runNumber >= m_fwdElectronBCIDCleaningMinRun &&
          runNumber <= m_fwdElectronBCIDCleaningMaxRun) {
        double distance = m_webBunchCrossingTool->gapBeforeTrain(bcid) >
                          500 ? m_webBunchCrossingTool->distanceFromFront(bcid) : (m_webBunchCrossingTool->
                                                                                    distanceFromFront(bcid) +
                                                                                   m_webBunchCrossingTool->
                                                                                    gapBeforeTrain(bcid) +
                                                                                   (m_webBunchCrossingTool->
                                                                                     distanceFromFront(bcid) +
                                                                                    m_webBunchCrossingTool->
        distanceFromTail(bcid)));
