...
public:
   ...
   virtual std::string operator()(Property * prop) override {
      if (m_convprop->setFrom(*prop) != 0)
         throw std::logic_error("TPropertyValueJsonDumper called with wrong type of argument");
...
boost::optional<std::string> PropertyValueJsonDumper::operator()(Property * prop) const {
   ...
   for (auto&& kv : m_dispatch) {
      if (kv.first == propType)
         return (*kv.second)(prop);
   }
   return boost::none;
}
