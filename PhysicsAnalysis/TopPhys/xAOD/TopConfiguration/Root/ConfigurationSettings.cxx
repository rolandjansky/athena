...
namespace top {
  ...
  void ConfigurationSettings::loadFromFile(const std::string& filename) {
	...
	while (tmppos <= strsize-1) {
	  ...
	  if (newstring.find_first_of("\"",tmppos) == std::string::npos) {
	    segments.push_back(newstring.substr(tmppos,strsize-tmppos));
	    segments_isquote.push_back(false);
	    break;
	  }
	  
	  // when the quote found has \ ahead of it, continue to next ieration
	  if (newstring.find_first_of("\"",tmppos) == newstring.find_first_of("\\\"",tmppos)+1) {
	    tmppos = newstring.find_first_of("\"",tmppos)+1;
	    continue;
	  }
	...
	//run the original trim_all for each segment that is NOT a quote
	  ...
	  if (seg.find("\"",0) != std::string::npos) continue;
