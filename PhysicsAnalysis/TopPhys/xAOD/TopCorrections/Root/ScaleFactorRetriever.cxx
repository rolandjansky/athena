    ...
    if (!isLoose) {
      if(tauSF_name.find(SFSyst) == tauSF_name.end()) {
        if(acc_tauSFs.find(tauSF_name.at(top::topSFSyst::TAU_SF_NOMINAL)) == acc_tauSFs.end()){
          acc_tauSFs.insert( std::pair<TString, SG::AuxElement::ConstAccessor<float>> (tauSF_name.at(top::topSFSyst::TAU_SF_NOMINAL) , tauSF_name.at(top::topSFSyst::TAU_SF_NOMINAL).Data()) );
        }
        return acc_tauSFs.at(tauSF_name.at(top::topSFSyst::TAU_SF_NOMINAL))(x);
      }
      if(acc_tauSFs.find(tauSF_name.at(SFSyst)) == acc_tauSFs.end()) {
        acc_tauSFs.insert( std::pair<TString, SG::AuxElement::ConstAccessor<float>> (tauSF_name.at(SFSyst) , tauSF_name.at(SFSyst).Data()) );
      }
