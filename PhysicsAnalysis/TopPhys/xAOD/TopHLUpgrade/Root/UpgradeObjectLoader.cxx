...
UpgradeObjectLoader::UpgradeObjectLoader(const std::shared_ptr<top::TopConfig>& cfg) :
  ...
  if (m_active) {
    ...
    top::check(m_upgrade->setProperty("ElectronWorkingPoint",
                                      UpgradePerformanceFunctionsxAOD::mediumElectron), "Failed to setProperty");
    top::check(m_upgrade->setProperty("ElectronRandomSeed", 171), "Failed to setProperty");
    top::check(m_upgrade->setProperty("MuonWorkingPoint",
                                      UpgradePerformanceFunctionsxAOD::tightMuon), "Failed to setProperty");
    top::check(m_upgrade->setProperty("METRandomSeed", 986), "Failed to setProperty");
    top::check(m_upgrade->setProperty("METFile",
                                      "UpgradePerformanceFunctions/CalibArea-00-01/sumetPU_mu200_ttbar_gold.root"),
    ...
    top::check(m_upgrade->setProperty("PileupEfficiency", UpgradePerformanceFunctionsxAOD::PU),
               "Failed to setProperty");
    top::check(m_upgrade->setProperty("JVT_PU_Efficiency", 0.02), "Failed to setProperty");
    top::check(m_upgrade->setProperty("PileupPath",
                                      "/cvmfs/atlas.cern.ch/repo/sw/database/GroupData/UpgradePerformanceFunctions/"),
               "Failed to setProperty");
    top::check(m_upgrade->setProperty("FlavourTaggingCalibrationFile",
                                      "UpgradePerformanceFunctions/CalibArea-00-01/flavor_tags_v2.0.root"),
               "Failed to setProperty");
    top::check(m_upgrade->setProperty("PhotonWorkingPoint",
                                      UpgradePerformanceFunctionsxAOD::tightPhoton), "Failed to setProperty");
    ...
    top::check(m_upgrade->setProperty("PhotonFakeFile",
                                      "UpgradePerformanceFunctions/CalibArea-00-01/PhotonFakes.root"),
               "Failed to setProperty");
...
ParticleLevelEvent UpgradeObjectLoader::load() {
  ...
  //make a list of all candidate particles that could fall inside the cone of the particle of interest from
  // listOfParticlesForIso

  std::vector<int> excludeFromCone = {
    -16, -14, -12, 12, 14, 16
  };
    ...
    if (m_dofakes && m_config->useTruthPhotons()) {
      ...
      if (m_upgrade->getRandom3()->Uniform() < fakeEff) {
        ...
        *fakePhoton = *electron;
        ...
        electron->auxdata<bool>("FakingPhoton") = true; //add auxdata to identify the electron that is used as fake
                                                        // photon
      } else electron->auxdata<bool>("FakingPhoton") = false; //add auxdata to identify the electron that is used as
                                                              // fake photon
    }
  ...
  for (auto jet : *jets) {
    ...
    if (m_upgrade->getRandom3()->Uniform() < trackEff) {
      ...
      const double MV1tagEff = m_upgrade->getFlavourTagEfficiency(jet->pt(),
                                                                  jet->eta(), type, "mv2c10", 70,
                                                                  m_upgrade->getPileupTrackConfSetting());
  ...
  if (m_config->useTruthPhotons()) {
    ...
    // Shallow copy
    std::pair< xAOD::TruthParticleContainer*,
               xAOD::ShallowAuxContainer* > photons_shallowCopy = xAOD::shallowCopyContainer(*origphotons);
    ...
    for (auto photon : *photons) {
      ...
      if (!((fabs(photon->eta()) > 1.37 && fabs(photon->eta()) < 1.52) ||
            fabs(photon->eta()) > 2.37)) m_upgrade->smearPhoton(*photon); // remove "bad eta" photons to avoid large
                                                                          // amount of warnings
  ...
  UpgradePerformanceFunctionsxAOD::MET smearedMET = m_upgrade->getMETSmeared(origmet->sumet(),
                                                                             origmet->mpx(), origmet->mpy());

  // Shallow copy
  std::pair< xAOD::MissingETContainer*, xAOD::ShallowAuxContainer* > metcont_shallowCopy = xAOD::shallowCopyContainer(
    *origmetcont);
