...
namespace top {
  ...
  void EventSaverxAOD::initialize(std::shared_ptr<top::TopConfig> config, TFile* file,
    ...
    if (config->isMC()) eventInfoList += ".mcChannelNumber.mcEventWeights.PileupWeight";
    ...
    //Muons
    evtStore()->event()->setAuxItemList(
      m_prefix + m_config->sgKeyMuons() + "Aux.", "pt.eta.phi.m.charge.muonType.combinedTrackParticleLink");
  ...
  void EventSaverxAOD::saveEvent(const top::Event& event) {
    ...
    if (event.m_hashValue != m_config->nominalHashValue()) return;
    ...
    if (m_config->doTopPartonHistory()) {
      if (evtStore()->contains<xAOD::PartonHistoryContainer>(m_config->sgKeyTopPartonHistory())) {
        ...
        top::check(evtStore()->event()->record(partonHistory_output,
                                               m_config->sgKeyTopPartonHistory()), "Failed to record Parton History");
      }
    }
    ...
    if (m_config->useElectrons()) {
      ...
      top::check(evtStore()->event()->record(electrons_output,
                                             m_prefix + m_config->sgKeyElectrons()),
                 "xAOD::TEvent record " + m_prefix + m_config->sgKeyElectrons() + " failed");
    }
    ...
    if (m_config->useMuons()) {
      ...
      top::check(evtStore()->event()->record(muons_output,
                                             m_prefix + m_config->sgKeyMuons()),
                 "xAOD::TEvent record " + m_prefix + m_config->sgKeyMuons() + " failed");
      ...
      check(evtStore()->event()->copy(
              "ExtrapolatedMuonTrackParticles"),
            "xAOD::TEvent copy InDetTrackParticles ExtrapolatedMuonTrackParticles");
      check(evtStore()->event()->copy(
              "CombinedMuonTrackParticles"), "xAOD::TEvent copy InDetTrackParticles CombinedMuonTrackParticles");
    }
    ...
    if (m_config->useJets()) {
      ...
      top::check(evtStore()->event()->record(jets_output,
                                             m_prefix + m_config->sgKeyJets()),
                 "xAOD::TEvent record " + m_prefix + m_config->sgKeyJets() + " failed");
    ...
    if (m_config->useLargeRJets()) {
      ...
      check(evtStore()->event()->record(largeJets_output,
                                        m_prefix + m_config->sgKeyLargeRJets()),
            "xAOD::TEvent record " + m_prefix + m_config->sgKeyLargeRJets() + " failed");
    }

    //MET
    top::check(evtStore()->event()->copy(m_config->sgKeyMissingEt()),
               "xAOD::TEvent copy " + m_config->sgKeyMissingEt() + " failed");
  ...
  void EventSaverxAOD::finalize() {
    ...
    top::check(evtStore()->event()->finishWritingTo(
                 m_outputFile), "EventSaverxAOD::finalise trying to call finishWritingTo");
  }
}
