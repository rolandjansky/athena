      ...
      if (objects.muons()){
        for (const auto m : *objects.muons()){
          if ( m->auxdata<char>("ana_select_or") &&
            ...
            double trig_eff = 1., trig_eff_data = 1.;
            QA_CHECK_WEIGHT
              ( double , trig_eff, muonSFToolForThisYear(my_runNumber)->getTriggerEfficiency(*m,trig_eff,m_muon_trig_str,false) );
            m->auxdata<double>(m_groupName + "_Mu_TrigEff_MC") = trig_eff;
            QA_CHECK_WEIGHT
