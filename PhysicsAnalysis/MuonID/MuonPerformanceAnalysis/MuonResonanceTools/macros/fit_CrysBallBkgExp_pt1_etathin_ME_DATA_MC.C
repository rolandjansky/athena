...
void fit_CrysBallBkgExp_pt1_etathin_ME_DATA_MC()
{
...
// --------------------------------- DATA
  ...
  TH1 * h1data;
  MyDirDATA->GetObject("Jpsimm_Mmumu_eta_0_ME_",h1data);
  h1data->Print();
  cMEdata->cd(1);
  h1data->Draw();

  TH2 * h2data;
  MyDirDATA->GetObject("Jpsimm_h_Zm_1stEta01_eta_0_ME_",h2data);
  h2data->Print();
  cMEdata->cd(2);
  h2data->Draw();
...
//    RooDataHist data("data","data",m,Import(*hh)) ;
    ...
    model.fitTo(data, Range(2.3,4.1) );
...
// -------------------------------------- MC CORR
  ...
  cMEmccorr->Divide(10,6);

  TDirectory * MyDirMCCORR = MyFileMCCORR->GetDirectory("ToolSvc.MuonResonanceJpsiTool");

  TH1 * h1mccorr;
  MyDirMCCORR->GetObject("Jpsimm_Mmumu_eta_0_ME_",h1mccorr);
  h1mccorr->Print();
  cMEmccorr->cd(1);
  h1mccorr->Draw();

  TH2 * h2mccorr;
  MyDirMCCORR->GetObject("Jpsimm_h_Zm_1stEta01_eta_0_ME_",h2mccorr);
  h2mccorr->Print();
  cMEmccorr->cd(2);
  h2mccorr->Draw();
...
// model(x) = fsig*sig(x) + (1-fsig)*bkg(x)
    ...
    model.fitTo(data, Range(2.3,3.9), SumW2Error(kFALSE) );
    model.plotOn(myframe);
    model.plotOn(myframe,Components("bkg"),LineStyle(kDashed));
...
// -------------------------------------- MC NO CORR
  ...
  cMEmcnocorr->Divide(10,6);

  TDirectory * MyDirMCNOCORR = MyFileMCNOCORR->GetDirectory("ToolSvc.MuonResonanceJpsiTool");

  TH1 * h1mcnocorr;
  MyDirMCNOCORR->GetObject("Jpsimm_Mmumu_eta_0_ME_",h1mcnocorr);
  h1mcnocorr->Print();
  cMEmcnocorr->cd(1);
  h1mcnocorr->Draw();

  TH2 * h2mcnocorr;
  MyDirMCNOCORR->GetObject("Jpsimm_h_Zm_1stEta01_eta_0_ME_",h2mcnocorr);
  h2mcnocorr->Print();
  cMEmcnocorr->cd(2);
  h2mcnocorr->Draw();
...
//  axis->SetLimits(-2.5,2.5);               // set axis limits along X
  mg3->SetTitle("");
  mg3->GetHistogram()->SetMaximum(3.2);   // set axis limits along Y
  mg3->GetHistogram()->SetMinimum(3.0);   // set axis limits along Y
...
//  axis->SetLimits(-2.5,2.5);               // set axis limits along X
  mg4->SetTitle("");
  mg4->GetHistogram()->SetMaximum(0.23);   // set axis limits along Y
  ...
  cMEratio->Print("mass_and_sigma_of_mu_pt1_vs_etathin_ME.gif");
