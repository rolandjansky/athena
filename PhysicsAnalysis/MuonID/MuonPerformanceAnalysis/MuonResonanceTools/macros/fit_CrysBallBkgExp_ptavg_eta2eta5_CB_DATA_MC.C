...
void fit_CrysBallBkgExp_ptavg_eta2eta5_CB_DATA_MC()
{
...
// --------------------------------- DATA
  ...
  TH2 * h2data;
  TH2 * h2data_2;
  MyDirDATA->GetObject("Jpsimm_h_Zm_mu_avPt_eta_2_CB_",h2data);
  h2data->Print();
  MyDirDATA->GetObject("Jpsimm_h_Zm_mu_avPt_eta_5_CB_",h2data_2);
  h2data_2->Print();
  h2data->Add(h2data_2); 
  h2data->Print();
  cCBdata->cd(2);
...
// Retrive mass bin limits to use in the REBIN PT AVG

  for (jj=0; jj<nmass; jj++){
    bl = h2data->GetYaxis()->GetBinLowEdge(jj+1);
    massbin[jj] = bl;
    if (jj==(nmass-1)) bl = h2data->GetYaxis()->GetBinUpEdge(jj+1);
    massbin[jj+1] = bl;
  }

// REBIN PT AVG

  TH2D * h2datanew = new TH2D("h2datarebin",h2data->GetTitle(),10,xrebins,150,massbin);
...
// -------------------------------------- MC CORR
  ...
  TH2 * h2mccorr;
  TH2 * h2mccorr_2;
  MyDirMCCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_2_CB_",h2mccorr);
  MyDirMCCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_5_CB_",h2mccorr_2);
...
// -------------------------------------- MC NO CORR
  ...
  TH2 * h2mcnocorr;
  TH2 * h2mcnocorr_2;
  MyDirMCNOCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_2_CB_",h2mcnocorr);
  MyDirMCNOCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_5_CB_",h2mcnocorr_2);
...
//    mass_ratio_error[jj]=sqrt((mass_error_data[jj]*mass_error_data[jj])/(mass_value_mccorr[jj]*mass_value_mccorr[jj]) + (mass_value_data[jj]*mass_value_data[jj]*mass_error_mccorr[jj]*mass_error_mccorr[jj])/(mass_value_mccorr[jj]*mass_value_mccorr[jj]));  // Claudia
  ...
  cCBratio->Print("mass_and_sigma_vs_ptavg_eta2eta5_CB.gif");
...
//  gr7->Draw("AP");
  ...
  for (jj=0; jj<nmass+1; jj++){
  cout << " massbin " << massbin[jj] << endl;
  }
