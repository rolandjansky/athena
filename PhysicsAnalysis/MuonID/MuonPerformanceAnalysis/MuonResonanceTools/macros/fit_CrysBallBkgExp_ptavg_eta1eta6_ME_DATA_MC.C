...
void fit_CrysBallBkgExp_ptavg_eta1eta6_ME_DATA_MC()
{
...
// --------------------------------- DATA
  ...
  TH2 * h2data;
  TH2 * h2data_2;
  MyDirDATA->GetObject("Jpsimm_h_Zm_mu_avPt_eta_1_ME_",h2data);
  h2data->Print();
  MyDirDATA->GetObject("Jpsimm_h_Zm_mu_avPt_eta_6_ME_",h2data_2);
  h2data_2->Print();
  h2data->Add(h2data_2); 
  h2data->Print();
  cMEdata->cd(2);
...
//    RooDataHist data("data","data",m,Import(*hh)) ;
    ...
    model.fitTo(data, Range(2.4,4.0) );
...
// -------------------------------------- MC CORR
  ...
  TH2 * h2mccorr;
  TH2 * h2mccorr_2;
  MyDirMCCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_1_ME_",h2mccorr);
  MyDirMCCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_6_ME_",h2mccorr_2);
...
// model(x) = fsig*sig(x) + (1-fsig)*bkg(x)
    ...
    model.fitTo(data, Range(2.4,4.0), SumW2Error(kFALSE) );
    model.plotOn(myframe);
    model.plotOn(myframe,Components("bkg"),LineStyle(kDashed));
...
// -------------------------------------- MC NO CORR
  ...
  TH2 * h2mcnocorr;
  TH2 * h2mcnocorr_2;
  MyDirMCNOCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_1_ME_",h2mcnocorr);
  MyDirMCNOCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_6_ME_",h2mcnocorr_2);
...
//    mass_ratio_error[jj]=sqrt((mass_error_data[jj]*mass_error_data[jj])/(mass_value_mccorr[jj]*mass_value_mccorr[jj]) + (mass_value_data[jj]*mass_value_data[jj]*mass_error_mccorr[jj]*mass_error_mccorr[jj])/(mass_value_mccorr[jj]*mass_value_mccorr[jj]));  // Claudia
  ...
  mg1->SetTitle("");
  mg1->GetHistogram()->SetMaximum(1.008);   // set axis limits along Y
  mg1->GetHistogram()->SetMinimum(0.992);   // set axis limits along Y
  ...
  axis->SetLimits(6.0,60.0);                // set axis limits along X
  mg2->SetTitle("");
  mg2->GetHistogram()->SetMaximum(1.3);   // set axis limits along Y
  mg2->GetHistogram()->SetMinimum(0.7);   // set axis limits along Y
  ...
  axis->SetLimits(6.0,60.0);               // set axis limits along X
  mg4->SetTitle("");
  mg4->GetHistogram()->SetMaximum(0.26);   // set axis limits along Y
  mg4->GetHistogram()->SetMinimum(0.1);   // set axis limits along Y
  ...
  cMEratio->Print("mass_and_sigma_vs_ptavg_eta1eta6_ME.gif");
