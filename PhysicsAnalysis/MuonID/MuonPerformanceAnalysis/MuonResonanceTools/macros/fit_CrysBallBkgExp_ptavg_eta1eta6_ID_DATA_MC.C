...
void fit_CrysBallBkgExp_ptavg_eta1eta6_ID_DATA_MC()
{
...
// --------------------------------- DATA
  ...
  TH2 * h2data;
  TH2 * h2data_2;
  MyDirDATA->GetObject("Jpsimm_h_Zm_mu_avPt_eta_1_ID_",h2data);
  h2data->Print();
  MyDirDATA->GetObject("Jpsimm_h_Zm_mu_avPt_eta_6_ID_",h2data_2);
  h2data_2->Print();
  h2data->Add(h2data_2); 
  h2data->Print();
  cIDdata->cd(2);
...
// -------------------------------------- MC CORR
  ...
  TH2 * h2mccorr;
  TH2 * h2mccorr_2;
  MyDirMCCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_1_ID_",h2mccorr);
  MyDirMCCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_6_ID_",h2mccorr_2);
...
// model(x) = fsig*sig(x) + (1-fsig)*bkg(x)
    ...
    model.fitTo(data, Range(2.7,3.7), SumW2Error(kFALSE) );
    model.plotOn(myframe);
    model.plotOn(myframe,Components("bkg"),LineStyle(kDashed));
...
// -------------------------------------- MC NO CORR
  ...
  TH2 * h2mcnocorr;
  TH2 * h2mcnocorr_2;
  MyDirMCNOCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_1_ID_",h2mcnocorr);
  MyDirMCNOCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_6_ID_",h2mcnocorr_2);
...
//    mass_ratio_error[jj]=sqrt((mass_error_data[jj]*mass_error_data[jj])/(mass_value_mccorr[jj]*mass_value_mccorr[jj]) + (mass_value_data[jj]*mass_value_data[jj]*mass_error_mccorr[jj]*mass_error_mccorr[jj])/(mass_value_mccorr[jj]*mass_value_mccorr[jj]));  // Claudia
  ...
  axis->SetLimits(6.0,60.0);               // set axis limits along X
  mg4->SetTitle("");
  mg4->GetHistogram()->SetMaximum(0.16);   // set axis limits along Y
  ...
  cIDratio->Print("mass_and_sigma_vs_ptavg_eta1eta6_ID.gif");
