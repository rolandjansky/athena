...
void fit_CrysBallBkgExp_ptavg_eta3eta4_ID_DATA_MC()
{
...
// --------------------------------- DATA
  ...
  TH1 * h1data;
  MyDirDATA->GetObject("Jpsimm_h_Zm_mu_avPt_eta_0_ID_",h1data);
  h1data->Print();
  cIDdata->cd(1);
  h1data->Draw();
  ...
  TH2 * h2data;
  TH2 * h2data_2;
  MyDirDATA->GetObject("Jpsimm_h_Zm_mu_avPt_eta_3_ID_",h2data);
  h2data->Print();
  MyDirDATA->GetObject("Jpsimm_h_Zm_mu_avPt_eta_4_ID_",h2data_2);
  h2data_2->Print();
  h2data->Add(h2data_2); 
  h2data->Print();
  cIDdata->cd(2);
...
// -------------------------------------- MC CORR
  ...
  cIDmccorr->Divide(5,3) ;

  TDirectory * MyDirMCCORR = MyFileMCCORR->GetDirectory("ToolSvc.MuonResonanceJpsiTool");

  TH1 * h1mccorr;
  MyDirMCCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_0_ID_",h1mccorr);
  h1mccorr->Print();
  cIDmccorr->cd(1);
  h1mccorr->Draw();
  ...
  TH2 * h2mccorr;
  TH2 * h2mccorr_2;
  MyDirMCCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_3_ID_",h2mccorr);
  MyDirMCCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_4_ID_",h2mccorr_2);
...
// -------------------------------------- MC NO CORR
  ...
  cIDmcnocorr->Divide(5,3) ;

  TDirectory * MyDirMCNOCORR = MyFileMCNOCORR->GetDirectory("ToolSvc.MuonResonanceJpsiTool");

  TH1 * h1mcnocorr;
  MyDirMCNOCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_0_ID_",h1mcnocorr);
  h1mcnocorr->Print();
  cIDmcnocorr->cd(1);
  h1mcnocorr->Draw();
  ...
  TH2 * h2mcnocorr;
  TH2 * h2mcnocorr_2;
  MyDirMCNOCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_3_ID_",h2mcnocorr);
  MyDirMCNOCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_4_ID_",h2mcnocorr_2);
...
//    mass_ratio_error[jj]=sqrt((mass_error_data[jj]*mass_error_data[jj])/(mass_value_mccorr[jj]*mass_value_mccorr[jj]) + (mass_value_data[jj]*mass_value_data[jj]*mass_error_mccorr[jj]*mass_error_mccorr[jj])/(mass_value_mccorr[jj]*mass_value_mccorr[jj]));  // Claudia
  ...
  mg1->SetTitle("");
  mg1->GetHistogram()->SetMaximum(1.002);   // set axis limits along Y
  mg1->GetHistogram()->SetMinimum(0.998);   // set axis limits along Y
  ...
  axis->SetLimits(6.0,60.0);                // set axis limits along X
  mg3->SetTitle("");
  mg3->GetHistogram()->SetMaximum(3.11);   // set axis limits along Y
  mg3->GetHistogram()->SetMinimum(3.09);   // set axis limits along Y
  ...
  TLegend *legend = new TLegend(0.40,0.27,0.70,0.47);
  ...
  axis->SetLimits(6.0,60.0);               // set axis limits along X
  mg4->SetTitle("");
  mg4->GetHistogram()->SetMaximum(0.07);   // set axis limits along Y
  ...
  cIDratio->Print("mass_and_sigma_vs_ptavg_eta3eta4_ID.gif");
