...
void fit_CrysBallBkgExp_ptavg_eta1eta6_CB_DATA_MC()
{
...
// --------------------------------- DATA
  ...
  TH1 * h1data;
  MyDirDATA->GetObject("Jpsimm_h_Zm_mu_avPt_eta_0_CB_",h1data);
  h1data->Print();
  cCBdata->cd(1);
  h1data->Draw();
  ...
  TH2 * h2data;
  TH2 * h2data_2;
  MyDirDATA->GetObject("Jpsimm_h_Zm_mu_avPt_eta_1_CB_",h2data);
  MyDirDATA->GetObject("Jpsimm_h_Zm_mu_avPt_eta_6_CB_",h2data_2);
  h2data->Add(h2data_2); 
  h2data->Print();
  cCBdata->cd(2);
  h2data->Draw();
...
//    RooDataHist data("data","data",m,Import(*hh)) ;
    ...
    model.fitTo(data, Range(2.7,4.0) );
...
// -------------------------------------- MC CORR
  ...
  cCBmccorr->Divide(5,3) ;

  TDirectory * MyDirMCCORR = MyFileMCCORR->GetDirectory("ToolSvc.MuonResonanceJpsiTool");

  TH1 * h1mccorr;
  MyDirMCCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_0_CB_",h1mccorr);
  h1mccorr->Print();
  cCBmccorr->cd(1);
  h1mccorr->Draw();
  ...
  TH2 * h2mccorr;
  TH2 * h2mccorr_2;
  MyDirMCCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_1_CB_",h2mccorr);
  MyDirMCCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_6_CB_",h2mccorr_2);
...
// model(x) = fsig*sig(x) + (1-fsig)*bkg(x)
    ...
    model.fitTo(data, Range(2.7,3.5), SumW2Error(kFALSE) );
    model.plotOn(myframe);
    model.plotOn(myframe,Components("bkg"),LineStyle(kDashed));
...
// -------------------------------------- MC NO CORR
  ...
  cCBmcnocorr->Divide(5,3) ;

  TDirectory * MyDirMCNOCORR = MyFileMCNOCORR->GetDirectory("ToolSvc.MuonResonanceJpsiTool");

  TH1 * h1mcnocorr;
  MyDirMCNOCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_0_CB_",h1mcnocorr);
  h1mcnocorr->Print();
  cCBmcnocorr->cd(1);
  h1mcnocorr->Draw();
  ...
  TH2 * h2mcnocorr;
  TH2 * h2mcnocorr_2;
  MyDirMCNOCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_1_CB_",h2mcnocorr);
  MyDirMCNOCORR->GetObject("Jpsimm_h_Zm_mu_avPt_eta_6_CB_",h2mcnocorr_2);
...
//    mass_ratio_error[jj]=sqrt((mass_error_data[jj]*mass_error_data[jj])/(mass_value_mccorr[jj]*mass_value_mccorr[jj]) + (mass_value_data[jj]*mass_value_data[jj]*mass_error_mccorr[jj]*mass_error_mccorr[jj])/(mass_value_mccorr[jj]*mass_value_mccorr[jj]));  // Claudia
  ...
  mg1->SetTitle("");
  mg1->GetHistogram()->SetMaximum(1.005);   // set axis limits along Y
  mg1->GetHistogram()->SetMinimum(0.995);   // set axis limits along Y
  ...
  axis->SetLimits(6.0,60.0);               // set axis limits along X
  mg4->SetTitle("");
  mg4->GetHistogram()->SetMaximum(0.09);   // set axis limits along Y
  mg4->GetHistogram()->SetMinimum(0.05);   // set axis limits along Y
  ...
  cCBratio->Print("mass_and_sigma_vs_ptavg_eta1eta6_CB.gif");
