...
void fit_CrysBallBkgExp_pt1_etathin_CB_DATA_MC()
{
...
// --------------------------------- DATA
  ...
  TH2 * h2data;
  MyDirDATA->GetObject("Jpsimm_h_Zm_1stEta01_eta_0_CB_",h2data);
  h2data->Print();
  cCBdata->cd(2);
  h2data->Draw();
...
// -------------------------------------- MC CORR
  ...
  TH2 * h2mccorr;
  MyDirMCCORR->GetObject("Jpsimm_h_Zm_1stEta01_eta_0_CB_",h2mccorr);
  h2mccorr->Print();
  cCBmccorr->cd(2);
  h2mccorr->Draw();
...
// -------------------------------------- MC NO CORR
  ...
  TH2 * h2mcnocorr;
  MyDirMCNOCORR->GetObject("Jpsimm_h_Zm_1stEta01_eta_0_CB_",h2mcnocorr);
  h2mcnocorr->Print();
  cCBmcnocorr->cd(2);
  h2mcnocorr->Draw();
...
//  axis->SetLimits(-2.5,2.5);               // set axis limits along X
  ...
  cCBratio->Print("mass_and_sigma_of_mu_pt1_vs_etathin_CB.gif");
