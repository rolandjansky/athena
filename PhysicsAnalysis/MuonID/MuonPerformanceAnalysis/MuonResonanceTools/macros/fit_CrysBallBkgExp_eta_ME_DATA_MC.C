...
void fit_CrysBallBkgExp_eta_ME_DATA_MC()
{
...
// --------------------------------- DATA
  ...
  TCanvas* cMEfitdata = new TCanvas("fit CrystalBallBkgExp - ME - DATA","fit CrystalBallBkgExp - ME - DATA",660,660) ;
...
// GET 2D HISTO Jpsi mass vs eta muon combined
  ...
  TH2 * h2data;
  MyDirDATA->GetObject("Jpsimm_h_Zm_Eta_eta_0_ME_",h2data);
  h2data->Print();
  cMEdata->cd(2);
  h2data->Draw();
...
// Exponential Bkg
    ...
    RooRealVar e0("e0","e0", -1.65,-100.,100.);
...
// RooDataHist data("data","data",m,Import(*hh)) ;
    ...
    model.fitTo(data, Range(2.3,4.0) );
...
// -------------------------------------- MC CORR
  ...
  TH2 * h2mccorr;
  MyDirMCCORR->GetObject("Jpsimm_h_Zm_Eta_eta_0_ME_",h2mccorr);
  h2mccorr->Print();
  cMEmccorr->cd(2);
  h2mccorr->Draw();
...
// -------------------------------------- MC NO CORR
  ...
  TH2 * h2mcnocorr;
  MyDirMCNOCORR->GetObject("Jpsimm_h_Zm_Eta_eta_0_ME_",h2mcnocorr);
  h2mcnocorr->Print();
  cMEmcnocorr->cd(2);
  h2mcnocorr->Draw();
...
//  axis->SetLimits(-2.5,2.5);                // set axis limits along X
  ...
  line1->Draw("P");
  TLegend *legend = new TLegend(0.50,0.83,0.80,0.97);
...
//  axis->SetLimits(-2.5,2.5);               // set axis limits along X
  ...
  TLegend *legend = new TLegend(0.50,0.77,0.80,0.97);
...
//  axis->SetLimits(-2.5,2.5);               // set axis limits along X
  ...
  mg4->GetHistogram()->SetMinimum(0.10);   // set axis limits along Y
  ...
  TLegend *legend = new TLegend(0.50,0.27,0.80,0.47);
  ...
  cMEratio->Print("mass_and_sigma_vs_eta_ME.gif");
