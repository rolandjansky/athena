...
// $Id: MuonLegacySelectionTool.cxx 677909 2015-06-24 11:38:45Z mbellomo $
...
namespace CP {
  ...
  MuonLegacySelectionTool::MuonLegacySelectionTool( const std::string& name )
    : asg::AsgTool( name ),
      m_name(name),
    ...
    ATH_MSG_DEBUG("Creating MuonLegacySelectionTool named "<<m_name);
  }
  
  
  MuonLegacySelectionTool::MuonLegacySelectionTool( const MuonLegacySelectionTool& toCopy  )
    ...
    ATH_MSG_DEBUG("Creating copy of MuonLegacySelectionTool named "<<m_name);
  }
  
  MuonLegacySelectionTool::~MuonLegacySelectionTool(){
    ATH_MSG_DEBUG(Form("Deleting MuonLegacySelectionTool named %s",m_name.c_str()));
  }
  
  StatusCode MuonLegacySelectionTool::initialize() {
    
    // Greet the user:
    ATH_MSG_INFO( "Initialising LEGACY version..." );
  ...
  const Root::TAccept& MuonLegacySelectionTool::getTAccept() const {
    
    return m_accept;
  }
  
  const Root::TAccept&
  MuonLegacySelectionTool::accept( const xAOD::IParticle* p ) const {
  ...
  const Root::TAccept& MuonLegacySelectionTool::accept( const xAOD::Muon& mu ) const {
  ...
  void MuonLegacySelectionTool::setQuality( xAOD::Muon& mu ) const {
    mu.setQuality(getQuality(mu)); 
    return;
  }
  
  xAOD::Muon::Quality MuonLegacySelectionTool::getQuality(const xAOD::Muon& mu ) const {
  ...
  void MuonLegacySelectionTool::setPassesIDCuts( xAOD::Muon& mu ) const {    
    mu.setPassesIDCuts(passedIDCuts(mu));
  }
  
  void MuonLegacySelectionTool::setPassesHighPtCuts( xAOD::Muon& mu ) const {    
    mu.setPassesHighPtCuts(passedHighPtCuts(mu));
  }
  
  bool MuonLegacySelectionTool::passedIDCuts( const xAOD::Muon& mu ) const {
    return passedIDCuts(*mu.primaryTrackParticle());
  }
  
  
  //Placeholder for now
  bool MuonLegacySelectionTool::passedHighPtCuts( const xAOD::Muon& mu ) const {
    using namespace xAOD;
    uint8_t nprecisionLayers;
    //uint8_t ninnerSmallHits;
    if (!mu.primaryTrackParticle()->summaryValue(nprecisionLayers, numberOfPrecisionLayers)){
  ...
  bool MuonLegacySelectionTool::passedIDCuts( const xAOD::TrackParticle & track ) const {
    using namespace xAOD;
    uint8_t value1=0;
    uint8_t value2=0;
  ...
  bool MuonLegacySelectionTool::passedCaloTagQuality (const xAOD::Muon& mu) const{
