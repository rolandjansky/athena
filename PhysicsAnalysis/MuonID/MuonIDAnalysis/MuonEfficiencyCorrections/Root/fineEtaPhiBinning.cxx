...
TH2Poly* fineEtaPhiBinning::getPolyHist(const std::string& name,
        ...
        for (unsigned int l = 0; l < m_phi_lower_BAR.size();l++){
            ...
            if (phiB > phiBnext){             // the -pi / pi bin!
                ...
                if (eta < -m_barrel_endcap_transition && etanext > -m_barrel_endcap_transition){
                    ...
                    double xe[12] = {eta, etanext, etanext, eta,eta,
                            -m_barrel_endcap_transition,-m_barrel_endcap_transition,
                            etanext,etanext,-m_barrel_endcap_transition,
                            -m_barrel_endcap_transition,eta} ;

                    double ye[12] = {-3.15,-3.15,3.15,3.15,phiedgePlus,phiedgePlus,
                            phiedgerPlus,phiedgerPlus,phiedgerMinus,
                            phiedgerMinus,phiedgeMinus,phiedgeMinus};
                    h->AddBin(12,xe,ye);
                    ...
                    double ye[12] = {-3.15,-3.15,3.15,3.15,
                            phiedgePlus,phiedgePlus,phiedgerPlus,phiedgerPlus,
                            phiedgerMinus,phiedgerMinus,phiedgeMinus,-phiedgeMinus};
                    h->AddBin(12,xe,ye);
                ...
                if (eta < - m_barrel_endcap_transition && etanext > -m_barrel_endcap_transition){//            first threshold passage
                    ...
                    double x[] = {eta,  -m_barrel_endcap_transition, -m_barrel_endcap_transition,
                            etanext,etanext,-m_barrel_endcap_transition,
                            -m_barrel_endcap_transition,eta};
                    double y[] = {phiEnext, phiEnext, phiBnext, phiBnext,
                            phiB,phiB,phiE,phiE};
                    h->AddBin(8,x,y);
                }
                    ...
                    double x[] = {eta,  m_barrel_endcap_transition, m_barrel_endcap_transition,
                            etanext,etanext, m_barrel_endcap_transition,
                            m_barrel_endcap_transition,eta};
                    double y[] = {phiBnext, phiBnext, phiEnext, phiEnext,
                            phiE,phiE, phiB,phiB};
                    h->AddBin(8,x,y);
                }
                else {          // normal case
