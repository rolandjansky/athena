...
namespace CP {
   ...
   StatusCode MuonEfficiencyCorrections_TestAlg::execute() {
      ...
      for( ; mu_itr != mu_end; ++mu_itr ) {
         ...
         if( ! m_sf_Tool->getEfficiencyScaleFactor((**mu_itr),sf) ) {
         ...
         if( ! m_sf_Tool->applyEfficiencyScaleFactor(mu) || !m_sf_Tool->applyRecoEfficiency(mu)) {
            ATH_MSG_WARNING( "Couldn't run efficiencies on muon!" );
            continue;
         }
         // read the efficiency we decorated the muon with
         ATH_MSG_INFO( "       efficiency from decorated Muon = "
                       << (mu.isAvailable<float> ("Efficiency") ? mu.auxdata< float >( "Efficiency" ) : -999. ));
         ...
         if (! m_sf_Tool->applyEfficiencyScaleFactorReplicas(mu,20)){
         ...
         ATH_MSG_INFO( "       Scale Factor Replica #2 = "
                       << mu.auxdata< std::vector<float> >( "EfficiencyScaleFactorReplicas" )[1] );
