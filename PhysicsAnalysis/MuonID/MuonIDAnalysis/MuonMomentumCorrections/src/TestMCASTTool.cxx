...
namespace CP {
    ...
    TestMCASTTool::TestMCASTTool(const std::string& name, ISvcLocator* svcLoc) :
        AthAlgorithm(name, svcLoc), m_MCaSTool("CP::MuonCalibrationAndSmearingTool/MuonCalibrationAndSmearingTool", this) {
    ...
    StatusCode TestMCASTTool::execute() {
        ...
        for (; mu_itr != mu_end; ++mu_itr) {
            ...
            for (std::vector<CP::SystematicSet>::iterator sysItr = m_sysList.begin() + 1; sysItr != m_sysList.end(); ++sysItr) {
                if (m_MCaSTool->applySystematicVariation(*sysItr) != StatusCode::SUCCESS) { continue; }
                xAOD::Muon* mu = 0;
