...
namespace CP {
    ...
    MuonCalibrationAndSmearingTool::~MuonCalibrationAndSmearingTool() {
        if (m_sagittasCB != NULL) {
            for (auto t : *m_sagittasCB)
                delete t;
            delete m_sagittasCB;
        }

        if (m_sagittasID != NULL) {
            for (auto t : *m_sagittasID)
                delete t;
            delete m_sagittasID;
        }

        if (m_sagittasME != NULL) {
            for (auto t : *m_sagittasME)
                delete t;
            delete m_sagittasME;
        }

        if (m_sagittaPhaseSpaceCB != NULL) delete m_sagittaPhaseSpaceCB;

        if (m_sagittaPhaseSpaceID != NULL) delete m_sagittaPhaseSpaceID;

        if (m_sagittaPhaseSpaceME != NULL) delete m_sagittaPhaseSpaceME;
    }
...
// This may cause a leak
    ...
    CorrectionCode MuonCalibrationAndSmearingTool::applySagittaBiasCorrection(const unsigned int SgCorrType, xAOD::Muon& mu, unsigned int iter, bool stop, bool isMC, InfoHelper& muonInfo) const {
        ...
        if (iter == 0) {
            ...
            if (m_useStatComb && muonInfo.ptcb > m_StatCombPtThreshold && isBadMuon(mu, muonInfo)) {
                if (m_doNotUseAMGMATRIXDECOR) {
                    ...
                    AmgVector(5)
                    parsCB = mu.auxdata < AmgVector(5) > ("StatCombCBPars");
        ...
        if (SgCorrType == MCAST::SagittaCorType::CB) {
            ...
            if (ID_track != NULL && ME_track != NULL) CBqOverPE = pow(sqrt(ID_track->definingParametersCovMatrix()(4, 4) + ME_track->definingParametersCovMatrix()(4, 4)) / 1e3, 2);

            double IDqOverPE = 1e10;
            if (ID_track != NULL) IDqOverPE = pow(ID_track->definingParametersCovMatrix()(4, 4) / 1e3, 2);

            double MEqOverPE = 1e10;
            if (ME_track != NULL) MEqOverPE = pow(ME_track->definingParametersCovMatrix()(4, 4) / 1e3, 2);
            ...
            if (iter == m_sagittasCB->size()) {
                if (dump) ATH_MSG_DEBUG("--> " << muonInfo.ptcb);
                return CorrectionCode::Ok;
            }
            ...
            double CBsagitta3 = (1 / (IDqOverPE * deltaID) * sagittaID + 1 / (MEqOverPE * deltaMS) * sagittaME) / (1 / (IDqOverPE * deltaID) + 1 / (MEqOverPE * deltaMS));
...
//isSystematics ==false
        ...
        if (SytCase == MCAST::SagittaSysType::BIAS && m_SagittaIterations.at(0) > 1) itersCB = m_SagittaIterations.at(0) - 1;

        unsigned int itersID = 0;

        if (SytCase == MCAST::SagittaSysType::BIAS && m_SagittaIterations.at(1) > 1) itersID = m_SagittaIterations.at(1) - 1;

        unsigned int itersME = 0;
        if (SytCase == MCAST::SagittaSysType::BIAS && m_SagittaIterations.at(2) > 1) itersME = m_SagittaIterations.at(2) - 1;
        ...
        if (DetType == MCAST::DetectorType::ID) {  // Correct the ID momentum
            ...
            if (muonInfo.ptcb == 0) {
                ...
                if (muonInfo.ptid != 0 && muonInfo.ptms != 0) {
                    if (applySagittaBiasCorrectionAuto(MCAST::DetectorType::ID, mu, isMC, SytCase, muonInfo) != CorrectionCode::Ok && applySagittaBiasCorrectionAuto(MCAST::DetectorType::MS, mu, isMC, SytCase, muonInfo) != CorrectionCode::Ok) return CorrectionCode::Error;
    ...
    CorrectionCode MuonCalibrationAndSmearingTool::applyCorrection(xAOD::Muon& mu) const {
        ...
        if ((m_doSagittaCorrection || m_doSagittaMCDistortion) && (m_currentParameters->SagittaRho != MCAST::SystVariation::Default || m_currentParameters->SagittaBias != MCAST::SystVariation::Default)) {
...
//Collects all the Large and Small sectors if they belong to the same eta bin
    void MuonCalibrationAndSmearingTool::CollectMacroRegionsSL() {
        ...
        for (int k = 0; k < m_nb_regions; ++k) {
            ...
            if (m_names[k].find("EL") != std::string::npos || m_names[k].find("BL") != std::string::npos) m_MacroRegionIdxMap[k] = macroRegionIdx - 2; //Large sectors
            if (m_names[k].find("ES") != std::string::npos || m_names[k].find("BS") != std::string::npos) m_MacroRegionIdxMap[k] = macroRegionIdx - 1; //Small sectors
        }
        return;
    ...
    void MuonCalibrationAndSmearingTool::CollectSectors() {
        ...
        for (int k = 0; k < m_nb_regions; ++k) {
            ...
            if (m_names[k].find("EL") != std::string::npos || m_names[k].find("BL") != std::string::npos) { //Large sectors
                if (m_names[k].find("11") != std::string::npos || m_names[k].find("13") != std::string::npos || m_names[k].find("15") != std::string::npos) {
            ...
            if (m_names[k].find("ES") != std::string::npos || m_names[k].find("BS") != std::string::npos) { //Small sectors
                if (m_names[k].find("10") != std::string::npos || m_names[k].find("12") != std::string::npos || m_names[k].find("14") != std::string::npos || m_names[k].find("16") != std::string::npos) {
...
// the barrel 12,14 smalls and 11+15 larges that may need different calibrations
    ...
    CorrectionCode MuonCalibrationAndSmearingTool::applyStatCombination(xAOD::Muon& mu, InfoHelper& muonInfo) const {
        ...
        if (applyStatCombination(id_track, ms_track, charge, parsCB, covCB, chi2) != CorrectionCode::Ok) return CorrectionCode::Error;
