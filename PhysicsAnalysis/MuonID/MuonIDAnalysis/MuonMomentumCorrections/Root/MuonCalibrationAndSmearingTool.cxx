...
namespace CP {
    ...
    StatusCode MuonCalibrationAndSmearingTool::initialize() {
        ...
        if (m_useStatComb) { ATH_MSG_INFO("Using statistical combination abovbe " << m_StatCombPtThreshold << " GeV"); }
        ...
        if (m_Tdata >= MCAST::DataType::Data15 && (m_doSagittaCorrection || m_doSagittaMCDistortion)) {
            ...
            for (unsigned int i = 0; i < m_SagittaIterations.size(); i++) {
                ...
                for (unsigned int j = 0; j < m_SagittaIterations.at(i); j++) {
                    ...
                    std::unique_ptr<TProfile2D> hist(GetHist(
                        PathResolverFindCalibFile(Form("MuonMomentumCorrections/%s/outqDeltamPlots_iter%d/", m_SagittaRelease.c_str(), j) +
                                                  trackNames.at(i) + "_data.root"),
    ...
    CorrectionCode MuonCalibrationAndSmearingTool::applySagittaBiasCorrection(const unsigned int SgCorrType, xAOD::Muon& mu,
                                                                              unsigned int iter, bool stop, bool isMC,
        ...
        if (SgCorrType == MCAST::SagittaCorType::CB) {
            ...
            if (ID_track != nullptr && ME_track != nullptr)
                CBqOverPE = std::pow(
                    std::sqrt(ID_track->definingParametersCovMatrix()(4, 4) + ME_track->definingParametersCovMatrix()(4, 4)) / 1e3, 2);

            double IDqOverPE = 1e10;
            if (ID_track != nullptr) IDqOverPE = std::pow(ID_track->definingParametersCovMatrix()(4, 4) / 1e3, 2);

            double MEqOverPE = 1e10;
            if (ME_track != nullptr) MEqOverPE = std::pow(ME_track->definingParametersCovMatrix()(4, 4) / 1e3, 2);

            CalcCBWeights(mu, muonInfo);

            double ptTilde = (1 / IDqOverPE * muonInfo.ptid + 1 / MEqOverPE * muonInfo.ptms) / (1 / IDqOverPE + 1 / MEqOverPE);
            double pTilde =
                ((1 / IDqOverPE) * (1 / muonInfo.ptid) + (1 / MEqOverPE) * (1 / muonInfo.ptms)) / (1 / IDqOverPE + 1 / MEqOverPE);
            ...
            if (iter == 0) {
                ...
                ATH_MSG_DEBUG(" ptCB: " << muonInfo.ptcb << " --> " << ptTilde << " diff "
                                        << (muonInfo.ptcb - ptTilde) * 100 / muonInfo.ptcb << " %");
                ATH_MSG_DEBUG(" 1/pT: " << 1 / muonInfo.ptcb << " --> " << pTilde << " diff " << deltaPTilde * 100 << " %"
                                        << "1/pttilde " << 1 / ptTilde);
                ATH_MSG_DEBUG(" deltaID " << (muonInfo.ptid - muonInfo.ptcb) * 100 / muonInfo.ptcb << " % delta MS "
                                          << (muonInfo.ptms - muonInfo.ptcb) * 100 / muonInfo.ptcb << " % ");
            ...
            if (meOK == CorrectionCode::Ok && tmpPtMS != 0)
                tmpDeltaMS = (-tmpPtMS + lvMECorr.Pt() / 1e3) / tmpPtMS;
            else
                tmpDeltaMS = 0;
            ...
            double simpleCombPt = (1 / (IDqOverPE * deltaID) * lvIDCorr.Pt() + 1 / (MEqOverPE * deltaMS) * lvMECorr.Pt()) /
                                  (1 / (IDqOverPE * deltaID) + 1 / (MEqOverPE * deltaMS));
            ...
            const ElementLink<xAOD::TrackParticleContainer>& ms_track = mu.extrapolatedMuonSpectrometerTrackParticleLink();
            const ElementLink<xAOD::TrackParticleContainer>& id_track = mu.inDetTrackParticleLink();
            CorrectionCode NominalCorrCode = applyStatCombination(id_track, ms_track, charge, parsCBNom, covCBNom, chi2Nom);
            ...
            ATH_MSG_VERBOSE(" Poor man's combination " << simpleCombPt << " Stat comb " << statCombPt << " Stat comb nom "
    ...
    CorrectionCode MuonCalibrationAndSmearingTool::applyCorrection(xAOD::Muon& mu) const {
        ...
        ATH_MSG_VERBOSE("mu.isAvailable< ElementLink< xAOD::TrackParticleContainer > >( \"primaryTrackParticleLink\" ) = "
                        << mu.isAvailable<ElementLink<xAOD::TrackParticleContainer> >("primaryTrackParticleLink"));
        ...
        ATH_MSG_VERBOSE("Checking Weights - weightID: " << muonInfo.weightID
                                                        << " - std::abs( weightID - 1 ): " << std::abs(muonInfo.weightID - 1));
        ...
        double res_idPt = GeVtoMeV * CalculatePt(MCAST::DetectorType::ID, muonInfo.smearDeltaID, muonInfo.smearDeltaMS,
                                                 m_currentParameters->Scale, muonInfo);
        double res_msPt = GeVtoMeV * CalculatePt(MCAST::DetectorType::MS, muonInfo.smearDeltaID, muonInfo.smearDeltaMS,
                                                 m_currentParameters->Scale, muonInfo);
        double res_cbPt = GeVtoMeV * CalculatePt(MCAST::DetectorType::CB, muonInfo.smearDeltaID, muonInfo.smearDeltaMS,
                                                 m_currentParameters->Scale, muonInfo);
    ...
    double MuonCalibrationAndSmearingTool::CalculatePt(const int DetType, const double inSmearID, const double inSmearMS,
                                                       const double scaleVar, InfoHelper& muonInfo) const {
    ...
    StatusCode MuonCalibrationAndSmearingTool::FillValues() {
        ...
        if (m_Trel >= MCAST::Release::Recs2020_03_03) {
            ...
            for (auto& kv : files_and_maps) {
                ...
                if (m_FilesPath == "")
                    extra_file = PathResolverFindCalibFile("MuonMomentumCorrections/" + m_release + "/" + kv.first + "_" + m_algo + "_" +
                                                           m_year + "_" + m_release + ".dat");
                else
            ...
            if (m_Trel >= MCAST::Release::Rec_2015_11_15) {
                ...
                if (useTan2 && m_p2_ID_TAN[muonInfo.detRegion] != 0.) {
                    ATH_MSG_VERBOSE("Case 3: Using p1ID = " << m_p1_ID[muonInfo.detRegion]
                                                            << " and p2ID = " << m_p2_ID[muonInfo.detRegion]);
                    smear = m_p1_ID[muonInfo.detRegion] * muonInfo.g3 +
                            m_p2_ID_TAN[muonInfo.detRegion] * muonInfo.g4 * muonInfo.ptid * std::sinh(muonInfo.eta) * sinh(muonInfo.eta);
                    return smear;
                } else {
    ...
    CorrectionCode MuonCalibrationAndSmearingTool::applyStatCombination(const ElementLink<xAOD::TrackParticleContainer>& inDetTrackParticle,
                                                                        ...
                                                                        int charge, AmgVector(5) & parsCB, AmgSymMatrix(5) & covCB,
                                                                        double& chi2) const {
        chi2 = 1e20;
