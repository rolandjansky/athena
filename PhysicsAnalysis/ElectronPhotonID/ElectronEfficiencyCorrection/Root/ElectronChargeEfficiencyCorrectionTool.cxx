...
CP::ElectronChargeEfficiencyCorrectionTool::getEfficiencyScaleFactor(const xAOD::Electron& part, double& sf) const {
  ...
  if ( goodEle == false ) {
    ...
    ATH_MSG_DEBUG("We don't have a truth electron (in the definition of charge flip), returning ...");
  ...
  if ( truth_ele_charge == 0 ) {
    ATH_MSG_DEBUG("Truth electron charge is 0!!");
    return CP::CorrectionCode::Ok;
  }
  ...
  //////////////////////////////////////
  // here determine, WHICH of the [histid] to choose (after cuts on runnumber etc....)
  ...
  if ( m_mySysConf.size()==0 ) {
    ATH_MSG_DEBUG(" ... nominal SF: "   << sf);
  } else if (*(m_mySysConf.begin()) == SystematicVariation ("EL_CHARGEID_STAT",  1)) { 
    sf = (sf+(val_stat));
    ...
    for (unsigned int i=0;i<m_systematics.size();i++){
      if (*(m_mySysConf.begin()) == SystematicVariation (Form("EL_CHARGEID_SYS%s",m_systematics.at(i).c_str()),  1)){
	sf = (sf+(val_sys));
...
/*
  ...
  The commented lines below were implemented to provide charge mis-ID efficiencies for which
  we decided to *not* provide recommendations at this stage
  
  Keep it commented in case we want to provide them at a later point
