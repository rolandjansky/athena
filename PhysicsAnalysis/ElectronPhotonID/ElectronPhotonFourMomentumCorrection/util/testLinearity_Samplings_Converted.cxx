...
int main() {
  ...
  for (Scale::Variation var = Scale::None; var<Scale::LastScaleVariation; var = Scale::Variation(var+1))
	...
	double eVar = ect12c.getCorrectedEnergy(runnumber, PATCore::ParticleDataType::Full, piCnv, 
						var, egEnergyCorr::Resolution::None);
	
	double eNom = ect12c.getCorrectedEnergy(runnumber, PATCore::ParticleDataType::Full, piCnv,
  ...
  for(int i=1; i<=nBins; i++) {
    ...
    for(int ix = 1; ix<=hprojp->GetNbinsX(); ix++) {
      ...
      if( !doTotal )
	hprojp->SetBinContent(ix, 0.);
    }
    hprojp->SetMinimum(-.005);
    hprojp->SetMaximum(.005);
    ...
    if( !doTotal ) {
      ...
      hprojp->SetFillStyle(0);
      hprojm->SetFillStyle(0);
    }

    hprojp->DrawCopy();
    hprojm->DrawCopy("same");    

    TLegend l(.2, .15, .45, .35);
    l.SetTextSize(.035);
    l.SetNColumns(1);
    l.SetBorderSize(0);
    l.SetFillColor(0);  
    ...
    gPad->RedrawAxis();
    char plotname[99];
    sprintf(plotname,"lin_samplings_conv_eta%d.png",i);
    c.SaveAs(plotname);
