...
int main() {
  ...
  for (Scale::Variation var = Scale::None; var<Scale::LastScaleVariation; var = Scale::Variation(var+1))
    hVar[var] = new TH2D(ect11d.variationName(var).c_str(), ect11d.variationName(var).c_str(), 24, -2.4, 2.4, 96, 4.5,  100.5);
  ...
  for(int i=1; i<=nBins; i++) {
	...
	continue;
      ...
      gPad->RedrawAxis();
      char plotname[99];
      sprintf(plotname,"lin_eta%d_%s_Electrons.png",i,varname.c_str());
      c.SaveAs(plotname);
  ...
  for(int i=1; i<=nBins; i++) {
    ...
    char htitle[99];
    sprintf(htitle,"Electrons, ET=%f GeV", ET);    
	...
	hproj->SetBinContent(iii,fabs(hproj->GetBinContent(iii)));
      ...
      gPad->RedrawAxis();
      char plotname[99];
      sprintf(plotname,"lin_ET%d_%s_Electrons.png",i,varname.c_str());
      c.SaveAs(plotname);
