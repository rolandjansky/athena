...
int main() {
  ...
  TH1D* hjpsi;  
  if( model==es2012c ) {
    fjpsi = TFile::Open("$ROOTCOREBIN/data/ElectronPhotonFourMomentumCorrection/scales_delta_Jpsi_Z_syst_4etaBins.root");
    hjpsi = (TH1D*) fjpsi->Get("hJPsiZ");
  }

  TFile* fline = TFile::Open("$ROOTCOREBIN/data/ElectronPhotonFourMomentumCorrection/Combine_MatandSum_AllEtaBins_4etaBins.root");
  ...
  double etaBins[] = {0, 0.6, 1.37, 1.82, 2.47};
