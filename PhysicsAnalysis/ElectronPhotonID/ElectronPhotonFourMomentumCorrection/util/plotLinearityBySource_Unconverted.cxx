...
int main() {
  ...
  for (Scale::Variation var = Scale::None; var<Scale::LastScaleVariation; var = Scale::Variation(var+1))
    hVar[var] = new TH2D(ect11d.variationName(var).c_str(), ect11d.variationName(var).c_str(), 48, 0, 2.4, 96, 4.5,  100.5);
  ...
  for (Scale::Variation var = Scale::None; var<Scale::LastScaleVariation; var = Scale::Variation(var+1))
	...
	if(fabs(eta)>1.37&&fabs(eta)<1.55)
	  continue;
	...
	double eVar = ect11d.getCorrectedEnergy(runnumber, PATCore::ParticleDataType::Full, piUnc, 
						var, egEnergyCorr::Resolution::None);
	
	double eNom = ect11d.getCorrectedEnergy(runnumber, PATCore::ParticleDataType::Full, piUnc, 
  ...
  //   char htitle[99];
  //   sprintf(htitle,"Unconverted photons, %3.2f<|#eta|<%3.2f", etamin, etamax);    
  ...
  //     gPad->RedrawAxis();
  //     char plotname[99];
  //     sprintf(plotname,"lin_eta%d_%s_Unconverted.png",i,varname.c_str());
  //     c.SaveAs(plotname);
  ...
  for(int i=1; i<=nBins; i++) {
    ...
    if( !(ET>10.5 && ET<11.5) && !(ET>29.5 && ET<30.5) && !(ET>59.5 && ET<60.5) )
      continue;

    char htitle[99];
    sprintf(htitle,"Unconverted photons, ET=%2f GeV", ET);    

    // ... envelope 
    
    TH1D* hprojp = hVar[Scale::AllUp]->ProjectionX("_px", i, i);
    hprojp->SetMinimum(0.);
    hprojp->SetMaximum(.02);
    hprojp->SetTitle("");
    hprojp->SetFillColor(kBlue-10);
    hprojp->SetFillStyle(1001);
    hprojp->SetLineColor(kBlue-10);
	...
	hproj->SetBinContent(iii,fabs(hproj->GetBinContent(iii)));
      ...
      gPad->RedrawAxis();
      char plotname[99];
      sprintf(plotname,"lin_ET%d_%s_Unconverted.png",i,varname.c_str());
      c.SaveAs(plotname);
