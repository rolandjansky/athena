...
int main() {
  ...
  for(int i=1; i<=res2012c.GetNbinsX(); i++)
    for(int j=1; j<=res2012c.GetNbinsY(); j++) {
      ...
      res2012c.SetBinContent(i, j, ect12c.resolution(ene, eta, PATCore::ParticleType::Electron, false));
      res2012c.SetBinError(i, j, 0.);

      res2012a.SetBinContent(i, j, ect12a.resolution(ene, eta, PATCore::ParticleType::Electron, false));
