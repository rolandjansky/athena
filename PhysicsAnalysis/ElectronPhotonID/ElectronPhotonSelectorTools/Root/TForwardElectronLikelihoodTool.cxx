...
const std::string Root::TForwardElectronLikelihoodTool::fVariables[s_fnVariables] = {"el_secondLambda", "el_lateral", "el_longitudinal", "el_centerLambda", "el_fracMax", "el_secondR", "el_significance", "el_secondDensity"};

Root::TForwardElectronLikelihoodTool::TForwardElectronLikelihoodTool(const char *name)
    : asg::AsgMessaging(std::string(name)), m_doPileupCorrection(false), m_variableNames(""), m_pdfFileName(""), m_variableBitMask(0x0), m_cutPosition_kinematicEta(-9), m_cutPosition_kinematicEt(-9), m_cutPosition_LH(-9)
{
...
StatusCode Root::TForwardElectronLikelihoodTool::initialize()
{
  ...
  {
    ATH_MSG_ERROR("Configuration issue :  CutLikelihood expected size " << number_of_expected_bin << " input size " << m_cutLikelihood.size());
  ...
  m_cutPosition_kinematicEta = m_acceptInfo.addCut("kinematicEta", "pass kinematic eta");
  if (m_cutPosition_kinematicEta < 0)
    sc = StatusCode::FAILURE;
  m_cutPosition_kinematicEt = m_acceptInfo.addCut("kinematicEt", "pass kinematic Et ");
...
int Root::TForwardElectronLikelihoodTool::loadVarHistograms(const std::string &vstr, unsigned int varIndex)
{
  for (unsigned int s_or_b = 0; s_or_b < 2; s_or_b++)
  {
    for (unsigned int ip = 0; ip < IP_FBINS; ip++)
...
asg::AcceptData Root::TForwardElectronLikelihoodTool::accept(LikeEnumForward::LHAcceptVars_t &vars_struct) const
{
  ...
  if (!m_cutLikelihood.empty())
  {
    ...
    {
      cutDiscriminant += vars_struct.ip * m_cutLikelihoodPileupCorrectionA[ibin_combined] + m_cutLikelihoodPileupCorrectionB[ibin_combined];
    }
  }
...
double Root::TForwardElectronLikelihoodTool::calculate(LikeEnumForward::LHCalcVars_t &vars_struct) const
{
  ...
  double result = -999;
  double arr[] = {vars_struct.secondLambda, vars_struct.lateral, vars_struct.longitudinal, vars_struct.centerLambda, vars_struct.fracMax, vars_struct.secondR, vars_struct.significance, vars_struct.secondDensity};
...
double Root::TForwardElectronLikelihoodTool::evaluateLikelihood(std::vector<double> &varVector, double et, double eta, double ip) const
{
  ...
  for (unsigned int var = 0; var < s_fnVariables; var++)
  {
    ...
    for (unsigned int s_or_b = 0; s_or_b < 2; s_or_b++)
    {
      ...
      if (integral == 0)
      {                                             // currently, the crack always has integral == 0
...
void Root::TForwardElectronLikelihoodTool::getBinName(char *buffer, int etbin, int etabin, int ipbin, const std::string &iptype) const
{
  double eta_bounds[s_fnEtaBins] = {2.5, 2.6, 2.7, 2.8, 2.9, 3.0, 3.1, 3.16, 3.35, 3.6};
