	...
	m_rootForwardTool(0)
{
  ...
  declareProperty("primaryVertexContainer", m_primVtxContName="PrimaryVertices", "The primary vertex container name" );
	...
	declareProperty("ConfigFile",m_configForwardFile="",
...
StatusCode AsgForwardElectronIsEMSelector::initialize()
{
	...
	if(!m_configForwardFile.empty()) {

		//find the file and read it in
		std::string filename = PathResolverFindCalibFile( m_configForwardFile);
		if(filename=="")
		{ 
		  ATH_MSG_ERROR("Could not locate " << m_configForwardFile );
