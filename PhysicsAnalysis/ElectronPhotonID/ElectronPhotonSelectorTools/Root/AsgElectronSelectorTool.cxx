...
//=============================================================================
const Root::TAccept& AsgElectronSelectorTool::accept( const xAOD::Electron* eg, double mu ) const
{

  // Clear accept so all cuts are not passed
  m_acceptMVA.clear();
  ...
  double mvaScore = calculate(eg, mu);
  ...
  m_acceptMVA.setCutResult(m_cutPosition_kinematic, passKine);
  if (!passKine){return m_acceptMVA;}
  ...
  m_acceptMVA.setCutResult(m_cutPosition_NSilicon, passNSilicon);
  m_acceptMVA.setCutResult(m_cutPosition_NPixel, passNPixel);
  m_acceptMVA.setCutResult(m_cutPosition_NBlayer, passNBlayer);
  m_acceptMVA.setCutResult(m_cutPosition_ambiguity, passAmbiguity);
  m_acceptMVA.setCutResult(m_cutPosition_MVA, passMVA);
...
//=============================================================================
const Root::TAccept& AsgElectronSelectorTool::accept( const xAOD::IParticle* part ) const
{
...
const Root::TResult& AsgElectronSelectorTool::calculate( const xAOD::IParticle* part ) const
{
  const xAOD::Electron* eg = dynamic_cast<const xAOD::Electron*>(part);
  if (eg){
    return calculate(eg);
...
const Root::TAccept& AsgElectronSelectorTool::accept( const xAOD::Egamma* eg, double mu ) const
{
    const xAOD::Electron* ele = dynamic_cast<const xAOD::Electron*>(eg);
    if (ele){
      return accept(ele, mu);
    }
    else {
      ATH_MSG_DEBUG("AsgElectronSelectorTool::could not cast to const Electron");
      m_acceptMVA.clear();
...
const Root::TResult& AsgElectronSelectorTool::calculate( const xAOD::Egamma* eg, double mu ) const
{
    const xAOD::Electron* ele = dynamic_cast<const xAOD::Electron*>(eg);
    if (ele){
      return calculate(ele, mu);
    }
    else {
      ATH_MSG_DEBUG("AsgElectronSelectorTool::could not cast to const Electron");
      // Set all entries of the TResult to a default of -9999.
...
double AsgElectronSelectorTool::combineOutputs( const std::vector<double>& mvaScores, double eta ) const{
  unsigned int etaBin = getDiscEtaBin(eta);
  double disc = 0;
