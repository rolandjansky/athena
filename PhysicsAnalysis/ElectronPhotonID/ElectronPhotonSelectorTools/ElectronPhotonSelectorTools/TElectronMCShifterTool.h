...
/*! @namespace LikeEnum
 ...
 This namespace is defined if the Likelihood tool is not
 defined. This is Only called when TElectronLikelihoodTool.h
 is not included in some other file.
*/
namespace LikeEnum {
  /*! @struct LHCalcVars_t
      
      A struct used to hold variable values used in egamma reconstruction.
      Specifically used in the Likelihood tool.
  */
  struct LHCalcVars_t{
    double eta;
    double eT;
...
/*! @namespace Root
    ...
    The namespce used for all Root tools.
*/
namespace Root {
  ...
  */
  class TElectronMCShifterTool{
     ...
     */
     ...
     //! shift all veriables in a struct.
     /*!
        This function takes as input the struct used to hold all of the variables used in the Likelihood tool.
        Use only in the likelihood tool itself. This does not return a new struct. Instead it replaces the values
        in the input struct.

        @param LHVars The structure containing the variables. This struct is used in the Likelihood tool.
        @return The same struture is returned with the values shifted.
     */
     void shiftStruct(LikeEnum::LHCalcVars_t& LHVars);
