 ...
 * Description: Extrapolator tools.
 *
...
**/
class Extrapolator: public AthAlgTool, virtual public IExtrapolator, public TICTListener
{
  public:
    Extrapolator (const std::string& type,const std::string& name,const IInterface* parent);
    ~Extrapolator();
    ...
    std::vector< std::vector<double> > getTrackPositions(const Trk::TrackParameters* track);
    ...
    std::vector< std::vector<double> > getTrackPositions(const Trk::Track* track)
    { return getTrackPositions(getTrackParameters(track)); }
    ...
    { return getMidPointForExtrapolations(getTrackParameters(track)); };
    std::vector< std::vector<double> > getTrackPositions(const INavigable4Momentum* track)
    { return getTrackPositions(getTrackParameters(track)); }
    ...
    const Trk::TrackParameters* getTrackParameters(const Trk::ParametersBase<5, Trk::Charged>* perigee);
