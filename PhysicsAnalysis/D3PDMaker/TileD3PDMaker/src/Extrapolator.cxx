...
#include "ITrackTools.h" // for CaloLayer
...
/////////////////////////////////////////////////////////
Extrapolator::Extrapolator (const std::string&type, const std::string&name, const IInterface*parent)
...
// ------------------------------------------------------------------
StatusCode Extrapolator::initialize()
{
...
// ------------------------------------------------------------------
StatusCode Extrapolator::finalize()
{
    return StatusCode::SUCCESS;
}
//--------------------------------------------------------------------
...
void Extrapolator::getCaloSamples()
{
    ...
    m_caloSamples[tile2].push_back(CaloCell_ID::TileGap3);
...
//--------------------------------------------------------------------
double Extrapolator::etamax(const CaloCell_ID::CaloSample sample, const bool isTile)
{
    double  etamax = 0;
...
// ------------------------------------------------------------------
double Extrapolator::etamin(const CaloCell_ID::CaloSample sample, const bool isTile) {
...
bool Extrapolator::newTrack(const Trk::TrackParameters* track)
{
    if(!m_listener)
    {
...
const Trk::TrackParameters* Extrapolator::extrapolateToSample(CaloCell_ID::CaloSample sample,const Trk::TrackParameters* param0, double offset, bool back_of_sample){
    ...
    if(sample!=CaloCell_ID::Unknown)
    {
        ...
        {
            ATH_MSG_DEBUG( "Cannot create surface on sample "<< sample << ".Returning null pointer TrackParameters" );
        }
    }
...
std::vector< const Trk::TrackParameters* > Extrapolator::getExtrapolationsToLayerEntrance(const Trk::TrackParameters* track)
{
    ATH_MSG_DEBUG( "In Extrapolator::getExtrapolationsToLayerEntrance" );
    if(!track)
    {
        ATH_MSG_WARNING( "Null pointer to TrackParameters!"
...
std::vector< const Trk::TrackParameters* > Extrapolator::getExtrapolationsToLayerExit(const Trk::TrackParameters* track)
{
    ATH_MSG_DEBUG( "In Extrapolator::getExtrapolationsToLayerExit" );
    ...
    for(int layer = ps; layer <= hec3; layer++)
    {
        ...
        if(!m_extrapolationsToExit[layer])
           ...
           Amg::Vector3D diff = m_extrapolationsToExit[layer]->position() - par->position();
           m_pathLength[layer] = diff.norm();
//          m_pathLength[layer] = m_extrapolationsToExit[layer]->position().distance( par->position() );
        }
    }
...
std::vector<double> Extrapolator::getPathLengths(const Trk::TrackParameters* track)
{
...
//=====================================================================================================================
std::vector< std::pair<double, double> > Extrapolator::getMidPointForExtrapolations(const Trk::TrackParameters* track){
//=====================================================================================================================
    ...
    return positions;
} // Extrapolator::getMidPointForExtrapolations

//====================================================================================================
std::vector< std::vector<double> > Extrapolator::getTrackPositions(const Trk::TrackParameters* track){
//====================================================================================================
    std::vector< std::vector<double> > positions;
    if(!track)
        return positions;
    std::vector<double> positionDummy;
    std::vector<double> positionUpToEM;
    positionUpToEM.push_back(track->parameters()[Trk::x]);
    positionUpToEM.push_back(track->parameters()[Trk::y]);
    positionUpToEM.push_back(track->parameters()[Trk::z]);
    positionUpToEM.push_back(track->eta());
    positionUpToEM.push_back(track->parameters()[Trk::phi]);
    if( newTrack(track) || m_extrapolationsToExit.size() == 0)
        if(getExtrapolationsToLayerExit(track).size() == 0){
            ATH_MSG_DEBUG( "Could not get extrapolations to layer exit" );
            return positions;
    ...
    for(int layer = ps; layer <= hec3; layer++){
        positions.push_back(positionDummy);
        
        //         if(layer == ps || layer == tile1 || layer == hec0)
        //             positions.push_back(positionUpToEM);
        //         else
        //             positions.push_back(positionDummy);
        const Trk::TrackParameters *entrance = m_extrapolationsToEntrance[layer];
        const Trk::TrackParameters *exit = m_extrapolationsToExit[layer];
        ...
        if(layer <= em3){
            positions[layer].push_back( positionUpToEM[0] );
            positions[layer].push_back( positionUpToEM[1] );
            positions[layer].push_back( positionUpToEM[2] );
            positions[layer].push_back( positionUpToEM[3] );
            positions[layer].push_back( positionUpToEM[4] );
        } // IF
        else if(entrance && exit){
            positions[layer].push_back( (entrance->position().x()   + exit->position().x())/2. );
            positions[layer].push_back( (entrance->position().y()   + exit->position().y())/2. );
            positions[layer].push_back( (entrance->position().z()   + exit->position().z())/2. );
            positions[layer].push_back( (entrance->position().eta() + exit->position().eta())/2. );
            positions[layer].push_back( phiMean(entrance->position().phi(), exit->position().phi()) );
        } // ELSE IF
        else if(entrance){
            positions[layer].push_back( entrance->position().x() );
            positions[layer].push_back( entrance->position().y() );
            positions[layer].push_back( entrance->position().z() );
            positions[layer].push_back( entrance->position().eta() );
            positions[layer].push_back( entrance->position().phi() );
        } // ELSE IF
        else{
            positions[layer].push_back( positionUpToEM[0] );
            positions[layer].push_back( positionUpToEM[1] );
...
std::vector<double> Extrapolator::getParametrizedEloss(const Trk::TrackParameters* track)
{
...
double Extrapolator::deltaR(const Trk::TrackParameters *a, const Trk::TrackParameters *b)
{
...
//-----------------------------------------------------------
void Extrapolator::setPropagationDirection(Trk::PropDirection direction)
{
...
bool Extrapolator::setPropagationDirection(const Trk::Track* track)
{
...
bool Extrapolator::setPropagationDirection(const INavigable4Momentum* track)
{
    Trk::PropDirection direction;
    if(!m_trackDirectionTool->getPropagationDirection(track, direction) )
        return false;
...
//-----------------------------------------------------------
const Trk::TrackParameters* Extrapolator::getTrackParameters(const Trk::Track *track)
{
    ...
    return getTrackParameters(track->perigeeParameters());
}
//------------------------------------------------------------------
const Trk::TrackParameters* Extrapolator::getTrackParameters(const INavigable4Momentum *track)
{
    ...
    return getTrackParameters(&trackParticle->definingParameters());
}
//------------------------------------------------------------------
const Trk::TrackParameters* Extrapolator::getTrackParameters(const Trk::ParametersBase<5, Trk::Charged>* perigee)
{
    ATH_MSG_DEBUG( "In getTrackParameters(const Trk::ParametersBase)" );
    const Trk::TrackParameters* trackParameters = 0;
    if(!perigee)
        ATH_MSG_WARNING( "Null pointer to perigee" );
    else
    {
        trackParameters = dynamic_cast<const Trk::TrackParameters*>(perigee);
        ATH_MSG_VERBOSE( "Still getTrackParameters(const Trk::ParametersBase), could convert: " << (m_currentTrackParameters != 0)  );
    }
    return trackParameters;
}
//-----------------------------------------------------------
/** Define the particle hypothesis for the extrapolation **/
void Extrapolator::setParticleHypothesis(const std::string pHypothesis)
{
...
double Extrapolator::phidiff(double phi1, double phi2){
     // FEW CONSTANTS
