/*
  Copyright (C) 2002-2017 CERN for the benefit of the ATLAS collaboration
*/

// System include(s):
#include <memory>
#include <cstdlib>

// ROOT include(s):
#include <TFile.h>
#include <TError.h>
#include <TString.h>

// Infrastructure include(s):
#ifdef ROOTCORE
#   include "xAODRootAccess/Init.h"
#   include "xAODRootAccess/TEvent.h"
#endif // ROOTCORE

// EDM include(s):
#include "xAODEventInfo/EventInfo.h"
#include "xAODTau/TauJetContainer.h"

// Local include(s):
#include "TauAnalysisTools/TauEfficiencyCorrectionsTool.h"
#include "TauAnalysisTools/TauSelectionTool.h"
#include "TauAnalysisTools/TauSmearingTool.h"

using namespace TauAnalysisTools;

#define CHECK( ARG )					\
  do {                                                  \
    const bool result = ARG;				\
    if( ! result ) {					\
      ::Error( APP_NAME, "Failed to execute: \"%s\"",	\
	       #ARG );					\
      return 1;						\
    }							\
  } while( false )

int main( int argc, char* argv[] ) {

  // The application's name:
  const char* APP_NAME = argv[ 0 ];

  // Check if we received a file name:
  if( argc < 2 ) {
    Error( APP_NAME, "No file name received!" );
    Error( APP_NAME, "  Usage: %s [xAOD file name]", APP_NAME );
    return 1;
  }

  // Initialise the application:
  CHECK( xAOD::Init( APP_NAME ) );

  // Open the input file:
  const TString fileName = argv[ 1 ];
  Info( APP_NAME, "Opening file: %s", fileName.Data() );
  std::auto_ptr< TFile > ifile( TFile::Open( fileName, "READ" ) );
  CHECK( ifile.get() );
   
  // Create a TEvent object:
  xAOD::TEvent event( xAOD::TEvent::kClassAccess );
  CHECK( event.readFrom( ifile.get() ) );

  Info( APP_NAME, "Number of events in the file: %i",
	static_cast< int >( event.getEntries() ) );

  // Decide how many events to run over:
  Long64_t entries = event.getEntries();
  if( argc > 2 ) {
    const Long64_t e = atoll( argv[ 2 ] );
    if( e < entries ) {
      entries = e;
    }
  }

  TauAnalysisTools::TauSelectionTool TauSelTool( "TauSelectorTool" );
  TauSelTool.msg().setLevel( MSG::INFO );
  TauSelTool.setRecommendedProperties();
  TauSelTool.setProperty("SelectionCuts", SelectionCuts(CutPt | CutAbsEta | CutAbsCharge | CutNTrack));
  TauSelTool.initialize();

  TauAnalysisTools::TauSmearingTool TauSmeTool( "TauSmaringTool" );
  TauSmeTool.msg().setLevel( MSG::INFO );
  TauSmeTool.setProperty("Direction", 1); 
  TauSmeTool.initialize();

  TauAnalysisTools::TauEfficiencyCorrectionsTool TauEffTool( "TauEfficiencyCorrectionsTool" );
  TauEffTool.msg().setLevel( MSG::INFO );
  TauEffTool.initialize();

  // Loop over the events:
  for( Long64_t entry = 0; entry < entries; ++entry ) {

    // Tell the object which entry to look at:
    event.getEntry( entry );
    // Print some event information for fun:
    const xAOD::EventInfo* ei = 0;
    CHECK( event.retrieve( ei, "EventInfo" ) );
    Info( APP_NAME,
	  "===>>>  start processing event #%i, "
	  "run #%i %i events processed so far  <<<===",
	  static_cast< int >( ei->eventNumber() ),
	  static_cast< int >( ei->runNumber() ),
	  static_cast< int >( entry ) );

    // Get the Taus from the event:
    const xAOD::TauJetContainer* taus = 0;
            
    CHECK( event.retrieve( taus, "TauRecContainer" ) ); 

    Info( APP_NAME, "Number of taus: %i",
	  static_cast< int >( taus->size() ) );
      
    // Print their properties, using the tools:
    xAOD::TauJetContainer::const_iterator tau_itr = taus->begin();
    xAOD::TauJetContainer::const_iterator tau_end = taus->end();
    for( ; tau_itr != tau_end; ++tau_itr ) {

      // Select "good" taus:
      if( ! TauSelTool.accept( **tau_itr ) ) continue;

      // copy constant objects to non-constant
      xAOD::TauJet* tau = 0; 
      tau = new xAOD::TauJet();
      tau->makePrivateStore( **tau_itr ); 

      TauEffTool.applyEfficiencyScaleFactor(*tau);
      TauEffTool.applyEfficiencyScaleFactorStatUnc(*tau);
      TauEffTool.applyEfficiencyScaleFactorSysUnc(*tau);

      // print some info about the selected tau:
      Info( "CPToolTester", "  Selected tau: eta = %g, phi = %g, pt = %g, prong = %g",
      	    ( *tau_itr )->eta(), ( *tau_itr )->phi(), ( *tau_itr )->pt(), 1.*( *tau_itr )->nTracks());
      Info( "CPToolTester", "  JetIDEff = %g, StatUnc = %g, SysUnc = %g", 
	    tau->auxdata< double >( "TauScaleFactorJetID" ),
	    tau->auxdata< double >( "TauScaleFactorJetIDStatUnc_Up" ),
	    tau->auxdata< double >( "TauScaleFactorJetIDSysUnc_Up" ));
      TauSmeTool.applyCorrection(*tau);
      Info( "CPToolTester", "Smeared tau pt: %g", tau->p4().Pt());
    }
  }
   
  return 0;
}
