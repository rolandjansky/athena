...
atlas_env_usage() {
   echo "Script setting up the build environment for AthDataQuality."
   echo ""
...
env_setup() {
    ...
    # This trick should do the right thing under ZSH:
        AthDataQualitySrcDir=$(dirname `print -P %x`)
        if [ $? != 0 ]; then
    ...
    AthDataQualitySrcDir=$(cd ${AthDataQualitySrcDir};pwd)
    ...
    if [ "${ATLAS_BUILD_DIR}" = "" ]; then
        ATLAS_BUILD_DIR=${AthDataQualitySrcDir}/../../../build
    fi
