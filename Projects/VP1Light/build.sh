...
VP1LightSrcDir=$(dirname ${BASH_SOURCE[0]})
if [ -z "$BUILDDIR" ]; then
    BUILDDIR=${VP1LightSrcDir}/../../../build
fi
mkdir -p ${BUILDDIR}
BUILDDIR=$(cd ${BUILDDIR} && pwd)
source $VP1LightSrcDir/build_env.sh -b $BUILDDIR

# create the actual build directory
mkdir -p ${BUILDDIR}/build/VP1Light
cd ${BUILDDIR}/build/VP1Light
...
if [ -n "$EXE_CPACK" ]; then
   ...
   FILES=$(ls VP1Light*.rpm VP1Light*.dmg VP1Light*.tar.gz)
   cp ${FILES} ${BUILDDIR}/
fi
