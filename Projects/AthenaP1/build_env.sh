# This script sets up the build enironment for an AthenaP1
...
env_setup() {
    ...
    AthenaP1SrcDir=$(cd ${thisdir};pwd)

    # The directory holding the helper scripts:
    scriptsdir=${AthenaP1SrcDir}/../../Build/AtlasBuildScripts
    ...
    export NICOS_PROJECT_VERSION=`cat ${AthenaP1SrcDir}/version.txt`
    export NICOS_ATLAS_RELEASE=${NICOS_PROJECT_VERSION}
    export NICOS_PROJECT_RELNAME=${NICOS_PROJECT_VERSION}
    export NICOS_PROJECT_HOME=$(cd ${BUILDDIR}/install;pwd)/AthenaP1
