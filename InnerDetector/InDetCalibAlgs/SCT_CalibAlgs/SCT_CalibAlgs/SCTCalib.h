...
//reinstated 21 May, sroe
  ...
  UnsignedIntegerProperty m_noisyStripDiff;
  ...
  bool retrievedService(S & service){
    //msg(MSG::INFO)<<"Retrieving: "<<service.name()<<endreq;
    if (service.retrieve().isFailure() ) return msg(MSG::ERROR)<<"Unable to retrieve "<<service.name()<<endreq, false;
    return true;
  }
  std::string 
