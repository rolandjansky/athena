...
StatusCode SCTCalib::initialize(){
  ...
  //
  if ( detStore()->retrieve( m_pManager, "SCT").isFailure() ) return msg( MSG::ERROR) << "Unable to retrieve SCTManager" << endreq,StatusCode::FAILURE;
  if ( not retrievedService(m_pCalibWriteSvc)) return StatusCode::FAILURE;
  if ( m_doHV) msg( MSG::FATAL ) << "Not yet properly implemented and tested!" << endreq;  
  if ( !m_useDCS ) {
    msg( MSG::DEBUG ) << "DCSConditionsSvc was removed in initialization" << endreq;
    if ( m_doHV ) {
	msg( MSG::ERROR ) << "DCSConditionsSvc has to be initialized for HvSvc" << endreq;
  ...
  if ( !m_useConfiguration ) {
    msg( MSG::DEBUG ) << "ConfigurationConditionsSvc was removed in initialization" << endreq;
  } else {
    if (not retrievedService(m_ConfigurationConditionsSvc)) return StatusCode::FAILURE;
  }

  if ( !m_useCalibration ) {
    msg( MSG::DEBUG ) << "ReadCalibDataSvc was removed in initialization" << endreq;
  } else {
    if (not retrievedService(m_ReadCalibDataSvc)) return StatusCode::FAILURE;
  }

  if ( !m_useMajority ) {
    msg( MSG::DEBUG ) << "MajorityConditionsSvc was removed in initialization" << endreq;
  } else {
  ...
  if ( !m_useBSError ) {
    msg( MSG::DEBUG ) << "ByteStreamErrorsSvc was removed in initialization" << endreq;
  } else {
  ...
  //--- LB range
  try {      
      m_LBRange = boost::lexical_cast<int>( m_LBMax );
      ISCT_CalibHistoSvc::setNumberOfLb(m_LBRange);
  } catch (...) {
      msg(MSG::ERROR) << "Couldn't cast m_LBMax=\""<< m_LBMax <<"\" to m_LBRange..."<<endreq;
      m_LBRange = 0;
  }
  ...
  if ( m_readBS  && m_readHIST ) {
    msg( MSG::ERROR ) << "Both BS and HIST are set to be read. Choose either of BS or HIST." << endreq;
    return StatusCode::FAILURE;
  }
  
  //--- Open BS
  if ( m_readBS ) {
      msg( MSG::INFO ) << "------------> Reading from ByteStream <-------------"<< endreq;
  ...
  //--- Open HIST
  if ( m_readHIST ) {
    msg( MSG::INFO ) << "------------> Reading from HIST <-------------" << endreq; 
    m_calibEvtInfoSvc->setSource("HIST");
    //--- List of HIST
    std::string hist("");
    std::vector<std::string> histCollection = m_input_hist.value();
    msg( MSG::INFO ) << "The input histogram name is : " << m_input_hist << endreq;
    if (not histCollection.empty()){
      hist=histCollection.back();
      if (histCollection.size() > 1) msg( MSG::WARNING ) << "The Histogram collection contains more than one histogram; using the last one."<< endreq;
    } else {
...
SCTCalib::notEnoughStatistics(const int required, const int obtained, const std::string & histogramName){
  if (obtained<required){
    msg( MSG::ERROR ) << "Number of events in "<<histogramName<<": "<<obtained<<" is less than the required minimum number of events " <<required<< endreq;
    return true;
  }
  return false;
}
...
StatusCode SCTCalib::beginRun(){
  msg( MSG::INFO ) << "----- in beginRun() -----" << endreq;
  //--- Check if configuration data is available or not
  if ( m_useConfiguration and ( m_doNoisyStrip or m_doDeadStrip or m_doDeadChip ) ) {
    if ( not m_ConfigurationConditionsSvc->filled() ) return msg( MSG::ERROR ) << "Configuration data is not available" << endreq, StatusCode::FAILURE;
  }
  //--- Check if calibration data is available or not
  if ( m_useCalibration and m_doNoisyStrip ) {
    if ( not m_ReadCalibDataSvc->filled() ) return msg( MSG::ERROR ) << "Calibration data is not available" << endreq, StatusCode::FAILURE;
  }
  return StatusCode::SUCCESS;
}
...
StatusCode SCTCalib::endRun(){
  ...
  if ( doNoisyStripAnalysis and getNoisyStrip().isFailure() ) return msg( MSG::ERROR ) << "Failed to run getNoisyStrip()" << endreq, StatusCode::FAILURE;
  ...
  if ( m_doHV ) {
    ...
    for( ; waferItr not_eq m_waferItrEnd; ++waferItr ) {
      ...
      const vector< pair<int,int> > & tvec= m_summarytrips[waferHash];
      const vector< pair<int,int> > & tlbn= m_summarytripslb[waferHash];
      ...
      if (numberOfElements > onlyDummy){
        ...
        for (unsigned int itrip(2); itrip != numberOfElements; ++itrip){ //skip 0 since that is just the dummy pair. 
          if (areConsecutiveIntervals(tvec[itrip],timeInterval,withinLimits)){
            timeInterval.second  =  tvec[itrip].second;
            lbRange.second = tlbn[itrip].second;
          } else {
            //not consecutive, so first print out the old one
            doHVPrintXML(timeInterval,lbRange,waferId);
            timeInterval =  tvec[itrip];
            lbRange =  tlbn[itrip];
          }
        } // end loop over times
        doHVPrintXML(timeInterval,lbRange,waferId );
  ...
  if ( (m_doDeadStrip or m_doDeadChip ) and getDeadStrip().isFailure() ) return msg( MSG::ERROR ) << "Failed to run getDeadStrip()" << endreq, StatusCode::FAILURE ;
  
  //--- Upload noise occupancy
  if ( m_doNoiseOccupancy  and getNoiseOccupancy().isFailure() ) return msg( MSG::ERROR ) << "Failed to run getNoiseOccupancy()" << endreq, StatusCode::FAILURE;
  
  //--- Upload raw occupancy
  if ( m_doRawOccupancy and getRawOccupancy().isFailure() ) return msg( MSG::ERROR ) << "Failed to run getRawOccupancy()" << endreq, StatusCode::FAILURE;
  
  //--- Upload efficiency
  if ( m_doEfficiency and getEfficiency().isFailure() ) return msg( MSG::ERROR ) << "Failed to run getEfficiency()" << endreq, StatusCode::FAILURE;

  //--- Upload ByteStream Errors
  if ( m_doBSErrorDB and getBSErrors().isFailure() ) return msg( MSG::ERROR ) << "Failed to run getBSErrors()" << endreq, StatusCode::FAILURE;
...
StatusCode SCTCalib::finalize(){
  ...
  msg( MSG::INFO ) << "Thank-you for using SCT_CalibAlgs, version "<<PACKAGE_VERSION << endreq;
  return StatusCode::SUCCESS;
}
...
StatusCode SCTCalib::getNoisyStrip(){
  ...
  //--- Check statistics
  //msg( MSG::INFO ) << m_calibEvtInfoSvc->counter() << "   " <<m_calibHitmapSvc->size() << endreq;
  int numOfEventsProcessed=m_calibEvtInfoSvc->counter();
  m_numOfEventsProcessed=m_calibHitmapSvc->size();
  if ((int) m_numberOfEvents < (int) m_noisyMinStat ) {
    msg( MSG::ERROR ) << "in getNoisyStrip() : Number of events processed "    << numOfEventsProcessed
  ...
  if (m_calibModuleListSvc->readModuleList(moduleLists[REF]).isFailure()) return msg( MSG::ERROR ) << "Could not read moduleList" << endreq, StatusCode::FAILURE;
  ...
  for( ; waferItr not_eq waferItrE; ++waferItr ) {
    ...
    if (numNoisyStripsInWafer!=0){
      if ( m_noisyWaferFinder && isNoisyWafer ) { //in noisy wafer
	    ...
	    //agrohsje shouldn't the second be false,true ?
          if ( addStripsToList( waferId, stripIdLists[ALL], false, false ).isFailure() or  addStripsToList( waferId, stripIdLists[NEW], false, false ).isFailure() ){
  ...
  msg( MSG::DEBUG ) << "------ Before writing into COOL ------" << endreq;
  if ( m_writeToCool ) {
    if ( writeModuleListToCool( moduleLists[ALL], moduleLists[NEW], moduleLists[REF] ).isFailure() ) {
      msg( MSG::ERROR ) << "Could not write NoisyStrips into COOL" << endreq;
      return StatusCode::FAILURE;
    }
  }
  //--- XML outputs
...
StatusCode SCTCalib::getDeadStrip(){
  ...
  if (msgLvl(MSG::INFO)) msg(MSG::INFO) << "getDeadStrip() called" << endreq;
  ...
  if(m_doDeadStrip){
    ...
    if(closeXML4DB(m_outDeadStrips).isFailure()){
      msg(MSG::ERROR)<<"Problem closing "<<m_deadStripsFile<<endreq;
      return StatusCode::FAILURE;
    }
  }
  if(m_doDeadChip){
    msg(MSG::INFO)<<"total #DeadChip : "<<n_deadChip<<", #noHitChip : "<<n_checkedChip<<endreq;
    if(closeXML4DB(m_outDeadChips).isFailure()){
      msg(MSG::ERROR)<<"Problem closing "<<m_deadChipsFile<<endreq;
      return StatusCode::FAILURE;
    }
  }
...
{
  msg( MSG::INFO ) << "----- in getNoiseOccupancy() -----" << endreq;
  ...
  //--- RunNumber
  std::ostringstream runnum; runnum << (int) m_runNumber;
  ...
  for ( int iDisk = 0; iDisk < n_disks ; ++iDisk ) {
    for ( int iSide = 0; iSide < 2; ++iSide ) {
      ostringstream streamHist; streamHist << "noiseoccupancymap";
  ...
  osHeader << "<channels server=\"ATLAS_COOLPROD\" schema=\"ATLAS_COOLONL_SCT\" dbname=\"COMP200\" folder=\"SCT/Derived/NoiseOccupancy\" "
  ...
  for ( int iDisk = 0; iDisk < n_disks ; ++iDisk ) {
    for ( int iSide = 0; iSide < 2; ++iSide ) {
	  ...
	  //--- For calculating average Noise Occupancy
         ...
         outFile << xmlChannelNoiseOccDataString(waferId, occupancy)<<endl;
          //--- DB output
          if ( m_writeToCool ) {
            if ( m_pCalibWriteSvc->createListNO( waferId, m_pSCTHelper, 10000, occupancy ).isFailure() ) {
              msg( MSG::ERROR ) << "Unable to run createListNO" << endreq;
              return StatusCode::FAILURE;
            }
          }
        }
  ...
  if( m_writeToCool ) {
    if ( m_pCalibWriteSvc->wrapUpNoiseOccupancy().isFailure() ) {
      msg( MSG::ERROR ) << "Could not get NoiseOccupancy" << endreq;
      return StatusCode::FAILURE;
    }
  }
...
{
  msg( MSG::INFO ) << "----- in getRawOccupancy() -----" << endreq;
  ...
  //--- Directory in HIST
  std::vector< std::pair<std::string, int> > EC_stems; EC_stems.clear();
    ...
    for(int iDisk=0; iDisk<n_disks; ++iDisk)
      for(int iSide=0; iSide<2; ++iSide)
        for(int iEta=0; iEta<n_etaBinsEC; ++iEta)
          for(int iPhi=0; iPhi<n_phiBinsEndcap[iDisk][iEta]; ++iPhi){
            ...
            std::string detector_part; detector_part.erase();
            if(m_histBefore2010){
      ...
      if( m_writeToCool ){
        if( m_pCalibWriteSvc->createListRawOccu(waferId, m_pSCTHelper, m_numberOfEvents, raw_occu).isFailure() ){
          msg( MSG::ERROR ) << "Unable to run createListRawOccu" << endreq;
          return StatusCode::FAILURE;
        }
      }
    }
  ...
  if( m_writeToCool ) {
    if ( m_pCalibWriteSvc->wrapUpRawOccupancy().isFailure() ) {
      msg( MSG::ERROR ) << "Could not get RawOccupancy" << endreq;
      return StatusCode::FAILURE;
    }
  }
...
StatusCode SCTCalib::getEfficiency(){
  msg(MSG::INFO) << "----- in getEfficiency() -----" << endreq;
          ...
          for(int iPhi=0; iPhi<n_phiBinsEndcap[iDisk][iEta]; ++iPhi){
            ...
            if( m_writeToCool ){
              if( m_pCalibWriteSvc->createListEff(waferId, m_pSCTHelper, eff_entry, eff).isFailure() ){
                msg( MSG::ERROR ) << "Unable to run createListEff" << endreq;
                return StatusCode::FAILURE;
              }
            }
         }
  ...
  if( m_writeToCool ) {
    if ( m_pCalibWriteSvc->wrapUpEfficiency().isFailure() ) {
      msg( MSG::ERROR ) << "Could not get Efficiency" << endreq;
      return StatusCode::FAILURE;
    }
  }
...
StatusCode SCTCalib::getBSErrors(){
  msg( MSG::INFO ) << "----- in getBSErrors() -----" << endreq;
  ...
  boost::array<std::string, numberOfErrorTypes> errorNames={{
    "TBSParseErrs","TTimeOutErrs", "TBCIdErrs","TLVL1IdErrs", "TpreambleErrs", "TformatterErrs",
    "TABCDErrs","TRawErrs", "TmaskedLinks", "TRODClockErrs",
    "TTruncatedROD", (m_histBefore2010?"TROBFragmentErrs":"TROBFragErrs" ) }};
  //
  boost::array<std::string, numberOfErrorTypes> errorNames_C={{
    "TBSParseErrs","TTimeOutErrs","TBCIdErrs","TLVL1IdErrs", "TpreambleErrs","TformatterErrs",
    "TABCDErrs","TRawErrs", (m_histBefore2010?"TMaskedLinks":"TmaskedLinks"), "TTRODClockErrs",
    "TTruncatedROD", (m_histBefore2010?"TROBFragmentErrs":"TROBFragErrs" ) }};
    ...
    for(int iDisk=0; iDisk<n_disks; ++iDisk)
      for(int iSide=0; iSide<2; ++iSide)
		...
		if ( iType == errItr->first ) {
		    ...
		    streamHist << errItr->second << detector_part << "_" << iDisk << "_" << iSide;
		    std::string profname = detectorStems[stemIndex] + streamHist.str();
		...
		osErrorList << errorProb;
		if ( iType != n_BSErrorType-1 ) osErrorList << " ";
	    }//end ErrorType Loop
            //--- DB writing
	    if(!(defecttype.empty())){
		n_errorLink++;
		if( thisBec==ENDCAP_C ) nErrLink_ECC[iDisk][iEta]++; 
		else if( thisBec==ENDCAP_A ) nErrLink_ECA[iDisk][iEta]++; 
		if( m_writeToCool ){
		    if( m_pCalibWriteSvc->createListBSErr(waferId, m_pSCTHelper, m_numberOfEvents, osErrorList.str()).isFailure() ){
			msg( MSG::ERROR ) << "Unable to run createListBSError" << endreq;
			return StatusCode::FAILURE;
		    }
		}
            }
  ...
  for(int iLayer=0; iLayer<n_barrels; ++iLayer)
    for(int iSide=0; iSide<2; ++iSide)
	  ...
	  for ( int iType = 0; iType < n_BSErrorType; ++iType ) {
	      ...
	      if ( iType == errItr->first ) {
		  ostringstream streamHist; 
		  streamHist << errItr->second << "_" << iLayer << "_" << iSide;
		  std::string profname = "/run_" + runnum.str() + "/SCT/SCTB/errors/" + streamHist.str();
		  TProfile2D* prof_tmp = (TProfile2D*) inputHist->Get( profname.c_str() );
		  unsigned long long n_errors = (unsigned long long)prof_tmp->GetBinContent( iEta+1, iPhi+1 );
		  if(n_errors!=0){
		      defecttype = m_pCalibWriteSvc->addNumber( defecttype, errItr->first );
  ...
  msg( MSG::INFO ) << "#Links which send BSError : " << n_errorLink << endreq;
  ...
  if( m_writeToCool ) {
    if ( m_pCalibWriteSvc->wrapUpBSErrors().isFailure() ) {
      msg( MSG::ERROR ) << "Could not get ByteStream Errors" << endreq;
      return StatusCode::FAILURE;
    }
  }
...
StatusCode SCTCalib::getLorentzAngle(){
  msg( MSG::INFO ) << "----- in getLorentzAngle() -----" << endreq;
  ...
  for ( int iLayer = 0; iLayer < n_barrels; ++iLayer ) {
    for ( int iSide = 0; iSide < 2; ++iSide ) {
     for ( int iModule = 0; iModule < 2; ++iModule ) { 
          ...
          msg( MSG::INFO ) << "LorentzAngle fit start : " << 4*iLayer + iSide +1 + iModule << " / 16" << endreq;
          Int_t fitResult;
          Double_t par[4],err_par[4];
          TF1 *LAfit = new TF1("LAfit", LA_func, -9., 2.,4 );
          ostringstream streamFile; streamFile << "h_phiVsNstrips_" << module[iModule] << "_" << iLayer << "Side" << iSide; 
      ...
      if ( m_LorentzAngleDebugMode  ){
          ostringstream streamFileTmp; streamFileTmp << "h_phiVsNstrips_" << module[iModule] << "_" << iLayer << "Side" << iSide << "_First_Fit";
      ...
      if( fitResult != 0 ){
          msg( MSG::INFO ) << "Try to use parabola Fit to determine initial value!" << endreq;
      ...
      if( fitResult != 0 ){
          msg( MSG::INFO ) << "Try to fix one parameter sigma=2.0 to determine other initial value!" << endreq;
      ...
      if ( fitResult == 0 ) {
           ...
           msg( MSG::WARNING ) << "Fit Failed! Unable to get LorentzAngle" << endreq;
      }
      double A      = par[0];
  ...
  if (found!=nameAssociation.end()){
    ...
    msg( MSG::ERROR ) << " argument \"type\" needs to be  LorentzAngle." << endreq;
    return StatusCode::FAILURE;
  }
  ...
  if( m_writeToCool ) {
    if ( m_pCalibWriteSvc->wrapUpLorentzAngle().isFailure() ) {
      msg( MSG::ERROR ) << "Could not get LorentzAngle" << endreq;
      return StatusCode::FAILURE;
    }
  }
...
StatusCode SCTCalib::openXML4DB( std::ofstream& file, const char* type, const char* tag, IOVTime start, IOVTime end ) const {
  if( !strcmp( type, "DeadStrip" ) ) {
    ...
    if( file == 0) return StatusCode::FAILURE;
    file << "<channels server=\"ATLAS_COOLPROD\" schema=\"ATLAS_COOLONL_SCT\" dbname=\"COMP200\" folder=\"SCT/Derived/DeadStrips\" ";
  } else if ( !strcmp( type, "DeadChip" ) ) {
    file.open( m_deadChipsFile.c_str(), std::ios::out );
    if( file == 0 ) return StatusCode::FAILURE;
    file << "<channels server=\"ATLAS_COOLPROD\" schema=\"ATLAS_COOLONL_SCT\" dbname=\"COMP200\" folder=\"SCT/Derived/DeadChips\" ";
  } else {
...
StatusCode SCTCalib::openXML4DeadSummary( std::ofstream& file, const char* type, int n_Module, int n_Link, int n_Chip, int n_Strip ) const {
  if ( !strcmp( type, "DEAD" ) ) {
    ...
    msg( MSG::ERROR ) << "in openXML4DeadSummary : argument \"type\" needs to be \"DEAD\"." << endreq;
    return StatusCode::FAILURE;
  }
...
std::string 
SCTCalib::xmlChannelNoiseOccDataString(const Identifier & waferId,  const float occupancy){
...
SCTCalib::writeModuleListToCool( const std::map< Identifier, std::set<Identifier> >& moduleListAll,
    ...
    if ( m_pSCTHelper->side( *idItr ) == 0 ) {
      ...
      if ( m_noisyUpdate ) { //--- UPD1/UPD4
	    ...
	    if(m_pCalibWriteSvc->createCondObjects( moduleId, m_pSCTHelper, 10000, "NOISY", noisyStripThr, defectStripsAll ).isFailure()){
            msg( MSG::ERROR ) << "Could not create defect strip entry in the CalibWriteSvc." << endreq;
          }
          nDefects++;
        } else msg( MSG::DEBUG ) << "Module "<< moduleId  <<" is identical to the reference output" << endreq;
      } else {
  ...
  //msg( MSG::INFO ) << "Number of modules for which conditions were created: " << nDefects <<"  !!!!"<< endreq;
  if ( moduleListAll.empty() || nDefects==0 ) {
    msg( MSG::INFO ) << "Number of noisy strips was zero or the same list of noisy strips. No local DB was created." << endreq;
  } else {
    if ( m_pCalibWriteSvc->wrapUpNoisyChannel().isFailure() ) {
      msg( MSG::ERROR ) << "Could not get NoisyStrips info" << endreq;
      return StatusCode::FAILURE;
    }
  }
  return StatusCode::SUCCESS;
...
SCTCalib::noisyStripsToXml( const std::map< Identifier, std::set<Identifier> >& moduleList, const std::string& badStripsFile ) const {
  ...
  outFile << "<channels server=\"ATLAS_COOLPROD\" schema=\"ATLAS_COOLONL_SCT\" dbname=\"COMP200\" folder=\"SCT/Derived/Monitoring\" "
    <<           "since=\""   << m_iovStart.re_time() << "\" "
    <<           "until=\""   << m_iovStop.re_time()  << "\" "
...
StatusCode SCTCalib::noisyStripsToSummaryXml( const std::map< Identifier, std::set<Identifier> >& moduleListAll,
  ...
  osNoisyModuleDiff << "Change of #(modules w/ at least 1 noisy strip) from average of recent runs less than " << m_noisyModuleDiff*100 << "%";
  osNoisyStripDiff  << "Change of #(noisy strips) from the last run less than "                                << (int) m_noisyStripDiff;
...
SCTCalib::getNoisyChips( const std::set<Identifier>& stripIdList ) const {
  std::set<int> chipIdList; chipIdList.clear();
...
SCTCalib::getNoisyLB( const Identifier& moduleId, int& chipId ) const {
  ...
  std::set<int> LBList; LBList.clear();
  if (!m_calibLbSvc){
     msg( MSG::ERROR ) << "NULL pointer m_calibLbSvc line "<<__LINE__ << endreq;
...
std::string SCTCalib::getLBList( const std::set<int>& LBList ) const {
  std::string strList; strList.erase();
  if ( !LBList.empty() ) {
    int firstLB = -1;
    int LBSize  = -1;
