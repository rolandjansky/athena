...
StatusCode RefitTracksAndVertex::execute() {
  ...
  if( primaryVertex ){
    ...
    if(trackParametersToFit.size() > 1){
      ...
      Trk::Vertex oldvtx( position );
      // Fit and store vertex 
      auto vertex =  m_vertexFitter->fit(trackParametersToFit, oldvtx );
      if(vertex){
        xAOD::Vertex* xAODVx = new xAOD::Vertex();
        theVertexContainer->push_back (xAODVx);
        xAODVx->setVertexType(xAOD::VxType::PriVtx);
        xAODVx->setPosition(vertex->recVertex().position());
        xAODVx->setCovariancePosition(vertex->recVertex().covariancePosition());
        xAODVx->setFitQuality(vertex->recVertex().fitQuality().chiSquared(), static_cast<float>(vertex->recVertex().fitQuality().doubleNumberDoF ()));
        

        ATH_MSG_DEBUG("Old Vtx " << primaryVertex->position());
        ATH_MSG_DEBUG("New Vtx " << xAODVx->position());
      }
    }
  }	
