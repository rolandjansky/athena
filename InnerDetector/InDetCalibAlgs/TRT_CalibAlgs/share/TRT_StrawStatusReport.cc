...
void initializeDeadStrawsList(){
  ...
  // bec sector straw strawlayer layer status
	...
	char filename[300]; sprintf(filename, "/afs/cern.ch/user/i/idcalib/w0/TRT_Calibration/tmp/Tier0/StrawStatusCode/TRT_StrawStatus_ExcludedStraws_2017.txt");
		  ...
		  if (!m_onlyMaskBoards){
		    ...
		    if ( (tmp[3]==0 && m_checkDead_LT)
			 ...
			 || (HToccupancy>0.5 && m_checkNoisy_HT   )
		      ...
		      continue;
		  printf("read ONE straw permanently masked is ALIVE!!! %d, %d, %d hits: %d occ: %f eff: %f  \n", tmp[0], tmp[1], tmp[2], tmp[3] ,occupancy, efficiency );
		  }
		}
...
void printAthenaBoardsOnly( int run ) { // print athena format
  ...
  char filename[1000]; sprintf(filename, "output/athenaFormat_DeadBoards_run%07d.txt", run );	
  std::cout << filename << endl;
  FILE *fout = fopen(filename, "w"); assert(fout);  
  ...
  for (int i=0; i<2; i++) for (int j=0; j<32; j++) {
	...
	if (allStrawsOnBoard[board].size()==newDeadStrawsOnBoard[board].size()) {
	  ...
	  int side = i ? -1 : 1;
	  printf("found a new dead board!!! \n");
	  printf("See TRT_StrawMap.h for board ID info \n");
	  printf("side: %d, phi: %d, board ID: %d \n\n", side, j, board);
	}
      }
    }
