...
C***********************************************************************
      ...
      SUBROUTINE JNDELT
CW 12/11/2002:
CW THIS ROUTINE HAS BEEN ADAPTED
CW TO ALLOW THE USE OF WEIGHTS
CW IN THE TRAINING PHASE.
CW
...
CW 12/11/2002: D(MI) corresponds to delta_i (greek symbol) in my diploma thesis
CW             they have to be multiplied with the weight to propagate it
CW             correctly to the final update (which is done in JNTRAL)      
      IF (MSTJN(4).EQ.-1) THEN
        DO 101 I=1,M(NL)
          MI=MV0(NL)+I
          DIFF=OUT(I)-O(MI)
CW          D(MI)=DIFF*GPJN(MI)/(1.-DIFF**2)
          D(MI)=OWGT * DIFF*GPJN(MI)/(1.-DIFF**2)
101     CONTINUE
      ELSE
        DO 100 I=1,M(NL)
          MI=MV0(NL)+I
CW          D(MI)=(OUT(I)-O(MI))*GPJN(MI)
          D(MI)=OWGT * (OUT(I)-O(MI))*GPJN(MI)
100     CONTINUE
      ENDIF
...
CW MULTIPLY THE ERROR WITH THE WEIGHT
CW (COULD BE DONE DIRECTLY IN FUNCTION ERRJN. HOWEVER, THIS IS
CW  THE ONLY PLACE WHERE ERRJN IS CALLED -> EASIER TO DO IT HERE!)      
CW      ERR=ERRJN(0)
      ERR=OWGT * ERRJN(0)
