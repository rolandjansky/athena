...
namespace Overlay {
				   ...
				   IDC_OverlayBase *tmp)
  {
	...
	if(p->first < currentStrip + 2) {
      ...
      for(std::set<SCT_RDO_TYPE*>::const_iterator r = origRDOs.begin(); r!=origRDOs.end(); ++r) {
	tbin |= (*r)->getTimeBin();
	...
	std::vector<int> tmp((*r)->getErrorCondensedHit());
	errvec.insert(errvec.end(), tmp.begin(), tmp.end());
      }
