...
// virtual destructor
  ...
  PixelClusterCollection* MergedPixelsTool::clusterize(const InDetRawDataCollection<PixelRDORawData> &collection,
    ...
    if(m_doITkClustering) return clusterizeITk(collection,manager,pixelID);
    ...
    // loop on all RDOs
    for(RDO_Collection::const_iterator nextRDO=collection.begin(); nextRDO!=collection.end() ; ++nextRDO) {
      ...
      if(ganged){
        ATH_MSG_VERBOSE("Ganged Pixel, row = " << pixelID.phi_index(rdoID) << "Ganged row = " << pixelID.phi_index(gangedID));   
      } else
      ...
      while( !found && firstGroup!= lastGroup) {
        ...
        if (areNeighbours(**firstGroup, rdoID, element, pixelID)) {
          ...
          if(!isDuplicated(**firstGroup, **lvl1Group, rdoID, lvl1, pixelID)) {
            ...
            checkForMerge(rdoID, firstGroup, lastGroup, totGroup, lvl1Group, element, pixelID); 
          } else { ATH_MSG_VERBOSE("duplicate found"); }
          found = true; 
        }
        ++firstGroup;
        ++totGroup;
  ...
  PixelCluster* MergedPixelsTool::makeCluster(const std::vector<Identifier>& group,
    ...
    ATH_MSG_VERBOSE("Cluster width (eta x phi) = " << colWidth << " x " << rowWidth);
    ATH_MSG_VERBOSE("Cluster width (eta x phi) = " << etaWidth << " x " << phiWidth);
    ATH_MSG_VERBOSE("Cluster local position (eta,phi) = " << (position)[0] << " " << (position)[1]);
  ...
  ///////////////////////////////////////////////////////////////////
  // Pixel clusters production for fast mode 
  ///////////////////////////////////////////////////////////////////
  
  PixelClusterCollection*  MergedPixelsTool::clusterizeITk(const InDetRawDataCollection<PixelRDORawData> &collection,
                                                           const InDetDD::SiDetectorManager& manager,
                                                           const PixelID& pixelID) const {
    Identifier elementID  = collection.identify();
    IdentifierHash idHash = collection.identifyHash();
