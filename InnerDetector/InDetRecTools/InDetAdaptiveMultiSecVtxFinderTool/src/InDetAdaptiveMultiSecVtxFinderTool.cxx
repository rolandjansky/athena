...
InDetAdaptiveMultiSecVtxFinderTool::InDetAdaptiveMultiSecVtxFinderTool(const std::string& t, const std::string& n, const IInterface*  p)
				: base_class(t,n,p)
{}
...
std::pair<xAOD::VertexContainer*, xAOD::VertexAuxContainer*> InDetAdaptiveMultiSecVtxFinderTool::findVertex(const xAOD::TrackParticleContainer* trackParticles)
{
		 ...
		 const std::vector<Trk::ITrackLink*> & trackVector) 
{
	...
	do{
		if (seedtracknumber == 0){
			ATH_MSG_DEBUG("New iteration. No tracks available after track selection for seeding.");
		}
		...
		for (const Trk::ITrackLink *seedtrkAtVtxIter : seedTracks) {
		...
		MvfFitInfo(*actualCandidate) = new Trk::MvfFitInfo(constraintVertex, new Amg::Vector3D(seedVertex), new Amg::Vector3D(seedVertex));
	...
	if (iteration >= m_maxIterations) {
		ATH_MSG_DEBUG("Maximum number of iterations (" << m_maxIterations << ") reached; to reconstruct more vertices, set maxIterations to a higher value.");
	}

	ATH_MSG_DEBUG("Secondary vertex finding complete with " << iteration << " iterations and " << myxAODVertices.size() << " vertices found.");
	...
	for (const xAODVertex_pair& vtxIter : myxAODVertices){
		...
		for (Trk::VxTrackAtVertex* MVFtrkIter : VTAV(*fittedVert)) {
			if ((*MVFtrkIter).initialPerigee()){
				(*MVFtrkIter).setPerigeeAtVertex(((*MVFtrkIter).initialPerigee())->clone());
			}
	...
	for (Trk::TrackToVtxLink *iterator : myTrackToVtxLinks) {
		delete iterator;
	}
	...
	for (unsigned int i = 0 ; i < theVertexContainer->size() ; i++)
	{
		...
		switch (vtxType){
			...
			case xAOD::VxType::SecVtx : 
			  ...
			  break;
			default: 
			    ATH_MSG_DEBUG( "Unfamiliar vertex type");
...
StatusCode InDetAdaptiveMultiSecVtxFinderTool::finalize()
{
...
void InDetAdaptiveMultiSecVtxFinderTool::countTracksAndNdf(xAOD::Vertex * myxAODVertex,
...
bool InDetAdaptiveMultiSecVtxFinderTool::checkFit(xAOD::Vertex *actualCandidate) const
{
	...
	ATH_MSG_DEBUG( " xAOD::Vertex : " << ( actualCandidate != 0 ? 1 : 0 ) << ",  #dof = " << m_ndf << ",  #tracks (weight>0.01) = " << m_ntracks );
...
int InDetAdaptiveMultiSecVtxFinderTool::removeTracksFromSeeds(xAOD::Vertex *actualCandidate, std::vector<Trk::ITrackLink*> & seedTracks) const
{
	...
	for (Trk::VxTrackAtVertex* trkAtVtxIter : VTAV(*actualCandidate)){
		...
		for (std::vector<Trk::ITrackLink*>::iterator seedtrkiter = seedBegin; seedtrkiter != seedEnd; ++seedtrkiter){
			...
			if ( (*seedtrkiter)->parameters() == (trkAtVtxIter)->trackOrParticleLink()->parameters() && (trkAtVtxIter)->weight() > m_minWghtAtVtx) {
...
float InDetAdaptiveMultiSecVtxFinderTool::findCompatibleTracks(Amg::Vector3D &seedVertex, Trk::ITrackLink* trkIter) const
{
	...
	try
	{
		std::unique_ptr<Trk::PlaneSurface> mySurface=m_ImpactPoint3dEstimator->Estimate3dIP((trkIter)->parameters(),&seedVertex,distance);
	...
	if (distance<0)
	{
		ATH_MSG_DEBUG( " Distance between track and seed vtx is negative: " << distance );
	}
...
void InDetAdaptiveMultiSecVtxFinderTool::removeClosestTrack(Amg::Vector3D &seedVertex, std::vector<Trk::ITrackLink*> & seedTracks, int &nFound) const
{
	...
	for (Trk::ITrackLink* trkIter : seedTracks){
		...
		try{
			...
			if(distance < 0){
				ATH_MSG_DEBUG("Distance was negative!");
			}
...
bool InDetAdaptiveMultiSecVtxFinderTool::V0check(const std::vector< Amg::Vector3D > & momenta, const Amg::Vector3D & posi) const
{
	...
	double eLam = Pv[0] > Pv[1] ?  std::sqrt( Pv[0] + prtn2 ) + std::sqrt( Pv[1] + pi2 ) : 
	                      std::sqrt( Pv[0] + pi2 ) + std::sqrt( Pv[1] + prtn2 )  ;
