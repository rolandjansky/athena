...
(float* P ,float* A,std::vector<InDet::SiDetElementLink_xk*>& lDE)
{
  const float pi = M_PI, pi2 = 2.*M_PI; 
  ...
  float Zc   = P[2]+A[2]*s                    ;
  float Sz   = (m_dr+P[4])*(fabs(A[2])/At)+70.;
  if(fabs(Zc-m_z) > (m_dz+Sz)) return         ;
  
  float dW   = P[4]/m_r+m_dfe                 ;
  float Fc   = atan2(P[1]+A[1]*s,P[0]+A[0]*s)-dW; if(Fc <-pi) Fc+=pi2; 

  int   ie   = m_elements.size();
  int   i1   = ie-1;

  if     (Fc <= m_elements[ 0].phi()) i1 = 0 ;
  else if(Fc >  m_elements[i1].phi()) i1 = ie;
  else  {
    int i0=0; while((i1-i0)>1) {int i = ((i0+i1)>>1); m_elements[i].phi() >= Fc ? i1=i : i0=i;}
  }
  ...
  for(int i = i1; i!=ie; ++i) {
    ...
    if( m_elements[i].phi() > Fc ) return;

    if(fabs(Zc-m_elements[i].z()) < (m_elements[i].dz()+Sz) &&  m_elements[i].intersectITk(P,A,s)) {
       m_elements[i].setUsed(s); lDE.push_back(&m_elements[i]); 
    }
  }
...
(float* P ,float* A,std::vector<InDet::SiDetElementLink_xk*>& lDE)
{
  ...
  float dW  = P[4]/rc+m_dfe;
  float Fc  = atan2(yc,xc)-dW; if(Fc <-pi) Fc+=pi2; 
  ...
  for(int i = i1; i!=ie; ++i) {
    ...
    if(m_elements[i].intersectITk(P,A,s)) {
      m_elements[i].setUsed(s); lDE.push_back(&m_elements[i]); 
    }
  }
