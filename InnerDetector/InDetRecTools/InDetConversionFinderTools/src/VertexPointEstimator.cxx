...
namespace InDet {
  ...
  // ----------------------------------
  void VertexPointEstimator::getStartingPoint(const Trk::Perigee *per1, const Trk::Perigee *per2, Amg::Vector3D &startingPoint, int &sflag, int sit,  double& m_ctvDr, double& m_ctvRmax, double& m_ctvDz, double& m_ctvArc, double& m_MDdr, double& m_area, double& m_h, double& m_hl, double& m_ddphi)
  {
    /*
      Calculates the initial approximation to the vertex position. Based on CTVMFT.
      Tries to intersect circles that are (R,Phi) projections of the helical trajectories of the two tracks
    ...
    */
    ...
    double DRMAX = 999999999; // = 0.; // maximum XY separation, non-intersecting circles
    double DZMAX = 999999999; // = 0.; // maximum allowed track Z separation at the vertex
    double RVMAX = 999999999; // = 0.; // maximum allowed vertex radius
    double DSMIN = -999999999; // = 1000.;
    double DSMAX = 999999999; // = -1000.;
    if(sit == 0) {
      DRMAX =  999999999;
      DZMAX =  999999999; 
      RVMAX =  999999999; 
    ...
    const AmgVector(5)& perigeeParms1 = per1->parameters();
    ...
    qOverPt[0] = qOverP1 / sin(perigeeParms1[Trk::theta]);
    ...
    const AmgVector(5)& perigeeParms2 = per2->parameters();
    ...
    qOverPt[1] = qOverP2 / sin(perigeeParms2[Trk::theta]);
    ...
    Amg::Vector3D a(X,Y,Z);
    startingPoint = a;  
  }
  
  // ----------------------------------
  /**simplified Billoir method */
  Amg::Vector3D VertexPointEstimator::estimateVertexPosition(const Trk::Perigee *per1, const Trk::Perigee *per2){

    Amg::Vector3D intPoint(0.,0.,0.);
    Amg::Vector3D spatialPoint(0.,0.,0.);
    double cot_theta_1 = 1./tan(per1->parameters()[Trk::theta]);
    double sphi_1 = sin(per1->parameters()[Trk::phi]);
    double cphi_1 = cos(per1->parameters()[Trk::phi]);
    AmgVector(3) locXpVec_1;
    locXpVec_1[0]= -per1->parameters()[Trk::d0]*sphi_1;
    locXpVec_1[1]= per1->parameters()[Trk::d0]*cphi_1;
    locXpVec_1[2]= per1->parameters()[Trk::z0];
    
    double cot_theta_2 = 1./tan(per2->parameters()[Trk::theta]);
    double sphi_2 = sin(per2->parameters()[Trk::phi]);
    double cphi_2 = cos(per2->parameters()[Trk::phi]);
    Amg::Vector3D locXpVec_2;
    locXpVec_2[0]= -per2->parameters()[Trk::d0]*sphi_2;
    locXpVec_2[1]= per2->parameters()[Trk::d0]*cphi_2;
    locXpVec_2[2]= per2->parameters()[Trk::z0];
    
    AmgSymMatrix(3) DtWD_sum;
    AmgVector(3) DtWDx_sum;
    AmgSymMatrix(3) DtWD_1; DtWD_1.setZero();
    AmgSymMatrix(3) DtWD_2; DtWD_2.setZero();
    AmgSymMatrix(2) cov_mat_1; cov_mat_1.setIdentity();
    AmgSymMatrix(2) cov_mat_2; cov_mat_2.setIdentity();
    
    if(per1->covariance()==0 || per2->covariance()==0){
      ATH_MSG_DEBUG("Failed to obtain covariance matrix! Skipping this pair");
      return intPoint;
    }else{
      cov_mat_1 = per1->covariance()->block(0,0,1,1);
      cov_mat_2 = per2->covariance()->block(0,0,1,1);
      // and the weightmatrix
      bool invertible=true; 
      double determinant;
      AmgSymMatrix(2) cov_mat_invert_1; 
      cov_mat_invert_1.setZero(); 
      cov_mat_1.computeInverseAndDetWithCheck(cov_mat_invert_1,determinant,invertible);
      if(!invertible){
        ATH_MSG_DEBUG("Failed to invert covariance matrix! Skipping this pair");
        return intPoint;
      }
      AmgSymMatrix(2) cov_mat_invert_2; 
      cov_mat_invert_2.setZero(); 
      cov_mat_2.computeInverseAndDetWithCheck(cov_mat_invert_2,determinant,invertible);
      if(!invertible){
        ATH_MSG_DEBUG("Failed to invert covariance matrix! Skipping this pair");
        return intPoint;
      }
      // calculate the D matrix
      AmgMatrix(2,3) D_1; D_1.setZero();
      AmgMatrix(2,3) D_2; D_2.setZero();
      ...
      DtWD_1 = D_1.transpose()*cov_mat_invert_1*D_1;
      DtWD_2 = D_2.transpose()*cov_mat_invert_2*D_2;
      DtWD_sum = DtWD_1 + DtWD_2;
      DtWDx_sum = D_1.transpose()*cov_mat_invert_1*D_1*locXpVec_1 + D_2.transpose()*cov_mat_invert_2*D_2*locXpVec_2;      
      AmgSymMatrix(3) cov_spatialPoint; 
      cov_spatialPoint.setZero(); 
      DtWD_sum.computeInverseAndDetWithCheck(cov_spatialPoint,determinant,invertible);
      if(!invertible){
        ATH_MSG_DEBUG("Failed to invert covariance matrix! Skipping this pair");
        return intPoint;
      }
      spatialPoint = Amg::Vector3D(cov_spatialPoint * DtWDx_sum);
      Amg::Vector3D XpVec_sp_1 = locXpVec_1 - spatialPoint;
      Amg::Vector3D XpVec_sp_2 = locXpVec_2 - spatialPoint;
    
      AmgVector(1) chi2=   (XpVec_sp_1.transpose()*DtWD_1*XpVec_sp_1+ XpVec_sp_2.transpose()*DtWD_2*XpVec_sp_2);
      if (chi2[0] <= m_maxChi2) {
        intPoint[0] = spatialPoint[0]; intPoint[1] = spatialPoint[1]; intPoint[2] = spatialPoint[2];
        if (msgLvl(MSG::VERBOSE)) msg(MSG::VERBOSE)<<"found spatial point = ("<<intPoint[0]<<", "<<intPoint[1]<<", "<<intPoint[2]<<")"<< endreq;
        return intPoint;
      } else {
        if (msgLvl(MSG::VERBOSE)) msg(MSG::VERBOSE) <<"found spatial point candidate doesn't pass chi2_cut" << endreq;
        return intPoint;
      }
    }//of obtain matrices
  }
