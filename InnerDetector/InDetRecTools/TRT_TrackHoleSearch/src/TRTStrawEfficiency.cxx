	...
	m_n_tube_hits(0)
{
    ...
    declareProperty("event_info_key",           m_event_info_key =      "ByteStreamEventInfo" );
    declareProperty("required_trigger",         m_required_trigger =    "" );

    declareProperty("VertexCollectionName",     m_vertexCollectionName = "VxPrimaryCandidate");	// added by dan
...
StatusCode TRTStrawEfficiency::initialize()
{
    ...
    // retrieve THistSvc
    if( m_hist_svc.retrieve().isFailure() )
    {
        ATH_MSG_FATAL( "Failed to retrieve THistSvc." );
        return StatusCode::FAILURE;
    }

    // retrieve TrigDecisionTool
    if(( !m_required_trigger.empty() )&&( m_trigDec.retrieve().isFailure() ))
    {
        ATH_MSG_FATAL( "Failed to retrieve TrigDecisionTool." );
        return StatusCode::FAILURE;
    }

    // ntuple
    m_tree = new TTree(m_tree_name.c_str(), m_tree_name.c_str());
    if(m_hist_svc->regTree(std::string("/") + m_stream_name + std::string("/") + m_tree_name, m_tree).isFailure())
    {
        ATH_MSG_FATAL("regTree failed.");
        return StatusCode::FAILURE;
    }
...
//    ATH_MSG_DEUBG( "getListOfTriggers: " << m_trigDec->getListOfTriggers() );
    ...
    {
        ATH_MSG_FATAL( "Failed to retrieve " << m_track_collection );
        return StatusCode::FAILURE;
    }

    const EventInfo* event_info = 0;
    if( evtStore()->retrieve( event_info, m_event_info_key ).isFailure() || !event_info )
    {
        ATH_MSG_ERROR( "Failed to retrieve " << m_event_info_key );
        return StatusCode::FAILURE;
    }
    ...
    for(TrackCollection::const_iterator track = tracks->begin(); track != tracks->end(); ++track)
    {
            ...
            it != track_states->end(); it++ )
        {
            ...
            {
                if(      dynamic_cast<const InDet::TRT_DriftCircleOnTrack*> ( (*it)->measurementOnTrack() )  ) m_n_trt_hits++;
                else if( dynamic_cast<const InDet::SCT_ClusterOnTrack*>     ( (*it)->measurementOnTrack() )  ) m_n_sct_hits++;
                else if( dynamic_cast<const InDet::PixelClusterOnTrack*>    ( (*it)->measurementOnTrack() )  ) m_n_pixel_hits++;
            }
        }
        ...
        else
        {
            ...
            m_n_sct_holes = 0;
            m_n_trt_holes = 0;
            for(DataVector<const Trk::TrackStateOnSurface>::const_iterator it = holes->begin();
                    it != holes->end(); it++ )
            {
                if((*it)->type(Trk::TrackStateOnSurface::Hole))
                {
                    int hole_det = fill_hole_data(**it);
                    switch(hole_det)
                    {
                        case 1: m_n_pixel_holes++; break;
                        case 2: m_n_sct_holes++; break;
                        case 3: m_n_trt_holes++; break;
                        default: ATH_MSG_ERROR("fill_hole_data returned an invalid det type: " << hole_det);
                    }
                }
                else
                {
...
int TRTStrawEfficiency::fill_hit_data(const Trk::TrackStateOnSurface& hit)
{
    ...
    int det = 0;
    if(m_TRT_ID->is_pixel(id))        det = 1;
    else if(m_TRT_ID->is_sct(id))     det = 2;
    else if(m_TRT_ID->is_trt(id))     det = 3;
    ...
    int chip = -1;
    if(det == 3) m_TRTStrawNeighbourSvc->getChip(id, chip);
    ...
    const Trk::TrackParameters* unbiased_track_parameters = m_updator->removeFromState(
                *(hit.trackParameters()),
