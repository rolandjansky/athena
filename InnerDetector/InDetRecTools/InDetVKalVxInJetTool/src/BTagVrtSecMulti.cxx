                                     ...
                                     compatibilityGraph_t& compatibilityGraph) const
{
  ...
  if (m_fillHist) {
    ...
    h.m_hb_ntrkjet->Fill((double)nTracks, evtWgt);
  }
...
//- Try to merge all vertices with common tracks
    ...
    if(m_fillHist){
      int cvgood=0; for(int iv=0; iv<(int)(*wrkVrtSet).size(); iv++) if((*wrkVrtSet)[iv].Good)cvgood++;
      Hists& h = getHists();
      h.m_hb_rawVrtN->Fill( (float)cvgood, evtWgt);
    }
...
//
    ...
    for(int iv=0; iv<(int)goodVertices.size(); iv++) {
          ...
          if( m_fillHist ){
            Hists& h = getHists();
            if(nth==1)h.m_hb_r1dc->Fill( goodVertices[iv].vertex.perp(), evtWgt);
            if(nth==2)h.m_hb_r2dc->Fill( goodVertices[iv].vertex.perp(), evtWgt);
            if(nth==3)h.m_hb_r3dc->Fill( goodVertices[iv].vertex.perp(), evtWgt);
            if(nth> 3)h.m_hb_rNdc->Fill( goodVertices[iv].vertex.perp(), evtWgt);
            Signif3D=vrtVrtDist(primVrt, goodVertices[iv].vertex, goodVertices[iv].vertexCov, jetDir);
            if( nth==2 ){
              if(goodVertices[iv].vertexCharge==0){ h.m_hb_totmass2T1->Fill( goodVertices[iv].vertexMom.M(), evtWgt);}
              else                                { h.m_hb_totmass2T2->Fill( goodVertices[iv].vertexMom.M(), evtWgt);}
              h.m_hb_sig3D2tr->Fill( Signif3D , evtWgt);
              if(goodVertices[iv].vertexCharge==0)h.m_hb_totmassEE->Fill(massV0(goodVertices[iv].trkAtVrt,m_massE,m_massE),evtWgt);
            //} else if( nth==1){
            } else if( goodVertices[iv].vertexMom.M()>6000.){
              h.m_hb_sig3D1tr->Fill( Signif3D, evtWgt);
            } else {
              h.m_hb_sig3DNtr->Fill( Signif3D, evtWgt);
            }
          }
//
...
//==============================================
    ...
    if(m_fillHist){
      Hists& h = getHists();
      h.m_hb_goodvrtN->Fill( ngoodVertices+0.1, evtWgt);
