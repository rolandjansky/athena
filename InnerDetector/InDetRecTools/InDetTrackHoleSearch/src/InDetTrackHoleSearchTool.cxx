						   ...
						   std::map<const Identifier, std::pair<const Trk::TrackParameters*,const bool> >& mapOfPredictions) const
{
  ...
  */
  ...
  int imeas = 0;
  const Trk::TrackParameters* firstsipar=0;
  ...
  for (  DataVector<const Trk::TrackStateOnSurface>::const_iterator iterTSOS = track.trackStateOnSurfaces()->begin();
	  ...
	  (*iterTSOS)->type(Trk::TrackStateOnSurface::Outlier))
	{
	  Identifier id ;
	  bool hasID = false;
	  if ( (*iterTSOS)->measurementOnTrack() !=0
	       && (*iterTSOS)->measurementOnTrack()->associatedSurface().associatedDetectorElement() != 0
  ...
  if (m_cosmic) {
    while (iterTSOS!=track.trackStateOnSurfaces()->end()
	   && (!(*iterTSOS)->type(Trk::TrackStateOnSurface::Measurement)
	       || !dynamic_cast<const Trk::PlaneSurface*>(&(*iterTSOS)->measurementOnTrack()->associatedSurface()))) iterTSOS++; 
  }
  ...
  int measno=0,nmeas=(int)track.measurementsOnTrack()->size();
	  ...
	  ((*iterTSOS)->type(Trk::TrackStateOnSurface::Perigee) && m_cosmic))
	{
	  ...
	  if (    (*iterTSOS)->measurementOnTrack() !=0
	    ...
	    { 
          ...
          else if (m_cosmic && per) surf=&((*iterTSOS)->trackParameters()->associatedSurface());
	  
	  // see if this is an Si state !
	  if ((m_cosmic && per) || (hasID && ( m_atlasId->is_pixel(id) || m_atlasId->is_sct(id) || m_atlasId->is_trt(id))) )
	    {
	      
	      if ( m_atlasId->is_trt(id) )
		ATH_MSG_VERBOSE ("Target is TRT, see if we can add something");
		   ...
		   it != paramList->end(); ++it)
		{
		  ...
		  const Trk::TrackParameters* thisParameters = *it;
                  ATH_MSG_VERBOSE ("extrapolated pos: " << thisParameters->position() 
				   << "   r: " << sqrt(pow(thisParameters->position().x(),2)
  ...
  if (!m_atlasId->is_trt(id) && (m_countDeadModulesAfterLastHit || m_extendedListOfHoles || m_cosmic))
	...
	if (paramList)
      ...
      // gotta clean up ...
      delete boundaryVol; boundaryVol = 0;
								    ...
								    std::vector<const Trk::TrackStateOnSurface*>* listOfHoles) const
{
  ...
  for (DataVector<const Trk::TrackStateOnSurface>::const_iterator it = oldTrack.trackStateOnSurfaces()->begin();
       it != oldTrack.trackStateOnSurfaces()->end(); ++it)
    {
      ...
      if ( !(*it)->type(Trk::TrackStateOnSurface::Hole) )
        trackTSOS->push_back(new Trk::TrackStateOnSurface(**it));
    }
  ...
  if (oldTrack.trackStateOnSurfaces()->size() == trackTSOS->size() && listOfHoles->size() == 0) {
    ...
    const Trk::Track* newTrack = new Trk::Track( oldTrack.info(),trackTSOS,oldTrack.fitQuality() ? oldTrack.fitQuality()->clone() : 0);
    return newTrack;
  }
  ...
  if (perigee) {
    ...
    else trackTSOS->sort( *CompFunc ); // respects DV object ownership
