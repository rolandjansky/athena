...
///////////////////////////////////////////////////////////////////
//   Implementation file for class CompetingStrip_ClustersOnTrackTool
///////////////////////////////////////////////////////////////////
...
// Header
#include "InDetCompetingRIOsOnTrackTool/CompetingStrip_ClustersOnTrackTool.h"
...
InDet::CompetingStrip_ClustersOnTrackTool::CompetingStrip_ClustersOnTrackTool(
    const std::string& ty,
    const std::string& na,
    const IInterface* pa ) :
        AthAlgTool(ty,na,pa),
        m_Strip__ROTCreator("InDet::Strip_ClusterOnTrackTool/Strip_ClusterOnTrackTool"),
        m_WeightCalculator("Trk::DAF_SimpleWeightCalculator/DAF_WeightCalculator"),
        m_extrapolator("Trk::Extrapolator/AtlasExtrapolator")
{
    declareInterface<ICompetingStrip_ClustersOnTrackCreator>(this);
    declareInterface<ICompetingRIOsOnTrackTool>(this);
    declareProperty("ToolForStrip_ClusterOnTrackCreation",    m_Strip__ROTCreator,          "Strip_ClusterOnTrackCreator needed for the creation of CompetingStrip_ClustersOnTrack");
...
StatusCode InDet::CompetingStrip_ClustersOnTrackTool::initialize() {
    StatusCode sc = AlgTool::initialize();

    ATH_MSG_INFO("Strip ROTCreation by   : " << m_Strip__ROTCreator.name() );
    ...
    // Get the correction tool to create Strip_Clusters on Track
    sc = m_Strip__ROTCreator.retrieve();
    if (sc.isFailure()) {
        ATH_MSG_FATAL( "Could not retrieve the Strip ROT creator: "<< m_Strip__ROTCreator );
        return sc;
    }
...
StatusCode InDet::CompetingStrip_ClustersOnTrackTool::finalize() {
    StatusCode sc = AlgTool::finalize();
    return sc;
}

///////////////////////////////////////////////////////////////////
// InDet::CompetingStrip_ClustersOnTrack production
///////////////////////////////////////////////////////////////////
const InDet::CompetingStrip_ClustersOnTrack* InDet::CompetingStrip_ClustersOnTrackTool::createCompetingROT(
    ...
    // vector of ROTs
    std::vector< const InDet::Strip_ClusterOnTrack* >* ROTvector = new std::vector<const InDet::Strip_ClusterOnTrack*>;
    ...
    //const InDet::Strip_ClusterOnTrack* ROTwithMaximumAssgnProb = 0;
    ...
    for (; rioIter!=RIO_List.end(); ++rioIter) {
        ...
        const InDet::Strip_Cluster* riopointer = dynamic_cast<const InDet::Strip_Cluster*>(*rioIter);
        if (!riopointer) {
            //ATH_MSG_WARNING("That's mean: given list of PrepRawData* contains a NULL pointer resp. not a Strip_Cluster!");
            ATH_MSG_WARNING("Given list of PrepRawData* contains a non Strip_Cluster!");
        ...
        if (riopointer->detectorElement()->surface() != (*detElementSurface)) {
            ATH_MSG_WARNING("Given Strip_ PrepRawData do not belong to the same detector element:");
        ...
        const InDet::Strip_ClusterOnTrack* rot = dynamic_cast<const InDet::Strip_ClusterOnTrack*>( m_Strip__ROTCreator->correct(*riopointer, *trkParAtRIOsurface));
        if (!rot) {
            ATH_MSG_WARNING("Strip_ClusterOnTrack could not be created");
        } else {
...
//             }
    ...
    if (ROTvector->size() <= 0) {
        ATH_MSG_ERROR("No valid Strip_ClusterOnTrack could be created:");
        ATH_MSG_ERROR("CompetingStrip_ClustersOnTrack creation aborted!");
        //clean-up
        delete ROTvector;
        delete assgnProbVector;
        return 0;
    }
    ATH_MSG_DEBUG("List of competing Strip_ ROTs contains "<< ROTvector->size() << " Strip_ClustersOnTrack");
    ...
    std::vector< const InDet::Strip_ClusterOnTrack*>::const_iterator rotIter = ROTvector->begin();
    ...
    // ---------------------------------------
    // create CompetingStrip_ClustersOnTrack
    //return (new CompetingStrip_ClustersOnTrack(assocSurface, ROTvector, assgnProbVector, effectiveLocalPar, effectiveErrMat, ROTsHaveCommonSurface));
    CompetingStrip_ClustersOnTrack* theCompetingROT = new CompetingStrip_ClustersOnTrack(ROTvector, assgnProbVector);
...
///////////////////////////////////////////////////////////////////
// InDet::CompetingStrip_ClustersOnTrack update
///////////////////////////////////////////////////////////////////
void InDet::CompetingStrip_ClustersOnTrackTool::updateCompetingROT(
    //const InDet::CompetingStrip_ClustersOnTrack& compROT,
    ...
    // cast baseCompROT to CompStrip_ClusterOnTrack:
    const InDet::CompetingStrip_ClustersOnTrack* compROT = dynamic_cast< const InDet::CompetingStrip_ClustersOnTrack* >(&baseCompROT);
    if (!compROT) {
        ATH_MSG_WARNING("Given CompetingRIOsOnTrack is not a CompetingStrip_ClustersOnTrack!");
        ATH_MSG_WARNING("Update of assignment probabilities aborted!!!");
        return;
    }
    // new vector of assignmentProbs
    std::vector< InDet::CompetingStrip_ClustersOnTrack::AssignmentProb >* assgnProbVector = new std::vector< InDet::CompetingStrip_ClustersOnTrack::AssignmentProb >;
    ...
    if (compROT->associatedSurface()==((trkPar.associatedSurface())) ) {
        ...
        if (!newTrackParameters){
            ...
            ATH_MSG_ERROR("    CompetingStrip_ClustersOnTrack could not be updated!");
            delete assgnProbVector;
            return;
        } // end if (extrapolation failed)
        // const Trk::AtaStraightLine* trkParAtRIOsurface1 = new Trk::AtaStraightLine(trkPar.position(), trkPar.momentum(), trkPar.charge(), *RIOsurfacePointer);
    ...
    if (maximumAssignProb > 0. ) {
        ...
        // copy ROTvector to base class vector (vector of RIO_OnTrack) because vector<> does not know inheritance structure
        std::vector< const Trk::RIO_OnTrack* >* baseROTvector = new std::vector<const Trk::RIO_OnTrack*>;
        std::vector< const InDet::Strip_ClusterOnTrack* >::const_iterator rotIter = compROT->containedROTs().begin();
    ...
    const_cast< InDet::CompetingStrip_ClustersOnTrack* >(compROT)->setLocalParametersAndErrorMatrix();
    if (msgLvl(MSG::VERBOSE)) testCompetingROT(*compROT);
}

void InDet::CompetingStrip_ClustersOnTrackTool::reequipCompetingROT
 ...
 const Trk::TrackState::MeasurementType ) const
{
  ...
  InDet::CompetingStrip_ClustersOnTrack* cst
    = dynamic_cast<InDet::CompetingStrip_ClustersOnTrack*>(modifiableCompROT);
  const InDet::Strip_ClusterOnTrack* newCluster 
    = dynamic_cast<const InDet::Strip_ClusterOnTrack*>(newROT);
  if (!cst || !newCluster) {
    ATH_MSG_WARNING( "inconsistent use of reequipCompetingROT() " );
    return;
  }
  std::vector<const InDet::Strip_ClusterOnTrack*>::const_iterator
    rotIter = cst->m_containedChildRots->begin();
  for (; rotIter!=cst->m_containedChildRots->end(); ++rotIter)
    delete (*rotIter);
  delete cst->m_containedChildRots;
  cst->m_containedChildRots = new std::vector<const InDet::Strip_ClusterOnTrack*>;
...
void InDet::CompetingStrip_ClustersOnTrackTool::testCompetingROT(const InDet::CompetingStrip_ClustersOnTrack& compROT)const {
    ...
    /*    msg()<<"  - clone()"<<endreq;
        CompetingStrip_ClustersOnTrack* newcompROT = compROT.clone();
        if (!newcompROT){
...
StatusCode InDet::CompetingStrip_ClustersOnTrackTool::updateCompetingROTprobs( 
        const Trk::CompetingRIOsOnTrack& baseCompROT, 
