    ...
    for(int ib(Nt); ib!=Nb; ++ib) {
      ...
      for(int it(it0);  it!=Nt; ++it) {
        ...
        if(Im <= m_diver) {
          ...
          if (m_writeNtuple) {
            m_SP[t]->setParam(Im);
            float meanOneOverTanTheta = std::abs(Tzb*m_Tn[it].Fl);
            ...
            float theta = atan(1./meanOneOverTanTheta);            
            m_SP[t]->setEta(-log(tan(0.5*theta)));
