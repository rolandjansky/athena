...
class HSvcRoute : public SvcRoute {
 ...
 HSvcRoute() : SvcRoute() {}
 HSvcRoute( double r, double zmi, double zma, double zexit, ServiceDynVolume::Routing routing, const std::string& name) :
  SvcRoute(routing, name), m_radius(r), m_zMin(zmi), m_zMax(zma), m_zExit(zexit), m_next(0) {}
