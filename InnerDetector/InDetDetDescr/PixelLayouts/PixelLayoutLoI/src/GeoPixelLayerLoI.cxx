...
GeoPixelLayerLoI::GeoPixelLayerLoI(const PixelGeoBuilderBasics* basics, int iLayer):
...
GeoVPhysVol* GeoPixelLayerLoI::Build() {
  ...
  std::cout<<layerName<<" "<<staveType<<"  #sectors "<<nSectors<<"  radius "<<layerRadius<<std::endl;
  std::cout<<"*****************************************************************************"<<std::endl;
  ...
  std::vector<InDet::StaveTmp *>staveTmp = m_xmlReader->getPixelStaveTemplate(m_layer);
  GeoPixelLadderLoI pixelLadder(getBasics(), staveTmp[0], m_layer, transRadiusAndTilt); 
  ...
  for(int ii = 0; ii < nSectors; ii++) {
    ...
    if(ii==0){  // buil teh layer envelope inly once the first ladder envelope is defined
...
void GeoPixelLayerLoI::ComputeLayerThickness(const GeoPixelLadderLoI& pixelLadder, double ladderTilt, double layerRadius)
{
