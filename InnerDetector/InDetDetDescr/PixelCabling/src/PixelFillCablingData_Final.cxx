...
{
  StatusCode sc;
  ATH_MSG_DEBUG("PixelFillCablingData_Final::initialize");
...
PixelCablingData* PixelFillCablingData_Final::FillMaps()
{
  ...
  if (file_name.size()==0) {
    ...
    ATH_MSG_INFO("Mapping File: " << m_final_mapping_file);   }
...
//            uint32_t rodid = sid_rob.code();
            ...
            if (hashId > m_idHelper->wafer_hash_max()) {
                ...
                if (eta_module > m_idHelper->eta_module_max(offlineId) || eta_module < m_idHelper->eta_module_min(offlineId)) {
                    ...
                    ATH_MSG_ERROR("Input geometry tag may not be compatible with mapping file " << file_name);
                }
    ...
    ATH_MSG_INFO("map size " << rodReadoutMap.size());
