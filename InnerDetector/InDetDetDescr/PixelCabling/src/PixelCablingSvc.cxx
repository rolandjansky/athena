...
StatusCode PixelCablingSvc::initialize() {
  ...
  if (!m_useIBLParameterSvc) {
    ATH_MSG_INFO("IBLParameterSvc not used");
  }
  else {
  ...
  // Get the values
  m_IBLParameterSvc->setCablingParameters(m_layer_columnsPerFE,m_layer_rowsPerFE,m_layer_FEsPerHalfModule,&m_dbm_columnsPerFE,&m_dbm_rowsPerFE,&m_dbm_FEsPerHalfModule);
  ...
  ATH_MSG_DEBUG("m_IBLpresent = " << m_IBLpresent);
  ATH_MSG_DEBUG("m_DBMpresent = " << m_DBMpresent);
  ATH_MSG_DEBUG("m_isHybrid = " << m_isHybrid);
  ATH_MSG_DEBUG("m_layer_columnsPerFE = " << m_layer_columnsPerFE);
  ATH_MSG_DEBUG("m_layer_rowsPerFE = " << m_layer_rowsPerFE);
  ATH_MSG_DEBUG("m_layer_FEsPerHalfModule = " << m_layer_FEsPerHalfModule);
  ATH_MSG_DEBUG("m_disk_columnsPerFE = " << m_disk_columnsPerFE);
  ATH_MSG_DEBUG("m_disk_rowsPerFE = " << m_disk_rowsPerFE);
  ATH_MSG_DEBUG("m_disk_FEsPerHalfModule = " << m_disk_FEsPerHalfModule);
  ATH_MSG_DEBUG("m_dbm_columnsPerFE = " << m_dbm_columnsPerFE);
  ATH_MSG_DEBUG("m_dbm_rowsPerFE = " << m_dbm_rowsPerFE);
  ATH_MSG_DEBUG("m_dbm_FEsPerHalfModule = " << m_dbm_FEsPerHalfModule);
...
Identifier PixelCablingSvc::getPixelId(Identifier offlineId, uint32_t FE, uint32_t row, uint32_t column) {
  ...
  switch (thisModule) {
    case NONE:
      ATH_MSG_ERROR("Module type not identified -- cannot build pixelId");
      return Identifier();
    ...
    case IBL:
      ...
      if (eta_module>=m_layer_FEsPerHalfModule[0].size()) {
        ATH_MSG_ERROR("getPixelId: Eta_module out of range (eta_module = " << eta_module << ", expected number of modules per stave = " << m_layer_FEsPerHalfModule[0].size() << ")");
        return Identifier();
      }
      // fall through
  ...
  if (eta_index>eta_index_max) {
    ATH_MSG_ERROR("Error! eta_index: " << eta_index << " > eta_index_max: " << eta_index_max);
  }
  if (phi_index>phi_index_max) {
    ATH_MSG_ERROR("Error! phi_index: " << phi_index << " > phi_index_max: " << phi_index_max);
  }
...
uint32_t PixelCablingSvc::getFE(Identifier *pixelId, Identifier offlineId) {
  ...
  switch (thisModule) {
    ...
    case IBL:
      ...
      if (eta_module>=m_layer_FEsPerHalfModule[0].size()) {
        ATH_MSG_ERROR("getFE: Eta_module out of range (eta_module = " << eta_module << ", expected number of modules per stave = " << m_layer_FEsPerHalfModule[0].size() << ")");
        return 0xffffffff;
      }
      // fall through
...
uint32_t PixelCablingSvc::getColumn(Identifier *pixelId, Identifier offlineId) {
  ...
  switch (thisModule) {
    ...
    case IBL:
      ...
      eta_module = m_idHelper->eta_module(offlineId)+m_eta_module_offset; //offset by 10 to start counting from 0
      if (eta_module >= m_layer_FEsPerHalfModule[0].size()) {
        ATH_MSG_ERROR("getColumn: Eta_module out of range (eta_module = " << eta_module << ", expected number of modules per stave = " << m_layer_FEsPerHalfModule[0].size() << ")");
        return 0xffffffff;
      }
      // fall through
...
uint32_t PixelCablingSvc::getRow(Identifier *pixelId, Identifier offlineId) {
  ...
  switch (thisModule) {
    ...
    case IBL:
      ...
      eta_module = m_idHelper->eta_module(offlineId)+m_eta_module_offset; //offset by 10 to start counting from 0
      if (eta_module >= m_layer_FEsPerHalfModule[0].size()) {
        ATH_MSG_ERROR("getRow: Eta_module out of range (eta_module = " << eta_module << ", expected number of modules per stave = " << m_layer_FEsPerHalfModule[0].size() << ")");
        return 0xffffffff;
      }
      // fall through
...
bool PixelCablingSvc::isIBL(const Identifier& id) {
  ...
  if (m_IBLpresent && m_idHelper->barrel_ec(id)==0 && m_idHelper->layer_disk(id)==0) {
    return true;
  }
  else {
    return false;
