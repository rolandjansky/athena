...
void SCT_ID::wafer_id_checks(int barrel_ec,
                             int layer_disk,
                             int phi_module,
                             int eta_module,
                             int side) const {
...
void SCT_ID::strip_id_checks(int barrel_ec,
                             int layer_disk,
                             int phi_module,
                             int eta_module,
                             int side,
...
int SCT_ID::layer_disk_max(const Identifier &id) const {
...
int SCT_ID::eta_module_min(const Identifier &id) const {
...
int SCT_ID::eta_module_max(const Identifier &id) const {
...
int SCT_ID::strip_max(const Identifier &id) const {
    // get max from dictionary - note that eta modules skip 0 for
    // sct, so we must search for absolute max
    ExpandedIdentifier expId;
    IdContext layer_context(expId, 0, m_LAYER_DISK_INDEX);
...
bool SCT_ID::is_eta_module_min(const Identifier &id) const {
    // get min from dictionary
    return(eta_module(id) == eta_module_min(id));
}

bool SCT_ID::is_eta_module_max(const Identifier &id)  const {
    // get max from dictionary
    return(eta_module(id) == eta_module_max(id));
}

int SCT_ID::phi_module_max(const Identifier &id) const {
    // get max from dictionary
    ExpandedIdentifier expId;
    IdContext wafer_context1 = wafer_context();
...
bool SCT_ID::is_phi_module_max(const Identifier &id) const {
    // get max from dictionary
    return(phi_module(id) == phi_module_max(id));
}

int SCT_ID::initialize_from_dictionary(const IdDictMgr &dict_mgr) {
    MsgStream log(m_msgSvc, "SCT_ID");
    ...
    if (!m_dict) {
        log << MSG::ERROR << " SCT_ID::initialize_from_dict - cannot access InnerDetector dictionary " << endreq;
        return 1;
    }
    ...
    if (m_msgSvc) {
        log << MSG::INFO << " SCT_ID::initialize_from_dict " << endreq;
        log << MSG::DEBUG
...
int SCT_ID::get_prev_in_phi(const IdentifierHash &id, IdentifierHash &prev) const {
    unsigned short index = id;

    if (index < m_prev_phi_wafer_vec.size()) {
        if (m_prev_phi_wafer_vec[index] == NOT_VALID_HASH) {
            return(1);
        }
...
int SCT_ID::get_next_in_phi(const IdentifierHash &id, IdentifierHash &next) const {
    unsigned short index = id;

    if (index < m_next_phi_wafer_vec.size()) {
        if (m_next_phi_wafer_vec[index] == NOT_VALID_HASH) {
            return(1);
        }
...
int SCT_ID::get_prev_in_eta(const IdentifierHash &id, IdentifierHash &prev) const {
    unsigned short index = id;

    if (index < m_prev_eta_wafer_vec.size()) {
        if (m_prev_eta_wafer_vec[index] == NOT_VALID_HASH) {
            return(1);
        }
...
int SCT_ID::get_next_in_eta(const IdentifierHash &id, IdentifierHash &next) const {
    unsigned short index = id;

    if (index < m_next_eta_wafer_vec.size()) {
        if (m_next_eta_wafer_vec[index] == NOT_VALID_HASH) {
            return(1);
        }
...
int SCT_ID::get_other_side(const IdentifierHash &hashId, IdentifierHash &other) const {
    if (m_dict) {
        // get max from dictionary
        Identifier id;
        IdContext wafer_context1 = wafer_context();
...
int SCT_ID::init_neighbors(void) {
    //
    // create a vector(s) to retrieve the hashes for compact ids for
    // wafer neighbors.
    //
    ...
    if (m_msgSvc) {
        log << MSG::DEBUG << "SCT_ID::init_neighbors " << endreq;
    }
    else {
        ...
        for (; first != last; ++first) {
            ...
            if (nphi) {
                ...
                if (get_hash(id, hash_id, &wcontext)) {
                    ...
                    id.getString() <<
                        " " << MSG::hex << id.getString() << MSG::dec << endreq;
                    return(1);
                }
            ...
            if (neta) {
                ...
                Identifier id = wafer_id(expId[m_BARREL_EC_INDEX],
                                         ...
                                         expId[m_SIDE_INDEX]);
                if (get_hash(id, hash_id, &wcontext)) {
                    log << MSG::ERROR << " SCT_ID::init_neighbors - unable to get next eta hash, exp/compact " <<
                    id.getString()
                        << " " << endreq;
                    return(1);
                }
...
int SCT_ID::initLevelsFromDict() {
    ...
    if (!m_dict) {
        log << MSG::ERROR << " SCT_ID::initLevelsFromDict - dictionary NOT initialized " << endreq;
        return(1);
    }
    ...
    if (field) {
        ...
        log << MSG::ERROR << "SCT_ID::initLevelsFromDict - unable to find 'subdet' field " << endreq;
        return(1);
    }
    ...
    if (field) {
        ...
        log << MSG::ERROR << "SCT_ID::initLevelsFromDict - unable to find 'part' field " << endreq;
        return(1);
    }
    ...
    if (field) {
        ...
        log << MSG::ERROR << "SCT_ID::initLevelsFromDict - unable to find 'barrel_endcap' field " << endreq;
        return(1);
    }
    ...
    if (field) {
        ...
        log << MSG::ERROR << "SCT_ID::initLevelsFromDict - unable to find 'layer' field " << endreq;
        return(1);
    }
    ...
    if (field) {
        ...
        log << MSG::ERROR << "SCT_ID::initLevelsFromDict - unable to find 'phi_module' field " << endreq;
        return(1);
    }
    ...
    if (field) {
        ...
        log << MSG::ERROR << "SCT_ID::initLevelsFromDict - unable to find 'eta_module' field " << endreq;
        return(1);
    }
    ...
    if (field) {
        ...
        log << MSG::ERROR << "SCT_ID::initLevelsFromDict - unable to find 'side' field " << endreq;
        return(1);
    }
    ...
    if (field) {
        ...
        // log << MSG::ERROR << "SCT_ID::initLevelsFromDict - unable to find 'row' field " << endreq;
        // return (1);
    }
    ...
    if (field) {
        ...
        log << MSG::ERROR << "SCT_ID::initLevelsFromDict - unable to find 'strip' field " << endreq;
        return(1);
    }
    ...
    if (m_msgSvc) {
        ...
        log << MSG::DEBUG << "sct      " << m_sct_impl.show_to_string() << endreq;
        ...
        if (m_hasRows) {
            log << MSG::DEBUG << "row     " << m_row_impl.show_to_string() << endreq;
        }
        log << MSG::DEBUG << "strip    " << m_strip_impl.show_to_string() << endreq;
    }
    else {
...
SCT_ID::size_type SCT_ID::wafer_hash_max(void) const {
    return m_wafer_hash_max;
}

SCT_ID::size_type SCT_ID::strip_hash_max(void) const {
...
int SCT_ID::get_id(const IdentifierHash &hash_id,
                   Identifier &id,
...
void SCT_ID::get_expanded_id(const Identifier &id,
...
int SCT_ID::get_hash(const Identifier &id,
                     IdentifierHash &hash_id,
                     const IdContext *context) const {
...
void SCT_ID::test_wafer_packing(void) const {
    MsgStream log(m_msgSvc, "SCT_ID");
    ...
    if (m_dict) {
        ...
        for (; first != last; ++first, ++nids) {
            ...
            if (!exp_id[m_SIDE_INDEX]) {
                ...
                if (id != new_id1) {
                    log << MSG::ERROR <<
                    "SCT_ID::test_wafer_packing: new and old module idsnot equal. New/old/expanded ids "
