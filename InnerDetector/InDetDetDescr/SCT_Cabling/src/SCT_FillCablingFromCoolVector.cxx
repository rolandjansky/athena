 ...
 *
 *   @brief Fills an SCT cabling object from the database using Cool Vector Payload
 *
 *   @author Shaun Roe
 *   @date 25/08/2014
 */
...
// Constructor
SCT_FillCablingFromCoolVector::SCT_FillCablingFromCoolVector( const std::string & name, ISvcLocator * pSvcLocator):
...
//
StatusCode
SCT_FillCablingFromCoolVector::initialize(){
   return StatusCode::SUCCESS;
}

//
StatusCode 
SCT_FillCablingFromCoolVector::finalize(){
  return StatusCode::SUCCESS;
}

//
StatusCode 
SCT_FillCablingFromCoolVector::queryInterface(const InterfaceID & riid, void** ppvInterface ){
...
//
StatusCode 
SCT_FillCablingFromCoolVector::setDataSource(const std::string & dataSource){
...
//
std::string 
SCT_FillCablingFromCoolVector::getDataSource() const {
  return m_source;
}

//
bool 
SCT_FillCablingFromCoolVector::filled() const{
  return m_filled;
}

//
StatusCode 
SCT_FillCablingFromCoolVector::fillMaps(SCT_CablingSvc * cabling){
  m_filled=false;
...
//
StatusCode
SCT_FillCablingFromCoolVector::readDataFromDb(SCT_CablingSvc * cabling){
  ...
  IntMap slotMap;
  //there are 12 crate slots, but they may be numbered (in the DB) as 0-11 or as some non-monotonic sequence
  //so here we take whatever is in the database and map it onto numbers 0-11
  {//scope this calculation, only slotMap should emerge
    //map slots onto 0-11
    typedef std::set<int> S;
    S slots;
    for (; rodIt != last_rod; ++rodIt) {
      slots.insert(int(rodIt->second["slot"].data<short>())); //all but 12 inserts will fail, and these 12 should be sorted
    }
    int counter(0);
  ...
  for (; rodIt != last_rod; ++rodIt) {
    ...
    //see note in header; these may be 0-11, but not necessarily, so we need to map these onto 0-11
...
bool  SCT_FillCablingFromCoolVector::successfulFolderRetrieve(const DataHandle<CondAttrListVec> &pDataVec, const std::string & folderName){
