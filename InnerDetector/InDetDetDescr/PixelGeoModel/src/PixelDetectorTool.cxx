    ...
    m_athenaComps(0)
{
  ...
  declareProperty("ReadXMLFromDB",m_bReadXMLFromDB);
}
/**
 ** Destructor
 **/
...
{ 
  ...
  if (pixelVersionTag.empty()) { 
   ...
   // BLM Tool.
	...
	// Fast-build GeoModel
	PixelDetectorFactoryFastGeo thePixel(m_athenaComps, switches,m_ConfigGeoBase,m_bConfigGeoAlgTool,m_bReadXMLFromDB);
	thePixel.create(world);     
	m_manager  = thePixel.getDetectorManager();
      } else {
	// DC3, SLHC, IBL
