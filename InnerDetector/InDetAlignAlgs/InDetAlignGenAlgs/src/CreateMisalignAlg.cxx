...
namespace InDetAlignment
{
  ...
  void CreateMisalignAlg::setupSCT_AlignModule(int& nSCT)
  {
    ...
    for (iter = m_SCT_Manager->getDetectorElementBegin(); iter != m_SCT_Manager->getDetectorElementEnd(); ++iter) {
      ...
      // add both det elements IDs to list of IDs not only the module ID if needed
      // this is checked via the InDetAlignDBTool
      // otherwise you convert from wafer id to module id
      const Identifier SCT_ModuleID = m_IDAlignDBTool->useSCTtwoSide() ? 
                  (*iter)->identify() : m_sctIdHelper->module_id((*iter)->identify()); 
      ...
      // dump both sides if needed, otherwise only the inner side case
      if (m_IDAlignDBTool->useSCTtwoSide() or m_sctIdHelper->side((*iter)->identify()) == 0) { 
        ...
        std::cout << "Adding alignable transform for element " << m_sctIdHelper->barrel_ec(SCT_ModuleID) 
          << "/" << m_sctIdHelper->layer_disk(SCT_ModuleID) << "/" << m_sctIdHelper->eta_module(SCT_ModuleID) 
          << "/" << m_sctIdHelper->phi_module(SCT_ModuleID) << "/" << m_sctIdHelper->side(SCT_ModuleID) << std::endl;
  ...
  StatusCode  CreateMisalignAlg::GenerateMisaligment()
  {
    ...
    for (std::map<Identifier, HepGeom::Point3D<double> >::const_iterator iter = m_ModuleList.begin(); iter != m_ModuleList.end(); ++iter) {
      ...
      else if (m_MisalignmentMode==99) {
        //shift whole detector                                                                                                        
        double deltaZ=0*CLHEP::micrometer;
        if ( m_idHelper->is_pixel(ModuleID) ) {
          deltaZ=100*CLHEP::micrometer;
          if (msgLvl(MSG::DEBUG)) msg() << m_pixelIdHelper->barrel_ec(ModuleID) << "/" <<
            m_pixelIdHelper->layer_disk(ModuleID) << "/" << m_pixelIdHelper->eta_module(ModuleID) << "/" << 
            m_pixelIdHelper->phi_module(ModuleID) << " ===> " <<
            "deltaZ for this PIXEL module: "  << deltaZ/CLHEP::micrometer << " um" << endreq;
        } else if  ( m_idHelper->is_sct(ModuleID)) {
          deltaZ=5000*CLHEP::micrometer;
          if (msgLvl(MSG::DEBUG)) msg() << m_sctIdHelper->barrel_ec(ModuleID) << "/" <<
            m_sctIdHelper->layer_disk(ModuleID) << "/" << m_sctIdHelper->eta_module(ModuleID) << "/" << 
            m_sctIdHelper->phi_module(ModuleID) << "/" << m_sctIdHelper->side(ModuleID) << " ===> " <<
            "deltaZ for this STRIP module: "  << deltaZ/CLHEP::micrometer << " um" << endreq;
        }
