    ...
    InDet::InDetEventCnvTool::getLinks( const Trk::RIO_OnTrack& rioOnTrack    )
{
  ...
  //  if (m_pixMgr->getIdHelper()->is_pixel(id) ) 
     if (m_pixelIDHelper->is_pixel(id) ) 
 ...
 //       if (m_sctMgr->getIdHelper()->is_sct(id) ) 
 
       if (m_sctIDHelper->is_sct(id) ) 
        {
 ...
 //           if (m_trtMgr->getIdHelper()->is_trt(id) ) 
	 //	 if (m_trtIDHelper->is_trt(id) ) 
	 if (m_sctIDHelper->is_trt(id) ) 
            {
                ...
                ATH_MSG_WARNING("Unknown type of ID detector from identifier :"
                    << id<<", in string form:" 
                    << m_trtIDHelper->show_to_string(id) 
                    ); 
            }
        }
    }
