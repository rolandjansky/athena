...
void TRT_HitCollectionCnv_p5::transToPers(const TRTUncompressedHitCollection* transCont, TRT_HitCollection_p5* persCont, MsgStream &log)
{
  ...
  /*
    ...
    In the p5 version, HepMcParticleLink_p3 uses an unsigned int rather than unsigned short for the event number
  ...
  */
  ...
  //    if (log.level() <= MSG::DEBUG) log << MSG::DEBUG << "In TRT_HitCollectionCnv_p5::transToPers()" << endreq;
...
void TRT_HitCollectionCnv_p5::persToTrans(const TRT_HitCollection_p5* persCont, TRTUncompressedHitCollection* transCont, MsgStream& /*log*/)
{
  ...
  //    if (log.level() <= MSG::DEBUG) log << MSG::DEBUG << "In TRT_HitCollectionCnv_p5::persToTrans()" << endreq;
