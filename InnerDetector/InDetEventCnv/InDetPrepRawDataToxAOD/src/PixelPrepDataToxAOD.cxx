                                           ...
                                           const unsigned int sizeX, const unsigned int sizeY ) const
{
  ...
  InDetDD::SiCellId cellIdWeightedPosition= getCellIdWeightedPosition( pixelCluster, &rowMin, &rowMax, &colMin, &colMax);
  ...
  {
    Identifier rId =  *rdosBegin;
    int absrow = m_PixelHelper->phi_index(rId)-rowWeightedPosition    + centralIndexX;
    int abscol = m_PixelHelper->eta_index(rId)-columnWeightedPosition + centralIndexY;
  
    ATH_MSG_VERBOSE( " Phi Index: " << m_PixelHelper->phi_index(rId) << " absrow: " << absrow << " eta Idx: " << m_PixelHelper->eta_index(rId) << " abscol: " << abscol << " charge " << *charge );
  
    if (absrow <0 || absrow >= (int)sizeX)
    {
      ATH_MSG_DEBUG(" problem with index: " << absrow << " min: " << 0 << " max: " << sizeX);
      continue;
    }
    
    if (abscol <0 || abscol >= (int)sizeY)
    {
    ...
    if ( totList.size() !=0 && tot    != totList.end()) matrixOfToT[absrow][abscol]   =*tot;
    else matrixOfToT[absrow][abscol]   = -1;

    if ( chList.size()  !=0 && charge != chList.end()) matrixOfCharge[absrow][abscol]=*charge;
    else matrixOfCharge[absrow][abscol]=-1;
  
    if (pitchY > 0.1)
    {
      vectorOfPitchesY[abscol]=pitchY;
    }
  }//end iteration on rdos
  ...
  xprd->auxdata< int >("NN_columnWeightedPosition") = columnWeightedPosition;
  xprd->auxdata< int >("NN_rowWeightedPosition") = rowWeightedPosition;

  xprd->auxdata< float >("NN_localColumnWeightedPosition") = localColumnWeightedPosition;
  xprd->auxdata< float >("NN_localRowWeightedPosition") = localRowWeightedPosition;
                                                                   ...
                                                                   int *rcolMax   ) const
{
  ...
  for (; rdosBegin!= rdosEnd; ++rdosBegin, ++charge)
  {
    ...
    Identifier rId =  *rdosBegin;
    int row = m_PixelHelper->phi_index(rId);
    int col = m_PixelHelper->eta_index(rId);
  
    ATH_MSG_VERBOSE(" Adding pixel row: " << row << " col: " << col << " charge: " << *charge );
  ...
  if(rrowMin) *rrowMin = rowMin;
  if(rrowMax) *rrowMax = rowMax;
  if(rcolMin) *rcolMin = colMin;
  if(rcolMax) *rcolMax = colMax;
