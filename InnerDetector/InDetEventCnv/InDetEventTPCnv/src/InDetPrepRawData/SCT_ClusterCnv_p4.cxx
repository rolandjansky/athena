...
#include "InDetEventTPCnv/InDetPrepRawData/SCT_ClusterCnv_p4.h"

SCT_ClusterCnv_p4::SCT_ClusterCnv_p4(const SCT_ID * sctid )
 :
  m_sctId2(sctid)
{}


void SCT_ClusterCnv_p4::
persToTrans( const InDet::SCT_Cluster_p4 *persObj, InDet::SCT_Cluster *transObj, MsgStream &log) 
{
...
// List of Id of the cluster
  ...
  for (std::vector<InDet::SCT_Cluster_p4::rdo_diff_type>::const_iterator it=persObj->m_rdoList.begin(); it != persObj->m_rdoList.end(); it++) {
...
void SCT_ClusterCnv_p4::transToPers( const InDet::SCT_Cluster *transObj, InDet::SCT_Cluster_p4 *persObj, MsgStream &log )
{
//   if (log.level() <= MSG::VERBOSE) log << MSG::VERBOSE << "In SCT_Cluster::transToPers" << endreq;
  ...
  persObj->m_rdoList.resize(transObj->m_rdoList.size() );
  std::vector<InDet::SCT_Cluster_p4::rdo_diff_type>::iterator pit = persObj->m_rdoList.begin(); 
  
  
  for (std::vector<Identifier>::const_iterator it=transObj->m_rdoList.begin(); it != transObj->m_rdoList.end(); it++) {

    *pit = static_cast<InDet::SCT_Cluster_p4::rdo_diff_type>( m_sctId2->calc_offset(id32, *it) );
    pit++;
  }
