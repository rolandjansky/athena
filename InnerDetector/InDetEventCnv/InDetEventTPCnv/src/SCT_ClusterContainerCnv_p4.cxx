...
void SCT_ClusterContainerCnv_p4::transToPers(const InDet::SCT_ClusterContainer* transCont, InDet::SCT_ClusterContainer_p4* persCont, MsgStream &log) {
	...
	log << MSG::FATAL << "Could not initialize SCT_ClusterContainerCnv_p4 " << endreq;
      }
    }

    SCT_ClusterCnv_p4  chanCnv(m_sctId);
	...
	idLast=collection.identifyHash();
        ...
        for (unsigned int i = 0; i < collection.size(); ++i) {
            InDet::SCT_Cluster_p4* pchan = &(persCont->m_rawdata[i + chanBegin]);
...
void  SCT_ClusterContainerCnv_p4::persToTrans(const InDet::SCT_ClusterContainer_p4* persCont, InDet::SCT_ClusterContainer* transCont, MsgStream &log) 
{
	...
	// IDet collection don't have the Ownership policy c'tor
        ...
        for (unsigned int ichan = 0; ichan < nchans; ++ ichan) {
            const InDet::SCT_Cluster_p4* pchan = &(persCont->m_rawdata[ichan + collBegin]);
...
//================================================================
InDet::SCT_ClusterContainer* SCT_ClusterContainerCnv_p4::createTransient(const InDet::SCT_ClusterContainer_p4* persObj, MsgStream& log) {
...
StatusCode SCT_ClusterContainerCnv_p4::initialize(MsgStream &log) {
