...
def createInDetConfigFlags():
  ...
  icf.addFlag("InDet.doPrintConfigurables",False) # if this is on the all the print(InDetXYZ lines are activated
  # FIXME: Algorithm property instead of flag
  icf.addFlag("InDet.doNewTracking",False) # Turn running of newTracking on and off
  # FIXME: Flags shouldn't turn on/off individual algorithms
  icf.addFlag("InDet.doPseudoTracking",True) # Turn running of the truth seeded pseudo tracking on and off
  ...
  icf.addFlag("InDet.doxKalman",True) # Turn running of xKalman on and off
  icf.addFlag("InDet.doiPatRec",False 	) # Turn running of iPatRec on and off
  icf.addFlag("InDet.preProcessing",False) # Turn running of pre processing on and off
  icf.addFlag("InDet.doPRDFormation",False) # Turn running of PRD formation on and off
  ...
  icf.addFlag("InDet.doRefit",True) # Turn running of refitting on and off
  icf.addFlag("InDet.doSLHC",True) # Turn running of SLHC reconstruction on and off
  ...
  icf.addFlag("InDet.postProcessing",False) # Turn running of post processing on and off
  icf.addFlag("InDet.doTruth",False) # Turn running of truth matching on and off
  icf.addFlag("InDet.loadTools",False) # Turn loading of tools on and off
  icf.addFlag("InDet.doBackTracking",False) # Turn running of backtracking on and off
  icf.addFlag("InDet.doLowPt",True) # Turn running of doLowPt second pass on and off
  icf.addFlag("InDet.doVeryLowPt",True) # Turn running of doVeryLowPt thrid pass on and off
  ...
  icf.addFlag("InDet.doCosmics",False) # Turn running of Cosmics on and off
  icf.addFlag("InDet.doHeavyIon",False) # Turn running of HeavyIons on and off
  icf.addFlag("InDet.doParticleCreation",False) # Turn running of particle creation on and off
  icf.addFlag("InDet.KeepParameters",True) # Keep extra parameters on slimmed tracks False to drop them
  icf.addFlag("InDet.KeepFirstParameters",True) # Keep the first set of track parameters in addition to the defining ones for TrackParticles. False to drop them
  ...
  icf.addFlag("InDet.loadTRTSeededSPFinder",False) # control which SP finder is used exclusive with loadSimpleTRTSeededSPFinder control which SP finder is used 
  ...
  icf.addFlag("InDet.doTRTStandalone",False) # control TRT Standalone 
  ...
  icf.addFlag("InDet.selectSCTIntimeHits",False) # defines if the X1X mode is used for the offline or not 
  icf.addFlag("InDet.cutSCTOccupancy",False )
  icf.addFlag("InDet.useDCS",True)
  icf.addFlag("InDet.truthMatchStrategy",True) # defines how truth matching is done. possible values TruthMatchRatio (old style) or TruthMatchTanimoto (new style)
  icf.addFlag("InDet.doFatras",True) # Switch for FATRAS running 
  icf.addFlag("InDet.doSGDeletion",'TruthMatchRatio') # Drop contianers from SG ones finished with them 
  icf.addFlag("InDet.doLowBetaFinder",False) # Switch for running Low-Beta finer 
  ...
  icf.addFlag("InDet.doxAOD",False ) # Switch for running AOD to xAOD conversion algs 
  icf.addFlag("InDet.doCaloSeededTRTSegments",False ) # Switch for running AOD to xAOD conversion algs 
  icf.addFlag("InDet.doInnerDetectorCommissioning",True) # Switch for running looser settings in ID for commissioning 
  icf.addFlag("InDet.doTIDE_Ambi",False) # Switch for running TIDE Ambi 
