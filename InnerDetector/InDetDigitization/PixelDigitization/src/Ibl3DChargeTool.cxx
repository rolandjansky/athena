...
StatusCode Ibl3DChargeTool::initialize() {
  ...
  ATH_MSG_INFO("You are using Ibl3DChargeTool, not Ibl3DChargeTool");

  if(m_doBichsel){
    ATH_MSG_INFO("Bichsel Digitization is turned ON in Ibl3DChargeTool!");
    CHECK(m_BichselSimTool.retrieve());
  }
  else {
    ATH_MSG_INFO("Bichsel Digitization is turned OFF in Ibl3DChargeTool!");
  }
...
// charge
//----------------------------------------------------------------------
StatusCode Ibl3DChargeTool::charge(const TimedHitPtr<SiHit> &phit, SiChargedDiodeCollection& chargedDiodes, const InDetDD::SiDetectorElement &Module) { 
