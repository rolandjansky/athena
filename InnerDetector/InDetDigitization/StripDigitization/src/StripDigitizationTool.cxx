...
StatusCode StripDigitizationTool::initDisabledCells() {
  // +++ Retrieve the StripRandomDisabledCellGenerator
  ATH_CHECK(m_sct_RandomDisabledCellGenerator.retrieve());

  storeTool(&(*m_sct_RandomDisabledCellGenerator));

  ATH_MSG_INFO("Retrieved the StripRandomDisabledCellGenerator tool:" << m_sct_RandomDisabledCellGenerator);
  return StatusCode::SUCCESS;
}

StatusCode StripDigitizationTool::processAllSubEvents(const EventContext& ctx) {
...
// prepareEvent
// ======================================================================
StatusCode StripDigitizationTool::prepareEvent(const EventContext& ctx, unsigned int /*index*/) {
  ATH_MSG_VERBOSE("StripDigitizationTool::prepareEvent()");
...
// mergeEvent
// =========================================================================
StatusCode StripDigitizationTool::mergeEvent(const EventContext& ctx) {
  ATH_MSG_VERBOSE("StripDigitizationTool::mergeEvent()");
...
void StripDigitizationTool::digitizeNonHits(const EventContext& ctx, SG::WriteHandle<SCT_RDO_Container>* rdoContainer, SG::WriteHandle<InDetSimDataCollection>* simDataCollMap, const std::vector<bool>* processedElements, CLHEP::HepRandomEngine * rndmEngine) const {
  // Get StripDetectorElementCollection
  SG::ReadCondHandle<InDetDD::SiDetectorElementCollection> stripDetEle(m_stripDetEleCollKey, ctx);
  const InDetDD::SiDetectorElementCollection* elements{stripDetEle.retrieve()};
  if (elements==nullptr) {
    ATH_MSG_FATAL(m_stripDetEleCollKey.fullKey() << " could not be retrieved");
    return;
  }
...
// -----------------------------------------------------------------------------
void StripDigitizationTool::applyProcessorTools(SiChargedDiodeCollection* chargedDiodes, CLHEP::HepRandomEngine * rndmEngine) const {
...
StatusCode StripDigitizationTool::processBunchXing(int bunchXing,
    ...
    ATH_MSG_VERBOSE("StripDigitizationTool::processBunchXing() " << bunchXing);
...
// property handlers
// =========================================================================
void StripDigitizationTool::SetupRdoOutputType(Gaudi::Details::PropertyBase &) {
}
...
// createAndStoreRDO                                                     //
// ----------------------------------------------------------------------//
StatusCode StripDigitizationTool::createAndStoreRDO(SiChargedDiodeCollection* chDiodeCollection, SG::WriteHandle<SCT_RDO_Container>* rdoContainer) const {
  ...
  return StatusCode::SUCCESS;
} // StripDigitization::createAndStoreRDO()

// ----------------------------------------------------------------------
// createRDO
// ----------------------------------------------------------------------
std::unique_ptr<SCT_RDO_Collection> StripDigitizationTool::createRDO(SiChargedDiodeCollection* collection) const {
  ...
  if (m_WriteSCT1_RawData.value()) {
    for (; i_chargedDiode != i_chargedDiode_end; ++i_chargedDiode) {
      ...
      if (!flagmask) { // now check it wasn't masked:
        ...
        unsigned int StripWord{strip_rdo | size_rdo};
        SCT1_RawData* p_rdo{new SCT1_RawData(id_readout, StripWord)};
        if (p_rdo) {
    ...
    for (; i_chargedDiode != i_chargedDiode_end; ++i_chargedDiode) {
      ...
      if (!flagmask) { // Check it wasn't masked
        ...
        // StripRawDataByteStreamCnv/src/StripRodDecoder.cxx)
        int size{SiHelper::GetStripNum((*i_chargedDiode).second)};
        int groupSize{size};
        ...
        unsigned int StripWord{static_cast<unsigned int>(groupSize | (stripIn11bits << 11) | (tbin << 22) | (ERRORS << 25))};
        SCT3_RawData *p_rdo{new SCT3_RawData(id_readout, StripWord, &dummyvector)};
        if (p_rdo) {
          p_rdocoll->push_back(p_rdo);
        }
      }
...
void StripDigitizationTool::addSDO(SiChargedDiodeCollection* collection, SG::WriteHandle<InDetSimDataCollection>* simDataCollMap) const {
  ...
  for (SiChargedDiodeIterator i_chargedDiode{collection->begin()}; i_chargedDiode != EndOfDiodeCollection; ++i_chargedDiode) {
    ...
    if (real_particle_hit or m_createNoiseSDO) {
      ...
      int row2D{sctDesign.row(strip)};
      Identifier id_readout;
      if (row2D < 0) { // SCT sensors and new-style ITkStrip sensors
        id_readout = m_detID->strip_id(collection->identify(),strip);
      } else { // old-style (21.9) ITkStrip sensors
        int strip2D{sctDesign.strip(strip)};
        id_readout = m_detID->strip_id(collection->identify(),row2D, strip2D);
      }
      std::pair<Identifier, InDetSimData> tmpSimData = std::make_pair(id_readout, InDetSimData(deposits, (*i_chargedDiode).second.flag()));
      (*simDataCollMap)->insert(tmpSimData);
    }
  }
}
