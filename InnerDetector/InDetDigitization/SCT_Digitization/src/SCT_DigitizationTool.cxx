...
void SCT_DigitizationTool::applyProcessorTools(
    ...
    for (std::list<ISiChargedDiodesProcessorTool *>::iterator p_proc =
             m_diodeCollectionTools.begin();
         p_proc != m_diodeCollectionTools.end(); ++p_proc) {
...
StatusCode SCT_DigitizationTool::processBunchXing(int bunchXing,
                                                  PileUpEventInfo::SubEvent::
                                                   const_iterator bSubEvents,
                                                  PileUpEventInfo::SubEvent::
                                                   const_iterator eSubEvents) {
    ...
    for (; iEvt != eSubEvents; iEvt++) {
        ...
        if (seStore.retrieve(pEI).isSuccess()) {
            ATH_MSG_DEBUG("SubEvt EventInfo from StoreGate " <<
                          seStore.name() << " :"
                                            " bunch crossing : "
                                                             << bunchXing <<
...
// create a new hits collection
    ...
    while (iColl != endColl) {
        ...
        ATH_MSG_DEBUG("SiTrackerHitCollection found with" <<
                      p_collection->size() << " hits");    // loop on the hit
                                                           // collections
        ++iColl;
    }
    return StatusCode::SUCCESS;
