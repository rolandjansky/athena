  ...
  chargedDiodes(NULL)
{
  ...
  declareProperty("OutputObjectName", m_outputRDOCollName="SCT_RDOs",            "Output Object name" );
  declareProperty("OutputSDOName", m_outputSDOCollName="SCT_SDO_Map",            "Output SDO container name");
...
StatusCode SCT_DigitizationTool::initialize() 
{
  ...
  //+++ Default off, since disabled cells taken form configuration in reconstruction stage
...
namespace {
  ...
  {
  public:
    SiDigitizationSurfaceChargeInserter (const InDetDD::SiDetectorElement* sielement, SiChargedDiodeCollection* chargedDiodes)
...
StatusCode SCT_DigitizationTool::initFrontEndTool() 
{
  ...
  //m_diodeCollectionTools.push_back( &(*m_sct_FrontEnd) ); // retrieve a ref to the from the tool handle, then take its address.
...
StatusCode SCT_DigitizationTool::initServices() 
{
  ...
  // Get SCT ID helper for hash function and Store them using methods from the SiDigitization.
...
//  m_diodeCollectionTools.push_back( &(*m_sct_RandomDisabledCellGenerator) ); // retrieve a ref to the from the tool handle, then take its address.
...
StatusCode SCT_DigitizationTool::prepareEvent(unsigned int /*index*/)
{
 ...
 // Create the IdentifiableContainer to contain the digit collections Create a new RDO container
  try 
    {  
...
if(m_atlasID == NULL) id = 0;
   ...
   id = sctID->wafer_id(Barrel, firstHit->getLayerDisk(), firstHit->getPhiModule(), firstHit->getEtaModule(), firstHit->getSide());
 }
 ...
 while (i != e) {
   ...
   if(fabs(phit->meanTime() ) < 10000. *CLHEP::ns ) {
     ATH_MSG_DEBUG("HASH = " << m_detID->wafer_hash(m_detID->wafer_id(phit->getBarrelEndcap(),phit->getLayerDisk(),phit->getPhiModule(),phit->getEtaModule(), phit->getSide())));
...
StatusCode SCT_DigitizationTool::processBunchXing(int bunchXing, PileUpEventInfo::SubEvent::const_iterator bSubEvents, PileUpEventInfo::SubEvent::const_iterator eSubEvents){
  ...
  for (; iEvt!=eSubEvents; iEvt++) {
    ...
    if (!seStore.retrieve(seHitColl,m_inputObjectName).isSuccess()) {
      msg(MSG::ERROR) << "SubEvent SCT SiHitCollection not found in StoreGate " << seStore.name() << endreq;
      return StatusCode::FAILURE;
    }
...
#ifdef SCT_DIG_DEBUG
      ATH_MSG_INFO ( "Add SCT RDO collection "<< m_detID->show_to_string (RDOColl->identify()) <<" to container!" ) ;
#endif
    }
  else
    {
...
SCT_RDO_Collection* SCT_DigitizationTool::createRDO(SiChargedDiodeCollection* collection)
{
...
#ifdef SCT_DIG_DEBUG
  ATH_MSG_INFO ( "SCTDigitization::createRDO() Collection" << m_detID->show_to_string (collection->identify()) );
#endif
 ...
 if(m_WriteSCT1_RawData.value()
      ...
      for(; i_chargedDiode!=i_chargedDiode_end; ++i_chargedDiode) 
	{
	  ...
	  if(!flagmask) 
	    {  //now check it wasn't masked:
	
	      // create new SCT RDO, using method 1 for mask: GroupSize=1: need readout id, make use of SiTrackerDetDescr 
	      ...
	      // user can define what GroupSize is, here 1: TC. Incorrect, GroupSize >= 1
	      ...
	      if (size>1) 
		{
		  ...
		  for (;it2!=i_chargedDiode_end;++it2) 
		    {
		      ++cluscounter;
		      if (cluscounter>=size) break;
		      if (it2->second.flag() & 0xDE) 
			{
			  int tmp=cluscounter;	    
			  while (it2!=i_chargedDiode_end && cluscounter<size-1 && (it2->second.flag() & 0xDE)) 
			    {it2++; cluscounter++;}
			  if (it2!=collection->end() && !(it2->second.flag() & 0xDE)) 
			    {
    ...
    //Under the current scheme time bin and ERRORS are hard-coded to default values.
    int tbin = 0x2 ; //assuming tbin is 010
    int ERRORS = 0;
	...
	if(!flagmask) 
	  {  //now check it wasn't masked:
	    ...
	    // build word (compatible with SCT_RawDataByteStreamCnv/src/SCT_RodDecoder.cxx)
	    int size=SiHelper::GetStripNum((*i_chargedDiode).second);
	    int groupSize = size;
