...
// constructor 
SCT_SurfaceChargesGenerator::SCT_SurfaceChargesGenerator(const std::string& type, const std::string& name,const IInterface* parent )
...
StatusCode SCT_SurfaceChargesGenerator::initialize() {
  ...
  if (m_doTrapping) {
    ...
    if (sc.isFailure()) {
      ATH_MSG_FATAL ("Could not retrieve SCT_RadDamageSummarySvc: " << m_radDamageSvc.name() );
      return sc ;
    }
  }
  ...
  if (m_doHistoTrap){  
    ...
    h_drift_electrode = new TH2F("drift_electrode" , "", 50, 0., 20., 50, 0., 20.);
...
//-------------------------------------------------------------------------------------------
//void SCT_SurfaceChargesGenerator::processFromTool(const SiHit* phit, std::list<SiSurfaceCharge> &charges, const float p_eventTime, const unsigned short p_eventId) const {
void SCT_SurfaceChargesGenerator::processFromTool(const SiHit* phit, const ISiSurfaceChargesInserter& inserter, const float p_eventTime, const unsigned short p_eventId) const {
...
//-------------------------------------------------------------------------------------------
//void SCT_SurfaceChargesGenerator::process(const TimedHitPtr<SiHit> & phit, std::list<SiSurfaceCharge> &charges) const {
void SCT_SurfaceChargesGenerator::process(const TimedHitPtr<SiHit> & phit, const ISiSurfaceChargesInserter& inserter) const {
...
//-------------------------------------------------------------------------------------------
//void SCT_SurfaceChargesGenerator::processSiHit(const SiHit& phit, std::list<SiSurfaceCharge> &charges, float p_eventTime, unsigned short p_eventId) const {
void SCT_SurfaceChargesGenerator::processSiHit(const SiHit& phit, const ISiSurfaceChargesInserter& inserter, float p_eventTime, unsigned short p_eventId) const {

  const InDetDD::SCT_ModuleSideDesign *p_design = dynamic_cast<const SCT_ModuleSideDesign *>(&(m_element->design() ) );
  if (!p_design) {
    ATH_MSG_ERROR ( "SCT_SurfaceChargesGenerator::process can not get "<<p_design) ;
    return ;
  }
  ...
  if(m_doDistortions){
    if(m_element->isBarrel() == 1){//Only apply disortions to barrel modules
      Amg::Vector2D BOW;
  ...
  for(int istep=0; istep<numberOfSteps; ++istep) {
    ...
    // double spess = 0.5 * sensorThickness - p_design->readoutSide() * depD;
    // if (spess<0) spess=0;
	...
	double z0 = (sensorThickness - zReadout); //mm 
	  ...
	  m_doCTrap=false;                                   //Set false for every iteration of the loop    
	  m_doCTrap =  m_radDamageSvc->doCTrap(m_hashId,spess);   //To be called once per loop: it returns a bool(true)
	  if (m_doCTrap) { //Prevents double random number shooting
	    ...
	    if (this->ChargeIsTrapped(spess, trap_pos, drift_time)) {
	      if(!m_doRamo) { break; }
		...
		m_radDamageSvc->HoleTransport(y0, z0, yfin, zfin, Q_m2, Q_m1, Q_00, Q_p1, Q_p2 ); 
		for (int strip=-2; strip<=2; strip++) { 
		  double ystrip = yd + strip*stripPitch;  //mm         <---- CHECK THIS QUANTITY 
		  SiLocalPosition position(m_element->hitLocalToLocal(xd,ystrip)); 
	...
	if(!m_doRamo){
	  ...
	  if(p_design->inActiveArea(position)) {
	    ...
	    //charges(SiSurfaceCharge(position, SiCharge(q1,totaltime,hitproc,trklink))) ;
	    inserter(SiSurfaceCharge(position, SiCharge(q1,totaltime,hitproc,trklink))) ;
	    //ATH_MSG_INFO ("Total Time: "<<totaltime<<" tdrift "<<tdrift<<" timeOfFlight  "<<timeOfFlight<<" tsurf  "<<tsurf) ;
	  }
	  else {
	    ATH_MSG_VERBOSE ( std::fixed<<std::setprecision(8)<<"Local position (phi, eta, depth): (" << position.xPhi() << ", " <<  position.xEta() <<", " << position.xDepth() << ") of the element is out of active area, charge = "<<q1) ;
	    //ATH_MSG_VERBOSE ( std::fixed<<std::setprecision(8)<<"Local position: "<<position<<" of the element is out of active area, charge = "<<q1) ;
	  }     
...
bool  SCT_SurfaceChargesGenerator::ChargeIsTrapped(double spess, double& trap_pos, double& drift_time) const {
  ...
  if (drift_time < t_electrode){
    ...
    if (m_doHistoTrap){
      ...
      h_mobility_trap->Fill(electric_field,z_trap/drift_time/electric_field);  
