...
void SCT_InducedChargedModel::init(const float vdepl, const float vbias, const IdentifierHash hashId, const ServiceHandle<ISiliconConditionsSvc> siConditionsSvc) {
  ...
  m_T = siConditionsSvc->temperature(hashId) + Gaudi::Units::STP_Temperature;
...
void SCT_InducedChargedModel::transport(const bool isElectron,
  ...
  while (t_current < m_transportTimeMax) {
    if (!isInBulk) break;
    if (!getVxVyD(isElectron, x, y, vx, vy, D, hashId, siPropertiesSvc)) break;
...
bool SCT_InducedChargedModel::getVxVyD(const bool isElectron,
  ...
  if (Ey > 0.) {
    ...
    const double mu = (isElectron ?
                       siPropertiesSvc->getSiProperties(hashId).calcElectronDriftMobility(m_T, E*CLHEP::volt/CLHEP::cm) :
                       siPropertiesSvc->getSiProperties(hashId).calcHoleDriftMobility    (m_T, E*CLHEP::volt/CLHEP::cm))
