...
InDet::InDetExtensionProcessor::execute(const EventContext& ctx) const{
  ...
  ATH_CHECK( extendedTracksResult.record( std::unique_ptr<TrackCollection>(createExtendedTracks(tracks.cptr(), trackExtensionMap.cptr()) ) ));
  ATH_MSG_DEBUG ("Container '" <<m_newTrackName.key() << "' recorded in StoreGate, size : " << extendedTracksResult->size());
  if (extendedTracksResult->size() != nTracks)
      ...
      if (m_refitPrds) {
        ...
        if (!m_cosmics) {
          ...
          if (!siPerigee) {
            ...
            DataVector<const Trk::MeasurementBase>::const_iterator RIOit=pThisExtensionPair->first->measurementsOnTrack()->begin();
            ...
            for (unsigned int i=0;i<vecPrd.size();i++) {
              double inprod=(vecPrd[i]->detectorElement()->surface(vecPrd[i]->identify()).center()-siPerigee->position()).dot(siPerigee->momentum());
              if (inprod<0){
        ...
        if (!m_cosmics) {
          ...
          if (!siPerigee) {
            ...
            for (unsigned int i=0;i<pThisExtensionPair->second.size();i++){
              double inprod = (pThisExtensionPair->second[i]->associatedSurface().center()-siPerigee->position()).dot(siPerigee->momentum());
              if (inprod<0){
...
InDet::InDetExtensionProcessor::trackPlusExtension(const Trk::Track* siTrack,
  ...
  auto cloneTSOS=[](const Trk::TrackStateOnSurface *pTSOS){return new Trk::TrackStateOnSurface(*pTSOS);};
  std::transform(trackStatesOnSurfaces.begin(), trackStatesOnSurfaces.end(),std::back_inserter(*pExtendedTrajectory),cloneTSOS);
  ...
  auto addNewTSOS_ForCosmics = [&pExtendedTrajectory,siTrack,createNewTSOS](const Trk::MeasurementBase* pm){
    ...
    const double inprod=(pm->associatedSurface().center()-perigee->position()).dot(perigee->momentum());
    if (inprod<0){
  ...
  Trk::Track* extTrack = new Trk::Track(siTrack->info(), pExtendedTrajectory,( pFitQuality ? pFitQuality->clone() : nullptr ));
  if (m_trackSummaryTool.isEnabled()) {
