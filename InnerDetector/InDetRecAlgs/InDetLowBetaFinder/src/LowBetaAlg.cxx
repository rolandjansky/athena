...
namespace InDet
{
  ...
  std::vector<float> InDet::LowBetaAlg::ChargedSMPindicators(const Trk::Track& track)
  {
    ...
    float RcorrZero2  = 9.06;
    float RcorrOne2   = -3.437;
    float RcorrTwo2   = 2.6573;
    float RcorrThree2 = -1.5598;
    float RcorrFour2  = 0.15772;
	...
	if (measurement) { //measurement
	  ...
	  if (driftcircle) { //driftcircle
		...
		RCorrectedTotBits = NLTbits*1.0-(RcorrZero2+RcorrOne2*(f_TrkAnodeDist)+RcorrTwo2*(pow(f_TrkAnodeDist,2))+ RcorrThree2*(pow(f_TrkAnodeDist,3))+RcorrFour2*(pow(f_TrkAnodeDist,4)));
