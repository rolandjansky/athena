...
class BeamSpotSummary(BeamSpotWebPage):
    ...
    def content(self,**args):
        ...
        for r in taskman.taskIterDict('distinct(DSNAME),RUNNR',["where TASKNAME like '%s%%' order by RUNNR desc" % type], limit):
            ...
            table += "<tr>"
            table += '<td><a href="http://atlas-runquery.cern.ch/query.py?q=find+r+%s+%%2F+sh+lhc+all+and+r+and+t+and+mag+and+dq+idbs%%2Cpix%%2Csct">%s</a></td>' % (runnr,runnr)
            ...
            try:
                ...
                for tag in cooltags.split():
                    table += '<a href="http://atlas-runquery.cern.ch/query.py?q=find+run+%s+%%2F+show+bs+%s">%s</a> ' % (runnr,tag,tag)
                    table += '<br>'
                table += "</td>"
            except Exception, e:
                table += "<td>%s</td>" % str(e)
