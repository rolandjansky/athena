#!/bin/bash
# Submit athena_tracking jobs to a PBS queue

if [ $# -lt 4 ]; then
  echo "Syntax:  . submit_tracking.sh npileup injobindex firstindex lastindex"
  exit 1
fi
...
while [ $index -le $lastindex ]; do
  ...
  job_name=${qname}_${NPILEUP}_${injobindex}_${index}
