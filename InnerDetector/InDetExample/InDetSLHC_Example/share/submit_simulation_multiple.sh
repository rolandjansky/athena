...
if [ $# -lt 2 ]; then
  echo "Syntax:  . submit_simulation_multiple.sh firstindex lastindex"
  exit 1
fi
...
scriptname="athena_simulation_multiple.sh"
...
while [ $index -le $lastindex ]; do
  ...
  if [ $EXEC_MODE != "INTERACTIVE" ]; then
    ...
    if [ $EXEC_MODE == "PBS" ]; then
      job_submitted=`qstat | grep $username | grep ${qname}_$index`
    elif [ $EXEC_MODE == "LSF" ]; then
      job_submitted=`bjobs -w | grep $username | grep ${qname}_$index`
    fi
  ...
  case $EXEC_MODE in
    PBS) (echo cd $outdir; echo $scriptrdir/$scriptname $index) | \
         qsub -N ${qname}_$index -l cput=${cpu_time} -V - ;;
    LSF) bsub -J ${qname}_$index -q ${cern_queue} $scriptrdir/$scriptname $index ;;
