...
dcuberef_sim=$artdata/InDetSLHC_Example/ReferenceHistograms/InclinedDuals_${particle}_${energy}_SiHitValid.root
dcuberef_rec=$artdata/InDetSLHC_Example/ReferenceHistograms/InclinedDuals_${particle}_${energy}_physval.root
dcuberef_digi_pixel=$artdata/InDetSLHC_Example/ReferenceHistograms/PixelRDOAnalysis_${particle}_${energy}.root
dcuberef_digi_strip=$artdata/InDetSLHC_Example/ReferenceHistograms/SCT_RDOAnalysis_${particle}_${energy}.root
...
#   test -n "$dcube_rec_fixref"  && echo "art-result: 22 plot-fixref"
  ...
  ( set -x
    inputDAOD_IDTRKVALIDFile="$daod_particle_energy" exec athena.py InDetSLHC_Example/PhysValITk_jobOptions.py
  )
