  ...
  if (element) {
   ...
   << asin(element->sinStereo())/CLHEP::degree << endl;
    ...
    for (unsigned int iTestPos = 0; iTestPos < positionsVec.size(); iTestPos++) {
      ...
      // using the surface bounds to check if the new local position is inside the detector element
      // SiIntersect intersectState = element->inDetector(globalPos, tolerance, tolerance);
      auto& pBounds = element->surface().bounds();
      // inside detector within tolerance
      bool inside = pBounds.inside(localPosNew,tolerance,tolerance);
