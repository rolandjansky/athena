...
class manageJob:
	def __init__(self,
		...
		if self.j==-1:
			self.RunPath="%s/Iter%d/" % (self.OutputPath, self.i)
			self.SCRIPTNAME = "%s_Iter%d_Solve.lsf" % (self.preName,self.i)
		else:
			self.RunPath="%s/Iter%d/%s/%02d/" % (self.OutputPath, self.i, self.dataName, self.j)
			self.SCRIPTNAME = "%s_Iter%d_%s_Part%02d.lsf" % (self.preName,self.i,self.dataName, self.j)
	...
	def writeScript(self):
		...
		if self.j!=-1:		
			script.write("#BSUB -J %s_Iter%d_%s_Part%02d \n" % (self.preName,self.i,self.dataName, self.j))
			script.write("#BSUB -o %s/Iter%d/logs/Iter%d_%s_Part%02d.log \n" % (self.OutputPath,self.i,self.i,self.dataName,self.j))
			script.write("#BSUB -e %s/Iter%d/logs/Iter%d_%s_Part%02d.err \n" % (self.OutputPath,self.i,self.i,self.dataName,self.j))
		else:
	...
	def send(self,runmode):
		...
		if runmode == "batch":
			...
			if self.j==-1:
				print "  Sending %s_Solve_Iter%d job " % (self.preName, self.i)
			else:
				print "  Sending %s_Iter%d_%s_Part%02d job to LxBatch" % (self.preName,self.i,self.dataName, self.j)
...
def mergeTFiles(OutputPath, iter, DataToRun):
...
class mergeScript:
	...
	def write(self):
		...
		script.write("#BSUB -J %s_Iter%d_%s_Merge \n" % (self.preName, self.i,self.dataName))
		script.write("#BSUB -o %s/Iter%d/logs/Iter%d_%s_Merge.log \n" % (self.OutputPath,self.i,self.i,self.dataName))
		script.write("#BSUB -e %s/Iter%d/logs/Iter%d_%s_Merge.err \n" % (self.OutputPath,self.i,self.i,self.dataName))
		...
		for subJob in range(self.nCPUs):
			mergeFiles.write("%s/Iter%d/%s/%02d/monitoring.root \n" %  (self.OutputPath,self.i,self.dataName,subJob))
		mergeFiles.close()
	...
	def send(self,runmode):
		...
		if runmode == "batch":
			...
			print "  Sending %s_Merge_%s_Iter%d job " % (self.preName, self.dataName, self.i)
			
		elif runmode == "local":
			print "  Running %s_Merge_%s_Iter%d job" % (self.preName, self.dataName, self.i)
			os.system("sh %s | tee %s/Iter%d/logs/Iter%d_%s_Merge.log \n" % (self.SCRIPTNAME, self.OutputPath,self.i,self.i,self.dataName))
