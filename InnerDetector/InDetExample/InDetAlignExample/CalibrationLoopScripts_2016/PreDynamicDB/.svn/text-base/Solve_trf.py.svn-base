...
# Function for e-mail notification (less than 60 LB case)

def sendStatusMailTooShort(runNumber) :
    mserver = 'cernmx.cern.ch'
    mfrom = 'atltzp1@cern.ch'
    mto = ['Pierfrancesco.Butti@cern.ch', 'Matthias.Danninger@cern.ch', 'Heather.Gray@cern.ch', 'Shih-Chieh.Hsu@cern.ch', 'Javier.Jimenez.Pena@cern.ch', 'Hideyuki.Oide@cern.ch', 'Armin.Nairz@cern.ch']
    msubject = "ID Alignment Processing for Run %d skipped: Less than 60 LB" % runNumber

    # assemble mail body
    mbody  = " %s: Tier-0 ID alignment processing was skipped for run %d as it had less than 60 LB. \n\n" % (time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime()), runNumber)
    mbody += " Output can be found on EOS and AFS: \n"
    mbody += "    /eos/atlas/atlastier0/tzero/caf/IDAlignment/data16_13TeV/calibration_IDTracks/00%d/ \n" % runNumber
    mbody += "    /afs/cern.ch/user/a/atlidali/w0/calibLoop/Tier0/data16_13TeV/calibration_IDTracks/00%d/ \n" % runNumber
    rc = 0
...
def runAthena(picklefile) :
    ...
    if not nfiles :   # problem with job definition or reading pickle file
        ...
        #JAVI: For first two iters it is enough to skip first nLBtoSkip LB
        #JAVI: Last block should take the last LBs, eg: blockLBsize + 56 LB
        #JAVI: for last two iterations, we should have a list of lists:
	    ...
	    dt = int(time.time() - t0) 
            retcode = 0
            acronym = 'OK'
            txt = 'only %d LBs or less. Too short run!' % (nLBtoSkip)  
            outmap = { 'prodsys': { 'trfCode': retcode,
                                    'trfAcronym': acronym,  
                                    'jobOutputs': [],
                                    'jobInputs': [],
            ...
            rc = sendStatusMailTooShort(runnr)
            rc2 = AddToListOfBadRuns(runnr, project)
            
            # Handshake
            handshake_command = "/afs/cern.ch/user/a/atlidali/CalibrationLoop2016/IDAlignDBTools/HandShake.py {0} {1} {2} --exec".format( runnr, "calibration_BeamSpot", "Solve_trf" )
            ...
            #JAVI: for iteration 2 we load the same align. const. for every block 
            if iteration == 2 :
                inputAlignmentConstants = []
                IBLDistDB = []
                for x in xrange(nBlocks) : 
                    inputAlignmentConstants.append("/afs/cern.ch/user/a/atlidali/w0/calibLoop/Tier0/%s/tmpconstants/%d/%d_AlignmentConstants_Iter%d.root" % (project, runnr, runnr, iteration-1))
                    IBLDistDB.append("/afs/cern.ch/user/a/atlidali/w0/calibLoop/Tier0/%s/tmpIBLDistDB/%d/%d_IBLDistDB_Iter%d.db" % (project, runnr, runnr, iteration-1))
            #JAVI: for last iteration we load for each block an specific set of constants
            if iteration == 3 :
                inputAlignmentConstants = []
                IBLDistDB = []
                for x in xrange(nBlocks) :
            ...
            #JAVI: For first two iterations it goes as before, just than first nLBtoSkip LB have been skipped
            if iteration <= 1 :
...
#globalTag = "CONDBR2-ES1PA-2015-11" #CONDBR2-BLKPA-2015-14
...
###############################################################
                ''' % (str(inputRAWFileList), str(inputfilenames), inputAlignmentConstants, outputAlignmentConstants, IBLDistDB, project)#,beamspottag)
                ...
                if retcode == 0 :
                    ...
                    if iteration >= 0 :
                        ...
                        #JAVI: Adding an IBLDistDB folder:
                        os.system("mkdir -p /afs/cern.ch/user/a/atlidali/w0/calibLoop/Tier0/%s/tmpIBLDistDB/%d" % (project, runnr))
                        os.system("cp %s /afs/cern.ch/user/a/atlidali/w0/calibLoop/Tier0/%s/tmpIBLDistDB/%d/%d_IBLDistDB_Iter%d.db" % (outputAlignmentConstantsdb, project, runnr, runnr, iteration))
            ...
            #JAVI: For last two iterations we process each blockLBsize LB with different alignment constants, in a consecutive way
            #JAVI: as it is done one after other, processing time will ingrease by a factor totalLB/blockLBsize
            #JAVI: also, the first nLBtoSkip LB have been skipped
...
include("InDetAlignExample/NewInDetAlignAlgSetup.py")
###############################################################
                    ''' % (str(inputRAWFileList), str(inputfilenames[x]), inputAlignmentConstants[x], outputAlignmentConstantsBlock[x], IBLDistDB[x], project)#,beamspottag)
