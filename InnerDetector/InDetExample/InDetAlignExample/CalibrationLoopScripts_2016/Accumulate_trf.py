...
def accumData(picklefile) :
    ...
    if not nfiles :   # problem with job definition or reading pickle file
        ...
        if not len(inputfilenames) :
            ...
            blockLBsize = 100
            nLB_FastPixelMovement = 60
            blockLBsize_FastStep = 20

            #JAVI: For iteration > 0 the proper alignment constants have to be taken
            #JAVI: Calculating relative position to the first LB
            #JAVI: We have to take special care of the first nLB_FastPixelMovement LB and the last ones
            #JAVI: there is need to know how many blocks of nLB_FastPixelMovement LB are. 
            if iteration > 0 : 
                listOfLB = []
                ...
                #firstLB = listOfLB[0]
                nBlocks = (len(listOfLB) - nLB_FastPixelMovement) // blockLBsize
                ...
                if relativePosition < nLB_FastPixelMovement : block = relativePosition // blockLBsize_FastStep
                else : block = (relativePosition - nLB_FastPixelMovement) // blockLBsize + nLB_FastPixelMovement // blockLBsize_FastStep
                if block == nBlocks : block -= 1 #JAVI: this is the case for the last LB, wich are added to previous block of nLB_FastPixelMovement LB              
            ...
            #JAVI: the IBLDistDB file contains the needed db file for inputBowingDatabase and inputDynamicGlobalDatabase
...
###############################################################
                ''' % (str(inputfilenames), inputAlignmentConstants, events, skipEvents, IBLDistDB, IBLDistDB, project)
            else :
                cont = '''
###############################################################
#
