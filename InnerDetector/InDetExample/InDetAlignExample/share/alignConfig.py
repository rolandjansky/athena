...
class alignConfigBase:
    def __init__(self, firstIter, iterations, strategies):
        ...
        self.readDynamicGlobalFoldersFromCool = False
        ...
        self.inputDynamicGlobalCoolFile = options.inputCoolFile
        ...
        if (len(self.userSuffix) > 0):
            self.folderSuffix = self.userSuffix
    ...
    def optParsing(self):
        ...
        parser.add_option("--inputCoolFile",      dest="inputCoolFile",      help="In the case you want to use a specific COOL .db file provide the absolute path here",                           default="")
        ...
        parser.add_option("--email",              dest="email",              help="email address for notification",                                                                                default="")
        parser.add_option("--runNumber",          dest="runNumber",          help="Run number specification to generate monitoring plots. Use \"multiple\" in case of multiple runs",              default="")
    ...
    def generate_dataset(self):
        ...
        for i_sample in range(len(self.inputList)):
            ...
            Data1.setByteStream(bool(self.isData))
            ...
            if (self.eventType[i_sample] == "collisions"):
                if self.isData:
                    # Users should lookup the following TWiki pages to make sure the appropriate global/Geo tags.
                    # CondDB global tags: https://twiki.cern.ch/twiki/bin/view/AtlasComputing/CoolProdTags
                    # Geometry      tags: https://twiki.cern.ch/twiki/bin/view/AtlasComputing/AtlasGeomDBTags
                    
                    # The following conditions are the release 21
                    Data1.setGlobalTag("CONDBR2-BLKPA-2017-04")        # the final condition in release 20.7
                    Data1.setDetDescrVersion("ATLAS-R2-2015-03-29-00") # the geometry tag used in 2016.
                    
                    # The following conditions are the release 20.7
                    # Data1.setGlobalTag("CONDBR2-BLKPA-2016-21")        # the final condition in release 20.7
                    # Data1.setDetDescrVersion("ATLAS-R2-2015-04-00-00") # the geometry tag used in 2016.
                else:
                    Data1.setGlobalTag("OFLCOND-RUN12-SDR-28")
                    Data1.setDetDescrVersion("ATLAS-R2-2015-03-01-00")

            elif (self.eventType[i_sample] == "cosmics"):
            ...
            if "cosmics" in self.eventType[i_sample].lower():
    ...
    def do_L2(self):
        ...
        # alignment level for the Pixel barrel. If set to -1 uses PixelAlignmentLevel, otherwise it overwrites it
        # alignment level for the Pixel endcaps. If set to -1 uses PixelAlignmentLevel, otherwise it overwrites it
        self.PixelAlignmentLevel        = self.get_array( 2 )
        self.PixelAlignmentLevelBarrel  = self.get_array( -1 )
        self.PixelAlignmentLevelEndcaps = self.get_array( -1 )
        ...
        # are we running SCT alignment (for barrel and endcaps)
        self.AlignSCT                 =  self.get_array( True )
        self.AlignSCTBarrel           =  self.get_array( True )
        self.AlignSCTEndcaps          =  self.get_array( True )
        
        # alignment level for the full SCT # see InDetAlignGeometryLevel wiki for more info
        # alignment level for the SCT barrel. If set to -1 uses SCTAlignmentLevel, otherwise it overwrites it
        # alignment level for the SCT endcaps. If set to -1 uses SCTAlignmentLevel, otherwise it overwrites it
        self.SCTAlignmentLevel        = self.get_array(  2 )
        self.SCTAlignmentLevelBarrel  = self.get_array( -1 )
        self.SCTAlignmentLevelEndcaps = self.get_array( -1 )
        ...
        self.extraOptions["pixelAlignBarrelRotY"] = True
        self.extraOptions["pixelAlignBarrelRotZ"] = False
        self.extraOptions["pixelAlignBarrelBowX"] = False
        ...
        ## SCT
        self.extraOptions["sctAlignBarrelX"]      = True
        self.extraOptions["sctAlignBarrelY"]      = True
        self.extraOptions["sctAlignBarrelZ"]      = False
        self.extraOptions["sctAlignBarrelRotX"]   = True
        self.extraOptions["sctAlignBarrelRotY"]   = True
        self.extraOptions["sctAlignBarrelRotZ"]   = False
        ...
        self.extraOptions["sctAlignEndcapY"]      = False
        self.extraOptions["sctAlignEndcapZ"]      = False
        self.extraOptions["sctAlignEndcapRotX"]   = True
        self.extraOptions["sctAlignEndcapRotY"]   = True
        self.extraOptions["sctAlignEndcapRotZ"]   = True
    ...
    def do_L3(self):
        ...
        self.extraOptions["ModCut"]                      = 0
        self.extraOptions["softModeCut"]                 = 1.
        self.extraOptions["solvingOption"]               = 2
        ...
        # alignment level for the Pixel barrel. If set to -1 uses PixelAlignmentLevel, otherwise it overwrites it
        # alignment level for the Pixel endcaps. If set to -1 uses PixelAlignmentLevel, otherwise it overwrites it
        self.PixelAlignmentLevel        = self.get_array( 3 )
        self.PixelAlignmentLevelBarrel  = self.get_array( -1 )
        self.PixelAlignmentLevelEndcaps = self.get_array( -1 )
        ...
        # alignment level for the SCT barrel. If set to -1 uses SCTAlignmentLevel, otherwise it overwrites it
        # alignment level for the SCT endcaps. If set to -1 uses SCTAlignmentLevel, otherwise it overwrites it
        self.SCTAlignmentLevel        = self.get_array(  3 )
        self.SCTAlignmentLevelBarrel  = self.get_array( -1 )
        self.SCTAlignmentLevelEndcaps = self.get_array( -1 )
        ...
        self.extraOptions["sctAlignBarrelX"]      = True
        self.extraOptions["sctAlignBarrelY"]      = True
        self.extraOptions["sctAlignBarrelZ"]      = True
        self.extraOptions["sctAlignBarrelRotX"]   = True
        self.extraOptions["sctAlignBarrelRotY"]   = True
        self.extraOptions["sctAlignBarrelRotZ"]   = True
        
        ## end-caps
        self.extraOptions["sctAlignEndcapX"]      = True
        self.extraOptions["sctAlignEndcapY"]      = True
        self.extraOptions["sctAlignEndcapZ"]      = True
        self.extraOptions["sctAlignEndcapRotX"]   = True
        self.extraOptions["sctAlignEndcapRotY"]   = True
        self.extraOptions["sctAlignEndcapRotZ"]   = True
    ...
    def do_beamspot_constraint(self):
        ...
        self.extraOptions["beamSpotTag"]          = "IndetBeampos-RUN2-ES1-UPD2-11"
    ...
    def do_EoverP_constraint(self):
        ...
        self.extraOptions["eoverpmapconstraint"]     = "/afs/cern.ch/work/k/krosbach/public/alignment/2015_5TeV_20.7_reprocessing/data/nullmap.root"
    ...
    def do_truncation(self):
        # do not wait for all jobs to finish
        self.doTruncate        = True
        self.TruncateThreshold = 0.05  # fraction of jobs to skip
