...
#----------------------------------------------------------------------------------------------------
def save_log( RunNumber, message_tag = "SUCCESS" ):
    
    now = datetime.datetime.now()
    save_path="log_forEachRun/{0}_{1}.log".format( RunNumber, int(time.mktime(now.timetuple())) )
    ...
    if( os.getenv("SEND_EMAIL") == "TRUE" ):
        for addr in to_addr:
            msg = create_message(from_addr, addr, '[{1}] IDAlign CalibrationLoopChecker Log Run {0}'.format( RunNumber, message_tag ) , checker_log)
