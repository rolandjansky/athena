...
class setIBLDist(AtlCoolLib.coolTool):
    def setup(self,args):
        ...
        self.runmin=int(args[2])            # Set here the RunNr. min
        self.lumimin=int(args[3])     # Set here the LB min

        if len(args) > 5:
            self.runmax=int(args[4])            # Set here the RunNr. max
            self.lumimax=int(args[5])     # Set here the LB max
    ...
    def execute(self):
        ...
        for iline in open(self.txtfile).readlines():
            ...
            if tokens[0]=='/Indet/IBLDist': continue
        ...
        for idata in bowdata:
            ...
            print "%7i" % channel, "%3i %6i %6.4f %6.1f %6.1f" % idata
...
smytool=setIBLDist('set_ibldist.py',False,5,7,[])
