...
def check_validity( package, dof, value, error) :
    return True if error < 0.3 else False
...
def judge_L16_result_segment( LBHead, alignIterLogs ):
    ...
    print "============================================================"
    for dofElem, validty_flag in zip( alignResultIterSum, validity_flags ):
        print "{0:20s} : {1:10s} {2:7.3f} um   ==> {3:20s}" \
            .format( dofElem["stave"], dofElem["dof"], dofElem["error"], "Valid" if validity_flags else "Invalid" )
    print "============================================================"
