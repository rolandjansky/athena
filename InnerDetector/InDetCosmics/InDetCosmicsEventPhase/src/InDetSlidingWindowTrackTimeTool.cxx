...
void InDet::InDetSlidingWindowTrackTimeTool::beginRun() 
{
  m_averageT0 = 0.;

  if (!m_useNewEP) 
    {
      ATH_MSG_INFO( "InDetSlidingWindowTrackTimeTool::beginRun, averageT0 = 0 (m_useNewEP=false) " ); 
      return;
    }
   ...
   TRT_ID const * TRTHelper;
    ...
    ATH_MSG_INFO( "InDetSlidingWindowTrackTimoeol::beginRun Using updated EP calculation (December 2009), subtracting: " << m_averageT0 
...
double InDet::InDetSlidingWindowTrackTimeTool::findPhase( Trk::Track const * track )
{
  ...
  for (int itr = 0; itr != m_nIterations; ++itr) {
	...
	if (itr == 0 
	    || fabs(timeresidual-windowCenter) < m_windowSize) {
...
double InDet::InDetSlidingWindowTrackTimeTool::findPhase( Trk::Segment const * segment)
{
  std::vector<float> data(4);
  data.push_back(0.); //0 0.1 -0.00087 0
  data.push_back(0.1);
...
double InDet::InDetSlidingWindowTrackTimeTool::findPhaseFromTE(Trk::Track const * track)
{
  ATH_MSG_DEBUG( "Finding phase..." );
