...
StatusCode InDet::PixelClusterValidationNtupleWriter::execute() {
    ...
    if( mjo_onTrack){
     sc = evtStore()->retrieve(m_trackCollection,mjo_trackCollectionName);
      //    std::string  m_trackCollectionName = "CombinedInDetTracks" ;
     /* if ( evtStore()->contains<TrackCollection>(mjo_trackCollectionName)
	  && evtStore()->retrieve(m_trackCollection,mjo_trackCollectionName).isFailure())
*/
     if ( sc.isFailure() ){
...
//    m_PixHitPhiIndex_delta->clear();
    ...
    if ( mjo_Hits || mjo_MC ||  mjo_NotAssociated || mjo_NN ) {
        ...
        for ( ; collectionIterator != m_hitcontainer->end(); collectionIterator++) {
            ...
            if(genP==0) m_PixHitPDGParticle->push_back(0);
            else m_PixHitPDGParticle->push_back(genP->pdg_id());
...
//              std::cout << " part: " << mp << "X: " << Ypos[partIndex] << " shift " << shift << 
//                  " Y: " << Zpos[partIndex] << std::endl;
...
//              std::cout << " truthPhiIndex " << truthPhiIndex << " truthEtaIndex " << truthEtaIndex << 
//                  " Zpos " << Zpos[partIndex] << " YposC: " << YposC << " pixelCenterY " << pixelCenterY << " pixelCenterX : " << pixelCenterX 
//                        << " pitchY " << pitchY << " pitchX " << pitchX << " length: " << design->length() 
//                        << " width: " << design->width() << " with max: " << design->maxWidth() << std::endl;
...
//              m_PixClusErrY->push_back(-100);
		...
		// end error code
	      ...
	      //					  << " Angle " << (*m_PixHitAngle)[part] << " Eta " << (*m_PixHitAngle2)[part]);
								...
								int sizeY)
{
  ...
  //  if(msgLvl(MSG::VERBOSE)) msg(MSG::VERBOSE) << " Number of charges: " << chList.size() << endreq;
  ...
  std::vector<float>::const_iterator charge = chList.begin();    
  ...
  for (; rdosBegin!= rdosEnd; ++rdosBegin, ++charge)
    {
      ...
      if(msgLvl(MSG::VERBOSE)) msg(MSG::VERBOSE) << " Adding pixel row: " << row << " col: " << col << " charge: " << *charge << endreq;
  ...
  //  ATH_MSG_INFO(" sizeX : " << sizeX << " sizeY: " << sizeY );
  ...
  if(msgLvl(MSG::VERBOSE)) msg(MSG::VERBOSE) << " Putting together the n. " << rdos.size() << " rdos into a matrix." << endreq;
  ...
  for (; rdosBegin!= rdosEnd; ++rdosBegin, ++charge)
    {
      ...
      if(msgLvl(MSG::VERBOSE)) msg(MSG::VERBOSE) << " phi Index: " << pixelID.phi_index(rId) << " absrow: " << absrow << " eta Idx: " << pixelID.eta_index(rId) << " abscol: " << abscol << " charge " << *charge << endreq;
      ...
      if (abscol <0 || abscol > sizeY)
	{
      ...
      //   if(msgLvl(MSG::VERBOSE)) msg(MSG::VERBOSE) << " PitchY: " << pitchY << endreq;
  ...
  if(msgLvl(MSG::VERBOSE)) msg(MSG::VERBOSE) << " Layer number: " << input->ClusterPixLayer << " Barrel / endcap: " << input->ClusterPixBarrelEC << endreq;
...
//  HepPoint3D globalPos = element->globalPosition(centroid);
//  HepVector3D my_track = globalPos-beamSpotPosition;
...
//  HepVector3D my_etaax = element->etaAxis();
  ...
  // if(msgLvl(MSG::VERBOSE)) msg(MSG::VERBOSE) << " Angle phi: " << angle << " theta: " << boweta << endreq;
  ...
  // if(msgLvl(MSG::VERBOSE)) msg(MSG::VERBOSE) << " RowWeightedPosition: " << rowWeightedPosition << " ColWeightedPosition: " << columnWeightedPosition << endreq;
  ...
  for (int u=0;u<sizeX;u++)
       ...
       for (int s=0;s<sizeY;s++)
 	{
	  ...
	  //           inputData.push_back(matrixOfToT[u][s]);
...
StatusCode InDet::PixelClusterValidationNtupleWriter::fillClusterVector( const InDet::PixelCluster& cluster, IdentifierHash element_hash  ) {
  ...
  if (  m_nRIOs   >= maxRIO_size ) {
    ...
    const AmgSymMatrix(5) locErrMat = cluster.localCovariance();
    
    m_PixClusErrX->push_back( Amg::error(locErrMat,Trk::x));
    m_PixClusErrY->push_back( Amg::error(locErrMat,Trk::y));
  ...
  //  HepPoint3D BS = beamposition.position();
    ...
    StatusCode   sc = createInput(cluster,BS,tanl, l_sizeX, l_sizeY);
