...
int process_object(string p_objname,TFile* p_ftarget,TFile* p_ftosumm,bool p_second_pass) {
  ...
  if (p_second_pass) {
    ...
    cout << "profile " << o1->GetName() << endl;
      ...
      if (!ret){
        // std::cout <<" writing into " <<p1->GetDirectory()->GetFile()->GetName() <<" @ "<<p1->GetDirectory()->GetName()<< std::endl;
    ...
    cout << "postwrote " << post << endl;
      if (!ret){
        // std::cout <<" writing into " <<e1->GetDirectory()->GetFile()->GetName() <<" @ "<<e1->GetDirectory()->GetName()<< std::endl;
