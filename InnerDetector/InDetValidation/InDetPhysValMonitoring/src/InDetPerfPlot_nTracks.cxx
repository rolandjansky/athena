...
InDetPerfPlot_nTracks::initializePlots() {
  ...
  m_counters[ALL] = Book1D(hd.name,hd.title,hd.nBinsX,hd.xAxis.first,hd.xAxis.second, prependDirectory);
  hd=retrieveDefinition("ntracksel");
  m_counters[SELECTED] = Book1D(hd.name,hd.title,hd.nBinsX,hd.xAxis.first,hd.xAxis.second, prependDirectory);
  hd=retrieveDefinition("nparticle");
  m_counters[TRUTH] = Book1D(hd.name,hd.title,hd.nBinsX,hd.xAxis.first,hd.xAxis.second, prependDirectory);
  hd=retrieveDefinition("num_truthmatch_match");
  m_counters[TRUTH_MATCHED] = Book1D(hd.name,hd.title,hd.nBinsX,hd.xAxis.first,hd.xAxis.second, prependDirectory);
}
