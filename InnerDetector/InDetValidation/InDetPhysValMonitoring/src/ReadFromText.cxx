...
namespace {
	std::vector<std::string> split(const std::string& str, int delimiter(int)=std::isspace){
...
std::string ReadFromText::source() const {
    return m_source;
}

std::string ReadFromText::format() const {
    return m_format;
}

bool ReadFromText::histoDefinitionMap(std::map<std::string, SingleHistogramDefinition> & usersmap) const{
    bool ok(true);
    for (auto i:m_vectorOfDefinitions){
        if (i.empty()) continue;
        if (not i.validType()) continue;
        ok =(usersmap.insert(std::pair<std::string, SingleHistogramDefinition>(i.stringIndex(),i))).second;
        if (not ok) std::cout<<"ReadFromText: bad insertion for "<<i.stringIndex()<<std::endl;
    }
    return (not usersmap.empty());
}

bool ReadFromText::insertDefinition(const SingleHistogramDefinition & oneDefinition){
    bool ok(true);
    m_vectorOfDefinitions.push_back(oneDefinition);
    return ok;
}

bool ReadFromText::initialize() {
    bool ok(true);
    std::ifstream myfile;
    myfile.open(source());
    if (myfile.is_open()){
...
SingleHistogramDefinition ReadFromText::parseTextLine(const std::string & line){
    SingleHistogramDefinition s;
    typedef std::vector<std::string> StringVec;
    enum RegXHistoGroups{TOTAL,TYPE,NAME,TITLE,NXBINS,XLO,XHI,NYBINS,YLO,YHI,XLABEL,YLABEL,FOLDER,NGROUPS};
    enum HistoType{TH1,TPROFILE,TH2,INVALID};
    //bool found(true);
    ...
    **/
    ...
    if (nBinsStartEnd.size()==5){		//TProfile; limits given for Y axis also
    	hType=TPROFILE;
      m[NYBINS]="";m[YLO]=nBinsStartEnd[3];m[YHI]=nBinsStartEnd[4];
    }
    if (nBinsStartEnd.size()==6){		//2D histogram; limits given for Y axis also
    	hType=TH2;
      m[NYBINS]=nBinsStartEnd[3];m[YLO]=nBinsStartEnd[4];m[YHI]=nBinsStartEnd[5];
    }
    m[XLABEL]=chunks[3];
   ...
   try{
	   ...
	   xHi=std::stof(m[XHI]);
	  	...
	  	std::cout<<"Problem converting some of the arguments : "<<m[NXBINS]<<"; "<<m[XLO]<<"; "<<m[XHI]<<std::endl;
	  	hType=INVALID;
	  }
	 ...
	 case TH1:
	   {
	 	 //SingleHistogramDefinition(Titles_t name, Titles_t histoType, Titles_t title, NBins_t nbinsX,Var_t xLo, Var_t xHi,Titles_t xName);
	 ...
	 case TPROFILE:
	   ...
	   SingleHistogramDefinition sx(m[NAME],m[TYPE],m[TITLE],nbinsX,nBinsY,xLo,xHi,yLo,yHi, m[XLABEL],m[YLABEL],m[FOLDER]);
	   return sx;
	   break;
	   }
	 case  TH2:
