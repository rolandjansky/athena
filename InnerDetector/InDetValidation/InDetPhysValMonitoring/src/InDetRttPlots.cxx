...
// also not a nice way of configuring the eta cut..
  bool isPrimary( const xAOD::TruthParticle* tp, float max_eta )
  {
    ...
    if( tp->pt() < 500.   ||  fabs(tp->eta()) > max_eta ) // Check the particle is within acceptance
      return false;
...
InDetRttPlots::fillTruth(const xAOD::TruthParticle& truth){
    if( isPrimary( &truth, m_truthPrimaryEtaCut ) ) { m_effPlots.fillDenominator(truth); }
}
...
InDetRttPlots::fill(const xAOD::TruthParticle& truthParticle){
  ...
  if( isPrimary( &truthParticle, m_truthPrimaryEtaCut ) ) { m_effPlots.fillNumerator(truthParticle); }
}
...
void InDetRttPlots::SetPrimaryEtaCut( float eta ) { 
  ...
  m_resPlots.SetTruthEtaCut( eta );
  m_resPlots.SetEtaBinning( int(2*eta/0.25), -1*eta, eta );
}
