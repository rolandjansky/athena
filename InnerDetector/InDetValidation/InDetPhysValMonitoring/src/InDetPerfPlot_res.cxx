...
InDetPerfPlot_res::initializePlots() {
  ...
  for (unsigned int var(0); var != NPARAMS; ++var) {
    ...
    TH2 *meanbaseHisto = Book2D(res_vs_etaName, res_vs_etaTitle, m_trackEtaBins, m_etaMin, m_etaMax, m_paramNames.at(
                                  var).second.second, -m_paramNames.at(var).second.first, m_paramNames.at(
                                  var).second.first, PrependDirectory);
    ATH_MSG_INFO("Booking " << res_vs_etaName);
    m_meanbasePlots[var] = meanbaseHisto;
    TH1 *meanHisto = Book1D(resmean_vs_etaName, resmean_vs_etaTitle, m_trackEtaBins, m_etaMin, m_etaMax,
                            PrependDirectory);
    m_meanPlots[var] = meanHisto;
    TH1 *rHisto =
      Book1D(reswidth_vs_etaName, reswidth_vs_etaTitle, m_trackEtaBins, m_etaMin, m_etaMax, PrependDirectory);
    ...
    TH2 *meanPtbaseHisto = Book2D(res_vs_ptName, mptTitle, m_PtBins, xMin, xMax, m_paramNames.at(
                                    var).second.second, -m_paramNames.at(var).second.first, m_paramNames.at(
                                    var).second.first, PrependDirectory);
    ...
    TH2 *pbHisto = Book2D(pull_vs_etaName, pbTitle, m_trackEtaBins, m_etaMin, m_etaMax, nPullBins, allLimits.first,
                          allLimits.second, PrependDirectory);
...
InDetPerfPlot_res::formName(const unsigned int p, std::string type, const std::string dir) const {
  ...
  if (dir == "IDPerformanceMon/Tracks/SelectedGoodTracks") {
    hdir = "GoodTrks_";
  }else if (dir == "IDPerformanceMon/Tracks/SelectedGoodJetTracks") {
    hdir = "GoodJetTrks_";
  }else if (dir == "IDPerformanceMon/Tracks/SelectedGoodJetHighPtTracks") {
    hdir = "GoodJetHighPtTrks_";
  }else {
    ATH_MSG_INFO("Unidentified Dir = " << dir);
  }
...
InDetPerfPlot_res::formTitle(const unsigned int param, std::string type) const {
  ...
  std::string titleLeft[NPARAMS] = {
    "d_{0}: ", "z_{0}: ", "#phi: ", "#theta: ", "z_{0}*sin(#theta): ", "q/pt: "
  };
  ...
  }else if ((type == "pull") || (type == "pull_vs_eta") || (type == "pullwidth_vs_eta") ||
            (type == "pullmean_vs_eta")) {
  ...
  if ((type == "res_vs_eta") || (type == "resmean_vs_eta") || (type == "reswidth_vs_eta") || (type == "pull_vs_eta") ||
      (type == "pullwidth_vs_eta")) {
    titlePostfix = " vs #eta";
  }
  if (log_mode == "ON") {
    if ((type == "res_vs_pt") || (type == "resmean_vs_pt") || (type == "reswidth_vs_pt")) {
      titlePostfix = " vs log(p_{T})";
    }
  } else {
    if ((type == "res_vs_pt") || (type == "resmean_vs_pt") || (type == "reswidth_vs_pt")) {
      titlePostfix = " vs p_{T}";
    }
  }
