...
unsigned int partialHdefFromAttributes(SingleHistogramDefinition &s,const xercesc::Attributes & attrs){
    unsigned int nFilled(0);
    //id, type and title are *possible* attributes
    XercesString histoStrings[]={fromNative("id"),fromNative("type"),fromNative("title")};
...
unsigned int xAxisFromAttributes(SingleHistogramDefinition &s,const xercesc::Attributes & attrs){
    unsigned int nFilled(0);
    //title, n, lo and hi are currently *necessary* attributes
    XercesString histoStrings[]={fromNative("title"),fromNative("n"),fromNative("lo"), fromNative("hi")};
    const XMLCh * val(nullptr);
    unsigned int idx{0};
...
unsigned int yAxisFromAttributes(SingleHistogramDefinition &s,const xercesc::Attributes & attrs){
    unsigned int nFilled(0);
    //title, n, lo and hi are currently *necessary* attributes
    XercesString histoStrings[]={fromNative("title"),fromNative("n"),fromNative("lo"), fromNative("hi")};
    const XMLCh * val(nullptr);
...
unsigned int partialHdefFromText(SingleHistogramDefinition &s,const std::string  & line){
    unsigned int nFilled(0);
    enum RegXHistoGroups{TOTAL,TITLE,NBINS,XLO,XHI,XAXIS,YAXIS,DUMMY,FOLDER,NGROUPS};
    std::string rex=R"delim(^\s+"([^"]+)"\s+(\d+)\s+([-+.0-9eE]+)\s+([-+.0-9eE]+)\s+"([^"]+)"\s+"([^"]+)"\s*(.*)\s*$)delim";
    std::regex reg(rex);
    std::smatch m;
    if (std::regex_match(line,m, reg)){
        ...
        if (hasFolder) s.folder=m[FOLDER].str();
        nFilled=(int)m.size() - 1;
    }
    return nFilled;
}
unsigned int partialHdefFromTProfText(SingleHistogramDefinition &s,const std::string  & line){
    unsigned int nFilled(0);
    enum RegXHistoGroups{TOTAL,TITLE,NBINS,XLO,XHI,YLO,YHI,XAXIS,YAXIS,DUMMY,FOLDER,NGROUPS};
    //text like: &quot;Test of TProfile&quot; 20 -50 50 0 200 &quot;#eta&quot;  &quot;testEntries&quot;
    std::string rex=R"delim(^\s+"([^"]+)"\s+(\d+)\s+([-+.0-9eE]+)\s+([-+.0-9eE]+)\s+([-+.0-9eE]+)\s+([-+.0-9eE]+)\s+"([^"]+)"\s+"([^"]+)"\s*(.*)\s*$)delim";
    std::regex reg(rex);
    std::smatch m;
    if (std::regex_match(line,m, reg)){
        const bool hasFolder=(m.size() == NGROUPS);
...
public:
    enum HistoTypes{UNKNOWN,TH1,TPROFILE,TH2,NTYPES};
    HDefContentHandler(std::vector<SingleHistogramDefinition>  & defs):m_histoType(UNKNOWN),m_numberFilled(0),m_currentDefinition{},m_definitions(defs),m_currentText(){
        //nop
    }
...
private:
    ...
    void setHistoType(const xercesc::Attributes & attrs){
        m_histoType= UNKNOWN;
        const XMLCh * val(nullptr);
        ...
        if (val==0) return;
        if (val==TH1Str) m_histoType = TH1;
        if (val==TProfileStr) m_histoType = TPROFILE;
        if (val==TH2Str) m_histoType = TH2;
    }
