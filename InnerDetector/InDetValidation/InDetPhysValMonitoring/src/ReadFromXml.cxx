...
std::string ReadFromXml::source() const {
    return m_source;
}

std::string ReadFromXml::format() const {
    return m_format;
}

bool ReadFromXml::histoDefinitionMap(std::map<std::string, SingleHistogramDefinition> & usersmap) const{
    bool ok(true);
...
bool ReadFromXml::insertDefinition(const SingleHistogramDefinition & /*oneDefinition*/){
    bool ok(true);
    //m_vectorOfDefinitions.push_back(oneDefinition);
    return ok;
}

bool ReadFromXml::initialize() {
    bool ok(true);
...
bool ReadFromXml::sourceExists() const {
    struct stat buffer;
