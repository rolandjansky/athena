...
InDetBasicPlot::fill(const xAOD::TruthParticle& particle){
  ...
  ++m_numCallsToFillTruth;
	for (const auto & p:m_paramNames){
      ...
      if ((i==0) and thisParameterValue==0.) ++m_d0IsExactlyZeroInTruthCounter;
...
InDetBasicPlot::fill(const xAOD::TrackParticle& particle){
  ...
  if (trkParticleParams[D0]==0.) ++m_d0IsExactlyZeroInTrackCounter;
...
InDetBasicPlot::finalizePlots(){
 	ATH_MSG_INFO("Number of exact zero values for d0 in tracks: "<<m_d0IsExactlyZeroInTrackCounter<<" out of "<<m_numCallsToFillTrack);
 	ATH_MSG_INFO("Number of exact zero values for d0 in truth: "<<m_d0IsExactlyZeroInTruthCounter<<" out of "<<m_numCallsToFillTruth);
 }
