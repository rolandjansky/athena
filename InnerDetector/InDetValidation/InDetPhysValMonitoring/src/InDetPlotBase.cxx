...
InDetPlotBase::book (TH1 * & pHisto, const SingleHistogramDefinition & hd){
  pHisto=Book1D(hd.name,hd.title,hd.nBinsX,hd.xAxis.first,hd.xAxis.second, false);
  return;
}

void
InDetPlotBase::book (TProfile * & pHisto, const SingleHistogramDefinition & hd){
  pHisto=BookTProfile(hd.name,hd.title,hd.nBinsX,hd.xAxis.first,hd.xAxis.second, false);
  return;
}
