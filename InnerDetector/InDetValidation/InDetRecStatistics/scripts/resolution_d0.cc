...
// File: $Id: resolution_d0.cc,v 1.9 2008-07-23 20:51:23 adye Exp $
// file      : d0_resolution.cc
// author    : Sven Vahsen, March 2006
// synoposis : Root script to be used with TTree generated by InDetRecStatistics. 
//             Performs gaussian fits for d0-resolution in different eta region, 
                    ...
                    const int ntrack_finders=3, const TCut data_cut="", double match=-1)
{
  ...
  sprintf(resolutiontitle,"d0 %s vs eta",resstring.c_str());
  ...
  for (int i=0; i<ntrack_finders; i++) {
    resname         [i] = resstring + string("_d0_eta_") + name[i];
	...
	sprintf (eta_bin_cut, "track_eta >= %f && track_eta < %f && %s",eta_low, eta_high, cut_track_preselection.GetTitle());
      ...
      resolution_fits [eta_bin] = new TH1F(hist_name, eta_bin_cut, n_d0_fit_bins, fithisto_min, fithisto_max);

      if (pull) {
	tree[i]->Project(hist_name, "(track_d0-track_truth_d0)/track_error_d0", eta_bin_cut);
      } else {
	tree[i]->Project(hist_name, "track_d0-track_truth_d0", eta_bin_cut);
      }
	...
	float sigma_d0           = func->GetParameter(2);
	float error_on_sigma_d0  = func->GetParError(2);
	if (pull) {
	  resolution[i]->SetBinContent (1+eta_bin, sigma_d0);
	  resolution[i]->SetBinError   (1+eta_bin, error_on_sigma_d0);
	} else {
          resolution[i]->SetBinContent (1+eta_bin, 1000*sigma_d0);
          resolution[i]->SetBinError   (1+eta_bin, 1000*error_on_sigma_d0);
	}          
      }
	...
	resolution[i]->GetXaxis()->SetTitle("|#eta|"); 
      ...
      if(pull) {
        resolution[i]->GetYaxis()->SetTitle("Pull(d0)");
      } else {
        resolution[i]->GetYaxis()->SetTitle("#sigma(d0) (#mu m)");
      }
