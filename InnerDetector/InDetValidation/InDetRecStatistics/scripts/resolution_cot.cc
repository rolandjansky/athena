...
// File: $Id: resolution_cot.cc,v 1.9 2008-07-23 20:51:23 adye Exp $
// file      : cot_resolution.cc
// author    : Sven Vahsen, March 2006
// synoposis : Root script to be used with TTree generated by InDetRecStatistics. 
//             Performs gaussian fits for cot-resolution in different eta region, 
                     ...
                     const int ntrack_finders=3, const TCut data_cut="", double match=-1)
{
  ...
  cout << "Generating cot(theta) " << resstring << " vs eta plots..." << endl;

  // -------------- parameters ---------------------
  const int n_eta_bins           = int(10*etamax/2.5)*(1+(!abseta));
  const int n_canvas_columns     = 4;
  const int n_cot_fit_bins       = 40;
  ...
  sprintf(resolutiontitle,"cot(#theta) %s vs eta",resstring.c_str());
  ...
  TLegend* leg_eff;
  if (abseta) {
    leg_eff = new TLegend(0.15,0.85,0.44,0.54); 
  } else {
    leg_eff = new TLegend(0.35,0.85,0.64,0.54);
  }

  // loop over the three trackfinders
  for (int i=0; i<ntrack_finders; i++) {
    resname         [i] = resstring + string("_cot_eta_") + name[i];
    ...
    for (int eta_bin = 0; eta_bin < n_eta_bins; eta_bin++)
    {
      ...
      //      float eta_bin_center = eta_binsize * (eta_bin + 0.5);
      ...
      resolution_fits [eta_bin] = new TH1F(hist_name, eta_bin_cut, n_cot_fit_bins, fithisto_min, fithisto_max);
      if(pull) {
        tree[i]->Project(hist_name, "(track_cot-track_truth_cot)/track_error_cot", eta_bin_cut);
      } else {
        tree[i]->Project(hist_name, "track_cot-track_truth_cot", eta_bin_cut);
      }
	...
	float sigma_cot           = func->GetParameter(2);
	float error_on_sigma_cot  = func->GetParError(2);
	resolution[i]->SetBinContent (1+eta_bin, sigma_cot);
	resolution[i]->SetBinError   (1+eta_bin, error_on_sigma_cot);
      }
	...
	resolution[i]->GetXaxis()->SetTitle("|#eta|"); 
      ...
      if(pull) { 
        resolution[i]->GetYaxis()->SetTitle("Pull(cot #theta)"); 
      } else {
        resolution[i]->GetYaxis()->SetTitle("#sigma(cot #theta)");
      }
