...
void SCTBarrelIniLocalPos() {
  ...
  if (iSCTHits>0) {
    ...
    Char_t inFile[180];
    if (NumberOfCPUs>1) sprintf(inFile,"%s/Iter%d/Iter%d_LocalPositions.txt",path,IterNumber,IterNumber);
    else sprintf(inFile,"%s/Iter%d_LocalPositions.txt",path,IterNumber);

    if (inFile==NULL) cout << "No LocalPositions.txt file found!" << endl;
    else {

      TTree *treeSCTPos = new TTree("SCTPositions","SCT Module Position");

      // milimeters and miliradians corrections
      treeSCTPos->ReadFile(inFile,"mcode:Tx:Ty:Tz:Rx:Ry:Rz");
      // treeSCTPos->Scan();

      cSCTBarrelPos = new TCanvas("cSCTBarrelPos","Initial SCT Module Position",1100,600);
	...
	if (mcode<2400000 && mcode>2000000) {
      ...
      cSCTBarrelMeanPos = new TCanvas("cSCTBarrelMeanPos",
					"Initial SCT Module Mean Position Vs Layer",1100,600);
