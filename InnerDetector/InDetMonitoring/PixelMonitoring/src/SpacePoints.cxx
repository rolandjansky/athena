...
StatusCode PixelMainMon::FillSpacePointMon(void)
{
   ...
   for(SpacePointContainer::const_iterator it=m_Pixel_spcontainer->begin(); it!=m_Pixel_spcontainer->end(); ++it)
   {
      ...
      if (!colNext)
      {
         m_storegate_errors->Fill(2.,5.);  //first entry (1). is for SP, second (4) is for data problem
         continue;
      }
   ...
   if(nhits==0) m_storegate_errors->Fill(2.,4.);//first entry for sp, second for size = 0
   return StatusCode::SUCCESS;
}
