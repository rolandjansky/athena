...
#reducedList =[]
#for x in inputFiles[2:5]:
#       reducedList.append(x)

athenaVersion = 'AtlasProduction,20.1.6.4'
nJobs=1
QUEUE = "atlasb1"
...
for itemlist in chunk:
	...
	os.system("mkdir -p batchscripts")
	path = str(testarea)+"InnerDetector/InDetMonitoring/InDetPerformanceMonitoring/"
#	path = "./"

	command = "athena.py "+path+"share/runzmumu_runallmonitoring.py -c \"inputFiles = "+str(itemlist)+"\""
	jobname = "batch_script_"+str(counter)
	# create submission script
	script = open("batchscripts/batch_script_"+str(counter)+".lsf","w")
#	script.write("source /afs/cern.ch/atlas/software/dist/AtlasSetup/scripts/asetup.sh "+str(athenaVersion)+", here \n")
	...
	script.write("#BSUB -o "+str(loc)+"/"+str(counter)+"/"+str(jobname)+".log \n")
	script.write("#BSUB -e "+str(loc)+"/"+str(counter)+"/"+str(jobname)+".err \n")
	...
	script.write("mkdir -p jobs \n")
        script.write("mkdir -p jobs/"+str(QUEUE)+" \n")
        script.write("mkdir -p jobs/"+str(QUEUE)+"/"+str(counter)+" \n")
        script.write("cd jobs/"+str(QUEUE)+"/"+str(counter)+" \n")
	...
	os.system("bsub -q "+str(QUEUE)+" < batchscripts/"+str(jobname)+".lsf")
	counter+=1
