...
//====================================================================================================
StatusCode SCTRatioNoiseMonTool::bookHistogramsRecurrent(){
  m_path= "";
  ...
  if(bookRatioNoiseHistos().isFailure() ) msg(MSG::WARNING) << "Error in bookRatioNoiseHistos()" << endreq;
...
//                       SCTRatioNoiseMonTool :: bookHistograms
//====================================================================================================
StatusCode SCTRatioNoiseMonTool::bookHistograms(){
  m_path= "";
  m_numberOfEvents=0;
  m_dataObjectName = "SCT_RDOs";
  ATH_MSG_DEBUG( "initialize being called");
...
StatusCode SCTRatioNoiseMonTool::fillHistograms(){
  ...
  if( isSelectedTrigger == true ){
    for( ; col_it!= lastCol ; ++col_it){
      ...
      nEta[modNum]= thisEta;  
      nPhi[modNum]= thisPhi;
      if(!goodModule) nNonGoodModule[modNum] = 1;
	...
	oneSideHit=false;
	twoSidesHit=false;
	if(nLink0[j]!=0 && nLink1[j]!=0) twoSidesHit=true;
	else if(nLink0[j]!=0 || nLink1[j]!=0 ) oneSideHit=true;
	else if(nLink0[j]==0 && nLink1[j]==0 && goodModules[j]) noSidesHit=true;
	
	if(isBarrel(j)){
	  if(noSidesHit)  nNoSidesBarrel_ev[nLayer[j]]+=1;
	  if(oneSideHit)  nOneSideBarrel_ev[nLayer[j]]+=1; 
	  if(twoSidesHit) nTwoSideBarrel_ev[nLayer[j]]+=1;
	  if(nNonGoodModule[j]) nNonGoodModulesBarrel_ev[nLayer[j]]+=1;
	}
	if(isEndcapA(j)){
	  if(noSidesHit)  nNoSidesEndcapA_ev[nLayer[j]]+=1;
	  if(oneSideHit)  nOneSideEndcapA_ev[nLayer[j]]+=1; 
	  if(twoSidesHit) nTwoSideEndcapA_ev[nLayer[j]]+=1;
	  if(nNonGoodModule[j]) nNonGoodModulesEndcapA_ev[nLayer[j]]+=1;
	}
	if(isEndcapC(j)){
	  if(noSidesHit)  nNoSidesEndcapC_ev[nLayer[j]]+=1;
	  if(oneSideHit)  nOneSideEndcapC_ev[nLayer[j]]+=1; 
	  if(twoSidesHit) nTwoSideEndcapC_ev[nLayer[j]]+=1;
	  if(nNonGoodModule[j]) nNonGoodModulesEndcapC_ev[nLayer[j]]+=1;
	}
	//--------------------------------------
	...
	int nosidehit = NumModBarrelLayer[l]-nNonGoodModulesBarrel_ev[l]-nOneSideBarrel_ev[l]-nTwoSideBarrel_ev[l];
	...
	int nosidehit = NumModEndcapDisk[m]-nNonGoodModulesEndcapC_ev[m]-nOneSideEndcapC_ev[m]-nTwoSideEndcapC_ev[m];
	...
	int nosidehit = NumModEndcapDisk[q]-nNonGoodModulesEndcapA_ev[q]-nOneSideEndcapA_ev[q]-nTwoSideEndcapA_ev[q];
...
//====================================================================================================
StatusCode  SCTRatioNoiseMonTool::procHistograms(){
  ...
  if(m_environment==AthenaMonManager::online || endOfRunFlag()){
    ...
    m_NOEC_Inner->Reset();
    m_NOEC_Middle->Reset();
    for(unsigned int i=0;i<N_BARRELS;++i) m_NOBAR_layer[i]->Reset();
  ...
  for (int bin=1; bin<NBINS_LBs+1; bin++){
    if(nNoSides_lb_allmod[bin]!=0){
      ...
      if(NOLB!=0) m_NO_vsLB->Fill(bin,NOLB);
    }
    if(noisyM[bin]!=0) m_NoisyModules_vsLB->Fill(bin,noisyM[bin]);
  }
  ...
  m_NOBAR->SetBinContent(m_NOBAR->GetNbinsX(),m_NOBAR->GetBinContent(m_NOBAR->GetNbinsX() ) + m_NOBAR->GetBinContent(m_NOBAR->GetNbinsX() +1));
  m_NOBARSide->SetBinContent(m_NOBARSide->GetNbinsX(),m_NOBARSide->GetBinContent(m_NOBARSide->GetNbinsX() ) + m_NOBARSide->GetBinContent(m_NOBARSide->GetNbinsX() +1));
...
StatusCode SCTRatioNoiseMonTool::bookRatioNoiseHistos(){       // hidetoshi 14.01.22
  if(newRunFlag()){                                          // hidetoshi 14.01.22
    ...
    m_NOBARSide = h1Factory("h_NObSide", "Noise Occupancy Barrel (calculated for each module)",RatioNoise, 0, 150, 500);
    ...
    m_NOECSide = h1Factory("h_NOECSide", "Noise Occupancy Endcap (calculated for each module)",RatioNoise, 0, 150, 500);
    m_NOECSide->SetTitle("Noise Occupancy for Endcaps (Ratio Noise);Noise Occupancy [10^{-5}];Num of Modules");
    m_NOECASide = h1Factory("h_NOECASide", "Noise Occupancy Endcap A (calculated for each module)",RatioNoise, 0, 150, 500);
    m_NOECASide->SetTitle("Noise Occupancy for Endcap A (Ratio Noise);Noise Occupancy [10^{-5}];Num of Modules");
    m_NOECCSide = h1Factory("h_NOECCSide", "Noise Occupancy Endcap C (calculated for each module)",RatioNoise, 0, 150, 500);
    ...
    m_NOEC_ShortMiddle=h1Factory("h_NOEC_ShortMiddle", "Noise Occupancy Endcap Short Middle Modules",RatioNoise, 0, 150, 500);
    ...
    m_NoisyModules_vsLB = pFactory("h_NoisyModules_vsLB", "Number of Noisy Modules vs LB",RatioNoise, 0.5, NBINS_LBs+0.5, NBINS_LBs);
    m_NoisyModules_vsLB->SetTitle("Number of Noisy Modules vs LB for Barrel and Endcaps (Ratio Noise);LB;Number of Noisy Modules");
	...
	for(int l=0;l< n_layers[BARREL_INDEX];l++){
	  ...
	  m_NOEVBAR[l] = h1Factory("h_NOEV_Barrel"+hNumBarrel[l], "Event Noise Occupancy Barrel Layer"+hNumBarrel[l], noiseOccMaps, 0, 150,500);
	  m_NOEVBAR[l]->GetXaxis()->SetTitle("Event Noise Occupancy [10^{-5}]");
	  m_NOEVBAR[l]->GetYaxis()->SetTitle("Events");
	  m_NOBAR_layer[l]=h1Factory("h_NOb_layer"+hNumBarrel[l], "Noise Occupancy Barrel Layer"+hNumBarrel[l],noiseOccMaps, 0, 100, 500);
	  m_NOBAR_layer[l]->GetXaxis()->SetTitle("Noise Occupancy [10^{-5}]");
	  m_NOBAR_layer[l]->GetYaxis()->SetTitle("Num of Modules");
	  m_NOBAR_layer_vsLB[l]=pFactory("h_NOb_layer"+hNumBarrel[l]+"_vsLB", "Noise Occupancy Barrel Layer"+hNumBarrel[l]+" vs LB",noiseOccMaps, 0.5, NBINS_LBs+0.5, NBINS_LBs);
	  ...
	  m_NZ1BAR_vsLB[l]=pFactory("h_NZ1BAR"+hNumBarrel[l]+"_vsLB", "Num of ZeroSide Hits in Barrel Layer"+hNumBarrel[l]+" vs LB", noiseOccHits, 0.5, NBINS_LBs+0.5, NBINS_LBs);
	  m_NZ1BAR_vsLB[l]->GetXaxis()->SetTitle("LumiBlock");
	  m_NZ1BAR_vsLB[l]->GetYaxis()->SetTitle("Num of ZeroSide Hits");
	  m_N11BAR_vsLB[l]=pFactory("h_N11BAR"+hNumBarrel[l]+"_vsLB", "Num of OneSide Hits in Barrel Layer"+hNumBarrel[l]+" vs LB", noiseOccHits, 0.5, NBINS_LBs+0.5, NBINS_LBs);
      ...
      if(systemIndex==0){
	for(int m=0; m<n_layers[ENDCAP_C_INDEX];m++){
	  m_NOEVECC[m] = h1Factory("h_NOEV_EndcapC"+hNumEndcap[m], "Event Noise Occupancy EndcapC Disk"+hNumEndcap[m], noiseOccMaps, 0, 150,500);
	  ...
	  m_NOECC_disk_vsLB[m] = pFactory("h_NOECC_disk"+hNumEndcap[m]+"_vsLB", "Noise Occupancy EndcapC Disk"+hNumEndcap[m]+" vs LB",noiseOccMaps, 0.5, NBINS_LBs+0.5, NBINS_LBs);
	  ...
	  m_NZ1ECC_vsLB[m]=pFactory("h_NZ1ECC"+hNumEndcap[m]+"_vsLB", "Num of ZeroSide Hits in EndcapC Disk"+hNumEndcap[m]+" vs LB", noiseOccHits, 0.5, NBINS_LBs+0.5, NBINS_LBs);
	  m_NZ1ECC_vsLB[m]->GetXaxis()->SetTitle("LumiBlock");  
	  m_NZ1ECC_vsLB[m]->GetYaxis()->SetTitle("Num of ZeroSide Hits"); 
	  m_N11ECC_vsLB[m]=pFactory("h_N11ECC"+hNumEndcap[m]+"_vsLB", "Num of OneSide Hits in EndcapC Disk"+hNumEndcap[m]+" vs LB", noiseOccHits, 0.5, NBINS_LBs+0.5, NBINS_LBs);
	...
	for(int p=0; p<n_layers[ENDCAP_A_INDEX];p++){
	  m_NOEVECA[p] = h1Factory("h_NOEV_EndcapA"+hNumEndcap[p], "Event Noise Occupancy EndcapA Disk"+hNumEndcap[p], noiseOccMaps, 0, 150,500);
	  ...
	  m_NOECA_disk_vsLB[p]=pFactory("h_NOECA_disk"+hNumEndcap[p]+"_vsLB", "Noise Occupancy Barrel Disk"+hNumEndcap[p]+" vs LB",noiseOccMaps, 0.5, NBINS_LBs+0.5, NBINS_LBs);
	  ...
	  m_NZ1ECA_vsLB[p]=pFactory("h_NZ1ECA"+hNumEndcap[p]+"_vsLB", "Num of ZeroSide Hits in EndcapA Disk"+hNumEndcap[p]+" vs LB", noiseOccHits, 0.5, NBINS_LBs+0.5, NBINS_LBs);
	  m_NZ1ECA_vsLB[p]->GetXaxis()->SetTitle("LumiBlock");  
	  m_NZ1ECA_vsLB[p]->GetYaxis()->SetTitle("Num of ZeroSide Hits");
	  m_N11ECA_vsLB[p]=pFactory("h_N11ECA"+hNumEndcap[p]+"_vsLB", "Num of OneSide Hits in EndcapA Disk"+hNumEndcap[p]+" vs LB", noiseOccHits, 0.5, NBINS_LBs+0.5, NBINS_LBs);
...
SCTRatioNoiseMonTool::pFactory(const std::string & name, const std::string & title, MonGroup & registry, const float lo, const float hi, const unsigned int nbins){
