...
SCT_ConditionsSummarySvc::initialize() {
  ...
  if (m_noReports) {
    ...
    ATH_MSG_INFO("No services were selected for the SCT summary");
  } else {
    bool successfulRetrieve{true};
    for (const string pSvcName: m_reportingServices.value()) {
      ServiceHandle<ISCT_ConditionsSvc> thisSvc{pSvcName, name()};
      sc = thisSvc.retrieve();
      if (sc.isSuccess()) {
        ATH_MSG_INFO("Using " << pSvcName);
        m_svcHandles.push_back(thisSvc);
      } else {
        ATH_MSG_ERROR("The service " << pSvcName << " is not available.");
...
SCT_ConditionsSummarySvc::finalize() {
  ATH_MSG_INFO("Thank-you for using the SCT_ConditionsSummarySvc, version " << PACKAGE_VERSION);
