...
#include "src/PixelSensorGmxSD.h"
...
class PixelSensorGmxSDtest : public ::testing::Test {	
  protected:
    virtual void SetUp() override {
    }
...
TEST_F( PixelSensorGmxSDtest, Initialize )
{
  G4HCofThisEvent hce;
  PixelSensorGmxSD sd1("name1", "name1" );
  sd1.Initialize( &hce );
  ASSERT_TRUE(sd1.m_HitColl.isValid()); //test if the initialization of the smart pointer m_HitColl is successful
}

TEST_F( PixelSensorGmxSDtest, ProcessHits )
{
  G4Step sp;
  ...
  PixelSensorGmxSD sd2("name2", "name2");
  sd2.Initialize( &hce );
...
TEST_F( PixelSensorGmxSDtest, AddHit )
{
  ...
  int phiMod = 0;
  int side = 0;
  PixelSensorGmxSD sd3("name3", "name3");
  G4HCofThisEvent hce;
  sd3.Initialize( &hce );
