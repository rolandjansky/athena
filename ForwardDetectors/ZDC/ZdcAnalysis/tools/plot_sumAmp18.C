...
void plot_sumAmp18(int side = 0)
{
    gStyle->SetOptStat(0);

    TChain *chain = new TChain("zdcTree");;

    chain->Add("out18_FEL_twoPass_fixTaus_ECalib_6M.root");

    float zdc_SumCalibAmp[2];
    int   zdc_ModuleMask = 0;
    chain->SetBranchAddress("zdc_SumCalibAmp", &zdc_SumCalibAmp);
    chain->SetBranchAddress("zdc_ModuleMask", &zdc_ModuleMask);
    Bool_t L1_ZDC_C;
    chain->SetBranchAddress("L1_ZDC_C", &L1_ZDC_C);

    TH1F* h0 = new TH1F("h0", "h0", 100, 0, 10);

    if (side == 0) chain->Draw("zdc_SumCalibAmp[0]/1000.>>h0", "L1_ZDC_A && (CountBits(zdc_ModuleMask&0xf)>1)");
    if (side == 1) chain->Draw("zdc_SumCalibAmp[1]/1000.>>h0", "L1_ZDC_C && (CountBits(zdc_ModuleMask>>4)>1)" );
    ...
    if (side == 0) leg->AddEntry("", "Trigger on A side", "");     // -------------
    if (side == 1) leg->AddEntry("", "Trigger on C side", "");     // -------------

    float peakpos = 2.5;

    TF1* fx_0 = new TF1("total", Nnutrons2, fitMin, fitMax, 7);
    fx_0->SetParName  (0, "Mean");
    fx_0->SetParameter(0, peakpos);
    fx_0->SetParLimits(0, 0.5 * peakpos, 1.5 * peakpos);
    // fx_0->SetParLimits(0, 0.8 * peakpos, 1.2 * peakpos);
    ...
    fx_0->SetParameter(2, 150000);
    fx_0->SetParLimits(2, 0, 200000);
    fx_0->SetParName  (3, "Constant_2");
    fx_0->SetParameter(3, 2000);
    fx_0->SetParLimits(3, 0, 100000);
    fx_0->SetParName  (4, "Constant_3");
    fx_0->SetParameter(4, 600);
    ...
    h0->SetMinimum(0);
    if (side == 0) leg->AddEntry(h0, "Side C (0,12)", "L");           // -------------
    if (side == 1) leg->AddEntry(h0, "Side A (1,81)", "L");           // -------------
    leg->AddEntry(fx_0, Form("#sigma/#mu = %f", resolution), "L");
    h0->SetTitle(";Energy [TeV];Counts;");
