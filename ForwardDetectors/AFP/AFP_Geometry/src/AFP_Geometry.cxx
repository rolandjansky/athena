...
HepGeom::Transform3D AFP_Geometry::GetStationTransform(const char* pszStationName)
{
	...
	{
	case EAS_AFP00:
		ReqTransform=HepGeom::Translate3D(LHCXOFFSET,0.0*CLHEP::mm,m_CfgParams.fLongHBZPos);
		break;
	case EAS_AFP01:
		ReqTransform=HepGeom::Translate3D(LHCXOFFSET, 0.0*CLHEP::mm, m_CfgParams.fShortHBZPos);
		break;
	case EAS_AFP02:
		ReqTransform=HepGeom::Translate3D(LHCXOFFSET,0.0*CLHEP::mm,-m_CfgParams.fShortHBZPos);
		break;
...
HepGeom::Transform3D AFP_Geometry::GetStationElementTransform(const char* pszStationName, eStationElement eElement)
{
	...
	{
	case EAS_AFP00:
		if(eElement==ESE_HBP) ReqTransform=HepGeom::Translate3D(m_CfgParams.fLongHBFloorXPos,m_CfgParams.fLongHBYPos,0.0*CLHEP::mm);
		else if(eElement==ESE_TD1) ReqTransform=HepGeom::Translate3D(m_CfgParams.fLongHBFloorXPos, m_CfgParams.fLongHBYPos+m_CfgParams.fQuarticYPosInTube[0], m_CfgParams.fQuarticZPosInTube[0]);
		else if(eElement==ESE_TD2) ReqTransform=HepGeom::Translate3D(m_CfgParams.fLongHBFloorXPos, m_CfgParams.fLongHBYPos+m_CfgParams.fQuarticYPosInTube[1], m_CfgParams.fQuarticZPosInTube[1]);
		...
		break;
	case EAS_AFP01:
		if(eElement==ESE_HBP) ReqTransform=HepGeom::Translate3D(m_CfgParams.fShortHBFloorXPos,m_CfgParams.fShortHBYPos,0.0*CLHEP::mm);
		else if(eElement==ESE_SID) ReqTransform=HepGeom::Translate3D(m_CfgParams.fShortHBFloorXPos,m_CfgParams.fShortHBYPos,0.0*CLHEP::mm);
		else {};
		break;
	case EAS_AFP02:
		if(eElement==ESE_HBP) ReqTransform=HepGeom::Translate3D(m_CfgParams.fShortHBFloorXPos,m_CfgParams.fShortHBYPos,0.0*CLHEP::mm)*HepGeom::Reflect3D(0.0,0.0,1.0,0.0);
		else if(eElement==ESE_SID) ReqTransform=HepGeom::Translate3D(m_CfgParams.fShortHBFloorXPos,m_CfgParams.fShortHBYPos,0.0*CLHEP::mm)*HepGeom::Reflect3D(0.0,0.0,1.0,0.0);
		else {};
		break;
	case EAS_AFP03:
		if(eElement==ESE_HBP) ReqTransform=HepGeom::Translate3D(m_CfgParams.fLongHBFloorXPos,m_CfgParams.fLongHBYPos,0.0*CLHEP::mm)*HepGeom::Reflect3D(0.0,0.0,1.0,0.0);
...
HepGeom::Transform3D AFP_Geometry::GetSIDTransform(const eSIDTransformType eType, const char* pszStationName, const int nPlateIndex)
{
	...
	HepGeom::Transform3D NominalPosInPocket=HepGeom::Translate3D(DETXSIDE*m_CfgParams.sidcfg.fDistanceFromPocket,0.0*CLHEP::mm,+fzm-fZCorrOffset);
	...
	if(eType==ESTT_SUPPORT){
		...
		case ESTT_VACUUMSENSOR:
			...
			else
			{
				...
				{
					if(eStation==EAS_AFP00) ReqTransform=HepGeom::TranslateZ3D(-SID_LHB_GLOBALVACUUMSENSORZOFFSET);
					else if(eStation==EAS_AFP01) ReqTransform=HepGeom::TranslateZ3D(-SID_SHB_GLOBALVACUUMSENSORZOFFSET);
					else if(eStation==EAS_AFP02) ReqTransform=HepGeom::TranslateZ3D(SID_SHB_GLOBALVACUUMSENSORZOFFSET);
					else if(eStation==EAS_AFP03) ReqTransform=HepGeom::TranslateZ3D(SID_LHB_GLOBALVACUUMSENSORZOFFSET);
				}
			}
			break;
		default:
