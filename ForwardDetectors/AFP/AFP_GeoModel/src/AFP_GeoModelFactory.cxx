...
void AFP_GeoModelFactory::create(GeoPhysVol *world)
{
...
#else
	pBSContainer->reserve(TDQUARTICCOLSCNT*TDQUARTICROWSCNT);
#endif
	...
	const GeoBox* pBoxLongEnv= new GeoBox(150*mm, 150*CLHEP::mm, 580*CLHEP::mm);
	...
	PosElementInEnv=m_pGeometry->GetStationElementTransform("AFP00",ESE_HBP); //HepGeom::Translate3D(m_CfgParams.fLongHBFloorXPos,m_CfgParams.fLongHBYPos,0.0*CLHEP::mm);
	if(!m_CfgParams.hbpcfg.windowPlatesInsteadOfHB) AddLongHamburgPipe(pPhysLongEnv,"AFP00",PosElementInEnv);
	else AddLongWindowPlates(pPhysLongEnv,"AFP00",PosElementInEnv);

	//add Timing detector - Q1
	PosElementInEnv=HepGeom::Translate3D(m_CfgParams.fLongHBFloorXPos, m_CfgParams.fLongHBYPos+m_CfgParams.fQuarticYPosInTube[0], m_CfgParams.fQuarticZPosInTube[0]);
	SCode=AddTimingDetector(0,pPhysLongEnv,"AFP00",PosElementInEnv, pBSContainer);

	//add Timing detector - Q2
	if(m_CfgParams.bAddSecondQuartic){
		PosElementInEnv=HepGeom::Translate3D(m_CfgParams.fLongHBFloorXPos, m_CfgParams.fLongHBYPos+m_CfgParams.fQuarticYPosInTube[1], m_CfgParams.fQuarticZPosInTube[1]);
		SCode=AddTimingDetector(1,pPhysLongEnv,"AFP00",PosElementInEnv, pBSContainer);
	}
	...
	PosElementInEnv=m_pGeometry->GetStationElementTransform("AFP01",ESE_HBP);
	if(m_CfgParams.bIsSHBRPotMode==true){
		AddRomanPot(pPhysShortEnv,"AFP01",PosElementInEnv);
	}
	else{
		if(!m_CfgParams.hbpcfg.windowPlatesInsteadOfHB) AddShortHamburgPipe(pPhysShortEnv,"AFP01",PosElementInEnv);
		else AddShortWindowPlates(pPhysShortEnv,"AFP01",PosElementInEnv);
	}
	...
	PosElementInEnv=m_pGeometry->GetStationElementTransform("AFP02",ESE_HBP); //HepGeom::Translate3D(m_CfgParams.fShortHBFloorXPos,m_CfgParams.fShortHBYPos,0.0*CLHEP::mm)*HepGeom::Reflect3D(0.0,0.0,1.0,0.0);
	if(m_CfgParams.bIsSHBRPotMode==true){
		AddRomanPot(pPhysShortEnv1,"AFP02",PosElementInEnv);
	}
	else {
		if(!m_CfgParams.hbpcfg.windowPlatesInsteadOfHB) AddShortHamburgPipe(pPhysShortEnv1,"AFP02",PosElementInEnv);
		else AddShortWindowPlates(pPhysShortEnv1,"AFP02",PosElementInEnv);
	}
	...
	const GeoBox* pBoxLongEnv1= new GeoBox(150*mm, 150*CLHEP::mm, 580*CLHEP::mm);
	...
	PosElementInEnv=m_pGeometry->GetStationElementTransform("AFP03",ESE_HBP); //HepGeom::Translate3D(m_CfgParams.fLongHBFloorXPos,m_CfgParams.fLongHBYPos,0.0*CLHEP::mm)*HepGeom::Reflect3D(0.0,0.0,1.0,0.0);
	if(!m_CfgParams.hbpcfg.windowPlatesInsteadOfHB) AddLongHamburgPipe(pPhysLongEnv1,"AFP03",PosElementInEnv);
	else AddLongWindowPlates(pPhysLongEnv1,"AFP03",PosElementInEnv);
	...
	SCode=AddTimingDetector(0,pPhysLongEnv1,"AFP03",PosElementInEnv, pBSContainer);

	//add Timing detector - Q2
	if(m_CfgParams.bAddSecondQuartic){
		PosElementInEnv=HepGeom::Translate3D(m_CfgParams.fLongHBFloorXPos, m_CfgParams.fLongHBYPos+m_CfgParams.fQuarticYPosInTube[1], -m_CfgParams.fQuarticZPosInTube[1])*HepGeom::Reflect3D(0.0,0.0,1.0,0.0);
		SCode=AddTimingDetector(1,pPhysLongEnv1,"AFP03",PosElementInEnv, pBSContainer);
	}
