...
void AFP_GeoModelFactory::addSiDetector(GeoPhysVol* pPhysMotherVol, const char* pszStationName, HepGeom::Transform3D& TransInMotherVolume)
{
	...
	for(int i=0;i<m_pGeometry->getSIDPlatesCnt(eStation);i++)
	{
		...
		pPhysMotherVol->add(new GeoNameTag(szLabel));
		pPhysMotherVol->add(new GeoTransform(TransInMotherVolume*m_pGeometry->getSIDTransform(ESTT_PLATE,pszStationName,i)));
		...
		pPhysMotherVol->add(new GeoNameTag(szLabel));
		pPhysMotherVol->add(new GeoTransform(TransInMotherVolume*m_pGeometry->getSIDTransform(ESTT_FEI4CHIP,pszStationName,i)));
		pPhysMotherVol->add(pPhysElement);
		...
		pPhysMotherVol->add(new GeoNameTag(szLabel));
		pPhysMotherVol->add(new GeoTransform(TransInMotherVolume*m_pGeometry->getSIDTransform(ESTT_SENSOR,pszStationName,i)));
		pPhysMotherVol->add(pPhysElement);
...
GeoShape* AFP_GeoModelFactory::createSolidSIDPlate()
{
	...
	//cut side edges
	double fd=sqrt(AfpConstants.SiT_Plate_CutEdge_length_x*AfpConstants.SiT_Plate_CutEdge_length_x+AfpConstants.SiT_Plate_CutEdge_length_y*AfpConstants.SiT_Plate_CutEdge_length_y);
	double falpha=atan(AfpConstants.SiT_Plate_CutEdge_length_y/AfpConstants.SiT_Plate_CutEdge_length_x);
	double fgamma=45.0*CLHEP::deg-falpha; double fbeta=90.0*CLHEP::deg-falpha;

	//--lower cut
	HepGeom::Vector3D<double> vecL=(fd/sqrt(2.0))*(HepGeom::RotateZ3D(-fgamma)*HepGeom::Vector3D<double>(1.0,0.0,0.0));
	...
	GeoShapeShift* pShiftCut=new GeoShapeShift(pPlateCut, TransCut);
	GeoShapeSubtraction* pShape1=new GeoShapeSubtraction(pMainPlate, pShiftCut);

	//--upper cut
	vecL=(fd/sqrt(2.0))*(HepGeom::RotateZ3D(+fgamma)*HepGeom::Vector3D<double>(1.0,0.0,0.0));
	vecX=vecL-vecB;
	...
	pShiftCut=new GeoShapeShift(pPlateCut,TransCut);
