...
{
  if (c->r2) throw std::runtime_error("Error in GeoStraightAccSection: Illegal Modification Sequence");
  if (!c->r1) c->buildFastCache();
  return c->r1->xcent[stackid][cellid];
}
double & GeoStraightAccSection::YCent(int stackid, int cellid) 
{
  if (c->r2) throw std::runtime_error("Error in GeoStraightAccSection: Illegal Modification Sequence");
  if (!c->r1) c->buildFastCache();
  return c->r1->ycent[stackid][cellid];
}
double & GeoStraightAccSection::Cosu(int stackid, int cellid)
{
  if (c->r2) throw std::runtime_error("Error in GeoStraightAccSection: Illegal Modification Sequence");
  if (!c->r1) c->buildFastCache();
  return c->r1->cosu[stackid][cellid];
}
double & GeoStraightAccSection::Sinu(int stackid, int cellid)
{
  if (c->r2) throw std::runtime_error("Error in GeoStraightAccSection: Illegal Modification Sequence");
  if (!c->r1) c->buildFastCache();
  return c->r1->sinu[stackid][cellid];
}
double  & GeoStraightAccSection::HalfLength(int stackid, int cellid)
{
  if (c->r2) throw std::runtime_error("Error in GeoStraightAccSection: Illegal Modification Sequence");
  if (!c->r1)c->buildFastCache();
  return c->r1->halfLength[stackid][cellid];
}


void GeoStraightAccSection::setHalfLength(int stackid, double halfLength) {
  if (c->r1) throw std::runtime_error("Error in GeoStraightAccSection: Illegal Modification Sequence");
  if (!c->r2) c->buildSmallCache();
  c->r2->halfLength[stackid]=halfLength;
}

void GeoStraightAccSection::setTransform(int stackid, GeoXF::TRANSFUNCTION f) {
  if (c->r1) throw std::runtime_error("Error in GeoStraightAccSection: Illegal Modification Sequence");
  if (!c->r2) c->buildSmallCache();
  delete c->r2->transfunction[stackid];
  c->r2->transfunction[stackid]=f.clone();
}

GeoStraightAccSection::GeoStraightAccSection():c(new Clockwork) {
  c->r1=NULL;
  c->r2=NULL;
}	

GeoStraightAccSection::~GeoStraightAccSection() {
  if (c->r2) for (int i=0;i<14;i++) delete c->r2->transfunction[i];
  delete c->r2;
  delete c->r1;
  delete c;
}
