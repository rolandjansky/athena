...
LArG4H62004CalibSD::~LArG4H62004CalibSD() 
{
  ...
  {
    MsgStream log(m_msgSvc, "LArG4H62004CalibSD");
...
//    MsgStream log(msgSvc(), "LArG4H62004CalibSD::Initialize");
...
{
//   MsgStream log(m_msgSvc, "LArG4H62004CalibSD::ProcessHits");
...
{
//   MsgStream log(m_msgSvc, "LArG4H62004CalibSD::SpecialHit");
#ifdef DEBUG_SD
  ...
  for(unsigned int i=0; i<m_hitMergers.size(); ++i)
  {
    ...
    if(_hitMerger && (!_hitMerger->process(ident,a_energies)))
      std::cout << MSG::DEBUG << "Wrong Identifier after conversion from LArG4Identifier" 
	  << std::string(ident) << std::endl;
  }
  return true;
}
