...
void LArG4H6BeamSD::EndOfEvent(G4HCofThisEvent* /*HCE*/)
{
   ...
   MsgStream log(m_msgSvc, "LArG4H6BeamSD");
   StatusCode status = m_storeGate->record(m_FrontCollection,collectionName[0],false);
  if (status.isFailure()) {
         log << MSG::ERROR << "Failed to record LArG4H6FrontHitCollection  in StoreGate!" << endreq;
  }
  status = m_storeGate->setConst(m_FrontCollection);
  if (status.isFailure()) { 
         log  << MSG::ERROR << "Failed to lock LArG4H6BeamHitCollection  in StoreGate!" << endreq;
  }
