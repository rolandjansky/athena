...
LArG4EMECSDTool::LArG4EMECSDTool(const std::string& type, const std::string& name, const IInterface *parent)
...
//  m_bibSD->EndOfAthenaEvent( &*m_HitColl );
  ...
  if (m_useFrozenShowers){
    LArG4SimpleSD * fastSD = dynamic_cast<LArG4SimpleSD*>( G4SDManager::GetSDMpointer()->FindSensitiveDetector("EndcapFastSimDedicatedSD") );
    if (fastSD){ 
