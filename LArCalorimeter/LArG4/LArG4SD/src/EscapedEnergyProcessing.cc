                                         ...
                                         G4double a_energy )
{
  ...
  static CalibrationSensitiveDetector* defaultSD = 0;
  if ( defaultSD == 0 )
    {
      defaultSD =
        dynamic_cast<CalibrationSensitiveDetector*>( CaloG4::CalibrationDefaultProcessing::GetDefaultSD() );

      // If we've made a mistake, abort here (rather than have a
      // segmentation fault).
      if ( defaultSD == 0 )
        {
          G4cerr << "LArG4::EscapedEnergyProcessing::Process - "
                 << " No appropriate default calculator"
                 << G4endl;
      ...
      if ( sensitiveDetector != 0 )
        {
          ...
          LArG4MultSD* larG4MultSD = dynamic_cast<LArG4MultSD*>(sensitiveDetector);

          // Most probably this is a LArG4MultSD
          if(larG4MultSD !=0)
            {
              G4bool found = false;
              for(unsigned int i=0; i<larG4MultSD->size(); i++)
                {
...
#endif
                  defaultSD->SpecialHit( fakeStep, energies );
                  return false; // error
                }// - !found (1)
            }
          else // larG4MultSD !=0
            {
              ...
              if(larG4CalibSD!=0)
              ...
              else // larG4CalibSD!=0
                {
                  ...
                  CalibrationSensitiveDetector* calibSD
                    = dynamic_cast<CalibrationSensitiveDetector*>(sensitiveDetector);
