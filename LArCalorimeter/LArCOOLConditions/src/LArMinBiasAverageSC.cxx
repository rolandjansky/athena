...
// retrieving LArMinBiasAverage using offline ID  
const float& LArMinBiasAverageSC::minBiasAverage(const Identifier& cellID) const  {
  const HWIdentifier hwid=m_scCablingTool->createSignalChannelID(cellID);
  return LArMinBiasAverageSC::minBiasAverage(hwid);
}
