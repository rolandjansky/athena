...
*/
/**
 * @file LArCondTPCnv/test/LArAutoCorrSubsetCnv_p1_test.cxx
 * @author scott snyder <snyder@bnl.gov>
 * @date Jan, 2020
 * @brief Tests for LArAutoCorrSubsetCnv_p1.
 */
...
typedef LArConditionsSubset<LArAutoCorrP1> TransType;


void compare (const LArAutoCorrP1& p1,
              const LArAutoCorrP1& p2)
{
  assert (p1.m_vAutoCorr == p2.m_vAutoCorr);
}
...
void testit (const TransType& trans1)
{
  ...
  LArAutoCorrSubsetCnv_p1 cnv;
  LArAutoCorrSubset_p1 pers;
...
LArAutoCorrP1 makePayload (int x)
{
  return LArAutoCorrP1 (std::vector<float> { 1.5f+x, 2.5f+x, 3.5f+x,
                     4.5f+x, 5.5f+x });
}


LArAutoCorrP1 makeEmpty (int)
{
  return LArAutoCorrP1();
}


typedef LArAutoCorrP1 payloadMaker_t (int x);
...
{
  std::cout << "LArCondTPCnv/LArAutoCorrSubsetCnv_p1_test\n";
  test1();
  return 0;
}
