...
*/
/**
 * @file LArCondTPCnv/test/LArCaliWaveSubsetCnv_p1_test.cxx
 * @author scott snyder <snyder@bnl.gov>
 * @date Jan, 2020
 * @brief Tests for LArCaliWaveSubsetCnv_p1.
 */
...
void compare (const TransType& p1, const TransType& p2)
{
  ...
  while (cit1 != p1.correctionVecEnd() &&
         cit2 != p2.correctionVecEnd())
  {
    if (!(*cit1).second.empty()) {
  ...
  while (cit1 != p1.correctionVecEnd() && (*cit1).second.empty()) {
    ++cit1;
  }
  assert (cit1 == p1.correctionVecEnd());
  assert (cit2 == p2.correctionVecEnd());
}
...
void testit (const TransType& trans1)
{
  ...
  LArCaliWaveSubsetCnv_p1 cnv;
  LArCaliWaveSubset_p1 pers;
...
{
  std::cout << "LArCondTPCnv/LArCaliWaveSubsetCnv_p1_test\n";
  test1();
  return 0;
}
