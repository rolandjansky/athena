...
*/
/**
 * @file LArCondTPCnv/test/LArShapeSubsetCnv_p2_test.cxx
 * @author scott snyder <snyder@bnl.gov>
 * @date Jan, 2020
 * @brief Tests for LArShapeSubsetCnv_p2.
 */
...
typedef LArConditionsSubset<LArShapeP2> TransType;


void compare (const LArShapeP2& p1,
              const LArShapeP2& p2)
{
  assert (p1.timeOffset() == p2.timeOffset());
  assert (p1.timeBinWidth() == p2.timeBinWidth());

  assert (p1.shapeSize() == p2.shapeSize());
  for (size_t i = 0; i < p1.shapeSize(); i++) {
    assert (p1.shape(i).asVector() == p2.shape(i).asVector());
  }

  assert (p1.shapeDerSize() == p2.shapeDerSize());
  for (size_t i = 0; i < p1.shapeDerSize(); i++) {
    assert (p1.shapeDer(i).asVector() == p2.shapeDer(i).asVector());
  }
}
...
void testit (const TransType& trans1)
{
  ...
  LArShapeSubsetCnv_p2 cnv;
  LArShapeSubset_p2 pers;
...
LArShapeP2 makePayload (int x)
{
  ...
  return LArShapeP2 (2.5, 0.5, 3, 5, v1, v2, 0);
}


LArShapeP2 makeEmpty (int)
{
  return LArShapeP2();
}


typedef LArShapeP2 payloadMaker_t (int x);
...
{
  std::cout << "LArCondTPCnv/LArShapeSubsetCnv_p2_test\n";
  test1();
  return 0;
}
