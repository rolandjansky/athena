...
*/
/**
 * @file LArCondAthenaPool/test/LArRampCompleteCnv_test.cxx
 * @author scott snyder <snyder@bnl.gov>
 * @date Jan, 2020
 * @brief Tests for LArRampCompleteCnv (partial).
 */


#undef NDEBUG
#include "../src/LArRampCompleteCnv.cxx"
...
typedef LArConditionsSubset<LArRampP>  OldType;
typedef LArConditionsSubset<LArRampP1> NewType;
const size_t nfeb = 10;


class TestCnv
  : public LArRampCompleteCnv
{
public:
  using LArRampCompleteCnv::LArRampCompleteCnv;
  using LArRampCompleteCnv::createTransient;
};
...
OldType::Payload makePayload (int x)
{
  ...
  payload.m_vRamp.assign ({ 1.5f+x, 2.5f+x, 3.5f+x,
                            4.5f+x, 5.5f+x });
  return payload;
}
...
void checkPayload (int x, const NewType::Payload& newPayload)
{
  ...
  assert (oldPayload.m_vRamp == newPayload.m_vRamp);
}
...
int main()
{
  ...
  std::cout << "LArCondAthenaPool/LArRampCompleteCnv_test\n";
  test1 (svcloc);
  return 0;
}
