...
*/
/**
 * @file LArCondAthenaPool/test/LArAutoCorrCompleteCnv_test.cxx
 * @author scott snyder <snyder@bnl.gov>
 * @date Jan, 2020
 * @brief Tests for LArAutoCorrCompleteCnv (partial).
 */


#undef NDEBUG
#include "../src/LArAutoCorrCompleteCnv.cxx"
...
typedef LArConditionsSubset<LArAutoCorrP>  OldType;
typedef LArConditionsSubset<LArAutoCorrP1> NewType;
const size_t nfeb = 10;


class TestCnv
  : public LArAutoCorrCompleteCnv
{
public:
  using LArAutoCorrCompleteCnv::LArAutoCorrCompleteCnv;
  using LArAutoCorrCompleteCnv::createTransient;
};
...
OldType::Payload makePayload (int x)
{
  ...
  payload.m_vAutoCorr.assign ({ 1.5f+x, 2.5f+x, 3.5f+x, 4.5f+x, 5.5f+x });
  return payload;
}
...
void checkPayload (int x, const NewType::Payload& newPayload)
{
  ...
  assert (oldPayload.m_vAutoCorr.size() == newPayload.m_vAutoCorr.size());
  for (size_t i = 0; i < oldPayload.m_vAutoCorr.size(); i++) {
    assert (oldPayload.m_vAutoCorr[i] == newPayload.m_vAutoCorr[i]);
  }
}
...
int main()
{
  ...
  std::cout << "LArCondAthenaPool/LArAutoCorrCompleteCnv_test\n";
  test1 (svcloc);
  return 0;
}
