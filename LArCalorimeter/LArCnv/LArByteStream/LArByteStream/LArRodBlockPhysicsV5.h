...
#define LARBYTESTREAM_LARRODBLOCKPHYSICSV5_H

/** This class provides decoding/encoding from/to ROD format. 
   * @authors W. Lampl, R. Lafaye
   * @version  0-0-1 , Jun 18, 2008
   *
   * Modified, 
   * new format for ATLAS (the last?)
   */
...
inline uint16_t LArRodBlockPhysicsV5::getFebConfig() const
{
  return getHeader16(FebConfig);
}

inline uint16_t LArRodBlockPhysicsV5::getFirstSampleIndex() const
{
...
inline int32_t  LArRodBlockPhysicsV5::getEx() const
{
  if(m_SumPointer) return (m_SumPointer[0]>>9);
  return 0;
}

inline int32_t  LArRodBlockPhysicsV5::getEy() const
{
  if(m_SumPointer) return (m_SumPointer[1]>>9);
  return 0;
}

inline int32_t LArRodBlockPhysicsV5::getEz() const
{
