...
LArAccumulatedDigitContByteStreamCnv::initialize()
{
  ...
  if (m_ByteStreamEventAccess==NULL)
    {
      (*m_log) <<MSG::ERROR<< "  LArAccumulatedDigitContByteStreamCnv: Can't cast to  ByteStreamCnvSvc " <<endreq; 
      return StatusCode::FAILURE ;
    }
  ...
  if(StatusCode::SUCCESS != serviceLocator()->getService("ROBDataProviderSvc",svc)){
    ...
    if(m_rdpSvc == 0 ) {
      (*m_log) <<MSG::ERROR<< "  LArAccumulatedDigitContByteStreamCnv: Can't cast to  ByteStreamInputSvc " <<endreq; 
      return StatusCode::FAILURE;
    }
  }
...
{//Convert Accumulated  Digits from ByteStream to StoreGate
  MsgStream log(messageService(), "LArAccumulatedDigitContByteStreamCnv");
  (*m_log) << MSG::VERBOSE << "Executing CreateObj method for LArAccumulatedDigitContainer " << endreq;
  ...
  (*m_log) << MSG::DEBUG << "Converting LArAccumulatedDigits (from ByteStream). key=" << key << " ,gain=" << gain << endreq; 
  ...
  if (sc!=StatusCode::SUCCESS)
    (*m_log) << MSG::WARNING << "Conversion tool returned an error. LArAccumulatedDigitContainer might be empty." << endreq;
...
{// convert LArAccumulatedDigits from StoreGate into ByteStream
  (*m_log) << MSG::ERROR << "CreateRep method of LArAccumulatedDigitContainer not implemented" << endreq;
  return StatusCode::SUCCESS;
}
