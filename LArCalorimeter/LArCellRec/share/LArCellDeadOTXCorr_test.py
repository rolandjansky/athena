...
def make_TriggerTowerContainer():
    ttc = ROOT.xAOD.TriggerTowerContainer_v1()
    aux = ROOT.xAOD.TriggerTowerAuxContainer_v1()
    ttc.setNonConstStore (aux)

    veccls = getattr (ROOT, 'std::vector<int>')
    for tt_data in tt_table:
        tt = ROOT.xAOD.TriggerTower_v1()
        ttc.push_back (tt)
