    ...
    m_keyHVScaleCorr("LArHVScaleCorr")
{
  ...
  declareProperty("FCalInMediumGain",m_FCalInMediumGain=0);
  declareProperty("EMECIWInMediumGain",m_EMECIWInMediumGain=0);
}

StatusCode LArOnlDbPrepTool::finalize()
{
...
StatusCode LArOnlDbPrepTool::initialize()
{
  MsgStream log( msgSvc(), name() );
...
StatusCode LArOnlDbPrepTool::prepare()
{
  ...
  sc = m_calibCont->initialize();
  if(!sc.isSuccess()){
    log<<MSG::ERROR<<" Failed to initialize LArOnlCalibDataContainer" << endreq;
    return sc; 
  }
  m_calibCont->setLArOnlineID(m_lar_on_id); 

  m_energyCont = new LArOnlEnergyDataContainer(); 
  sc = m_energyCont->initialize();
  if(!sc.isSuccess()){
    log<<MSG::ERROR<<" Failed to initialize LArOnlEnergyDataContainer" << endreq;
    return sc; 
  }
  ...
  { 
	...
	if(ofc_a.size()==0  ){
	  ...
	  continue ;
        ...
        ILArShape::ShapeRef_t shapeDer = shapes->ShapeDer(chanId,gain,0); 	
        log << MSG::DEBUG << " shapeDer Size  " << shapeDer.size()<< endreq;


	// add check ERRORCODE... 
        if(shape.size()<ofc_a.size() || shapeDer.size()<ofc_a.size()){
	...
	if(m_applyRampIntercept){
	  ...
	  // FCal & EMEC IW treated the same if they are set to Medium/Low gains only
	  if((m_lar_on_id->isHECchannel(chanId)) ||
	     (m_lar_on_id->isFCALchannel(chanId) && m_FCalInMediumGain) ||
	     (m_lar_on_id->isEMECchannel(chanId) && m_lar_on_id->isEMECIW(chanId) && m_EMECIWInMediumGain) ){
	       if(gain==2 && 
	...
	for(unsigned int i = 0; i<ofc_a.size();++i)
	{
	  ...
	  t.m_shapeDer.push_back(float(shapeDer[i])/f);
	}
...
StatusCode LArOnlDbPrepTool::getXML(const std::vector<unsigned int>& febIds, std::string& XMLoutputString){
  ...
  for(; febit != febit_end; febit++) {
	...
	if(!data.isEmpty()) {
	  ...
	  log << MSG::DEBUG << "ShapeDer = " << data.m_shapeDer[0] << endreq;
	  ...
	  vecit = data.m_shapeDer.begin();
	  vecit_end = data.m_shapeDer.end();
	  for(; vecit != vecit_end; vecit++) {
	    float shD = (*vecit);
	    XMLstream << std::fixed << shD << " "; 
	  }
