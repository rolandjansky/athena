...
def GetBeamIntensities(runnumber,beam):    
    ...
    trigDB = indirectOpen('COOLONL_TDAQ/'+DBINST,oracle=True)
    if (trigDB is None):
        print "ERROR: Cannot connect to COOLONL_TDAQ/"+DBINST
        return None
    try:
...
def GetLBTimeStamps(runnumber):
    res = []
    trigDB=indirectOpen('COOLONL_TRIGGER/'+DBINST,oracle=True)
    if (trigDB is None):
...
def GetReadyFlag(runnumber):
    ...
    LBtimes = GetLBTimeStamps(runnumber)
    nLB = len(LBtimes)-1
    db  = indirectOpen('COOLONL_TDAQ/'+DBINST,oracle=True)
