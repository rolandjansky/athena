...
class queryhvtripdb(Thread):
  ...
  def run(self):
    dbName='/afs/cern.ch/user/l/larmon/public/development/LArCalorimeter/LArMonitoring/python/HVTripsDB/trips.db'    
...
class queryatlasready(Thread):
  ...
  def run(self):
    athena_version = "19.3.0"
    import subprocess as sp
    cmd="export AtlasSetup=/afs/cern.ch/atlas/software/dist/AtlasSetup;"
    cmd+="source $AtlasSetup/scripts/asetup.sh %s,here;"%(athena_version)
    #cmd+="python /afs/cern.ch/user/l/larmon/public/development/LArCalorimeter/LArMonitoring/python/WebDisplayExtractor/aux/GetATLASReadyLBs.py %s"%(self.run_spec['low_run'])
    cmd+="python /afs/cern.ch/user/l/larmon/public/development/LArCalorimeter/LArMonitoring/python/MiscLibraries/MyCOOLlib.py %s"%(self.run_spec['low_run'])
...
class queryeventsperlb(Thread):
  ...
  def run(self):
    ...
    cmd+="source $AtlasSetup/scripts/asetup.sh %s,here;"%(athena_version)
    cmd+="python /afs/cern.ch/user/l/larmon/public/development/LArCalorimeter/LArMonitoring/python/WebDisplayExtractor/aux/GetNEventsLB.py --run %s --stream %s;"%(self.run_spec['low_run'],stream)
...
%d samples<br />
<hr />
Percentage of corrupted data<br />
