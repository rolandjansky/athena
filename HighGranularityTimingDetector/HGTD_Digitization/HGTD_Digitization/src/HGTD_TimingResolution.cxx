...
HGTD_TimingResolution::HGTD_TimingResolution(float integratedLumi, float Rmin,
                                             float Rmax, float z) {
  m_integratedLumi = std::fabs(integratedLumi);
  m_z = z;
  m_Rmin = Rmin;
  m_Rmax = Rmax;
  init();
}
void HGTD_TimingResolution::init() {
  ...
  if (m_radii.size() != (m_doseInner1000.size() + m_doseMiddle2000.size() +
    ...
    std::cout << "ERROR while Initializing the timing resolution. Vector of "
                 "different size."
              << std::endl;
    return;
  }
...
bool HGTD_TimingResolution::changeResolution2(float integratedLumi) {
...
float HGTD_TimingResolution::gain(float radius) {
  float gain = 1.;
...
// check if the request is in range:
bool HGTD_TimingResolution::etaInRange(float eta) {
  float radius = translateEta2R(eta);
  return radiusInRange(radius);
}

// translate radius to eta
float HGTD_TimingResolution::translateR2Eta(float radius) {
  return std::fabs(
      -std::log(std::tan(std::atan(std::fabs(radius) / m_z) / 2.)));
}

// translate eta to radius
float HGTD_TimingResolution::translateEta2R(float eta) {
...
void HGTD_TimingResolution::print() {
  std::cout << "HGTD_TimingResolution version: " << m_version << std::endl;
  std::cout << "Integrated Lumi: " << m_integratedLumi << std::endl;
  ...
  for (unsigned int i = 0; i < m_resolution.size(); i++) {
    std::cout << "Resolution r = " << m_radii[i] << " mm "
              << hitTimingResolution(m_radii[i]) << " ps" << std::endl;
    std::cout << "Gain r = " << m_radii[i] << " mm " << gain(m_radii[i])
              << std::endl;
  }
}
