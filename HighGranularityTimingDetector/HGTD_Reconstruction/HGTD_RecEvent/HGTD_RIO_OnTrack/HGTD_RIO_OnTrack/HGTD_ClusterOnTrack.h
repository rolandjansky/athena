...
public:
  HGTD_ClusterOnTrack();
  HGTD_ClusterOnTrack(const HGTD_ClusterOnTrack&);
  HGTD_ClusterOnTrack& operator=(const HGTD_ClusterOnTrack&);
  HGTD_ClusterOnTrack(HGTD_ClusterOnTrack&&);
  HGTD_ClusterOnTrack& operator=(HGTD_ClusterOnTrack&&);
  ...
  /** Destructor:*/
  virtual ~HGTD_ClusterOnTrack();

  /** Pseudo-constructor : needed to avoid excessive RTTI*/
  virtual HGTD_ClusterOnTrack* clone() const;
  ...
  virtual const InDetDD::SolidStateDetectorElementBase* detectorElement() const;

  /** returns the PrepRawData - is a SiCluster in this scope
    - fullfills Trk::RIO_OnTrack interface*/
  virtual const HGTD_Cluster* prepRawData() const;
...
protected:
  ...
  mutable std::unique_ptr<const Amg::Vector3D> m_global_position;
