...
class TileCalibDrawerOfc : public TileCalibDrawerBase
{
 public:
  ...
  int32_t  getPhase(unsigned int channel, unsigned int adc, unsigned int phaseIdx) const;
  ...
  float  getOfc(unsigned int field, unsigned int channel, unsigned int adc, 
		int phase, unsigned int sample) const;

  /** @brief Fill all OFC for optimazation.
      @param channel The channel number; if >= getNChans() it is reset to 0 without warning (default policy) 
      @param adc The gain index; if >= getNGains() it is reset to 0 without warning (default policy)
      @param phase The phase of interest */
  void fillOfc (unsigned int channel,unsigned int adc,int phase,
  ...
  void setOfc(unsigned int field, unsigned int channel, unsigned int adc, 
	      int phase, unsigned int sample, float value);
  
  /** @brief Sets a phase value.
      @param channel The channel number
      @param adc The gain index
      @param phases Vector containing the phases to set */
  void setPhases(unsigned int channel, unsigned int adc, const std::vector<int>& phases); 
 ...
 private:
  /** @brief Returns pointer to first data OFC for a given field, ADC & phase.
      ...
      @param adc The gain index
      @param phase The phase */
  float* getOfcStartAddress(unsigned int field, unsigned int channel, 
			    unsigned int adc, int phase) const;

  /** @brief Returns pointer to the requested phase value
      @param channel The channel number
      @param adc The gain index
      @param phaseIdx The phase index */
  int32_t* getPhaseStartAddress(unsigned int channel,
				unsigned int adc,
  ...
  /** Returns the index for a given phase 
      ...
      @param adc The gain index
      @param phase The phase of interest */
  unsigned int getPhaseNumber(unsigned int channel, unsigned int adc, int phase) const;
};

//
//______________________________________________________________
__attribute__((always_inline)) inline uint32_t 
TileCalibDrawerOfc::getNSamples() const
{
...
TileCalibDrawerOfc::getNPhases() const
{
...
TileCalibDrawerOfc::getPhaseNumber(unsigned int channel, unsigned int adc, int phase) const
{
  int32_t* beg = getPhaseStartAddress(channel,adc,0);
  int32_t* end = beg + std::abs(getNPhases());
  int32_t* pos = std::lower_bound(beg, end, phase);
  if(pos==end || *pos!=phase){ throw TileCalib::NoSuchPhase("TileCalibDrawerOfc::getPhaseNumber",phase); }
  return (pos-beg);
}
...
//______________________________________________________________
__attribute__((always_inline)) inline int32_t
TileCalibDrawerOfc::getPhase(unsigned int channel, unsigned int adc, unsigned int phaseIdx) const
{
  return *(getPhaseStartAddress(channel,adc,phaseIdx));
}

//
//______________________________________________________________
__attribute__((always_inline)) inline uint32_t 
TileCalibDrawerOfc::getNFields(uint16_t objVersion) const
{
...
//______________________________________________________________
inline float 
TileCalibDrawerOfc::getOfc(unsigned int field, unsigned int channel, unsigned int adc, 
			   int phase, unsigned int sample) const
{
...
inline void
TileCalibDrawerOfc::fillOfc (unsigned int channel,unsigned int adc, int phase,
                             float* w_a, float* w_b, float* w_c, float* g, float* dg) const
{
...
//______________________________________________________________
inline void 
TileCalibDrawerOfc::setOfc(unsigned int field,unsigned int channel, unsigned int adc, 
			   int phase, unsigned int sample, float value)
{
  if(sample>=getNSamples())
...
//______________________________________________________________
__attribute__((always_inline)) inline int32_t* 
TileCalibDrawerOfc::getPhaseStartAddress(unsigned int channel,
					 unsigned int adc,
					 unsigned int phaseIdx) const
{
...
TileCalibDrawerOfc::getOfcStartAddress(unsigned int field, unsigned int channel, 
				       unsigned int adc, int phase) const
{
  ...
  if(objVer!=1 && objVer!=2 && objVer!=3){ 
    throw TileCalib::VersionConflict("TileCalibDrawerOfc::getOfcStartAddress",
				     objVer);
  }
  ...
  if(field>=nField){
    throw TileCalib::IndexOutOfRange("TileCalibDrawerOfc::getOfcStartAddress",
				     field, nField);
  }
