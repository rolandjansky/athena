...
TH2F *z_R_lost = new TH2F("z_R_lost", "z_R_lost",Zbin,Zbeg,Zend,Rbin,Rbeg,Rend);
    ...
    // Open input file
    ifstream dat_f(hits_file);
    if(!dat_f.good()) {
        cout<<"Error openning file: "<<hits_file<<endl;
        return 1;
    } else {
        cout<<"Open file: "<<hits_file<<endl;
    }

    Int_t code1, code2, subdet, nphi, neta, sample, region, IDcalc, event;
    ...
    while (!dat_f.eof()) {
        code1=subdet=nphi=neta=sample=region=IDcalc=event=0;
        code2=x=y=z=E=0;
        dat_f>>code1>>subdet>>nphi>>neta>>sample>>region>>IDcalc>>event;
        dat_f>>code2>>x>>y>>z>>E;
        if((code1 != 1111111) || (code2 != 2222222)) {
