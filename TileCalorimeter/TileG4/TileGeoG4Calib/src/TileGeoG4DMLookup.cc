	      ...
	      //Boundary reached - move to the next cell and next boundary
	          ...
	          if(!passed)                                                                             //If we are in Barrel, then we didn't passed B-subsample of BC sample yet.   
	              ...
	              if(++l_indCurrentCell[samp] < static_cast<int>((samples[samp])->cells.size()))
	                { l_currentBoundary[samp] += ((samples[samp])->cells[l_indCurrentCell[samp]])->nrOfPeriodsInCell[0] ; }  // A,B-Barrel or A,b,C,D - Ext. Barrel or ITC      
	            }    
                  else
	            {                                                                                     // passed==true
		        ...
		        {    
		          if(++l_indCurrentCell[samp] < static_cast<int>((samples[samp-1])->cells.size()))
                        ...
                        {
                          if(++l_indCurrentCell[samp] < static_cast<int>((samples[samp-1])->cells.size()))
	                    { l_currentBoundary[samp] += ((samples[samp-1])->cells[l_indCurrentCell[samp]])->nrOfPeriodsInCell[0] ; }    // D sample == 3 sample   Barrel          
	                }
		    }
               }
	        ...
	        {
		  if(l_indCurrentCell[samp] < static_cast<int>((samples[samp])->cells.size()))
	            { m_DMToCell.push_back ((tile_section->samples[samp])->cells[l_indCurrentCell[samp]]) ; }     // A,B - Barrel or A,B,C,D - Ext. Barrel or ITC             
                }
              else
	        {                                     	                                                 // passed==true
	          if(l_indCurrentCell[samp] < static_cast<int>((samples[samp-1])->cells.size()))
	            { m_DMToCell.push_back((tile_section->samples[samp-1])->cells[l_indCurrentCell[samp]]) ; }    // C,D - Barrel
		}
