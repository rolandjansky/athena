  ...
  , totalTime_down(0)
{
  ...
  for (int i=m_options.deltaTHit.size()-1; i>-1; --i) m_options.deltaTHit[i] *= CLHEP::ns;
  // protection agaist wrong vector size
  if (m_options.deltaTHit.size()%3 != 1 || m_deltaT<=0.0) { m_options.deltaTHit.resize(1); m_options.deltaTHit[0]=m_deltaT; }
  ...
  if (m_options.Ushape == -1) {
    ...
    G4cout << "Using U-shape flag from GeoModel = " << UshapeFromGM
    ...
    if (UshapeFromGM > 0 && m_options.Ushape > 0) { // both are > 0, so it's fine to use value from SimOptions
      ...
      G4cout << "Please, use correct GeoModel tag or make sure that you set both "
             << "tileSimInfoConfigurator.Ushape and GeoModelSvc.DetectorTools[ \"TileDetectorTool\" ].Ushape to the same value"  << G4endl;
  ...
  if (m_deltaT>0.0) m_options.timeCut = ((int)(m_options.timeCut/m_deltaT) + 0.5) * m_deltaT;
  ...
  if (m_options.timeCut > m_tilesize_deltaT-m_deltaT) {
    m_options.timeCut = m_tilesize_deltaT-m_deltaT;
  ...
  G4cout << "All hits with time above " << m_options.timeCut/CLHEP::ns
...
{
  if (m_options.verboseLevel>10) G4cout << "Process Hits" << G4endl;

  G4double edep = aStep->GetTotalEnergyDeposit();
  if (edep==0. && aStep->GetTrack()->GetDefinition() != G4Geantino::GeantinoDefinition()) {
    if (m_options.verboseLevel>10) G4cout << "Edep=0" << G4endl;
    return false;
  }
  ...
  for (int i=0; i<level; i++) {
    ...
    if (m_options.verboseLevel>10) G4cout << " Level--> " << i << ", LogVol--> "
                                          << (lv->GetName()).c_str() << ", PhysVol--> " << (pv->GetName()).c_str() << G4endl;
  }
  ...
  if (m_options.verboseLevel>10) G4cout << "Tile volume level --->" << level << G4endl;
  ...
  if (m_options.verboseLevel>10) {
    ...
    G4cout << "Period number:  " << tileperiod << G4endl;
    G4cout << "Scin Copy No:   " << tilesize << G4endl;
  }
  ...
  if(!_cell) {
    ...
    _section->PrintScinToCell("ERROR in _cell");
    return false;
  }

  nrOfPMT = _cell->nrOfPMT;
  if (m_options.verboseLevel>10) G4cout << "Number of PMTs: " << nrOfPMT << G4endl;
    ...
    if( m_lookup->TileGeoG4npmtD4(std::max(0,nSide),nModule-1) == 0 ) {
      ...
      if (m_options.verboseLevel>10) {
        ...
        G4cout << "Disabling special D4, all energy goes to cell "
               << nm[_cell->sample] << _cell->cellNum << G4endl;
      }
    }
  } else if (sect==4) {
    // Change number of PMTs in Special C10 Cells
    nrOfPMT = m_lookup->TileGeoG4npmtC10(std::max(0,nSide),nModule-1);
    if (m_options.verboseLevel>10 && nrOfPMT!=2) {
      G4cout << "Changing number of PMTs in special C10 to " << nrOfPMT << G4endl;
    }
  } else if (sect==6 && _cell->tower>16) {
    // Check number of PMTs in Special E4' Cells (side C, modules 32,33)
    nrOfPMT = m_lookup->TileGeoG4npmtE5(std::max(0,nSide),nModule-1);
    if (nrOfPMT != _cell->nrOfPMT) {
      G4cout << "WARNING: Changing number of PMTs in special E4' from " << _cell->nrOfPMT << " to " << nrOfPMT << G4endl;
    }
  }
...
G4bool TileGeoG4SDCalc::MakePmtEdepTime(const G4Step* aStep)
{
  ...
  G4double edep;
  if (m_options.doBirk) edep = BirkLaw(aStep);
  ...
  if (scinSolid !=0 || booleanScin !=0) {
    ...
    if (nrOfPMT==2) {
      ...
      if (isE5) { //special E5(E4') cells in EBC (negative)
        if (yLocal>=0.) { edep_up = edep; edep_down = 0.; }
        else            { edep_up = 0.;   edep_down = edep; }
      } else {
        ...
        case 20: case 21: case 22: case 23:
          edep_up   = edep * Tile_1D_profileRescaled_zeroBins (tilesize, yLocal, zLocal, 1, nDetector, nSide, m_options.Ushape-20);
          edep_down = edep * Tile_1D_profileRescaled_zeroBins (tilesize, yLocal, zLocal, 0, nDetector, nSide, m_options.Ushape-20);
          break;
        ...
        case -2: {
          ...
          if (m_options.verboseLevel>10) {
            ...
            G4cout << "Tile Row = " << tilesize << G4endl;
  ...
  if (totalTime+deltaT_up   > m_options.timeCut ||
      totalTime+deltaT_down > m_options.timeCut ) {
    totalTime = m_lateHitTime;
    deltaT_up = deltaT_down = 0.0;
    if (m_options.verboseLevel>10) G4cout << " hit time set to " << totalTime << G4endl;
  }
  ...
  if (m_options.verboseLevel>10) G4cout <<"MakePmtEdepTime: right pmtID_up,pmtID_down,"
                                        <<"edep_up,edep_down,scin_Time_up,scin_Time_down:\t"<<pmtID_up<<"\t"<<pmtID_down<<"\t"
                                        <<edep_up<<"\t"<<edep_down<<"\t"<<scin_Time_up<<"\t"<<scin_Time_down << G4endl;
...
TileMicroHit TileGeoG4SDCalc::GetTileMicroHit(const G4Step* aStep)
{
  ...
  if ( !stat ) {
    if (m_options.verboseLevel>5) G4cout <<"GetTileMicroHit: FindTileScinSection(aStep) is false!" << G4endl;
    return microHit;
  }

  stat = MakePmtEdepTime(aStep);  //calculation of pmtID, edep and scin_Time with aStep
  if ( !stat ) {
    if (m_options.verboseLevel>5) G4cout <<"MakePmtEdepTime: wrong pmtID_up,pmtID_down,edep_up,"
...
G4bool TileGeoG4SDCalc::ManageScintHit()
{
  ...
  if(is_negative) {
    if(nrOfPMT==2) {
      ...
      G4cout << "FATAL ERROR: ManageScintHit: Unexpected number of PMTs in a Cell -->" << nrOfPMT << G4endl;
      return false;
    }
  } else { //positive
    if(nrOfPMT==2) {
...
void TileGeoG4SDCalc::CreateScintHit(int pmt)
{
  ...
  if (pmt==1) { //Uper PMT of Cell
    ...
    if (m_options.doTileRow)     aHit->add(edep_up,scin_Time_up,m_deltaT);
    ...
    if (m_options.doTileRow)       aHit->add(edep_down,scin_Time_down,m_deltaT);
...
G4double TileGeoG4SDCalc::BirkLaw(const G4Step* aStep) const
{
  ...
  if ( (charge!=0.) && (material->GetName()==myMaterial) ) {
    ...
    if(aStep->GetStepLength() != 0 ) {
      ...
      if (m_options.verboseLevel>5) G4cout
                                      << "BirkLaw() - Current Step in scintillator has zero length."
                                      << "Ignore Birk Law for this Step" << G4endl;
      response = destep;
    }
...
G4double TileGeoG4SDCalc::Tile_1D_profileRescaled_zeroBins(int row, G4double x, G4double y, int PMT, int nDetector, int nSide, int nZeroBins)
{
  if(PMT) x *= -1.;
