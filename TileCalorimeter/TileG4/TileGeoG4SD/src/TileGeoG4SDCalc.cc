    ...
    , m_row(0)
{
    ...
    if (status.isFailure())
        ...
        (*m_log) << MSG::DEBUG << "TileTBID helper retrieved" << endreq;
...
G4bool TileGeoG4SDCalc::FindTileScinSection(const G4Step* aStep)
{
    ...
    if(!_cell) {
        (*m_log) << MSG::ERROR << "Zero pointer to current cell, nScin=" << nScin << endreq;
    ...
    if (sect==3) {
        ...
        if (nrOfPMT != _cell->nrOfPMT) {
            (*m_log) << MSG::WARNING << "Changing number of PMTs in special E4' from " << _cell->nrOfPMT << " to " << nrOfPMT << endreq;
        }
    }
        ...
        if (nrOfPMT==2) {
            ...
            if (isE5) { //special E5(E4') cells in EBC (negative)
                ...
                case 20: case 21: case 22: case 23:
                    edep_up   = edep * Tile_1D_profileRescaled_zeroBins (tilesize, yLocal, zLocal, 1, nDetector, nSide, m_Ushape-20);
                    edep_down = edep * Tile_1D_profileRescaled_zeroBins (tilesize, yLocal, zLocal, 0, nDetector, nSide, m_Ushape-20);
                    break;
    ...
    if( m_deltaT > 0.0 ) {
        ...
        if (m_verbose) {
            ...
            if(totalTime!=0.0) {
                ...
                (*m_log) << MSG::VERBOSE << "CellNum  >> " << _cell->cellNum << endreq;
                ...
                if (isE5) {
                    (*m_log) << MSG::VERBOSE << "Module-1 >> " << nModule-1 << ((yLocal>=0.0)?" Up":" Down") << endreq;
                    (*m_log) << MSG::VERBOSE << "Phi ind  >> " << (nModule-32)*2+((yLocal>=0.0)?1:0);
                    (*m_log) << "  E5 Y-coord global " << position.y() << " local " << yLocal << endreq;
                    (*m_log) << MSG::VERBOSE << "Eta ind  >> 2"<< endreq;
                } else {
                    (*m_log) << MSG::VERBOSE << "Module-1 >> " << nModule-1 << endreq;
                    (*m_log) << MSG::VERBOSE << "Tower-1  >> " << nTower-1 << endreq;
                    (*m_log) << MSG::VERBOSE << "Sample-1 >> " << nSample-1 << endreq;
                }
...
G4bool TileGeoG4SDCalc::ManageScintHit()
{
    ...
    //if (nTower>16) (*m_log)<< MSG::DEBUG <<" Cells E5: nModule="<<nModule
    //                       <<"  edep_up="<<edep_up<<"  edep_down="<<edep_down
    //                       <<"  pmtID_up="<<pmtID_up<<"  pmtID_down="<<pmtID_down<<endreq;
