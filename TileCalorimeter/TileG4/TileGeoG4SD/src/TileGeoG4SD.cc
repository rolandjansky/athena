...
G4bool TileGeoG4SD::ProcessHits(G4Step* aStep, G4TouchableHistory* /*ROhist*/)
{
  if ( !(m_calc->FindTileScinSection(aStep)) ) //Search for the tilecal sub-section, its module and some identifiers
    {
      if (verboseLevel>5) G4cout <<"ProcessHits: FindTileScinSection(aStep) is false!" << G4endl;
      return false;
    }

  if ( !(m_calc->MakePmtEdepTime(aStep)) ) //calculation of pmtID, edep and scin_Time with aStep (Sergey)
    {
      if (verboseLevel>10) G4cout <<"ProcessHits: wrong pmtID_up,pmtID_down,edep_up,edep_down,"
                                  <<"scin_Time_up,scin_Time_down:\t"<<m_calc->pmtID_up<<"\t"<<m_calc->pmtID_down<<"\t"<<m_calc->edep_up
                                  <<"\t"<<m_calc->edep_down<<"\t"<< m_calc->scin_Time_up<<"\t"<<m_calc->scin_Time_down << G4endl;
      return false;
    }
