...
{ 
  ...
  OutInHighGain.push_back(book1D("/Summary","OverFlowInHighGain",
                                 "Injected charge with overflow in high gain",
                                 10010,-0.5,1000.5));
...
//! 5 - CRC  errors (only gain=0 used, so should be the last histogram in a vector)
  ...
  for (int gn=mingain; gn<maxgain; ++gn) {
    ...
    hist2[ros][drawer][adc].push_back(book2F(subDir,histName,histTitle,
					     16,0.,16.,6,-0.5,5.5));
  }
  ...
  for (int ch=0; ch<48; ++ch) {
    ...
    for (int gn=mingain; gn<maxgain; ++gn) {
      ...
      histP[ros][drawer][ch][adc].push_back(bookProfile(subDir,histName,histTitle,
                                                        m_nSamples,0.0,m_nSamples*1.0,-0.5,1023.5));
      ...
      shifted_hist[ros][drawer][ch][adc] = book1S(subDir,histName,histTitle,
						  shiftnbins,shiftxmin,shiftxmax);
...
//  ++counter;
  ...
  //  log << MSG::INFO << "in fillHists()" << endreq;
  ...
  StatusCode sc = m_eventStore->retrieve(DigitsCnt, "TileDigitsCnt");
...
//        bookHists(ros,drawer);
      ...
      for(; digitsItr!=lastDigits; ++digitsItr) {
        ...
        if (dsize > 16 ) {
          log << MSG::ERROR << "length of digits vector " << dsize << " - greater than 16 !" << endreq; 
          dsize = 16;
        }
...
//          hist1[ros][drawer][ch][0][4]->Fill(2.0,1.0);
	  ...
	  if (headsize > 16 ) {
	    log << MSG::ERROR << "length of BCIDhigh vector " << headsize << " - greater than 16 !" << endreq;
	    headsize = 16;
	  }
      }
...
/*---------------------------------------------------------*/
{
  ...
  const char *HistName[8] = {"_ped","_rms_lfr","_rms_hfr","_bits",
                             " Pedestal[0]"," RMS noise low frequency",
                             " RMS noise high frequency"," Stuck bits and zero amp"};
    ...
    for (int drawer = 0; drawer < 64; ++drawer) {
	  ...
	  final_hist_stucks[ros][drawer][adc]->GetYaxis()->SetBinLabel(8,"All 1023");
	    ...
	    if ( (nevents!=0)&&(nevents!=m_nEvents) ) {
	      ...
	      log << MSG::WARNING << "Number of entries in histo " << hist1[ros][drawer][ch][adc][0]->GetTitle() << " doesnt match n. of events! Using the first one in mean and RMS calculation\n";
	      log << MSG::WARNING << "Number of entries in histo =" << nevents << "\tNumber of events= "<< m_nEvents << "\n" ; 	
	    }
	    else {//all ok
	      nevents=m_nEvents;
	    }
	    ...
	    stuckBits_Amp2(hist, adc, final_hist_stucks[ros][drawer][adc], ch);
          ...
          if (m_runType == PedRun) {
            ...
            for (int type = 0; type < 2; ++type) {
              ...
              histTitle=sStr.str();
              final_hist2[ros][drawer][adc].push_back(book2F(subDir,histName,histTitle,
                                                             48,0.0,48.0,48,0.0,48.0));
            }
		...
		//                int pmt_i = abs(m_cabling->channel2hole(ros,ch_i))-1;
		  ...
		  //                  int pmt_j = abs(m_cabling->channel2hole(ros,ch_j))-1;
                  covar[ch_i][ch_j] = MeanAmp_ij[ros][drawer][adc][ch_i][ch_j] - 
                                        MeanAmp[ros][drawer][adc][ch_i]*MeanAmp[ros][drawer][adc][ch_j];
		...
		m_cov_ratio[ros][drawer][adc] = (2.*mean_cov_ij) / (mean_cov_ii*47.) ; //(2*cov_ij/(48*47))/(cov_ii/48)
	      ...
	      else { m_cov_ratio[ros][drawer][adc] =0.;
	      }
	 ...
	 if (m_runType==CisRun) {
	   ...
	   for (int gn = mingain; gn < maxgain; ++gn){ 
	     ...
	     std::ostringstream s;
	     s << part[ros] << std::setfill('0') << std::setw(2) 
	       << drawer+1  << std::setfill(' ');
...
/*---------------------------------------------------------*/
{
  ...
  if (palette_init_done == false)
  {
    ...
    if (ind < 0)
      ind = 0; // fall-back colours
    for (i = 0; i < m_NCont; i++) pal51[i] = ind + i;
    ind = define_palette(52);
    if (ind < 0)
      ind = 0; // fall-back colours
    for (i = 0; i < m_NCont; i++) pal52[i] = ind + i;
    TString s;
  ...
  for (int gain = 0; gain < maxgain; ++gain) {
	...
	if (do_plots)
	  hist0[ros][drawer][0]->Draw(); // global CRC in bottom right corner
  ...
  if (m_runType == PedRun) {
	...
	if (do_plots) {
	  ...
	  if (type==0) {
	    ...
	    covar_label[gain].SetLabel(label_text.str().c_str()); covar_label[gain].SetTextSize(0.4);
...
int TileDigitsMonTool::stuckBits(TH1S * hist, int adc)
{
  ...
  if ( MinBin == 513 ||
       (MaxBin == 512 && hist->GetBinContent(MaxBin) > 3) ) {
    ++NSB;
...
int TileDigitsMonTool::stuckBits_Amp2(TH1S * hist, int /*adc*/, TH2C *outhist, int ch)
{
  ...
  for (last_non0 = 1023; last_non0 >= 0; last_non0--)
    if (hist -> GetBinContent(last_non0 + 1) > 0)
      break;
...
void TileDigitsMonTool::CRCcheck(uint32_t crc32, uint32_t crcMask, int headsize, int ros, int drawer)
{
  ...
  if ( m_beamInfo->calibMode() == 1 ) { //!bigain: dummy information
    ...
    if (crc32 == 0) {  //std::cout << "Global crc is zero\n";
      for (int ch=0; ch<headsize; ++ch) { hist1[ros][drawer][ch][0][5]->Fill(0.0,1.0);}
    }
    else if (crcMask==0xFFFFFFFF) {
      for (int ch=0; ch<headsize; ++ch) { hist1[ros][drawer][ch][0][5]->Fill(1.0,1.0);}
	}
	else {
...
StatusCode TileDigitsMonTool::RODCRCcalc()
{
  ...
  if(sc == StatusCode::FAILURE) {
    log<<MSG::WARNING<<"Can't retrieve DSP Raw Channel Container from TES"<<endreq;    
    return sc;
  }
  ...
  for(; collItr!=lastColl; ++collItr) {
    ...
    if (chItr!=lastCh) 
	  ...
	  if (crc32==0) {crc32=0xFFFFFFFF;} // means OK CRC match 
	  else {crc32=0xFFFF;} //means NOT OK, CRC mismatch
	  uint32_t CRCmask  = (*collItr)->getFragRODChipMask(); CRCmask = CRCmask << 16 ; // ROD mask is stored in the 16 most significant bits ce
