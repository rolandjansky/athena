...
class TileDigitsMaker: public AthAlgorithm {
  ...
  private:
    ...
    std::vector<std::unique_ptr<double[]> > m_drawerBufferHi; //!< Vector used to store pointers to digits for a single drawer (high gain)
    std::vector<std::unique_ptr<double[]> > m_drawerBufferLo; //!< Vector used to store pointers to digits for a single drawer (low gain)
