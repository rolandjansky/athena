...
StatusCode TileHitVecToCntTool::initialize() {
  ...
  m_run2 = (m_cabling->getCablingType() == TileCablingService::RUN2Cabling
            || m_cabling->getCablingType() == TileCablingService::UpgradeABC);

  int towerE2(E2_TOWER);

  if (m_cabling->getCablingType() == TileCablingService::UpgradeABC) {
    m_towerE1 = E1_TOWER_UPGRADEABC;
    towerE2 = E2_TOWER_UPGRADEABC;
  }

  ATH_MSG_DEBUG( "E1 tower is " << m_towerE1 << ", E2 tower is " << towerE2 );
  ...
  for (int i = 0; i < 7; ++i) {
    ...
    switch (i) { // invent pmt_id in a given part(sampling) of the calorimeter
      case 6:
        ...
        pmt_id = m_tileID->pmt_id(3/*section*/, 1/*side*/, 0/*module*/, towerE2/*tower*/, 3/*sample*/, 0);
    ...
    ATH_MSG_DEBUG( i << " " << m_tileInfo->NPhElec(i) 
                   << " hit calib " << m_tileInfo->HitCalib(pmt_id) 
                   << " id " << m_tileID->to_string(pmt_id, -1) );
    ...
    if (m_pileUp) {
      ...
      for (int ros = 3; ros < 5; ++ros) {
        for (int drawer = 0; drawer < 64; ++drawer) {
          ...
          if (m_cabling->E1_merged_with_run2(ros, drawer) != 0) {

            IdentifierHash pmt_hash;
            IdContext pmt_context = m_tileID->pmt_context();
          
            int side = (ros & 1) ? TileID::POSITIVE : TileID::NEGATIVE;

            Identifier pmt_id = m_tileID->pmt_id(TileID::GAPDET, side, drawer, m_towerE1, TileID::SAMP_E, 0);
            m_tileID->get_hash(pmt_id, pmt_hash, &pmt_context);
            TileHit* toHit = m_allHits[pmt_hash];

            pmt_id = m_tileID->pmt_id(TileID::GAPDET, side, 
                                      m_cabling->E1_merged_with_run2(ros, drawer), 
                                      m_towerE1, TileID::SAMP_E, 0);
            m_tileID->get_hash(pmt_id, pmt_hash, &pmt_context);
            TileHit* fromHit = m_allHits[pmt_hash];

            m_hitsForMergingFromTo.push_back(std::make_pair(fromHit, toHit));

          }
        }
      }

      nMergedE1 = m_hitsForMergingFromTo.size();

      // By convention odd phi mbts are merged into even one
      for (int side = 0; side < N_SIDE; ++side) {
        for (int phi = 0; phi < 7; phi += 2) {

          TileHit* toHit = m_allHits[mbts_index(side, phi, 1)];
          TileHit* fromHit = m_allHits[mbts_index(side, phi + 1, 1)];
      ...
      nMergedMBTS = m_hitsForMergingFromTo.size() - nMergedE1;
      ...
      nMergedE1 = std::count (m_E1merged.begin(), m_E1merged.end(), true);
      nMergedMBTS = std::count (m_MBTSmerged.begin(), m_MBTSmerged.end(), true);
    }

    ATH_MSG_INFO("Number of E1 cell to be merged: " << nMergedE1);
    ATH_MSG_INFO("Number of MBTS cell to be merged: " << nMergedMBTS);
  ...
  switch (m_timeFlag) {
    ...
    case 0: {
      for (; inpItr != end; ++inpItr) {
        ...
        const TileHWID* tileHWID;
        detStore()->retrieve(tileHWID).ignore();
...
StatusCode TileHitVecToCntTool::mergeEvent() {
  ...
  if (m_pileUp) {
    ...
    if (m_run2) {
      ...
      for (const std::pair<TileHit*, TileHit*>& hits : m_hitsForMergingFromTo) {
        TileHit* fromHit = hits.first;
        if (fromHit->size() > 1 || fromHit->energy() != 0.0) {  // hit exists
          TileHit* toHit = hits.second;

          if (msgLvl(MSG::DEBUG)) {
            if (m_tileTBID->is_tiletb(fromHit->pmt_ID())) {
              msg(MSG::DEBUG) << "TileHit (MBTS) Id: " << m_tileTBID->to_string(fromHit->pmt_ID(), 0) 
                              << " will be merged to " << m_tileTBID->to_string(toHit->pmt_ID(), 0)  << endmsg;
            } else {
              msg(MSG::DEBUG) <<  "TileHit (E1 cell) Id: " << m_tileID->to_string(fromHit->pmt_ID(), -1) 
                              << " will be merged to " << m_tileID->to_string(toHit->pmt_ID(), -1)  << endmsg;
            }

            if (msgLvl(MSG::VERBOSE)) {
              msg(MSG::VERBOSE) << "Before merging => " << (std::string) (*fromHit) << endmsg;
              msg(MSG::VERBOSE) << "Before merging => " <<  (std::string) (*toHit) << endmsg;
            }

          }

          toHit->add(fromHit, 0.1);

          ATH_MSG_VERBOSE( "After merging => " << (std::string) (*toHit) );
    ...
    if (m_run2) {
      ...
      for (; collIt != endcollIt; ++collIt) {
        ...
        IdentifierHash frag_hash = m_fragHashFunc(frag_id);
        if (m_E1merged[frag_hash])        findAndMergeE1(coll, frag_id);
        else if (m_MBTSmerged[frag_hash]) findAndMergeMBTS(coll, frag_id);
      }
    }
...
double TileHitVecToCntTool::applyPhotoStatistics(double energy, Identifier pmt_id) {
 ...
 // fix for upgrade geometry with split B-C cells
  if (pmt_sample >= (int) TileID::SAMP_X && !m_tileTBID->is_tiletb(pmt_id)) pmt_sample = TileID::SAMP_B;
...
void TileHitVecToCntTool::findAndMergeE1( TileHitCollection* coll, int frag_id) {
  ...
  for (; hitIt != endHitIt; hitIt++) {
    ...
    if (m_tileID->tower(pmt_id) == m_towerE1 && m_tileID->sample(pmt_id) == TileID::SAMP_E) {
  ...
  if (fromHitIt != coll->end()) {
    ...
    if (toHit == 0) {
      ...
      Identifier to_pmt_id = m_tileID->pmt_id(TileID::GAPDET, side, module, m_towerE1, TileID::SAMP_E, 0);
