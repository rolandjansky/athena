...
TileHitVecToCntTool::TileHitVecToCntTool(const std::string& type,
    ...
    , m_photoStatWindow(62.5  * Gaudi::Units::nanosecond)
    , m_PhElStat(2)
    , m_skipNoHit(false)
    ...
    , m_cabling(0)
{
    ...
    declareProperty("PhotostatWindow", m_photoStatWindow, "Sum up energy in [-m_photoStatWindow,+m_photoStatWindow] and use it for photostatistics");
    declareProperty("PhotostatType", m_PhElStat,          "Method to apply photostatistics (default=2)");
...
StatusCode TileHitVecToCntTool::createContainers() {
    ...
    do_checks = m_tileID->do_checks();
    m_tileID->set_do_checks(false);
    do_checks_tb = m_tileTBID->do_checks();
...
void TileHitVecToCntTool::processHitVectorForPileUp(const TileHitVector* inputHits, double SubEvtTimOffset, int& nHit, double& eHitTot) {
    ...
    case 2: {
        ...
        for (; inpItr != end; ++inpItr) {
            ...
            if (msgLvl(MSG::VERBOSE)) {
                ...
                if (eHit > 0.0) tHit /= eHit;
                else tHit = cinp->time(); // just first time
...
StatusCode TileHitVecToCntTool::processBunchXing(int bunchXing
    ...
    if (m_rndmEvtOverlay && bunchXing != 0)
        iEvt = eSubEvents; // in overlay skip all events except BC=0
            ...
            if (m_pileUp || m_rndmEvtOverlay) {
                ...
                if (m_rndmEvtOverlay) { // overlay code
                    if (fabs(SubEvtTimOffset) > 0.1) {
                        ATH_MSG_ERROR( "Wrong time for in-time event: " << SubEvtTimOffset
                                       << " Ignoring all hits ");
                    } else {
                        ATH_MSG_DEBUG( " New HitCont.  TimeOffset=" << SubEvtTimOffset
                                       << ", size =" << inputHits->size());
...
StatusCode TileHitVecToCntTool::processAllSubEvents() {
    ...
    for (; hitVecNamesItr != hitVecNamesEnd; ++hitVecNamesItr) {
        ...
        if (m_pileUp || m_rndmEvtOverlay) {
            ...
            if (!(m_mergeSvc->retrieveSubEvtsData(hitVectorName, hitContList).isSuccess())
                || hitContList.size() == 0) {
            ...
            if (m_rndmEvtOverlay) {  // overlay code
                ...
                for (; iCont != iEndCont; ++iCont) {
                    ...
                    // get HitVector for this subevent
                    const TileHitVector* inputHits = &(*(iCont->second));
                    ATH_MSG_VERBOSE( " New HitCont.  TimeOffset=" << SubEvtTimOffset
                                     << ", size =" << inputHits->size());
                    this->processHitVectorForPileUp(inputHits, SubEvtTimOffset, nHit, eHitTot);
                }
...
StatusCode TileHitVecToCntTool::mergeEvent() {
	...
	IdentifierHash frag_hash = m_fragHashFunc(frag_id);
	if (m_E1merged[frag_hash]) findAndMergeE1((*collIt), frag_id);
    ...
    for (; collIt != endcoll; ++collIt) {
        ...
        for (; hitItr != hitEnd; hitItr++) {
            ...
            for (int i = 0; i < hitsize; ++i) {
                ...
                if (fabs(thit) < m_photoStatWindow) ehit += pHit->energy(i);
            }
            ...
            pHit->scale(ApplyPhotoStat(ehit, pmt_id));
        }
    }
...
double TileHitVecToCntTool::ApplyPhotoStat(double energy, Identifier pmt_id) {
    ...
    double pe = energy * numPhElec[pmt_sample];
            ...
            if (pe > 0.) {
                ...
                if (RndmPois > 0) {
                    ...
                    for (int i = 0; i < RndmPois; i++)
                        pe_scale += 1 / (1.08332)
                            * std::max(0., RandGaussQ::shoot(m_pHRengine, singleMEAN, singleSIGMA)); // FIXME CLHEP::RandGaussZiggurat is faster and more accurate.
    ...
    ATH_MSG_VERBOSE( "PhotoElec: id=" << m_tileID->to_string(pmt_id,-1)
                     << " totEne=" << energy
                     << ", numPhElec[" << pmt_sample << "]=" << numPhElec[pmt_sample]
                     << ", pe=" << pe
