                                           ...
                                           float                 zshift)
{
  ...
  // Hardcoded values for BC separated
  float rBarrelBC[] = {2450., 2795., 3215., 3630.};
  float drBarrelBC[] = {300., 390., 450., 380.};
  ...
  if (addPlates) {
    ...
    drGap[1] += 40*CLHEP::mm;
    rBarrelBC[0] -= 10*CLHEP::mm/2;
    rBarrelBC[3] += 40*CLHEP::mm/2;
    drBarrelBC[0] += 10*CLHEP::mm;
    drBarrelBC[3] += 40*CLHEP::mm;
  }
  ...
  if (detector==TILE_REGION_CENTRAL || detector ==  TILE_REGION_EXTENDED)
  {
    ...
    for(index=0; index<samples.size(); index++, indHardcoded++)
    {
      ...
      if(detector == TILE_REGION_CENTRAL)
      {
         rcenter[etaIndex-1] = (BCseparate) ? rBarrelBC[indHardcoded] : rBarrel[indHardcoded];
         dr[etaIndex-1] = (BCseparate) ? drBarrelBC[indHardcoded] : drBarrel[indHardcoded];
      }
      else
      {
  ...
  else if(detector == TILE_REGION_GAP)
  {
    ...
    for(unsigned int det = detector; det < detector+2; det++)
      ...
      {
        if (fabs(m_dbManager->TICLeta())>1.6) continue;
	if(det==detector)
	  etaIndex = (int)m_dbManager->TICLsample();
	else {
	  etaIndex = (int)m_dbManager->TICLtower();
          if (etaIndex > 16) {
              etaIndex = int(fabs(m_dbManager->TICLeta())*10.)+1;
          }
        }
	if(etaIndex > nsamp) nsamp = etaIndex;
  ...
  if(detector != TILE_REGION_GAP ) 
  {
    ...
    */
	...
	do
	{
	  ...
	  /** -------- BC ---------- */
	  if(m_dbManager->TICLsample()==2 && detector == TILE_REGION_CENTRAL && !BCseparate)
	  {
	    isSquare = true;
	  ...
	  if(m_dbManager->TICLsample()==2 && detector == TILE_REGION_CENTRAL && !BCseparate) nLastRow -=3;
