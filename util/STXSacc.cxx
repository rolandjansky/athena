...
int main( int argc, char* argv[] ) {
   ...
   int  Nbins=14; double min=0, max=14; Str tit=";STXS fine index";
   ...
   for (int j=0;j<Npdf;++j) printf("  PDF%2i",j+1); printf("\n");
   ...
   for (int j=0;j<Npdf;++j) printf("  PDF%2i",j+1); printf(" PDF1-30\n");
   double V = 0.0, nom=0;
   for (int i=1;i<=Nstxs;++i) {
     int bin=i+1; nom=h_STXS->GetBinContent(bin);
     // Nominal cross section and MC stat error
     printf("%5i%6.2f pb",i,nom);
     V=0;
     for (int j=0;j<30;++j) { // loop over all 30 PDF vars
       double nj = h_STXS_pdf4lhc[j]->GetBinContent(bin); V += pow(nj-nom,2);
       if (j<Npdf) printf("%7s",per(nj,nom).Data());
     }
     printf("%8s\n",per(sqrt(V)/nom).Data());
   }
   V=0; nom=h_STXS->Integral(0,-1);
   ...
   for (int j=0;j<Npdf;++j) printf("  PDF%2i",j+1); printf(" PDF1-30\n");
   for (int i=1;i<=Nstxs;++i) {
     int bin=i+1; double acc_nom=h_STXS->GetBinContent(bin)/tot;
