...
#ifndef ANP_EXECTABLEPRED_H
#define ANP_EXECTABLEPRED_H

/**********************************************************************************
 * @Package: TrigCostExec
 * @Class  : ExecTablePred
 * @Author : Doug Schaefer and Rustem Ospanov
 *
 * @Brief  : 
 *
...
namespace Anp
{
  class ExecTablePred: public virtual ExecClient {
  public:

    ExecTablePred();
    virtual ~ExecTablePred();
  ...
  private:
    ...
    typedef std::map<uint32_t,rCount>               ROSCountMap;
  ...
  private:
    ...
    ExecClient* GetROSClient(const uint32_t ros_id, const Exec::AlgCall &alg);
    rCount*     GetROSCount( const uint32_t ros_id, const Handle<ExecClient> client);
    pCount*     GetALGCount( const unsigned alg_id, CountVec& algs);
    pCount*     GetCHNCount( const unsigned chn_id, CountVec& chns);
  ...
  struct SortpCount {
    ...
    bool operator()(const ExecTablePred::pCount &lhs, const ExecTablePred::pCount &rhs) const;
  ...
  struct SortrCount {
    ...
    bool operator()(const ExecTablePred::rCount &lhs, const ExecTablePred::rCount &rhs) const;
