...
"""Read partition and dump configuration for ros/robin/rob to xml file.
...
xmldoc = Document()
xmldata = xmldoc.createElement("ros-robin-rob-data")
...
for ros in part.get('ROS'):
  ...
  for robin in ros.Contains:
    ...
    for rom in robin.Contains:
      ...
      del xmlrob
    del xmlrobin
    print '  ',robin.id,'= (',', '.join(rob_ids_print),')',
  del xmlros
  print ']'
