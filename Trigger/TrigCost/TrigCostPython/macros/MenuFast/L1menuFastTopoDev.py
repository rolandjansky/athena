...
def usage():
  print " usage: %s [-1|-2|-3] [-x] [-p [-b] [-w]] [-l lumi] <item1> [item2] ... " % sys.argv[0]
  ...
  print "     -x  use input ntuples with increased RoI multiplicity (better accuracy for topo selections)"
  sys.exit(1)
...
def main():
    lumi=1
    try:
        opts, triggers = getopt.getopt(sys.argv[1:], "ix123pbwl:")
    ...
    for opt,arg in opts:
        ...
        if opt=='-x':
            extendedRoIMultiplicity = True
            inputDir = "sorted5"
        if opt=='-i':
            extendedRoIMultiplicity = True
            inputDir = "sortedWithIso"
        

    global vars
    if extendedRoIMultiplicity:
        vars={'EM':    5,
              'EMH':   5,
              'EMHI':  5,
              'TAU':   5,
    ...
    eb=Tuple([('%s/eb.root' % (inputDir),ebweight*xsec8TeV)])
    mc8TeV=Tuple([('%s/low8TeV.root' % (inputDir),mcfudge*xsec8TeV*(1-eff8TeV)),
                  ('%s/high8TeV.root' % (inputDir),mcfudge*xsec8TeV*eff8TeV),
                  ])
#    mc8TeV=Tuple([('sorted/low14TeV.root',mcfudge*xsec14TeV*(1-eff14TeV)),
#                   ('sorted/high14TeV_v2.root',mcfudge*xsec14TeV*eff14TeV),
#                  ])
    mc14TeV=Tuple([('%s/low14TeV.root' % (inputDir),mcfudge*xsec14TeV*(1-eff14TeV)),
                   ('%s/high14TeV.root '% (inputDir),mcfudge*xsec14TeV*eff14TeV),
                  ])
    ...
    if pileup:
        ...
        mc8TeV=Tuple([('%s/mc8TeV_mu%d.root' % (inputDir,mu),mcfudge*xsec8TeV/mu)])
        if not realMC:
            print 'Added combinatorial pileup at mu=%d for 14 TeV MC' % muMC        
            mc14TeV=Tuple([('%s/mc14TeV_mu%d.root' % (inputDir,mu),mcfudge*xsec14TeV/mu)])
        else:
            ...
            mc14TeV=Tuple([('%s/mu%d_%s%s.root' % (inputDir,muMC,bcspace,ignorebc),mcfudge*xsec14TeV/muMC)])
