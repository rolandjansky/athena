...
namespace TrigCostRootAnalysis {
  ...
  void ROSConfService::parseRosXml() {
    ...
    if ( Config::config().getIsSet(kROSXMLPath) == kFALSE ) {
      Error("ROSConfService::parseRosXml","Unable to find ROS mapping: %s. No named ROS data.",Config::config().getStr(kROSXMLName).c_str());
      return;
    }
    ...
    if (_xmlDoc == 0) {
      Error("ROSConfService::parseRosXml", "Unable to parse ROS mapping: %s. No named ROS data.", Config::config().getStr(kROSXMLPath).c_str());
      delete _xml;
      return;
    }
    ...
    if ( Config::config().debug() ) {
      Info("ROSConfService::parseRosXml", "Using ROS <-> ROBIN and ROBIN <-> ROB Maps from %s", Config::config().getStr(kROSXMLName).c_str());
