...
// Each ChainItem has a Prescale and a chainProb, that is the probability that the chain passes raw each time it is called.
class ChainItem {
public:
...
public:
  ...
  void end() { 
    //Double_t _significanceWei = TMath::Abs(m_rateDirect - m_rateWeight)    / TMath::Sqrt( TMath::Max((Double_t)m_rateDirect, m_rateWeight   ));
...
public:
  ...
  virtual void runDirect() {
    ...
    for (ChainItemSetIt_t _L2It = m_L2s.begin(); _L2It != m_L2s.end(); ++_L2It) {
      ...
      if (_L2->getPassRawAndPS() == kFALSE ) return; // L2 did not pass - stop
      Bool_t _l1Pass = kFALSE;
      for (ChainItemSetIt_t _lowerIt = _L2->getLowerStart(); _lowerIt != _L2->getLowerEnd(); ++_lowerIt) {
        ChainItem* _L1 = (*_lowerIt);

        if ( _L1->getPassRawAndPS() == kFALSE ) continue; // L1 did not pass, try next
        _l1Pass = kTRUE;
        break;
  ...
  void removeRedundancies() {
    ...
    while (_L1ToTryRemoving < _L1Set.size()) {
      ...
      if (_L2Set.size() == 0) { // If true we could still reach all the counters *without* this L1
        Info("ChainAND::removeRedundancies","Removing redundant L1 chain %s from %s", (*_L1ToTryRemovingIt)->m_name.c_str(), m_name.c_str());
...
public:
  ...
  virtual void runDirect() {
    ...
    for (ChainItemSetIt_t _L2It = m_L2s.begin(); _L2It != m_L2s.end(); ++_L2It) {
      ...
      if (_L2->getPassRawAndPS() == kFALSE ) continue; // L2 did not pass, try next
  ...
  void classify() {
    ...
    if (_allToAll == kTRUE) {
      Info("ChainOR::classify","Chain %s topology classified as All-To-All (All L2 chains share an identical set of L1 seeds)", 
        m_name.c_str());
    ...
    if (_allOneToOne == kTRUE) {
      Info("ChainOR::classify","Chain %s topology classified as All-One-To-One (Parallel set of L2 items with only one unique seed each)",
        m_name.c_str());
    ...
    if (_allOneToMany == kTRUE) {
      Info("ChainOR::classify","Chain %s topology classified as All-One-To-Many (L1s may seed many L2s, but each L2 has only one seed)",
        m_name.c_str());
    ...
    Info("ChainOR::classify","Chain %s topology classified as Many-To-Many (Arbitrary mapping of L1s to L2s). Comp complexity (2^NL1-1)=%i",
