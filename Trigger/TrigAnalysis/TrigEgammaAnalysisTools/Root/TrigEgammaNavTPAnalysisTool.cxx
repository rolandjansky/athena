...
StatusCode TrigEgammaNavTPAnalysisTool::childExecute() /* const */ {

    m_eventCounter++;
    // cd(m_dir+"/Expert/Event"); // replaced by findGroup("Expert_Event")
    auto eventMG = findGroup("Expert_Event");

    hist1(eventMG, m_anatype+"_CutCounter")->Fill("Events",1);
    ...
    hist1(eventMG, m_anatype+"_CutCounter")->Fill("EventWise",1);
    // Select TP Pairs
    ...
    for(unsigned int ilist = 0; ilist != m_trigList.size(); ilist++) {
        ...
        // cd(m_dir+"/Expert/Event"); // replaced by findGroup("Expert_Event")
        if(tdt()->isPassed(probeTrigger))  hist1(eventMG, m_anatype+"_trigger_counts")->Fill(cprobeTrigger,1);
        ...
        for(unsigned int i=0;i<m_pairObj.size();i++){
            ...
            if(et > info.trigThrHLT + 1.0) {
                hist1(eventMG, m_anatype+"_nProbes")->Fill(cprobeTrigger,1);
            }
            if ( feat ) {
                if(et > info.trigThrHLT + 1.0){
                    hist1(eventMG, m_anatype+"_EffL1")->Fill(cprobeTrigger,acceptData.getCutResult("L1Calo"));
                    hist1(eventMG, m_anatype+"_EffL2Calo")->Fill(cprobeTrigger,acceptData.getCutResult("L2Calo"));
                    hist1(eventMG, m_anatype+"_EffL2")->Fill(cprobeTrigger,acceptData.getCutResult("L2"));
                    hist1(eventMG, m_anatype+"_EffEFCalo")->Fill(cprobeTrigger,acceptData.getCutResult("EFCalo"));
                    hist1(eventMG, m_anatype+"_EffHLT")->Fill(cprobeTrigger,acceptData.getCutResult("HLT"));
                    if( acceptData.getCutResult("L1Calo")){
                        hist1(eventMG, m_anatype+"_nProbesL1")->Fill(cprobeTrigger,1);
                    }
                    if( acceptData.getCutResult("L2Calo") ){
                        hist1(eventMG, m_anatype+"_nProbesL2Calo")->Fill(cprobeTrigger,1);
                    }
                    if( acceptData.getCutResult("L2") ){
                        hist1(eventMG, m_anatype+"_nProbesL2")->Fill(cprobeTrigger,1);
                    }
                    if( acceptData.getCutResult("EFCalo") ){
                        hist1(eventMG, m_anatype+"_nProbesEFCalo")->Fill(cprobeTrigger,1);
                    }
                    if( acceptData.getCutResult("HLT") ){
                        hist1(eventMG, m_anatype+"_nProbesHLT")->Fill(cprobeTrigger,1);
                    }
                }
            } // Features
            // Fill TProfile for no feature found (means no match)
            else {
                hist1(eventMG, m_anatype+"_EffL1")->Fill(cprobeTrigger,0);
                hist1(eventMG, m_anatype+"_EffL2Calo")->Fill(cprobeTrigger,0);
                hist1(eventMG, m_anatype+"_EffL2")->Fill(cprobeTrigger,0);
                hist1(eventMG, m_anatype+"_EffEFCalo")->Fill(cprobeTrigger,0);
                hist1(eventMG, m_anatype+"_EffHLT")->Fill(cprobeTrigger,0);
            }
        } // End loop over electrons
    } // End loop over trigger list
    // cd(m_dir+"/Expert/Event"); // replaced by findGroup("Expert_Event")
    hist1(eventMG, m_anatype+"_CutCounter")->Fill("Success",1);
...
StatusCode TrigEgammaNavTPAnalysisTool::childFinalize()
{
    ...
    ATH_MSG_WARNING("Old TrigEgammaNavTPAnalysisTool::childFinalize. plot() removal refactor TODO");
