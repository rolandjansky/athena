    ...
    m_lumiBlockMuTool("LumiBlockMuTool/LumiBlockMuTool") //offline mu
{
    ...
    // declareProperty("PlotTool",m_plot); // will be removed after migration completed
    // declareProperty("Tools", m_tools); // in header, private tools
...
StatusCode TrigEgammaAnalysisBaseTool::initialize() {
    ...
    ATH_MSG_DEBUG("Retrieving subtools... " << m_tools.size());

    for ( const auto htool : m_tools ) {
        ATH_MSG_DEBUG("Retrieving "<< htool <<"..." );
    ...
    ATH_MSG_DEBUG("Subtools retrieval completed.");
...
StatusCode TrigEgammaAnalysisBaseTool::book() {
    ...
    if (! m_MGlookup.isValid() ) {
        ATH_MSG_ERROR("Tool parent does not provides capability to lookup Monitoring groups by name");
        return StatusCode::FAILURE;
    }

    for ( auto htool : m_tools ) {
        ATH_MSG_INFO("Booking "<< htool <<"..." );
        htool->setMGLookup(m_MGlookup);
        sc = htool->book();
        if ( ! sc.isSuccess() ) {
...
ToolHandle<GenericMonitoringTool> TrigEgammaAnalysisBaseTool::findGroup( const std::string& name ) const {
    return m_MGlookup->findGroup(name);
}

void TrigEgammaAnalysisBaseTool::cd(const std::string &dir) {
    ATH_MSG_WARNING("Old TrigEgammaAnalysisBaseTool::cd     ('"<< dir << "') called");
    ATH_MSG_WARNING("Old TrigEgammaAnalysisBaseTool::cd@name='"<< name() << "'");
//    plot()->cd(dir);
}

void TrigEgammaAnalysisBaseTool::addDirectory(const std::string &dir) {
    ATH_MSG_WARNING("Old TrigEgammaAnalysisBaseTool::addDirectory('"<< dir << "') called");
//    plot()->addDirectory(dir);
}

void TrigEgammaAnalysisBaseTool::addHistogram(TH1 */*h*/, const std::string &/*dir*/) {
    ATH_MSG_WARNING("Old TrigEgammaAnalysisBaseTool::addHistogram TH1 called");
//    plot()->addHistogram(h,dir);
}

void TrigEgammaAnalysisBaseTool::addHistogram(TH2 */*h*/, const std::string &/*dir*/) {
    ATH_MSG_WARNING("Old TrigEgammaAnalysisBaseTool::addHistogram TH2 called");
//    plot()->addHistogram(h,dir);
}

void TrigEgammaAnalysisBaseTool::addTree(TTree */*t*/, const std::string &/*dir*/) {
    ATH_MSG_WARNING("Old TrigEgammaAnalysisBaseTool::addTree called");
//    plot()->addTree(t,dir);
}

void TrigEgammaAnalysisBaseTool::setLabels(TH1* /*histo*/, const std::vector<std::string>& /*labels*/) {
    ATH_MSG_WARNING("Old TrigEgammaAnalysisBaseTool::setLabels called");
//    plot()->setLabels(histo,labels);
}

std::unique_ptr<TrigEgammaAnalysisTools::TH1Wrapper> TrigEgammaAnalysisBaseTool::hist1(const ToolHandle<GenericMonitoringTool>& mgHandle, const std::string &histName) {
    return std::make_unique<TrigEgammaAnalysisTools::TH1Wrapper>(mgHandle, histName);
}

std::unique_ptr<TrigEgammaAnalysisTools::TH2Wrapper> TrigEgammaAnalysisBaseTool::hist2(const ToolHandle<GenericMonitoringTool>& mgHandle, const std::string &histName) {
    return std::make_unique<TrigEgammaAnalysisTools::TH2Wrapper>(mgHandle, histName);
}

std::unique_ptr<TrigEgammaAnalysisTools::TH1Wrapper> TrigEgammaAnalysisBaseTool::hist1(const std::string &histName/*, const std::string &dir*/) {
    ATH_MSG_WARNING("Transitional TrigEgammaAnalysisBaseTool::hist1('"<< histName  <<"') called");
    return std::make_unique<TrigEgammaAnalysisTools::TH1Wrapper>(m_TMPgroupHandle, histName);
//    return plot()->hist1(histName,dir);
}

std::unique_ptr<TrigEgammaAnalysisTools::TH2Wrapper> TrigEgammaAnalysisBaseTool::hist2(const std::string &histName/*, const std::string &dir*/) {
    ATH_MSG_WARNING("Transitional TrigEgammaAnalysisBaseTool::hist2('"<< histName  <<"') called");
    return std::make_unique<TrigEgammaAnalysisTools::TH2Wrapper>(m_TMPgroupHandle, histName);
//    return plot()->hist2(histName,dir);
}

TTree *TrigEgammaAnalysisBaseTool::tree(const std::string &treeName, const std::string &dir) {
    ATH_MSG_WARNING("Old TrigEgammaAnalysisBaseTool::tree('"<< treeName << "', '" << dir  <<"') called");
    //return plot()->tree(treeName,dir);
    return 0; // cause a crash for debug
}
...
void TrigEgammaAnalysisBaseTool::setMGLookup(SmartIF<IMonitoringGroupLookup> MGlookup) {
    m_MGlookup = MGlookup;
}

void TrigEgammaAnalysisBaseTool::setDetail(bool detail) {
    m_detailedHists=detail;
}

void TrigEgammaAnalysisBaseTool::setTP(bool tp) {
    m_tp=tp;
}

void TrigEgammaAnalysisBaseTool::setEmulation(bool doEmu) {
    m_doEmulation=doEmu;
}

void TrigEgammaAnalysisBaseTool::setEmulationTool(ToolHandle<Trig::ITrigEgammaEmulationTool> tool) {
    m_emulationTool=tool;
}

void TrigEgammaAnalysisBaseTool::setPVertex(const float onvertex, const float ngoodvertex) {
    m_nPVertex = onvertex;
    m_nGoodVertex = ngoodvertex;
}

void TrigEgammaAnalysisBaseTool::setAvgMu(const float onlmu, const float offmu) {
    m_onlmu=onlmu;
    m_offmu=offmu;
}

void TrigEgammaAnalysisBaseTool::setSGContainsRnn(bool contains) {
    m_sgContainsRnn=contains;
}

void TrigEgammaAnalysisBaseTool::setSGContainsTrigPhoton(bool contains) {
    m_sgContainsTrigPhoton=contains;
}
