...
StatusCode TrigEgammaAnalysisBaseTool::initialize() {
    ...
    ATH_MSG_INFO("Child initialize... ");
    ATH_CHECK( childInitialize() );
    ATH_CHECK( m_tools.retrieve() );
    
    // Always will be Distribution, Efficiency or Resolution tool
    for ( auto &htool : m_tools ) {
        ATH_CHECK( htool->initialize() );
        htool->setGMTools(m_GMTools);
        if ( htool->childInitialize().isFailure() ){
            ATH_MSG_ERROR("Tool child initialize failed: " << htool);
            return StatusCode::FAILURE;
        }
    }
...
StatusCode TrigEgammaAnalysisBaseTool::execute( const EventContext& ctx) const {

    ATH_MSG_DEBUG( "Executing tool " << name() );
    
    ATH_CHECK(childExecute(ctx));
    return StatusCode::SUCCESS;
}
...
StatusCode TrigEgammaAnalysisBaseTool::finalize() {
  ...
  ATH_MSG_DEBUG( "Finalizing tool " << name() );
    m_trigInfo.clear();
    ATH_CHECK(childFinalize());
    return StatusCode::SUCCESS;
}
...
bool TrigEgammaAnalysisBaseTool::isIsolated(const xAOD::Electron *eg, const std::string isolation) const {
...
bool TrigEgammaAnalysisBaseTool::isPrescaled(const std::string trigger) const {
...
asg::AcceptData TrigEgammaAnalysisBaseTool::setAccept( const TrigEgammaMatchingUtils::Element obj, const TrigInfo info) const {
    ...
    if(!info.trigL1){ // HLT item get full decision
        ...
        passedL2Calo = match()->ancestorPassed<xAOD::TrigEMClusterContainer>(obj);  
        passedEFCalo = match()->ancestorPassed<xAOD::CaloClusterContainer>(obj, "TrigEFCaloCalibFex");


        if(info.trigType == "electron"){
            ATH_MSG_DEBUG("Check for active features: TrigElectron, ElectronContainer, TrackParticleContainer");
            passedL2    = match()->ancestorPassed<xAOD::TrigElectronContainer>(obj);
            passedEF    = match()->ancestorPassed<xAOD::ElectronContainer>(obj);
            passedEFTrk = match()->ancestorPassed<xAOD::TrackParticleContainer>(obj,"InDetTrigTrackingxAODCnv_Electron_IDTrig");
        }
        else if(info.trigType == "photon"){
            ATH_MSG_DEBUG("Check for active features: TrigPhoton, PhotonContainer");
            passedL2 = match()->ancestorPassed<xAOD::TrigPhotonContainer>(obj);
            passedEF = match()->ancestorPassed<xAOD::PhotonContainer>(obj);
...
float TrigEgammaAnalysisBaseTool::dR(const float eta1, const float phi1, const float eta2, const float phi2) const {
...
float TrigEgammaAnalysisBaseTool::getEta2(const xAOD::Egamma* eg) const{
...
float TrigEgammaAnalysisBaseTool::getEt(const xAOD::Electron* eg) const{
...
float TrigEgammaAnalysisBaseTool::getEtCluster37(const xAOD::Egamma* eg) const{
    if(eg && (eg->caloCluster())){
        const xAOD::CaloCluster*   cluster  = eg->caloCluster();
...
float TrigEgammaAnalysisBaseTool::getDEmaxs1(const xAOD::Egamma *eg) const{
    if(eg){
        float emax2=0.;
...
float TrigEgammaAnalysisBaseTool::rTRT  (const xAOD::Electron* eg) const{
...
float TrigEgammaAnalysisBaseTool::getSigmaD0(const xAOD::Electron *eg) const{
...
float TrigEgammaAnalysisBaseTool::getD0sig(const xAOD::Electron *eg) const{
    const xAOD::TrackParticle* t = eg->trackParticle();
    float d0sigma=0.;
    if (t)
    {
...
float TrigEgammaAnalysisBaseTool::getEnergyBE0(const xAOD::Egamma *eg) const{
    if(eg && (eg->caloCluster())){
        const xAOD::CaloCluster*   cluster  = eg->caloCluster();
...
float TrigEgammaAnalysisBaseTool::getEnergyBE1(const xAOD::Egamma *eg) const{
    if(eg && (eg->caloCluster())){
        const xAOD::CaloCluster*   cluster  = eg->caloCluster();
...
float TrigEgammaAnalysisBaseTool::getEnergyBE2(const xAOD::Egamma *eg) const{
    if(eg && (eg->caloCluster())){
        const xAOD::CaloCluster*   cluster  = eg->caloCluster();
...
float TrigEgammaAnalysisBaseTool::getEnergyBE3(const xAOD::Egamma *eg) const{
    if(eg && (eg->caloCluster())){
        const xAOD::CaloCluster*   cluster  = eg->caloCluster();
...
float TrigEgammaAnalysisBaseTool::getEaccordion(const xAOD::Egamma *eg) const{
    if(eg && (eg->caloCluster())){
        const xAOD::CaloCluster*   cluster  = eg->caloCluster();
...
float TrigEgammaAnalysisBaseTool::getE0Eaccordion(const xAOD::Egamma *eg) const{
    if(eg && (eg->caloCluster())){
        const xAOD::CaloCluster*   cluster  = eg->caloCluster();
...
/*! Macros for plotting */
#define GETTER(_name_) float TrigEgammaAnalysisBaseTool::getShowerShape_##_name_(const xAOD::Egamma* eg) const \
{ float val{-99}; \
...
#define GETTER(_name_) float TrigEgammaAnalysisBaseTool::getIsolation_##_name_(const xAOD::Electron* eg) const\
{ float val{-99}; \
...
#define GETTER(_name_) float TrigEgammaAnalysisBaseTool::getIsolation_##_name_(const xAOD::Egamma* eg) const\
{ float val{-99}; \
    eg->isolationValue(val,xAOD::Iso::_name_); \
    return val; }
    GETTER(etcone20)
...
#define GETTER(_name_) float TrigEgammaAnalysisBaseTool::getCluster_##_name_(const xAOD::Egamma* eg) const\
{    if(eg && eg->caloCluster()) \
...
#define GETTER(_name_) float TrigEgammaAnalysisBaseTool::getTrack_##_name_(const xAOD::Electron* eg) const\
{    if(eg && eg->trackParticle()) \
...
#define GETTER(_name_) float TrigEgammaAnalysisBaseTool::getTrackSummary_##_name_(const xAOD::Electron* eg) const \
{ uint8_t val_uint8{0}; \
    if(eg){ \
...
#define GETTER(_name_) float TrigEgammaAnalysisBaseTool::getTrackSummaryFloat_##_name_(const xAOD::Electron* eg) const\
{ float val_float{0}; \
    if(eg){ \
...
#define GETTER(_name_) float TrigEgammaAnalysisBaseTool::getCaloTrackMatch_##_name_(const xAOD::Electron* eg) const\
{ float val={-99.}; \
    if(eg){ \
...
TrigInfo TrigEgammaAnalysisBaseTool::getTrigInfo(const std::string trigger) const{ 
...
bool TrigEgammaAnalysisBaseTool::splitTriggerName(const std::string trigger, 
...
void TrigEgammaAnalysisBaseTool::parseTriggerName(const std::string trigger, 
...
std::string TrigEgammaAnalysisBaseTool::getProbePid(const std::string pidtype) const{
...
std::string TrigEgammaAnalysisBaseTool::getL1Item(std::string trigger) const{
