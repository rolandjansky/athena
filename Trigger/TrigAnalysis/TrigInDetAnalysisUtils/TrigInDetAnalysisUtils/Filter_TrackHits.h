...
class Filter_TrackHits : public TrigInDetAnalysis::TrackFilter { 
  
 public:
  
  Filter_TrackHits(int bLayer, int pixel, int sct, int silicon, int straw, int tr) :
    m_bLayer(bLayer), m_pixel(pixel), m_sct(sct), m_silicon(silicon), m_straw(straw), m_tr(tr) { } 
  
    bool select(const TrigInDetAnalysis::Track* t, const TrigInDetAnalysis::TIDARoiDescriptor* =0) {
