...
public:
...
#else
      double dtheta      = (*measPer->covariance())(Trk::theta,Trk::theta);
      double dqovp       = (*measPer->covariance())(Trk::qOverP,Trk::qOverP); 
...
#else
      double dphi = (*measPer->covariance())(Trk::phi0,Trk::phi0);
      double dz0  = (*measPer->covariance())(Trk::z0,Trk::z0);
      double dd0  = (*measPer->covariance())(Trk::d0,Trk::d0);
#endif
  ...
  TrigInDetAnalysis::Track* makeTrack( const TruthParticle* track, unsigned long tid=0 ) { 
    ...
    if ( track->status() != 1 ) final_state = false;   /// check for final state
  ...
  void selectTrack( const xAOD::TrackParticle* track, void* =0) {
    ...
    if ( track ) { 
      ...
      double dtheta      = measPer->definingParametersCovMatrix()(Trk::theta,Trk::theta);
      double dqovp       = measPer->definingParametersCovMatrix()(Trk::qOverP,Trk::qOverP); 
      double covthetaOvP = measPer->definingParametersCovMatrix()(Trk::qOverP,Trk::theta); 

      double deta = 0.5*dtheta/(std::cos(0.5*theta)*std::cos(0.5*theta)*std::tan(0.5*theta));

      double dphi = measPer->definingParametersCovMatrix()(Trk::phi0,Trk::phi0);
      double dz0  = measPer->definingParametersCovMatrix()(Trk::z0,Trk::z0);
      double dd0  = measPer->definingParametersCovMatrix()(Trk::d0,Trk::d0);
