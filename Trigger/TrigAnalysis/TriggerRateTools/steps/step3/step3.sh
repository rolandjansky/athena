...
if [ $# -ne 2 ]; then
    if [ $# -ne 1 ]; then
        ...
        echo ""
        echo "USAGE example:"
        echo "    ./step3.sh config_ttbar.sh /raid/gaia/atlas/data/mc/cern\_ttbar"
        echo ""
...
export OUTPUT_PATH=$MAIN_OUTPUT_PATH/step3/$PREFIX"_"$FOLDER


cd $SCRIPT_PATH/step3
#rm -fr $PYTHON_DIR
mkdir -p $OUTPUT_PATH
...
MIN_FILE=0
MAX_FILE=$NUM_AODS_PER_STEP3

if [ $NUM_FILES -gt $MAX_NUM_AODS ]; then
    NUM_FILES=$MAX_NUM_AODS
fi

if [ $NUM_AODS_PER_STEP3 -ge 1 ]; then

    if [ $(( ((NUM_FILES/NUM_AODS_PER_STEP3)*NUM_AODS_PER_STEP3)-NUM_FILES )) -lt 0 ]; then
        export MAX_COUNT=$(( (NUM_FILES/NUM_AODS_PER_STEP3) ))
    else
        export MAX_COUNT=$(( (NUM_FILES/NUM_AODS_PER_STEP3)-1 ))
    fi
...
do
    ...
    echo "PoolAODInput = [" >> $OUTPUT_PATH/$PYTHON_DIR/$PREFIX.$COUNT_LBL.py
    
    MIN_FILE=$(( COUNT*NUM_AODS_PER_STEP3 ))
    MAX_FILE=$(( (COUNT+1)*NUM_AODS_PER_STEP3 ))
    ...
    if [ -n "$DATA_PATH" ]; then
    	...
    	#echo "data path; prefix"
        	python convert_data_files.py $DATA_PATH $PATH_PREFIX ../step3/temp_data_$PREFIX ../step3/conv_data_$PREFIX $MIN_FILE $MAX_FILE
        else
    	#echo "data path; no prefix"
        	python convert_data_files.py $DATA_PATH none ../step3/temp_data_$PREFIX ../step3/conv_data_$PREFIX $MIN_FILE $MAX_FILE
        fi
    else
        if [ -n "$PATH_PREFIX" ]; then
    	#echo "no data path; prefix"
            if [ `echo $USE_FULL_PATH | grep -c -i "true"` == 1 ]; then
                python convert_data_files.py $NON_LOCAL_ID $PATH_PREFIX ../step3/temp_data_$PREFIX ../step3/conv_data_$PREFIX $MIN_FILE $MAX_FILE
            else
                python convert_data_files.py none $PATH_PREFIX ../step3/temp_data_$PREFIX ../step3/conv_data_$PREFIX $MIN_FILE $MAX_FILE
            fi
        else  
    	#echo "no data path; no prefix"
            if [ `echo $USE_FULL_PATH | grep -c -i "true"` == 1 ]; then
        	python convert_data_files.py $NON_LOCAL_ID none ../step3/temp_data_$PREFIX ../step3/conv_data_$PREFIX $MIN_FILE $MAX_FILE
            else
        	python convert_data_files.py none none ../step3/temp_data_$PREFIX ../step3/conv_data_$PREFIX $MIN_FILE $MAX_FILE
            fi
        fi
    fi
    cd ../step3/
    ...
    echo "include(\"TriggerRateTools/TriggerRateTools_common_readAOD.py\")" >> $OUTPUT_PATH/$PYTHON_DIR/$PREFIX.$COUNT_LBL.py
