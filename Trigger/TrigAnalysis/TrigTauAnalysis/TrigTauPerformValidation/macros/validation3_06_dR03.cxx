...
void validation3_06_dR03(const char * filename, int type = 2, bool debug = false ) {
  ...
  TChain * chain = new TChain("eventTree");
   ...
   validation3_06_dR03(chain, type, debug);

}


void validation3_06_dR03(TTree * tree, int type = 2, bool debug = false)
{
  ...
  TrigTauBugFix bugfix(TrigTauBugFix::TTP6, TrigTauBugFix::RDO);
  ...
  if ( debug ) selmanager.l2CalSel.setDebug(true);
  ...
  for(int iev = 0; iev<nev; ++iev) {
    ...
    event->reset();
    if (debug) cout << "Event reset..."<< endl;
    tree->GetEvent(iev);
    if (debug) cout << "Event choosen..." << endl;
    bugfix.fix(event);
    
    if ( debug ) cout << "Analyze truth..." << endl;
	...
	if ( debug ) cout << " There are " << nTrigChains << " chains to analyze." << endl;
	   ...
	   if( truth->DeltaR(*l1object)<0.2 ) {
	     ...
	     if ( debug ) selmanager.l1Sel.setDebug(true);
	   ...
	   if ( debug ) cout << "     L2 DeltaR is :" << ( truth->DeltaR(*l2object) ) << endl;
           if( truth->DeltaR(*l2object)<0.2 ) {
		    ...
		    if (debug) selmanager.l2TrkSel.setDebug(true);
	   ...
	   if (efobject == 0 ) {
	     ...
	     if (debug) selmanager.efSel.setDebug(true);
  ...
  case 1:
    ...
    for (int j = 0; j != nHistos ; ++j ) {
      ...
      if (!((j-1)%3)) TH1F * effHist =(TH1F*)getEffHist(hTot[1],histograms[j],hnames[(j-1)/3]+"_absolut_eff", debug);
      if (!((j-2)%3)) TH1F * effHist =(TH1F*)getEffHist(hTot[2],histograms[j],hnames[(j-2)/3]+"_absolut_eff", debug);      
