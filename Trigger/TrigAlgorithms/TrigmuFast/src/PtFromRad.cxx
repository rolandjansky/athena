...
const string PtFromRad::s_name = "Pt estimate";

PtFromRad::PtFromRad(MuMessageInterface& msg,
                       MuFeature& feature,
                       const MuLUT* mu_lut)
...
PtFromRad::~PtFromRad(void)
{
}

bool
PtFromRad::run(MuonFeatureDetails* det)
{
	...
	if(phibin>=lut.NbinPhi[add]) phibin = lut.NbinPhi[add]-1;
        ...
        if(m_feature.Radius!=0.) {
            ...
            if((neweta<0||neweta>=lut.NbinEta[add])&&
               (newphi<0||newphi>=lut.NbinPhi[add])) {
                m_feature.Pt = m_feature.Radius*A0[0] + A1[0];
		...
		return true;
                ...
                m_feature.Pt = m_feature.Radius*A0[2] + A1[2];
		if(det!=0) det->setPt(m_feature.Pt*m_feature.Charge);
                return true;
		...
		if(det!=0) det->setPt(m_feature.Pt*m_feature.Charge);
                ...
                if(disteta >= distphi*lut.EtaStep[add]/lut.PhiStep[add]) {
                    ...
                    m_feature.Pt = m_feature.Radius*A0[5] + A1[5];
		    if(det!=0) det->setPt(m_feature.Pt*m_feature.Charge);
                    return true;
                } else {
                    A0[1] = lut.table[add][ch][etabin][newphi][0];
