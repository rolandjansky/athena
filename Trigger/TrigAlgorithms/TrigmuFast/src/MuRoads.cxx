...
MuRoads::MuRoads(float scaleInner, float scaleMiddle, float scaleOuter)
{
    if( scaleInner==0. ) scaleInner  = 1.;
    if( scaleMiddle==0.) scaleMiddle = 1.;
    if( scaleOuter==0. ) scaleOuter  = 1.;
    
    for(int th=0;th<2;++th)
        for(int pos=0;pos<4;++pos) 
	    for(int ly=0;ly<8;++ly) {
	        m_roads[th][pos*3][ly] = scaleInner*MuonRoads[th][pos*3][ly];
		m_roads[th][pos*3+1][ly] = scaleMiddle*MuonRoads[th][pos*3+1][ly];   
	    	m_roads[th][pos*3+2][ly] = scaleOuter*MuonRoads[th][pos*3+2][ly];   
            }
    ...
    for(i=0;i<2;i++) for(j=0;j<12;j++) {
        max=0.;
        for(k=0;k<8;k++) if(m_roads[i][j][k]>=max) max = m_roads[i][j][k];
        m_max_roads[i][j] = max;
    }
}

MuRoads::~MuRoads(void)
{
    // probably nothing to do, since s_roads and s_max_roads are static
}
...
//const MuRoads*
//MuRoads::instance(void)
//{
//    if (! s_instance) {
//        s_instance = new MuRoads;
//    }
