/*
  ...
  Authors: JJamieson, University of Glasgow, Created: 09/10/2017, Last edit: 05/02/2018
  ...
  for(auto muon : *muonContainer) {
	...
	//------------------------10 GeV minimum pT cut applied--------------------------------------------
        ...
        if (!tr || tr->pt()/CLHEP::GeV < 10.0) { // if this muon has NOT been formed from a combined track > 10GeV
          ...
          ATH_MSG_DEBUG("Bad muon is of type: " << muon->muonType() << " With 0=combined, 1=MuonSA, 2=SegmentTagged, 3=CaloTagged, 4=SiliconAssociatedForwardMuon, Abs pt = " << muon->pt()/CLHEP::GeV << " GeV. Charge=" << muon->charge() << " Psuedorapidity = " << muon->eta() << " Phi = " << muon->phi());
          continue;
        } 
	...
	//---------------------------Collect good muons--------------------------------------------------
	  ...
	  ATH_MSG_DEBUG("Retrieved muon of type " << muon->muonType() << ": Combined track with abs pt " << tr->pt()/CLHEP::GeV << " GeV. Charge=" << tr->charge() << " match chi2 = " << tr->chiSquared() << " Psuedorapidity = " << tr->eta() << " Phi = " << tr->phi());
...
void TrigMuonEFTagandProbe::match_thresh (TaPMuon tap ,MonitoringObject Thresh_Mon){
  ...
  if (abs(tap.Mu->eta())<1.05){
    for (unsigned int i=0;i<tap.Thresh;i++){ //Loop over all thresholds <= current probe threshold value
      if ((tap.Mu->pt()/CLHEP::GeV) > 25.0){ //Apply extra 25GeV pT cut to eta and phi variables
	...
	Thresh_Mon.fill_eta_matchfail_merged(i,1.0);
      ...
      Thresh_Mon.fill_pt_total_barrel(i,tap.Mu->pt()/CLHEP::GeV);
  ...
  //Endcap
	...
	Thresh_Mon.fill_eta_matchfail_merged(i,1.0);
      ...
      Thresh_Mon.fill_pt_total_endcap(i,tap.Mu->pt()/CLHEP::GeV);
...
void TrigMuonEFTagandProbe::match_thresh (const xAOD::Muon* muon , MonitoringObject Thresh_Mon){
  ...
  if (abs(muon->eta())<1.05){
	...
	Thresh_Mon.fill_eta_matchfail_merged(i,0.0);
      ...
      Thresh_Mon.fill_pt_total_barrel(i,muon->pt()/CLHEP::GeV);
  ...
  //Endcap
	...
	Thresh_Mon.fill_eta_matchfail_merged(i,0.0);
      ...
      Thresh_Mon.fill_pt_total_endcap(i,muon->pt()/CLHEP::GeV);
...
void TrigMuonEFTagandProbe::trim_container (std::vector<const xAOD::Muon*>& good_muons){
  ...
  for (unsigned int itmu1=0;itmu1<good_muons.size();itmu1++){ //Loop over muons
	...
	continue;
      ...
      double tempinvmass = abs(91.1876-((mu1->p4() + mu2->p4()).M())/CLHEP::GeV);
