...
namespace HLT { namespace MET {
    ...
    m_cvfTool("CVFTool/CVFTool", this)
  {
    ...
    declareProperty("doVS", m_doVS = true, "Run VS algorithm?");
    declareProperty("doSK", m_doSK = true, "Run SK algorithm?");
    declareProperty("doCVF", m_doCVF = true, "Run CVF algorithm?");
    declareProperty("doTST", m_doTST = true, "Use Track Soft Term?");
  ...
  HLT::ErrorCode TrkTCFex::hltInitialize()
  {
    ...
    if (!m_doCVF && m_doTST) {
      ATH_MSG_ERROR("TST is set up to ONLY be ably to operate when CVF is called; This will break things!");
      return HLT::ERROR;
    }
  ...
  {
    ATH_MSG_DEBUG("Within hltFinalize.");
    return HLT::OK;
  }
      ...
      const std::vector<std::vector<HLT::TriggerElement*>>& tes_in)
  {
    ...
    ATH_MSG_DEBUG("Retrieveing input trigger elements:");
    if ( (m_doTST && m_doMuonOR && tes_in.size() != 3) || (m_doCVF && !m_doTST && tes_in.size() != 2) 
	 || (!m_doCVF && tes_in.size() != 1) || tes_in.at(0).size() != 1 
	 || (m_doCVF && tes_in.at(1).size() != 1) ) {
    ...
    if (m_doTST && m_doMuonOR && tes_in.at(2).size() != 0) {
	...
	ATH_MSG_DEBUG("modifying calo clusters successfull for tool:");
	ATH_MSG_DEBUG(modTool);
      }
    }
    // Now do CVF
    if (m_doCVF && !m_cvfTool->calculateCVF(&modClusters, tracks, priVtx).isSuccess() ) {
    ...
    for (std::size_t ii = 0; ii < sums.size(); ++ii) {
      ...
      ATH_MSG_DEBUG("Filled MET Component:" << ii << "!");
    }
    if(m_doTST) {
