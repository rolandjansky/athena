...
HLT::ErrorCode TrigCheckForMuons::hltExecute(std::vector<std::vector<HLT::TriggerElement*> >& inputTE, unsigned int output)
{
  ...
  for(const auto& muon : muonVec ){
	...
	TrigRoiDescriptor roi( m_roIEta, etaMin, etaMax, m_roIPhi, phiMin, phiMax );
	...
	if( m_addOppositePhiRoI || m_addOppositeEtaPhiRoI) {
	  ...
	  if( m_addOppositePhiRoI ) { //To now look for muons in the opposite phi direction, create a second TrigRoiDescriptor
	    TrigRoiDescriptor roi2( m_roIEta, etaMin, etaMax, m_roIPhi_2 + M_PI, phiMin_2, phiMax_2 );

	    // now add ROBs
	    ec = m_robSelector->fillPEBInfo(*pebInfo, roi2, &m_dets, &m_nROBs);
	    if (ec != HLT::OK) {
	      if (debug) msg() << MSG::DEBUG << "Failed to record PEB for roi2" << endmsg;
	      return ec;
	    }
	  }
	  if( m_addOppositeEtaPhiRoI ) {//To now look for muons in the opposite eta and phi direction, 
	    ...
	    TrigRoiDescriptor roi2( m_roIEta, etaMax_2, etaMin_2, m_roIPhi_2 + M_PI, phiMin_2, phiMax_2 );
