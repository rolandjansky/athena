                                                    ...
                                                    double                              roiEtaMaxHigh) const
{
  ...
  if (m_use_rpc && !insideOut) {
    ...
    if ( (*rpcPatFinder)->findPatternEta(aw, bw, rpc_pattern, rpcLayerHits) ) {
      ...
      for(int i=0;i<3;i++){
	if(std::abs(1.0/aw[i]) <= ZERO_LIMIT) rpcFitResult.isSuccess = false;
      }
    } else {
  ...
  if (rpcFitResult.isSuccess) {
    ...
    const double thetaLow  = std::atan(std::exp(-std::abs(roiEtaLow)))*2.;
    const double thetaHigh  = std::atan(std::exp(-std::abs(roiEtaHigh)))*2.;
    const double awLow     = (std::abs(roiEtaLow) > ZERO_LIMIT)? std::tan(thetaLow)*(std::abs(roiEtaLow)/roiEtaLow): 0.;
    const double awHigh     = (std::abs(roiEtaHigh) > ZERO_LIMIT)? std::tan(thetaHigh)*(std::abs(roiEtaHigh)/roiEtaHigh): 0.;
