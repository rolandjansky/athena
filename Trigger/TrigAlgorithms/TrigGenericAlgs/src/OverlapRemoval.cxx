...
HLT::ErrorCode OverlapRemoval::acceptInputs(HLT::TEConstVec& tes_in, bool& pass)
{
  if ( msgLvl() <= MSG::VERBOSE )
    msg() << MSG::VERBOSE << "Executing this OverlapRemoval " << name() << endreq;
  ...
  for(int j=0; j < (NumArg-1); ++j) {
	...
	if (deltaphi <= m_MinPhiDist || deltaeta<=m_MinEtaDist) {
	  if ( msgLvl() <= MSG::DEBUG ) {
	    msg() << MSG::DEBUG << "The two RoIs are overlapping:" << endreq;
	    msg() << MSG::DEBUG << *descr0 << endreq;
	    msg() << MSG::DEBUG << *descr1 << endreq;
	  ...
	  if ( msgLvl() <= MSG::DEBUG ) {
	    msg() << MSG::DEBUG <<"The two RoIs do not not overlap: " << endreq;
	    msg() << MSG::DEBUG << *descr0 << endreq;
	    msg() << MSG::DEBUG << *descr1 << endreq;
  ...
  if ( msgLvl() <= MSG::DEBUG ) {
    msg() << MSG::DEBUG << "This combination is non-overlaping" << endreq;
  }
