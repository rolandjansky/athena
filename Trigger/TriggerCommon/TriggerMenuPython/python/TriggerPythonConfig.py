...
#from TrigConfigSvc.TrigLVL1configExtented import extendLVL1config (now in this file)
        ...
        for sig, chains in self.allChains.iteritems():
            ...
            for c in chains:
                ...
                if c.chain_name in keys: # chain already defined before
                    ...
                    log.debug(' TE list already found TEs: %s' % str(TEsigs.keys()))
        ...
        for  cl in self.allChains.values():
            for chain in cl:
                if chain.chain_name == name:
                    found.append(chain)
        if len(found)>1:
#            print 'ARGH - multiple instances for',name,' found -',found
            idx=0
            for i in range(1,len(found)):
                if len(found[idx].siglist)<len(found[i].siglist):
#                    print 'ARGH - length not equal',name,len(found[idx].siglist), len(found[i].siglist)
                    idx=i
            return found[idx]
        if found:
            return found[0]
...
####################################################################################################
    ...
    def checkChains(self):
        ...
        for counter in repcounters:
            ...
            logger().error( str(map( lambda x: x.chain_name,  filter(lambda x: x.chain_counter == counter, self.theL2HLTChains))) )
            correct=False
        ...
        for counter in repcounters:
            ...
            logger().error( str(map( lambda x: x.chain_name, filter(lambda x: x.chain_counter == counter, self.theEFHLTChains))) )
            correct=False
    ...
    def resolveDuplicateTEs(self):
        ...
        while True:
            ...
            seq_list1 = seq_list2
            iterationCount += 1
            log.debug('N TE remapping after iteration %d: %d' % \
        ...
        for chain in (self.allL2Chains() + self.allEFChains()):
