...
class CommonSliceHelper:
    ...
    def enableSignature(self, l):
        """ for backward compatibility """
        self.enableSignatures([l])
        
    def enableSignatures(self, list):
        current=self.signatures.get_Value()        
        new = [sig for sig in list if sig not in self.signatures.get_Value() ]
        new.extend(self.signatures.get_Value())
    ...
    def disableSignatures(self, list):
        current = self.signatures.get_Value()        
        new = [sig for sig in current if sig not in list ]
        self.signatures.set_Value(new)
