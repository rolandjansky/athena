...
def _addTopoInfo(theChainDef,chainDicts,listOfChainDefs,doAtL2AndEF=True): 
    ...
    if any("razor" in alg for alg in topoAlgs):
        ...
        # tau-tau, tau-mu, tau-el topo chains with dR cut
        ...
        print chainDict
        if chainDict['chainName'].count("TAU") == 2:
            chain_type = "ditau"
        elif "MU" in chainDict['chainName'].lower() and "TAU" in chainDict['chainName'].lower():
            chain_type = "mutau"
        elif "EM" in chainDict['chainName'].lower() and "TAU" in chainDict['chainName'].lower():
            chain_type = "eltau"

        log.debug("Chain {} is of type {}".format(chainDict['chainName'], chain_type))
        ...
        TauTaudR_Hypo = EFTauTopoHypo(
            'EFTauTopo_{0}dR{1}_{2}'.format(mindR, maxdR, chain_type),
            theVars, theThresh)
