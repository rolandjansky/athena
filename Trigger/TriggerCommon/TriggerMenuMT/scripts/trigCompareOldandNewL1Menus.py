...
class MenuComp:
    def __init__(self, files ):
        ...
        for fileName in files:
            if fileName.endswith(".json"):
                ...
                if fileName.startswith("L1Topo"):                
                    ...
                    elif "21.3" in fileName: # r21.3
                        self.fn_r213_topo = fileName
        ...
        self.r22_l1_xml  = L1MenuXMLReader(self.fn_r22_l1_xml) if self.fn_r22_l1_xml else None
        ...
        self.r213_topo   = ET.parse(self.fn_r213_topo).getroot() if self.fn_r213_topo else None
        ...
        log.info("Release 21.3 L1Topo     : %s", self.fn_r213_topo  )
    ...
    def compareL1MenusXMLwithJSON(self):
        if self.r22_l1_xml is None or self.r22_l1_json is None: 
            return
        l1items_xml  = self.r22_l1_xml.getL1Items()
        ...
        log.info("CHECK %i: All items from r22 xml should be in json: %s", cc, boolStr(self.check[cc]) )
        if not self.check[cc]:
            log.debug("These %i items are in xml but not in json (but should be!):", len(itemsOnlyInXML))
            for x in itemsOnlyInXML:
                log.debug("    %s", x)
        # CHECK 2
        cc = len(self.check)
        self.check += [ len(legacyItemsOnlyInJson)==0 ]
        log.info("CHECK %i: There should be no legacy items in json that are not in xml: %s", cc, boolStr(self.check[cc]) )
        if not self.check[cc]:
