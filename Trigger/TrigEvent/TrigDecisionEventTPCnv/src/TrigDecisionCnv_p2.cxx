                                     ...
                                     TrigDecision_p2* pers, MsgStream &log)
{
  ...
  log << MSG::DEBUG << "TrigDecisionCnv_p2::transToPers called " << trans << " " << pers << endreq;

  pers->m_configMasterKey = trans->m_configMasterKey;
  
  
  m_lvl1ResultCnv.transToPers(&trans->m_l1_result, &pers->m_l1_result, log);
  //     log << MSG::ERROR << "TrigDecisionCnv_p2::done L1 " << &trans->m_l1_result << " " << &pers->m_l1_result << endreq;


     const_cast<DataLink<HLT::HLTResult>& >(trans->m_l2_result).toPersistent();
     //     log << MSG::ERROR << "TrigDecisionCnv_p2::doooing L2 " 
     //	 << " " << trans->m_l2_result.dataID() <<endreq;

     //     log << MSG::ERROR << "TrigDecisionCnv_p2::printing L2 >" 
  ...
  dataLinkConverter.transToPers(trans->m_l2_result, pers->m_l2_result, log);
     ...
     //     log << MSG::ERROR << "TrigDecisionCnv_p2::done0 L2 " << endreq;
     //     log << MSG::ERROR << "TrigDecisionCnv_p2::done L2 " << &trans->m_l2_result << " " << &pers->m_l2_result <<endreq;

     //     log << MSG::ERROR << "TrigDecisionCnv_p2::doing EF " << &trans->m_ef_result << " " << &pers->m_ef_result <<endreq;

  dataLinkConverter.transToPers(trans->m_ef_result, pers->m_ef_result, log);

  //     log << MSG::ERROR << "TrigDecisionCnv_p2::done EF " << &trans->m_ef_result << " " << &pers->m_ef_result << endreq;
...
{
  log << MSG::DEBUG << "TrigDecisionCnv_p2::persToTrans called " << endreq;
