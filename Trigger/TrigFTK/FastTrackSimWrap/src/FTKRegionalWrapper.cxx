...
StatusCode FTKRegionalWrapper::initialize()
{
  ...
  if( m_ITkMode ) {
    ...
    ATH_MSG_FATAL("Failed to retrieve tool " << m_sct_cabling_svc);
    return StatusCode::FAILURE;
  } else {
    ATH_MSG_VERBOSE("Retrieved tool " << m_sct_cabling_svc);
  }
  ...
  //    ATH_MSG_FATAL <<"EventStore service not found" );
  //    return StatusCode::FAILURE;
  //  }
  ...
  if (m_getOffline) {
	...
	for (; it!=it_end; it++) {
	  ...
	  if ((*it)->type(Trk::TrackStateOnSurface::Measurement) ){
		...
		offline_locX->push_back(static_cast<float>(measurement->localParameters()[Trk::locX]));
		offline_locY->push_back(static_cast<float>(measurement->localParameters()[Trk::locY]));
	      }
