...
FTK_RawTrack* FTK_RDO_CreatorAlgo::SimToRaw(const FTKTrack &track)
{
  ...
  for (unsigned int iPlane = 0, isct=0; iPlane<(unsigned int)track.getNPlanes(); ++iPlane){
	...
	if (truth_ok) sct_clus.setBarcode(hitbarcode.first);
	...
	if (truth_ok) pixel_clus.setBarcode(hitbarcode.first);
...
void FTK_RDO_CreatorAlgo::printTrack(const FTKTrack& track, const FTK_RawTrack *raw_track) {
  ...
  for( unsigned int i = 0; i < raw_track->getPixelClusters().size(); ++i, iHit++){
    ...
    if (hit.getDim() ==0) {
      ...
      if (ok) hitbarcode = mtbarcode.first;
      std::cout << " FTKHit              " << i <<
	...
	" Barcode= 0x" << std::hex << hitbarcode << " Event 0x" << std::hex << mtbarcode.second << std::dec << std::endl;	
	...
	" Hash ID= 0x" << std::hex << raw_track->getPixelClusters()[i].getModuleID() << 
	" Barcode= 0x" << raw_track->getPixelClusters()[i].getBarcode() << std::dec<< std::endl;
    }
  }
  for( unsigned int isct = 0; isct < raw_track->getSCTClusters().size(); ++isct, iHit++){
	...
	" Barcode= 0x" << std::hex << hitbarcode << std::dec <<std::endl;	
	...
	" Barcode= 0x" << std::hex<< raw_track->getPixelClusters()[isct].getBarcode() << std::dec<< std::endl;
    }
  }
  std::cout << std::endl;
}
