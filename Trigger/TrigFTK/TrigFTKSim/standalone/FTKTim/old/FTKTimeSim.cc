...
FTKTimeSim::FTKTimeSim()   //Instance set initialize
{
  ...
  for(int idx=0;idx<NREGION;idx++) {
   ...
   // * hit distribution * //
   int n_hitmax=80000;
   h_ftkhit = new TH1D("h_ftkhit", "h_ftkhit", n_hitmax/3, 0, n_hitmax);
   h_ftkss = new TH1D("h_ftkss", "h_ftkss", n_hitmax/3, 0, n_hitmax);
...
FTKTimeSim::FillResults(int dump, int jentry, char *description, int temp_denom, int s_split)  //Fill Section
{
  ...
  if(FTKTimeSim_Debug>2)cout<<"Event Summary  : This is Fill results ()    event = "<<jentry<<endl;
  if(FTKTimeSim_Debug>2)cout<<"XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"<<endl;
  //  int temp_denom=1;
  Region regtemp(-99);
  vector<double> temp;
...
//   DF_EwOut=regtemp.GetMaxEwOut(iot)/temp_denom;
...
//   DF_EwIn=regtemp.GetMaxEwIn(iot)/temp_denom;
  ...
  //  if(dump){
    ...
    h_t->GetYaxis()->SetBinLabel(3,"Towers");
    h_t->GetYaxis()->SetBinLabel(2," ");
    h_t->GetYaxis()->SetBinLabel(1,"total time"); //2013
    if(temp_denom==1000)h_t->SetXTitle("Execution time (#mu sec)");
    ...
    //    char temp_outdir[256];
    sprintf(temp_outdir,"output/%s/c_time_detail_%03d.eps",description,jentry);
    c_t->SaveAs(temp_outdir);
    sprintf(temp_outdir,"output/%s/gif/c_time_detail_%03d.gif",description,jentry);
    //c_t->SaveAs(temp_outdir);
...
//     cout<<SecStageTF_EwIn<<" "<<SecStageTF_EwOut<<endl;
    ...
    sprintf(temp_outdir,"output/%s/c_time_split_detail_%d.eps",description,jentry);
    c_t->SaveAs(temp_outdir);
    sprintf(temp_outdir,"output/%s/gif/c_time_split_detail_%d.gif",description,jentry);
    //    c_t->SaveAs(temp_outdir);
...
FTKTimeSim::PrintResults(double sf_hit_overlapregion, int nloop, int nloopsloop, double sf_nregion, char *description2, int ntrack_ok, int ntrack_tot, int s_split) 
{
  ...
  cout<<"# of total boad            : "<<//temp_ntimeregion<<"     "<<
    //    (NDATAFORMATTER+NDATAORGANIZER+NASSOCIATIVEMEMORY+NTRACKFITTER/NGF)*NREGION*temp_ntimeregion<<endl;
    "-- "<<endl;
...
FTKTimeSim::PlotInputValues(char *description, int s_split, int nrbin)  //For Input Values 
{
  ...
  sprintf(temp_outdir,"mkdir -p output/%s",description);
  system(temp_outdir);
  sprintf(temp_outdir,"mkdir -p output/%s/gif",description);
//  system(temp_outdir);
  ...
  if(plot_nss){
    ...
    h_ftkss->Fit("gaus");
    h_ftkss->Draw();
    sprintf(temp_outdir,"output/%s/c_nftkss.eps",description);
    c_nftkss->SaveAs(temp_outdir);
    sprintf(temp_outdir,"output/%s/gif/c_nftkss.gif",description);
    //    c_nftkss->SaveAs(temp_outdir);
  }
  ...
  if(plot_nftkhit_layer){
    ...
    h_ftkhit->Fit("gaus");
    h_ftkhit->Draw();
    sprintf(temp_outdir,"output/%s/c_nftkhit.eps",description);
    c_nftkhit->SaveAs(temp_outdir);
    sprintf(temp_outdir,"output/%s/gif/c_nftkhit.gif",description);
    //    c_nftkhit->SaveAs(temp_outdir);
    // each layer //
    ...
    sprintf(temp_outdir,"output/%s/c_nftkhit_layer.eps",description);
    c_nftkhit_layer->SaveAs(temp_outdir);
    sprintf(temp_outdir,"output/%s/gif/c_nftkhit_layer.gif",description);
    //    c_nftkhit_layer->SaveAs(temp_outdir);
  }
  ...
  if(plot_nroad){
    ...
    //    h_road->Fit("gaus");
    h_road->Draw();
    sprintf(temp_outdir,"output/%s/c_road.eps",description);
    c_road->SaveAs(temp_outdir);
    sprintf(temp_outdir,"output/%s/gif/c_road.gif",description);
    //    c_road->SaveAs(temp_outdir);
    ...
    sprintf(temp_outdir,"output/%s/c_road_AM.eps",description);
    c_road_AM->SaveAs(temp_outdir);
    sprintf(temp_outdir,"output/%s/gif/c_road_AM.gif",description);
    //    c_road_AM->SaveAs(temp_outdir);
   }
  ...
  if(plot_nroad_rw){
    ...
    //    h_road_rw->Fit("gaus");
    h_road_rw->Draw();
    sprintf(temp_outdir,"output/%s/c_road_rw.eps",description);
    c_road_rw->SaveAs(temp_outdir);
    sprintf(temp_outdir,"output/%s/gif/c_road_rw.gif",description);
    //    c_road_rw->SaveAs(temp_outdir);
    ...
    sprintf(temp_outdir,"output/%s/c_road_rw_AM.eps",description);
    c_road_rw_AM->SaveAs(temp_outdir);
    sprintf(temp_outdir,"output/%s/gif/c_road_rw_AM.gif",description);
    //    c_road_rw_AM->SaveAs(temp_outdir);
   }
  cout<<"check!"<<endl;
  ...
  if(plot_nfit){
    ...
    h_fit->Draw();
    cout<<"check6!"<<endl;   
    sprintf(temp_outdir,"output/%s/c_fit.eps",description);
    c_fit->SaveAs(temp_outdir);
    sprintf(temp_outdir,"output/%s/gif/c_fit.gif",description);
    //    c_fit->SaveAs(temp_outdir);
  }
  ...
  if(plot_vs){
    ...
    sprintf(temp_outdir,"output/%s/c_vs.eps",description);
    c_vs->SaveAs(temp_outdir);
    sprintf(temp_outdir,"output/%s/gif/c_vs.gif",description);
    //    c_vs->SaveAs(temp_outdir);
  }
  ...
  if(plot_nfit_2nd){
    ...
    h_SecStage_fit->Draw();
    sprintf(temp_outdir,"output/%s/c_SecStage_fit.eps",description);
    c_SecStage_fit->SaveAs(temp_outdir);
    sprintf(temp_outdir,"output/%s/gif/c_SecStage_fit.gif",description);
    //      c_SecStage_fit->SaveAs(temp_outdir);
  }
  ...
  if(s_split){
    if(plot_nroad){
      ...
      sprintf(temp_outdir,"output/%s/c_SecStage_road.eps",description);
      c_SecStage_road->SaveAs(temp_outdir);
      sprintf(temp_outdir,"output/%s/gif/c_SecStage_road.gif",description);
      //      c_SecStage_road->SaveAs(temp_outdir);
	...
	h_SecStage_road_AM[i]->Draw();
      ...
      sprintf(temp_outdir,"output/%s/c_SecStage_road_AM.eps",description);
      c_SecStage_road_AM->SaveAs(temp_outdir);
      sprintf(temp_outdir,"output/%s/gif/c_SecStage_road_AM.gif",description);
      //      c_SecStage_road_AM->SaveAs(temp_outdir);
    }
    // ** plot for number of fit  ** //
    if(plot_nfit){
...
FTKTimeSim::PlotResults(char *description, int temp_denom, int s_split, int nrbin)  // For Results
{
  ...
  if(plot_timeee){
    ...
    c_ntimeee->cd(1);
    if(temp_denom==1000)h_timeee->SetXTitle("Execution time (#mu sec)");
    if(temp_denom==1)h_timeee->SetXTitle("Execution time (#mu sec");
    ...
    h_timeee->Draw();
    tex->Draw("same");
    sprintf(temp_outdir,"output/%s/c_ntimeee.eps",description);
    c_ntimeee->SaveAs(temp_outdir);
    sprintf(temp_outdir,"output/%s/gif/c_ntimeee.gif",description);
    //    c_ntimeee->SaveAs(temp_outdir);
  }
  if(plot_timeee){
    //    gStyle->SetOptStat(0111001);
    ...
    c_ntimeee_full->cd(1);
    if(temp_denom==1000)h_timeee->SetXTitle("#mu sec");
    if(temp_denom==1)h_timeee->SetXTitle("n sec");
    h_timeee->SetYTitle("events");
    //    h_timeee->Rebin(rebin);
    ...
    h_timeee->Draw();
    tex->Draw("same");
    sprintf(temp_outdir,"output/%s/c_ntimeee_full.eps",description);
    c_ntimeee_full->SaveAs(temp_outdir);
    sprintf(temp_outdir,"output/%s/gif/c_ntimeee_full.gif",description);
    //    c_ntimeee_full->SaveAs(temp_outdir);
  }
  ...
  if(plot_each_ewout){
    ...
    sprintf(temp_outdir,"output/%s/event_detail.eps",description);
    c_event_detail->SaveAs(temp_outdir);
    sprintf(temp_outdir,"output/%s/gif/event_detail.gif",description);
    //    c_event_detail->SaveAs(temp_outdir);
  ...
  if(plot_maxregion){ // plot maxregion
    ...
    sprintf(temp_outdir,"output/%s/c_maxregion.eps",description);
    c_maxregion->SaveAs(temp_outdir);
    sprintf(temp_outdir,"output/%s/gif/c_maxregion.gif",description);
    //    c_maxregion->SaveAs(temp_outdir);
  }
  ...
  if(plot_history){ // histroy plot
    ...
    double DO_EwOut_array[NREGION][xAxis_vec.size()];
    double AM_EwOut_array[NREGION][xAxis_vec.size()];
    double SecDO_EwOut_array[NREGION][xAxis_vec.size()];
    double TF_EwOut_array[NREGION][xAxis_vec.size()];
    double SecStageSecDO_EwOut_array[NREGION][xAxis_vec.size()];
    double SecStageTF_EwOut_array[NREGION][xAxis_vec.size()];
    ...
    sprintf(temp_outdir,"output/%s/TFEwOutHistory.eps",description);
    //    sprintf(temp_outdir,"output/%s/TFEwOutDivHistory.eps",description);
...
//    leg->SetFillColor(0);
    ...
    sprintf(temp_outdir,"output/%s/AllWeOutHistory.eps",description);
...
//    leg_reg1->SetFillColor(0);
    ...
    sprintf(temp_outdir,"output/%s/Reg1EwOutHistory.eps",description);
    ...
    sprintf(temp_outdir,"output/%s/Regsep0EwOutHistory.eps",description);
    ...
    sprintf(temp_outdir,"output/%s/Regsep16EwOutHistory.eps",description);
    ...
    sprintf(temp_outdir,"output/%s/Regsep32EwOutHistory.eps",description);
    ...
    sprintf(temp_outdir,"output/%s/Regsep48EwOutHistory.eps",description);
