...
FTKClusteringEngine::FTKClusteringEngine(bool saveClusterContent, bool diagClustering,
    ...
    saveClusterContent(saveClusterContent),
    diagClustering(diagClustering),
    sctClustering(sctClustering),
    ibl3DRealistic(ibl3DRealistic),
    duplicateGanged(duplicateGanged),
    gangedPatternRecognition(gangedPatternRecognition),
    splitBlayerModules(splitBlayerModules),
    pixelClusteringMode(pixelClusteringMode)
{
...
bool FTKClusteringEngine::neighborhood(const FTKRawHit &hit1, const FTKRawHit &hit2)
{
    ...
    case ftk::PIXEL:
        ...
        if (diagClustering) { /* Accept contiguity along diagonal as well */
            if ( (hit1.getEtaStrip() == hit2.getEtaStrip()+1 || hit1.getEtaStrip()+1 == hit2.getEtaStrip())
...
void FTKClusteringEngine::averageCluster(std::unique_ptr<cluster>& clu)
{
    ...
    /// precalculate values for case pixelClusteringMode>0
    ...
    if (isIBLmodule) {
        ...
        if( first.getModuleType() == ftk::MODULETYPE_IBL3D && ibl3DRealistic ) {
    ...
    case ftk::PIXEL: {
        ...
        if (pixelClusteringMode == 0) {
            for (const auto& hit: clu->hitlist) { //loop over hits in cluster
                if (saveClusterContent) { // if enabled the cluster also stores also the single channels
                    FTKRawHit tmpch = *hit;
        ...
        for (auto& hit: clu->hitlist) { //loop over hits in cluster
            ...
            if (isPixelmodule) {
                ...
                if((hit->getModuleType() == ftk::MODULETYPE_IBL3D && ibl3DRealistic) || !ibl3DRealistic){
        ...
        if (pixelClusteringMode == ftk::pcm::PIXEL_CLUSTERING_HARDWARE) {
            float etaRow = -1;
            float etaCol = -1;
        ...
        //pixelClusteringMode 1 and 100
        if (pixelClusteringMode <= ftk::pcm::PIXEL_CLUSTERING_MIXED) {
            ...
            if (pixelClusteringMode>=ftk::pcm::PIXEL_CLUSTERING_IDEAL_APRIL_2014_FIX) /* Fixing an error in the formula */
        ...
        av.setSplit(false);
        if (pixelClusteringMode >= ftk::pcm::PIXEL_CLUSTERING_MIXED && isSplitCluster(clu.get()))
            av.setSplit(true);
...
void FTKClusteringEngine::atlClusteringLNF(std::vector<FTKRawHit> &hits)
{
    ...
    for(const auto& hit: hits) {
        ...
        if (duplicateGanged && hitIsGanged(hit)) {
     ...
     * 1. pixelClusteringMode is less than 100 (PIXEL_CLUSTERING_MIXED)
    ...
    for (auto& module: hitsByModule) { // loop over modules
	  ...
	  else if (pixelClusteringMode >= ftk::pcm::PIXEL_CLUSTERING_MIXED) {
    ...
    for (auto& module: clustersByModule) {
        for (const auto& clu: module.second) {
            ...
            if ( gangedPatternRecognition && clu->isKilled && !clu->isSafe ) {
