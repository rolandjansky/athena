...
void FTKDataInput::initClustering()
{
    //  new clustering class and pass options
     clusteringEngine = new FTKClusteringEngine(m_SaveClusterContent,
                                         m_DiagClustering,
                                         m_SctClustering,
                                         m_Ibl3DRealistic,
                                         m_DuplicateGanged,
...
void FTKDataInput::processEvent(bool clearOrig)
{
    ...
    if (!m_read_clusters) {
        ...
        if (m_Clustering ) {
            clusteringEngine->atlClusteringLNF(m_original_hits);
            clusteringEngine->atlClusteringBlayer(m_original_hits);  // split blayer modules in half, if requested
        }
        else {
...
void FTKDataInput::processRegion(int curreg, bool clearOrig)
{
    ...
    if (m_read_FTKhits_directly) {
        ...
        if (!m_read_clusters) {
            // if raw channels are read the clustering algorithm can be applyied (default)
            if (m_Clustering ) {
                clusteringEngine->atlClusteringLNF(currawhits);
                clusteringEngine->atlClusteringBlayer(currawhits);  // split blayer modules in half, if requested
            }
            else {
                // If the clustering it is not executed the FTKRawHit.m_truth field has to be fixed
