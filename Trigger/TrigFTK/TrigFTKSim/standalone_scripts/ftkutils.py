...
class FTKOfflineCluster :
    def hitToModuleId(self) :
        # returns moduleId of given hit
        MOD_ID_LAYER_VAL  = 10000
        MOD_ID_LAYER_MASK = 100
        MOD_ID_ETA_VAL  = 100
        MOD_ID_ETA_MASK = 100
        MOD_ID_PHI_VAL  = 1
        MOD_ID_PHI_MASK = 100
        MAX_BARREL_ETA=20 
        MAX_ETA=MAX_BARREL_ETA+6
        MAX_PHI=56
        MAX_LAYER_PIXEL=3
        MAX_LAYER=56
        
        ieta = 0
        if self.IBLMode and self.layer == 0:
          # eta index should range -8 to 8 for ibl
            ieta = 8 + self.eta_module
        else:
            if self.BarEcp==0:
                # eta index should range -6 to 6 for barrel
                ieta = 6 + self.eta_module
            else:
                # eta index should range 0 to 2 for endcaps (just 0 for pixels)
                #if (FTKSetup::getFTKSetup().getIBLMode()==1){
                #  if (hit.getBarrelEC() == POSEC)
                #    ieta = MAX_BARREL_ETA+hit.getEtaModule()+3;
                #  if (hit.getBarrelEC() == NEGEC)
                #    ieta = MAX_BARREL_ETA+3+hit.getEtaModule()+3;
                #} else {
                if self.BarEcp>0:
                    ieta = MAX_BARREL_ETA+self.eta_module
                if self.BarEcp<0:
                    ieta = MAX_BARREL_ETA+3+self.eta_module

        iphi = self.phi_module
        ilayer = 0
        #switch (self.getHitType()) {
        #case PIXEL:
        ilayer = self.layer
        #case SCT:
        # for endcaps self.layer_disk goes from 0 to 17
        #ilayer = 10 + hit.getLayer();

        
        ModuleId = ilayer*MOD_ID_LAYER_VAL + ieta*MOD_ID_ETA_VAL + iphi*MOD_ID_PHI_VAL
        #print ieta,ModuleId
        return ModuleId

    def setByTuple(self,tuple,inputIBLMode) :
        self.IBLMode = (inputIBLMode!=0)
        self.x = 0
        self.y = 0
        self.z = 0
        # type of the detector: 1 pxl, 0 SCT
        self.type = 1 #int(raw_elements[3])
        self.BarEcp = tuple[0]
        self.layer = tuple[1]
        self.phi_module = tuple[2]
        self.eta_module = tuple[3]
        # for SCT side, PXL phi position of the channel
        self.side = -999
        # SCT number of strip, PXL eta position
        self.strip = -999
        self.offline_phi_center = tuple[5]
        self.offline_eta_center = tuple[6]
        self.barcode = tuple[7]
        #cluster size numbers
        self.phiSize = tuple[8]
        self.etaSize = tuple[9]
        self.NChannels = tuple[10]
        self.trackEta = tuple[11]
        self.trackCotTheta = tuple[12]
        self.trackZ0Raw = tuple[13]
        self.pixelEstimateCotTheta = tuple[14]
        self.includesGanged = tuple[15]

        self.moduleId = self.hitToModuleId()
        return None
        

    def __init__(self,raw_elements,inputIBLMode) :
        """FTKOfflineCluster stores the values from the wrapper lines. For the moment only pixel letter 'P' is implemented. See FastTrackSimWrap/src/DumpSp.cxx for further information."""
#Exmaple:I see the same fields in user.JordanWebster.training.20110803_fulldetector_16.0.3_nominal_100k_indep_muons/
# code is https://svnweb.cern.ch/trac/atlasoff/browser/Trigger/TrigFTK/FastTrackSimWrap/trunk/src/DumpSp.cxx#L1264
#  (*oflraw) << "P\t" ....
#P         -48.52855261    -22.38959999     269.9783514  0       0       12      4       327     126     327     126     1       1       1       0       10001   771     0
#P          -43.9769278     -20.2012541     252.4718691  0       0       13      4       19      84      19      85      1       3       3       0       10001   771     0
#P         -78.20749591    -37.32910748     387.5868276  0       1       22      6       68      111     68.25   112.2368421     2       3       4       0       10001   771     0
        ...
        self.type = 1 #int(raw_elements[3])
        # detector position: 0 bar
        self.BarEcp = int(raw_elements[3])
        # modul layer number, phi and eta position
        self.layer = int(raw_elements[4])
        if self.BarEcp!=0: # MANUAL PATCH THAT REFERS TO raw_11L.pmap
            if self.layer==0:
                self.layer=2
            elif self.layer==1:
                self.layer=2
        ...
        self.phi_module = int(raw_elements[5])
        self.eta_module = int(raw_elements[6])
        # for SCT side, PXL phi position of the channel
        self.side = int(raw_elements[7])
        # SCT number of strip, PXL eta position
        self.strip = int(raw_elements[8])
        # PXL ToT, SCT number of strips
#        self.num_strips = int(raw_elements[10])

        # PXL offline cluster phi position in channel #
        self.offline_phi_center = float(raw_elements[9])
        # PXL offline cluster eta position in channel #
        self.offline_eta_center = float(raw_elements[10])

        #cluster size numbers
        self.phiSize = float(raw_elements[11])
        self.etaSize = float(raw_elements[12])
        self.NChannels = float(raw_elements[13])
        self.trackEta = -999 
        self.trackCotTheta = -999
        self.trackZ0Raw = -999
        self.pixelEstimateCotTheta = -999
        self.includesGanged = -999
        
        # event number of the associated track
        self.event = int(raw_elements[14])
        # barcode of the associated track
        self.barcode = int(raw_elements[15])
        # pt of the associated track
        self.barcode_pt = float(raw_elements[16])
        # fraction of the association
#        self.barcode_frac = float(raw_elements[14])
        ...
        ##below is pixelcode from FTKHit to be VERIFIED!!!!
        layer_offset = 0 # 3 if you focus on the barrel and after don't use the plane map
        # here elaborate SCT ID for barrel
        if self.BarEcp == 0 :
            if self.layer==0 and self.side == 1 :
    ...
    def Print(self) :
        print "(x,y,z)=(%f,%f,%f)" % (self.x, self.y, self.z), "(pix?,bar?,layer,phiModule,etaModule,moduleId)=(%d,%d,%d,%d,%d,%d)" %(self.type,self.BarEcp,self.layer,self.phi_module,self.eta_module,self.moduleId), "(side,strip,includesGanged)=(%d,%d,%d)" %(self.side, self.strip, self.includesGanged), "(phiCenter,etaCenter) = (%f,%f)" %(self.offline_phi_center, self.offline_eta_center), "(phiSize,etaSize,NChannels)=(%d,%d,%d)" %(self.phiSize, self.etaSize, self.NChannels), "(evtID,barcode,barcodePt)=(%d,%d,%f)" % (self.event,self.barcode,self.barcode_pt), "(trackEta, trackCotTheta, trackZ0Raw, pixelEstimateCotTheta)=(%f,%f,%f,%f)" %(self.trackEta, self.trackCotTheta, self.trackZ0Raw, self.pixelEstimateCotTheta)
#        self.barcode_frac = float(raw_elements[14])
        return None
    def Distance(self, hit, useOfflineLocalPosition) :
        if (self.moduleId != hit.moduleId):
            return None
        else: #use same weight for eta and phi values
            if useOfflineLocalPosition:
                # when useOfflineLocalPosition is used for offline_phi_center
                # use side and strip (i.e. integer pixel coordinates) for distance
                return sqrt((self.offline_phi_center-hit.side)**2 + (self.offline_eta_center-hit.strip)**2)
            else:
                return sqrt((self.offline_phi_center-hit.offline_phi_center)**2 + (self.offline_eta_center-hit.offline_eta_center)**2)
...
class FTKEvent :
    def __init__(self, outFile=None, inputIBLMode=1) :
    ...
    def ReadFromFile(self,stream) :
        ...
        for line in stream :
            ...
            if line[0] == "R" :
                ...
                if not self.ignoreClusters :
                    self.offline_clusters.append(FTKOfflineCluster(line.split()[1:], self.IBLMode))
            elif line[0] == "L" :
                break
        return inEvent
    ...
    def PrintTruthTracks(self) :
        ...
        for track in self.truth_tracks :
            track.Print()
    def SetIBLMode(self, inputIBLMode) :
        self.IBLMode = (inputIBLMode!=0)
