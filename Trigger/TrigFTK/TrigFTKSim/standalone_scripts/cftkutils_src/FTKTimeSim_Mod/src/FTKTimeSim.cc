...
FTKTimeSim::FTKTimeSim()   //Instance set initialize
{
  ...
  for(int idx=0;idx<NREGION;idx++) {
   ...
   // * hit distribution * //
   int n_hitmax=80000;
   h_ftkhit = new TH1D("h_ftkhit", "# of ftkhit", n_hitmax/3, 0, n_hitmax);
   h_ftkss = new TH1D("h_ftkss", "# of ftkss", n_hitmax/3, 0, n_hitmax);
   h_maxftkhit = new TH1D("h_maxftkhit", "# of max ftkhit", n_hitmax/3, 0, n_hitmax);
   for(int i=0 ; i<NLAYER ; i++){
     sprintf(temp_name, "# of ftkhit on layer %d", i);
     h_ftkhit_layer[i] = new TH1D(temp_name, temp_name, n_hitmax/3, 0, n_hitmax);
   }
   // * road distribution * //
   int n_roadmax=300000;
   h_road = new TH1D("h_road", "# of road", n_roadmax/60, 0, n_roadmax);
   h_road_rw = new TH1D("h_road_rw", "# of road_rw", n_roadmax/60, 0, n_roadmax);
   h_maxroad = new TH1D("h_maxroad", "# of max road", n_roadmax/60, 0, n_roadmax);
   h_SecStage_road = new TH1D("h_SecStage_road", "# of road", n_roadmax/60, 0, n_roadmax);
   h_SecStage_maxroad = new TH1D("h_SecStage_maxroad", "# of max road", n_roadmax/60, 0, n_roadmax);
   for(int i=0 ; i<NASSOCIATIVEMEMORY ; i++){
     sprintf(temp_name, "# of road on AM %d", i);
     h_road_AM[i] = new TH1D(temp_name, temp_name, n_roadmax/60, 0, n_roadmax);
     sprintf(temp_name, "# of road_rw on AM %d", i);
     h_road_rw_AM[i] = new TH1D(temp_name, temp_name, n_roadmax/60, 0, n_roadmax);
     sprintf(temp_name, "# of SecStage road on AM %d", i);
     h_SecStage_road_AM[i] = new TH1D(temp_name, temp_name, n_roadmax/60, 0, n_roadmax);
   }
   // * fit distribution * //
   //  int n_fitmax=20000000;
   int n_fitmax=20000000;
   h_fit = new TH1D("h_fit", "# of fit", n_fitmax/20, 0, n_fitmax);
   h_maxfit = new TH1D("h_maxfit", "# of max fit", n_fitmax/20, 0, n_fitmax);
   h_SecStage_fit = new TH1D("h_SecStage_fit", "# of fit", n_fitmax/20, 0, n_fitmax);
   h_SecStage_maxfit = new TH1D("h_SecStage_maxfit", "# of max fit", n_fitmax/200, 0, n_fitmax);
   // * vs plot * //
...
FTKTimeSim::FillResults(int dump, int jentry, char *description, int temp_denom, int s_split)  //Fill Section
{
  ...
  if(FTKTimeSim_Debug>2)cout<<"Event Summary  : This is Fill results () "<<endl;
  if(FTKTimeSim_Debug>2)cout<<"XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"<<endl;
  //  int temp_denom=1;
  Region regtemp(-99);
  vector<double> temp;
...
//  if(FTKTimeSim_Debug>2) cout<<"DF_EwOut = "<<DF_EwOut<<endl;
  ...
  //  if(dump){
    ...
    h_t->GetYaxis()->SetBinLabel(3,"8 Regions");
    h_t->GetYaxis()->SetBinLabel(2,"total time");
    ...
    if(pre_DF_EwOut-l1gap>0) l_pre.DrawLine(0,9,pre_DF_EwOut-l1gap,9);
    ...
    char par2[256];
    sprintf(par2,"hits = %d,  roads = %d, fits = %d,  region = %d",
    ...
    //    char temp_outdir[256];
    sprintf(temp_outdir,"output/%s/c_time_detail_%d.eps",description,jentry);
    c_t->SaveAs(temp_outdir);
    sprintf(temp_outdir,"output/%s/gif/c_time_detail_%d.gif",description,jentry);
    //c_t->SaveAs(temp_outdir);
...
FTKTimeSim::PlotResults(char *description, int temp_denom, int s_split, int nrbin)  // For Results
{
  ...
  if(plot_maxregion){ // plot maxregion
    ...
    // fit //
    c_maxregion->cd(3);
    h_maxroad->SetXTitle("number of fit in max region");
    h_maxroad->SetYTitle("events");
...
//    leg_reg1->SetFillColor(0);
    ...
    TCanvas *c_historyplot_regsep = new TCanvas("c_historyplot_regsep","c_historyplot_regsep",800,800);
    c_historyplot_regsep->Divide(3,3);
    gStyle->SetOptStat(0);

    //    for(int i=0 ; i<NREGION ; i++){
    for(int i=0 ; i<NREGION ; i++){
      c_historyplot_regsep->cd(i+1);
      g_EwOut->Draw("AL");
      g_DO_EwOut[i]->Draw("L same");
      g_AM_EwOut[i]->Draw("L same");
      g_SecDO_EwOut[i]->Draw("L same");
...
//    leg_regsep->SetFillColor(0);
    ...
    sprintf(temp_outdir,"output/%s/RegsepEwOutHistory.eps",description);
    c_historyplot_regsep->SaveAs(temp_outdir);
