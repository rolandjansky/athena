...
TrackFitter::Calc()
{
  ...
  double InputDelay = (ave_hit_ss*this->iv.nroad_norw_AM[this->bd_index]*this->bd_t_in); // more reallistic  
  //  double InputDelay = 0;
  ...
  temp_io.push_back(fw_out + (this->iv.nfit/NTRACKFITTER) * this->bd_t_proc);
  temp_priio.push_back(primary_fw_out + (this->iv.nfit/NTRACKFITTER) * this->bd_t_proc);
  ...
  if(FTKTimeSim_TF_Debug){
    ...
    cout<<"#of fit = "<<this->iv.nfit<<"   ("<<(this->iv.nfit/NTRACKFITTER)*this->bd_t_proc<<" nsec)"<<endl;
