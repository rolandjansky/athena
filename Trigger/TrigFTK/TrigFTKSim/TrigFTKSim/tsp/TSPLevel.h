...
public:
  ...
  TSPLevel(TFile* file, int bankID, FTKTSPBank *AMbank,const std::list<AMSelection>  &amlist, unsigned threshold, FTKSSMap* ssTopMap, FTKSSMap* ssBottomMap, std::map<int,FTKSS> *ssmaps, bool onlydc=false);
  TSPLevel(int bankID, int, unsigned , FTKSSMap* ssTopMap, FTKSSMap* ssBottomMap, std::map<int,FTKSS> *ssmaps, bool onlydc=false);
