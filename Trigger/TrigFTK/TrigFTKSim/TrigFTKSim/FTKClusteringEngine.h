...
private:
    ...
    bool saveClusterContent;
    bool diagClustering; //!< If true. clusters are allowed to be connected only by diagonal pixels
    bool sctClustering; //!< If true, sct hits are also clustered
    bool ibl3DRealistic; //!<If true, you can get the same IBL 3D centorid as the FTK_IM FW.
    bool duplicateGanged; //!< If true, if a ganged pixel is hit, its partner is selected as an active hit
    bool gangedPatternRecognition; //!< The code contains functions that allow for some "smart"
				   //!< recognition on which hits are the "real" ones and which are
                                   //!< and which are fake. If true, this test is enabled.
    bool splitBlayerModules;
    int pixelClusteringMode; //!< Which clustering mode to use. 
