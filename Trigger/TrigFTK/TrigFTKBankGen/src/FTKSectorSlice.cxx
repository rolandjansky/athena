...
void FTKSectorSlice::loadSlices(std::string slice_file_path) {
  ...
  //TClonesArray *m_bits_phi = (TClonesArray*)slice_file.Get("c_bits_phi");
  m_bits_phi = (TClonesArray*) (slice_file.Get("c_bits_phi")->Clone("bitsphi"));
  m_phi_nbins = m_bits_phi->GetEntries();
  m_step_phi = (m_phi_max - m_phi_min)/m_phi_nbins;

  autotune(m_bits_phi,m_phi_min,m_phi_max,m_autotune_phi_min,m_autotune_phi_max,
           true,"phi");
  //TClonesArray *m_bits_c = (TClonesArray*) slice_file.Get("c_bits_c");
  m_bits_c = (TClonesArray*) slice_file.Get("c_bits_c");
  m_c_nbins = m_bits_c->GetEntries();
  m_step_c = (m_c_max - m_c_min)/m_c_nbins;

  autotune(m_bits_c,m_c_min,m_c_max,m_autotune_c_min,m_autotune_c_max,false,"c");

  //TClonesArray *m_bits_d = (TClonesArray*)slice_file.Get("c_bits_d");
  m_bits_d = (TClonesArray*)slice_file.Get("c_bits_d");
  m_d0_nbins = m_bits_d->GetEntries();
  m_step_d0 = (m_d0_max - m_d0_min)/m_d0_nbins;

  autotune(m_bits_d,m_d0_min,m_d0_max,m_autotune_d0_min,m_autotune_d0_max,false,
           "d0");

  //TClonesArray *m_bits_z0 = (TClonesArray*)slice_file.Get("c_bits_z0");
  m_bits_z0 = (TClonesArray*)slice_file.Get("c_bits_z0");
  m_z0_nbins = m_bits_z0->GetEntries();
  m_step_z0 = (m_z0_max - m_z0_min)/m_z0_nbins;

  autotune(m_bits_z0,m_z0_min,m_z0_max,m_autotune_z0_min,m_autotune_z0_max,false,
           "z0");

  //TClonesArray *m_bits_ctheta = (TClonesArray*)slice_file.Get("c_bits_ctheta");
  m_bits_ctheta = (TClonesArray*)slice_file.Get("c_bits_ctheta");
  m_ctheta_nbins = m_bits_ctheta->GetEntries();
  m_step_ctheta = (m_ctheta_max - m_ctheta_min)/m_ctheta_nbins;

  autotune(m_bits_ctheta,m_ctheta_min,m_ctheta_max,
  ...
  for (int ibin=0;ibin!=m_phi_nbins;++ibin) {
    TBits *curtbit = (TBits*) m_bits_phi->At(ibin);
    
  }
#endif
  cout<<"number of sectors in the slide file \""<<slice_file_path
      <<"\" : "<<((TBits const *)m_bits_ctheta->UncheckedAt(0))->GetNbits()
      <<"\n";
}
...
vector<int>* FTKSectorSlice::searchSectors(FTKTrack& track) {
  ...
  //bin_phi = bin;
  phi_bits = (TBits*)(m_bits_phi->UncheckedAt(bin));
  //phi_bits = (TBits*)(m_bits_phi->At(bin));
  ...
  //bin_c = bin;
  c_bits = (TBits*)(m_bits_c->UncheckedAt(bin));
  //c_bits = (TBits*)(m_bits_c->At(bin));

  bin = (Int_t)((track.getIP()-m_d0_min)/m_step_d0);
  bin = (bin < 0) ? 0 : bin;
  bin = (bin > m_d0_nbins-1) ? m_d0_nbins-1 : bin;
  //bin_d0 = bin;
  d0_bits = (TBits*)(m_bits_d->UncheckedAt(bin));
  //d0_bits = (TBits*)(m_bits_d->At(bin));
  ...
  //bin_z0 = bin;
  z0_bits = (TBits*)(m_bits_z0->UncheckedAt(bin));
  //z0_bits = (TBits*)(m_bits_z0->At(bin));

  bin = (Int_t)((track.getCotTheta()-m_ctheta_min)/m_step_ctheta);
  bin = (bin < 0) ? 0 : bin;
  bin = (bin > m_ctheta_nbins-1) ? m_ctheta_nbins-1 : bin;
  //bin_ctheta = bin;
  ctheta_bits = (TBits*)(m_bits_ctheta->UncheckedAt(bin));
  //ctheta_bits = (TBits*)(m_bits_ctheta->At(bin));
