...
#define TRG_MSG_LVL_NOCHK(lvl, x)               \
  this->msg(lvl) << x << endmsgtc

#define TRG_MSG_LVL(lvl, x)                     \
  if (this->msgLvl (lvl)) {TRG_MSG_LVL_NOCHK(lvl, x);}

#define TRG_MSG_VERBOSE(x) TRG_MSG_LVL(MSGTC::VERBOSE, x)
#define TRG_MSG_DEBUG(x)   TRG_MSG_LVL(MSGTC::DEBUG, x)
// note that we are using the _NOCHK variant here
#define TRG_MSG_INFO(x)    TRG_MSG_LVL_NOCHK(MSGTC::INFO, x)
#define TRG_MSG_WARNING(x) TRG_MSG_LVL_NOCHK(MSGTC::WARNING, x)
#define TRG_MSG_ERROR(x)   TRG_MSG_LVL_NOCHK(MSGTC::ERROR,  x)
#define TRG_MSG_FATAL(x)   TRG_MSG_LVL_NOCHK(MSGTC::FATAL,  x)
#define TRG_MSG_ALWAYS(x)  TRG_MSG_LVL_NOCHK(MSGTC::ALWAYS, x)

// can be used like so: TRG_MSG(INFO) << "hello" << endmsg;
#define TRG_MSG(lvl) \
  if (this->msgLvl(MSGTC::lvl)) this->msg(MSGTC::lvl)
