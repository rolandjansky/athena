...
enum class HitType { unmapped, mapped, clustered, wildcard, guessed, undefined };
...
class HTTHit
{
    public:
        ...
        void setEtaModule(unsigned v) { m_etaModule = v; }
    ...
    protected:
        ...
        DetectorZone m_detectorZone = DetectorZone::undefined; // barrel / EC
        ...
        unsigned m_etaModule = 0; // eta index of the module that the hit is located on
        ...
        unsigned m_layer = -1;   // Logical layer this hit is on
        unsigned m_section = -1; // Index of detector element in the logical layer (see HTTPlaneMap.h)

        // --- Local Coordinates ---
        unsigned m_phiIndex = -1; // phi index for pixel, strip for strip
        unsigned m_etaIndex = -1; // eta index for pixel, row for strip
