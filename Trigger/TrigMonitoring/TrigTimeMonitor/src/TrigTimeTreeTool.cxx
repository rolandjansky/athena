...
StatusCode TrigTimeTreeTool::bookHists() {
	...
	if ( rootHistSvc->regTree(treeName,
				  m_tree).isFailure() ) {
	    log << MSG::WARNING << "Unable to register TTree" << endreq;
	}

    } else {
	log << MSG::DEBUG << "Attached info to global tree" << endreq;	
	m_localTree = false;
    }
	...
	if ( selected((*tIter)->name()) ) {
		...
		log << MSG::WARNING << "Unable to add the timomg info branch " 
		    << (*tIter)->name()
		    << "to the tree" << endreq;
	    }
	}
...
StatusCode TrigTimeTreeTool::fillHists() {
    MsgStream log(msgSvc(), name());
    log << MSG::DEBUG << " execHLTAlgorithm" << endreq;
