...
def configureMonitoring(MCK_number):
  ...
  if HLTMonFlags.doCalo and DQMonFlags.monManEnvironment == 'tier0Raw':
    ...
    config_caloraw = readSliceConfigFromDB('oracle',MCK_number,'Physics', 'CALORAW' , 'RAW') 
  ...
  if HLTMonFlags.doCalo and DQMonFlags.monManEnvironment == 'tier0ESD':
    # here we check which entry there is in the DB for this slice
    config_caloesd = readSliceConfigFromDB('oracle',MCK_number,'Physics', 'CALOESD' , 'ESD') 
    # if entry is not None then we take monitoring config from DB, else it is taken from the release in a standard way 
  ...
  if HLTMonFlags.doTau:
    # here we check which entry there is in the DB for this slice
    config_tau = readSliceConfigFromDB('oracle',MCK_number,'Physics', 'TAU' , 'ESD') 
    # if entry is not None then we take monitoring config from DB, else it is taken from the release in a standard way 
    if config_tau != None:
  ...
  if HLTMonFlags.doBjet:
    # here we check which entry there is in the DB for this slice
    config_bjet = readSliceConfigFromDB('oracle',MCK_number,'Physics', 'BJET' , 'ESD') 
    # if entry is not None then we take monitoring config from DB, else it is taken from the release in a standard way 
    if config_bjet != None:
  ...
  if HLTMonFlags.doBphys:
    # here we check which entry there is in the DB for this slice
    config_bphys = readSliceConfigFromDB('oracle',MCK_number,'Physics', 'BPHYS' , 'ESD') 
    # if entry is not None then we take monitoring config from DB, else it is taken from the release in a standard way 
  ...
  if HLTMonFlags.doMET:
    # here we check which entry there is in the DB for this slice
    config_met = readSliceConfigFromDB('oracle',MCK_number,'Physics', 'MET' , 'ESD') 
    # if entry is not None then we take monitoring config from DB, else it is taken from the release in a standard way 
    if config_met != None:
  ...
  if HLTMonFlags.doJet:
    # here we check which entry there is in the DB for this slice
    config_jet = readSliceConfigFromDB('oracle',MCK_number,'Physics', 'JET' , 'ESD') 
    # if entry is not None then we take monitoring config from DB, else it is taken from the release in a standard way 
    if config_jet != None:
  ...
  if HLTMonFlags.doEgamma:
    # here we check which entry there is in the DB for this slice
    config_egamma = readSliceConfigFromDB('oracle',MCK_number,'Physics', 'EGAMMA' , 'ESD') 
    # if entry is not None then we take monitoring config from DB, else it is taken from the release in a standard way 
  ...
  if HLTMonFlags.doMuon:
    # here we check which entry there is in the DB for this slice
    config_muon = readSliceConfigFromDB('oracle',MCK_number,'Physics', 'MUON' , 'ESD') 
    # if entry is not None then we take monitoring config from DB, else it is taken from the release in a standard way 
    if config_muon != None:
  ...
  if HLTMonFlags.doIDtrk:
    # here we check which entry there is in the DB for this slice
    config_idtrk = readSliceConfigFromDB('oracle',MCK_number,'Physics', 'IDTRK' , 'ESD') 
    # if entry is not None then we take monitoring config from DB, else it is taken from the release in a standard way 
  ...
  if HLTMonFlags.doMinBias:
    # here we check which entry there is in the DB for this slice
    config_minbias = readSliceConfigFromDB('oracle',MCK_number,'Physics', 'MINBIAS' , 'ESD') 
    # if entry is not None then we take monitoring config from DB, else it is taken from the release in a standard way 
  ...
  if HLTMonFlags.doIDJpsiMon:
    # here we check which entry there is in the DB for this slice
    config_idjpsi = readSliceConfigFromDB('oracle',MCK_number,'Physics', 'IDJPSI' , 'ESD') 
    # if entry is not None then we take monitoring config from DB, else it is taken from the release in a standard way 
