...
#def upload(active_flag):
#def upload():
def upload(MakeActive='False'):
      ...
      if ConfigRunTypeDict[i]=='Physics':
        if ((flag==0 and similar_config[-1]!=0) or flag==1) and physics_leave_active_type==0:
          ...
          if flag==0 and similar_config[-1] != 0:
            ...
            physicsConfigID.append(getConfigIdForMCK('oracle', activeKeyMCK, 'Physics', ConfigNameDict[i], ConfigRecostepDict[i]))
            CHECK_PHYSICS_LIST.append(1)
      ...
      if ConfigRunTypeDict[i]=='Standby':
        if ((flag==0 and similar_config[-1]!=0) or flag==1) and standby_leave_active_type==0:
          ...
          if flag==0 and similar_config[-1] != 0:
            ...
            # we take number from active MCK
            # put config ID from active config 
            standbyConfigID.append(getConfigIdForMCK('oracle', activeKeyMCK, 'Standby', ConfigNameDict[i], ConfigRecostepDict[i]))
            CHECK_STANDBY_LIST.append(1)
        elif similar_config[-1]==0:
            CHECK_STANDBY_LIST.append(0)
      ...
      if ConfigRunTypeDict[i]=='Cosmics':
        if ((flag==0 and similar_config[-1]!=0) or flag==1) and cosmics_leave_active_type==0:
          ...
          if flag==0 and similar_config[-1] != 0:
            ...
            # we take number from active MCK
            # put config ID from active config 
            cosmicsConfigID.append(getConfigIdForMCK('oracle', activeKeyMCK, 'Cosmics', ConfigNameDict[i], ConfigRecostepDict[i]))
            CHECK_COSMICS_LIST.append(1)
        elif similar_config[-1]==0:
            CHECK_COSMICS_LIST.append(0)
...
###########################################################    
    ...
    if take_existing_MCK==1:
      #we make existing MCK as an active MCK. if there is such MCK available leave its number as an active combination (return MCK from upload)                
        uploaded_MCK=existing_MCK_ID
        print "we take existing MCK number ", uploaded_MCK
        # because this existing MCK was not active in the case if MakeActive=='True' we need to add a an entry to the table with active MCKs to indacate that there is a new active MCK:
        if MakeActive=='True':
           makeActive('oracle',uploaded_MCK)

        else:
          print "Uploaded configuration is one of the existing configurations in the database but it is different than the active configuration. Because you have chosen not to make an uploaded configuration active we leave active MCK number not changed"
          print "Your uploaded MCK: ", uploaded_MCK, " Current active MCK: ", activeKeyMCK
    else:
      # else create a new MCK

      new_MCK=getMaxMCK('oracle')+1
      uploaded_MCK=new_MCK
      c.execute("INSERT INTO master_table VALUES ("+str(new_MCK)+", 'MCK', 'Monitoring Configuration Key')")
      # here we fill table activeMCK_table if there is a new MCK and 'True' flag for MakeActive

      if MakeActive=='True':
         makeActive('oracle',uploaded_MCK)
      else:
         print "Uploaded configuration is new and it gets a new MCK number. But because you have chosen not to make it active we leave active MCK number not changed."
         print "Your uploaded MCK: ", uploaded_MCK, " Current active MCK: ", activeKeyMCK
      ...
      for i,flag in enumerate(EQUAL_CONFIG):
        ...
        if flag==0 and similar_config[i]==0:
           ...
           if changePhysics==1 and ConfigRunTypeDict[i]=='Physics':
              equal_config_id = getConfigIdForMCK('oracle', activeKeyMCK, 'Physics', ConfigNameDict[i], ConfigRecostepDict[i])
              ...
              equal_config_id = getConfigIdForMCK('oracle', activeKeyMCK, 'Standby', ConfigNameDict[i], ConfigRecostepDict[i])
              ...
              equal_config_id = getConfigIdForMCK('oracle', activeKeyMCK, 'Cosmics', ConfigNameDict[i], ConfigRecostepDict[i])
              c.execute("INSERT INTO type2sig VALUES ("+str(newCosmicsId)+","+str(equal_config_id)+")")

  else:
     uploaded_MCK=activeKeyMCK
     print "Configuration hasn't changed, uploaded configuration is equal to the active configuration. We keep an old active MCK number"
     print "Uploaded MCK == Current active MCK: ", uploaded_MCK
