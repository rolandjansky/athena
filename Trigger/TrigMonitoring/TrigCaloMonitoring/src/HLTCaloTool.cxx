...
HLTCaloTool::HLTCaloTool(const std::string & type, const std::string & name, const IInterface* parent) 
...
StatusCode HLTCaloTool::fill() {
  SG::ReadHandle<CaloCellContainer> allCalo(m_CaloCellContainerName);
  if(!allCalo.isValid()) {
  ...
  for(; tbeg!=tend ; ++tbeg){
	...
	hist_PercentDiffTile->Fill(diffp);
      }
      else {
	hist_PercentDiffLAr->Fill(diffp);
      }
