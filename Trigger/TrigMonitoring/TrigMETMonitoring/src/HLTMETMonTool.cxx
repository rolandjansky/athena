...
StatusCode HLTMETMonTool::book() {
  ...
  addHistogram(new TH1F("HLT_MET_phi1",  "HLT MET #phi (|Missing E_{T}|);MET #phi (rad)", m_phi_bins, m_phi_min, m_phi_max));
  ...
  addHistogram(new TH2F("HLT_MET_etaphi1", "HLT MET #eta/#phi(|Missing E_{T}|);#phi (rad);#eta", m_phi_bins, m_phi_min, m_phi_max, 24, -4.8, 4.8));
  // to implement:
  // Efficiency_xe80
  // Efficiency_xe100
  // Mu_pt
...
StatusCode HLTMETMonTool::fillMETHist() {
  ...
  if (l1_met > epsilon_l1met) {
    ...
    if ((h = hist("L1_MET_phi1")))  h->Fill(l1_phi, l1_met);
  ...
  if ((h = hist("HLT_MET_phi1")))  h->Fill(hlt_phi, hlt_met);
  ...
  if ((h2 = hist2("HLT_MET_etaphi1"))) h2->Fill(hlt_phi, hlt_eta, hlt_met);
  ...
  for (it = m_hlt_met_signatures_tolook.begin(); it != m_hlt_met_signatures_tolook.end(); it++) {
    ...
    if (getTDT()->isPassed(name, TrigDefs::eventAccepted)) {
      ...
      if (m_hlt_met) {  
        ...
        if ((h = hist("HLT_MET_phi1")))  h->Fill(tmp_hlt_phi, hlt_met); 
