...
: IHLTMonTool(type, n, parent){
    ...
    declareProperty("TurnOnCurves", 		m_turnOnCurves);
    declareProperty("doTruth", 			m_truth);
    declareProperty("doEmulation", 		m_emulation);
    declareProperty("doTestTracking", 		m_doTestTracking);
    declareProperty("LowestSingleTau", 		m_lowest_singletau);
    declareProperty("LowestDiTau", 		m_lowest_ditau);
    declareProperty("LowestETau", 		m_lowest_etau);
    declareProperty("LowestMuTau", 		m_lowest_mutau);
    declareProperty("LowestMetTau", 		m_lowest_mettau);
    declareProperty("doIncludeL1deactivateTE", 	m_doIncludeL1deactivateTE);
    declareProperty("doIncludePreseldeactivateTE", m_doIncludePreseldeactivateTE);
    declareProperty("doIncludeHLTdeactivateTE", m_doIncludeHLTdeactivateTE);
    declareProperty("nTrkMax",			m_selection_nTrkMax);
    declareProperty("nTrkMin",			m_selection_nTrkMin);
    declareProperty("PtMax",			m_selection_ptMax);
    declareProperty("PtMin",			m_selection_ptMin);
    declareProperty("AbsEtaMax",		m_selection_absEtaMax);
    declareProperty("AbsEtaMin",		m_selection_absEtaMin);
    declareProperty("AbsPhiMax",		m_selection_absPhiMax);
    declareProperty("AbsPhiMin",		m_selection_absPhiMin);
...
#endif
{
  ...
  if(endOfRun)
	...
	{  
	  ...
	  plotUnderOverFlow(hist("hEFdRmax1PNCorr"));
	  ...
	  plotUnderOverFlow(hist("hEFdRmax1PCorr"));
  ...
  for(unsigned int i=0;i<lowest_names.size();++i)
      ...
      for(unsigned int j=0;j<m_trigItems.size();++j)
	{
      ...
      //      cloneHistogram("hEFdRmax1PNCorr","HLT/TauMon/Expert/"+lowest_trigger_names.at(i)+"/EFTau/BDT/1p_nonCorrected");
      ...
      //      cloneHistogram("hEFdRmax1PCorr","HLT/TauMon/Expert/"+lowest_trigger_names.at(i)+"/EFTau/BDT/1p_Corrected");
...
void HLTTauMonTool::bookHistogramsForItem(const std::string & trigItem){
    ...
    addHistogram(new TH1F("hEFdRmax1PNCorr", "Max dR of associated tracks at EF 1-prong non-corrected; dRmax at EF; Candidates",50,-0.1,0.3));
    ...
    addProfile(new TProfile("hEFdRmax1PNCmu", "Max dR of associated tracks at EF vs mu 1-prong non-corrected;Average interactions per bunch crossing;",nbin_mu-1,bins_mu));
    ...
    addHistogram(new TH1F("hEFdRmax1PCorr", "Max dR of associated tracks at EF 1-prong mu-corrected; dRmax at EF; Candidates",50,-0.1,0.3));
    ...
    addProfile(new TProfile("hEFdRmax1PCmu", "Max dR of associated tracks at EF vs mu 1-prong mu-corrected;Average interactions per bunch crossing;",nbin_mu-1,bins_mu));
...
//      addMonGroup(new MonGroup(this,"HLT/TauMon/Shifter/"+lowest_names.at(i)+"/EFVsOffline/BDT/mp_nonCorrected",run));
    ...
    if(m_emulation){
      ...
      addHistogram(new TH1F("hL1Emulation","; mismatched events",m_trigItems.size(),-0.5,m_trigItems.size()-0.5));
...
StatusCode HLTTauMonTool::fillHistogramsForItem(const std::string & trigItem){
    ...
    if(trig_item_L1=="") {ATH_MSG_WARNING("L1 chain for "<< trig_item_EF << " not found, skipping item!");}
	...
	const xAOD::EmTauRoIContainer* l1Tau_cont = 0;
      ...
      if (getTDT()->isPassed(trig_item_L1)) {
        ...
        if(comb==combEnd){
          ATH_MSG_WARNING("No features for chain " << trig_item_L1);
          return StatusCode::FAILURE;
        }
      ...
      if (getTDT()->isPassed(trig_item_EF)) {
        ...
        Trig::FeatureContainer::combination_const_iterator comb(f.getCombinations().begin()), combEnd(f.getCombinations().end());
        if(comb==combEnd){
          ATH_MSG_WARNING("No features for chain " << trig_item_EF);
          return StatusCode::FAILURE;
        }
        for(;comb!=combEnd;++comb){
  ...
  if(BDTinput_type == "basicVars")
  ...
  else if(BDTinput_type == "1p_NonCorr")
    {
      ...
      if(is1P)
	    ...
	    {
	      hist("hEFdRmax1PNCorr")->Fill(dRmax);      
	      profile("hEFdRmax1PNCmu")->Fill(mu, dRmax);      
	    }
        }
    }
  ...
  else if(BDTinput_type == "1p_muCorr")
    {
	  ...
	  if(aEFTau->detail(xAOD::TauJetParameters::ptRatioEflowApproxCorrected, ptRatioEflowApproxCorr))
	    ...
	    {
	      hist("hEFdRmax1PCorr")->Fill(dRmaxCorr);      
	      profile("hEFdRmax1PCmu")->Fill(mu, dRmaxCorr);      
	    }
        }
    }
...
void  HLTTauMonTool::testClusterNavigation(const xAOD::TauJet *aEFTau){
  ...
  ATH_MSG_WARNING("HLTTauMonTool::testClusterNavigation");
...
bool HLTTauMonTool::L1TauMatching(const std::string & l1_item, const TLorentzVector & TLV, double DR)
{
  if(l1_item == "Dump")
    {
      ATH_MSG_WARNING("No L1 matching for Dump Item return false");
      return false;
    }
  ...
  if(getTDT()->isPassed( l1_item ))
    {     
      ...
      if(comb==combEnd)
	{
	  ATH_MSG_WARNING("No features for chain " <<l1_item <<", L1Matching return false");
	  return false;
	}
      for(;comb!=combEnd;++comb)
	{
...
int HLTTauMonTool::PrimaryVertices(){
  ...
  if( evtStore()->retrieve(primary, "PrimaryVertices" ).isFailure() ){
    ATH_MSG_WARNING("Failed to retrieve PrimaryVertices container, returning -1!");
    return -1;
    }
...
float HLTTauMonTool::Pileup(){
  ...
  if( evtStore()->retrieve(evtInfo, "EventInfo" ).isFailure() ){
    ATH_MSG_WARNING("Failed to retrieve EventInfo container, returning -1!");
    return -1;
    }
