...
StatusCode HLTTauMonTool::fill() {
  ...
  m_taus_BDT.clear();
  m_taus_RNN.clear();
  const auto *const tauCont = SG::get(m_offlineTauJetKey);
  if( !tauCont ){
    ...
    for(offlinetau=tauCont->begin();offlinetau!=offlinetau_end; ++offlinetau){
      if((*offlinetau)){
        ...
        bool good_tau_BDT = false;  
        if( (*offlinetau)->isAvailable<float>("BDTJetScoreSigTrans") && (*offlinetau)->auxdata<float>("BDTJetScoreSigTrans") > 0){
           good_tau_BDT = (ntrack_TAU == 1 && ( (*offlinetau)->auxdata<float>("BDTJetScoreSigTrans") > 0.25)) || (ntrack_TAU == 3 && ( (*offlinetau)->auxdata<float>("BDTJetScoreSigTrans") > 0.4));   
        } else {
           good_tau_BDT = (*offlinetau)->isTau(xAOD::TauJetParameters::JetRNNSigMedium);
        }
        ...
        if (good_tau_BDT) {
           m_taus_BDT.push_back( *offlinetau );
        }
        if (good_tau_RNN) {
      ...
      if (getTDT()->getNavigationFormat() == "TriggerElement") {
        ...
        for(;comb!=combEnd;++comb){
          ...
          for(; topoTau != topoTau_e; ++topoTau){
            if(topoTau->cptr()){
              if(topoTau->cptr()->empty()) ATH_MSG_DEBUG("item "<< chain << ": TauJetContainer with " << topoTau->cptr()->size() << " TauJets");
...
StatusCode HLTTauMonTool::fillHistogramsForItem(const std::string & trigItem, const bool & monRNN, const bool & monBDT, const std::string & goodTauRefType){
  ...
  if(trigItem=="Dump"){
    ...
    const xAOD::TauJetContainer * tauJetCont = nullptr;
    ...
    if (getTDT()->isPassed(trig_item_EF,m_HLTTriggerCondition)) {
      ...
      Trig::FeatureContainer f; // Run 2

      if (getTDT()->getNavigationFormat() == "TriggerElement") {

        f = ( getTDT()->features(trig_item_EF,m_HLTTriggerCondition) );  
              ...
              for(itEMTau = l1Tau_cont->begin(); itEMTau!=itEMTau_e; ++itEMTau){
                ...
                if(roi->cptr()->roiWord()==(*itEMTau)->roiWord()){
                  ...
                  for(unsigned int l1tau=0;l1tau<tau_roIWord.size();l1tau++) {
                    if(roi->cptr()->roiWord()==tau_roIWord.at(l1tau)) newRoI=false;
                  }
                  if(!newRoI) break; 
        ...
        for(const auto& tauJetLinkInfo : features){
          ...
          for(itEMTau = l1Tau_cont->begin(); itEMTau!=itEMTau_e; ++itEMTau){
            if( (*itEMTau)->roiType()!=2 ) continue; // only Run2 TAU RoI!
            if((*initialRoIEL)->roiWord()==(*itEMTau)->roiWord()){
              if(!Selection(*itEMTau)) continue;
              bool newRoI=true;
              for(unsigned int l1tau=0; l1tau < tau_roIWord.size(); l1tau++) {
                if((*initialRoIEL)->roiWord() == tau_roIWord.at(l1tau)) {
                  newRoI=false;
                  break;
                }
              }
              if(!newRoI) break; 
              tau_roIWord.push_back((*initialRoIEL)->roiWord()); 
              tau_roi_eta.push_back((*itEMTau)->eta());
              tau_roi_phi.push_back((*itEMTau)->phi());
              ATH_MSG_DEBUG("Found RoI in (" << (*itEMTau)->eta() << "," << (*itEMTau)->phi() <<")");
              setCurrentMonGroup("HLT/TauMon/Expert/"+trigItemShort+"/L1RoI");
      ...
      if(trig_item_EF=="HLT_tau35_medium1_tracktwo_tau25_medium1_tracktwo_L1TAU20IM_2TAU12IM" && getTDT()->isPassed(trig_item_EF,m_HLTTriggerCondition)){
        ...
        if(!jet_cont) {
          ...
          for(jetItr=jet_cont->begin(); jetItr!=jet_cont_end; ++jetItr){
            if(fabs((*jetItr)->eta())>3.2) {
              continue;
            }
            bool tauRoi = false;
            for(unsigned int l1tau = 0; l1tau<tau_roIWord.size();l1tau++){
              if(deltaR((*jetItr)->eta(), tau_roi_eta.at(l1tau), (*jetItr)->phi(), tau_roi_phi.at(l1tau)) < 0.3) {
                tauRoi = true;
              }
            }
            if(tauRoi) {
          ...
          if(getTDT()->isPassed("HLT_tau35_medium1_tracktwo_tau25_medium1_tracktwo",m_HLTTriggerCondition)) {
            profile("TProfRecoL1_J25PtEfficiency")->Fill(maxPt/GeV,1);
          }else {
            profile("TProfRecoL1_J25PtEfficiency")->Fill(maxPt/GeV,0);
          }
        }
      } 
        ...
        if( !tf.getCombinations().empty()){
          ...
          for(;combo!=comboEnd;++combo){
            ...
            for(; ditauCI != ditauCI_e; ++ditauCI){
              if(ditauCI->cptr()){
                if(ditauCI->cptr()->empty()) ATH_MSG_DEBUG("item "<< trigItem << ": TauJetContainer with " << ditauCI->cptr()->size() << " TauJets");
...
//  float PSSFraction = 0;
  ...
  if(BDTinput_type == "basicVars")
  {
    ...
    hist("hEFEt")->Fill(aEFTau->pt()/GeV);
    hist("hEFEt2")->Fill(aEFTau->pt()/GeV);
    ...
    hist2("hEFEt2VsPhi")->Fill(aEFTau->phi(),aEFTau->pt()/GeV);
    hist2("hEFEtVsEta")->Fill(aEFTau->eta(),aEFTau->pt()/GeV);
    hist2("hEFEt2VsEta")->Fill(aEFTau->eta(),aEFTau->pt()/GeV);
...
StatusCode HLTTauMonTool::fillL1TauVsOffline(const xAOD::EmTauRoI *aL1Tau, const std::string & goodTauRefType){
  ...
  const xAOD::TauJet *aOfflineTau = nullptr;
  float tmpR = 0.3;
    ...
    if (getTDT()->getNavigationFormat() == "TriggerElement") {
      ...
      if ( !l1Tau_cont ){
        ...
        for(const auto& tauJetLinkInfo : features){
          ...
          ATH_MSG_WARNING("RoI Word " << (*roi)->roiWord() << ", eta " << (*roi)->eta() << ", phi " << (*roi)->phi() << ", roiId " << (*roi)->roiId() << ", l1Id " << (*roi)->l1Id() );
          for(itEMTau = l1Tau_cont->begin(); itEMTau!=itEMTau_e; ++itEMTau){
            if((*roi)->roiWord()!=(*itEMTau)->roiWord()) {
              continue;
            }
            ATH_MSG_WARNING("EMTau RoI Word " << (*itEMTau)->roiWord() << ", eta " << (*itEMTau)->eta() << ", phi " << (*itEMTau)->phi() );
            const std::vector<std::string>& names = (*itEMTau)->thrNames();
            for(unsigned int i=0;i<names.size(); i++) {
              ATH_MSG_WARNING("thrNames: " << names.at(i) );
            }
          }
        }
      }
...
bool HLTTauMonTool::Match_Offline_L1(const xAOD::TauJet *aOfflineTau, const std::string & trigItem){
    ...
    // const std::vector<Trig::Feature<TrigRoiDescriptor> > vec_L1roi_EFall = (getTDT()->features("EF_"+trigItem,TrigDefs::alsoDeactivateTEs)).get<TrigRoiDescriptor>("initialRoI",TrigDefs::alsoDeactivateTEs);
      ...
      const std::vector< TrigCompositeUtils::LinkInfo<xAOD::TauJetContainer> > features
         = getTDT()->features<xAOD::TauJetContainer>( "HLT_"+trigItem, TrigDefs::includeFailedDecisions);
      ...
      for(const auto& tauJetLinkInfo : features){
        ...
        const xAOD::EmTauRoI *aEmTau_ROI = findLVL1_ROI(*roi);
        if(!aEmTau_ROI) continue;
        if ( deltaR(aOfflineTau, aEmTau_ROI ) < tmpR) {
          tmpR = deltaR(aOfflineTau, aEmTau_ROI);
        }
...
bool HLTTauMonTool::Match_Offline_EF(const xAOD::TauJet *aOfflineTau, const std::string & trigItem){
    ...
    if (getTDT()->getNavigationFormat() == "TriggerElement") {
      ...
      const std::vector< TrigCompositeUtils::LinkInfo<xAOD::TauJetContainer> > features
         = getTDT()->features<xAOD::TauJetContainer>( "HLT_"+trigItem);
...
bool HLTTauMonTool::Selection(const xAOD::TauJet *aTau=nullptr) const{

   int nTrk(-1);
   float pt(0.),eta(-11.),phi(-11.);
   if(aTau==nullptr) return false;
...
bool HLTTauMonTool::Selection(const xAOD::EmTauRoI *aTau=nullptr) const{
...
StatusCode HLTTauMonTool::TauEfficiency(const std::string & trigItem, const std::string & TauDenom, const std::string & goodTauRefType){
  ...
  if(TauDenom.find("Presel")!=std::string::npos){
    ...
    const xAOD::TauJetContainer * hlt_cont = nullptr;
    ...
    if( sc.isFailure() || !hlt_cont ){
      ...
      sc = evtStore()->retrieve(hlt_cont, "HLT_TrigTauRecMerged_Precision");
      if( sc.isFailure() || !hlt_cont ){
          ATH_MSG_WARNING("Failed to retrieve HLT_TrigTauRecMerged_Precision container. Exiting.");
          return StatusCode::FAILURE;
      }
    }
  ...
  for(unsigned int t=0;t<taus_here.size();t++){
    ...
    if(trigItem.find("tau160_idperf_tracktwo_") != std::string::npos){
      setCurrentMonGroup("HLT/TauMon/Expert/HLTefficiency");
    ...
    if(trigItemShort == "tau25_idperf_tracktwo"){
      ...
      if (getTDT()->getNavigationFormat() == "TriggerElement") {
        ...
        if( HLTTauMatching("tau25_idperf_tracktwo_L1TAU12IM", TauTLV, 0.2)){
          hist2("hRecoTau25EtaVsPhiDenom")->Fill(eta,phi);
        }

        if( HLTTauMatching("tau25_perf_tracktwo_L1TAU12IM", TauTLV, 0.2)){
          hist2("hRecoHLT25EtaVsPhiNum")->Fill(eta,phi);
          profile("TProfRecoHLT25PtEfficiency")->Fill(pt/GeV,1);
          if(ntracks==1)   profile("TProfRecoHLT25Pt1PEfficiency")->Fill(pt/GeV,1);
          if(ntracks>1)   profile("TProfRecoHLT25Pt3PEfficiency")->Fill(pt/GeV,1);
        ...
        else if( HLTTauMatching("tau25_idperf_tracktwo_L1TAU12IM", TauTLV, 0.2)  ){
          profile("TProfRecoHLT25PtEfficiency")->Fill(pt/GeV,0);
          profile("TProfRecoHLT25EtaEfficiency")->Fill(eta,0);
          if(ntracks==1)   profile("TProfRecoHLT25Pt1PEfficiency")->Fill(pt/GeV,0);
          if(ntracks>1)   profile("TProfRecoHLT25Pt3PEfficiency")->Fill(pt/GeV,0);
    ...
    if(trigItem.find("tau160_perf_tracktwo_") != std::string::npos){
      setCurrentMonGroup("HLT/TauMon/Expert/HLTefficiency");
    ...
    if(trigItemShort=="tau25_perf_tracktwo"){
      ...
      if (getTDT()->getNavigationFormat() == "TriggerElement") {
        ...
        if( HLTTauMatching("tau25_medium1_tracktwo_L1TAU12IM", TauTLV, 0.2)  ){
          if(ntracks == 1) {
            profile("TProfRecoHLT25Pt1PEfficiency_2")->Fill(pt/GeV,1);
          }
          if(ntracks > 1) {
        ...
        else if( HLTTauMatching("tau25_perf_tracktwo_L1TAU12IM", TauTLV, 0.2)  ){
          if(ntracks==1)   profile("TProfRecoHLT25Pt1PEfficiency_2")->Fill(pt/GeV,0);
          if(ntracks>1)   profile("TProfRecoHLT25Pt3PEfficiency_2")->Fill(pt/GeV,0);
          profile("TProfRecoHLT25PtEfficiency_2")->Fill(pt/GeV,0);
          profile("TProfRecoHLT25EtaEfficiency_2")->Fill(eta,0);
  ...
  if(getTDT()->isPassed( HLT_item,(TrigDefs::Physics | TrigDefs::allowResurrectedDecision | TrigDefs::alsoDeactivateTEs)  )) {     
    ...
    if (getTDT()->getNavigationFormat() == "TriggerElement") {
      ...
      for(const auto& tauJetLinkInfo : features){
        ...
        float L1_eta = (*roi)->eta();
        float L1_phi = (*roi)->phi();
        float delR = deltaR(L1_eta,TLV.Eta(),L1_phi,TLV.Phi());
        if(delR <= DR) return true;
      }
    } // TriggerElement or TrigComposite
...
bool HLTTauMonTool::TrigBiasCheck(){
  ...
  const auto *chainGroup = getTDT()->getChainGroup("HLT_.*tau.*");
  for(auto &trig : chainGroup->getListOfTriggers()) {
    const auto *cg =  getTDT()->getChainGroup(trig);
    if(trig.find("RNN")!=string::npos && cg->isPassed()) fireRNN = true;
    if(trig.find("medium1")!=string::npos && cg->isPassed()) fireBDT = true;    
  } 

  noVeto = ( (fireRNN && fireBDT) ); // we want to veto events where the RNN trig is fired but not the BDT and vice versa.
...
StatusCode HLTTauMonTool::getTracks(const xAOD::TauJet *aEFTau, std::vector<const xAOD::TauTrack *> &out) {
  ...
  for (const ElementLink<xAOD::TauTrackContainer>& trackEL : tauTrackAcc(*aEFTau)) {
    if (!trackEL.isValid()) {
       linksValid = false;
    }
    break;
  ...
  if (!linksValid) {
      ATH_MSG_WARNING("Invalid track element links from HLT TauJet");
      return StatusCode::FAILURE;
  }
...
StatusCode HLTTauMonTool::getClusters(const xAOD::TauJet *aEFTau, std::vector<const xAOD::CaloCluster *> &out) {
  ...
  if(!aEFTau->jetLink().isValid()){
    return StatusCode::FAILURE;
  }

  const xAOD::Jet *jet_seed = aEFTau->jet();
  if (!jet_seed) {
    ATH_MSG_ERROR("Tau jet link is invalid.");
    return StatusCode::FAILURE;
  }

  if(!jet_seed->getConstituents().isValid()) {
    return StatusCode::FAILURE;
  }

  for (const auto *const jc : jet_seed->getConstituents()) {
