...
StatusCode HLTTauMonTool::Emulation(){
  ...
  const xAOD::EnergySumRoI *l1xe = nullptr;
  ...
  const xAOD::TauJetContainer * tauPreselCont = nullptr;
  ...
  // retrieve HLT containers                                                                                                                  
  const xAOD::TauJetContainer * tauHLTCont = nullptr;
...
//
 ...
 for (const auto& it: m_emulation_l1_tau) {  
  ...
  const auto *chain_group = getTDT()->getChainGroup(it);  
