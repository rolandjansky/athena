...
bool BjetLikelihoodMon::l2TrackSel(const TrigInDetTrack*& track, unsigned int i, float zv) {
//bool BjetLikelihoodMon::l2TrackSel(const TrigInDetTrackCollection*& track, unsigned int i, float zv) {
...
void BjetLikelihoodMon::getLikelihoodTag(const TrigInDetTrackCollection*& pointerToL2TrackCollections, 
					 float m_prmvtx,					
					 const ToolHandle<ITrigTrackJetFinderTool> m_trackJetFinderTool) {

  *m_log << MSG::DEBUG << "Executing BjetLikelihoodMon::getLikelihoodTag" << endreq;
  ...
  for (unsigned int j = 0; j < m_totTracks; j++) {
    ...
    if (l2TrackSel(track, j, m_zPrmVtx)) isSelected[j] = true;
  ...
  if (m_bjetMon->m_jetDirection) {
    ...
    *m_log << MSG::DEBUG << "Computing jet direction in BjetLikelihoodMon at LVL2." << endreq;
    ...
    if (etaTrackJet != -99 && phiTrackJet != -99) {
      ...
      *m_log << MSG::DEBUG << "eta RoI = " << m_etaRoI << " ; eta Jet at L2 = " << etaTrackJet << endreq;
      *m_log << MSG::DEBUG << "phi RoI = " << m_phiRoI << " ; phi Jet at L2 = " << phiTrackJet << endreq;

      m_etaRoI = etaTrackJet; m_phiRoI = phiTrackJet;

    } else
      *m_log << MSG::DEBUG << "eta Jet = eta RoI, phi Jet = phi RoI" << endreq;
  }
  ...
  for (unsigned int j = 0; j < m_totTracks; j++) {
    ...
    if (m_bjetMon->m_useErrIPParam) {
      errIP1D = getIP1DErr(track->param()->eta());
      errIP2D = getIP2DErr(track->param()->pT());
    } else {
      errIP1D = track->param()->ez0();
      errIP2D = track->param()->ea0();
    }
    
    d0 = track->param()->a0();
    z0 = track->param()->z0();
    phiTrack = track->param()->phi0();
    etaTrack = track->param()->eta();
    
    m_taggerHelper->IPCorr(track->param()->a0(), track->param()->z0(), d0, z0, phiTrack, etaTrack, track->param()->pT(), m_xBeamSpot, m_yBeamSpot);
	...
	*m_log << MSG::DEBUG << "Calculating " << (*pTagger) << " likelihood weight" << endreq;
	...
	w = getW("L2_IP1D", scaleVar(m_IP1D));

      } else if ((*pTagger) == "IP2D") {

	m_IP2D = d0Sign/sqrt(errIP2D*errIP2D + m_sigmaBeamSpot*m_sigmaBeamSpot);
	w = getW("L2_IP2D", scaleVar(m_IP2D));

      } else if ((*pTagger) == "IP3D") {

	m_IP1D = z0Sign/sqrt(errIP1D*errIP1D);
	m_IP2D = d0Sign/sqrt(errIP2D*errIP2D + m_sigmaBeamSpot*m_sigmaBeamSpot);
	w = getW("L2_IP3D", scaleVar(m_IP2D), scaleVar(m_IP1D));

	//* for monitoring purposes *//
	m_bjetMon->m_mon_l2_trk_Sa0_sel.push_back(m_IP1D);
	m_bjetMon->m_mon_l2_trk_Sz0_sel.push_back(m_IP2D);
