...
#ifndef __FIXED_32_BITSET_H__
#define __FIXED_32_BITSET_H__
...
class Fixed32BitSet {
 ...
 public:		
  ...
  Fixed32BitSet(const Fixed32BitSet& set) { 
    bits = set.bits; 
  }

  bool get(const int bitIndex) const { 
    return ((bits&setMask[bitIndex]) != 0); 
  }
  ...
  void clear(const int bitIndex) { 
    bits &= clearMask[bitIndex]; 
  }
  ...
  void And(Fixed32BitSet& set) { 
    bits &= set.bits; 
  }

  void NAnd(Fixed32BitSet& set) { 
    bits &= ~set.bits; 
  }
  
  void Or(Fixed32BitSet& set) { 
    bits |= set.bits; 
  }
  
  void XOr(Fixed32BitSet& set) { 
    bits ^= set.bits; 
  }
  
  bool operator==(const Fixed32BitSet& set) const { 
 ...
 private:
  ...
  // The bits used to store this Fixed32BitSet.  
  int bits;
};
