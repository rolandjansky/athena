...
/** static members **/
StrBTrk*   TrigL2PattRecoStrategyBC::m_trks=0;
StrBTrack* TrigL2PattRecoStrategyBC::m_full_trks=0;


//** ----------------------------------------------------------------------------------------------------------------- **//


TrigL2PattRecoStrategyBC::TrigL2PattRecoStrategyBC(const std::string& t, const std::string& n, const IInterface* p) : 
  AlgTool(t,n,p),
  m_useZvertexTool(false),
  m_nfreeCut(5)         ,
  ...
  declareProperty("nUsedLayers", m_nUsedLayers);
  ...
  declareProperty("maxFoundTracks",                   m_maxFoundTracks             = 1);
...
StatusCode TrigL2PattRecoStrategyBC::initialize() {
  ...
  if ( sc.isFailure() ) {
    athenaLog  << MSG::FATAL << "Unable to locate TrackFitter tool " << m_trigInDetTrackFitter << endreq;
    return sc;
  }  
  ...
  if (outputLevel <= MSG::DEBUG) {
    ...
    athenaLog << MSG::DEBUG << "MaxFoundTracks = "                  << m_maxFoundTracks                 << endreq;
  ...
  athenaLog << MSG::INFO << "TrigL2PattRecoStrategyBC initialized" << endreq;
...
StatusCode TrigL2PattRecoStrategyBC::finalize() {
...
TrigL2PattRecoStrategyBC::~TrigL2PattRecoStrategyBC() {
...
HLT::ErrorCode TrigL2PattRecoStrategyBC::findTracks(const std::vector<const TrigSiSpacePoint*>& vsp, const IRoiDescriptor& roi, TrigInDetTrackCollection*  trackColl) {
  ...
  if (outputLevel <= MSG::DEBUG) 
    athenaLog << MSG::DEBUG << "TrigL2PattRecoStrategyBC called in RoI-based mode" << endreq;
  ...
  TrigInDetTrackCollection seedTracks;

  executeTrackMaking(&seedTracks);

  m_trigInDetTrackFitter->fit(&seedTracks);

  TrackCollection* foundTracks = new TrackCollection;

  runCombinatorialTracking(seedTracks, foundTracks); 
...
HLT::ErrorCode TrigL2PattRecoStrategyBC::findTracks(const std::vector<const TrigSiSpacePoint*>& vsp, const IRoiDescriptor& roi, const TrigVertexCollection* vtx, TrigInDetTrackCollection*  trackColl) {
  ...
  /// NB: HOW IS THIS ROI SEEDED MODE< IT IT JUST USES THE ROI DIRECTION BUT
  ///     NOT THE ROI SIZE ENCODED IN THE ROI ITSELF!!!!

  roiPhiMin = m_roiPhi - m_phiHalfWidth;  
  roiPhiMax = m_roiPhi + m_phiHalfWidth;
  roiEtaMin = m_roiEta - m_etaHalfWidth;
  roiEtaMax = m_roiEta + m_etaHalfWidth;
...
HLT::ErrorCode TrigL2PattRecoStrategyBC::findTracks(const std::vector<const TrigSiSpacePoint*>& vsp, const TrigVertexCollection* vtx, TrigInDetTrackCollection* trackColl) {
...
HLT::ErrorCode TrigL2PattRecoStrategyBC::findTracks(const std::vector<const TrigSiSpacePoint*>& vsp, TrigInDetTrackCollection* trackColl) {
...
void TrigL2PattRecoStrategyBC::executeSeedRoads(float etaMin, float etaMax, float /*phiMin*/, float /*phiMax*/) {
...
void TrigL2PattRecoStrategyBC::executeSeedDataSorting(const std::vector<const TrigSiSpacePoint*>& spacePointData) {
...
HLT::ErrorCode TrigL2PattRecoStrategyBC::executeSeedMaking() {
...
HLT::ErrorCode TrigL2PattRecoStrategyBC::executeSeedMakingVertex(std::vector<float>& preZv) {
...
void TrigL2PattRecoStrategyBC::executeSeedFilter_Vertex() {
...
void TrigL2PattRecoStrategyBC::executeTripletDataSorting(const std::vector<const TrigSiSpacePoint*>& spacePointData) {
...
HLT::ErrorCode TrigL2PattRecoStrategyBC::executeTripletMaking() {
  ...
  float r12inv; // extrapolation scale along R or Z
  float dphi;
...
void TrigL2PattRecoStrategyBC::executeTripletAmbiguity() {
...
void TrigL2PattRecoStrategyBC::executeTrackAmbiguity() {
...
void TrigL2PattRecoStrategyBC::executeTrackMaking(TrigInDetTrackCollection* trackColl) {
...
void TrigL2PattRecoStrategyBC::executeSetup() {
...
void TrigL2PattRecoStrategyBC::executeCleanup() {
...
float TrigL2PattRecoStrategyBC::tripletAmbiguityDistance(StrBTriplet* t1, StrBTriplet* t2) {
...
float TrigL2PattRecoStrategyBC::tripletAmbiguityDistance(StrBTrack* t1, StrBTrack* t2) {
...
void TrigL2PattRecoStrategyBC::tripletMergeAmbiguity(StrBTrack* t1, StrBTrack* t2) {
...
float TrigL2PattRecoStrategyBC::trackAmbiguityFraction(StrBTrack* t1, StrBTrack* t2) {
...
float TrigL2PattRecoStrategyBC::trackAmbiguityDistance(StrBTrack* t1, StrBTrack* t2) {
...
void TrigL2PattRecoStrategyBC::trackAmbiguityShared(StrBTrack* t1, StrBTrack* t2) {
...
void TrigL2PattRecoStrategyBC::trackMergeAmbiguity(StrBTrack* t1, StrBTrack* t2) {
...
void TrigL2PattRecoStrategyBC::trackRemoveAmbiguity(StrBTrack* t1, StrBTrack* t2) {
...
bool TrigL2PattRecoStrategyBC::fit(TrigInDetTrack* t) {
...
void TrigL2PattRecoStrategyBC::circlefitRPhi(TrigInDetTrack* t, float &xc, float &yc, float &rc, float &phic, float &q, float &pt) {
...
void TrigL2PattRecoStrategyBC::fitRZ(TrigInDetTrack* t, float &xc, float &yc, float &rc, float &phic, float &q, float &pt) {
...
bool TrigL2PattRecoStrategyBC::fitRPhi(TrigInDetTrack* t, float &xc, float &yc) {
...
void TrigL2PattRecoStrategyBC::shiftRPhi(TrigInDetTrack* t) {
...
void TrigL2PattRecoStrategyBC::runCombinatorialTracking(const TrigInDetTrackCollection& input, TrackCollection* foundTracks) {

  MsgStream athenaLog(msgSvc(), name());
  int outputLevel = msgSvc()->outputLevel( name() );

  if (outputLevel <= MSG::DEBUG) 
    athenaLog<< MSG::DEBUG<<"In combinatorial tracking ... "<<endreq;

  StatusCode sc = getIDCs();

  if(sc.isFailure()) {
    athenaLog<< MSG::ERROR<<"Cannot retrieve IDCs with online spacepoints"<<endreq;
    return;
  }
  ...
  for(TrigInDetTrackCollection::const_iterator trIt = input.begin();trIt!=input.end();++trIt) {

    bool SP_based_mode=false;

    SP_based_mode = (m_nUsedLayers==3);

    if(!SP_based_mode) {

      float phi0=(*trIt)->param()->phi0(); 
      ...
      (*pCV)(0,0)=scale_factor*(*trIt)->param()->ea0()*(*trIt)->param()->ea0(); 
      (*pCV)(1,1)=scale_factor*(*trIt)->param()->ez0()*(*trIt)->param()->ez0(); 
      (*pCV)(2,2)=scale_factor*(*trIt)->param()->ephi0()*(*trIt)->param()->ephi0(); 
      (*pCV)(3,3)=scale_factor*(*trIt)->param()->eeta()*(*trIt)->param()->eeta()*sin(theta)*sin(theta);
      double Pt = (*trIt)->param()->pT();
      (*pCV)(4,4)=scale_factor*(sin(theta)*sin(theta)*(*trIt)->param()->epT()*(*trIt)->param()->epT()/ 
				   (Pt*Pt)+cos(theta)*cos(theta)*(*pCV)(3,3))/(Pt*Pt); 
      
			Trk::PerigeeSurface dummySurface;
      Trk::Perigee* pMP=new Trk::Perigee((*trIt)->param()->a0(), 
							 (*trIt)->param()->z0(), 
							 phi0,theta, 
      ...
      if (outputLevel <= MSG::DEBUG) {
	athenaLog<< MSG::DEBUG<<"Seed: "<<endreq;
	athenaLog<< MSG::DEBUG<<(*pMP)<<endreq;
      }
      ...
      int layerMap[19];
      int nGP=0;
      memset(&layerMap[0],0,sizeof(layerMap));
      
      const std::vector<const TrigSiSpacePoint*>* pSpVec=(*trIt)->siSpacePoints();
      for(std::vector<const TrigSiSpacePoint*>::const_iterator spIt=pSpVec->begin();spIt!=pSpVec->end();++spIt) {
	const TrigSiSpacePoint* p = (*spIt);
	int l = p->layer();
	
	if(layerMap[l]==0) {
	  gpList.push_back(Amg::Vector3D(p->original_x(), p->original_y(), p->z()));
	  nGP++;
	}
	layerMap[l]++;
	if(nGP>=m_nUsedLayers) break;
      }
      
      if (outputLevel <= MSG::DEBUG) {
	athenaLog<< MSG::DEBUG<<"created "<<gpList.size()<<" GlobalPositions on a track-seed"<<std::endl;
      }
      
      const std::list<Trk::Track*>& T = m_trackmaker->getTracks(*pMP,gpList); 
      delete pMP;
      
      if (outputLevel <= MSG::DEBUG) {
	athenaLog<< MSG::DEBUG<<"Found "<<T.size()<<" tracks using this seed"<<std::endl;
      }
	...
	if((*t)) 
      ...
      const std::vector<const TrigSiSpacePoint*>* pSpVec=(*trIt)->siSpacePoints();
      std::vector< Trk::SpacePoint*> offlineSpacePoints;
      std::vector<std::pair<const TrigSiSpacePoint*, const Trk::SpacePoint* > > onl2offl; 
      for(std::vector<const TrigSiSpacePoint*>::const_iterator spIt=pSpVec->begin();spIt!=pSpVec->end();++spIt) {
	const TrigSiSpacePoint* p = (*spIt);
	if(p->clusters().second==NULL) {//Pixel
	  ...
	  offlineSpacePoints.push_back(sp);
	  onl2offl.push_back(std::pair<const TrigSiSpacePoint*, const Trk::SpacePoint* >(p,sp));
	}
	else {//SCT
	  ...
	  if(locpos==NULL) {
	    onl2offl.push_back(std::pair<const TrigSiSpacePoint*, const Trk::SpacePoint* >(p,NULL));
	    continue;
	  }
	  delete locpos;
      ...
      std::map<int, std::vector<const Trk::SpacePoint* > > layerMap;

      for(std::vector<std::pair<const TrigSiSpacePoint*, const Trk::SpacePoint* > >::const_iterator vIt=onl2offl.begin();
	  vIt!=onl2offl.end();++vIt) {

	const TrigSiSpacePoint* p = (*vIt).first;
	int l = p->layer();

	std::map<int, std::vector<const Trk::SpacePoint* > >::iterator mIt=layerMap.find(l);
	if(layerMap.end()==mIt) {
	  layerMap.insert(std::pair<int, std::vector<const Trk::SpacePoint* > >(l,std::vector<const Trk::SpacePoint* >(0)));
	  mIt=layerMap.find(l);
	}
	(*mIt).second.push_back((*vIt).second);
      }
      ...
      int nTriplets = 0;
      int nTracks = 0;
      for(std::map<int, std::vector<const Trk::SpacePoint* > >::iterator l1It=layerMap.begin();(l1It!=layerMap.end()) && (nTracks==0);++l1It)
	for(std::vector<const Trk::SpacePoint* >::iterator v1It=(*l1It).second.begin();(v1It!=(*l1It).second.end()) && (nTracks!=m_maxFoundTracks);++v1It) {
	  std::map<int, std::vector<const Trk::SpacePoint* > >::iterator l2It=l1It;
	  ++l2It;
	  for(;(l2It!=layerMap.end()) && (nTracks!=m_maxFoundTracks);++l2It) 
	    for(std::vector<const Trk::SpacePoint* >::iterator v2It=(*l2It).second.begin();(v2It!=(*l2It).second.end()) && (nTracks!=m_maxFoundTracks);++v2It) {
	      std::map<int, std::vector<const Trk::SpacePoint* > >::iterator l3It=l2It;
	      ++l3It;
	      for(;(l3It!=layerMap.end()) && (nTracks!=m_maxFoundTracks);++l3It) 
		for(std::vector<const Trk::SpacePoint* >::iterator v3It=(*l3It).second.begin();v3It!=((*l3It).second.end()) && (nTracks!=m_maxFoundTracks);++v3It) {

		  std::list<const Trk::SpacePoint*> lSP;
		  ...
		  ++nTriplets;
		  if(m_timers) m_timer[5]->resume();
		  const std::list<Trk::Track*>& T = m_trackmaker->getTracks(lSP);
		  
		  if (outputLevel <= MSG::DEBUG) {
		    athenaLog<< MSG::DEBUG<<"Found "<<T.size()<<" tracks using triplet"<<endreq;
		  }
      ...
      if (outputLevel <= MSG::DEBUG) {
	athenaLog<< MSG::DEBUG<<"Created "<<nTriplets<<" triplets from this track and"<<endreq;
	athenaLog<< MSG::DEBUG<<"Found "<<nTracks<<" tracks using triplets"<<endreq;
      }
    }
  }
  if(m_timers) { 
  ...
  if (outputLevel <= MSG::DEBUG) {
    athenaLog<< MSG::DEBUG<<"After clone removal "<<m_ntracks<<" tracks left"<<endreq;
  }
}
...
double TrigL2PattRecoStrategyBC::trackQuality(const Trk::Track* Tr)
{
...
void TrigL2PattRecoStrategyBC::filterSharedTracks(std::multimap<double,Trk::Track*>& QT) {
...
TrigInDetTrack* TrigL2PattRecoStrategyBC::convertOutputTracks(Trk::Track* t) {
  ...
  pTrack->algorithmId(TrigInDetTrack::STRATEGY_BC_ID);

  return pTrack;

}

StatusCode TrigL2PattRecoStrategyBC::getIDCs() {
