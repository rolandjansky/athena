...
protected:  // member functions
  ...
  long fillVectors( const std::vector<SpacePoint *>& spVec, const IRoiDescriptor& roi,
...
template<class SpacePoint> long IDScanZFinderInternal<SpacePoint>::fillVectors (const std::vector<SpacePoint *>& spVec, 
										...
										std::vector<long>& filledLayers)
{
  ...
  // full scan check
  typename std::vector<SpacePoint *>::const_iterator SpItr( spVec.begin() );
...
template<class SpacePoint> 
std::vector<typename IDScanZFinderInternal<SpacePoint>::vertex>* IDScanZFinderInternal<SpacePoint>::findZInternal( const std::vector<SpacePoint *>& spVec, 
														   const IRoiDescriptor& roi)
{
  ...
  double zMin = -m_ZFinder_MaxZ;
  double zMax =  m_ZFinder_MaxZ;

  if ( roi.zedMinus()>zMin && roi.zedMinus()<zMax ) zMin = roi.zedMinus(); 
  if ( roi.zedPlus()<zMax && roi.zedPlus()>zMin)    zMax = roi.zedPlus(); 
