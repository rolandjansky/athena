...
namespace LVL1
{
  ...
  StatusCode JGTowerMaker::FexAlg(const std::vector<std::shared_ptr<JGTowerHelper>> &jgT, xAOD::JGTowerContainer *jgTContainer)
  {
    ...
    for (unsigned hs = 0; hs < jgT.size(); ++hs)
    {
      ...
      std::shared_ptr<JGTowerHelper> jgt = jgT.at(hs);
      float jgEt = 0;
      float lar_et = 0;
      float tile_et = 0;
      ...
      if (!m_useAllRun2TT)
      {
        for (unsigned i = 0; i < jgTowerSCIndex.size(); i++)
        {
          ...
          if (m_EmulateSC)
          {
            if (scell_et < 10e3 && std::abs(time) > 8)
              continue;
            if (scell_et >= 10e3 && (time > 16 || time < -8))
              continue;
          }
          else
          {
            if (m_useSCQuality && !(scell->provenance() & m_scQuality))
              continue;
            if (scell_et > 0 && scell_et < m_minSCETp)
              continue; //allows us to implement supercell ET cuts when building towers
            if (scell_et < 0 && scell_et > m_maxSCETm)
              continue;
          }
          jgEt += scell_et;
          lar_et += scell_et;
          ...
          else
          {
            ...
            for (unsigned tt_hs = 0; tt_hs < TTs->size(); tt_hs++)
            {
              ...
              if (tt->sampling() != 1 || std::abs(tt->eta()) > 1.5)
                continue;                                    // Tile raneg upto 1.5 with sampling == 1
              float ttPhi = TVector2::Phi_mpi_pi(tt->phi()); //converted [0,2pi] -> [-pi,pi]
              if (!jgt->inBox(tt->eta(), ttPhi))
                continue;
      ...
      else
      {
        ...
        {
          const xAOD::TriggerTower *tt = TTs->at(tt_hs);
          if (tt->sampling() != jgt->sampling() && std::abs(jgt->Eta()) < 3.1)
            continue;                                    // Tile raneg upto 1.5 with sampling == 1
          float ttPhi = TVector2::Phi_mpi_pi(tt->phi()); //converted [0,2pi] -> [-pi,pi]
          if (!jgt->inBox(tt->eta(), ttPhi))
            continue;
      ...
      if (jgEt < m_minTowerEt)
        jgEt = 0;
      xAOD::JGTower *trigTower = new xAOD::JGTower();
      jgTContainer->push_back(trigTower);
      trigTower->initialize(hs, jgt->Eta(), jgt->Phi());
      trigTower->setdEta(jgt->dEta());
      trigTower->setdPhi(jgt->dPhi());
      trigTower->setEt(jgEt);
      trigTower->setSCIndex(jgTowerSCIndex);
      trigTower->setTileIndex(jgTowerTileIndex);
      trigTower->setSampling(jgt->sampling());
    }
    return StatusCode::SUCCESS;
  }
  ...
  StatusCode JGTowerMaker::ForwardMapping()
  {
    ...
    for (unsigned sc_hs = 0; sc_hs < sc_hashMax; ++sc_hs)
    {
      ...
      if (std::abs(scEta) < 3.1)
        continue;
      float scDEta = dde->deta();
      float scDPhi = dde->dphi();

      std::shared_ptr<JGTowerHelper> JGT = std::make_shared<JGTowerHelper>(scEta, scDEta, scPhi, scDPhi);
      ...
      if (detName.find("FCAL0") != std::string::npos)
        JGT->SetSampling(2);
      if (detName.find("FCAL1") != std::string::npos)
        JGT->SetSampling(3);
      if (detName.find("FCAL2") != std::string::npos)
        JGT->SetSampling(4);
      m_jT.push_back(JGT);
    }
    ...
    for (unsigned int iEta = 0; iEta < 4; iEta++)
    {
      ...
      for (int isign = 1; isign > -2; isign = isign - 2)
      {
        ...
        for (unsigned int iPhi = 0; iPhi < nTowers; iPhi++)
        {
          ...
          if (fgT_Phi > TMath::Pi())
            fgT_Phi = fgT_Phi - 2 * TMath::Pi();
          //Allocate supercells into gTowers
          std::shared_ptr<JGTowerHelper> JGT = std::make_shared<JGTowerHelper>(fgT_Eta, fgT_dEta, fgT_Phi, fgT_dPhi);
          JGT->SetSampling(2);
          ...
          for (unsigned sc_hs = 0; sc_hs < sc_hashMax; ++sc_hs)
          {
            ...
            float scEta = dde->eta_raw();
            float scPhi = dde->phi_raw();
            if (std::abs(scEta) < 3.2)
              continue;
            if (JGT->inBox(scEta, scPhi) && isign * m_scid->pos_neg(scid) > 0)
            {
              JGT->SetSCIndices(sc_hs);
            }
          }
  ...
  StatusCode JGTowerMaker::TileMapping()
  {
    ...
    if (m_useAllCalo)
    {
      ...
      for (unsigned cell_hs = 0; cell_hs < cells->size(); cell_hs++)
      {
        ...
        if (cell == nullptr)
          continue;
        if (!(m_ccIdHelper->is_tile(cell->ID())))
          continue;

        for (unsigned hs = 0; hs < m_jT.size(); ++hs)
        {
          std::shared_ptr<JGTowerHelper> jt = m_jT.at(hs);
          if (jt->sampling() == 0)
            continue;
          if (jt->inBox(cell->eta(), cell->phi()))
            jt->SetTileIndices(cell_hs);
        }

        for (unsigned hs = 0; hs < m_gT.size(); ++hs)
        {
          std::shared_ptr<JGTowerHelper> gt = m_gT.at(hs);
          if (gt->sampling() == 0)
            continue;
          if (gt->inBox(cell->eta(), cell->phi()))
            gt->SetTileIndices(cell_hs);
        }
      }
    }
  ...
  StatusCode JGTowerMaker::SCTowerMapping()
  {
    ...
    for (int hs = 0; hs < m_jTowerHashMax; ++hs)
    {
      ...
      if (jPhi > TMath::Pi())
        jPhi = jPhi - 2 * TMath::Pi(); //m_jTowerId->phi0(rid)-jDPhi*(2*nTowers-m_jTowerId->phi(jid));

      std::shared_ptr<JGTowerHelper> JT = std::make_shared<JGTowerHelper>(jEta, jDEta, jPhi, jDPhi);
      ...
      for (unsigned sc_hs = 0; sc_hs < sc_hashMax; ++sc_hs)
      {
        ...
        if (std::abs(scEta) > 3.2)
          continue;
        if (std::abs(std::abs(dde->eta_raw()) - 1.4) < 0.001 && m_scid->region(scid) == 0 && m_scid->sampling(scid) == 2)
        {
          if (scEta > 0)
            scEta += 0.05;
          else
        ...
        if (JT->inBox(scEta, scPhi) && m_jTowerId->pos_neg(jid) * m_scid->pos_neg(scid) > 0 && ((m_scid->is_em(scid) && m_jTowerId->sampling(jid) == 0) || (!m_scid->is_em(scid) && m_jTowerId->sampling(jid) == 1)))
        {
    ...
    for (int hs = 0; hs < m_gTowerHashMax; ++hs)
    {
      ...
      if (gPhi > TMath::Pi())
        gPhi = gPhi - 2 * TMath::Pi();
      std::shared_ptr<JGTowerHelper> GT = std::make_shared<JGTowerHelper>(gEta, gDEta, gPhi, gDPhi);
      ...
      for (unsigned sc_hs = 0; sc_hs < sc_hashMax; ++sc_hs)
      {
        ...
        if (GT->inBox(scEta, scPhi) && m_gTowerId->pos_neg(gid) * m_scid->pos_neg(scid) > 0 && ((m_scid->is_em(scid) && m_gTowerId->sampling(gid) == 0) || (!m_scid->is_em(scid) && m_gTowerId->sampling(gid) == 1)))
        {
