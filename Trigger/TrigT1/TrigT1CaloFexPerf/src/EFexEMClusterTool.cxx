                                  ...
                                  const CaloCell_SuperCell_ID* &idHelper, float digitScale, float digitThreshold) const 
{
   ...
   if (!(sub_calo == 0 || sub_calo == 1) || !(abs(pos_neg) < 4)){
      ATH_MSG_WARNING ( "WARNING with local max logic");
      return false;
   }
                                        ...
                                        const CaloCell_SuperCell_ID* idHelper, float digitScale, float digitThresh ) const
{
   ...
   if (phiWidth == 2) addOnce(energeticPhiCell, centCells); //centCells.push_back(energeticPhiCell);
      ...
      ATH_MSG_WARNING ( "WARNING: phiWidth not 2 or 3!!!");
   }
...
LVL1::EFexEMClusterTool::matchingHCAL_TT(const CaloCell* &inputCell, const xAOD::TriggerTowerContainer* &TTContainer) const
{
   ...
   if (matchingTTs.size()==1) return matchingTTs[0];
      ...
      ATH_MSG_WARNING ( "WARNING: More than one matching HCAL TT!!! (Returned Null)");
   }
   return nullptr;
}
...
LVL1::EFexEMClusterTool::matchingHCAL_LAr(const CaloCell* &inputCell, const CaloConstCellContainer* &SCContainer, const  CaloCell_SuperCell_ID* &idHelper) const
{
   ...
   if (matchingCells.size()==0){
      ...
      ATH_MSG_DEBUG ( "DEBUG: No match betweem LAr ECAL SC and LAr HCAL SC!!! Input coords: " << inputCell->eta() << ", " << inputCell->phi());

   } else if (matchingCells.size()!=0) {

      ATH_MSG_DEBUG ( "More than one matching LAr HCAL SC!!! (Returned Null)");
      ATH_MSG_DEBUG ( "Input cell coords: " << inputCell->eta() << " x " << inputCell->phi());
      for (auto ithMatch : matchingCells){
         ATH_MSG_DEBUG ( "    " << ithMatch->eta() << " x " << ithMatch->phi() << ", dR = " 
                                       ...
                                       const CaloCell_SuperCell_ID* idHelper, float digitScale, float digitThresh ) const
{
   ...
   if (phiWidth > 1){  
      ...
      else if (phiWidth > 3) ATH_MSG_WARNING ( "WARNING: phiWidth not 2 or 3!!!. Value = " << phiWidth);
   }
                                                ...
                                                const CaloCell_SuperCell_ID* &idHelper) const
{
   ...
   if (ithRegion==0){
      ...
      else ATH_MSG_DEBUG ( "DEBUG: " << __LINE__);
   }
   else if (ithRegion==1){
                                             ...
                                             const CaloCell_SuperCell_ID* &idHelper) const
{
   ...
   if (ithSampling==0) maxEta_index = 2;
      ...
      case 0:
         ...
         break;
      case 1:
         ATH_MSG_WARNING ( "WARNING " << __LINE__);
         break;
      case 2:
         maxEta_index=11;
         break;
   ...
   else ATH_MSG_WARNING ( "WARNING: " << __LINE__ );
                                             ...
                                             const  CaloCell_SuperCell_ID* &idHelper) const
{
   ...
   else if (ithRegion!=1) ATH_MSG_WARNING ( "WARNING: " <<__LINE__);
