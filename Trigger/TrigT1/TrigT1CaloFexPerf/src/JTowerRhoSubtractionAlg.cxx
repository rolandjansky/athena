...
namespace LVL1
{
      ...
      : AthAlgorithm(name, pSvcLocator)
  {
    ...
    declareProperty("MappingMaker", m_mappingMaker, "The mapping maker");
  }
  ...
  StatusCode JTowerRhoSubtractionAlg::initialize()
  {
    ...
    ATH_CHECK(buildFexBins());
    return StatusCode::SUCCESS;
  }

  StatusCode JTowerRhoSubtractionAlg::buildFexBins()
  {
    ...
    const std::vector<JGTowerHelper> &helpers = m_mappingMaker->getMapping();
    ...
    {
      float eta = std::abs(helpers.at(idx).Eta());
      if (eta < 1.5)
    ...
    for (std::size_t regionIdx = 0; regionIdx < regions.size(); ++regionIdx)
    {
      ...
      {
        const JGTowerHelper &helper = helpers.at(towerIdx);
        for (std::size_t binIdx : findBinNumbers(helper.Eta(), helper.Phi(), etaBins, phiBins, phiOffset))
          m_jFEXBins.at(binIdx + regionIdx * nBins).push_back(towerIdx);
        for (std::size_t binIdx : findBinNumbers(helper.Eta(), helper.Phi(), etaBinsCore, phiBins, phiOffset))
  ...
  StatusCode JTowerRhoSubtractionAlg::execute()
  {
