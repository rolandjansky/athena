...
namespace LVL1
{
  ...
  StatusCode JGTowerBuilder::execute(const EventContext &ctx) const
  {
    ATH_MSG_DEBUG("Executing " << name());
    auto superCells = SG::makeHandle(m_superCellsKey, ctx);
    ATH_CHECK(superCells.isValid());
    const CaloCellContainer *cells = nullptr;
    const xAOD::TriggerTowerContainer *triggerTowers = nullptr;
    if (m_useAllCalo)
    {
      auto cellsHandle = SG::makeHandle(m_cellsKey, ctx);
      ATH_CHECK(cellsHandle.isValid());
      cells = cellsHandle.ptr();
    }
    else
    {
      auto triggerTowersHandle = SG::makeHandle(m_triggerTowersKey, ctx);
      ATH_CHECK(triggerTowersHandle.isValid());
      triggerTowers = triggerTowersHandle.ptr();
    }

    auto towerHandle = SG::makeHandle(m_towersKey, ctx);
    ...
    for (const JGTowerHelper &towerHelper : **mapping)
    {
      float LArEt = 0;
      float tileEt = 0;
      if (m_useAllRun2TT)
