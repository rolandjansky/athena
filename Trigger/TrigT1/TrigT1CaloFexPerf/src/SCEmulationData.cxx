...
namespace LVL1
{
  ...
  StatusCode SCEmulationData::initialize()
  {
    ...
    std::regex pattern("Layer(\\d+)_([\\d.]+)eta([\\d.]+)_([\\d.-]+)E([\\d.-]+)_midtrain");
    std::map<CaloSampling::CaloSample, std::set<std::pair<float, float>>> etaBins;
    std::map<CaloSampling::CaloSample, std::set<std::pair<float, float>>> eBins;
    ...
    for (TObject *obj : *tdir->GetListOfKeys())
    {
      ...
      auto eBin = std::make_pair(std::stof(match.str(4)), std::stof(match.str(5)));

      etaBins[sample].insert(etaBin);
      eBins[sample].insert(eBin);
      tmpHistMap[sample][std::make_pair(etaBin, eBin)] = (TH1F *)key->ReadObj();
    }
    ...
    for (CaloSampling::CaloSample sample : samples)
    {
      ...
      itr = eBins[sample].begin();
      end = eBins[sample].end();
      m_eBins[sample].reserve(std::distance(itr, end) + 1);
      m_eBins[sample].push_back(itr->first);
      m_eBins[sample].push_back(itr->second);
      ++itr;
      for (; itr != end; ++itr)
      {
        // Make sure that the bin edges match up
        if (itr->first != m_eBins[sample].back())
        {
          ATH_MSG_ERROR("E bins do not match up for sample " << sample);
          return StatusCode::FAILURE;
        }
        m_eBins[sample].push_back(itr->second);
      }
      ...
      for (const auto &p : tmpHistMap[sample])
      {
        ...
        std::size_t eIndex = getEIndex(sample, p.first.second.first);
        auto mapKey = std::make_tuple(sample, etaIndex, eIndex);
  ...
  StatusCode SCEmulationData::execute()
  {
    ...
    std::mt19937_64 generator1;
    std::mt19937_64 generator2;
    // seed the generator for the whole event
    generator1.seed(evtInfo->eventNumber() * evtInfo->runNumber());
    generator2.seed(evtInfo->eventNumber() * evtInfo->runNumber());
    if (m_useBCID)
      bcid = evtInfo->bcid();
	...
	if (cell->time() != 0)
	  ...
	  enTime[scIDHash] += cell->energy() * cell->time();
	...
	else if (!isTile)
        {
          ...
          std::size_t iE = getEIndex(sample, cell->energy());
          if (iE == SIZE_MAX)
          {
            ATH_MSG_ERROR("E value " << cell->energy() << " for sampling " << sample << " does not fall in a bin");
            return StatusCode::FAILURE;
          }
          float cellTime = getRandom(m_timingSamples.at(std::make_tuple(sample, iEta, iE)), generator2);
    ...
    for (std::size_t idx = 0; idx < energies.size(); ++idx)
    {
      ...
      if (!dde->is_tile() && sigmaNoise > 0.0)
      {
        ...
        energy += distribution(generator1);
      }
  ...
  std::size_t SCEmulationData::getEIndex(CaloSampling::CaloSample sample, float e)
  {
    ...
    if (warn)
      ATH_MSG_WARNING("E value " << e << " for sampling " << sample << " does not fall in a bin");
    return idx;
  }
} // namespace LVL1
