...
/*---------------------------------------------------------*/
{
  ...
  const CMXJetTobCollection* CMXJetTob = 0;
  ...
  CMXJetTobCollection::const_iterator it_CMXJetTob ;
  ...
  const CMXJetHitsCollection* CMXJetHits = 0;
  ...
  CMXJetHitsCollection::const_iterator it_CMXJetHits ;
  ...
  for (it_CMXJetHits = CMXJetHits->begin(); it_CMXJetHits != CMXJetHits->end();
    ...
    const int source = (*it_CMXJetHits)-> source();
    ...
    if (source == LVL1::CMXJetHits::TOTAL_MAIN) {
    ...
    if (crate == s_crates-1 && err0.get(DataError::Overflow)) {
      const int x = (source == LVL1::CMXJetHits::REMOTE_MAIN)
                     ? 0
                     : (source == LVL1::CMXJetHits::LOCAL_MAIN)
		        ? 1 : (source == LVL1::CMXJetHits::TOTAL_MAIN) ? 2 : 3;
	...
	if (p0) { // cable 0/phase 0 or cable 1/phase 0
          const int xpos = (source == LVL1::CMXJetHits::REMOTE_MAIN) ? 0 : 2;
	  m_h_cmx_1d_thresh_ParityRem->Fill(xpos);
	}
	if (p1) { // cable 0/phase 1 or cable 1/phase 1
	  const int xpos = (source == LVL1::CMXJetHits::REMOTE_MAIN) ? 1 : 3;
	  m_h_cmx_1d_thresh_ParityRem->Fill(xpos);
	}
	m_h_cmx_1d_ErrorSummary->Fill(JetParity);
	m_histTool->fillEventNumber(m_h_cmx_2d_ErrorEventNumbers, JetParity);
  ...
  const CMXEtSumsCollection* CMXEtSums = 0;
  ...
  for (it_CMXEtSums = CMXEtSums->begin(); it_CMXEtSums != CMXEtSums->end();
    ...
    // -----------------------------------------------------------------------
    // total energy sums
    else if (source == LVL1::CMXEtSums::TOTAL_STANDARD ||
      ...
      if (source == LVL1::CMXEtSums::TOTAL_STANDARD) {
    ...
    else if (source == LVL1::CMXEtSums::MISSING_ET_STANDARD ||
             source == LVL1::CMXEtSums::SUM_ET_STANDARD ||
	     source == LVL1::CMXEtSums::MISSING_ET_SIG_STANDARD) {
      const int nHits = 8;
      TH1F_LW* hist = (source == LVL1::CMXEtSums::MISSING_ET_STANDARD)
    ...
    else if (source == LVL1::CMXEtSums::MISSING_ET_RESTRICTED ||
             source == LVL1::CMXEtSums::SUM_ET_RESTRICTED) {
      const int nHits = 8;
      TH1F_LW* hist = (source == LVL1::CMXEtSums::MISSING_ET_RESTRICTED)
    ...
    if (source < s_modules || source == LVL1::CMXEtSums::REMOTE_STANDARD ||
    ...
    if (crate == 1 && (source == LVL1::CMXEtSums::REMOTE_STANDARD ||
                       source == LVL1::CMXEtSums::LOCAL_STANDARD  ||
	               source == LVL1::CMXEtSums::TOTAL_STANDARD)) {
      const double ypos = (source == LVL1::CMXEtSums::REMOTE_STANDARD) ? 0.
                        : (source == LVL1::CMXEtSums::LOCAL_STANDARD)  ? 1. : 2.;
    ...
    else if (crate == 1 && (source == LVL1::CMXEtSums::REMOTE_RESTRICTED ||
                            source == LVL1::CMXEtSums::LOCAL_RESTRICTED  ||
	                    source == LVL1::CMXEtSums::TOTAL_RESTRICTED)) {
      const double ypos = (source == LVL1::CMXEtSums::REMOTE_RESTRICTED) ? 0.
                        : (source == LVL1::CMXEtSums::LOCAL_RESTRICTED)  ? 1. : 2.;
  ...
  const LVL1::CMXRoI* CR = 0;
  sc = evtStore()->retrieve (CR, m_CMXRoILocation);
  ...
  const int rawExM = (CR)->ex(LVL1::CMXRoI::MASKED);
  const int rawEyM = (CR)->ey(LVL1::CMXRoI::MASKED);
  const int etM    = (CR)->et(LVL1::CMXRoI::MASKED);
  const int exErrorM = (CR)->exError(LVL1::CMXRoI::MASKED);
  const int eyErrorM = (CR)->eyError(LVL1::CMXRoI::MASKED);
  const int etErrorM = (CR)->etError(LVL1::CMXRoI::MASKED);
  const int sumEtHitsM = (CR)->sumEtHits(LVL1::CMXRoI::MASKED);
  const int missingEtHitsM = (CR)->missingEtHits(LVL1::CMXRoI::MASKED);
