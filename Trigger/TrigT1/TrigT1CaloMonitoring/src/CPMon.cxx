...
/*---------------------------------------------------------*/
{
  ...
  if (triggerTowerTES) {
    xAOD::TriggerTowerContainer_v2::const_iterator ttIterator =
                                                      triggerTowerTES->begin(); 
    xAOD::TriggerTowerContainer_v2::const_iterator ttIteratorEnd =
                                                      triggerTowerTES->end(); 
  ...
  if (cmxCpTobTES) {
    CmxCpTobCollection::const_iterator crIterator    = cmxCpTobTES->begin(); 
    CmxCpTobCollection::const_iterator crIteratorEnd = cmxCpTobTES->end(); 
    for (; crIterator != crIteratorEnd; ++crIterator) {
      const int crate     = (*crIterator)->crate();
      const int cpm       = (*crIterator)->cpm();     // 1-14
      const int cmx       = (*crIterator)->cmx();     // 0=Left, 1=Right  (Assumed in Sim to be Left Tau, Right EM)
      const int chip      = (*crIterator)->chip();    // 4 bits
      const int location  = (*crIterator)->location();// 2 bits
      const int energy    = (*crIterator)->energy();
      const int isolation = (*crIterator)->isolation();
      const int error     = (*crIterator)->error();
      const int x = crate*s_modules + cpm - 1;
  ...
  if (cmxCpHitsTES) {
    CmxCpHitsCollection::const_iterator cmIterator    = cmxCpHitsTES->begin(); 
    CmxCpHitsCollection::const_iterator cmIteratorEnd = cmxCpHitsTES->end(); 
    for (; cmIterator != cmIteratorEnd; ++cmIterator) {
      const unsigned int hits0 = (*cmIterator)->hits0();
      const unsigned int hits1 = (*cmIterator)->hits1();
      const int crate  = (*cmIterator)->crate();
      const int cmx    = (*cmIterator)->cmx();
      const int source = (*cmIterator)->source();
      const int peak   = (*cmIterator)->peak();
      const int slices = ((*cmIterator)->hitsVec0()).size();
      const int crateCmx = crate*2 + cmx;
      m_h_cmx_2d_thresh_Slices->Fill(crate*s_maxSlices + slices -1, peak, 1.);
      if (source == LVL1::CMXCPHits::TOPO_CHECKSUM) {
	if (hits0) m_h_cmx_1d_topo_OutputChecksum->Fill(crateCmx);
      } else if (source == LVL1::CMXCPHits::TOPO_OCCUPANCY_MAP) {
	if (hits0) {
          m_histTool->fillThresholdsVsY(m_h_cmx_2d_topo_CPMOccupancyMap,
	                                hits0, crateCmx, s_modules, 1, 1);
        }
      } else if (source == LVL1::CMXCPHits::TOPO_OCCUPANCY_COUNTS) {
	TH1F_LW* hist = (cmx) ? m_h_cmx_1d_topo_TOBsPerCPMRight
	  ...
	  for (int mod = 0; mod < s_modules/2; ++mod) {
        ...
        int bin = 0;
	if      (source == LVL1::CMXCPHits::LOCAL)    bin = crate;
	else if (source == LVL1::CMXCPHits::REMOTE_0) bin = s_crates;
	else if (source == LVL1::CMXCPHits::REMOTE_1) bin = s_crates + 1;
	else if (source == LVL1::CMXCPHits::REMOTE_2) bin = s_crates + 2;
	else if (source == LVL1::CMXCPHits::TOTAL)    bin = s_crates + 3;
