...
L1CaloPedestalGenerator::L1CaloPedestalGenerator(const std::string& name, ISvcLocator* pSvcLocator): Algorithm(name, pSvcLocator), m_detStore(0), m_storeGate(0), m_caloMgr(0), m_lvl1Helper(0), m_towerKey(0), m_IntTTContainer(0)
{
...
{
    MsgStream log( msgSvc(), name() );
    log << MSG::DEBUG <<"From Execute..."<<endreq;
    ...
    if (sc.isSuccess()) log << MSG::DEBUG << "Stored TTs in TES at "<< m_TTLocation << endreq;
    else {
    log << MSG::ERROR << "failed to write TTs to TES at " << m_TTLocation << endreq;
    return StatusCode::FAILURE;
    }
	...
	log <<MSG::DEBUG<< VectorOfTTs->size()<<" TTs have been generated"<<endreq;

    sc = m_storeGate->setConst(VectorOfTTs);
    if (sc.isSuccess()) log << MSG::DEBUG << "TT container locked"<< endreq;
    else {
        log << MSG::ERROR << "failed to lock TT container"<< endreq;
        return StatusCode::FAILURE;
    }
