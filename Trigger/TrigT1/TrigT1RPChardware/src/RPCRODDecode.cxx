...
int RPCRODDecode::pushWord(const ubit16 inword) {
    ...
    if (m_enablePrintOut) { std::cout << " RPCRODDecode; inword= " << std::hex << inword << std::dec << std::endl; }
    //
    ...
    if (SLROS.isHeader()) {
        ...
        // SL Header
        //
        if (m_enablePrintOut) { std::cout << " RPCRODDecode: SectorLogic Header " << std::endl; }
        m_isSLBody = true;
        SLRO.reset();
        ...
        // SL Footer
        //
        if (m_enablePrintOut) { std::cout << " RPCRODDecode: SectorLogic Footer " << std::endl; }
        m_isSLBody = false;
        thereIsASL = true;
        ...
        // SL Body
        //
        if (m_enablePrintOut) { std::cout << " RPCRODDecode: SectorLogic Body " << std::endl; }
        SLRO.writeRecord(inword, false);
        thereIsASL = false;
        ...
        // RX header
        //
        if (m_enablePrintOut) { std::cout << " RPCRODDecode: RX Header " << std::endl; }
        m_RXFlag++;
        RXROS.decodeFragment(inword, recField);
        ...
        switch (m_previousRecord) {
            ...
            case CMFoot:
                ...
                break;
            case PadHead:
                if (m_enablePrintOut) { std::cout << " RPCRODDecode Fragment problem: PadSubHeader expected" << std::endl; }
                break;
            case PadPre:
                if (m_enablePrintOut) { std::cout << " RPCRODDecode Fragment problem: CMA fragment expected" << std::endl; }
                break;
            case PadSub:
                if (m_enablePrintOut) { std::cout << " RPCRODDecode Fragment problem: CMA Footer expected" << std::endl; }
                break;
            case PadFoot:
                if (m_enablePrintOut) { std::cout << " RPCRODDecode Fragment problem: Pad PreFooter expected" << std::endl; }
                break;
            default:
                if (m_enablePrintOut) { std::cout << " RPCRODDecode ERROR in previousRecord " << std::endl; }
        }  // end-of-switch
        m_previousRecord = Empty;
        ...
        if (recField == 'F') {}
        thereIsACM = false;
        if (m_enablePrintOut) { std::cout << " RPCRODDecode: Footer of Sector with ID = " << SectorID << std::endl; }
        ...
        switch (m_previousRecord) {
            ...
            case CMHead:
                ...
                break;
            case CMSub:
                if (m_enablePrintOut) { std::cout << " RPCRODDecode Fragment problem: CM Body expected and RX Footer found" << std::endl; }
                thereIsACM = true;  // close this matrix fragment
                CMRO.writeRecord(inword, true);
                // gimeCMROData();
                break;
        ...
        switch (m_previousRecord) {
            case Empty: break;
            case CMHead:
                if (m_enablePrintOut) { std::cout << " RPCRODDecode Fragment problem: CM SubHeader expected and not found" << std::endl; }
                thereIsACM = true;  // close this matrix fragment
                CMRO.writeRecord(inword, true);
                // gimeCMROData();
                break;
            case CMSub:
                if (m_enablePrintOut) { std::cout << " RPCRODDecode Fragment problem: CM Body expected and not found" << std::endl; }
                thereIsACM = true;  // close this matrix fragment
                CMRO.writeRecord(inword, true);
                // gimeCMROData();
                break;
        ...
        switch (m_previousRecord) {
            ...
            case CMBod:
                ...
                break;
            case CMFoot:
                if (m_enablePrintOut) { std::cout << " RPCRODDecode Fragment problem: Pad Header expected and not found" << std::endl; }
                break;
            case PadHead:
                if (m_enablePrintOut) { std::cout << " RPCRODDecode Fragment problem: PadSubHeader expected" << std::endl; }
                break;
        ...
        switch (m_previousRecord) {
            ...
            case CMBod:
                ...
                break;
            case CMFoot:
                if (m_enablePrintOut) { std::cout << " RPCRODDecode Fragment problem: Pad PreFooter expected and not found" << std::endl; }
                break;
            case PadHead:
                if (m_enablePrintOut) { std::cout << " RPCRODDecode Fragment problem: PadSubHeader expected" << std::endl; }
                break;
        ...
        switch (m_previousRecord) {
            case Empty: break;
            case CMHead:
                if (m_enablePrintOut) { std::cout << " RPCRODDecode warning: CM SubHeader expected and CM Header found" << std::endl; }
                break;
            case CMSub: break;
            case CMBod:
                if (m_enablePrintOut) {
        ...
        switch (m_previousRecord) {
            case Empty:
                if (m_enablePrintOut) { std::cout << " RPCRODDecode warning: previous record was not a CM Header" << std::endl; }
                break;
            case CMHead: break;
            case CMSub:
                if (m_enablePrintOut) { std::cout << " RPCRODDecode warning: CM Body expected and CM SubHeader found" << std::endl; }
                break;
            case CMBod:
                if (m_enablePrintOut) {
        ...
        switch (m_previousRecord) {
            ...
            case CMHead:
                ...
                break;
            case CMSub:
                if (m_enablePrintOut) { std::cout << " RPCRODDecode warning: previous record was not a Body record" << std::endl; }
                break;
            case CMBod: break;
            case CMFoot:
                if (m_enablePrintOut) {
        ...
        switch (m_previousRecord) {
            ...
            case CMHead:
                ...
                break;
            case CMSub:
                if (m_enablePrintOut) { std::cout << " RPCRODDecode Fragment problem: CM Body expected and (reserved) found" << std::endl; }
                thereIsACM = true;  // close this matrix fragment
                CMRO.writeRecord(inword, true);
                // gimeCMROData();
                break;
...
//---------------------------------------------------------------------------//
void RPCRODDecode::RODHeader(const RODword *RODData) {
    //
...
//----------------------------------------------------------------------------//
// void RPCRODDecode::ROBAddresses(const RODword *ROBData) {
////
...
//}//end-of-ROBAddresses
//----------------------------------------------------------------------------//
void RPCRODDecode::RODAddresses(const RODword *RODData, const sbit32 numberOfStatusElements, const sbit32 statusBlockPosition) {
    RODHeader(RODData);
