...
//----------------------------------------------------------------------------//
void RPCRXRODDecode::disablePrintOut() { m_enablePrintOut = false; }  // dRPCRXRODDecode::disablePrintOut
//----------------------------------------------------------------------------//
...
int RPCRXRODDecode::pushWord(const ubit16 inword) {
    ...
    if (m_enablePrintOut) { std::cout << " RPCRXRODDecode; inword= " << std::hex << inword << std::dec << std::endl; }
    //
    ...
    if (SLROS.isHeader()) {
        ...
        // SL Header
        //
        if (m_enablePrintOut) { std::cout << " RPCRXRODDecode: SectorLogic Header " << std::endl; }
        m_isSLBody = false;
        SLRO.reset();
        ...
        // SL Header
        //
        if (m_enablePrintOut) { std::cout << " RPCRXRODDecode: SectorLogic SubHeader " << std::endl; }
        m_isSLBody = true;
        SLRO.writeRecord(inword, false);
        ...
        // SL Footer
        //
        if (m_enablePrintOut) { std::cout << " RPCRXRODDecode: SectorLogic Footer " << std::endl; }
        m_isSLBody = false;
        thereIsASL = true;
        ...
        // SL Body
        //
        if (m_enablePrintOut) { std::cout << " RPCRXRODDecode: SectorLogic Body " << std::endl; }
        SLRO.writeRecord(inword, false);
        thereIsASL = false;
        ...
        // RX header
        //
        if (m_enablePrintOut) { std::cout << " RPCRXRODDecode: RX Header " << std::endl; }
        m_RXFlag++;
        RXROS.decodeFragment(inword, recField);
        if (recField == 'H') { SectorID = RXROS.RXid(); }
        thereIsACM = false;
        if (m_enablePrintOut) { std::cout << " Sector ID = " << SectorID << std::endl; }
        ...
        switch (m_previousRecord) {
            ...
            case CMFoot:
                ...
                break;
            case PadHead:
                if (m_enablePrintOut) { std::cout << " RPCRXRODDecode Fragment problem: PadSubHeader expected" << std::endl; }
                break;
            case PadPre:
                if (m_enablePrintOut) { std::cout << " RPCRXRODDecode Fragment problem: CMA fragment expected" << std::endl; }
                break;
            case PadSub:
                if (m_enablePrintOut) { std::cout << " RPCRXRODDecode Fragment problem: CMA Footer expected" << std::endl; }
                break;
            case PadFoot:
                if (m_enablePrintOut) { std::cout << " RPCRXRODDecode Fragment problem: Pad PreFooter expected" << std::endl; }
                break;
            default:
                if (m_enablePrintOut) { std::cout << " RPCRXRODDecode ERROR in previousRecord " << std::endl; }
        }  // end-of-switch
        m_previousRecord = Empty;
        ...
        // RX footer
        //
        if (m_enablePrintOut) { std::cout << " RX footer " << std::endl; }
        m_RXFlag--;
        RXROS.decodeFragment(inword, recField);
        if (recField == 'F') {}
        thereIsACM = false;
        if (m_enablePrintOut) { std::cout << " RPCRXRODDecode: Footer of Sector with ID = " << SectorID << std::endl; }
        ...
        // Pad header
        //
        if (m_enablePrintOut) { std::cout << " PAD Header " << std::hex << inword << std::dec << std::endl; }
        m_PADFlag++;
        PDROS.decodeFragment(inword, recField);
        if (recField == 'H') { PadID = PDROS.padid(); }
        thereIsACM = false;
        if (m_enablePrintOut) {
        ...
        switch (m_previousRecord) {
            case Empty: break;
            case CMHead:
                if (m_enablePrintOut) { std::cout << " RPCRXRODDecode Fragment problem: CM SubHeader expected and not found" << std::endl; }
                thereIsACM = true;  // close this matrix fragment
                CMRO.writeRecord(inword, true);
                // gimeCMROData();
                break;
            case CMSub:
                if (m_enablePrintOut) { std::cout << " RPCRXRODDecode Fragment problem: CM Body expected and not found" << std::endl; }
                thereIsACM = true;  // close this matrix fragment
                CMRO.writeRecord(inword, true);
                // gimeCMROData();
                break;
        ...
        // Pad Subheader
        //
        if (m_enablePrintOut) { std::cout << "   BCID " << std::hex << (inword & 0x0fff) << std::dec << std::endl; }
        switch (m_previousRecord) {
            case Empty: break;
            case CMHead:
                if (m_enablePrintOut) { std::cout << " RPCRXRODDecode Fragment problem: CM SubHeader expected and not found" << std::endl; }
            ...
            case CMBod:
                ...
                break;
            case CMFoot:
                if (m_enablePrintOut) { std::cout << " RPCRXRODDecode Fragment problem: Pad Header expected and not found" << std::endl; }
                break;
            case PadHead:
                if (m_enablePrintOut) { std::cout << " RPCRXRODDecode Fragment problem: PadSubHeader expected" << std::endl; }
                break;
        ...
        // Pad PreFooter
        //
        if (m_enablePrintOut) { std::cout << "   STATUS ERROR " << std::hex << (inword & 0x000f) << std::dec << std::endl; }
        switch (m_previousRecord) {
            case Empty: break;
            case CMHead:
                if (m_enablePrintOut) { std::cout << " RPCRXRODDecode Fragment problem: CM SubHeader expected and not found" << std::endl; }
        ...
        //
        if (m_enablePrintOut) {
            std::cout << " PAD Footer " << std::hex << inword << std::dec << "  ERROR FLAG " << std::hex << (inword & 0x0fff) << std::dec
                      << std::endl;
        }
        m_PADFlag--;
        PDROS.decodeFragment(inword, recField);
        if (recField == 'F') {}
        thereIsACM = false;
        if (m_enablePrintOut) { std::cout << " Footer of Pad with ID = " << PadID << std::endl; }
        ...
        switch (m_previousRecord) {
            case Empty: break;
            case CMHead:
                if (m_enablePrintOut) { std::cout << " RPCRXRODDecode warning: CM SubHeader expected and CM Header found" << std::endl; }
                break;
            case CMSub: break;
            case CMBod:
                if (m_enablePrintOut) {
        ...
        switch (m_previousRecord) {
            case Empty:
                if (m_enablePrintOut) { std::cout << " RPCRXRODDecode warning: previous record was not a CM Header" << std::endl; }
                break;
            case CMHead: break;
            case CMSub:
                if (m_enablePrintOut) { std::cout << " RPCRXRODDecode warning: CM Body expected and CM SubHeader found" << std::endl; }
                break;
            case CMBod:
                if (m_enablePrintOut) {
        ...
        switch (m_previousRecord) {
            ...
            case CMHead:
                ...
                break;
            case CMSub:
                if (m_enablePrintOut) { std::cout << " RPCRXRODDecode warning: previous record was not a Body record" << std::endl; }
                break;
            case CMBod: break;
            case CMFoot:
                if (m_enablePrintOut) {
        ...
        //
        thereIsACM = false;
        if (m_enablePrintOut) { std::cout << " (reserved) " << std::endl; }
...
void RPCRXRODDecode::gimeCMROData() {
    ...
    if (m_enablePrintOut) {
        ...
        for (ubit16 i = 0; i < nCMROData; i++) {
            ...
            if (ijk < 6) {
                std::cout << " gimeCMROData, hit chan: " << CMROS.channel() << " gimeCMROData, hit stri: " << CMROS.global_channel()
                          << " hit BCID: " << CMROS.bcid() << std::endl;
            } else if (ijk == 6) {
                std::cout << " gimeCMROData, trg chan: " << CMROS.channel() << " hit BCID: " << CMROS.bcid() << std::endl;
            } else if (ijk == 7) {
                std::cout << " gimeCMROData, trg thr: " << CMROS.threshold() << " trg ovl: " << CMROS.overlap() << std::endl;
            } else {
...
//}//end-of-ROBAddresses
//----------------------------------------------------------------------------//
void RPCRXRODDecode::RODAddresses(const RODword *RODData, const sbit32 numberOfStatusElements, const sbit32 statusBlockPosition) {
    RODHeader(RODData);
