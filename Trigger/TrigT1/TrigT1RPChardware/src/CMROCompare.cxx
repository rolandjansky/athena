...
//----------------------------------------------------------------------------//
CMROCompare::CMROCompare(MatrixReadOut *h, MatrixReadOut *s) : BaseObject(Hardware, "CMROCompare") {
    m_hardware = h;
    m_simulation = s;
    m_diffOutput = 0;
    //
...
//----------------------------------------------------------------------------//
CMROCompare::~CMROCompare() {}  // distructor
//----------------------------------------------------------------------------//
...
void CMROCompare::compareBody() {
    ...
    if (numberOfBodyRecHard != numberOfBodyRecSimu) {
        ...
        cout << " CMROCompare: different number of body words " << endl
             << "   hardware: " << numberOfBodyRecHard << "   simulation: " << numberOfBodyRecSimu << endl;
    } else {
        m_hardware->topCMABody();
        m_simulation->topCMABody();
        for (ubit16 i = 0; i < numberOfBodyRecHard; i++) {
            ...
            if (hardRec != simuRec) {
                ...
                cout << " CMROCompare: different body words "
                     << "   hardware: " << std::hex << hardRec << std::dec << "   simulation: " << std::hex << simuRec << std::dec
                     << std::endl;
                if (firstDiff) {
                    if (i < 50000)
