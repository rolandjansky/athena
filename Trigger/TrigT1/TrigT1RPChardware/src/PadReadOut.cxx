...
//----------------------------------------------------------------------------//
PadReadOut::PadReadOut(ubit16 padID, MatrixReadOut *MROlist[8]) : BaseObject(Hardware, "PadReadOut") {
    m_padID = padID;
    //
...
//----------------------------------------------------------------------------//
PadReadOut::~PadReadOut() {}  // end-of-PadReadOut::PadReadOut
//----------------------------------------------------------------------------//
...
void PadReadOut::makeBody(bool debugPrint) {
    ...
    for (i = 0; i < 8; i++) {
        if (m_MROlist[i]) {
            ...
            if (debugPrint)
                cout << " Identificatore di questa Matrice e': " << cmid << endl
    ...
    for (i = 0; i < 8; i++) {
        if (CMAFragment[i]) {
            ...
            if (debugPrint) cout << " makeBody; number of CM WOrds=" << m_CMAFragment[j]->numberOfFragmentWords() << endl;
...
//----------------------------------------------------------------------------//
ubit16 PadReadOut::readHeader() { return m_Header; }  // end-of-PadReadOut::readHeader()
//----------------------------------------------------------------------------//
ubit16 PadReadOut::readFooter() { return m_Footer; }  // end-of-PadReadOut::readFooter()
//----------------------------------------------------------------------------//
...
ubit16 PadReadOut::readPADWord() {
    ...
    if (m_addressOfWordScanned == 0)
        output = readHeader();
    else if (m_addressOfWordScanned == (m_numberOfWordsInFragment - 1))
        output = readFooter();
    else
        output = readBody();
