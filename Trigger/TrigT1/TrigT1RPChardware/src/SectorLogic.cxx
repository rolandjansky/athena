...
void SectorLogic::check(void) {
  ...
  for(bcid=0;bcid<=m_nBunMax-1;bcid++) {
    ...
    cout << "LowPtFilter_in BCID is " << m_LowPtFilter_in[bcid].out.bcid << endl
         ...
         << "TileCalConfirm_out BCID is " << m_TileCalConfirm_out[bcid].out.bcid << endl

         << "SolveEtaOverlap_in BCID is " << m_SolveEtaOverlap_in[bcid].out.bcid << endl
         << "SolveEtaOverlap_out BCID is " << m_SolveEtaOverlap_out[bcid].out.bcid << endl

         << "SortHighest_in BCID is " << m_SortHighest_in[bcid].out.bcid << endl
         << "SortHighest_out BCID is " << m_SortHighest_out[bcid].out.bcid << endl
...
void SectorLogic::execute(){
  ...
  for (ibx=0; ibx<=m_nBunMax-1; ibx++) {
    ...
    int ipad=0;
    for(ipad=0;ipad<=7;ipad++) {
      m_LowPtFilter_in[ibx].pad[ipad] = m_InFromPad[ibx][ipad];
    }
    ...
    int OPLfrompads=0;
    for(i1=0;i1<=7;i1++) {
      OPLfrompads=setbits(OPLfrompads,i1,i1,m_LowPtFilter_in[ibx].pad[i1].opl);
    }
    // external cycle on the pads
    for(i1=0;i1<=7;i1++) {
	...
	if( getbits(EnableTCCheck,j1*4+Pt_reg2,j1*4+Pt_reg2) == 1 ) {
	  ...
	  if(TileCalCheck==0) {
	    m_TileCalConfirm_out[ibx].pad[j1].pt = 0;
	  }
	}
      }    
    }
    ...
    for(m1=0;m1<=7;m1++) {
      ...
      if( m_Sort2ndHighest_in[ibx].pad[m1].pt>Pt_reg5 && 
            (m_Sort2ndHighest_in[ibx].pad[m1].pt!=Pt_reg4 || 
	      m_Sort2ndHighest_in[ibx].pad[m1].roi!=ROI_reg4 ||
	      m1!=pad_reg4 ) ) {
...
ostream &operator<<(ostream &stream, SectorLogic &o) {
  ...
  stream.width(8);
  stream.fill('0');
  stream << o.m_EnableTCCheckLow << endl;
  stream << "EnableTCCheckHigh  : ";
  stream.width(8);
  stream.fill('0');
  stream << o.m_EnableTCCheckHigh << endl;
  int jj=0;
  int kk=0;
  for(jj=0;jj<=7;jj++) {
  ...
  stream.width(8);
  stream.fill('0');
  stream << o.m_EnableOPLCheck << endl;
  for(jj=0;jj<=7;jj++) {
