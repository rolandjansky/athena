...
void CTP_Decoder::setRDO(const CTP_RDO* rdo)
{
  ...
  for(unsigned int i = 0 ; i < nBunches ; ++i) {
    ...
    for(unsigned int pit = 0; pit < CTPdataformat::PITwords; ++pit) {
      unsigned int index = i*CTPdataformat::PITwords + pit;
      if(index >= PITs.size()) {
	getlog() << MSG::FATAL <<"Invalid PIT position " << index << endreq;
	return;
      }
      m_BCs[i].setPITWord(PITs[index],pit);
    }

    for(unsigned int tbp = 0; tbp < CTPdataformat::TBPwords; ++tbp) {
      unsigned int index = i * CTPdataformat::TBPwords + tbp;
      if(index >= TBPs.size()) {
	getlog() << MSG::FATAL <<"Invalid TBP position " << index << endreq;
	return;
      }
      m_BCs[i].setTBPWord(TBPs[index],tbp);
    }
    
    for(unsigned int tap = 0; tap < CTPdataformat::TAPwords; ++tap) {
      unsigned int index = i * CTPdataformat::TAPwords + tap;
      if(index >= TAPs.size()) {
	getlog() << MSG::FATAL <<"Invalid TAP position " << index << endreq;
	return;
      }
      m_BCs[i].setTAPWord(TAPs[index],tap);
    }
    
    for(unsigned int tav = 0; tav < CTPdataformat::TAVwords; ++tav) {
      unsigned int index = i * CTPdataformat::TAVwords + tav;
      if(index >= TAVs.size()) {
	getlog() << MSG::FATAL <<"Invalid TAV position " << index << endreq;
	return;
      }
      m_BCs[i].setTAVWord(TAVs[index],tav);
    }
  }
}
...
void CTP_BC::dumpData(MsgStream& msglog) const
{
  ...
  if(m_pit.any()) {
    ...
    int count = 0;
    for(unsigned int i = 0; i<m_pit.size() ; ++i) {
      if(m_pit.test(i)) {
    ...
    if( msglog.level() <= MSG::DEBUG )
      msglog << MSG::DEBUG << "No PIT with input!" << endreq;
  }
  ...
  if( msglog.level() <= MSG::VERBOSE ){
    msglog << MSG::VERBOSE << "PIT - total size: " << m_pit.size() << ", with input: " 
	<< m_pit.count() << ", pattern:" << std::endl << printPIT() << endreq;
...
{
  std::bitset<32> bcid = (m_pit5 >> CTPdataformat::BcidShift);
  bcid &= CTPdataformat::BcidMask;
  return bcid;
}
...
{
  std::bitset<32> rnd = (m_pit5 >> CTPdataformat::RandomTrigShift);
  rnd &= CTPdataformat::RandomTrigMask;
  return rnd;
}
...
{
  std::bitset<32> prcl = (m_pit5 >> CTPdataformat::PrescaledClockShift);
  prcl &= CTPdataformat::PrescaledClockMask;
  return prcl;
}
...
void CTP_BC::setPITWord( uint32_t word, uint32_t pos) 
{
  ...
  if( pos < (CTPdataformat::AuxPITwordPos-1) ) m_pit |= bs;
  else setPITWord5(word);
...
std::string CTP_BC::printPITWord5() const
{
  return m_pit5.to_string<char, 
    std::char_traits<char>, std::allocator<char> >();
}

void CTP_BC::setTBPWord( uint32_t word, uint32_t pos) 
{
  if(pos >= CTPdataformat::TBPwords) {
    getlog() << MSG::ERROR <<"Invalid TBP position " << pos <<endreq;
    return;
  }
  std::bitset<256> bs = word;
  bs <<= (pos * CTP_RDO::SIZEOF_WORDS);
  m_tbp |= bs;
}
