...
void JEMHitsCnv_p1::persToTrans( const JEMHits_p1* persObj, JEMHits* transObj, MsgStream &log ) {
  ...
  //log << MSG::INFO << "Converting JEMHits from persistent state..." << endmsg;
  ...
  *transObj = JEMHits (persObj->m_crate,
  ...
  if (log.level() <= MSG::DEBUG) log << MSG::DEBUG << "Converted JEMHits from persistent state [OK]" << endmsg;
...
void JEMHitsCnv_p1::transToPers( const JEMHits* transObj, JEMHits_p1* persObj, MsgStream &log ) {
  ...
  //log << MSG::INFO << "Creating persistent state of JEMHits..." << endmsg;
  ...
  if (log.level() <= MSG::DEBUG) log << MSG::DEBUG << "Created persistent state of JEMHits [OK]" << endmsg;
