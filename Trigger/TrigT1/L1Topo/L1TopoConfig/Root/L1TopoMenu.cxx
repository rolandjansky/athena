...
void TXC::L1TopoMenu::writeXML(const std::string& filename){
   ...
   xmlfile << "<?xml version=\"1.0\" ?>\n"
        ...
        << "  <OutputList>\n";
        for(auto& in : m_outputlist.getOutputList()){
           xmlfile << "    <Output algname=\"" << in.algoname() << "\" triggerline=\"" << in.algoname() << "\" algoId=\"" << in.algoid() << "\" module=\"" << in.module() << "\" fpga=\"" << in.fpga() << "\" firstbit=\"" << in.firstbit() << "\" clock=\"" << in.clock() << "\"/>" << std::endl;
        }
    xmlfile << "  </OutputList>\n\n"
            << "  <TopoConfig>\n";
