...
*********************************/
    ...
    public:
       ...
       L1TopoDataTypes& operator*=(const int& factor){
          short int neg = (m_tvalue >> (PREC-1ull)) ? 1 : 0;
          if(neg && factor<0){
             neg = 0;
       ...
       float to_float() const {
          // Find sign
          float res = ((m_tvalue>>(PREC-1ull))&1ull) ? -(1ll<<(PREC-F)) : 0.;
          // Get integer part
          res += (m_tvalue>>F)&((1ull<<(PREC-F))-1ull) ? float((m_tvalue>>F)&((1ull<<(PREC-F))-1ull)) : 0;
    ...
    template<unsigned P1, unsigned F1, unsigned P2, unsigned F2> L1TopoDataTypes<(P1+P2-1 > MAXBITS) ? MAXBITS : P1+P2-1, (P1+P2-1 > MAXBITS) ? (F2>F1 ? F2 - ((P1+P2-1) - MAXBITS) : F1 - ((P1+P2-1) - MAXBITS)) : ((F1 > F2) ? F1 : F2)> operator*(const L1TopoDataTypes<P1,F1> lhs, const L1TopoDataTypes<P2,F2> rhs){
       ...
       if((lhs.m_tvalue >> (P1-1ull)) & 1ull){
          lhsconvert = complement(lhsconvert,P1);
       }
       if((rhs.m_tvalue >> (P2-1ull)) & 1ull){ 
       ...
       if(!(((lhs.m_tvalue >> (P1-1ull)) & 1ull) ^ ((rhs.m_tvalue >> (P2-1ull)) & 1ull))){
            return result;
       } else 
            return -result;
    }
