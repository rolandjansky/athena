...
namespace TrigT1CTMonitoring {
	...
	m_configSvc("TrigConf::TrigConfigSvc/TrigConfigSvc", name)
  {
    ...
    declareProperty("N_slots_ctpin", N_slots_ctpin=3);
    declareProperty("N_connectors_ctpin", N_connectors_ctpin=4);
    declareProperty("N_connectors_fp", N_connectors_fp=3);
  }
  ...
  StatusCode DeriveSimulationInputs::execute() {
    ...
    for (int i = 0; i < N_cablebits; i++)
      for (int j = 0; j < N_slots_ctpin; j++)
				for (int k = 0; k < N_connectors_ctpin; k++)
					CTPINCables[i][j][k]=0;
		
		
    for (int i = 0; i < N_cablebits; i++) {
			for (int j = 0; j < N_connectors_fp; j++) {
				for(int k = 0; k<2; k++) {
					FPCables[i][j][k]=0;
				}
			}
    ...
    //std::bitset < 320 > currentPIT(BCs[theCTP_RDO->getL1AcceptBunchPosition()].getPIT());
    //std::bitset < 192 > currentFPI(BCs[theCTP_RDO->getL1AcceptBunchPosition()].getFPI());
...
//    }
//		
//		for (size_t bitNum = 0; bitNum < currentFPI.size(); ++bitNum) {
//      if (currentFPI.test(bitNum))
//				sc = fillFPIMap(bitNum);
//    }
		
		for (size_t tipNum = 0; tipNum < currentTIP.size(); ++tipNum) {
      if (currentTIP.test(tipNum)) {
				if (ctpVersionNumber<4) {
					sc = fillPitMap(tipNum, m_pitVector);
				}else{ //assume that LUT input 0..11 are PIT, 12..15 are FPI
					bool isPIT = false;
					bool isFPI = false;
					int LUTinputs = 16;
					unsigned int nLut = currentTIP.size()/LUTinputs;
					for (unsigned int lut = 0; lut<nLut; lut++) {
						if ( (tipNum >= (LUTinputs*lut) ) && tipNum<(LUTinputs*lut+12) ) isPIT = true;
						if ( (tipNum >= (LUTinputs*lut+12)) && tipNum<(LUTinputs*(lut+1)) ) isFPI = true;
					}
  ...
  StatusCode DeriveSimulationInputs::fillPitMap(int pitNum, std::vector<TrigConf::PIT*> m_pitVector) {
	...
	StatusCode DeriveSimulationInputs::fillFPIMap(int bitNum) {
  ...
  StatusCode DeriveSimulationInputs::fillStoreGate(unsigned int ctpVersNum) {
    StatusCode sc = StatusCode::SUCCESS;
		...
		if (ctpVersNum>3) {
			...
			for (int i = 0; i < N_cablebits; i++){ 
				for (int j = 0; j < N_connectors_fp; j++) {
					for (int k= 0; k < 2; k++) {
						ATH_MSG_DEBUG(" Cable = " << i << 
													" Slot = " << "CTPCORE front panel" << 
													" Connector = " << k << 
													" Cable bit = " << FPCables[i][j][k]);
					}
				}
			}
		}
    ...
    unsigned int fpCables0 = 0;
    unsigned int fpCables1 = 0;
    unsigned int fpCables2 = 0;
    ...
    for ( std::vector< TrigConf::TriggerThreshold* >::const_iterator it = m_configSvc->ctpConfig()->menu().thresholdVector().begin(); 
         it != m_configSvc->ctpConfig()->menu().thresholdVector().end(); ++it){
			...
			if ( ctpVersNum>3 && ((*it)->input()!="ctpin") ) { //front panel inputs
				
				std::string s = (*it)->cableConnector();
				std::string::size_type pos = s.find_last_of("N");
				char* dummy = &s[pos+1];
				int Connector = std::atoi(dummy);
				int clock = (*it)->clock();
				
				if ((Connector < 0) || (Connector > N_connectors_fp) || (clock>1) ) {
					ATH_MSG_DEBUG("Null connection: front panel connector " << Connector);
					continue;
				}
				...
				for (int cablebit = (*it)->cableStart(); cablebit <= (*it)->cableEnd(); cablebit++) {
					...
					cable |= FPCables[cablebit][Connector][clock] ? (1 << (N_cablebits * N_connectors_fp * clock + cablebit) ) : 0;
				}
				
				
				ATH_MSG_DEBUG("Dumping TriggerThreshold: " << (*it)->cableName() << " " << (*it)->type() << " front panel " << (*it)->cableConnector() << " " << (*it)->cableStart() << " " << (*it)->cableEnd());
				...
				if ((SlotNumber < 0) || (Connector < 0) || (SlotNumber > N_slots_ctpin) || (Connector > N_connectors_ctpin)) {
				...
				for (int cablebit = (*it)->cableStart(); cablebit <= (*it)->cableEnd(); cablebit++) {
					...
					cable |= CTPINCables[cablebit][SlotNumber][Connector] ? (1 << cablebit) : 0;
				}
			...
			//belof 4 are a hack but I don't think there is another way other than connecting threshold->pit
				...
				if (((*it)->cableCtpin() == "SLOT9")&&((*it)->cableConnector() == "CON1"))
					nimCables2 |= cable;
			}else if ((*it)->cableName()=="FP1")
				fpCables0 |= cable;
			else if ((*it)->cableName()=="FP2")
				fpCables1 |= cable;
			else if ((*it)->cableName()=="FP3")
				fpCables2 |= cable;
    ...
    if (ctpVersNum>3) {
       ...
       newFP->setCableWord0(0, fpCables0);
       newFP->setCableWord1(0, fpCables1);
       newFP->setCableWord2(0, fpCables2);
  ...
  StatusCode DeriveSimulationInputs::ReadInputMappingFromCool(){
    StatusCode sc = StatusCode::SUCCESS;
