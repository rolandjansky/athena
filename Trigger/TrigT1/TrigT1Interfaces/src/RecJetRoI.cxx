...
namespace LVL1 {
   ...
   RecJetRoI::RecJetRoI( unsigned int RoIWord,
      ...
      if (m_version > 1) this->constructRun2( caloThresholds );
      else               this->constructRun1( caloThresholds );
   ...
   /** The "construct" calls unpack the RoIWord and copy information about the passed thresholds
   ...
   void RecJetRoI::constructRun1( const std::vector< TriggerThreshold* >* caloThresholds ) {

      m_thresholdMask = m_roiWord&0xfff;

      std::string jetTriggerType = L1DataDef::jetType();
      if( this->isForwardJet() ) jetTriggerType = ( (this->eta() > 0) ?
                                                    L1DataDef::jfType() : L1DataDef::jbType() );
      ...
      for( vector< unsigned int >::const_iterator itp = passed_thresholds.begin();
	    ...
	    if (jtv) {
               m_triggerThresholdValue.insert(std::map<int, unsigned int>::value_type(*itp,jtv->thresholdValueCount()));
               m_windowSize.insert( std::map< int, unsigned int >::value_type( *itp, jtv->window() ) );
            }
   ...
   void RecJetRoI::constructRun2( const std::vector< TriggerThreshold* >* caloThresholds ) {
   ...
   unsigned int RecJetRoI::roiWord() const {
   ...
   unsigned int RecJetRoI::etLarge() const {
      if (m_version > 1) {
        ...
        float highest = 0;
        std::map< int, unsigned int >::const_iterator it = m_triggerThresholdValue.begin();
        for (; it != m_triggerThresholdValue.end(); ++it) if (it->second > highest) highest = it->second;
        return highest;
      }
   }
   ...
   unsigned int RecJetRoI::thresholdPattern() const {
   ...
   bool RecJetRoI::passedThreshold( int thresholdNumber ) const {
      ...
      std::map< int, unsigned int >::const_iterator it = m_triggerThresholdValue.find( thresholdNumber );
      if ( it != m_triggerThresholdValue.end() ) value = true;
   ...
   bool RecJetRoI::passedFwdThreshold( int thresholdNumber ) const {
      ...
      if( (m_version < 2) && this->isValidThreshold( thresholdNumber ) ) {
         return ( ( ( 1 << ( thresholdNumber + 7 ) ) & m_roiWord ) == true );
      }
      return false;
   ...
   CoordinateRange RecJetRoI::coord() const {
   ...
   std::vector< unsigned int >* RecJetRoI::thresholdsPassed() const {
      ...
      if (m_version == 1) {
        if (this->isForwardJet()) nThresh = TrigT1CaloDefs::numOfFwdJetThresholds;
        else                      nThresh = TrigT1CaloDefs::numOfJetThresholds;
      }
   ...
   unsigned int RecJetRoI::triggerThreshold( const unsigned int thresh ) const {
      ...
      map< int, unsigned int >::const_iterator it = m_triggerThresholdValue.find( thresh );
   ...
   bool LVL1::RecJetRoI::isForwardJet() const {
      ...
      if ( m_version <= 1 && ( ( 1 << 18 ) & m_roiWord ) ) return true;
      return false;
