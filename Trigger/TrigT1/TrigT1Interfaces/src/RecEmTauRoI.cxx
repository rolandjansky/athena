// $Id: RecEmTauRoI.cxx 613867 2014-08-28 15:42:10Z watsona $
/***************************************************************************
...
namespace LVL1 {
   ...
   RecEmTauRoI::RecEmTauRoI( unsigned int RoIWord,
      ...
      else if (m_type == TrigT1CaloDefs::EMRoIWordType || m_type == TrigT1CaloDefs::TauRoIWordType)
   ...
   /** The "construct" call unpacks the RoIWord and copies information about the passed thresholds
       ...
       rather than 0 in the calorimeter trigger simulation and Lvl1ConfigSvc. Hence the odd "-1"
       in a couple of places in this class */

   void RecEmTauRoI::constructRun1( const std::vector< TriggerThreshold* >* caloThresholds ) {
      ...
      const vector< unsigned int >& passed_thresholds = m_decoder->thresholdsPassed( m_roiWord );
      ...
      for( vector< unsigned int >::const_iterator itp = passed_thresholds.begin();
         ...
         std::map< int, TriggerThreshold* >::const_iterator thr = thrMap.find( *itp - 1 );
         if( thr != thrMap.end() ) {
            TriggerThresholdValue* ttv = thr->second->triggerThresholdValue( ieta,iphi );
            ClusterThresholdValue* ctv = dynamic_cast< ClusterThresholdValue* >( ttv );
	    if (ctv) {
               m_triggerThresholdValue.insert(std::map<int, unsigned int>::value_type(*itp,ctv->thresholdValueCount()));
               m_emIsolation.insert(std::map<int, unsigned int>::value_type(*itp,ctv->emIsolationCount()));
               m_hadIsolation.insert(std::map<int, unsigned int>::value_type(*itp,ctv->hadIsolationCount()));
               m_hadCoreIsolation.insert(std::map<int, unsigned int>::value_type(*itp,ctv->hadVetoCount()));
               TrigT1CaloDefs::ClusterAlgorithm type = ((ctv->type() == L1DataDef::emType()) ? TrigT1CaloDefs::EMAlg : TrigT1CaloDefs::TauAlg);
               m_thresholdType.insert(std::map<int, TrigT1CaloDefs::ClusterAlgorithm>::value_type(*itp,type));
            }
   ...
   /** Content of RoI word different in Run 2. To obtain thresholds passed need to simulate
    *  thresholding.
    *  Hence have an alternate constructor here */
   void RecEmTauRoI::constructRun2( const std::vector< TriggerThreshold* >* caloThresholds ) {
      ...
      for( std::vector< TriggerThreshold* >::const_iterator it = caloThresholds->begin();
	    ...
	    if (ctv) {
               ...
               if ( et() > etCut && isolationPassed) { 
                 ...
                 TrigT1CaloDefs::ClusterAlgorithm type = ((ctv->type() == L1DataDef::emType()) ? TrigT1CaloDefs::EMAlg : TrigT1CaloDefs::TauAlg);
                 m_thresholdType.insert(std::map<int, TrigT1CaloDefs::ClusterAlgorithm>::value_type(num,type));
   ...
   unsigned int RecEmTauRoI::roiWord() const {
   ...
   unsigned int RecEmTauRoI::et() const {
   ...
   unsigned int RecEmTauRoI::thresholdPattern() const {
   ...
   /** returns type of RoI */
   TrigT1CaloDefs::RoIType RecEmTauRoI::roiType() const {
   ...
   bool RecEmTauRoI::passedThreshold( int threshold_number ) const {
      ...
      if( this->isValidThreshold( threshold_number ) ) {
          return ( ( ( 1 << (threshold_number - 1) ) & m_thresholdMask ) == true );
      }
      return false;
   ...
   CoordinateRange RecEmTauRoI::coord() const {
   ...
   vector< unsigned int >* RecEmTauRoI::thresholdsPassed() const {
      ...
      if (m_type == TrigT1CaloDefs::EMRoIWordType) nThresh = TrigT1CaloDefs::numOfEMCMXThresholds;
      else if (m_type == TrigT1CaloDefs::TauRoIWordType) nThresh = TrigT1CaloDefs::numOfTauCMXThresholds;
            
      for (unsigned int iTh = 0; iTh <= nThresh; ++iTh) {
         std::map< int, unsigned int >::const_iterator it = m_triggerThresholdValue.find( iTh );
         if ( it != m_triggerThresholdValue.end() ) newVec->push_back(iTh);
      }
   ...
   unsigned int RecEmTauRoI::triggerThreshold( const unsigned int thresh ) const {
      ...
      std::map< int, unsigned int >::const_iterator it = m_triggerThresholdValue.find( thresh );
   ...
   /** returns the value of the isolation mask for the threshold passed. (Run 2 only) */
   unsigned int RecEmTauRoI::isolationMask( const unsigned int thresh ) const {

      if (m_type == TrigT1CaloDefs::CpRoIWordType) return 0;
   ...
   /** returns the value of the em isolation ring for the threshold passed. (Run 1 only) */
   ...
   /** returns the value of the hadronic layer isolation ring for the threshold passed. (Run 1 only) */
   ...
   /** returns the value of the hadronic layer core isolation for the threshold passed. (Run 1 only) */
   ...
   TrigT1CaloDefs::ClusterAlgorithm RecEmTauRoI::thresholdType( const unsigned int thresh ) const {

      map< int, TrigT1CaloDefs::ClusterAlgorithm >::const_iterator it = m_thresholdType.find( thresh );
