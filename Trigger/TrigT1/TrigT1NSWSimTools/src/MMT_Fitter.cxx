...
ROI MMT_Fitter::Get_ROI(float32fixed<2> M_x,float32fixed<2> M_u,float32fixed<2> M_v,const vector<Hit>&track) const{
  ...
  //if(debug) 
    cout<<"(b,A,B,my,mxu,mxv,mx)=("<<b.getFloat()<<","<<A.getFloat()<<","<<B.getFloat()<<","<<m_y.getFloat()<<","<<m_xu.getFloat()<<","<<m_xv.getFloat()<<","<<m_x.getValue()<<")\n";
...
//     exit(2);
  ...
  float32fixed<2> m_x=m_par->m_x_min+m_par->h_mx*xdex, m_y=m_par->m_y_min+m_par->h_my*ydex;
  //if(debug)
    cout<<"m_par->m_x_min+m_par->h_mx*xdex="<<m_par->m_x_min.getValue()<<"+"<<m_par->h_mx.getValue()<<"*"<<xdex<<"="<<m_x.getValue()<<", ";
  //if(debug)
    cout<<"m_par->m_y_min+m_par->h_my*ydex="<<m_par->m_y_min.getValue()<<"+"<<m_par->h_my.getValue()<<"*"<<ydex<<"="<<m_y.getValue()<<", ";
