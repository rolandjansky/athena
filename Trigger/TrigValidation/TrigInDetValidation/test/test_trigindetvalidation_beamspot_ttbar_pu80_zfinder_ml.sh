#!/bin/bash
# art-description: art job for beamspot_ttbar_pu80_zfinder_ml
...
export RTTJOBNAME=TrigInDetValidation_beamspot_ttbar_pu80_zfinder_ml
      ...
      case "$ArtProcess" in
            ...
            *)
                  ...
                  if [ $RUNATHENA -eq 1 ]; then
                      ARGS="$jobList;EventMax=1000;fastZFinder=True;mlExtensions=True;rec.doFloatingPointException.set_Value_and_Lock(False)"
...
for git in $jobList ; do 
    ...
    ARGS="$git;EventMax=1000;fastZFinder=True;mlExtensions=True;rec.doFloatingPointException.set_Value_and_Lock(False)"
...
TIDArun-art.sh data-all.root data-beamspot_ttbar_pu80_zfinder_ml-reference.root HLT_beamspot_allTE_trkfast_InDetTrigTrackingxAODCnv_BeamSpot_FTF -d HLTL2-test-plots  2>&1 | tee TIDArun_2.log
