...
JpsiValidation::JpsiValidation(const std::string & name, ISvcLocator * pSvcLocator)
  : FEXValidationAlgo(name, pSvcLocator, "Validation")
{
  // e5_medium
  m_L1EmClus = 3.*CLHEP::GeV;
  m_L1EmIsol = 999.*CLHEP::GeV;
  m_L1HadCore = 999.*CLHEP::GeV;
  m_L1HadIsol = 999.*CLHEP::GeV;
  
  m_L2Rcore_cut = 0.85;
  m_L2Rstrip_cut = 0.60;
  m_L2Et_cut = 4.*CLHEP::GeV;
  m_L2Ehad_cut = 2.*CLHEP::GeV;
  
  m_L2IDPt = 1.*CLHEP::GeV;
  m_L2IDEP = 999;
  m_L2IDDeta = 0.05;
  m_L2IDDphi = 0.1;
  
  m_EFCaloEt = 5.*CLHEP::GeV;
  m_EFCaloDeta = 999;
  m_EFCaloDphi = 999;
 
  m_EFTrackeCalibFactor = 1.0;
  m_EFTrackDeta = 999.;
  m_EFTrackDphi = 999.;
  m_EFTrackEtaCut = 1.37;
  m_EFTrackLowEoPHighEta = 0;
  m_EFTrackHighEoPHighEta = 999;
  m_EFTrackLowEoPLowEta = 0;
  m_EFTrackHighEoPLowEta = 999;
 
  m_lowEtCut = 0;
  m_maxEtaCut = 2.5;
}

JpsiValidation::~JpsiValidation()
{
}

StatusCode JpsiValidation::childInitialize()
{
  StatusCode sc;
  ...
  addDirectory(name()+"/JpsiReconstruction");
  addHistogram(new TH1F("TruthElectronsFound", "Number of Truth Electrons found per event ; Number of Electrons; Count", 10, 0, 10));
  addHistogram(new TH1F("TruthRecJpsiMass", "Reconstructed invariant J/\\Psi Mass from Truth Analysis ; m_{J/\\Psi} ; Count", 100, 40, 140));
  addHistogram(new TH1F("TruthJpsiMass", "Invariant J/\\Psi Mass from Truth Analysis ; m_{J/\\Psi} ; Count", 100, 40, 140));
  addHistogram(new TH1F("L1ElectronsFound", "Number of L1 Electrons found per event ; Number of Electrons; Count", 10, 0, 10));
  addHistogram(new TH1F("L1CaloJpsiMass", "Reconstructed invariant J/\\Psi Mass from L1 Calo Analysis ; m_{J/\\Psi} ; Count", 100, 40, 140));
  addHistogram(new TH1F("L2CaloElectronsFound", "Number of L2 Calo Electrons found per event ; Number of Electrons; Count", 10, 0, 10));
  addHistogram(new TH1F("L2CaloJpsiMass", "Reconstructed invariant J/\\psi Mass from L2 Calo Analysis ; m_{J/\\Psi} ; Count", 100, 40, 140));
  ...
  addHistogram(new TH1F("EFCaloScanJpsiMass", "Reconstructed invariant J/\\Psi Mass from EF Calo [L2 Scan] Analysis ; m_{J/\\Psi} ; Count", 100, 40, 140));
  ...
  addHistogram(new TH1F("EFCaloSiJpsiMass", "Reconstructed invariant J/\\Psi Mass from EF Calo [L2 SiTrack] Analysis ; m_{J/\\Psi} ; Count", 100, 40, 140));


  return StatusCode::SUCCESS;
}

StatusCode JpsiValidation::childExecute() {
  StatusCode sc;
  ...
  DataHandle<TrigEMClusterContainer> EMClusterContainer;
    ...
    if ( themcpartTES.key() == "TruthEvent" ) { // As long as it is in "TruthEvent"
      ...
      for (; Itr != ItrE; ++Itr) { // For each event
        ...
        for ( ; itrParticle != itrParticleE; ++itrParticle) { // For each particle in the event
          ...
          // Finds a Jpsi
          if ( (*itrParticle)->pdg_id() == 443) {
            cd(name()+"/JpsiReconstruction");
            hist1("TruthJpsiMass")->Fill((*itrParticle)->momentum().m()/1e3);
  ...
  if (pe_efcaloscan.size() == 2) {
    ...
    hist1("EFCaloScanJpsiMass")->Fill(pz_efcaloscan.m()/1e3);
  }
  ...
  if (pe_efcalosi.size() == 2) {
    ...
    hist1("EFCaloSiJpsiMass")->Fill(pz_efcalosi.m()/1e3);
  }
...
StatusCode JpsiValidation::childFinalize()
{
  cd(name()+"/Dump");
  FillTable();
