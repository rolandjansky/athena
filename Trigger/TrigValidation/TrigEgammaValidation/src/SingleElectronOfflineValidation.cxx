...
SingleElectronOfflineValidation::SingleElectronOfflineValidation(const std::string & name, ISvcLocator * pSvcLocator)
  : FEXValidationAlgo(name, pSvcLocator, "Validation")

{
}

SingleElectronOfflineValidation::~SingleElectronOfflineValidation()
{
}

StatusCode SingleElectronOfflineValidation::childInitialize()
{
  StatusCode sc;
  ...
  addHistogram(new TH1F("OffL1Et", "Level 1 Offline E_{T} for single electron ; E_{T} [GeV] ; Count", 80, 0., 80.));
  addHistogram(new TH1F("OffL1Eta", "Level 1 Offline \\eta for single electron ; \\eta ; Count", 128, -3.2, 3.2));
  addHistogram(new TH1F("OffL1Phi", "Level 1 Offline \\phi for single electron ; \\phi [rad] ; Count", 128, -3.2, 3.2));
  
  addHistogram(new TH1F("OffL2Et", "Level 2 Offline E_{T} for single electron ; E_{T} [GeV] ; Count", 80, 0., 80.));
  addHistogram(new TH1F("OffL2Eta", "Level 2 Offline \\eta for single electron ; \\eta ; Count", 128, -3.2, 3.2));
  addHistogram(new TH1F("OffL2Phi", "Level 2 Offline \\phi for single electron ; \\phi [rad] ; Count", 128, -3.2, 3.2));
  
  // For ID Efficiencies
  addHistogram(new TH1F("OffL2IDScanEt", "p_{T} after Level 2 ID (ID Scan Match) for single electron ; p_{T} [GeV] ; Count", 80, 0., 80.));
  addHistogram(new TH1F("OffL2IDScanPhi", "\\phi after Level 2 ID (ID Scan Match) for single electron ; \\phi [rad] ; Count", 128, -3.2, 3.2));
  addHistogram(new TH1F("OffL2IDScanEta", "\\eta after Level 2 ID (ID Scan Match) for single electron ; \\eta ; Count", 128, -3.2, 3.2));
  
  addHistogram(new TH1F("OffL2IDSiEt", "p_{T} after Level 2 ID (SiTrack Match) for single electron ; p_{T} [GeV] ; Count", 80, 0., 80.));
  addHistogram(new TH1F("OffL2IDSiPhi", "\\phi after Level 2 ID (SiTrack Match) for single electron ; \\phi [rad] ; Count", 128, -3.2, 3.2));
  addHistogram(new TH1F("OffL2IDSiEta", "\\eta after Level 2 ID (SiTrack Match) for single electron ; \\eta ; Count", 128, -3.2, 3.2));

  // EF Calo
  addHistogram(new TH1F("OffEFScanCaloEt", "Offline E_{T} after EF Calo (and L2 ID Scan Match) for single e^{-} ; E_{T} [GeV] ; Count", 80, 0, 80.));
  addHistogram(new TH1F("OffEFScanCaloPhi", "Offline \\phi after EF Calo (and L2 ID Scan Match) for single e^{-} ; \\phi [rad] ; Count", 128, -3.2, 3.2));
  addHistogram(new TH1F("OffEFScanCaloEta", "Offline \\eta after EF Calo (and L2 ID Scan Match) for single e^{-} ; \\eta ; Count", 128, -3.2, 3.2));

  addHistogram(new TH1F("OffEFSiCaloEt", "Offline E_{T} after EF Calo (and L2 ID SiTrack Match) for single e^{-} ; E_{T} [GeV] ; Count", 80, 0, 80.));
  addHistogram(new TH1F("OffEFSiCaloPhi", "Offline \\phi after EF Calo (and L2 ID SiTrack Match) for single e^{-} ; \\phi [rad] ; Count", 128, -3.2, 3.2));
  addHistogram(new TH1F("OffEFSiCaloEta", "Offline \\eta after EF Calo (and L2 ID SiTrack Match) for single e^{-} ; \\eta ; Count", 128, -3.2, 3.2));

  // EF ID
  addHistogram(new TH1F("OffEFScanTrackEt", "Offline E_{T} after EF Track (and L2 ID Scan Match) for single e^{-} ; E_{T} [GeV] ; Count", 80, 0, 80.));
  addHistogram(new TH1F("OffEFScanTrackPhi", "Offline \\phi after EF Track (and L2 ID Scan Match) for single e^{-} ; \\phi [rad] ; Count", 128, -3.2, 3.2));
  addHistogram(new TH1F("OffEFScanTrackEta", "Offline \\eta after EF Track (and L2 ID Scan Match) for single e^{-} ; \\eta ; Count", 128, -3.2, 3.2));

  addHistogram(new TH1F("OffEFSiTrackEt", "Offline E_{T} after EF Track (and L2 ID SiTrack Match) for single e^{-} ; E_{T} [GeV] ; Count", 80, 0, 80.));
  addHistogram(new TH1F("OffEFSiTrackPhi", "Offline \\phi after EF Track (and L2 ID SiTrack Match) for single e^{-} ; \\phi [rad] ; Count", 128, -3.2, 3.2));
  addHistogram(new TH1F("OffEFSiTrackEta", "Offline \\eta after EF Track (and L2 ID SiTrack Match) for single e^{-} ; \\eta ; Count", 128, -3.2, 3.2));
...
StatusCode SingleElectronOfflineValidation::childExecute()
{
  StatusCode sc;
  ...
  if (sc.isFailure()) {
    (*m_log) << MSG::WARNING
	     << "No egammaContainer found in TDS. Aborting!!!"
	     << endreq;
    return StatusCode::SUCCESS; // Don't stop proccessing events
  }

  (*m_log) << MSG::DEBUG << "start processing" << endreq;
  ...
  for ( ; egCbegin != egCend; ++egCbegin) { // For each event collection
	...
	if ( ((*Itr)->isem()) == 0 ) { // If it is an electron/positron/gamma
		...
		if ( (matchedL1->getEMClus() > m_L1EmClus) &&
		  ...
		  if (doL2) {
		    ...
		    if (matchedL2) {
		      ...
		      // Fill FEX histograms for electrons
                      cd(name()+"/Dump");
			...
			if (doL2ID) {
			  ...
			  (*m_log) << MSG::DEBUG << "runTrack() ok" << endreq;
			  if (runTrack(lvl2t_first, lvl2t_last, matchedL1->eta(), matchedL2->phi(), Pt, Eta, Phi, A0, Z0, TrigInDetTrack::IDSCANID)) {
                            cd(name()+"/Dump");
			  ...
			  if (runTrack(lvl2t_first, lvl2t_last, matchedL1->eta(), matchedL2->phi(), Pt, Eta, Phi, A0, Z0, TrigInDetTrack::SITRACKID)) {
                            cd(name()+"/Dump");
  ...
  (*m_log) << MSG::DEBUG << "end of SingleElectronValidation::execute()" << endreq;
  return StatusCode::SUCCESS;
}

StatusCode SingleElectronOfflineValidation::childFinalize() {
  cd(name()+"/Dump");
  FillTable();
