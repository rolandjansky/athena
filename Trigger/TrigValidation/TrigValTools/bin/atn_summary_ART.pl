...
<script type=\"text/javascript\">
    ...
    function drawCrosshairs9000() {
                ...
                <?php
                        ...
                        \$nightlies = glob(dirname(__FILE__) . '/../../*');
                        \$nightlies = array_values(\$nightlies);
                        \$content = file_get_contents(dirname(__FILE__).'/test_HLT_physics_menu_ART_and_ROSsim_build/HLT_physicsV8_ROSsim_9000/ATLASros2rob2018.py');
                        //echo \$content;
                        preg_match_all('/ROS-.[A-Z]+-.[A-Z]+-.[0-9]+/i',\$content,\$ROSes);
                        \$ROSes = array_values(\$ROSes);
                        foreach(\$ROSes as \$ros){
                        ...
                        echo \"                data.addRows([\";
                        foreach(\$nightlies as \$nightly){
                                \$content = file_get_contents(\$nightly.'/TrigP1Test/test_HLT_physics_menu_ART_and_ROSsim_build/HLT_physicsV8_ROSsim_9000/ROStest.reference.new');
                                //echo \$content.'\\n';                           
                                echo \"                ['\".basename(\$nightly).\"',\";
                                foreach(\$ROSes as \$ros){
                                        \$ros = array_values(\$ros);
