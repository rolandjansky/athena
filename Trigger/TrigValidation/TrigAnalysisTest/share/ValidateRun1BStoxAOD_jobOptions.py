...
printfunc ("the path for AOD from yesterday's ATN is: ", s_nightly_aod_yesterday)
...
printfunc ("check if the ATN output file exists...")
s_file = "";

import os
if not (os.path.isfile(s_nightly_aod_yesterday)):
    printfunc ("WARNING: it seems that the yesterday's ATN TrigHLT_Run1BStoxAOD in AtlasAnalysis,20.7.X-VAL was unsuccessful, going to use input file from the eos space")
    s_file = s_ref_file
else:
...
if int(rc_release) == 23:
    printfunc ("This is ROOTCORE_RELEASE_SERIES = 23, will use the corresponding input file")
    s_file = s_ref_file

printfunc (s_file," will be used as the input for this test")
