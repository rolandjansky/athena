...
echo $(date "+%FT%H:%M %Z")"     Execute TrigAnalysisTest post processing"
...
if [ -f AOD.pool.root ]; then
  ...
  echo "No AOD.pool.root to check with checkFileTrigSize_RTT"
fi

echo $(date "+%FT%H:%M %Z")"     Done executing TrigAnalysisTest post processing"
