...
HLT::ErrorCode TrigJetSplitterAllTE::hltExecute(std::vector<std::vector<HLT::TriggerElement*> >& inputTEs, unsigned int output) {
  ...
  if (inputTEs.size() == 1) {
    msg() << MSG::WARNING << "Only received one TE, expected two." << endreq;
    afterExecMonitors().ignore();
    return HLT::ErrorCode(HLT::Action::ABORT_CHAIN, HLT::Reason::MISSING_FEATURE);
  }
  ...
  if(msgLvl() <= MSG::DEBUG)
    msg() << MSG::DEBUG << " inputTEs.size() " << inputTEs.size() << " jetTE.size() " << jetTE.size() << " vtxTE.size() " << vtxTE.size() << endreq;
