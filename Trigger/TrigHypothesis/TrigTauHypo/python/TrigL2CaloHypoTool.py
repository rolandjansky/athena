...
def decodeThreshold( threshold ):
    ...
    if threshold[0].isdigit(): # is if the from NeX, return as list [X,X,X...N times...]
        ...
        assert threshold[1] == 't', "Two digit multiplicity not supported"
        return [ threshold[2:] ] * int( threshold[0] )
    print "count ('t'): ", threshold.count('t')
    if threshold.count('t') > 1: # is of the form eXeYeZ, return as [X, Y, Z]
        print 'here'
        return threshold.strip('tau').split('tau')
        print 'threshold is '
    if threshold.count('tau') == 1:
        return threshold.strip('tau').split('tau')
        print 'threshold is '
