...
HLT::ErrorCode EFTauTopoFex :: accept_eltau_Inputs(HLT::TEConstVec& inputTE, bool& pass)
{
  ...
  if ( getFeatures(te1, vectorElectronContainers) != HLT::OK || getFeatures(te2, vectorTauContainers) != HLT::OK )
    {
      msg() << MSG::WARNING << "Failed to get xAOD::ElectronContainer and xAOD::TauJetContainer collections" << endmsg;
      return HLT::MISSING_FEATURE;
    }
  else { msg() << MSG::DEBUG  << "xAOD::TauJetContainer collections successfully retrieved" << endmsg; }
  
  if (vectorElectronContainers.size() < 1 || vectorTauContainers.size() < 1) {
    msg() << MSG::DEBUG << " empty xAOD::ElectronContainer or xAOD::TauJetContainer collections" << endmsg;
    return HLT::OK;
  }
...
HLT::ErrorCode EFTauTopoFex :: accept_mutau_Inputs(HLT::TEConstVec& inputTE, bool& pass)
{
  ...
  if ( getFeatures(te1, vectorMuonContainers) != HLT::OK || getFeatures(te2, vectorTauContainers) != HLT::OK )
    {
      msg() << MSG::WARNING << "Failed to get xAOD::MuonContainer and xAOD::TauJetContainer collections" << endmsg;
      return HLT::MISSING_FEATURE;
    }
  else { msg() << MSG::DEBUG  << "xAOD::TauJetContainer collections successfully retrieved" << endmsg; }
  
  if (vectorMuonContainers.size() < 1 || vectorTauContainers.size() < 1) {
    msg() << MSG::DEBUG << " empty xAOD::MuonContainer or xAOD::TauJetContainer collections" << endmsg;
    return HLT::OK;
  }
