...
HLT::ErrorCode EFPhotonTauFex::acceptInputs(HLT::TEConstVec& inputTE, bool& pass )
{
  ...
  if (status1 != HLT::OK || status2 != HLT::OK) {
    ATH_MSG_WARNING( "Failed to get xAOD::PhotonContainer and xAOD::TauJetContainer collections" );
    return HLT::MISSING_FEATURE;
  } else {
    ATH_MSG_DEBUG( "xAOD::PhotonContainer and xAOD::TauJetContainer collections successfully retrieved" );
  }

  if (vectorEgammaContainers.size() < 1 || vectorTauContainers.size() < 1) {
    ATH_MSG_WARNING( "Empty xAOD::PhotonContainer or xAOD::TauJetContainer from the trigger elements" );
    return HLT::OK;
  }  
  ...
  for ( photon = photonContainer1->begin(); photon != photonContainer1->end(); ++photon ) {
    ...
    if((*photon)==0){ 
      ATH_MSG_WARNING( "Null pointer in egammaContainer. Skipping." );
      continue;
    }

    if(!bits->isPassing((*photon),photonContainer1)){
      ATH_MSG_WARNING( "Photon found not passing Hypo object" );
      continue;
    }

    const xAOD::CaloCluster* clus = (*photon)->caloCluster();
    if(!clus) { 
      ATH_MSG_WARNING( "REGTEST no cluster pointer in egamma object" );
      continue;
    }
    ...
    for ( tau = TauJetContainer1->begin(); tau != TauJetContainer1->end(); ++tau ) {
      ...
      if((*tau)->nTracks()==0){
	ATH_MSG_DEBUG( "0-prong tau skipped" );
	continue;
      }			
	...
	try 
	  ...
	  {
	    ATH_MSG_WARNING( "REGTEST: EFPhotonTauFex, failed to get tau track link!" );
	  } 
	if(trk) {
      ...
      ATH_MSG_DEBUG( "photon+tau mass " << mass );
