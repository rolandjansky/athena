...
JVTCondition::JVTCondition(double workingPoint):
  m_workingPoint(workingPoint) {
}
  

bool JVTCondition::isSatisfied(const HypoJetVector& ips, const std::unique_ptr<ITrigJetHypoInfoCollector>& collector) const{

  if(ips.size() != 1){
    std::stringstream ss;
    ss << "Dijet::isSatisfied must see exactly 1 particle, but received "
       << ips.size()
       << '\n';
...
std::string JVTCondition::toString() const {

  std::stringstream ss;
  const void* address = static_cast<const void*>(this);
  ss << "JVTCondition: (" << address << ") Capacity: " << s_capacity
