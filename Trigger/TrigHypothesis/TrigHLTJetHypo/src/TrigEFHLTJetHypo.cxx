...
/////////////////////////////////////////////////////////////////////
//
TrigEFHLTJetHypo::TrigEFHLTJetHypo(const std::string& name, ISvcLocator* pSvcLocator):
...
/////////////////////////////////////////////////////////////////////
//
TrigEFHLTJetHypo::~TrigEFHLTJetHypo()
{  }
...
// ----------------------------------------------------------------------
HLT::ErrorCode TrigEFHLTJetHypo::hltInitialize()
  // ----------------------------------------------------------------------
{
  ...
  if (m_timersvc) {
    TrigTimer* tmp = m_timersvc->addItem("TrigEFHLTJetHypo.TrigEFHLTJetHypoTot");
  ...
  if ( m_EtThresholdsInput.empty() ) { 
    ...
    if ( m_EtThresholds.size()!=m_multiplicitycut ) {
      ...
      ATH_MSG_ERROR(name()
                    << ": mismatch between number of jets required "
                    << "and number of thresholds: "
                    << m_multiplicitycut 
    ...
    if  ( !sorted ) { 
      ATH_MSG_WARNING("sorting thresholds into reverse order");
...
// ----------------------------------------------------------------------
HLT::ErrorCode TrigEFHLTJetHypo::hltFinalize(){
  // ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
HLT::ErrorCode TrigEFHLTJetHypo::hltExecute(const HLT::TriggerElement* outputTE,
                                         bool& pass) {
...
bool TrigEFHLTJetHypo::getJetAttribute(const std::string& label,
                               float& val,
                               const xAOD::Jet* j){
...
bool TrigEFHLTJetHypo::doBasicCleaning(const xAOD::Jet* aJet, bool& isClean){
