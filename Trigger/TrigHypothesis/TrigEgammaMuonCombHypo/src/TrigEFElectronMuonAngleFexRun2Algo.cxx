...
HLT::ErrorCode TrigEFElectronMuonAngleFexRun2Algo::hltExecute(HLT::TEConstVec& inputTE, HLT::TriggerElement* outputTE){
  ...
  // sanity checks
  if ( debug )  msg() << MSG::DEBUG << "Running TrigEFElectronMuonAngleFexRun2Algo" << endreq;
  ...
  if ( getFeatures(te1, vectorEgammaContainers) != HLT::OK )
    { 
	  ...
	  if ( msgLvl() <= MSG::WARNING) 
	    msg() << MSG::WARNING << "Failed to get xAOD::ElectronContainer collection" << endreq;	  
	  return HLT::MISSING_FEATURE;
	}
      else{
	if ( debug )
	  msg() << MSG::DEBUG  << "xAOD::ElectronContainer collection successfully retrieved" << endreq; 	
      } 
  ...
  if (vectorEgammaContainers.size() < 1) {
    msg() << MSG::DEBUG << " empty xAOD::ElectronContainer from the trigger element" << endreq;
    return HLT::OK;
  }  
  ...
  if(msgLvl() <= MSG::DEBUG) {
	msg() << MSG::DEBUG << "Size of electron container: " << electronContainer1->size() << " size of muon container " << MuEFTracksEF->size() << endreq; 
   }
  ...
  for ( electron1 = electronContainer1->begin(); electron1 != electronContainer1->end(); ++electron1 ) {
	...
	continue;
      ...
      if(msgLvl() <= MSG::DEBUG) {
	mu_count++;
	msg() << MSG::DEBUG << "Processing electron no. " << el_count << " and muon no. " << mu_count << endreq; 
      }
  ...
  if (hltStatus != HLT::OK ){
    ...
    if(debug) msg() << MSG::DEBUG << "successfully recorded EF_ElectronMuonTopoFEX" << endreq;
  }
