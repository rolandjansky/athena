...
TrigEFJetDphiHypo::TrigEFJetDphiHypo(const std::string& name, ISvcLocator* pSvcLocator):
  ...
  declareProperty("Etcut1",       m_EtCut1       = 30*CLHEP::GeV); // Default: 30 GeV
  declareProperty("Etcut2",       m_EtCut2       = 30*CLHEP::GeV); // Default: 30 GeV
...
HLT::ErrorCode TrigEFJetDphiHypo::hltExecute(const HLT::TriggerElement* outputTE, bool& pass) {
  ...
  if( theJets.size() == 0){
    ...
    msg()<< MSG::WARNING << " Size of JetCollection is not empty but nJet < 2 - Cannot compute dPhi" << endreq;
    m_errors++;
    if (m_acceptAll) m_cutCounter = 1;
    //    if (m_timersvc) m_timers[0]->stop();
    return HLT::OK;
...
//   double passedDphiCut_jet2Phi = -999.;  
  ...
  for(auto firstJet = theJets.begin(); firstJet!=theJets.end(); firstJet++) {
	...
	if(msgLvl() <= MSG::DEBUG)msg() << MSG::DEBUG << "Jet1 - Et: " << aJet->et() << " Eta: " << aJet->eta() << " PASSED Et cut: " << m_EtCut1 << " |Eta| cut: " << m_etaMaxCut << endreq;
	...
	if(bJet->et() > m_EtCut2 && std::fabs(bJet->eta())>=m_etaMinCut && std::fabs(bJet->eta())<m_etaMaxCut) {
	  ...
	  if(msgLvl() <= MSG::DEBUG)msg() << MSG::DEBUG << "Jet2 - Et: " << bJet->et() << " Eta: " << bJet->eta() << " PASSED Et cut: " << m_EtCut2 << " |Eta| cut: " << m_etaMaxCut << endreq;
	  ...
	  if( dphi > m_minDphiCut && dphi < (M_PI - m_DphiCut) ){	  
	    ...
	    if(msgLvl() <= MSG::DEBUG) msg() << MSG::DEBUG << "Dphi: " << dphi << " PASSED Dphi cut of " << m_minDphiCut << " < dphi < Pi - " << m_DphiCut << " = " << (M_PI - m_DphiCut) << endreq;
	    ...
	    m_passedDphiCut_et1.push_back(jet1Et/CLHEP::GeV);
	    m_passedDphiCut_eta1.push_back(jet1Eta);
	    m_passedDphiCut_phi1.push_back(jet1Phi);
	    
	    m_passedDphiCut_et2.push_back(jet2Et/CLHEP::GeV);
	    ...
	    if(msgLvl() <= MSG::DEBUG) msg() << MSG::DEBUG << "Dphi: " << dphi << " FAILED Dphi cut of Pi - " << m_DphiCut << " = " << (M_PI - m_DphiCut) << endreq;
	  }
...
//   m_passedDphiCut_phi2 = passedDphiCut_jet2Phi;  
  ...
  if( (passedJetCuts_minDphi >= m_minDphiCut && passedJetCuts_minDphi < (M_PI - m_DphiCut) ) || m_acceptAll) {
    ...
    if(msgLvl() <= MSG::DEBUG) msg() << MSG::DEBUG << ">>>>> TRIGGER PASSED <<<<<       Dphi: " << passedJetCuts_minDphi << " PASSED Dphi cut of " << m_minDphiCut << " < Dphi < Pi - " << m_DphiCut << " = " << (M_PI - m_DphiCut) << endreq;
