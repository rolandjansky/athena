...
TrigEFJetMctHypo::TrigEFJetMctHypo(const std::string& name, ISvcLocator* pSvcLocator):
  ...
  declareProperty("MctCut",   m_MctCut = 150*CLHEP::GeV); // Default: 150 GeV
  declareProperty("Etcut1",   m_EtCut1 = 30*CLHEP::GeV); // Default: NO CUT
  declareProperty("Etcut2",   m_EtCut2 = 30*CLHEP::GeV); // Default: NO CUT
  ...
  // ----------------------------------------------------------------------
{
  ...
  if (msgLvl() <= MSG::DEBUG) {
    ...
    msg() << MSG::DEBUG	  << ", ptHardJetCut: " << m_EtCut1 << ", ptSoftJetCut: " << m_EtCut2 << endreq;
  }
  
  if(m_multiplicitycut>1){
    if (msgLvl()<=MSG::WARNING) msg() << MSG::WARNING << "UNKNOWN CONFIGURATION: Mulitplicity cut set to " <<  m_multiplicitycut << endreq; 
...
HLT::ErrorCode TrigEFJetMctHypo::hltExecute(const HLT::TriggerElement* outputTE, bool& pass) {
  ...
  if( theJets.size() == 0){
    ...
    msg()<< MSG::WARNING << " Size of JetCollection is not empty but nJet < 2 - Cannot compute MCT" << endreq;
    m_errors++;
    if (m_acceptAll) m_cutCounter = 1;
    //    if (m_timersvc) m_timers[0]->stop();
    return HLT::OK;
  ...
  for (const Jet* testJet : theJets) {
    if(msgLvl() <= MSG::DEBUG){
      msg() << MSG::DEBUG << " Jet "<< (nJet+1) << " Et: "<< testJet->et() << " Eta: "<< testJet->eta() << endreq;
    }
    nJet++;
  }
  ...
  for(auto firstJet = theJets.begin(); firstJet!=theJets.end(); firstJet++) {
    ...
    for(auto secondJet = secondJetBegin ; secondJet!=theJets.end(); secondJet++) {
      ...
      //Monitoring
      m_et1.push_back(jet1Et/CLHEP::GeV);
      m_eta1.push_back(jet1Eta);
      m_phi1.push_back(jet1Phi);

      m_et2.push_back(jet2Et/CLHEP::GeV);
	...
	if(msgLvl() <= MSG::DEBUG)msg() << MSG::DEBUG << "Jet1 - Et: " << jet1Et << " Eta: " << jet1Eta << " PASSED Et cut: " << m_EtCut1 << " |Eta| cut: " << m_etaMaxCut << endreq;
	

	//Jet2 Cuts
	if(jet2Et > m_EtCut2 && std::fabs(jet2Eta)>=m_etaMinCut && std::fabs(jet2Eta)<m_etaMaxCut){
	  
	  if(msgLvl() <= MSG::DEBUG)msg() << MSG::DEBUG << "Jet2 - Et: " << jet2Et << " Eta: " << jet2Eta << " PASSED Et cut: " << m_EtCut2 << " |Eta| cut: " << m_etaMaxCut << endreq;
	     
	  double mct = -999.;
	  double dphi = -999.;

	  //Monitoring	     
	  m_passedJetCuts_et1.push_back(jet1Et/CLHEP::GeV);
	  m_passedJetCuts_eta1.push_back(jet1Eta);
	  m_passedJetCuts_phi1.push_back(jet1Phi);

	  m_passedJetCuts_et2.push_back(jet2Et/CLHEP::GeV);
	  ...
	  if(msgLvl() <= MSG::DEBUG) msg() << MSG::DEBUG << 
	    ...
	    " Jet2 Et: " << bJet->et() << " Eta: " << bJet->eta() << endreq;
	  ...
	  if(msgLvl() <= MSG::DEBUG) {
	    msg() << MSG::DEBUG << "Phi1: " << jet1Phi 
		  << " Phi2: " << jet2Phi << " Dphi: " << dphi << endreq;
	  }
	  if (fabs(dphi) > M_PI ) dphi = 2*M_PI - fabs(dphi);
	  if(msgLvl() <= MSG::DEBUG) msg() << MSG::DEBUG << "After correction (to get equivalent angle below PI) Dphi: " << dphi << endreq;
	  dphi = fabs(dphi);
	  if(msgLvl() <= MSG::DEBUG) msg() << MSG::DEBUG << "Absolute value of Dphi: " << dphi << endreq;


	  //MCT calculation    
	  mct = sqrt(2.*jet1Et*jet2Et*(1+cos(dphi)));
	  if(msgLvl() <= MSG::DEBUG) msg() << MSG::DEBUG << "Value of Mct: " << mct << endreq;
	  
	  //Monitoring
	  m_passedJetCuts_dphi.push_back(dphi);
	  m_passedJetCuts_mct.push_back(mct/CLHEP::GeV);
	  ...
	  if( mct > m_MctCut){	  
	    ...
	    if(msgLvl() <= MSG::DEBUG) msg() << MSG::DEBUG << "Mct: " << mct << " PASSED Mct cut of " << m_MctCut << endreq;

	    m_cutCounter=3;

	    passedMctCut_maxmct = maxmct;

	    m_passedMctCut_mct.push_back(mct/CLHEP::GeV);
	    m_passedMctCut_dphi.push_back(dphi);
	    
	    m_passedMctCut_et1.push_back(jet1Et/CLHEP::GeV);
	    m_passedMctCut_eta1.push_back(jet1Eta);
	    m_passedMctCut_phi1.push_back(jet1Phi);
	    
	    m_passedMctCut_et2.push_back(jet2Et/CLHEP::GeV);
	    ...
	    if(msgLvl() <= MSG::DEBUG) msg() << MSG::DEBUG << "Mct: " << mct << " FAILED Mct cut of " << m_MctCut << endreq;
	  }
	  ...
	  if(msgLvl() <= MSG::DEBUG){
	    msg() << MSG::DEBUG << "Jet2 - Et: " << bJet->et() << " Eta: " << bJet->eta() << " FAILED Et cut: " << m_EtCut2 << " |Eta| cut: " << m_etaMaxCut << endreq;
	    //innerLoopBreak=true;
	    break;
	  }
	}
	...
	if(msgLvl() <= MSG::DEBUG){
	  msg() << MSG::DEBUG << "Jet1 - Et: " << aJet->et() << " Eta: " << aJet->eta() << " FAILED Et cut: " << m_EtCut1 << " |Eta| cut: " << m_etaMaxCut << endreq;
	}
	//break both loops
	innerLoopBreak=true;
	break;
  ...
  if( m_multiplicitycut==1 && multiplicity > m_multiplicitycut) {
    if (msgLvl()<=MSG::WARNING) msg() << MSG::WARNING << "There are " <<  multiplicity << " combinations of jets being considered, LEADINGMCT should only consider "<< m_multiplicitycut << endreq;
    return HLT::ErrorCode(HLT::Action::ABORT_CHAIN,  HLT::Reason::MISSING_FEATURE);
  ...
  m_passedJetCuts_maxmct = maxmct/CLHEP::GeV;  
//   m_mct   = mct; 
//   m_dphi  = dphi;  
//   m_et1  = jet1Et;  
//   m_et2  = jet2Et;  
...
//   m_phi2 = jet2Phi;  
  ...
  m_passedMctCut_maxmct = passedMctCut_maxmct/CLHEP::GeV;  
...
//   m_passedMctCut_phi2 = passedMctCut_jet2Phi;  
  ...
  if( maxmct > m_MctCut || m_acceptAll) {
    ...
    if(msgLvl() <= MSG::DEBUG) msg() << MSG::DEBUG << ">>>>> TRIGGER PASSED <<<<<       Mct: " << maxmct << " PASSED Mct cut of " << m_MctCut << endreq;
  ...
  if(msgLvl() <= MSG::DEBUG) {
    ...
    msg() << MSG::DEBUG << " with leading Jet of et: " << (*theJets.begin())->et() << " cut: " << m_EtCut1 << (pass ? " accepted ": " rejected ") << endreq;
    
    //msg() << MSG::DEBUG << "TrigPassBits: "<<bits.size()<<"jets: "<<theJets.size()<<endreq;
