...
Neural::Neural(const std::vector<unsigned> &n, const std::vector<float> &w, const std::vector<float> &b)
  : nodes(n),
    weight(w),
    bias(b),
    wM(0),
...
{
    if ( !n.size() )
        throw BAD_BIAS_SIZE; //Nothing to do
    //Verifying weight vector size
    unsigned wSize=0;
    for (unsigned k=0; k<n.size()-1; ++k)
        wSize+=n[k]*n[k+1];
    try{
