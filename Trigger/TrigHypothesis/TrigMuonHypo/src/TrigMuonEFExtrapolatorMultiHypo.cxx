...
TrigMuonEFExtrapolatorMultiHypo::TrigMuonEFExtrapolatorMultiHypo(const std::string & name, ISvcLocator* pSvcLocator):
...
HLT::ErrorCode TrigMuonEFExtrapolatorMultiHypo::hltExecute(const HLT::TriggerElement* outputTE, bool& pass){
  ...
  for (unsigned int j=0; j< muonContainer->size(); ++j ) {
	...
	const xAOD::TrackParticle *tr =  muon->trackParticle(xAOD::Muon::TrackParticleType::MuonSpectrometerTrackParticle);  // get the TrackContainer
	...
	if (!tr) {
	  ...
	  if (debug) msg() << MSG::DEBUG
			   << "Retrieved MuonSpectrometerTrackParticle with abs pt "
...
int TrigMuonEFExtrapolatorMultiHypo::bitsum(unsigned long il) {
  int ret = 0;
  while(il) { 
    ret += il&1;
    il = il >> 1;
