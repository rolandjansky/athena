...
TrigMuonEFTrackBuilderMultiHypo::TrigMuonEFTrackBuilderMultiHypo(const std::string & name, ISvcLocator* pSvcLocator):
...
TrigMuonEFTrackBuilderMultiHypo::~TrigMuonEFTrackBuilderMultiHypo(){
}
...
HLT::ErrorCode TrigMuonEFTrackBuilderMultiHypo::hltExecute(const HLT::TriggerElement* outputTE, bool& pass){
  ...
  for (unsigned int j=0; j<muonContainer->size(); j++ ) {
	...
	if (!tr) {
	  ...
	  if (debug) msg() << MSG::DEBUG
			   << "Retrieved MuonSpectrometerTrackParticle track with abs pt "
...
int TrigMuonEFTrackBuilderMultiHypo::bitsum(unsigned long il) {
  int ret = 0;
  while(il) { 
    ret += il&1;
    il = il >> 1;
