...
//////////////// end of initialization of the monitoring variables /////////////
  ...
  if(m_acceptAll){
    ...
    //////////////////////////// muon candidates ///////////////////////////////
    const CombinedMuonFeature* pCombinedMuonFeature1 = NULL;
    const CombinedMuonFeature* pCombinedMuonFeature2 = NULL;
    ...
    if(msgLvl() <= MSG::DEBUG) {
      msg() << MSG::DEBUG << "Muon candidate1: track with "
                          << "pt=" << pCombinedMuonFeature1->IDTrack()->param()->pT()
                          << ", eta=" << pCombinedMuonFeature1->IDTrack()->param()->eta()
                          << ", phi=" << pCombinedMuonFeature1->IDTrack()->param()->phi0()
                          << ", Z0=" << pCombinedMuonFeature1->IDTrack()->param()->z0()
                          << ", a0=" << pCombinedMuonFeature1->IDTrack()->param()->a0() 
                          << endreq;
      msg() << MSG::DEBUG << "Muon candidate2: track with "
                          << "pt=" << pCombinedMuonFeature2->IDTrack()->param()->pT()
                          << ", eta=" << pCombinedMuonFeature2->IDTrack()->param()->eta()
                          << ", phi=" << pCombinedMuonFeature2->IDTrack()->param()->phi0()
                          << ", Z0=" << pCombinedMuonFeature2->IDTrack()->param()->z0()
                          << ", a0=" << pCombinedMuonFeature2->IDTrack()->param()->a0() 
                          << endreq;
    }
    muonCandidates.push_back(pCombinedMuonFeature1);
    muonCandidates.push_back(pCombinedMuonFeature2);
    // monitoring muons
    mon_muon1Eta.push_back( pCombinedMuonFeature1->IDTrack()->param()->eta() );
    mon_muon1Phi.push_back( pCombinedMuonFeature1->IDTrack()->param()->phi0() );
    mon_muon1Pt.push_back( pCombinedMuonFeature1->IDTrack()->param()->pT()/1000. );
    mon_muon1Z0.push_back( pCombinedMuonFeature1->IDTrack()->param()->z0() );
    mon_muon1A0.push_back( pCombinedMuonFeature1->IDTrack()->param()->a0() );
    mon_muon2Eta.push_back( pCombinedMuonFeature2->IDTrack()->param()->eta() );
    mon_muon2Phi.push_back( pCombinedMuonFeature2->IDTrack()->param()->phi0() );
    mon_muon2Pt.push_back( pCombinedMuonFeature2->IDTrack()->param()->pT()/1000. );
    mon_muon2Z0.push_back( pCombinedMuonFeature2->IDTrack()->param()->z0() );
    mon_muon2A0.push_back( pCombinedMuonFeature2->IDTrack()->param()->a0() );
  ...
  // opposite charge check
    ...
    msg() << MSG::DEBUG << "JWW: muon1 with IDTrack()->param()->pT(): " << (muonCandidates[0]->IDTrack() && muonCandidates[0]->IDTrack()->param() ? muonCandidates[0]->IDTrack()->param()->pT() : 0) << endreq;
    msg() << MSG::DEBUG << "JWW: " << muonCandidates[0]->charge() << "  " << muonCandidates[0]->pt() << "  " << muonCandidates[0]->ptq() << endreq;
    msg() << MSG::DEBUG << "JWW: muon2 with IDTrack()->param()->pT(): " << (muonCandidates[1]->IDTrack() && muonCandidates[1]->IDTrack()->param() ? muonCandidates[1]->IDTrack()->param()->pT() : 0) << endreq;
    msg() << MSG::DEBUG << "JWW: " << muonCandidates[1]->charge() << "  " << muonCandidates[1]->pt() << "  " << muonCandidates[1]->ptq()<< endreq;


  if(m_oppositeCharge && muonCandidates[0]->ptq()*muonCandidates[1]->ptq() > 0) {
  ...
  if(m_doVertexFit && m_doMuMuVertexFit){
    ...
    TrigL2Vertex* muMuVtx = NULL;
      ...
      StatusCode status;
      status = m_vertexingTool->addTrack(muonCandidates[0]->IDTrack(),muMuVtx,Trk::muon);
      if( status.isFailure() ){
	    if(msgLvl() <= MSG::DEBUG) msg() <<MSG::DEBUG <<"Failed to add muon track 1 to vertexingTool of in DiMu vertex, with pT " <<muonCandidates[0]->IDTrack()->param()->pT() << " ." << endreq;
        mon_Errors.push_back( ERROR_AddMuonTrack_Fails );
	    delete muMuVtx;
	    return HLT::OK;
      }
      status = m_vertexingTool->addTrack(muonCandidates[1]->IDTrack(),muMuVtx,Trk::muon);
      if( status.isFailure() ){
	    if(msgLvl() <= MSG::DEBUG) msg() <<MSG::DEBUG <<"Failed to add muon track 2 to vertexingTool of in DiMu vertex, with pT " <<muonCandidates[1]->IDTrack()->param()->pT() << " ." << endreq;
        mon_Errors.push_back( ERROR_AddMuonTrack_Fails );
	    delete muMuVtx;
	    return HLT::OK;
      }
...
//   } // end of loop over muons 
  ...
  std::vector<const TrigInDetTrackCollection*> vectorOfTrackCollections= merge_no_dupl(vectorOfTrackCollections1, vectorOfTrackCollections2);
    ...
    std::vector<const TrigInDetTrackCollection*>::const_iterator iTrackCollection = vectorOfTrackCollections.begin();
    std::vector<const TrigInDetTrackCollection*>::const_iterator endTrackCollection = vectorOfTrackCollections.end();
    ...
    for(; iTrackCollection!=endTrackCollection;iTrackCollection++) {
      ...
      TrigInDetTrackCollection::const_iterator iTrack = (*iTrackCollection)->begin();
      TrigInDetTrackCollection::const_iterator endTrack = (*iTrackCollection)->end();
      GoodTrack.resize ((*iTrackCollection)->size());
      for(int itrk=0; iTrack != endTrack; ++itrk, ++iTrack) {
        GoodTrack[itrk]=false;
        
        if( (*iTrack)->param() != NULL && (*iTrack)->chi2() < 200000. ) {
//           GoodTrack[itrk]=true;
//           nGoodTrack++;
          if(msgLvl() <= MSG::DEBUG){
...
//           const Trk::Perigee* perigee = iTrack->perigeeParameters();
          if ( 
              ( fabs((*iTrack)->param()->pT() - muonCandidates[0]->IDTrack()->param()->pT())<100. 
                && fabs((*iTrack)->param()->eta() - muonCandidates[0]->IDTrack()->param()->eta())<0.02 
                && fabs((*iTrack)->param()->phi0() - muonCandidates[0]->IDTrack()->param()->phi0())<0.02 ) ||
              ( fabs((*iTrack)->param()->pT() - muonCandidates[1]->IDTrack()->param()->pT())<100. 
                && fabs((*iTrack)->param()->eta() - muonCandidates[1]->IDTrack()->param()->eta())<0.02 
                && fabs((*iTrack)->param()->phi0() - muonCandidates[1]->IDTrack()->param()->phi0())<0.02 )
          ) {
...
//             continue;
            ...
            mon_pTTracks.push_back((*iTrack)->param()->pT());
            mon_etaTracks.push_back((*iTrack)->param()->eta());
            mon_phiTracks.push_back((*iTrack)->param()->phi0());
            // end of monitoring
          }
...
//       }
      ...
      TrigInDetTrackCollection::const_iterator iTrack2 = (*iTrackCollection)->begin();
      TrigInDetTrackCollection::const_iterator lastiTrack2 = (*iTrackCollection)->end();
      ...
      for (int itrk=0; iTrack2 !=lastiTrack2; ++itrk, iTrack2++) {
        ...
        if(iTrack2 != lastiTrack2) {
          ...
          TrigInDetTrackCollection::const_iterator iTrack3= iTrack2+1;
          for (int jtrk=itrk+1; iTrack3 !=lastiTrack2; ++jtrk, ++iTrack3) {
            ...
            if((*iTrack2)->param()->pT() * (*iTrack3)->param()->pT() < 0 ) {
              ...
              if(m_doBc_DsMuMuDecay){
                ...
                if ( xPhiMass > m_lowerPhiDs_MassCut && xPhiMass < m_upperPhiDs_MassCut ) {
                  ...
                  if ( xPhiMuMuMass < m_upperBc_DsMuMuMassCut - PIMASS ) {
                    ...
                    TrigInDetTrackCollection::const_iterator iTrack4 = (*iTrackCollection)->begin();
                    TrigInDetTrackCollection::const_iterator lastiTrack4 = (*iTrackCollection)->end();
              ...
              ATH_MSG(DEBUG) << "Track pair rejected by opposite charge check: " 
                                 << *iTrack2 << " = " << (*iTrack2)->param()->pT()/fabs( (*iTrack2)->param()->pT() ) << ",  "
                                 << *iTrack3 << " = " << (*iTrack3)->param()->pT()/fabs( (*iTrack3)->param()->pT() ) << endreq;
            }
  ...
  if ((m_trigBphysColl_b!=NULL) && (m_trigBphysColl_b->size() > 0)) {
    if(msgLvl() <= MSG::DEBUG) msg()  << MSG::DEBUG << "REGTEST: Store Bphys Collection size: " << m_trigBphysColl_b->size() << endreq;
    mon_nBphys = m_trigBphysColl_b->size();
    HLT::ErrorCode sc = attachFeature(outputTE, m_trigBphysColl_b, "L2BMuMuXFex" );
    if(sc != HLT::OK) {
      if(msgLvl() <= MSG::WARNING) msg()  << MSG::WARNING << "Failed to store B+/-, Bd, Bs or Lb (Bar) trigBphys Collection" << endreq;
      return sc;
    }
    sc = attachFeature(outputTE, m_trigBphysColl_lambda, "L2BMuMuXFex_Lambda" );
    if(sc != HLT::OK) {
      if(msgLvl() <= MSG::WARNING) msg()  << MSG::WARNING << "Failed to store Lambda trigBphys Collection" << endreq;
      return sc;
    }
    sc = attachFeature(outputTE, m_trigBphysColl_phi, "L2BMuMuXFex_Phi" );
    if(sc != HLT::OK) {
      if(msgLvl() <= MSG::WARNING) msg()  << MSG::WARNING << "Failed to store Phi trigBphys Collection" << endreq;
      return sc;
    }
    sc = attachFeature(outputTE, m_trigBphysColl_kStar, "L2BMuMuXFex_Kstar" );
    if(sc != HLT::OK) {
      if(msgLvl() <= MSG::WARNING) msg()  << MSG::WARNING << "Failed to store Kstar trigBphys Collection" << endreq;
      return sc;
    }
    sc = attachFeature(outputTE, m_trigBphysColl_ds, "L2BMuMuXFex_Ds" );
    if(sc != HLT::OK) {
