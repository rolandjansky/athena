    ...
    for(pItr=muidIDtracks1.begin(); pItr != muidIDtracks1.end(); pItr++) {
      for(mItr=muidIDtracks2.begin(); mItr != muidIDtracks2.end(); mItr++) {
        ...
        if(!muMuVxCandidate) {
          ...
          HLT::ErrorCode status = getFeatures(outputTE, vectorTrkParticleCollections);
          ...
          if(status != HLT::OK/* or vectorTrkParticleCont==NULL*/) {
            msg() << MSG::WARNING << "Failed to get TrackParticleCollections from the trigger element" << endreq;
            ...
            if ( msgLvl() <= MSG::DEBUG ) msg() << MSG::DEBUG << " Got " << vectorTrkParticleCollections.size()
          ...
          for (std::vector<const Rec::TrackParticleContainer*>::const_iterator pTrackColl = vectorTrkParticleCollections.begin();
               pTrackColl != vectorTrkParticleCollections.end(); 
               ++pTrackColl) {
...
//             return(HLT::OK);
          ...
          ElementLink<Rec::TrackParticleContainer> trackELmu1(*TrkParticleCont,piTrk);
          ElementLink<Rec::TrackParticleContainer> trackELmu2(*TrkParticleCont,miTrk);
          
          // Main outer loop over tracks
          Rec::TrackParticleContainer::const_iterator trkIt1 =  TrkParticleCont->begin();
          for (int itrk1=0 ; trkIt1 != lastTrkIt; itrk1++, trkIt1++) 
          {
            const Trk::Track* track1 = (*trkIt1)->originalTrack();
            ...
            if(m_doB_KMuMuDecay) {
              ...
              if (trigPartBplusMuMuKplus) {
                ElementLink<Rec::TrackParticleContainer> trackEL3(*TrkParticleCont,itrk1);
                trigPartBplusMuMuKplus->addTrack(trackEL3);
                if(Found1Track) trigPartBplusMuMuKplus->addTrack(trackELmu1);
                if(Found2Track) trigPartBplusMuMuKplus->addTrack(trackELmu2);
...
//                 delete trigPartBplusMuMuKplus;
            ...
            Rec::TrackParticleContainer::const_iterator trkIt2 = trkIt1 + 1;
            for (int itrk2=itrk1+1 ; trkIt2 != lastTrkIt; itrk2++, trkIt2++) 
            {
              ...
              if(m_doBd_KstarMuMuDecay || m_doBs_Phi1020MuMuDecay || m_doLb_LambdaMuMuDecay || m_doBc_DsMuMuDecay) {
                const Trk::Track* track1 = (*trkIt1)->originalTrack();
                const Trk::Track* track2 = (*trkIt2)->originalTrack();
...
//                     trigPartBdMuMuKstar->addTrack(trackELmu2);
                    ElementLink<Rec::TrackParticleContainer> trackEL3(*TrkParticleCont,itrk1);
                    ElementLink<Rec::TrackParticleContainer> trackEL4(*TrkParticleCont,itrk2);
                    trigPartKstar->addTrack(trackEL3);
                    trigPartKstar->addTrack(trackEL4);
                    m_trigBphysColl_X->push_back(trigPartKstar);
                    int iKstar = m_trigBphysColl_X->size() - 1;
                    ElementLink<TrigEFBphysContainer> trigPartXEL(*m_trigBphysColl_X,iKstar);
                    trigPartBdMuMuKstar->addTrack(trackEL3);
                    trigPartBdMuMuKstar->addTrack(trackEL4);
                    if(Found1Track) trigPartBdMuMuKstar->addTrack(trackELmu1);
                    if(Found2Track) trigPartBdMuMuKstar->addTrack(trackELmu2);
                    trigPartBdMuMuKstar->pSecondDecay(trigPartXEL);
...
//                     trigPartBdMuMuKstar->addTrack(trackELmu1);
//                     trigPartBdMuMuKstar->addTrack(trackELmu2);
                    ElementLink<Rec::TrackParticleContainer> trackEL3(*TrkParticleCont,itrk2);
                    ElementLink<Rec::TrackParticleContainer> trackEL4(*TrkParticleCont,itrk1);
                    trigPartKstar->addTrack(trackEL3);
                    trigPartKstar->addTrack(trackEL4);
                    m_trigBphysColl_X->push_back(trigPartKstar);
                    int iKstar = m_trigBphysColl_X->size() - 1;
...
//                     delete trigPartBdMuMuKstar;
                ...
                if(m_doBs_Phi1020MuMuDecay) {
                  TrigEFBphys* trigPartPhi;
                  
                  TrigEFBphys* trigPartBsMuMuPhi = checkBMuMu2X(*mItr,*pItr,track1,track2,bS_to_Phi,trigPartPhi);
                  nTriedCombinations++;
                  if (trigPartBsMuMuPhi) {
//                     trigPartBsMuMuPhi->addTrack(trackELmu1);
//                     trigPartBsMuMuPhi->addTrack(trackELmu2);
                    ElementLink<Rec::TrackParticleContainer> trackEL3(*TrkParticleCont,itrk1);
                    ElementLink<Rec::TrackParticleContainer> trackEL4(*TrkParticleCont,itrk2);
                    trigPartPhi->addTrack(trackEL3);
                    trigPartPhi->addTrack(trackEL4);
                    m_trigBphysColl_X->push_back(trigPartPhi);
                    int iPhi = m_trigBphysColl_X->size() - 1;
                    ElementLink<TrigEFBphysContainer> trigPartXEL(*m_trigBphysColl_X,iPhi);
                    trigPartBsMuMuPhi->addTrack(trackEL3);
                    trigPartBsMuMuPhi->addTrack(trackEL4);
                    if(Found1Track) trigPartBsMuMuPhi->addTrack(trackELmu1);
                    if(Found2Track) trigPartBsMuMuPhi->addTrack(trackELmu2);
                    trigPartBsMuMuPhi->pSecondDecay(trigPartXEL);
...
//                     trigPartLbMuMuLambda->addTrack(trackELmu1);
//                     trigPartLbMuMuLambda->addTrack(trackELmu2);
                    ElementLink<Rec::TrackParticleContainer> trackEL3(*TrkParticleCont,itrk1);
                    ElementLink<Rec::TrackParticleContainer> trackEL4(*TrkParticleCont,itrk2);
                    trigPartLambda->addTrack(trackEL3);
                    trigPartLambda->addTrack(trackEL4);
                    m_trigBphysColl_X->push_back(trigPartLambda);
                    int iLambda = m_trigBphysColl_X->size() - 1;
                    ElementLink<TrigEFBphysContainer> trigPartXEL(*m_trigBphysColl_X,iLambda);
                    trigPartLbMuMuLambda->addTrack(trackEL3);
                    trigPartLbMuMuLambda->addTrack(trackEL4);
                    if(Found1Track) trigPartLbMuMuLambda->addTrack(trackELmu1);
                    if(Found2Track) trigPartLbMuMuLambda->addTrack(trackELmu2);
                    trigPartLbMuMuLambda->pSecondDecay(trigPartXEL);
...
//                     delete trigPartLbMuMuLambda;
                ...
                if(m_doBc_DsMuMuDecay) {
                  ...
                  if ( xPhiMass > m_lowerPhiDs_MassCut && xPhiMass < m_upperPhiDs_MassCut ) {
                    ...
                    if ( xPhiMuMuMass < m_upperBc_DsMuMuMassCut - PIMASS ) {
                      ...
                      Rec::TrackParticleContainer::const_iterator trkIt3 =  TrkParticleCont->begin();
                      for (int itrk3=0 ; trkIt3 != lastTrkIt; itrk3++, trkIt3++) 
                      {
                        const Trk::Track* track3 = (*trkIt3)->originalTrack();
                        ...
                        if (trigPartBcMuMuDs) {
                          ...
                          ElementLink<Rec::TrackParticleContainer> trackEL3(*TrkParticleCont,itrk1);
                          ElementLink<Rec::TrackParticleContainer> trackEL4(*TrkParticleCont,itrk2);
                          ElementLink<Rec::TrackParticleContainer> trackEL5(*TrkParticleCont,itrk3);
                          trigPartDs->addTrack(trackEL3);
                          trigPartDs->addTrack(trackEL4);
                          trigPartDs->addTrack(trackEL5);
                          m_trigBphysColl_X->push_back(trigPartDs);
                          int iDs = m_trigBphysColl_X->size() - 1;
                          ElementLink<TrigEFBphysContainer> trigPartXEL(*m_trigBphysColl_X,iDs);
                          trigPartBcMuMuDs->addTrack(trackEL3);
                          trigPartBcMuMuDs->addTrack(trackEL4);
                          trigPartBcMuMuDs->addTrack(trackEL5);
                          if(Found1Track) trigPartBcMuMuDs->addTrack(trackELmu1);
                          if(Found2Track) trigPartBcMuMuDs->addTrack(trackELmu2);
                          trigPartBcMuMuDs->pSecondDecay(trigPartXEL);
  ...
  if(m_trigBphysColl_b!=0 && m_trigBphysColl_b->size()>0) {
    if ( msgLvl() <= MSG::DEBUG ) msg()  << MSG::DEBUG << "REGTEST: Store Bphys Collection " << m_trigBphysColl_b << " size: " << m_trigBphysColl_b->size() << endreq;
    HLT::ErrorCode sc = attachFeature(outputTE, m_trigBphysColl_b, "EFBMuMuXFex" );
    if(sc != HLT::OK) {
    ...
    sc = attachFeature(outputTE, m_trigBphysColl_X, "EFBMuMuXFex_X" );
    if(sc != HLT::OK) {
