...
TrigEFDiMuHypo_xAOD::TrigEFDiMuHypo_xAOD(const std::string& name, ISvcLocator* pSvcLocator) :
...
TrigEFDiMuHypo_xAOD::~TrigEFDiMuHypo_xAOD()
{}

HLT::ErrorCode TrigEFDiMuHypo_xAOD::hltInitialize()
{
  msg()<<MSG::INFO<<"Initializing TrigEFDiMuHypo_xAOD"<< endreq;
  return HLT::OK;
}

HLT::ErrorCode TrigEFDiMuHypo_xAOD::hltFinalize()
{
  msg() << MSG::INFO << "Finalizing TrigEFDiMuHypo_xAOD" << endreq;
  return HLT::OK;
}

HLT::ErrorCode TrigEFDiMuHypo_xAOD::hltExecute(const HLT::TriggerElement* outputTE, bool& pass)
{
  if (msgLvl() <= MSG::DEBUG) msg()<<MSG::DEBUG<< "Executing TrigEFDiMuHypo_xAOD <--*-->"<<endreq;
