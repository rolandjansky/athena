...
TrigEFDiMuFex_xAOD::TrigEFDiMuFex_xAOD(const std::string& name, ISvcLocator* pSvcLocator) :
...
TrigEFDiMuFex_xAOD::~TrigEFDiMuFex_xAOD()
{}

HLT::ErrorCode TrigEFDiMuFex_xAOD::hltInitialize()
{
    msg() << MSG::INFO << "Initializing TrigEFDiMuFex_xAOD" << endreq;
    
    // Initialize NTuple
    if (m_doNTuple) {
        msg()<< MSG::WARNING << "Ntuple booking not supported in TrigEFDiMuFex_xAOD yet " << m_ntupleName << endreq;
        m_doNTuple = false;
    }
...
HLT::ErrorCode TrigEFDiMuFex_xAOD::hltFinalize()
{
    msg() << MSG::INFO << "Finalizing TrigEFDiMuFex_xAOD" << endreq;
    return HLT::OK;
}

HLT::ErrorCode TrigEFDiMuFex_xAOD::hltExecute(const HLT::TriggerElement* inputTE, HLT::TriggerElement* outputTE)
{
    if (msgLvl() <= MSG::DEBUG) msg()<<MSG::DEBUG<<"Executing TrigEFDiMuFex_xAOD <--*-->"<<endreq;
    ...
    for (xAOD::MuonContainer::const_iterator tr=xAODTrackCont->begin();tr != xAODTrackCont->end(); tr++)
    {
        ...
        if(m_doNTuple)
        {
            ++nMuons;
            // tan(x/2)=sqrt(1+cot**2)-cot
            StatusCode statusfill = m_pTrigEFDiMuNtuple->fillReco(nMuons,muon->phi(),cotTh,muon->eta(),muon->pt(), (int)tpCombMuon->charge(), muon->muonType());
    ...
    if (m_doNTuple)
    {
        ...
        {
            msg() << MSG::ERROR << "TrigEFDiMuFex_xAOD::execute Cannot write NTuple" << endreq;
            return HLT::BAD_JOB_SETUP;
        }
    }
    
    // no need to attached an empty vector  
    if (m_xAODTrigBphysColl->size()!=0)
    {
        status= attachFeature(outputTE, m_xAODTrigBphysColl, "EFMuPairs");
        if(status != HLT::OK) msg()<<MSG::WARNING<<"Failed to attach xAOD::TrigBphysContainer"<<endreq;
        else {
            if (msgLvl() <= MSG::DEBUG) msg()<<MSG::DEBUG<<"Successfully attached a xAOD::TrigBphysContainer with size "<<m_xAODTrigBphysColl->size()<<endreq;
        }
    }
    else {
...
HLT::ErrorCode TrigEFDiMuFex_xAOD::doMuTruth()
{
    int nTruthMu = 0;
