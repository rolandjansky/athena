...
// ********************************************************************
//
// NAME:     TrigEFTrkMassFex_xAOD.cxx
...
/*------------------------------------------------------------------------------------*/
TrigEFTrkMassFex_xAOD::TrigEFTrkMassFex_xAOD(const std::string & name, ISvcLocator* pSvcLocator):
...
/*-------------------------------------------*/
TrigEFTrkMassFex_xAOD::~TrigEFTrkMassFex_xAOD()
/*-------------------------------------------*/
{
...
/*-------------------------------------------*/
HLT::ErrorCode TrigEFTrkMassFex_xAOD::hltInitialize()
/*-------------------------------------------*/
{
...
/*-------------------------------------------*/
HLT::ErrorCode TrigEFTrkMassFex_xAOD::hltFinalize()
/*-------------------------------------------*/
{
    msg() << MSG::INFO << "|----------------------- SUMMARY FROM TrigEFTrkMassFex_xAOD -------------|" << endreq;
...
/*--------------------------------------------------------------*/
//HLT::ErrorCode TrigEFTrkMassFex_xAOD::acceptInput(const HLT::TriggerElement* inputTE, bool& pass)
/*--------------------------------------------------------------*/
//{
//
//  return HLT::OK;
...
/*--------------------------------------------------------------*/
HLT::ErrorCode TrigEFTrkMassFex_xAOD::hltExecute(const HLT::TriggerElement*  inputTE , HLT::TriggerElement* outputTE)
/*--------------------------------------------------------------*/
{
    bool result = false;
    ...
    const xAOD::TrackParticleContainer* xAODTrkParticleCont(0);
    ...
    if(status != HLT::OK ) {
        msg() << MSG::WARNING << "Error when getting vectorxAODTrkParticleCont from the trigger element" << endreq;
        if ( timerSvc() ) m_TotTimer->stop();
        ...
        if ( msgLvl() <= MSG::DEBUG )msg() << MSG::DEBUG << " Got Track Collections, size " << vectorxAODTrkParticleCont.size() << endreq;
    }
    mon_Acceptance.push_back( ACCEPT_Got_TrackColl );
    //    if (TrkParticleCont->size() == 0) {
    if (vectorxAODTrkParticleCont.size() == 0) {
        if ( timerSvc() ) m_TotTimer->stop();
    ...
    std::vector<const xAOD::TrackParticleContainer*>::iterator pTrackColl    = vectorxAODTrkParticleCont.begin();
    std::vector<const xAOD::TrackParticleContainer*>::iterator lastTrackColl = vectorxAODTrkParticleCont.end();
    ...
    if (!m_matchL1) {
        ...
        for ( unsigned int i_mu=0; i_mu<MuEFTracks.size(); i_mu++ ) {
            ...
            HLT::ErrorCode status = GetxAODMuonTracks(MuEFTracks[i_mu], idTrks, msg());
            //FIXME - why is this loop here. What happens ti idTrks?
    ...
    if (xAODTrkParticleCont->size() > 0 ) mon_Acceptance.push_back( ACCEPT_Full_IDTracks );
    mon_nTracks = xAODTrkParticleCont->size();
    ...
    for (int itrk=0 ; trkIt != lastTrkIt; itrk++, trkIt++) {
        if ((*trkIt)->definingParametersCovMatrixVec().size() == 0) {
            if ( msgLvl() <= MSG::DEBUG ) msg() << MSG::DEBUG << "track has no ParametersCovMatrix - will reject" << endreq;
            //#FIXME add monitoring flags here
            continue;
        }

        // JW EDM const Trk::MeasuredPerigee* trckPerigee =  (*trkIt)->measuredPerigee();
        const Trk::Perigee* trckPerigee =  &(*trkIt)->perigeeParameters();

        //JW        double px = trckPerigee->momentum()[Trk::px];
        //        double py = trckPerigee->momentum()[Trk::py];
        //
        
        //        double ptSquare=std::pow(px,2) + std::pow(py,2);
        //        int signpT = -1 ? trckPerigee->parameters()[Trk::qOverP] < 0 : +1 ;
        //        double pT = sqrt(ptSquare) * signpT;
        //        double phi = trckPerigee->parameters()[Trk::phi];
        //      double theta = trckPerigee->parameters()[Trk::theta];
        //        double eta = trckPerigee->eta();

        //JW  Replace above lines with simple case of momentum retrieval
        
        double pT = (*trkIt)->pt() *(*trkIt)->charge();
        //m_trackPt.push_back(fabs(pT)/CLHEP::GeV);
        double eta =  (*trkIt)->eta() ;
        double phi =  (*trkIt)->pt() ;

        //        if ( msgLvl() <= MSG::DEBUG ) msg() << MSG::DEBUG << "Track: " << itrk << " pT = "  <<  pT << " phi = " << trckPerigee->parameters()[Trk::phi] << endreq;
        if ( msgLvl() <= MSG::DEBUG ) msg() << MSG::DEBUG << "Track: " << itrk << " pT = "  <<  pT << " phi = " << phi << endreq;
        // Tracks monitoring
        ...
        float trackChi2 = (*trkIt)->chiSquared();
        ...
        { PassedRoIMatch = true;
            ...
            xAOD::TrackParticleContainer::const_iterator trkIt2 =  xAODTrkParticleCont->begin();
            for (int jtrk=0; trkIt2 != lastTrkIt; ++jtrk, trkIt2++)
            {
                if(itrk==jtrk) continue;
                if ((*trkIt2)->definingParametersCovMatrixVec().size() == 0) {
                    if ( msgLvl() <= MSG::DEBUG ) msg() << MSG::DEBUG << "track2 has no ParametersCovMatrix - will reject" << endreq;
                    //#FIXME add monitoring flags here
                    continue;
                }

                //JW EDM const Trk::MeasuredPerigee* trckPerigee2 =  (*trkIt2)->measuredPerigee();
                const Trk::Perigee* trckPerigee2 =  &(*trkIt2)->perigeeParameters();
                
                double track2phi = trckPerigee2->parameters()[Trk::phi];
                double track2eta = trckPerigee2->eta();
               
                float track2Pt(0.0); 
                if ( trckPerigee2->parameters()[Trk::qOverP] == 0.0) {
                    if (msgLvl() <= MSG::DEBUG ) msg() << MSG::DEBUG << "Track: qOverP is zero" << endreq;
                } else {
                    track2Pt = sin(trckPerigee2->parameters()[Trk::theta])/trckPerigee2->parameters()[Trk::qOverP];
                }
                ...
                track2Pt = fabs(track2Pt); // remove any sign info
                float track2Chi2 = (*trkIt2)->chiSquared();
                ...
                //                trigEFBphys->addTrackParticleLink(track1EL);
                //                trigEFBphys->addTrackParticleLink(track2EL);
                //m_Mass.push_back(Mass);
                xAOD::TrigBphys* xaodObj = new xAOD::TrigBphys();
                m_xAODTrigBphysColl->push_back( xaodObj );
                xaodObj->initialise(roiDescriptor->roiId(), roiDescriptor->eta(), roiDescriptor->phi(),
                                    xAOD::TrigBphys::JPSIMUMU, Mass, xAOD::TrigBphys::EF );
                if ( msgLvl() <= MSG::DEBUG) msg()  << MSG::DEBUG << "Create Bphys particle with roIId " << xaodObj->roiId()
                    <<" mass " << Mass << " phi, eta " << xaodObj->phi() << " "
                    << xaodObj->eta() << " vertex type " << xaodObj->particleType() << endreq;
                ElementLink<xAOD::TrackParticleContainer> track1EL(*xAODTrkParticleCont,itrk);
                ElementLink<xAOD::TrackParticleContainer> track2EL(*xAODTrkParticleCont,jtrk);
                xaodObj->addTrackParticleLink(track1EL);
                xaodObj->addTrackParticleLink(track2EL);
                ...
                if (m_doVertexFit) {
                    ...
                    std::vector<const xAOD::TrackParticle*> pair;
                    ...
                    myxAODVertex = m_iVKVVertexFitter->fit(pair,vertex);
                    if(myxAODVertex) {
                        ...
                        xaodObj->setFitchi2(((myxAODVertex->chiSquared())));
                        xaodObj->setFitndof(2*(pair.size())-3);
                        xaodObj->setFitx(myxAODVertex->x());  // xaodVertex->x();
                        xaodObj->setFity(myxAODVertex->y());
                        xaodObj->setFitz(myxAODVertex->z());

                        
                        // Monitoring of the fit results
                        mon_FitMass       .push_back( invariantMass / CLHEP::GeV );
                        mon_InvMass_okFit .push_back( Mass / CLHEP::GeV );
                        double chisq = myxAODVertex->chiSquared();
                        unsigned int ndf = (unsigned int) myxAODVertex->numberDoF();
                        //if ( ndf==0 )  ndf = 1;
                        if (msgLvl() <= MSG::DEBUG) msg()<<MSG::DEBUG<<"Vertex chi2 / ndf: "
                                    << chisq << "  " << ndf  <<endreq;

                        if ( ndf == 0 && chisq == 0) {
                            mon_Chi2toNDoF.push_back( -1 ); // #FIXME need a better bin choice?
                            double chi2prob = 0.0;
                            mon_Chi2toNDoFProb.push_back( chi2prob );
                        } else if (ndf > 0 && chisq == 0) {
                            mon_Chi2toNDoF.push_back( 0.0 ); // #FIXME need a better bin choice?
                            double chi2prob = 0.0;
                            mon_Chi2toNDoFProb.push_back( chi2prob );
                        } else {
                        ...
                        // }
                        mon_FitVtxR.push_back( myxAODVertex->x()*myxAODVertex->x() + myxAODVertex->y()*myxAODVertex->y()) ;
                        mon_FitVtxZ.push_back( myxAODVertex->z() );
    ...
    if ( m_xAODTrigBphysColl->size() > 0 ) mon_Acceptance.push_back( ACCEPT_BphysColl_not_Empty );
    mon_nBphys = m_xAODTrigBphysColl->size();
    ...
    if ( msgLvl() <= MSG::DEBUG) {
        if (m_xAODTrigBphysColl != 0) msg() << MSG::DEBUG << "Bphys collection size : " << m_xAODTrigBphysColl->size() << " m_xAODTrigBphysColl " << m_xAODTrigBphysColl << endreq;
    }
