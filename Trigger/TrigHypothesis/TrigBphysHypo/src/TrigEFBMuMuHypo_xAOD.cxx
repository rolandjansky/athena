...
/**************************************************************************
 **
 **   File: Trigger/TrigHypothesis/TrigBPhysHypo/TrigEFBMuMuHypo_xAOD.cxx
 **
...
TrigEFBMuMuHypo_xAOD::TrigEFBMuMuHypo_xAOD(const std::string & name, ISvcLocator* pSvcLocator):
...
TrigEFBMuMuHypo_xAOD::~TrigEFBMuMuHypo_xAOD()
{ }

HLT::ErrorCode TrigEFBMuMuHypo_xAOD::hltInitialize()
{
...
HLT::ErrorCode TrigEFBMuMuHypo_xAOD::hltFinalize()
{
  msg() << MSG::INFO << "in finalize()" << endreq;
  msg() << MSG::INFO << "|----------------------- SUMMARY FROM TrigEFBMuMuHypo_xAOD -------------|" << endreq;
...
//-------------------------------------------------------------------------------------
HLT::ErrorCode TrigEFBMuMuHypo_xAOD::hltExecute(const HLT::TriggerElement* outputTE, bool& pass)
{
...
//std::cout<<" BBBBBBB:  m_beamSpot.x()= "<< m_beamSpot.x()<<" m_beamSpot.y()= "<<m_beamSpot.y()<<std::endl;               
  ...
  // now loop over Bphys particles to see if one passes cuts
    ...
    */
    ...
    if(msgLvl() <= MSG::DEBUG) msg() << MSG::DEBUG << " Run " << IdRun << " Event " << IdEvent << " TrigEFBMuMuHypo_xAOD result : " << pass << endreq;
    // store result
