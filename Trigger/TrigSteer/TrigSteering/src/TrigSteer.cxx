...
StatusCode TrigSteer::initialize()
{
  ...
  if( m_robDataProvider.retrieve().isFailure() ) {
    (*m_log) << MSG::ERROR << "can't get ROBDataProviderSvc"
             << endreq;
    return StatusCode::FAILURE;
  }
  ...
  if ( m_navigation.retrieve().isFailure() ) {
    ...
    (*m_log) << MSG::DEBUG << "successfully retrieved navigation tool: "
             << m_navigation << endreq;
  }
  ...
  if (m_coreDumpSvc.retrieve().isFailure()) {
    (*m_log) << MSG::FATAL << "Failed to retrieve CoreDumPSvc: " << m_coreDumpSvc << endreq;
    return StatusCode::FAILURE;
  }
    
  // Get the TrigConfiguration Service
  if (m_configSvc.retrieve().isFailure()) {
    (*m_log) << MSG::FATAL << "failed to retrieve config service: " << m_configSvc
             << endreq;
    return StatusCode::FAILURE;
  }
  ...
  if (m_scalerSvc.retrieve().isFailure()) {
    (*m_log) << MSG::FATAL << "Failed to retrieve ScalerSvc: " << m_scalerSvc
             << endreq;
    return StatusCode::FAILURE;
  }
  (*m_log) << MSG::DEBUG << "successfully retrieved ScalerSvc: " << m_scalerSvc
           << endreq;
  ...
  BOOST_FOREACH(TrigConf::HLTChain* confchain, *chainList) {
    ...
    if (confchain->level() == m_hltLevel) {
      ...
      (*m_log) << MSG::DEBUG <<(*confchain)<<endreq;
      m_chains.push_back(chain);
      if (!chain->isValid()) {
        (*m_log) << MSG::ERROR << "Error creating chain " << confchain->chain_name()
                 << ", will now abort the job." << endreq;
        return StatusCode::FAILURE;
      }
    } else {
      (*m_log) << MSG::DEBUG << "ignore chain: " << confchain->chain_name()
               << " which is not configured as " << m_hltLevel << endreq;
    }
  }
  ...
  (*m_log) << MSG::DEBUG << "creating LvlConverter sub algorithm." << endreq;
  if ( m_lvlCnvTool.retrieve().isFailure() ) {
    (*m_log) << MSG::ERROR << "Failed to retreive LvlConverter Tool: " << m_lvlCnvTool << endreq;
    return StatusCode::FAILURE;
  } else {
    (*m_log) << MSG::DEBUG << "Retrieved " << m_lvlCnvTool << endreq;
  ...
  (*m_log) << MSG::DEBUG << "creating ResultBuilder sub algorithm." << endreq;

  if ( m_resultBuilder.retrieve().isFailure() ) {
    (*m_log) << MSG::ERROR << "Failed to retreive ResultBuilder Tool: " << m_resultBuilder << endreq;
    return StatusCode::FAILURE;
  } else {
    (*m_log) << MSG::DEBUG << "Retrieved " << m_resultBuilder << endreq;
  ...
  // Create OPI tools
  if ( m_opiTools.retrieve().isFailure() ) {
    (*m_log) << MSG::ERROR << "Failed to retrieve OPI monitoring Tools: " << m_opiTools << endreq;
    return StatusCode::FAILURE;
  } else {
    (*m_log) << MSG::DEBUG << "Retrieved " << m_opiTools << endreq;
  }

  // Create monitoring tools
  if ( m_monTools.retrieve().isFailure() ) {
    (*m_log) << MSG::ERROR << "Failed to retrieve monitoring Tools: " << m_monTools << endreq;
    return StatusCode::FAILURE;
  } else {
    (*m_log) << MSG::DEBUG << "Retrieved " << m_monTools << endreq;
  }
  ...
  if ( m_incSvc.retrieve().isFailure() ) {
    (*m_log) << MSG::ERROR << "Failed to retrieve IncidentSvc" << endreq;
    return StatusCode::FAILURE;
  }
  else {  
    (*m_log) << MSG::DEBUG << "Adding listener for event timeout" << endreq;
  ...
  (*m_log) << MSG::DEBUG << "initializing done" << endreq;
...
{
  (*m_log) << MSG::DEBUG << "finalizing " << name() << endreq;

  // delete all chains
  std::vector<HLT::SteeringChain*>::iterator chainIt = m_chains.begin();
  for (; chainIt != m_chains.end(); ++chainIt) {
    delete (*chainIt); (*chainIt) = 0;
  }
  m_chains.clear();
  m_activeChains.clear();
  (*m_log) << MSG::DEBUG << "finalized chains " << name() << endreq;

  // delete all sequences
  std::vector<HLT::Sequence*>::iterator seqIt = m_sequences.begin();
  for (; seqIt != m_sequences.end(); ++seqIt) {
    delete (*seqIt); (*seqIt) = 0;
  }
  m_algos.clear();
  m_sequences.clear();
  (*m_log) << MSG::DEBUG << "finalized sequences " << name() << endreq;


  delete m_log; m_log=0;
  delete m_config; m_config=0;
  //  std::cerr << "realy finished " << std::endl;
  return StatusCode::SUCCESS;
}
...
{  
  ...
  if (m_logLvl <= MSG::DEBUG) {
    (*m_log) << MSG::DEBUG << " Executing TrigSteer for new event" << endreq;
    (*m_log) << MSG::DEBUG << "\n //////////////////////////////////////////////////\n";
    (*m_log) << MSG::DEBUG << " /////////  Start of HLT Processing in "
...
void TrigSteer::runChains(bool secondPass) {
  ...
  while ( chainsStillActive && noError ) {
	...
	break;
      ...
      //EB strategy:
      // method L2EF: call EB when all L2 are completed
      // method allL2: call EB at the end of processing
      // method final - with final menu, call the EB when
      //   - one chain is completed and accepted the event
      //   - one chain requires the EB    
      eventPassed = (*iterChain)->chainPassed() || eventPassed;
      if (( m_hltLevel == "HLT" ) && m_strategyEB == 0 ){
	if(((*iterChain)->isActive() && (*iterChain)->nextStepAfterEB()) || eventPassed){

	  if ((*iterChain)->isActive() && (*iterChain)->nextStepAfterEB()) 
...
void TrigSteer::issueEventBuildingRequest(int step) {
  // issue the EB request thorugh the TrigROBDataProviderSvc
  // only if not already requested here
  // for offline running, just set the EB step, doing nothing else

  // skip if the EB has been already called by the Steering
  bool doEB= (m_stepForEB == -1);
  if ( !doEB ){
    if (m_config->getMsgLvl() <=MSG::DEBUG) 
      (*m_log) << MSG::DEBUG << "Request of Event Building already issued! " << endreq; 
    return;
  }
  ...
  if (!m_trigROBDataProvider.isValid()) {
    if (m_config->getMsgLvl() <=MSG::DEBUG) 
      (*m_log) << MSG::DEBUG << "Request of Event Building is issued for offline running " << endreq; 
    return;
  }
...
HLT::Algo* TrigSteer::getAlgo(std::string name)
{
  ...
  if ( dynamic_cast<HLT::TECreateAlgo*>(algo) ) {
    ...
    algo->setProperty(enabled).ignore();
    algo->setProperties().ignore();
    (*m_log) << MSG::DEBUG << (m_doFex ? "enabling " : "disabling ") << subAlg_name << " because it is Fex" << endreq;
  }
...
{
  // look if sequence already exists in vector ...
  std::vector<HLT::Sequence*>::const_iterator it = m_sequences.begin();
  for (; it != m_sequences.end(); ++it) {
    if ( (*it)->outputTEType() == teType)
      return (*it);
  }
  ...
  BOOST_FOREACH (TrigConf::HLTSequence* confseq, *m_configSvc->sequenceList()) {
    ...
    std::vector<std::string>& hltAlgoVec = confseq->algorithms();
    ...
    for (unsigned int j = 0; j < hltAlgoVec.size(); ++j) {
      ...
      if(hltAlgoVec[j]=="") {
         ...
         (*m_log) << MSG::FATAL << "Name of algorithm number " << j << " in sequence '" << temp << "' is empty -> abort." << endreq;
      }
    ...
    // get all input TE types:

    std::vector<TrigConf::HLTTriggerElement*>& hltInputTEs = confseq->inputTEs();
    std::vector<unsigned int> inputTypes;
    std::vector< Sequence* > inputSequences;

    for (unsigned int j = 0; j < hltInputTEs.size(); ++j) {
      unsigned int inputId = hltInputTEs[j]->id();
      ...
      if ( producedFirstAtLevel(inputId) == m_hltLevel ) {
        Sequence* seq = findSeqForOutputTeType( inputId );
        // if seq not NULL and not in vector already, save it:
        if (seq && find(inputSequences.begin(), inputSequences.end(), seq) == inputSequences.end() ) {
          inputSequences.push_back(seq);
    ...
    // get output TE type
    // note: we assume only one type is possible!
    TrigConf::HLTTriggerElement* hltOutputTE = confseq->outputTE();
    unsigned int outputType = hltOutputTE->id();

    // get topo_start_from TE types
    const TrigConf::HLTTriggerElement* hltTopoTE = confseq->topoStartTE();
    std::vector<unsigned int> topoStartTypes;
    // only add an entry if pointer not NULL
    if (hltTopoTE) topoStartTypes.push_back( hltTopoTE->id() );


    // create new HLT::Sequence
    if (inputTypes.size() == 0) {
      (*m_log) << MSG::DEBUG << "creating a sequence without input types! Output is: " << outputType
               << endreq;
    }
    if (algos.size() == 0) {
      std::string temp; 
      TrigConf::HLTTriggerElement::getLabel(outputType, temp);
      (*m_log) << MSG::ERROR << "failed to create sequence: sequence has no algorithms. Output type: " << temp
               << endreq;
      return 0;
    }
    ...
    HLT::Sequence* seq = new HLT::Sequence(inputTypes, outputType,
    ...
    for (std::vector< Sequence* >::const_iterator it = inputSequences.begin();
      ...
      seq->m_previousSequences.push_back( *it );
    }
    m_sequences.push_back(seq);
    return seq;
  }

  // No luck, no sequence was found .. so there's nothing we can return
  return 0;
}
...
void TrigSteer::configureCoherentPrescaling() {  
  ...
  BOOST_FOREACH(HLT::SteeringChain* ch, m_chains ) {
  ...
  BOOST_FOREACH( PrescalingGroup::value_type g, prescaling_group) {
    //(*m_log) << MSG::DEBUG << "Prescaling group " << g << endreq;
    HLT::SteeringChain* previous = 0;
    BOOST_FOREACH(HLT::SteeringChain* ch, g.second ) {
      (*m_log) << MSG::DEBUG << "Prescaling group: " << ch->prescaleGroup()
	       << " chain: " << ch->getChainName() 
	       << " PS: " << ch->prescaleFactor() 
	       << " seeded of: " << ch->getLowerChainName()   << endreq;
