...
StatusCode TestMergeAlg::execute() {
  ...
  if (m_distance >= 0. && input.size()==1){
	...
	if (find(groupsOfMergedRoIs[groupname.str()].begin(), groupsOfMergedRoIs[groupname.str()].end(),nRoI) == groupsOfMergedRoIs[groupname.str()].end()){ 
	  if (nRoI!=0) ngroup++;
	  groupsOfMergedRoIs[groupname.str()]=std::vector<int>();
	...
	if ( objWithLinkToRoI ) {
	  ...
	  while(it2 != (*coll.cptr()).end()){
	    ...
	    if ( obj2WithLinkToRoI ) {
	      ...
	      float_t deltaR = sqrt(pow(((*roi)->eta() - (*roi2)->eta()),2.) + pow(( (*roi)->phi() - (*roi2)->phi()),2.));
	      if ( deltaR < m_distance ) {
		auto i = std::distance((*coll.cptr()).begin(),it2);
		groupsOfMergedRoIs[groupname.str()].push_back(i);
  ...
  for (int group=0; group<=ngroup;group++){
	...
	linkVector.push_back(ElementLink<xAOD::TrigCompositeContainer>(daukey, index));
      ...
      ATH_MSG_DEBUG("Proxy "<<proxy->name() <<" Added Detail: ProxyVecIndices of size " << groupsOfMergedRoIs[groupname.str()].size() << " with  Key = " <<daukey  <<" for collection "<< coll.name());
    }

    ATH_MSG_DEBUG("Created linkVector of size "<< linkVector.size());
    bool addedVector = ViewAlgs::setObjectLinks( proxy, linkVector);
  ...
  for (const auto& outp : *m_outputProxyContainer) {
    ElementLinkVector<xAOD::TrigCompositeContainer> linkVector;
    bool hasChildren = ViewAlgs::getObjectLinks( outp, linkVector);
    if (! hasChildren){
