...
TestFEXAlg::TestFEXAlg(const std::string& name, ISvcLocator* pSvcLocator) 
...
StatusCode TestFEXAlg::initialize() {
  return StatusCode::SUCCESS;
}

StatusCode TestFEXAlg::execute() {
  ...
  for ( const auto roi : *m_roisContainer.cptr() ) {
    ...
    if ( objWithLinkToRoI ) {
      ...
      proxy->setObjectLink("cluster", ElementLink<TestClusterContainer>(m_outputClusterContainer.name(), nRoI) );
      proxy->setObjectLink("seed", ElementLink<xAOD::TrigCompositeContainer>(m_roisContainer.name(), nRoI) );
    } else {
