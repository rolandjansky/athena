...
*/
  ...
  // Open again the root file
  TFile file2("../METPerformance_zeeall_HadAc.root");
  file2->cd( "Zee/Resolutions/Data_Res_Par_HadRecoil" );
  file2.ls();
...
gr->SetMinimum(-m_curvaslin);
  gr->GetYaxis()->SetTitle("Linearity(Data_Res_axis_HadRecoil)");
  gr->GetXaxis()->SetTitle("HadRecoil");
myText( 0.72,0.85,1,"Z#rightarrow ee");
myMarkerText(xxx,yyy,4,21,"Perp");

TGraphErrors* gr = new TGraphErrors(nbinplot2,nxbin2,mean2,ex2,mean_err2);
...
myMarkerText(xxx,yyy2,1,27,"Par");
  ...
  c2.Print("Zee_Lin_Data_Res_ParPerp2_HadRecoil.eps");
