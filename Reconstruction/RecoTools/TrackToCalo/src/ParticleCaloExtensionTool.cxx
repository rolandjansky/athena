...
namespace Trk {
    ParticleCaloExtensionTool::ParticleCaloExtensionTool(const std::string& t, const std::string& n, const IInterface* p) :
        AthAlgTool(t, n, p), m_detID(nullptr), m_particleType(muon) {
    ...
    StatusCode ParticleCaloExtensionTool::initialize() {
        ...
        if (m_particleTypeName == "nonInteracting") {
            ...
            ATH_MSG_WARNING("Unsupported particle type, using muon " << m_particleTypeName);
        }
        ATH_MSG_INFO(" Using particle type " << m_particleTypeName << " enum value " << m_particleType);
        return StatusCode::SUCCESS;
    }

    StatusCode ParticleCaloExtensionTool::finalize() { return StatusCode::SUCCESS; }

    std::unique_ptr<Trk::CaloExtension> ParticleCaloExtensionTool::caloExtension(const EventContext& ctx,
        ...
        if (particle.type() == xAOD::Type::TruthParticle) {
            ...
            if (el->trackParticle()) { return caloExtension(ctx, *(el->trackParticle())); }
        } else if (particle.type() == xAOD::Type::Muon) {
            const xAOD::Muon* muon = static_cast<const xAOD::Muon*>(&particle);
            if (muon->primaryTrackParticle()) { return caloExtension(ctx, *(muon->primaryTrackParticle())); }
    ...
    const Trk::CaloExtension* ParticleCaloExtensionTool::caloExtension(const EventContext& ctx, const xAOD::IParticle& particle,
        ...
        if (extension == nullptr) { extension = caloExtension(ctx, particle); }
    ...
    const Trk::CaloExtension* ParticleCaloExtensionTool::caloExtension(const xAOD::IParticle& particle,
        ...
        if (index < cache.size()) { return cache[index]; }
        ATH_MSG_WARNING("cache size smaller than particle index");
        return nullptr;
    }

    StatusCode ParticleCaloExtensionTool::caloExtensionCollection(const EventContext& ctx, const xAOD::IParticleContainer& particles,
    ...
    std::unique_ptr<Trk::CaloExtension> ParticleCaloExtensionTool::caloExtension(const EventContext& ctx,
        ...
        if (m_particleType == electron || particle.particleHypothesis() == xAOD::electron) {
        ...
        if (!idExitParamers && !muonEntryParamers) { idExitParamers = track.perigeeParameters(); }
    ...
    std::unique_ptr<Trk::CaloExtension> ParticleCaloExtensionTool::caloExtension(const EventContext& ctx, const TrackParameters& startPars,
                                                                                 PropDirection propDir,
                                                                                 ParticleHypothesis particleType) const {
        ATH_MSG_DEBUG("looking up calo states: r " << startPars.position().perp() << " z " << startPars.position().z() << " momentum "
        ...
        if (material) {
            ...
            for (auto& m : *material) {
                ATH_MSG_DEBUG(" layer "
                              ...
                              << (m->trackParameters() ? m->trackParameters()->position().perp() : -999) << " z "
                              << (m->trackParameters() ? m->trackParameters()->position().z() : -999) << " pt "
                              << (m->trackParameters() ? m->trackParameters()->momentum().perp() : -999) << " mat "
                              << (m->materialEffectsOnTrack() ? m->materialEffectsOnTrack()->thicknessInX0() : -999));
                delete m;
            }
            delete material;
        }
        ...
        for (const auto& p : *caloParameters) {
            ...
            ATH_MSG_DEBUG(" param " << p.first << " id " << p.second << " pos: r " << p.first->position().perp() << " z "
                                    << p.first->position().z() << " pt " << p.first->momentum().perp() << " cov " << p.first->covariance());
            ...
            if (p.second == 1 && propDir == Trk::alongMomentum) {
                ...
                TrackParametersIdentifier id = parsIdHelper.encode(AtlasDetDescr::fFirstAtlasCaloTechnology,
                                                                   static_cast<CaloSampling::CaloSample>(abs(p.second) % 1000), isEntry);
                /*
                ...
                if (p.first->type() != Trk::Curvilinear) {
                    const CurvilinearParameters cpars(p.first->position(), p.first->momentum(), p.first->charge(), std::nullopt, id);
                    caloLayers.push_back(cpars);
                    delete p.first;
                } else {
                    std::optional<AmgSymMatrix(5)> covariance(std::nullopt);
                    if (p.first->covariance()) { covariance = AmgSymMatrix(5)(*(p.first->covariance())); }
                    CurvilinearParameters cpars(p.first->position(), p.first->momentum(), p.first->charge(), std::move(covariance), id);
                    caloLayers.push_back(cpars);
                    delete p.first;
                }
            }
