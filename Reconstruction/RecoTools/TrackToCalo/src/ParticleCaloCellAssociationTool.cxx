  ...
  for( auto cell : cells ){
    ...
    if(drFix==0.) { 
      ...
      if(cell->caloDDE()->deta()>0) {
        ...
        drFix = drMin;
        ATH_MSG_VERBOSE(" Problem cell sampling " << cell->caloDDE()->getSampling() << " x " 
                        << cell->caloDDE()->x() << " y " << cell->caloDDE()->y() << " z " << cell->caloDDE()->z() 
                        << " dr " << cell->caloDDE()->dr() << " drFix "  << drFix << " drTG " << drTG ); 
        if(cellFound) ATH_MSG_VERBOSE(" cellFound sampling " << cellFound->caloDDE()->getSampling() 
                                      << " x " << cellFound->caloDDE()->x() << " y " << cellFound->caloDDE()->y() << " z " 
                                      << cellFound->caloDDE()->z() << " dr " << cellFound->caloDDE()->dr() << " dscut " << dscut  << " drFix "  << drFix );
      }
    }
    ...
    if(dzFix==0.) { 
      ...
      if(cell->caloDDE()->deta()>0) {
        ...
        double detaCheck = -log(tan((theta+dtheta)/2.)) + log(tan((theta)/2.));
        ATH_MSG_VERBOSE(" Fix cell sampling " << cell->caloDDE()->getSampling() << " deta " 
                        << cell->caloDDE()->deta() << " detaCheck  " << detaCheck  << " dtheta " << dtheta << " dzFix " << dzFix);
      } else {
        double dzMin = 100000.;
        int dscut = 1;
        if(barrel) dscut = 0;
        ...
        dzFix = dzMin;
        ATH_MSG_VERBOSE(" Problem cell sampling " << cell->caloDDE()->getSampling() << " x " << cell->caloDDE()->x() 
                        << " y " << cell->caloDDE()->y() << " z " << cell->caloDDE()->z() << " dz " 
                        << cell->caloDDE()->dz() << " dzFix "  << dzFix << " dzTG " << dzTG  ); 
        if(cellFound) ATH_MSG_VERBOSE(" cellFound sampling " << cellFound->caloDDE()->getSampling() << " x " 
                                      << cellFound->caloDDE()->x() << " y " << cellFound->caloDDE()->y() << " z " 
                                      << cellFound->caloDDE()->z() << " dz " << cellFound->caloDDE()->dz()  
                                      << " dscut " << dscut << " dzFix "  << dzFix );
      }    
    }
    // 
    ...
    if(use3D) {
      ...
      if(path>0||cell->energy()>50.) { 
        ATH_MSG_DEBUG(" cell sampling and size "  << cell->caloDDE()->getSampling() 
                      << " cell energy " << cell->energy() << " drFix " << drFix << " dzFix " << dzFix << " path " 
                      << path << " length TG " << totpath);
        ATH_MSG_DEBUG(" cell dr "  << cell->caloDDE()->dr() << " cell dz " 
                      << cell->caloDDE()->dz() << " deta " << cell->caloDDE()->deta()); 
      }
    }
    ...
    ATH_MSG_DEBUG(" PATH3D = " << path << " PATH2D = " << path2 << " eLoss " 
                  << eLoss << " cell energy " << (cell)->energy() << " radius " << cell->caloDDE()->r() 
                  << " phi " << cell->caloDDE()->phi() << " dr " << cell->caloDDE()->dr() << " dphi " 
