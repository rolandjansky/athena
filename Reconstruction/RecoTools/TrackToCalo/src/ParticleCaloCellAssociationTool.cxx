...
namespace Rec {
    ...
    ParticleCaloCellAssociationTool::ParticleCaloCellAssociationTool(const std::string& t, const std::string& n, const IInterface* p) :
        AthAlgTool(t, n, p), m_defaultSelector(0.4) {
    ...
    StatusCode ParticleCaloCellAssociationTool::initialize() {
        ...
        if (!m_cellContainerName.key().empty()) { ATH_CHECK(m_cellContainerName.initialize()); }

        return StatusCode::SUCCESS;
    }

    StatusCode ParticleCaloCellAssociationTool::finalize() { return StatusCode::SUCCESS; }

    std::unique_ptr<ParticleCellAssociation> ParticleCaloCellAssociationTool::particleCellAssociation(
        const xAOD::IParticle& particle, float dr, const CaloCellContainer* container,
        ...
        return std::make_unique<ParticleCellAssociation>(caloExtensionUPtr.release(), std::move(cells), dr, std::move(cellIntersections),
                                                         container);
    }

    ParticleCellAssociation* ParticleCaloCellAssociationTool::particleCellAssociation(const xAOD::IParticle& particle, float dr,
        ...
        if (association == nullptr) { association = particleCellAssociation(particle, dr, container, extensionCache); }
    ...
    void ParticleCaloCellAssociationTool::getCellIntersections(const Trk::CaloExtension& extension,
        ...
        for (const auto *cell : cells) {
            ...
            if (sample == CaloSampling::PreSamplerB) {
                ...
                if (pos2 == entryExitLayerMap.end()) { pos2 = pos; }
            }
            ...
            if (sample == CaloSampling::PreSamplerB || sample == CaloSampling::EMB1 || sample == CaloSampling::EMB2 ||
                sample == CaloSampling::EMB3)
                barrel = true;
            ...
            if (barrel)
                ATH_MSG_VERBOSE(" barrel cell sampling " << cell->caloDDE()->getSampling() << " dr " << cell->caloDDE()->dr() << " drTG "
                                                         << drTG);
            if (!barrel)
                ATH_MSG_VERBOSE(" endcap cell sampling " << cell->caloDDE()->getSampling() << " dz " << cell->caloDDE()->dz() << " dzTG "
                                                         << dzTG);
            ...
            if (drFix == 0.) {
                ...
                if (cell->caloDDE()->deta() > 0) {
                    ...
                    if (theta + dtheta < M_PI) {
                        ...
                        ATH_MSG_VERBOSE(" FIX cell sampling " << cell->caloDDE()->getSampling() << " deta " << cell->caloDDE()->deta()
                                                              << " detaCheck " << detaCheck << " drFix " << drFix);
                    } else {
                        ATH_MSG_WARNING(" FIXR cell sampling failed: theta " << theta << " dtheta " << dtheta << " sum/pi "
                                                                             << (theta + dtheta) * M_1_PI << " deta "
                                                                             << cell->caloDDE()->deta());
                    }
                    //          ATH_MSG_VERBOSE(" FIX cell sampling deta " << deta << " dtheta " << dtheta  << "  scale " <<  scale << "
                    //          theta " << theta );
                } else {
                    double drMin = 100000.;
                    int dscut = 1;
                    ...
                    drFix = drMin;
                    ATH_MSG_VERBOSE(" Problem cell sampling " << cell->caloDDE()->getSampling() << " x " << cell->caloDDE()->x() << " y "
                                                              << cell->caloDDE()->y() << " z " << cell->caloDDE()->z() << " dr "
                                                              << cell->caloDDE()->dr() << " drFix " << drFix << " drTG " << drTG);
                    if (cellFound)
                        ATH_MSG_VERBOSE(" cellFound sampling " << cellFound->caloDDE()->getSampling() << " x " << cellFound->caloDDE()->x()
                                                               << " y " << cellFound->caloDDE()->y() << " z " << cellFound->caloDDE()->z()
                                                               << " dr " << cellFound->caloDDE()->dr() << " dscut " << dscut << " drFix "
                                                               << drFix);
                }
            }
            ...
            if (dzFix == 0.) {
                ...
                if (cell->caloDDE()->deta() > 0) {
                    ...
                    if (theta + dtheta < M_PI) {
                        ...
                        ATH_MSG_WARNING(" FIXZ cell sampling failed: theta " << theta << " dtheta " << dtheta << " sum/pi "
                                                                             << (theta + dtheta) * M_1_PI << " deta "
                                                                             << cell->caloDDE()->deta());
                    }
                    double detaCheck = -log(tan((theta + dtheta) / 2.)) + log(tan((theta) / 2.));
                    ATH_MSG_VERBOSE(" Fix cell sampling " << cell->caloDDE()->getSampling() << " deta " << cell->caloDDE()->deta()
                                                          << " detaCheck  " << detaCheck << " dtheta " << dtheta << " dzFix " << dzFix);
                } else {
                    double dzMin = 100000.;
                    int dscut = 1;
                    if (barrel) dscut = 0;
                    ...
                    dzFix = dzMin;
                    ATH_MSG_VERBOSE(" Problem cell sampling " << cell->caloDDE()->getSampling() << " x " << cell->caloDDE()->x() << " y "
                                                              << cell->caloDDE()->y() << " z " << cell->caloDDE()->z() << " dz "
                                                              << cell->caloDDE()->dz() << " dzFix " << dzFix << " dzTG " << dzTG);
                    if (cellFound)
                        ATH_MSG_VERBOSE(" cellFound sampling " << cellFound->caloDDE()->getSampling() << " x " << cellFound->caloDDE()->x()
                                                               << " y " << cellFound->caloDDE()->y() << " z " << cellFound->caloDDE()->z()
                                                               << " dz " << cellFound->caloDDE()->dz() << " dscut " << dscut << " dzFix "
                                                               << dzFix);
                }
            }
            //
            ...
            if (cell->energy() > 50.)
                ATH_MSG_DEBUG(" cell sampling and size " << cell->caloDDE()->getSampling() << " cell energy " << cell->energy() << " dzFix "
                                                         << dzFix << " dzTG " << dzTG << " drFix " << drFix << " drTG " << drTG
                                                         << " barrel " << barrel);
            ...
            if (use3D) {
                ...
                if (path > 0 || cell->energy() > 50.) {
                    ATH_MSG_DEBUG(" cell sampling and size " << cell->caloDDE()->getSampling() << " cell energy " << cell->energy()
                                                             << " drFix " << drFix << " dzFix " << dzFix << " path " << path
                                                             << " length TG " << totpath);
                    ATH_MSG_DEBUG(" cell dr " << cell->caloDDE()->dr() << " cell dz " << cell->caloDDE()->dz() << " deta "
                                              << cell->caloDDE()->deta());
                }
            }
            ...
            if (eLossPair != eLossLayerMap.end()) {
                ...
                if (sample == CaloSampling::PreSamplerB) {
                    ...
                    if (eLossPair2 != eLossLayerMap.end()) { eLoss = 0.5 * (eLossPair->second) + 0.5 * (eLossPair2->second); }
            ...
            ATH_MSG_DEBUG(" PATH3D = " << path << " PATH2D = " << path2 << " eLoss " << eLoss << " cell energy " << (cell)->energy()
                                       << " radius " << cell->caloDDE()->r() << " phi " << cell->caloDDE()->phi() << " dr "
                                       << cell->caloDDE()->dr() << " dphi " << cell->caloDDE()->dphi() << " x " << cell->caloDDE()->x()
                                       << " y " << cell->caloDDE()->y() << " z " << cell->caloDDE()->z() << " dx " << cell->caloDDE()->dx()
                                       << " dy " << cell->caloDDE()->dy() << " dz " << cell->caloDDE()->dz() << " volume "
                                       << cell->caloDDE()->volume());
    ...
    void ParticleCaloCellAssociationTool::associateCells(const CaloCellContainer& container, const Trk::CaloExtension& caloExtension,
                                                         float dr, std::vector<const CaloCell*>& cells) const {
