...
StatusCode Trk::CaloExtensionBuilderAlg::execute(const EventContext& ctx) const {
    ...
    for (auto track : *tracks) {
        ...
        uint8_t dummy(0);
        uint8_t nSiHits(0);
        if (track->summaryValue(dummy, xAOD::numberOfPixelHits)) { nSiHits += dummy; }
        if (track->summaryValue(dummy, xAOD::numberOfPixelDeadSensors)) { nSiHits += dummy; }
        if (track->summaryValue(dummy, xAOD::numberOfSCTHits)) { nSiHits += dummy; }
        if (track->summaryValue(dummy, xAOD::numberOfSCTDeadSensors)) { nSiHits += dummy; }
        // Cut out TRT alone, but do it for all else
        if (nSiHits < 4) { continue; }
