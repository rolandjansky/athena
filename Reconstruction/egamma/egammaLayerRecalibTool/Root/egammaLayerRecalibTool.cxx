...
const std::string& egammaLayerRecalibTool::resolve_alias(const std::string& tune) const {
  ...
  else if ("ps_2016" == tune)             result = "ps_2016_v1";
...
CP::CorrectionCode egammaLayerRecalibTool::scale_inputs(StdCalibrationInputs & inputs) const
{
  ...
  for (const auto modifier : m_modifiers)  {
    ATH_MSG_DEBUG("applying modifier");
    ATH_MSG_DEBUG("  before E0|E1|E2|E3|run|mu|eta = " << inputs.E0raw << "|" << inputs.E1raw << "|" << inputs.E2raw << "|" << inputs.E3raw <<
		  "|" << inputs.RunNumber << "|" << inputs.averageInteractionsPerCrossing << "|" << inputs.eta);
