...
//
/////////////////////////////////////////////////////////////
#ifndef TopphysicsMonTool_H
#define TopphysicsMonTool_H
...
class TopphysicsMonTool : public IEgammaMonTool {
public:

	TopphysicsMonTool( const std::string & type, const std::string & name,
	                 const IInterface* parent ); 

	virtual ~TopphysicsMonTool();
...
private:
	...
	StatusCode fillHistogramsForItem(const std::string &itemName, const std::string &selectionName);
        ...
        //egammaContainer names for physics and Topphysics
        std::string		m_ElectronCnt;

        //jetContainer for Topphysics
        std::string             m_JetContainer;

        //deltaRcut for Topphysics when identify jet and real electrons
        double m_deltaRcut;
