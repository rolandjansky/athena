...
eflowObjectCreatorTool::eflowObjectCreatorTool(const std::string &type, const std::string &name, const IInterface *parent) : AthAlgTool(type, name, parent),
  ...
  if (theVertexContainer)
  {
    ...
    for (auto theChargedPFO : *m_chargedPFOContainer)
    {
      ...
      if (theTrack)
      {
        ...
        if (!haveSetLink)
          msg(MSG::WARNING) << " Could not set vertex link on charged PFO " << endmsg;
...
void eflowObjectCreatorTool::createChargedEflowObjects(eflowCaloObject *energyFlowCaloObject, bool addClusters)
{
  ...
  for (int iTrack = 0; iTrack < nTracks; ++iTrack)
  {
    ...
    if (!isSet)
    {
      msg(MSG::WARNING) << "Could not set Track B in PFO " << endmsg;
    }
    ...
    if (true == addClusters)
    {
      ...
      for (unsigned int iCluster = 0; iCluster < nClusters; ++iCluster)
      {
        ...
        //theSisterClusterLink.toContainedElement(*m_caloClusterContainer, theSisterCluster);
        bool isSet = myEflowObject->addClusterLink(theSisterClusterLink);
        if (!isSet)
          msg(MSG::WARNING) << "Could not set Cluster in PFO " << endmsg;
      } //cluster loop
...
void eflowObjectCreatorTool::createNeutralEflowObjects(eflowCaloObject *energyFlowCaloObject)
{
  ...
  for (unsigned int iCluster = 0; iCluster < nClusters; ++iCluster)
  {
    ...
    if (0.0 == energyFlowCaloObject->efRecCluster(iCluster)->getCluster()->e())
      continue;
...
void eflowObjectCreatorTool::addMoment(xAOD::CaloCluster::MomentType momentType, xAOD::PFODetails::PFOAttributes pfoAttribute, const xAOD::CaloCluster *theCluster, xAOD::PFO *thePFO)
{
  ...
  if (true == isRetrieved)
  {
    ...
    msg(MSG::WARNING) << " Could not retrieve moment from the CaloCluster " << endmsg;
}
