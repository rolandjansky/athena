...
void  TrackParticleCnv_p3::persToTrans(const Rec::TrackParticle_p3 * persObj, Rec::TrackParticle * transObj, MsgStream &log){
    ...
    transObj->m_trackParameters.resize(indexMax);    
    for (;index<indexMax;index++) {      
      (transObj->m_trackParameters)[index] = createTransFromPStore(&m_parameterCnv,(persObj->m_trackParameters)[index],log);   
      // std::cout<<"For TP "<<transObj<<", \tcreating parameter "<<index<<" at " <<(transObj->m_trackParameters)[index]<<std::endl;
      // This will call virt_createTransFromPStore, which in turn calls createTransient
    }
...
void  TrackParticleCnv_p3::transToPers(const Rec::TrackParticle * transObj, Rec::TrackParticle_p3 * persObj, MsgStream &log){
    ...
    unsigned int index=0, indexMax=transObj->m_trackParameters.size();
    persObj->m_trackParameters.resize(indexMax);    
    for (;index<indexMax;index++) {
      (persObj->m_trackParameters)[index] = toPersistent(&m_parameterCnv,(transObj->m_trackParameters)[index],log);      
    }
