...
StatusCode TauPi0Selector::execute(TauCandidateData *data) 
{
    xAOD::TauJet *tauJet = data->xAODTau;

    // Clear vector of cell-based pi0 PFO Links. Required when rerunning on xAOD level.
    tauJet->clearProtoPi0PFOLinks();
    // Set proto decay mode to "not set". Will be overwritten for taus with 1-5 tracks
    tauJet->setPanTauDetail(xAOD::TauJetParameters::pantau_CellBasedInput_DecayModeProto, xAOD::TauJetParameters::DecayMode::Mode_NotSet);
    ...
    tauJet->setPanTauDetail(xAOD::TauJetParameters::pantau_CellBasedInput_DecayModeProto, xAOD::TauJetParameters::DecayMode::Mode_Other);
    ...
    for(unsigned int iNeutPFO=0; iNeutPFO<nNeutPFO; iNeutPFO++) {
        ...
        if(tauJet->p4().DeltaR(curNeutPFO->p4()) > 0.2) continue; // TODO: Replace by shrinking cone?
        ...
        ElementLink<xAOD::PFOContainer> pfoLink = tauJet->protoNeutralPFOLinks().at(iNeutPFO);
        tauJet->addProtoPi0PFOLink(pfoLink);
    }
    ...
    if(tauJet->nTracks()==1){
      if(nRecoPi0s==0)      tauJet->setPanTauDetail(xAOD::TauJetParameters::pantau_CellBasedInput_DecayModeProto, xAOD::TauJetParameters::DecayMode::Mode_1p0n);
      else if(nRecoPi0s==1) tauJet->setPanTauDetail(xAOD::TauJetParameters::pantau_CellBasedInput_DecayModeProto, xAOD::TauJetParameters::DecayMode::Mode_1p1n);
      else                  tauJet->setPanTauDetail(xAOD::TauJetParameters::pantau_CellBasedInput_DecayModeProto, xAOD::TauJetParameters::DecayMode::Mode_1pXn);
    }
    if(tauJet->nTracks()==3){
      if(nRecoPi0s==0) tauJet->setPanTauDetail(xAOD::TauJetParameters::pantau_CellBasedInput_DecayModeProto, xAOD::TauJetParameters::DecayMode::Mode_3p0n);
      else             tauJet->setPanTauDetail(xAOD::TauJetParameters::pantau_CellBasedInput_DecayModeProto, xAOD::TauJetParameters::DecayMode::Mode_3pXn);
    }
...
TLorentzVector TauPi0Selector::getP4(xAOD::TauJet* tauJet)
{
    // Get 4mom of first charged PFO, which is available for all taus that are treated in this algorithm
    const xAOD::PFO* firstChargedPFO = tauJet->protoChargedPFO( 0 );
    TLorentzVector p4 = firstChargedPFO->p4();
    // Add other PFO momenta
    unsigned nChargedPFO = tauJet->nProtoChargedPFOs();
    for(unsigned int iChargedPFO=1; iChargedPFO<nChargedPFO; iChargedPFO++){
        const xAOD::PFO* curChargedPFO = tauJet->protoChargedPFO( iChargedPFO );
        p4+=curChargedPFO->p4();
    }
    unsigned nPi0NeutPFO = tauJet->nProtoPi0PFOs();
    for(unsigned int iPi0NeutPFO=0; iPi0NeutPFO<nPi0NeutPFO; iPi0NeutPFO++){
        const xAOD::PFO* curPi0NeutPFO = tauJet->protoPi0PFO( iPi0NeutPFO );
        TLorentzVector cluster_corrP4;
        if(tauJet->vertexLink())
          cluster_corrP4 = xAOD::CaloVertexedCluster(*curPi0NeutPFO->cluster(0) , (*tauJet->vertexLink())->position()).p4();
        else
