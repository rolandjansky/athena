...
namespace met {
  ...
  static const SG::AuxElement::ConstAccessor< std::vector<iplink_t > > dec_constitObjLinks("ConstitObjectLinks");
	...
	std::string toolName;
	m_jetcoll = "AntiKt4EMTopoJets";
	toolName = "JERTool_" + m_jetcoll;
	...
	ATH_CHECK(m_jerTool.setProperty("CollectionName", m_jetcoll));
  ...
  StatusCode METSignificance::varianceMET(xAOD::MissingETContainer* metCont, float avgmu, std::string jetTermName, std::string softTermName, std::string totalMETName)
	...
	for(const auto& el : dec_constitObjLinks(*met)) {
	  ...
	  if(obj->type()==xAOD::Type::Muon){
	    ...
	    AddJet(obj, pt_reso, phi_reso, avgmu);
	    metTerm=1;
  ...
  //
  void METSignificance::AddJet(const xAOD::IParticle* obj, float &pt_reso, float &phi_reso, float &avgmu){
