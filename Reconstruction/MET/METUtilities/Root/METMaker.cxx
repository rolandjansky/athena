	...
	double jpx = constjet.Px();
        ...
        if(hardJet){
          ...
          if(metSoftClus && !JVT_reject) {
            ...
            if (fabs(jet->eta())<2.5 || !(coreSoftClus->source()&MissingETBase::Source::Central)) metSoftClus->add(opx,opy,opt);
          }
        }
        if(metSoftTrk && (!hardJet || tracksForHardJets)){
          ...
          else if (fabs(jet->eta())<2.5 || !(coreSoftClus->source()&MissingETBase::Source::Central)) metSoftTrk->add(opx,opy,opt);
        } // soft track
      } // association exists
      else {
        ATH_MSG_WARNING( "WARNING: Jet without association found!" );
      }
      if(tracksForHardJets) m_jetUsedDec(*jet)=isUsedinMET;
    } // jet loop
