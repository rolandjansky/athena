...
namespace met {
  ...
  StatusCode METMaker::initialize()
  {
    ...
    else if (m_jetSelection == "TightJVT04")  { m_CenJetPtCut = 20e3; m_FwdJetPtCut = 30e3; m_JvtCut = 0.4; m_JvtPtMax = 60e3; if(!m_doPFlow) {ATH_MSG_WARNING("Caution: For testing with PFlow collections only!");} }
    else if (m_jetSelection == "TighterJVT04"){ m_CenJetPtCut = 20e3; m_FwdJetPtCut = 35e3;  m_JvtCut = 0.4; m_JvtPtMax = 60e3; if(!m_doPFlow) {ATH_MSG_WARNING("Caution: For testing with PFlow collections only!");} }
    else if (m_jetSelection == "TenaciousJVT421")  {
        m_CenJetPtCut  = 20e3; m_FwdJetPtCut = 35e3;
        m_JvtCutTight  = 0.4; m_JvtTightPtMax  = 40.0e3;
        m_JvtCutMedium = 0.2; m_JvtMediumPtMax = 60.0e3;
        m_JvtCut       = 0.1; m_JvtPtMax = 120e3;
        if(!m_doPFlow) {ATH_MSG_WARNING("Caution: For testing with PFlow collections only!");}
    }
