...
namespace met {
  ...
  StatusCode METMaker::rebuildMET(xAOD::MissingET* met,
	...
	if(assocs.empty()) {
	  ATH_MSG_VERBOSE("Object is not in association map. Did you make a deep copy but fail to set the \"originalObjectLinks\" decoration?");
	  ATH_MSG_VERBOSE("If not, Please apply xAOD::setOriginalObjectLinks() from xAODBase/IParticleHelpers.h");
	  ...
	  if(orig->type()==xAOD::Type::Electron){
	    ...
	    ATH_MSG_VERBOSE("Missing an electron from the MET map. Included as a track in the soft term. pT: " << obj->pt());
	    continue;
	  }else{
