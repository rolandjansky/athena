...
namespace lwt::atlas {
  ...
  typedef FastGraph::NodeMap NodeMap;
  FastGraph::FastGraph(const GraphConfig& config,
                                     std::string default_output):
    m_graph(new Graph(config.nodes, config.layers))
  {
    for (const auto& node: config.inputs) {
      m_preprocs.emplace_back(
        node.name, new FastInputPreprocessor(node.variables));
    }
    for (const auto& node: config.input_sequences) {
      m_vec_preprocs.emplace_back(
        node.name, new FastInputVectorPreprocessor(node.variables));
    }
    size_t output_n = 0;
  ...
  ValueMap FastGraph::compute(const NodeMap& nodes,
  ...
  VectorMap FastGraph::scan(const NodeMap& nodes,
                                     const SeqNodeMap& seq) const {
