...
void
TrackFitter::createMeasurements (State& state, hit_const_iterator begin, hit_const_iterator end) const
{
    state.m_driftHits		= 0;
    ...
    int		hit 	= 0;
    if (state.m_vertexMeasurement) state.m_measurements.push_back(state.m_vertexMeasurement);
	...
	case strip_cluster:
	    ...
	    if ((**h).rot())
	    {
		state.m_measurements.push_back(new Trk::FitMeasurement(hit,
								 *h,
								 (**h).rot()));
	...
	case inert:  	    
	case scatterer:
	    state.m_measurements.push_back(new Trk::FitMeasurement((**h).materialEffects(),
			   ...
			   hit_const_iterator 		end) const
{
    ...
    state.m_parameters =
      new Trk::FitParameters(perigee.transverseImpact(),
                             perigee.z(),
                             perigee.cosPhi(),
                             perigee.sinPhi(),
                             perigee.cotTheta(),
    ...
    state.m_fitCode = 0;
    for (std::vector<Trk::FitMeasurement*>::const_iterator m = state.m_measurements.begin();
	...
	state.m_fitCode = 1;
	state.m_fitProcedureQuality	= std::make_unique<Trk::FitProcedureQuality>(state.m_fitCode,-1);
    }
	...
	state.m_fitProcedureQuality	=
          std::make_unique<Trk::FitProcedureQuality>(m_fitProcedure->execute(false,
								 log,
								 state.m_measurements,
								 state.m_parameters,
								 0,
								 true));
                            ...
                            PerigeeParameters& perigeeParameters) const
{
    ...
    double d3		= -1./(state.m_parameters->sinTheta()*state.m_parameters->sinTheta());
    covariance(3,3)	*= d3;
		    ...
		    const FitQuality&		fitQuality) const
{
    ...
    msg() << std::setiosflags(std::ios::fixed) << std::setw(6)
	  << state.m_planarHits << "(" << std::setw(2) << state.m_driftHits << ") planar(drift) hits "
	  << std::setw(5) << state.m_parameters->numberScatterers() << " scattering parameters "
	  << endmsg;
}

void
TrackFitter::createQuality (const State& state,
                            FitQuality*& fitQuality) const
{
...
TrackFitter::createScatterers (const State& state) const
{
    ...
    if (! state.m_parameters->numberScatterers()) return scattererParameters;
...
void
TrackFitter::setResiduals (const State& state) const
{
			 ...
			 hit_const_iterator      	end) const
{
    ...
    state.m_sigma_pt1			=  scatteringAngle *
    ...
    state.m_sigma_pt3			=  scatteringAngle *
	...
	// 	    double sigma_scat1 = sqrt(state.m_sigma_pt1);
	// 	    double sigma_scat3 = sqrt(state.m_sigma_pt3);
	...
	case broad_pixel:
		...
		sigmaLength = sqrt(state.m_second_sigma_pt1 + sigmaLength*sigmaLength);
	    }
	    state.m_measurements.push_back(new Trk::FitMeasurement(hit,
							     *h,
							     Amg::Vector3D((**h).position()),
...
TrackFitter::initialize()
{
    ...
    m_fitProcedure = std::make_unique<Trk::FitProcedure>(false,
                                                         false,
                                                         false,
			      ...
			      hit_const_iterator 	original_end) const
{
	...
	int 		i;
	for (i = 1; i < state.m_fitProcedureQuality->worstMeasurement(); ++i) ++h;
	bool original = false;
			    ...
			    hit_const_iterator		end) const
{
    ...
    state.m_vertexMeasurement			= new Trk::FitMeasurement(d0,
								  vertex.position(),
								  sigma);
