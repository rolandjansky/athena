...
void eflowObjectCnv_p3::persToTrans( const eflowObject_p3 *persObj, eflowObject *transObj, MsgStream &eflowStream ){
  ...
  ElementLink<Analysis::MuonContainer> muonElementLink;
  m_elMuCnv.persToTrans(&persObj->m_muonElementLink, &muonElementLink, eflowStream);
  transObj->addMuon (muonElementLink);
...
void eflowObjectCnv_p3::transToPers( const eflowObject *transObj,eflowObject_p3 *persObj, MsgStream &eflowStream ){
  ...
  m_elMuCnv.transToPers(&transObj->muonLink(), &persObj->m_muonElementLink, eflowStream);
