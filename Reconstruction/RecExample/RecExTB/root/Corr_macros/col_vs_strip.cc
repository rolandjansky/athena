...
void col_vs_strip(int SaveOpt=0,int Phi=0) {
  ...
  for (Int_t i=0;i<12;i++) {
    ...
    else if (i==2 || i==6 || i==10) sprintf(axisX,"strip (SCT) - [%d,%d,%d]",2,Phi,0);
  ...
  else if(SaveOpt==2) {sprintf(sname,"PixelsSCT_corr0.gif"); c11->Print(sname); }
