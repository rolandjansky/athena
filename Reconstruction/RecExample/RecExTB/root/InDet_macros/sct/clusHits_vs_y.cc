...
void clusHits_vs_y(int SaveOpt=0) {
  ...
  for (int i=0;i<MODPLANES;i++) {
    ...
    sprintf(name2,"clusHits_vs_y[%d,%d,%d]",layer,phi,side);
    histo[i] = new TH1F(name2,name,150,-0.5,60.5);
    ...
    tree->Draw("sct_clus_y",options,"goff");
    ...
    histo[i]->GetXaxis()->SetTitle("Y axis (mm)");
    histo[i]->GetYaxis()->SetTitle("Cluster Hits");
    histo[i]->SetTitle(name);
  }

  c11 = new TCanvas("clusHits_vs_y","SCT Offline Monitor - Cluster Hits vs Y");
  c11->Divide(4,4);
  ...
  else if(SaveOpt==2) {sprintf(sname,"SCTclusHits_vs_y.gif"); c11->Print(sname); }
