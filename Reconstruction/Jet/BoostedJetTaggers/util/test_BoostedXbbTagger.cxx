...
int main( int argc, char* argv[] ) {
  ...
  asg::AnaToolHandle<BoostedXbbTagger> m_Tagger; //!
  ASG_SET_ANA_TOOL_TYPE( m_Tagger, BoostedXbbTagger);
  m_Tagger.setName("MyTagger");
  if(verbose) m_Tagger.setProperty("OutputLevel", MSG::DEBUG);
  m_Tagger.setProperty( "ConfigFile",   "XbbTagger/XbbTagger_AntiKt10LCTopoTrimmed_2BTagTightMass_MC15c_20161118.dat");
  m_Tagger.retrieve();
  ...
  for( Long64_t entry = 0; entry < entries; ++entry ) {
    ...
    for(const xAOD::Jet* jet : * myJets ){
      ...
      if(verbose) std::cout<<"result masslow      = "<<res.getCutResult("PassMassLow" )<<std::endl;
      if(verbose) std::cout<<"result masshigh     = "<<res.getCutResult("PassMassHigh")<<std::endl;
      if(verbose) std::cout<<"result btagging     = "<<res.getCutResult("PassBTag"    )<<std::endl;
      if(verbose) std::cout<<"result jsscut       = "<<res.getCutResult("PassJSS"     )<<std::endl;
