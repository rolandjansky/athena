...
int main( int argc, char* argv[] ) {
  ...
  TFile* outputFile = TFile::Open( "output_SmoothedTopTaggers.root", "recreate" );
  int pass;
  ...
  m_Tagger.setProperty( "ConfigFile",   "SmoothedTopTaggers/SmoothedTopTagger_AntiKt10LCTopoTrimmed_Tau32Split23FixedSignalEfficiency80_MC15c_20161209.dat");
  m_Tagger.retrieve();
  ...
  for( Long64_t entry = 0; entry < entries; ++entry ) {
    ...
    for(const xAOD::Jet* jet : * myJets ){
      ...
      if(verbose) std::cout<<"result tau32pass   = "<<res.getCutResult("PassTau32")<<std::endl;
      if(verbose) std::cout<<"result split23pass = "<<res.getCutResult("PassSplit23")<<std::endl;
