...
int main( int argc, char* argv[] ) {
  ...
  // arguments
  TString fileName = "/eos/atlas/unpledged/group-tokyo/users/tnobe/ForAntonio/DAODs/JetTagging/mc16_13TeV.700441.Sh_2211_Wqq_ptW_200_ECMS.deriv.DAOD_JETM6.e8419_s3126_r10201_p5037/DAOD_JETM6.28568654._000169.pool.root.1";
  int  ievent=-1;
  int  nevents=100;
  bool m_isMC=true;
  bool verbose=true;
  ...
  float sf,pt,eta,m,eff,effSF,sigeffSF, disco;
  ...
  Tree->Branch( "DisCoJet", &disco, "DisCoJet/F" );

  std::unique_ptr<JetUncertaintiesTool> m_jetUncToolSF(new JetUncertaintiesTool(("JetUncProvider_SF")));
  m_jetUncToolSF->setProperty("JetDefinition", "AntiKt10UFOCSSKSoftDropBeta100Zcut10");
  ...
  /*
  ToolHandle<SmoothedWZTagger> m_Tagger {"MyTagger"}; //!
  asg::AsgToolConfig config ("SmoothedWZTagger/MyTagger");
  if(verbose) config.setProperty("OutputLevel", MSG::DEBUG);
  config.setProperty( "CalibArea", "Local");
  config.setProperty( "ConfigFile",   "SmoothedContainedWTagger_AntiKt10VanillaSD_FixedSignalEfficiency50_2VarDisCoJet_v2.dat");
  config.setProperty( "IsMC", m_isMC );
  m_Tagger.retrieve();
  */
  ...
  m_Tagger.setProperty( "ConfigFile",   "SmoothedContainedWTagger_AntiKt10VanillaSD_FixedSignalEfficiency50_2VarDisCoJet_v2.dat");
  ...
  for( Long64_t entry = 0; entry < entries; ++entry ) {
	...
	truthLabel = jetSC->auxdata<int>("R10TruthLabel_R21Consolidated");
      ...
      if(verbose) std::cout<<"result discopass    = "<<res.getCutResult("PassDisCoDNN")<<std::endl;
      ...
      sf = jetSC->auxdata<float>("SmoothWContained50DisCoJet_SF");
      eff = jetSC->auxdata<float>("SmoothWContained50DisCoJet_efficiency");
      effSF = jetSC->auxdata<float>("SmoothWContained50DisCoJet_effSF");
      sigeffSF = jetSC->auxdata<float>("SmoothWContained50DisCoJet_sigeffSF");

      disco = jetSC->auxdata<float>("SmoothWContained50DisCoJet_DisCoJet");
      ...
      if ( m_isMC ){
		...
		std::cout << sysSet.name() << " " << jetSC->auxdata<float>("SmoothWContained50DisCoJet_SF") << std::endl;
	      }
	    }
	}
      }
