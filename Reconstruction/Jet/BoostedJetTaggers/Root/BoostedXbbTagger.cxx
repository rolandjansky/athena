...

{
  ...
  declareProperty( "JetPtMin",              m_jetPtMin = 250.0e3);
  ...
  declareProperty( "MuonPtMin",             m_muonPtMin = 10.0e3);
  declareProperty( "MuonEtaMax",            m_muonEtaMax = 2.5);
  declareProperty( "MuonMatchDR",           m_muonMatchDR = 0.2);

  /// Track jets
  declareProperty( "TrackJetPtMin",         m_trackJetPtMin = 10.0e3);
...
StatusCode BoostedXbbTagger::initialize() {
  ...
  // set up muon tools
  m_muonSelectionTool.setTypeAndName("CP::MuonSelectionTool/" + name() + "MuonSelection");
  ANA_CHECK (m_muonSelectionTool.retrieve());

  m_muonCalibrationAndSmearingTool.setTypeAndName("CP::MuonCalibrationAndSmearingTool/" + name() + "MuonCalibration");
  ...
  if (m_jetSubCutStr != "") {
    ...
    m_jetSubCutTF1 = std::make_unique<TF1>((name() + "SubstructureCut").c_str(), m_jetSubCutStr.c_str(), 0, 14000);
  ...
  m_jetMassMinTF1 = std::make_unique<TF1>((name() + "MassMin").c_str(), m_jetMassMinStr.c_str(), 0, 14000);
  ...
  m_jetMassMaxTF1 = std::make_unique<TF1>((name() + "MassMax").c_str(), m_jetMassMaxStr.c_str(), 0, 14000);
...
Root::TAccept& BoostedXbbTagger::tag( const xAOD::Jet& jet ) const {
  ...
  for(const auto& trackJetEL: associated_trackJets) {
    ...
    if (std::abs(trackJet->eta()) > m_trackJetEtaMax) continue;
...
std::vector<const xAOD::Muon*> BoostedXbbTagger::getCorrectionMuons(const xAOD::Jet& jet) const {
  ...
  std::string accName = name() + "CorrectionMuons";
...
std::vector<const xAOD::Jet*> BoostedXbbTagger::getTrackJets(const xAOD::Jet& jet) const {
  ...
  std::string accName = name() + "TrackJets";
