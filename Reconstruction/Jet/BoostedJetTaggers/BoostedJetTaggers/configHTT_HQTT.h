...
//IJetCalibrationTool* calibTool(const std::string & alg, float r, const std::string & input){
IJetCalibrationTool* calibTool(const std::string & alg, const std::string & radius, const std::string & input){
  ...
  //TString config = "JES_Full2012dataset_Rscan_June2014.config"; //Path to global config used to initialize the tool
...
HEPTopTagger * configHTTTool(const std::string & treename){
  ...
  CAJetsHTTtagged_container += treename.c_str();
...
JetRecTool * buildFullHTTagger(const std::string & treename){
  ...
  TString CAJets_HTT_container = "CA15LCTopoJets_HTT";
  CAJets_HTT_container+= "_"; 
  CAJets_HTT_container += treename.c_str();
