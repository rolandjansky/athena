...
KtDistance* getDistanceScheme(int angle, int collision_type) {
  ...
  else if (angle == 4) return new KtDistanceMass(collision_type);
  else{
...
KtDistanceMass::KtDistanceMass(int collision_type) : m_type(collision_type), m_name("Mass") {}

std::string KtDistanceMass::name() const {return m_name;}

KtFloat KtDistanceMass::operator()(const KtLorentzVector & a) const {
  return (m_type==1) ? -1 : a.perp2(); // If e+e-, no beam remnant, so result will be ignored anyway
}

KtFloat KtDistanceMass::operator()(const KtLorentzVector & a, const KtLorentzVector & b) const {
  KtFloat minE, cosine;
  minE = a.e() > b.e() ? b.e() : a.e();
  cosine = (a.px()*b.px()+a.py()*b.py()+a.pz()*b.pz()) /
    sqrt(a.px()*a.px()+a.py()*a.py()+a.pz()*a.pz()) /
    sqrt(b.px()*b.px()+b.py()*b.py()+b.pz()*b.pz());
  return sqrt( 2 * minE * minE * ( 1 - cosine ) );
}
