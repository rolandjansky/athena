...
class JERProvider : public TNamed
{
 ...
 public: 
  ...
  //DV Testing new scheme
  double getRelJER_nominal_Data(double pt);
  double getRelJER_nominal_MC(double pt);
  double getRelJER_nominal_MC_corrected(double pt);
  double getJER_NP(double pt, int iNP);
 ...
 private:
  ...
  //Needed for final 2012 JER
  map<int,TGraph*> m_jerNP;
  map<int,TGraphErrors*> m_jerMCtruth;
  map<int,TGraph*> m_jerMC_corr;
