...
void JERProvider::setInputCollection(std::string CollectionName, std::string MethodName)
{
  ...
  std::string NPname[m_nNP] = {"_0","_1","_2"};
  
  for(int i=0; i <m_nNP; i++) {
    TString graphName = "grNP" + NPname[i];
    m_jerNP[i] = pullFromFile<TGraph>(graphName);
  }
  
  TString graphName = "g_resolution_eta_0_45_GS";
  m_jerMCtruth[0] = pullFromFile<TGraphErrors>(graphName);
  
  graphName = "MC_corr";
  m_jerMC_corr[0] = pullFromFile<TGraph>(graphName);
...
// Final 2012 JER
//=-=-=-=-=-=-=-=-=-=-
double JERProvider::getRelJER_nominal_Data(double pt)
{
  double N0 = 2.1 ;
  double N1 = 147. ;
  double stohastic = 0.69;
  double constant = 0.026 ;
  ...
  jer_error = pow(N1/pow(pt*m_invGeV, 3) + pow(N0/pt*m_invGeV, 2) + pow(stohastic, 2)/pt*m_invGeV + pow(constant, 2), 0.5);
  
  //std::cout<<" JER_Data("<<pt*m_invGeV<<") = "<<jer_error<<std::endl;
  return jer_error;
}

double JERProvider::getRelJER_nominal_MC(double pt)
{
  ...
  jer_error = m_jerMCtruth[0]->Eval(pt*m_invGeV);
  
  //std::cout<<" JER_MC("<<pt*m_invGeV<<") = "<<jer_error<<std::endl;
  return jer_error;

  
}

double JERProvider::getRelJER_nominal_MC_corrected(double pt)
{
  ...
  jer_error = m_jerMC_corr[0]->Eval(pt*m_invGeV);
...
double JERProvider::getJER_NP(double pt, int iNP)
{

  if (iNP>=m_nNP || iNP<0) fatal(Form("Wrong NP index: %i. It has to be >= 0 or < %i", iNP, m_nNP));
			 

  if (pt*m_invGeV < 10 ) pt=10.0*m_GeV; if (pt*m_invGeV > 2000 ) pt=2000.0*m_GeV;  
  
  return m_jerNP[iNP]->Eval(pt*m_invGeV);
