...
double FFJetSmearingTool::Read3DHistogram(TH3* histo, double x, double y, double z)
{
	...
	double weight = FFJetSmearingTool::My_Interpolate(histo, aux_x, aux_y, aux_z);
...
Int_t FFJetSmearingTool::My_FindBin(const TAxis* axis, const double x)
{
...
double FFJetSmearingTool::Interpolate2D(const TH1* histo, const double x, const double y, const int xAxis, const int yAxis, const int otherDimBin)
{
    ...
    Double_t dx,dy;
    Int_t bin_x = My_FindBin(fXaxis,x);
    Int_t bin_y = My_FindBin(fYaxis,y);
    ...
    Int_t bin_x1 = My_FindBin(fXaxis,x1);
    if(bin_x1<1) bin_x1=1;
    Int_t bin_x2 = My_FindBin(fXaxis,x2);
    if(bin_x2>fXaxis->GetNbins()) bin_x2=fXaxis->GetNbins();
    Int_t bin_y1 = My_FindBin(fYaxis,y1);
    if(bin_y1<1) bin_y1=1;
    Int_t bin_y2 = My_FindBin(fYaxis,y2);
...
double FFJetSmearingTool::My_Interpolate(TH3* histo, double x, double y, double z)
{
    // Copied from ROOT directly and trivially modified, all credit to ROOT authors of TH1, TH2, and TH3 Interpolate methods
    // This is done because I want a const version of interpolation, and none of the methods require modification of the histogram
    // Probable reason is that FindBin isn't const, but there should be a const version...
    ...
    Int_t ubx = My_FindBin(fXaxis,x);
    Int_t uby = My_FindBin(fYaxis,y);
    Int_t ubz = My_FindBin(fZaxis,z);
