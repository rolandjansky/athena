...
int EnergyCorrelatorTool::modifyJet(xAOD::Jet &injet) const {
  if(checkForConstituents(injet) == false) return 1;
  ...
  if (!m_inputContainer.empty()){
	...
	ATH_MSG_WARNING("EnergyCorrelatorTool::InputContainer "<<m_inputContainer<<" unknown. Exiting..");
	return 1;
      }
      jet = buildPseudoJet(AssociatedParticles);
      if (!jet.has_constituents()) {
	return 1;
      }
  }
  else 
    jet = buildPseudoJet(injet);
  ...
  injet.setAttribute(m_inputContainer+"ECF1", ECF1.result(jet));
  injet.setAttribute(m_inputContainer+"ECF2", ECF2.result(jet));
  injet.setAttribute(m_inputContainer+"ECF3", ECF3.result(jet));
  if(m_includeECF4) {
    JetSubStructureUtils::EnergyCorrelator ECF4(4, m_Beta, JetSubStructureUtils::EnergyCorrelator::pt_R);
    injet.setAttribute(m_inputContainer+"ECF4", ECF4.result(jet));
  }
  ...
  if(m_includeBeta2) {
    ...
    injet.setAttribute(m_inputContainer+"ECF1_Beta2", ECF1_beta2.result(jet));
    injet.setAttribute(m_inputContainer+"ECF2_Beta2", ECF2_beta2.result(jet));
    injet.setAttribute(m_inputContainer+"ECF3_Beta2", ECF3_beta2.result(jet));
    if(m_includeECF4) {
      JetSubStructureUtils::EnergyCorrelator ECF4_beta2(4, 2.0, JetSubStructureUtils::EnergyCorrelator::pt_R);
      injet.setAttribute(m_inputContainer+"ECF4_Beta2", ECF4_beta2.result(jet));
    }
  }
