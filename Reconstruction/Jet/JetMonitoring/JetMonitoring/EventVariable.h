...
#ifndef JETMONITORING_EVENTVARIABLE_H
#define JETMONITORING_EVENTVARIABLE_H
#include <vector>
...
/// EventVar::Variable allows access to the variables independently of their type or implementation details. It can then be used in generic tools to fill histograms (JetHistoAttributeFiller) or to select Jets (JetHistoSelectSort).
///
/// EventVar::Variable is the base class providing the acces methods. Concrete classes derive from it, in
///  particular VariableAtt is the generic (templated) class to access atributes stored in the xAOD::Jet object.
/// The function EventVar::Variable::create() allows to dynamicaly instantiate a variable from a name and a type passed as strings.
///
...
namespace EventVar {
  ...
  public:
    ...
    virtual VectorValue vector(const xAOD::EventInfo &) const {return VectorValue();}
  ...
  struct VariableAtt : public Variable {
    ...
    virtual float value(const xAOD::EventInfo & e) const { return m_acc(e)*m_scale;}
    Accessor<T> m_acc;    
  };
  ...
  struct VariableAtt<std::vector<T> > : public Variable {
    ...
    virtual float value(const xAOD::EventInfo & e) const { return m_acc(e)[m_index]*m_scale;}

    virtual VectorValue vector(const xAOD::EventInfo &e) const {
      VectorValue v( new VectorWrapperT(&m_acc(e)) , m_scale ) ;
      return v;
    }
  ...
  // *******************************************
  // The classes below represent common EventInfo variables
  // 
  
  struct ActMuVar : public Variable {
    using Variable::Variable;
    virtual float value(const xAOD::EventInfo & e) const { return e.actualInteractionsPerCrossing();}    
  };

  struct AvgMuVar : public Variable {
    using Variable::Variable;
    virtual float value(const xAOD::EventInfo & e) const { return e.averageInteractionsPerCrossing();}    
  };
