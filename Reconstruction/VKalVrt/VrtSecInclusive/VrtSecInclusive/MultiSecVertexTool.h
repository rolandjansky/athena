...
//
// MultiSecVertexTool.h - Description
//
/*
...
#ifndef _VKalVrt_MultiSecVertexTool_H
#define _VKalVrt_MultiSecVertexTool_H
#include <vector>
#include <memory>
#include <deque>
...
namespace Rec {
  ...
  class MultiSecVertexTool : public AthAlgTool, virtual public IVrtInclusive
  {


  public:
       /* Constructor */
      MultiSecVertexTool(const std::string& type, const std::string& name, const IInterface* parent);
       /* Destructor */
      virtual ~MultiSecVertexTool();
...
//
      ...
      void DisassembleVertex(std::vector<WrkVrt> *WrkVrtSet, int iv, 
                             std::vector<const xAOD::TrackParticle*>  AllTracks) const;
      ...
      StatusCode refitVertex( std::vector<WrkVrt> *WrkVrtSet, int SelectedVertex,
                              std::vector<const xAOD::TrackParticle*> & SelectedTracks, bool ifCovM) const;

      double refitVertex( WrkVrt &Vrt,std::vector<const xAOD::TrackParticle*> & SelectedTracks, bool ifCovM) const;
      ...
      void   mergeAndRefitOverlapVertices( std::vector<WrkVrt> *WrkVrtSet, int V1, int V2,
                                           std::vector<const xAOD::TrackParticle*> & AllTrackLis) const;

      double  improveVertexChi2( std::vector<WrkVrt> *WrkVrtSet, int V, std::vector<const xAOD::TrackParticle*> & AllTracks) const;
