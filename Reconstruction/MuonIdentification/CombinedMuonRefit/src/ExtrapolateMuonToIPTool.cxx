...
ExtrapolateMuonToIPTool::ExtrapolateMuonToIPTool(const std::string& t, const std::string& n,const IInterface* p)  :  
  AthAlgTool(t,n,p),
  m_trackThroughCaloTool("Rec::MuonTrackThroughCalo/MuonTrackThroughCalo"),
...
StatusCode ExtrapolateMuonToIPTool::initialize()
{
  ...
  if(!m_trackThroughCaloTool.empty()){
    if (m_trackThroughCaloTool.retrieve().isFailure()) {
      ATH_MSG_ERROR("Failed to retrieve trackThroughCaloTool ");
      return StatusCode::FAILURE;
    }else {
      ATH_MSG_DEBUG("Retrieved trackThroughCaloTool " << m_trackThroughCaloTool);
    }
  }
...
Trk::Track* ExtrapolateMuonToIPTool::extrapolate(const Trk::Track& track) const {
  ...
  if (msgLvl(MSG::DEBUG)) msg(MSG::DEBUG) << " extrapolating track " << m_printer->print(track) << std::endl
  ...
  const Trk::TrackParameters* ipPars = m_trackThroughCaloTool->extrapolateToPerigee(*closestPars, false, propDir);
  if( !ipPars ){

    // if extrapolation failed go in other direction
    propDir = (propDir == Trk::alongMomentum) ? Trk::oppositeMomentum : Trk::alongMomentum;
    ipPars = m_trackThroughCaloTool->extrapolateToPerigee(*closestPars, false, propDir);
    if( !ipPars){
