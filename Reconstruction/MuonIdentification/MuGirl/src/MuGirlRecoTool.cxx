...
MuGirlRecoTool::print_configuration() const {
  ...
  ATH_MSG_INFO(setw(32) << setfill('.') << setiosflags(ios::left) << m_doANNSelection.name() <<
    m_doANNSelection.value());
  ...
  ATH_MSG_INFO(setw(32) << setfill('.') << setiosflags(
                 ios::left) << m_doHoughTransform.name() << m_doHoughTransform.value());
  ...
  ATH_MSG_INFO(setw(32) << setfill('.') << setiosflags(ios::left) << m_CaloCollection.name() <<
    m_CaloCollection.value());
  ATH_MSG_INFO("Tools:");
  ...
  ATH_MSG_INFO(setw(32) << setfill('.') << setiosflags(
                 ios::left) << m_pParticleCreatorTool.name() << m_pParticleCreatorTool);
  ...
  ATH_MSG_INFO(setw(32) << setfill('.') << setiosflags(
                 ios::left) << m_pMuonLayerHoughTool.name() << m_pMuonLayerHoughTool);
}
...
StatusCode MuGirlRecoTool::initialize() {
  ...
  if (m_pCandidate.retrieve().isFailure()) return StatusCode::RECOVERABLE;
  ...
  ATH_MSG_INFO(
    "Interface id=" << id.id() << ",  majorVersion=" << id.majorVersion() << ",  minorVersion=" << id.minorVersion());
...
StatusCode MuGirlRecoTool::MuGirlReco(const InDetCandidateCollection& InDetSeeds) {
  ...
  if (evtStore()->contains<xAOD::CaloClusterContainer>(m_CaloCollection)) {
    ...
    } else ATH_MSG_DEBUG("Calo Collection Size: " << m_pClusCollection->size());
  }
  ...
  for (InDetCandidateCollection::const_iterator ipIt = InDetSeeds.begin();
    ...
    // FIXME: const_cast
    CaloParticle* pParticle = new CaloParticle(const_cast<MuonCombined::InDetCandidate*>(*ipIt), &tp_id, extrParameters,
                                               pt_calo_local.eta(), pt_calo_local.phi());
  ...
  if (m_doHoughTransform) {
    ...
    if (this->hough_data_size() && m_doNTuple) {
      ...
      if (m_pCandidate->fillNTuple(m_hough_data).isFailure()) ATH_MSG_WARNING(
          "execute: cannot fill the unassociated maxima in the NTuple");
    }
...
bool MuGirlRecoTool::RunFromID(MuGirlNS::CandidateSummaryList& summaryList) {
  ...
  for (unsigned int i = 0; i < m_caloParticles.size(); i++) {
    ...
    ATH_MSG_DEBUG("RunFromID: start processing CaloParticle seed at eta=" << pParticle->eta << " phi=" <<
      pParticle->phi);
    ...
    if (m_pClusCollection && m_doLHR && m_pMuLHR && pParticle->isolated && m_pClusCollection->size() > 0) calculateLHR(
        pParticle);
    else {
    ...
    else if (m_doANNSelection) ANNaccept = generateMuGirl(*summary);
    ...
    if ((m_doANNSelection && ANNaccept) || !(m_doANNSelection)) {
      ...
      if (m_doGlobalFit && (pTrackParticle->trackLink().isValid()) &&
        ...
        ATH_MSG_DEBUG("RunFromID: performing the global track fit.  muonSegemnts.size=" <<
                      (m_pCandidate->vectorMuonSegments()).size()
                                                                                        << " nmdts " << summary->numMdtHits << " nmdtSegs " << summary->numMdtSegs << " nmdtHitsInSeg " <<
          summary->numMdtHitsInSeg);
    ...
    if (doStau && summary->numMdtHits > 4 && (summary->numTgcEtaHits > 2 || summary->numTgcPhiHits > 2 ||
      ...
      if (Beta > 0.2) {
        ...
        if (m_pStauTool->fillStauSummary(summary.get(), stauSummary.get()).isFailure()) ATH_MSG_DEBUG(
            "RunFromID: Cannot fill stauSummary");
        ...
        if (m_pStauTool->fillStauExtras(stauSummary.get(), stauExtras.get()).isFailure()) ATH_MSG_DEBUG(
            "RunFromID: Cannot fill stauExtras");
      ...
      if (m_doNTuple) {
        if (m_pStauTool->fillNTuple().isFailure()) ATH_MSG_DEBUG("RunFromID: stauTool->fillNtuple failed ");
      }
...
bool MuGirlRecoTool::generateMuGirl(MuGirlNS::CandidateSummary& summary) {
  ...
  if (fabs(eta) > 2.0 &&
      (summary.numMdtHits <= 2 && summary.numCscSegs == 0 && summary.numTgcEtaHits <= 2 &&
       summary.numTgcPhiHits <= 2)) return false;

  if (fabs(eta) < 2.0 && summary.numMdtHits <= 2) return false;

  ATH_MSG_DEBUG("generateMuGirl:  eta/hits topology satisfied");

  if ((summary.innAngle == 999.) && (summary.midAngle == 999.)) return false;
  ...
  ATH_MSG_DEBUG(
    " ANN [0] " << value[0] << " ANN [1] " << value[1] << " ANN [2] " << value[2] << " ANN [3] " << value[3] << " ANN [4] " <<
      value[4] << " ANN [5] " << value[5]);
  /*
      ...
      for (unsigned int region = 0; region < sector_data.maxVec.size(); region++) {
        ...
        for (unsigned int maximum = 0; maximum < region_data.size(); maximum++) {
          ...
          ATH_MSG_DEBUG("reference layer:" << ((region > 2) ? "Z=" : "R=") << max_refPos
                                           << ",  maximum position:" <<
                        ((region >
                          2) ? "R=" : "Z=") << max_position << ((region > 2) ? ",  R range: [" : ",  Z range: [")
          ...
          while (it != max->hits.end()) {
            ...
            if ((*it)->prd) {
              ...
              if (msgLvl(MSG::DEBUG)) {
                ...
                ATH_MSG_DEBUG(
                  "Hough Hit has a PrePrawData belonging to chamber " << chamber_name.c_str() << side.c_str());
              }
            } else {
              if ((*it)->tgc) {
                ...
                if (msgLvl(MSG::DEBUG)) {
                  ...
                  ATH_MSG_DEBUG(
                    "Hough Hit has not a PrepRawData but a TgcClusterObj3D belonging to " << chamber_name.c_str() <<
                      side.c_str());
                }
            ...
            /** Old check done when the region index was not handled within the MuonIdhelper package.
               ...
               ATH_MSG_ERROR("Inconsistency between MuGirl distance and Hough transform regLayer: distance=" <<
                  MuGirl_distance
                         << ", regLayer=" << region);
               return StatusCode::FAILURE;
               }
...
void MuGirlRecoTool::doHoughTransformForNtuple(const xAOD::TrackParticle* pTrackParticle,
                                               MuGirlNS::CandidateSummary& summary) {
  ...
  if (m_doHoughTransform) {
    ...
    if (m_pCandidate->fillNTuple(associated_maxima).isFailure()) ATH_MSG_WARNING(
        "RunFromID: Could not include the Hough data into the Candidate NTuple");
  }

  if (m_doNTuple && m_doTruth && pTrackParticle != NULL) {
    ATH_MSG_DEBUG("Calling m_pTruthTool->fillNTuple");

    if (m_pTruthTool->fillNTuple(&summary).isFailure()) ATH_MSG_WARNING("RunFromID: Cannot fill Truth in NTuple");
  }
