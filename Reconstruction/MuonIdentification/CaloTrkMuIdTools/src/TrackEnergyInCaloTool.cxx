...
///////////////////////////////////////////////////////////////////////////////
const Trk::TrackParameters* TrackEnergyInCaloTool::paramInSample(const Trk::Track* trk, const double offset, CaloCell_ID::CaloSample sample,
    ...
    if (surf) {
        param = m_extrapolator->extrapolate(*trk, *surf, Trk::alongMomentum, checkBoundary, particleHypo);
...
// TrackEnergyInCaloTool::paramInSample
///////////////////////////////////////////////////////////////////////////////
const Trk::TrackParameters* TrackEnergyInCaloTool::paramInSample(const Trk::TrackParameters* par, const double offset,
                                                                 const CaloCell_ID::CaloSample sample, const bool checkBoundary,
    ...
    if (par) {
        ...
        if (surf) {
            param = m_extrapolator->extrapolate(*par, *surf, Trk::alongMomentum, checkBoundary, particleHypo);
            if (param)
                ATH_MSG_DEBUG("Extrapolation succesful");
            else
                ATH_MSG_DEBUG("Extrapolation failed");
...
///////////////////////////////////////////////////////////////////////////////
const Trk::TrackParameters* TrackEnergyInCaloTool::paramInNextSample(const Trk::Track* trk, const double offset,
                                                                     CaloCell_ID::CaloSample sample, const bool checkBoundary,
                                                                     Trk::ParticleHypothesis particleHypo) const {
    double trketa = 0.;
    // Take eta of the last measured hit as best guess and create surface :
    const DataVector<const Trk::TrackParameters>* paramvec = trk->trackParameters();
...
// TrackEnergyInCaloTool::paramInNextSample
///////////////////////////////////////////////////////////////////////////////
const Trk::TrackParameters* TrackEnergyInCaloTool::paramInNextSample(const Trk::TrackParameters* par, const double offset,
                                                                     const CaloCell_ID::CaloSample sample, const bool checkBoundary,
                                                                     Trk::ParticleHypothesis particleHypo) const {
    ATH_MSG_DEBUG("in paramInNextSample()");
...
///////////////////////////////////////////////////////////////////////////////
const Trk::TrackParameters* TrackEnergyInCaloTool::paramInLastSample(const Trk::Track* trk, const double offset,
    ...
    if (sample != CaloCell_ID::TileBar2 && sample != CaloCell_ID::TileExt2 && sample != CaloCell_ID::TileGap2 &&
        sample != CaloCell_ID::TileGap3 && sample != CaloCell_ID::HEC3)
        return param;
    ...
    if (trk) {
        if (surf) {
            param = m_extrapolator->extrapolate(*trk, *surf, Trk::alongMomentum, false, particleHypo);
            if (param)
                ATH_MSG_DEBUG("Extrapolation succesful");
            else
                ATH_MSG_DEBUG("Extrapolation failed");
...
// TrackEnergyInCaloTool::paramInLastSample
///////////////////////////////////////////////////////////////////////////////
const Trk::TrackParameters* TrackEnergyInCaloTool::paramInLastSample(const Trk::TrackParameters* par, const double offset,
                                                                     const CaloCell_ID::CaloSample sample,
                                                                     Trk::ParticleHypothesis particleHypo) const {
    ATH_MSG_DEBUG("in paramInLastSample()");
    ...
    if (par) {
        ...
        if (surf) {
            param = m_extrapolator->extrapolate(*par, *surf, Trk::alongMomentum, false, particleHypo);
            if (param)
                ATH_MSG_DEBUG("Extrapolation succesful");
            else
                ATH_MSG_DEBUG("Extrapolation failed");
...
///////////////////////////////////////////////////////////////////////////////
const Trk::TrackParameters* TrackEnergyInCaloTool::paramAfterGirder(const Trk::Track* trk, Trk::ParticleHypothesis particleHypo) const {
    ATH_MSG_DEBUG("in paramAfterGirder");
    Trk::Surface* surf = m_calosurf->CreateGirderSurface();
    const Trk::TrackParameters* param = m_extrapolator->extrapolate(*trk, *surf, Trk::alongMomentum, false, particleHypo);
    if (param)
        ATH_MSG_DEBUG("Extrapolation succesful");
    else
        ATH_MSG_DEBUG("Extrapolation failed");
...
const Trk::TrackParameters* TrackEnergyInCaloTool::paramAfterGirder(const Trk::TrackParameters* par,
    ...
    ATH_MSG_DEBUG("in paramAfterGirder");
    Trk::Surface* surf = m_calosurf->CreateGirderSurface();
    const Trk::TrackParameters* param = m_extrapolator->extrapolate(*par, *surf, Trk::alongMomentum, false, particleHypo);
    if (param)
        ATH_MSG_DEBUG("Extrapolation succesful");
    else
        ATH_MSG_DEBUG("Extrapolation failed");
...
///////////////////////////////////////////////////////////////////////////////
std::vector<CaloPair> TrackEnergyInCaloTool::paramInAllSamples(const Trk::Track* trk, const double offset, const bool checkBoundary,
                                                               Trk::ParticleHypothesis particleHypo) const {
    ...
    sample = getSample(eta, layer);
    if (sample == CaloCell_ID::TileBar2 || sample == CaloCell_ID::TileGap2 || sample == CaloCell_ID::TileGap3 ||
...
std::vector<CaloPair> TrackEnergyInCaloTool::paramInAllSamples(const Trk::TrackParameters* param, const double offset,
                                                               const bool checkBoundary, Trk::ParticleHypothesis particleHypo) const {
    ATH_MSG_DEBUG("in paramInAllSamples");
...
double TrackEnergyInCaloTool::etamax(const CaloCell_ID::CaloSample sample, const bool isTile) const {
    ...
    for (const CaloDetDescriptor* descr : isTile ? m_calo_dd->tile_descriptors_range() : m_calo_dd->calo_descriptors_range()) {
        if (descr) {
...
///////////////////////////////////////////////////////////////////////////////
double TrackEnergyInCaloTool::energy(const Trk::TrackParameters* param, Trk::ParticleHypothesis particleHypo) const {
...
CaloCell_ID::CaloSample TrackEnergyInCaloTool::getNextSample(const CaloCell_ID::CaloSample sample, double eta) const {
    ...
    switch (sample) {
        ...
        case (CaloCell_ID::TileExt0):
            if (eta < etamax(CaloCell_ID::TileExt1, true)) return CaloCell_ID::TileExt1;
            break;

        case (CaloCell_ID::TileExt1):
            if (eta < etamax(CaloCell_ID::TileExt2, true)) return CaloCell_ID::TileExt2;
            break;
        ...
        default: return CaloCell_ID::Unknown;
    }
...
CaloCell_ID::CaloSample TrackEnergyInCaloTool::getSample(double eta, const int layer) const {
    ...
    switch (layer) {
        ...
        case 3:  // HEC0
            if (eta > etamin(CaloCell_ID::HEC0, false)) sample = CaloCell_ID::HEC0;
            break;
