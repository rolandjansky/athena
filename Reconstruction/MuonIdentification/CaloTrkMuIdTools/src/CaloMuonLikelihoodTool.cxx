...
StatusCode CaloMuonLikelihoodTool::initialize() {
    ...
    ATH_CHECK(retrieveHistograms(fileNames));
...
///////////////////////////////////////////////////////////////////////////////
StatusCode CaloMuonLikelihoodTool::retrieveHistograms(const std::vector<std::string>& file_names) {
    ATH_MSG_DEBUG("in CaloMuonLikelihoodTool::retrieveHistograms()");

    for (int iFile = 0; iFile < 9; iFile++) {
        m_numKeys[iFile] = 0;
        const std::string& fileName = file_names[iFile];
        ...
        for (int iHist = 0; iHist < listOfKeys->GetSize(); iHist++) {
            ...
            /// Dettach the histogram from the file memory-wise
            std::unique_ptr<TH1> hist_ptr{hist};
            ...
            if (isSignal) {
                ...
                m_TH1F_sig[iFile][numKeysSignal] = std::move(hist_ptr);
                ATH_MSG_VERBOSE(m_TH1F_sig[iFile][numKeysSignal]->GetNbinsX());
                ++numKeysSignal;
            } else {
                m_TH1F_bkg[iFile][numKeysBkg] = std::move(hist_ptr);
                ++numKeysBkg;
            }
        }
...
double CaloMuonLikelihoodTool::getLHR(const xAOD::TrackParticle* trk, const xAOD::CaloClusterContainer* ClusContainer,
    ...
    if (trk && ClusContainer) {
        ...
        const double eta_trkAtCalo = caloEntryInterSec->eta();
        const double phi_trkAtCalo = caloEntryInterSec->parameters()[Trk::phi];
...
double CaloMuonLikelihoodTool::getLHR(const xAOD::CaloClusterContainer* ClusCollection, const double eta_trk, const double p_trk,
    ...
    for (const xAOD::CaloCluster* theClus : *ClusCollection) {
        const double dphi = xAOD::P4Helpers::deltaPhi(theClus->phi(), phi_trkAtCalo);
