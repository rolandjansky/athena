...
public:
    ...
    */
    const Trk::TrackParameters* paramInSample(const Trk::Track* trk, const double offset, CaloCell_ID::CaloSample sample,
                                              const bool checkBoundary, Trk::ParticleHypothesis particleHypo) const;

    /**
        Extrapolate to a given sample using trackParameters as input
    */
    const Trk::TrackParameters* paramInSample(const Trk::TrackParameters* par, const double offset, CaloCell_ID::CaloSample sample,
                                              const bool checkBoundary, Trk::ParticleHypothesis particleHypo) const;

    /**
        Extrapolate to a next sample using track as input
    */
    const Trk::TrackParameters* paramInNextSample(const Trk::Track* trk, const double offset, CaloCell_ID::CaloSample sample,
                                                  const bool checkBoundary, Trk::ParticleHypothesis particleHypo) const;

    /**
        Extrapolate to a next  sample using trackParameters as input
    */
    const Trk::TrackParameters* paramInNextSample(const Trk::TrackParameters* par, const double offset, CaloCell_ID::CaloSample sample,
                                                  const bool checkBoundary, Trk::ParticleHypothesis particleHypo) const;

    /**
        Extrapolate to a last surface in calo using track as input
    */
    const Trk::TrackParameters* paramInLastSample(const Trk::Track* trk, const double offset, CaloCell_ID::CaloSample sample,
                                                  Trk::ParticleHypothesis particleHypo) const;
    /**
        Extrapolate to a last surface in calo using trackParameters as input
    */
    const Trk::TrackParameters* paramInLastSample(const Trk::TrackParameters* par, const double offset, CaloCell_ID::CaloSample sample,
                                                  Trk::ParticleHypothesis particleHypo) const;
    /**
        Extrapolate to the surface after the Girder surface in calo using track as input
    */
    const Trk::TrackParameters* paramAfterGirder(const Trk::Track* trk, Trk::ParticleHypothesis particleHypo) const;
    /**
         Extrapolate to the surface after the Girder surface in calo using trackParameters as input
     */
    const Trk::TrackParameters* paramAfterGirder(const Trk::TrackParameters* par, Trk::ParticleHypothesis particleHypo) const;
    /**
        Extrapolate to the all surface and fills CaloPair track as input
    */
    std::vector<CaloPair> paramInAllSamples(const Trk::Track* trk, const double offset, const bool checkBoundary,
                                            Trk::ParticleHypothesis particleHypo) const;
    /**
        Extrapolate to the all surface and fills CaloPair trackParameters as input
    */
    std::vector<CaloPair> paramInAllSamples(const Trk::TrackParameters* par, const double offset, const bool checkBoundary,
                                            Trk::ParticleHypothesis particleHypo) const;

    /**
        get max eta of the sample to extrapolate to
    */
    double etamax(const CaloCell_ID::CaloSample sample, const bool isTile) const;
    /**
        get min eta of the sample to extrapolate to
    */
    double etamin(const CaloCell_ID::CaloSample sample, const bool isTile) const;
    /**
        Compute parametrised energy Loss
    */
    double energy(const Trk::TrackParameters* par, Trk::ParticleHypothesis particleHypo) const;
    /**
...
private:
    ...
    ToolHandle<Trk::IExtrapolator> m_extrapolator{this, "ExtrapolatorHandle", "Trk::Extrapolator/AtlasExtrapolator",
