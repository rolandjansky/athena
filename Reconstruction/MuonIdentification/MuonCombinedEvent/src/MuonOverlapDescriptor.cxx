...
Rec::MuonOverlapDescriptor::MuonOverlapDescriptor(bool SharesIDTrack, bool SharesSpectroTrack, unsigned int SharedPrecisionHits,
                                                  unsigned int TotalPrecisionHits, unsigned int SharedSpectroPhiHits,
                                                  unsigned int TotalSpectroPhiHits, std::set<Identifier> msIntersection)
...
// if namespace, need to use MsgStream& Rec::operator :-(
{
    if (itself.hasOverlap()) {
        ...
        sl << "ID pointers: " << (itself.sharesIndetTrack() ? "identical" : "different")
           << ",  MS pointers: " << (itself.sharesSpectroTrack() ? "identical" : "different") << endmsg;
        sl << "MS hits: " << itself.nSharedPrecisionHits() << " shared prec. hits of " << itself.nTotalPrecisionHits() << " total."
           << endmsg;
        sl << "         " << itself.nSharedSpectroPhiHits() << " shared phi hits of " << itself.nTotalSpectroPhiHits() << " totoal."
           << endmsg;
    } else
        sl << "Muon overlap desriptor: no Overlap." << endmsg;
    return sl;
}
...
// if namespace, need to use MsgStream& Rec::operator :-(
{
    if (itself.hasOverlap()) {
        ...
        sl << "ID pointers: " << (itself.sharesIndetTrack() ? "identical" : "different")
           << ",  MS pointers: " << (itself.sharesSpectroTrack() ? "identical" : "different") << std::endl;
        sl << "MS hits: " << itself.nSharedPrecisionHits() << " shared prec. hits of " << itself.nTotalPrecisionHits() << " total."
           << std::endl;
        sl << "         " << itself.nSharedSpectroPhiHits() << " shared phi hits of " << itself.nTotalSpectroPhiHits() << " totoal."
           << std::endl;
    } else
        sl << "Muon overlap desriptor: no Overlap." << std::endl;
