...
//========================================
StatusCode CandidateTool::fillBetaRefitTrack(int iTrack, const xAOD::TrackParticle* lowBetaTrack, const Trk::Track* trkLowBetaTrack,bool bSaveMdtSegmentMakerInfo,bool has_combined)
{     
        if (msgLvl(MSG::DEBUG)) msg() << "in CandidateTool::fillBetaRefit" << endreq;
        if (msgLvl(MSG::DEBUG)) msg() << "candidate has combined track: " << has_combined<< endreq;
    ...
    if(lowBetaTrack!=NULL) m_pCandidate->setMuonBetaRefitTrack(lowBetaTrack);     /** Old running schema, using TrackParticle */
    else                   m_pCandidate->setMuonBetaRefitTrack(trkLowBetaTrack);  /** New running schema, using Trk::Track */
...
StatusCode CandidateTool::fillMF(int iFeature, const MuonFeature* muonFeature, bool bSaveMdtSegmentMakerInfo,bool has_combined)
{    
        if (msgLvl(MSG::DEBUG)) msg() << "in CandidateTool::fillMF" << endreq;
    ...
    {   
        ...
        if (msgLvl(MSG::DEBUG)) msg() << "CandidateTool::fillMF - Calling m_technologies["
           << TechnologyTypeName((TechnologyType)iTech) << "]->retrievePrepData()" << endreq;
        if (m_technologies[iTech]->retrievePrepData().isFailure())
            continue;
    }

    m_pCandidate->setCombinedMF(has_combined);
    m_pCandidate->clear();
    m_pCandidate->setSaveMdtSegmentMakerInfo(bSaveMdtSegmentMakerInfo);
    //m_pCandidate->setIsolation(0);
    m_pCandidate->setMuonFeature(muonFeature);
    ...
    if (m_doTGC)
    {
        m_pCandidate->buildSegments(TGC_TECH, MIDDLE_DIST, ENDCAP_REGION, ID_TECH);
    ...
    if (m_doRPC)
    {
        m_pCandidate->buildSegments(RPC_TECH, MIDDLE_DIST, BARREL_REGION, ID_TECH);
   ...
   // Inner
    if (m_doTGC)
    {
        m_pCandidate->buildSegments(TGC_TECH, INNER_DIST, ENDCAP_REGION, COMB_TECH); 
        m_pCandidate->computeChamberIntersections(TGC_TECH, MIDDLE_DIST, INNER_DIST, ENDCAP_REGION);
    }
    if (m_doCSC)
    {
        m_pCandidate->buildSegments(CSC_TECH, INNER_DIST, ENDCAP_REGION, COMB_TECH); 
        m_pCandidate->computeChamberIntersections(CSC_TECH, MIDDLE_DIST, INNER_DIST, ENDCAP_REGION);
    }
    if (m_doMDT)
    {
        m_pCandidate->buildSegments(MDT_TECH, INNER_DIST, BARREL_REGION, COMB_TECH); 
        m_pCandidate->computeChamberIntersections(MDT_TECH, MIDDLE_DIST, INNER_DIST, BARREL_REGION);  
        ...
        m_pCandidate->computeChamberIntersections(MDT_TECH, MIDDLE_DIST, INNER_DIST, ENDCAP_REGION);
...
//===========================================================================================
StatusCode CandidateTool::fillEFTrack(int iFeature, const TrigMuonEFTrack* muonEFTrack, bool bSaveMdtSegmentMakerInfo,bool has_combined)
{    
        if (msgLvl(MSG::DEBUG)) msg() << "in CandidateTool::fillEFTrack" << endreq;
    ...
    {   
        TechnologyType eTech = (TechnologyType)iTech;
        if (!m_doCSC  && eTech  == CSC_TECH) continue; 
        if (msgLvl(MSG::DEBUG)) msg() << "CandidateTool::fillEFTrack - Calling m_technologies["
           << TechnologyTypeName((TechnologyType)iTech) << "]->retrievePrepData()" << endreq;
        if (m_technologies[iTech]->retrievePrepData().isFailure())
            continue;
    }
    ...
    m_pCandidate->setSaveMdtSegmentMakerInfo(bSaveMdtSegmentMakerInfo);
    //m_pCandidate->setIsolation(0);
    m_pCandidate->setMuonEFTrack(muonEFTrack);
    StatusCode sc = m_pCandidate->crudeExtrapolate();
    if(sc != StatusCode::SUCCESS) return StatusCode::RECOVERABLE;
    m_pCandidate->selectChambers();
