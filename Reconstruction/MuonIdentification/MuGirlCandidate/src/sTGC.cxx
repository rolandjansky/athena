...
#include "MuonReadoutGeometry/sTgcReadoutElement.h"  // needed because sTgcPrepData is not included by the MuonPrepDataContainer.h
...
sTGC::sTGC(CandidateTool* pMuGirl, const std::string& sPrepDataCollection) :
        ...
        m_pPrepDataContainer(NULL)
{
    ...
    if(m_pIdHelper == 0)
      m_pMuGirl->msg(MSG::ERROR) << "IdHelper should be sTgcIdHelper, but it is NOT!" << endmsg;
}

const MuonGM::MuonReadoutElement* sTGC::readoutElement(const Identifier& id) const
{
    return m_pMuGirl->muonManager()->getsTgcReadoutElement(id);
}
...
{
    if (m_pMuGirl->evtStore()->contains<Muon::sTgcPrepDataContainer>(m_sPrepDataCollection))
    {
       ...
       if (sc.isFailure() || m_pPrepDataContainer == NULL)
       {
           ...
           if (m_pMuGirl->msgLvl(MSG::DEBUG))
               m_pMuGirl->msg(MSG::DEBUG) << "Cannot retrieve sTGC PrepData Container " << m_sPrepDataCollection << endmsg;
       }
    ...
    else
    {
        m_pPrepDataContainer=NULL;
        if (m_pMuGirl->msgLvl(MSG::DEBUG))
            m_pMuGirl->msg(MSG::DEBUG) << "EventStore does not contain sTGC PrepData Container " << m_sPrepDataCollection << endmsg;
    }
    return StatusCode::SUCCESS;
}

unsigned sTGC::prepData(Chamber* pChamber, PrepDataList& array)
{
    array.clear();

    if (m_pPrepDataContainer == NULL)
    {
        m_pMuGirl->msg(MSG::INFO) << "Cannot retrieve sTGC PrepData Container " << m_sPrepDataCollection << endmsg;
        return 0;
    ...
    if (m_pPrepDataContainer != NULL)
    {
        Muon::sTgcPrepDataContainer::const_iterator itColl = m_pPrepDataContainer->indexFind(pChamber->hashId());
        ...
        if (itColl == m_pPrepDataContainer->end())
        {
            ...
            m_pMuGirl->msg(MSG::WARNING) << "sTGC chamber hash ID " << pChamber->hashId() 
                     << " may be invalid: no collection found into the sTGCPrepDataContainer" << endmsg;
        ...
        if (itColl != m_pPrepDataContainer->end())
        {
            ...
            const Muon::sTgcPrepDataCollection* pColl = *itColl;
            //const sTgcIdHelper * helper = dynamic_cast<const sTgcIdHelper*>(m_pIdHelper);
            for (const Muon::sTgcPrepData* prd : *pColl)
            {
                //if ( helper->channelType(prd->identify())==1 ) 
                array.push_back(prd);  // collecting sTgc strip, wires and PADs  hit
            }
        }
     }
    return array.size();
...
std::vector<IdentifierHash> sTGC::retrieveAvailableCollections() const {
    ...
    if (m_pPrepDataContainer != NULL)
    {
        Muon::sTgcPrepDataContainer::const_iterator itColl;
...
Amg::Vector3D sTGC::hitPosition(const Trk::PrepRawData* pPrepData)
{
    const Muon::sTgcPrepData* psTgcPrepData = dynamic_cast<const Muon::sTgcPrepData*>(pPrepData);
    if (psTgcPrepData == NULL)
    {
        m_pMuGirl->msg(MSG::WARNING) << "Cannot convert from Trk::PrepRawData* to Muon::sTgcPrepData*" << endmsg;
        return Amg::Vector3D();
    }

    const MuonGM::sTgcReadoutElement* pReadoutElement = psTgcPrepData->detectorElement();
    if (pReadoutElement == NULL)
    {
        m_pMuGirl->msg(MSG::WARNING) << "Cannot get sTgcReadoutElement" << endmsg;
        return Amg::Vector3D();
    }
