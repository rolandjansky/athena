...
MuTagAmbiguitySolverTool::MuTagAmbiguitySolverTool(const std::string& t, const std::string& n, const IInterface* p)
    : AthAlgTool(t, n, p)
{
...
std::vector<MuonCombined::MuonSegmentInfo>
MuTagAmbiguitySolverTool::solveAmbiguities(std::vector<MuonCombined::MuonSegmentInfo> mtos) const
{
    ...
    for (unsigned int ns1 = 0; ns1 < mtos.size(); ns1++) {
        ATH_MSG_DEBUG(" index " << ns1 << " nsegments " << mtos[ns1].nsegments << " track pointer " << mtos[ns1].track
        ...
        ATH_MSG_DEBUG(" Updated index " << ns1 << " nsegments " << mtos[ns1].nsegments << " track pointer "
    ...
    for (unsigned int ns1 = 0; ns1 < mtos.size(); ns1++) {
        ATH_MSG_DEBUG(" First pass  index " << ns1 << " nsegments " << mtos[ns1].nsegments << " track pointer "
    ...
    for (unsigned int ns1 = 0; ns1 < mtos.size(); ns1++) {
        ...
        ATH_MSG_DEBUG(" Second pass index " << ns1 << " nsegments " << mtos[ns1].nsegments << " track pointer "
                                            << mtos[ns1].track << " stationLayer " << mtos[ns1].stationLayer
                                            << " selected " << mtos[ns1].selected);
        ATH_MSG_DEBUG(" ns1 " << ns1 << " nsegments " << nsegments);
    }
    ...
    for (unsigned int ns1 = 0; ns1 < mtos.size(); ns1++) {
        ...
        if (mtos[ns1].nsegments == 1 && mtos[ns1].stationLayer == 21) {
            mtos[ns1].selected  = 0;
            mtos[ns1].nsegments = 0;
            ATH_MSG_DEBUG(" DROP CSC index " << ns1 << " nsegments " << mtos[ns1].nsegments << " track pointer "
                                             << mtos[ns1].track << " stationLayer" << mtos[ns1].stationLayer
                                             << " selected " << mtos[ns1].selected);
        }
    }
    ...
    for (unsigned int ns1 = 0; ns1 < mtos.size(); ns1++) {
        if (mtos[ns1].nsegments == 1 && mtos[ns1].nholes > 2) {
            mtos[ns1].selected  = 0;
            mtos[ns1].nsegments = 0;
            ATH_MSG_DEBUG(" DROP HOLES index " << ns1 << " nsegments " << mtos[ns1].nsegments << " track pointer "
                                               << mtos[ns1].track << " stationLayer" << mtos[ns1].stationLayer
                                               << " selected " << mtos[ns1].selected);
        }
    }
    ...
    if (m_rejectMatchPhi) {
        for (unsigned int ns1 = 0; ns1 < mtos.size(); ns1++) {
            if (mtos[ns1].nsegments == 1 && mtos[ns1].minimumPullPhi > 3 && mtos[ns1].hasPhi > 0) {
                mtos[ns1].selected  = 0;
                mtos[ns1].nsegments = 0;
                ATH_MSG_DEBUG(" DROP MatchPhi index " << ns1 << " nsegments " << mtos[ns1].nsegments
                                                      << " track pointer " << mtos[ns1].track << " stationLayer"
    ...
    if (multiplicity > 50) {
        for (unsigned int ns1 = 0; ns1 < mtos.size(); ns1++) {
            if (mtos[ns1].nholes > 1) {
                mtos[ns1].selected  = 0;
                mtos[ns1].nsegments = 0;
                ATH_MSG_DEBUG(" DROP multiplicity 50 index "
                              << ns1 << " nsegments " << mtos[ns1].nsegments << " track pointer " << mtos[ns1].track
                              << " stationLayer" << mtos[ns1].stationLayer << " selected " << mtos[ns1].selected);
            }
        }
        ...
        for (unsigned int ns1 = 0; ns1 < mtos.size(); ns1++) {
            if (mtos[ns1].nholes > 2) {
                mtos[ns1].selected  = 0;
                mtos[ns1].nsegments = 0;
                ATH_MSG_DEBUG(" DROP multiplicity 30 index "
                              << ns1 << " nsegments " << mtos[ns1].nsegments << " track pointer " << mtos[ns1].track
                              << " stationLayer" << mtos[ns1].stationLayer << " selected " << mtos[ns1].selected);
            }
        }
    ...
    for (unsigned int ns1 = 0; ns1 < mtos.size(); ns1++) {
        if (mtos[ns1].nsegments == 1 && mtos[ns1].nholes > 0 && mtos[ns1].singleML == 1) {
            mtos[ns1].selected  = 0;
            mtos[ns1].nsegments = 0;
            ATH_MSG_DEBUG(" DROP single multilayers single tag "
                          << ns1 << " nsegments " << mtos[ns1].nsegments << " track pointer " << mtos[ns1].track
                          << " stationLayer" << mtos[ns1].stationLayer << " selected " << mtos[ns1].selected);
        }
    }
    ...
    for (unsigned int ns1 = 0; ns1 < mtos.size(); ns1++) {
        ...
        if (pT < pTcut) {
            if (mtos[ns1].nsegments == 1 && mtos[ns1].singleML == 1) {
                mtos[ns1].selected  = 0;
                mtos[ns1].nsegments = 0;
                ATH_MSG_DEBUG(" DROP single multilayers low pT "
                              << ns1 << " nsegments " << mtos[ns1].nsegments << " track pointer " << mtos[ns1].track
                              << " stationLayer" << mtos[ns1].stationLayer << " selected " << mtos[ns1].selected);
            }
        }
    ...
    if (m_rejectOuterEndcap) {
        for (unsigned int ns1 = 0; ns1 < mtos.size(); ns1++) {
            if (mtos[ns1].nsegments == 1 && mtos[ns1].stationLayer == 13) {
                mtos[ns1].selected  = 0;
                mtos[ns1].nsegments = 0;
                ATH_MSG_DEBUG(" DROP single tag OuterEndcap "
                              << ns1 << " nsegments " << mtos[ns1].nsegments << " track pointer " << mtos[ns1].track
                              << " stationLayer" << mtos[ns1].stationLayer << " selected " << mtos[ns1].selected);
            }
        }
...
int
MuTagAmbiguitySolverTool::ambiguousSegment(const Muon::MuonSegment& seg1, const Muon::MuonSegment& seg2) const
{
        ...
        if (compareSegments.intersectionSize > 0) {
            ...
            if (m_hitOverlapMatching) {
                if (msgLvl(MSG::VERBOSE))
                    msg(MSG::VERBOSE) << " found overlapping segment pair: " << compareSegments.print(result)
                                      << std::endl
                                      << p_muonPrinter->print(seg1) << std::endl
...
std::vector<MuonCombined::MuonSegmentInfo>
MuTagAmbiguitySolverTool::selectBestMuTaggedSegments(std::vector<MuonCombined::MuonSegmentInfo> mtss) const
{
        ...
        for (unsigned int mts2 = mts1 + 1; mts2 < mtss.size(); ++mts2) {
            ...
            if (ambiguousSegment(*museg1, *museg2)) {
                if (museg1->numberOfContainedROTs() > museg2->numberOfContainedROTs() - 1) {
                    ATH_MSG_DEBUG("segments in the same station with rots keep First "
                                  << museg1->numberOfContainedROTs() << " and " << museg2->numberOfContainedROTs());
                    accept[mts2] = false;
                } else if (museg1->numberOfContainedROTs() < museg2->numberOfContainedROTs() - 1) {
                    ATH_MSG_DEBUG("segments in the same station with rots keep Second "
                                  << museg1->numberOfContainedROTs() << " and " << museg2->numberOfContainedROTs());
                    accept[mts1] = false;
                } else {
                    ...
                    if (std::abs(chi2mts1) < std::abs(chi2mts2)) {
                        ATH_MSG_DEBUG("dropping mts " << mts2 << " since he has a larger MatchLocY: " << chi2mts2
                                                      << " vs " << chi2mts1);
                        accept[mts2] = false;
                    }
                    if (std::abs(chi2mts1) > std::abs(chi2mts2)) {
                        ATH_MSG_DEBUG("dropping mts " << mts1 << " since he has a larger MatchLocY: " << chi2mts1
                                                      << " vs " << chi2mts2);
                        accept[mts1] = false;
                    }
                }
...
double
MuTagAmbiguitySolverTool::Rseg(unsigned int nseg) const
{
