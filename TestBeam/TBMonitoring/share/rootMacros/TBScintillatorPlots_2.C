...
void TBScintillatorPlots_2(TString path, TString canvasTitle, TString canvasNameBase, int nx=2, int ny=2, int Flag=99) {
  ...
  while (key = (TKey*)myIter.Next()) {
    ...
    if (Flag==0) { // Raw==0 or TDC==1  
      if (Index%2==0) { // if_even
    ...
    } // end of flag choice==0

    if (Flag==1) { // Raw==0 or TDC==1  
      if (Index%2!=0) { // if_odd
	// pad and canvas management
	if (padIndex%(nx*ny) == 0) {
	  padIndex = 0;
