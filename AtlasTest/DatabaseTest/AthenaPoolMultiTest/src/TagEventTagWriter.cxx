...
/**
 * @file TagEventTagWriter.cxx 
 * @brief Implementation of class TagEventTagWriter
 * @author K. Karr (Kristo.Karr@cern.ch)
 * $Id: TagEventTagWriter.cxx,v 1.5 2009-02-19 22:04:49 cranshaw Exp $
 */
...
TagEventTagWriter::~TagEventTagWriter() 
{
  if ( m_attribListSpec )
  {
...
StatusCode TagEventTagWriter::initialize() 
{
  ...
  m_attribListSpec->extend("TestShort", AthenaAttributeType("short") );
  m_attribListSpec->extend("TestUShort", AthenaAttributeType("unsigned short","NUMBER","TEST") );
  m_attribListSpec->extend("TestInt", AthenaAttributeType("int","NUMBER","TEST") );
  ...
  m_attribListSpec->extend("RunNumber",   AthenaAttributeType("unsigned int","NUMBER","EVENT") );
  m_attribListSpec->extend("EventNumber", AthenaAttributeType("unsigned int","NUMBER","EVENT") );
  m_attribListSpec->extend("Luminosity",  AthenaAttributeType("float"         ,"NB(-1)","VTX") );
  m_attribListSpec->extend("GoodVertex",  AthenaAttributeType("bool"          ,"NONE"  ,"VTX") );
  m_attribListSpec->extend("XVertex",     AthenaAttributeType("float"         ,"CM"    ,"VTX") );
  m_attribListSpec->extend("YVertex",     AthenaAttributeType("float"         ,"CM"    ,"VTX") );
  m_attribListSpec->extend("ZVertex",     AthenaAttributeType("float"         ,"CM"    ,"VTX") );
  m_attribListSpec->extend("PxMiss", "float");
  m_attribListSpec->extend("PyMiss", "float");
  m_attribListSpec->extend("NEGamma", "unsigned short");

  AthenaAttributeType phystype("float","GEV","PHYSICS_EGAMMA");
  ...
  phystype.setType("unsigned short");
  phystype.setUnit("NONE");
  for(unsigned short i=0; i<m_maxNum; i++)
  {
    std::sprintf(attribNum, "%i", i+1);
  ...
  phystype.setGroup("PHYSICS_MUON");
  phystype.setType("float");
  phystype.setUnit("GEV/C");
  for(unsigned short i=0; i<m_maxNum; i++)
  {
    std::sprintf(attribNum, "%i", i+1);
  ...
  phystype.setGroup("PHYSICS_TAU");
  phystype.setType("unsigned short");
  phystype.setUnit("NONE");
  m_attribListSpec->extend("NTau", phystype);
  phystype.setType("float");
  phystype.setUnit("GEV");
  for(unsigned short i=0; i<m_maxNum; i++)
  {
    std::sprintf(attribNum, "%i", i+1);
  ...
  phystype.setGroup("PHYSICS_JET");
  phystype.setUnit("NONE");
  m_attribListSpec->extend("NJet", phystype);
  phystype.setType("float");
  phystype.setUnit("GEV");
  for(unsigned short i=0; i<m_maxNum; i++)
  {
