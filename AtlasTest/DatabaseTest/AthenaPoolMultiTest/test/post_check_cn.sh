...
if [ -z "$status" ]
    then
    echo "[93;1m post_check_cn.sh> Warning: athena exit status is not available [m"
else 
    # check exit status
    joblog=${test}.log
    if [ "$status" = 0 ]
	then 
	#echo "[92;1m post_check_cn.sh> OK: ${test} exited normally. Output is in $joblog [m"
...
#	    echo " post_check_cn.sh> Now comparing output with reference"
		...
		exit 1
                ...
                true
		#echo "[92;1m post_check_cn.sh> OK: $joblog and $reflog identical [m"
	    fi
	else
	    tail $joblog
	    echo "[93;1m post_check_cn.sh> WARNING: reference output $reflog not available [m"
	    echo  " post_check_cn.sh> Please check ${PWD}/$joblog"
            exit 1
	fi
    else
	tail $joblog
	echo  "[97;101;1m post_check_cn.sh> ERROR: Athena exited abnormally! Exit code: $status [m"
	echo  " post_check_cn.sh> Please check ${PWD}/$joblog"
    fi
fi
