...
if [ -z "$status" ]
    then
    echo "[93;1m post_check_ci.sh> Warning: athena exit status is not available [m"
else 
    # check exit status
    joblog=${test}.log
    if [ "$status" = 0 ]
...
#		exit 1
	...
	tail $joblog
	echo  "[97;101;1m post_check_ci.sh> ERROR: Athena exited abnormally! Exit code: $status [m"
	echo  " post_check_ci.sh> Please check ${PWD}/$joblog"
    fi
fi
