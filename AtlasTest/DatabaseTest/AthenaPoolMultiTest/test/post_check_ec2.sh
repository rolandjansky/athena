...
if [ -z "$status" ]
    then
    echo "[93;1m post_check_ec2.sh> Warning: athena exit status is not available [m"
else 
    # check exit status
    joblog=${test}.log
    if [ "$status" = 0 ]
	then 
	echo "[92;1m post_check_ec2.sh> OK: ${test} exited normally. Output is in $joblog [m"
...
#	    echo " post.sh> Now comparing output with reference"
	    ...
	    if [ $diffStatus = 0 ] 
		then
		echo "[97;101;1m post_check_ec2.sh> ERROR: $joblog and $reflog differ [m"
#		exit 1
	    else
		echo "[92;1m post_check_ec2.sh> OK: $joblog and $reflog identical [m"
	    fi
	else
	    tail $joblog
	    echo "[93;1m post_check_ec2.sh> WARNING: reference output $reflog not available [m"
	    echo  " post_check_ec2.sh> Please check ${PWD}/$joblog"
	fi
    else
	tail $joblog
	echo  "[97;101;1m post_check_ec2.sh> ERROR: Athena exited abnormally! Exit code: $status [m"
	echo  " post_check_ec2.sh> Please check ${PWD}/$joblog"
    fi
fi
