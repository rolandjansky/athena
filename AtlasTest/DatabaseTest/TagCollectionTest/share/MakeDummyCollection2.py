...
## $Id: MakeDummyCollection2.py,v 1.3 2009-05-01 15:20:32 cranshaw Exp $
...
from AthenaPoolMultiTest.AthenaPoolMultiTestConf import TagEventTagWriter
...
RegStreamTool2 = RegistrationStreamLCGTool( "LCGTool2")
RegStreamTool2.Slave = False
RegStreamTool2.OutputCollection = "ToolSupremacy.root"

RegStream2 = RegistrationStream( "RegStream2" )

RegStream2.OutputLevel = INFO
RegStream2.Tool = RegStreamTool2

# Specifies whether collection references input or output data file:
try:
  RegStream2.WriteInputDataHeader = WriteInputHeader
except:
...
# Output collection name:
try:
  RegStream2.OutputCollection = OutColl
except:
  RegStream2.OutputCollection = "OutputCollection"
...
# Output collection open mode:
try:
  RegStream2.CollectionOpenMode = OpenMode
except:
...
# Output collection name:
try:
  RegStream3.OutputCollection = OutColl
except:
  RegStream3.OutputCollection = "OutputCollection"

# Output collection type:
try:
  RegStream3.CollectionType = OutCollType
except:
  RegStream3.CollectionType = "ExplicitROOT"
  
# Output collection database connection string:
try:
  RegStream3.Connection = OutCollConnect
except:
  RegStream3.Connection = ""

# Output collection open mode:
try:
  RegStream3.CollectionOpenMode = OpenMode
except:
  RegStream3.CollectionOpenMode = "CREATE_AND_OVERWRITE"

# Reference to input file
addProv = False
try:  
  addProv = ProvOn
  RegStream3.WriteAllProv = ProvOn
except:
  print "Writing only primary token to collection"
  RegStream3.WriteAllProv = False

# prov stage included
if addProv:
  try:
    RegStream3.IncludeProvStages = ProvIncludes
  except:
    print "Prov in attributelist will be set based on first data header" 

# List of data object references to be added to collection:
RegStream3.ItemList += [ "DataHeader#*" ]

# List of attribute list objects to be added to collection:
RegStream3.ItemList +=  [ "TagAthenaAttributeList#SimpleTag" ]
