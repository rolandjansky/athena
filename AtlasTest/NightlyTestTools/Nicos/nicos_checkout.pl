...
#    } 
     ...
     if ( -d ${package1} && ${NICOS_CHECKOUT_COMMAND} ne "cmt" ){
     chdir "${package1}";
     $infoword="updated"; 
        #
        #   update:
...
if ( ${NICOS_CHECKOUT_COMMAND} eq "cmt" ){
          ...
          $stat=system("cvs","-d","${CVSROOT}","co","-r","${newver}","-d","${newver}","${cvsloc}/${package1}");
             ...
             {
                if ( -d ${package1} ){
                opendir(DDD, ${package1});
	        @allfiles = readdir DDD;
	        closedir DDD;
          	$npack=basename(${package1});
# removal of old version if new version comes
if ( ${CMTSTRUCTURINGSTYLE} eq "without_version_directory" ){
                $fname="${NICOS_RELHOME}/${package1}/cmt/version.cmt";
                if ( -f ${fname} ){
                  my @version=();
                  open(UT, "$fname");
                  chomp(@version=<UT>);
                  ...
                  ( $lf = $vrs ) =~ s/ //g;
                    ...
                    $erere=cwd;
                    system("rm -rf ${package1}");
                    print " old version removed ${package1}, ${lf} (decision based on version.cmt) \n";
                    }
                    else{
                    print " the version exists: ${package1}, ${lf} (decision based on version.cmt) \n";
                    $version_exists="yes";
                    }  
                  }
}
else {# if ( ${CMTSTRUCTURINGSTYLE}
                @listf = grep /${npack}(.*)([0-9]*)\-([0-9]*)\-([0-9]*)(.*)$/, @allfiles;
                  foreach $lf (@listf){
                  next if $lf =~ /^\.\.?$/;
		  next if $lf =~ /~$/;
                    if ( -d "${package1}/${lf}" && ${lf} ne ${newver} ){
                    system("rm -rf ${package1}/${lf}");
                    print " old version removed ${package1}/${lf} \n";
		    }
                    if ( -d "${package1}/${lf}" && ${lf} eq ${newver} ){
                    print " the version exists: ${package1}/${lf} \n";
                    $version_exists="yes";
		    }
                  }
} # if ( ${CMTSTRUCTURINGSTYLE} eq
#  end of removal
                }
             print " version_exists ${version_exists}, cmt command ${newver} ${package1}\n";      
             if ( ${version_exists} ne "yes" ){     
             $stat=system("cmt","co","-r","${newver}","${package1}");
	     } # if ( ${version_exists} ne "yes" )
             else{
             $stat=0;
             } # else if ( ${version_exists} ne "yes" )    
             }
...
####
####    CHECKOUT PACKAGES with ZERO CODE ONLY 
####    (i.e. non-containers in ATLAS case)
####
          if ( $code eq 0 ) {newrel_link($newver, $package1);}
...
# commented out. This belongs to nicos_project_conf
