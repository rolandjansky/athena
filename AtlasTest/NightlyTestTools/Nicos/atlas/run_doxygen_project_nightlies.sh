...
sf=`echo ${NICOS_COPY_TESTS_DIR} | cut -d"/" -f1`

# --- Set up log files

pass=1
globalLog="nicos_doxygen_${sf}_${pass}.log"
while [ -e ${globalLog} ]; do
    let pass=${pass}+1
    globalLog="nicos_doxygen_${sf}_${pass}.log"
done
startStamp="nicos_doxygen_${sf}_${pass}.`hostname`.started"
finishStamp="nicos_doxygen_${sf}_${pass}.`hostname`.finished"
#NICOS_TESTDIR=`dirname ${NICOS_TESTLOG}`
NICOS_TESTDIR="/build/atnight"
localLog="${NICOS_TESTDIR}/${globalLog}"
errorLog="${NICOS_TESTDIR}/nicos_errorsInDoxygen_${sf}_${pass}.log"
