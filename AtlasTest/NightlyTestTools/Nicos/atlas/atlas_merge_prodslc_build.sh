...
relnum=$NICOS_PROJECT_RELNUMB_COPY
PROJECT_RELNAME=${NICOS_PROJECT_RELNAME_GEN}_${relnum}
if [ "${relnum}" = "" ]; then
PROJECT_RELNAME=${NICOS_PROJECT_RELNAME_GEN}
fi

echo "copy is performed for platform i686-slc3-gcc323-opt to release ${PROJECT_RELNAME}"

/afs/cern.ch/user/a/alibrari/scripts/copy_release.sh -R -v -f $NICOS_PROJECT_HOME -f_r atlrel_0 -p i686-slc3-gcc323-opt -t /afs/cern.ch/atlas/software/dist/nightlies/rd -r ${PROJECT_RELNAME}
