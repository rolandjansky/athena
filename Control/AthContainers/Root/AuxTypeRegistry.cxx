                                     ...
                                     const void* src, size_t src_index)
{
  ...
  if (factory) {
    ...
    if (m_types[auxid].m_isEL) applyELThinning (reinterpret_cast<char*>(dst) + dst_index * factory->getEltSize());
    if (m_types[auxid].m_isELVec) applyELVecThinning (reinterpret_cast<char*>(dst) + dst_index * factory->getEltSize());
  }
}
 ...
 */
void AuxTypeRegistry::setELFlags (lock_t& /*lock*/, typeinfo_t& info)
{
  info.m_isEL = false;
  info.m_isELVec = false;
  std::string tname =  normalizedTypeinfoName (*info.m_ti);
