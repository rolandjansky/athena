...
sc,out = doValidation( "Input Data",
                       ...
                       workDir("my.data.toslim.pool.log"),
...
jobOptions = [
    ChapPy.JobOptionsCmd( "INPUT=[\"%s\"]" % workDir("my.data.toslim.pool") ),
    ChapPy.JobOptionsCmd( "OUTPUT=\"%s\""  % workDir("slimmed.data.pool") ),
    ChapPy.JobOptions( "AthExThinning/WriteSlimmedData_jobOptions.py" ),
    ]

athena = ChapPy.Athena( jobOptions = jobOptions,
                        logFile    = workDir("slimmed.data.pool.log"),
                        checkLeak  = False )
athena.EvtMax = -1
athena.run()
...
jobOptions = [
    ChapPy.JobOptionsCmd( "INPUT=[\"%s\"]" % workDir("slimmed.data.pool") ),
    ChapPy.JobOptionsCmd( "OUTPUT=\"%s\""  % workDir("reaccessed.slimmed.data.pool") ),
    ChapPy.JobOptions( "AthExThinning/ReadSlimmedData_jobOptions.py" ),
    ]

athena = ChapPy.Athena( jobOptions = jobOptions,
                        logFile    = workDir("reaccessed.slimmed.data.pool.log"),
                        checkLeak  = False )
athena.EvtMax = -1
athena.run()
...
sc,out = doValidation( "ReadSlimmedData",
                       ...
                       workDir("reaccessed.slimmed.data.pool.log"),
...
jobOptions = [
    ChapPy.JobOptionsCmd( "INPUT=[\"%s\"]" % workDir("non.slimmed.data.pool") ),
    ChapPy.JobOptionsCmd( "OUTPUT=\"%s\""  % workDir("reaccessed.non.slimmed.data.pool") ),
    ChapPy.JobOptions( "AthExThinning/ReadNonSlimmedData_jobOptions.py" ),
    ]

athena = ChapPy.Athena( jobOptions = jobOptions,
                        logFile    = workDir("reaccessed.non.slimmed.data.pool.log"),
                        checkLeak  = False )
athena.EvtMax = -1
athena.run()
...
sc,out = doValidation( "ReadNonSlimmedData",
                       ...
                       workDir("reaccessed.non.slimmed.data.pool.log"),
                       "grep \"^Py:ReadFatObject \"" )
if sc != 0:
    raise SystemExit("ERROR")
