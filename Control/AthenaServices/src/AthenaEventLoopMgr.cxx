...
/// @c struct to hold the enum for the 'clear-store' policy
...
AthenaEventLoopMgr::AthenaEventLoopMgr(const std::string& nam, 
  ...
  : MinimalEventLoopMgr(nam, svcLoc), 
    ...
    m_storeGateSvc( "StoreGateSvc", nam ), 
...
StatusCode AthenaEventLoopMgr::nextEvent(int maxevt)   
{
  ...
  while((maxevt == -1 || m_nevt < maxevt) && 
	...
	sc = StatusCode::SUCCESS;
      ...
      if (0 != addr) {
	//create its proxy
	sc = m_storeGateSvc->createProxy(addr);
	if( !sc.isSuccess() ) {
	  m_msg << MSG::WARNING 
      ...
      if ((sc=m_storeGateSvc->loadEventProxies()).isFailure()) {
	m_msg << MSG::ERROR 
	...
	continue;
      ...
      sc = m_storeGateSvc->retrieve(pEvent);
      if( !sc.isSuccess() ) {
	m_msg << MSG::ERROR 
    ...
    else 
    {
      ...
      sc = m_storeGateSvc->record(pEvent,"");
      if( !sc.isSuccess() )  {
        m_msg << MSG::ERROR 
...
void AthenaEventLoopMgr::handle(const Incident& inc)
{
  ...
  if(m_storeGateSvc->loadEventProxies().isFailure()) {
