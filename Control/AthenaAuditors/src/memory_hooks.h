...
static void *my_malloc_hook (size_t size, const void* /* caller */)
{
  ...
  do
    ...
    } while ( result < m_sbrk_ptr );
  
  node* n = new ((void*)result) node( m_first.getNext(), &m_first );
  m_first.getNext()->setPrev(n);
  m_first.setNext(n);
  n->setSize(size);
...
my_realloc_hook(void *ptr, size_t size, const void * /* caller */)
{
  ...
  // allocate in freshly deallocated memory below m_sbrk_ptr in case of free
  if ( size == 0 )
    {
      if ( p < m_sbrk_ptr )
	{
  ...
  else
    {
      ...
      if ( ( result > m_sbrk_ptr ) && ( size > deltaLow+deltaHigh ) )
	{
	  ...
	  if ( ( p == result ) )
	  ...
	  else
	    {
	      ...
	      if ( p && ( p < m_sbrk_ptr ) && ( result > m_sbrk_ptr ) && ( size > deltaLow+deltaHigh ) )
		{
		  ...
		  n = new ((void*)result) node( m_first.getNext(), &m_first );
		  m_first.getNext()->setPrev(n);
		  m_first.setNext(n);
	      ...
	      if ( p && ( p > m_sbrk_ptr ) && ( result > m_sbrk_ptr ) && (p != result ) && ( size > deltaLow+deltaHigh ) )
		{
	      ...
	      if ( p == 0 && ( result > m_sbrk_ptr ) && ( size > deltaLow+deltaHigh ) )
		{
		  node* n = new ((void*)result) node( m_first.getNext(), &m_first );
		  m_first.getNext()->setPrev(n);
		  m_first.setNext(n);
