...

{
  ...
  declareProperty("EnableConditions", m_enableCondSvc = false);
...
StatusCode ForwardSchedulerSvc::initialize(){
  ...
  */
  ...
  info() << " --------- Dumping Data Flow ---------" << endmsg;
  m_efg = algPool->getExecutionFlowGraph();
  m_efg->dumpDataFlow();


  // info() << "\n\n --------- Dumping Execution Plan ---------" << endmsg;
  // algPool->getExecutionFlowGraph()->dumpExecutionPlan();
  ...
  auto action = [this,eventContext] () -> StatusCode {
    ...
    if (m_enableCondSvc &&
      ...
      if (validIDs.size() > 0) {
        ...
        // std::set<std::string> unchangedAlgs = 
        //   m_condSvc->getUnchangedAlgs(validIDs);
        // info() << "unchangedAlgs " << eventContext->eventID() << ": " 
        //        << unchangedAlgs.size() << endmsg;

        // for (auto alg: unchangedAlgs) {
        //   unsigned int idx = algname2index(alg);
        //   info() << "marking " << alg << " [" << idx 
        //          << "] as unchanged for "
...
unsigned int
ForwardSchedulerSvc::getAlgIdx(IAlgorithm* ialg) {

    std::lock_guard<std::mutex> lock(m_ssMut);

    std::map<IAlgorithm*,unsigned int>::iterator itr = m_algIdxMap.find(ialg);
    if(itr != m_algIdxMap.end()) {
      return itr->second;
    }

    const std::list<IAlgorithm*>& algos = m_algResourcePool->getFlatAlgList();
    unsigned int ia = algname2index(ialg->name());

    info() << " getAlgIdx: " << ialg->name() << " at " << ialg 
           << " not found. adding as " << ia
           << endmsg;
