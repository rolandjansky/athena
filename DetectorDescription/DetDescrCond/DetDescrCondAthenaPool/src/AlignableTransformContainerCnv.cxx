...
AlignableTransform_PERS* AlignableTransformContainerCnv::createPersistent(AlignableTransform_TRANS* transObj) {
    MsgStream log(messageService(), "AlignableTransformContainerCnv" ); 
    AlignableTransformCnv_p1 cnv;
    AlignableTransform_PERS* persObj = cnv.createPersistent( transObj, log );
    if (log.level()<=MSG::DEBUG) log << MSG::DEBUG << "AlignableTransformContainer write a new AlignableTransform_PERS " << endreq;
    return persObj; 
}
...
AlignableTransform_TRANS* AlignableTransformContainerCnv::createTransient() {
    ...
    if( compareClassGuid(p1_guid) ) {
        ...
        if (log.level()<=MSG::DEBUG) log << MSG::DEBUG << "AlignableTransformContainer read p1" << endreq;
        ...
        if (log.level()<=MSG::DEBUG) log << MSG::DEBUG << "AlignableTransformContainer read p0" << endreq;
