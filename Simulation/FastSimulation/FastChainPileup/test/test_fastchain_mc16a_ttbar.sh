...
dcubeXmlID="${inputXmlDir}/dcube_physval_af2.xml"
dcubeRefID="${inputRefDir}/physval_af2.root"
...
Reco_tf.py --inputRDOFile 'RDO.pool.root' --maxEvents '-1' \
           ...
           --outputNTUP_PHYSVALFile 'physval_af2.root' \
...
then
    ...
    $ATLAS_LOCAL_ROOT/dcube/current/DCubeClient/python/dcube.py \
    -p -x dcube-id \
    -c ${dcubeXmlID} -r ${dcubeRefID} physval_af2.root
    rc5=$?
