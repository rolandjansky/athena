...
StatusCode FastHitConv::initialize(){
    ...
    status = service("MessageSvc", m_msgSvc);
    if (status.isFailure()) {
      log << MSG::FATAL << "MessageSvc not found!" << endreq;
      return status;
    }
    else log << MSG::DEBUG << "MessageSvc initialized." << endreq;
    ...
    else log << MSG::DEBUG << "DetectorStore Svc initialized." << endreq;
    ...
    status = m_detStore->regHandle(m_dd_fSampl,"LArfSampl");
    if (status.isFailure()) {
      log << MSG::ERROR << " Unable to register handle for LArfSampl" << endreq;
      return StatusCode::FAILURE;
    } else log << MSG::DEBUG << " Got the register handle for LArfSampl" << endreq;
    
    status = m_detStore->retrieve(m_tileInfo, "TileInfo");
    if (status.isFailure()) {
      log << MSG::ERROR
...
StatusCode FastHitConv::execute(){
  ...
  if ( m_storeGate->contains<CaloCellContainer>(m_caloCellsOutputName)) {
...
StatusCode FastHitConv::finaliseEvent() {
  ...
  if(m_pileup){//pileup or not?
    ...
    if((*m_storeGateFastCalo).record(embHitCont,  m_embHitContName).isFailure())  log << MSG::ERROR << "SG record failed: embHitCont"<< endreq;;
    if((*m_storeGateFastCalo).record(emecHitCont, m_emecHitContName).isFailure()) log << MSG::ERROR << "SG record failed: emecHitCont"<< endreq;;
    if((*m_storeGateFastCalo).record(fcalHitCont, m_fcalHitContName).isFailure()) log << MSG::ERROR << "SG record failed: fcalHitCont"<< endreq;;
    if((*m_storeGateFastCalo).record(hecHitCont,  m_hecHitContName).isFailure())  log << MSG::ERROR << "SG record failed: embHitCont"<< endreq;;
    if((*m_storeGateFastCalo).record(m_tileHits,  m_tileHitContName).isFailure()) log << MSG::ERROR << "SG record failed: m_tileHits"<< endreq;;
    ...
    if ( m_storeGate->retrieve( pOverEvent, "OverlayEvent" ).isFailure() ) {
      log << MSG::ERROR << "  Could not get OverlayEvent" << endreq;      
      return StatusCode::FAILURE;
    }else {
...
//     helper.ExportCollection< LArHitContainer >(hecHitCont);
    ...
    if((*m_storeGate).record(embHitCont,  m_embHitContName).isFailure()) log << MSG::ERROR << "SG record failed: embHitCont"<< endreq;
    if((*m_storeGate).record(emecHitCont, m_emecHitContName).isFailure()) log << MSG::ERROR << "SG record failed: emecHitCont"<< endreq;
    if((*m_storeGate).record(fcalHitCont, m_fcalHitContName).isFailure()) log << MSG::ERROR << "SG record failed: fcalHitCont"<< endreq;
    if((*m_storeGate).record(hecHitCont,  m_hecHitContName).isFailure()) log << MSG::ERROR << "SG record failed: hecHitCont"<< endreq;
    if((*m_storeGate).record(m_tileHits,  m_tileHitContName).isFailure()) log << MSG::ERROR << "SG record failed: m_tileHits"<< endreq;
...
StatusCode FastHitConv::hitConstruction(){
  ...
  //
    ...
    if (!m_storeGate->retrieve(cellContainer, m_caloCellsOutputName)) {
