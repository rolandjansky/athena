...
StatusCode PixelFastRDOAnalysis::initialize() {
  ...
  CHECK(m_thistSvc->regHist(m_path + h_omegax->GetName(), h_omegax));

  h_omegay = new TH1F("h_omegay", "omegay", 100, -3, 3);
  h_omegay->StatOverflows();
  CHECK(m_thistSvc->regHist(m_path + h_omegay->GetName(), h_omegay));
  ...
  CHECK(m_thistSvc->regHist(m_path + h_totalTot->GetName(), h_totalTot));
  ...
  CHECK(m_thistSvc->regHist(m_path + h_totalCharge->GetName(), h_totalCharge));

  h_eLoss = new TH1F("h_eLoss", "eLoss", 100, -2000, 0);
  h_eLoss->StatOverflows();
  CHECK(m_thistSvc->regHist(m_path + h_eLoss->GetName(), h_eLoss));

  h_fake = new TH1F("h_fake", "fake", 100, 0, 2);
  h_fake->StatOverflows();
  CHECK(m_thistSvc->regHist(m_path + h_fake->GetName(), h_fake));

  h_ambig = new TH1F("h_ambig", "ambig", 100, 0, 2);
  h_ambig->StatOverflows();
  CHECK(m_thistSvc->regHist(m_path + h_ambig->GetName(), h_ambig));

  h_split = new TH1F("h_split", "split", 100, -1, 1);
  h_split->StatOverflows();
  CHECK(m_thistSvc->regHist(m_path + h_split->GetName(), h_split));

  h_splitProb1 = new TH1F("h_splitProb1", "splitProb1", 100, -1, 1);
  h_splitProb1->StatOverflows();
  CHECK(m_thistSvc->regHist(m_path + h_splitProb1->GetName(), h_splitProb1));

  h_splitProb2 = new TH1F("h_splitProb2", "splitProb2", 100, -1, 1);
  h_splitProb2->StatOverflows();
  CHECK(m_thistSvc->regHist(m_path + h_splitProb2->GetName(), h_splitProb2));

  // LOOK AT
  h_splitInfoRaw = new TH1F("h_splitInfoRaw", "splitInfoRaw", 100, -1, 1);
  h_splitInfoRaw->StatOverflows();
  CHECK(m_thistSvc->regHist(m_path + h_splitInfoRaw->GetName(), h_splitInfoRaw));
