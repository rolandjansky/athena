...
// ********************************************************************
//
// $Id: G4mplAtlasIonisationWithDeltaModel.cxx 637068 2014-12-19 15:17:25Z jchapman $
...
{
  nmpl = abs(magCharge) * 2 * CLHEP::fine_structure_const;
  //if(nmpl > 6)      { nmpl = 6; }
  ...
  cout << "### Monopole ionisation model with d-electron production, Gmag= " 
	 << magCharge/CLHEP::eplus << endl;
}
						...
						G4double cutEnergy)
{
  ...
  G4double dedx = 
    0.5*(log(2.0 * CLHEP::electron_mass_c2 * bg2*cutEnergy / (eexc*eexc)) - 1.0);
  ...
  if(nmpl > 6.0) { dedx += 0.5 * k - B[6]; }
    ...
    dedx += 0.5 * k - B[G4int(abs(magCharge) * 2 * CLHEP::fine_structure_const + 0.5)];
...
void 
G4mplAtlasIonisationWithDeltaModel::SampleSecondaries(vector<G4DynamicParticle*>* vdp,
						 ...
						 G4double maxEnergy)
{
  ...
  G4double deltaMomentum =
           sqrt(deltaKinEnergy * (deltaKinEnergy + 2.0*CLHEP::electron_mass_c2));
