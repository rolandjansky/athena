...
{
  // get the MCTruthSteppingAction from the ToolSvc
  if(!theSteppingAction){

    IToolSvc* toolSvc = 0;
    StatusCode sc = Gaudi::svcLocator()->service("ToolSvc", toolSvc);
    if (sc.isFailure()) {
      std::cout <<  "TruthStrategyManager: Trouble getting the Tool service.  Should never happen. Will crash now." << std::endl;
      std::abort();
    }

    IUserAction* mctaction=0;
    sc = toolSvc->retrieveTool("MCTruthSteppingAction", mctaction, 0,false); // do not create the tool if not existing

    if (sc.isFailure()) {

      std::cout << "TruthStrategyManager: cannot retrieve MCTruthSteppingAction, aborting"<<std::endl;
      std::abort();
    }

    theSteppingAction= dynamic_cast<MCTruthSteppingAction*>(mctaction);

    if(!theSteppingAction){
      std::cout<< "TruthStrategyManager: cannot cast MCTruthSteppingAction, aborting"<<std::endl;
      std::abort();
    }
