...
namespace G4UA {
  ...
  namespace iGeant4{
    ...
    TrackProcessorUserActionPassBack::TrackProcessorUserActionPassBack(const Config& config):
      AthMessaging(Gaudi::svcLocator()->service< IMessageSvc >( "MessageSvc" ),"TrackProcessorUserActionPassBack"),
      m_config(config),m_particleBrokerQuick(0), m_geoIDSvcQuick(0){}
    
    
    void TrackProcessorUserActionPassBack::beginOfRun(const G4Run*){
    ...
    //_______________________________________________________________________
    HepMC::GenParticle* TrackProcessorUserActionPassBack::findMatchingDaughter(HepMC::GenParticle* parent, bool verbose=false) const {
      // Add all necessary daughter particles
      if(NULL==parent->end_vertex()) {
	if(verbose) ATH_MSG_INFO ( "Number of daughters of "<<parent->barcode()<<": 0" );
	return parent;
    ...
    //________________________________________________________________________
    ISF::ITruthBinding*
    TrackProcessorUserActionPassBack::newTruthBinding(const G4Track* aTrack) const
    {
      ISF::ITruthBinding* tBinding = 0;
      
      VTrackInformation* trackInfo = ::iGeant4::ISFG4Helpers::getISFTrackInfo(*aTrack);
    ...
    //________________________________________________________________________
    ISF::ISFParticle*
    TrackProcessorUserActionPassBack::attachNewISFParticle(G4Track* aTrack,
							   const ISF::ISFParticle* parent,
							   AtlasDetDescr::AtlasRegion  nextGeoID)
    {
      ISF::ITruthBinding* tBinding = newTruthBinding(aTrack);
      
      ISF::ISFParticle* isp = ::iGeant4::ISFG4Helpers::convertG4TrackToISFParticle( *aTrack, *parent, tBinding );
	...
	int trackID  = aTrack->GetTrackID();
	ATH_MSG_VERBOSE("Setting ISFParticle to "<<isp<<" for trackID "<<trackID<<" (from new ISFParticle)");
	::iGeant4::ISFG4Helpers::linkG4TrackToISFParticle( *aTrack, isp );
      }  
    ...
    bool TrackProcessorUserActionPassBack::checkVolumeDepth( G4LogicalVolume * lv , int volLevel , int d ) {
      if (lv==0) return false;
      bool Cavern = false;
