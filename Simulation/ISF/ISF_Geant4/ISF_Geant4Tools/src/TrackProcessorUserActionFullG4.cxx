...
namespace G4UA{
  ...
  namespace iGeant4{
    ...
    TrackProcessorUserActionFullG4::TrackProcessorUserActionFullG4(const Config& config)
      : AthMessaging(Gaudi::svcLocator()->service< IMessageSvc >( "MessageSvc" ),"TrackProcessorUserActionFullG4"),
	TrackProcessorUserActionBase(),m_config(config),
	m_entryLayerToolQuick(0),
	m_hasCavern(true),
	m_currentTrack(0),
	m_BPholder(0), m_IDholder(0), m_CALOholder(0) , m_MUholder(0), m_TTRholder(0)
    {
      
      ATH_MSG_DEBUG("create TrackProcessorUserActionFullG4");
    ...
    void TrackProcessorUserActionFullG4::beginOfRun(const G4Run*)
    {
      ATH_MSG_DEBUG("Initializing TrackProcessorUserActionFullG4");
      
      if ( !m_config.entryLayerTool.empty() ) {
	if ( m_config.entryLayerTool.retrieve().isFailure()) {
	  ATH_MSG_FATAL("Could not retrieve ISF Entry Layer Tool: " << m_config.entryLayerTool);
	  return;
	}
	...
	if ( (*lvs)[i]->GetName() == "BeamPipe::BeamPipe" ) m_BPholder = (*lvs)[i];
      ...
      checkVolumeDepth( G4TransportationManager::GetTransportationManager()->GetNavigatorForTracking()->GetWorldVolume()->GetLogicalVolume() , m_config.truthVolLevel ); 
      return;
    }
    ...
    //________________________________________________________________________
    ISF::EntryLayer
    TrackProcessorUserActionFullG4::entryLayer(const G4Step* aStep)
    {
      //
      // this is the same prescription for getting the entry layer as in
      // MCTruth/MCTruthSteppingAction
    ...
    //________________________________________________________________________
    AtlasDetDescr::AtlasRegion
    TrackProcessorUserActionFullG4::nextGeoId(const G4Step* aStep)
    {
	...
	if (m_config.truthVolLevel>1 &&
	    step.PostStepBranchDepth()>m_config.truthVolLevel-1 && m_hasCavern &&
	    step.GetPostStepLogicalVolumeName(m_config.truthVolLevel-1).find("CavernInfra") ) m_nextGeoID = AtlasDetDescr::fAtlasCavern;
	return m_nextGeoID;
      }
	...
	//ATH_MSG_ERROR("vol1: "<<step.GetPostStepLogicalVolumeName(1)<<", vol2: "<<step.GetPostStepLogicalVolumeName(2)<<", postname="<<postname<<", returning undefined geoID");
	G4ThreeVector myPos = aStep->GetPostStepPoint()->GetPosition();
	ATH_MSG_ERROR("Returning undefined geoID from " << step.GetPostStepLogicalVolume() << " requesting " << step.GetPostStepLogicalVolume(m_config.truthVolLevel) << " at " << myPos.x() << " " << myPos.y() << " " << myPos.z() );
      }
