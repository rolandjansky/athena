...
typedef std::map< Identifier , CaloDetDescrElement* > t_cellmap;
...
public:
   ...
   bool IsCompatible(CaloDetDescrElement* cell);
   void add(CaloDetDescrElement* cell);
...
protected:
   ...
   float calculate_distance_eta_phi(CaloDetDescrElement* DDE,float eta,float phi,float& dist_eta0,float& dist_phi0);

   int m_index;
   t_cellmap m_cells;
   std::vector< std::vector< CaloDetDescrElement* > > m_cell_grid;
...
protected:
   virtual void addcell(CaloDetDescrElement* cell);
