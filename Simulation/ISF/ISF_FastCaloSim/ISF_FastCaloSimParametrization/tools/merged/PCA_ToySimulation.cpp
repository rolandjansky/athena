...
#include "myTMVARegressionApplication.C"
...
////////////////////////////////////////////////////
    ...
    //****************************************************//
    //    THIS IS DOING THE TMVA REGRESSION APPLICATION   //
    
    //make a tree with random numbers
    int n=100000;
    TTree* randomtree=new TTree("randomtree","randomtree");
    Float_t num;
    randomtree->Branch("y",&num,"y/F");
    TRandom ran;
    ran.SetSeed(0);
    for(int i=0;i<n;i++)
    {
    ...
    //run the regression
    int neurons=5;
    for(int k=0;k<Relevant_Layers.size();k++)
    {
     myTMVARegressionApplication(randomtree, neurons, Relevant_Layers[k]);
    }
    
    //Plots comparing the MLP output to the Input variables
    for(int k=0;k<Relevant_Layers.size();k++)
    {
     TFile *tmvaout=TFile::Open(Form("TMVARegApp_layer%i_neurons%i.root",Relevant_Layers[k],neurons));
     TH1D* h_mlp=(TH1D*)tmvaout->Get("MLP"); h_mlp->SetName("h_mlp");
     
     //initial
     double data;
     T_Id->SetBranchAddress( Form("data_%i",Relevant_Layers[k]), &data );
     TH1D* h_ini=new TH1D("h_ini","h_ini",100,0,1);
     for(Long64_t ievt=0; ievt<T_Id->GetEntries();ievt++)
     {
      T_Id->GetEntry(ievt);
      h_ini->Fill(data);  
     }
     ...
     h_ini->SetFillColor(17);
     h_ini->Draw("hist");
     h_ini->GetXaxis()->SetTitle(Form("E fraction Layer %i",Relevant_Layers[k]));
     h_mlp->SetLineColor(2); h_mlp->SetLineWidth(3);
     h_mlp->Scale(h_ini->Integral()/h_mlp->Integral());
     h_mlp->Draw("histsame");
     double max=h_ini->GetBinContent(h_ini->GetMaximumBin());
     double chi2 =h_mlp->Chi2Test(h_ini,"UW");
     double kolmo=h_mlp->KolmogorovTest(h_ini);
     TLatex* la=new TLatex(0.6,max/2,Form("#splitline{Chi2 Test %.2f}{Kolmo %.2f}",chi2,kolmo)); la->Draw();
     can->Print(Form("mlptest_layer%i_neurons%i.png",Relevant_Layers[k],neurons));
    }
