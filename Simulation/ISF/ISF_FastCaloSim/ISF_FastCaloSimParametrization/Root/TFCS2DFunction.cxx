...
// TString outfileName("TMVAReg.root");
 ...
 if(mycase==1){
   ...
   factory->AddVariable("r","r",'F');
   if(!doNNHit)
     factory->AddTarget("energydensityLN");
   if(doNNHit)
     factory->AddTarget("energyLN");
 }
 if(mycase==2){
   factory->AddVariable("probalpha","probalpha",'F');
   factory->AddVariable("probr","probr",'F');
   factory->AddTarget("bina");
   factory->AddTarget("binrLN");
 }

 //TFile *input(0);
 //Double_t regWeight  = 1.0;   
 
 // Changed to give the same tree for test and training
 ...
 factory->AddSignalTree( regTree, 1.0, "Training" );
 factory->AddSignalTree( regTree, 1.0, "Test" );
 //if(mycase==2){
