...
persToTrans( const iFatras::PlanarCluster_p2 *persObj, iFatras::PlanarCluster *transObj, MsgStream &log) 
{
  ...
  if (persObj->m_clusId == 0) {
      ...
      clusId =  isPixel ? m_pixId2->pixel_id_from_shortened(persObj->m_clusId) : Identifier(m_sctId2->strip_id_offset(transObj->m_clusId, persObj->m_rdoList[0]) );
  }
  ...
  for (; it != persObj->m_rdoList.end(); ++it)  {
    ...
    if (is64BitId) {
      *idit =  Identifier(clusId.get_compact() + ( (static_cast<Identifier::value_type>(*it)) << (isPixel ? m_pixId2->base_bit() : m_sctId2->base_bit()) ));
    } 
    else {
      (*idit) =   isPixel ? m_pixId2->pixel_id_from_shortened(persObj->m_clusId + (*it)) : Identifier(m_sctId2->strip_id_offset(transObj->m_clusId, *it));
    }
    ++idit;
  }
...
void PlanarClusterCnv_p2::transToPers( const iFatras::PlanarCluster *transObj, iFatras::PlanarCluster_p2 *persObj, MsgStream &log )
{
  ...
  *pit = static_cast<iFatras::PlanarCluster_p2::rdo_diff_type>( isPixel ? m_pixId2->calc_offset(waferId, clusId) : m_sctId2->calc_offset(waferId, clusId) );
  ...
  for (std::vector<Identifier>::const_iterator it=transObj->m_rdoList.begin(); 
    ...
    *pit = static_cast<iFatras::PlanarCluster_p2::rdo_diff_type>( isPixel ? m_pixId2->calc_offset(clusId, *it) : m_sctId2->calc_offset(clusId, *it));
    pit++;
  }
