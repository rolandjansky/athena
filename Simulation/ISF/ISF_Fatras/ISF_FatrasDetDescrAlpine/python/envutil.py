...
##                     subdirlist.append( fulldir )
##             if subdirlist:
##                 found = find_file( filename, subdirlist, access, depth - 1 )
##                 if found: return found
##     elif depth < 0:
##         # not found at this level. Go one level up in directory structure
##         for dir in dirlist:
...
##             updir = os.path.dirname(dir)
##             if updir != dir:
##                 found = find_file( filename, [ updir ], access, depth + 1 )
##                 if found: return found
...
def find_files_split( filename, dirlist = [ os.getcwd() ], access = os.R_OK, depth = 0 ):
...
def find_files( filename, dirlist = [ os.getcwd() ], access = os.R_OK, depth = 0 ):
...
##     # to speed up search, do a single file search if the filename does not contain wildcards
##     if not has_wildcards(filename):
##         singleFile = find_file( filename, dirlist, access, depth )
##         if singleFile:
##             return [ singleFile ]
##         else:
##             return []
##     foundList = [] # the list of files to return
##     basenameList = [] # keep track of files already found
##     for dir in dirlist:
##         dir = os.path.abspath( os.path.expandvars( os.path.expanduser(dir) ) )
##         # protect against non-existing entries
##         if not os.path.isdir(dir): continue
##         fullfile = os.path.join( dir, filename )
## ##        print "Checking files %s..." % fullfile
##         filelist = glob.glob(fullfile)
##         for f in filelist:
## ##            print "Trying %s..." % f
##             if not os.path.isfile(f) or not os.access(f, access): continue
##             base = os.path.basename(f)
##             if not base in basenameList:
## ##                print "==> Adding %s to list from %s" % (base,f)
##                 basenameList.append(base)
##                 foundList.append(f)
##             else:
##                 pass
## ##                print "==> Already have %s in list" % (base)
...
##                     subdirlist.append( fulldir )
##             if subdirlist:
##                 filelist = find_files( filename, subdirlist, access, depth - 1 )
##                 for f in filelist:
##                     base = os.path.basename(f)
##                     if not base in basenameList:
##                         basenameList.append(base)
...
def find_file_env( filename, env_var_name, access = os.R_OK, sep = defaultPathSeps, depth = 0 ):
    ...
    if not env:
        ...
        return find_file( filename, re.split(sep,env), access, depth )
...
def find_files_env( filename, env_var_name, access = os.R_OK, sep = defaultPathSeps, depth = 0 ):
    ...
    if not env:
        ...
        return find_files( filename, re.split(sep,env), access, depth )
...
def cmt_package_version(packageRoot,withVersionDirectory=True):
    ...
    if os.path.exists(cmtversionfilename):
