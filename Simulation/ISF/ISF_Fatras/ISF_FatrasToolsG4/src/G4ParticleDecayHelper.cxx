...
iFatras::G4ParticleDecayHelper::G4ParticleDecayHelper(const std::string& t, const std::string& n, const IInterface* p):    
   ...
   m_particleDecayCreator("iFatras::G4ParticleDecayCreator/FatrasParticleDecayCreator"),
   ...
   m_nNeutralParticles(0)
{
  ...
  declareProperty("ParticleDecayer",          m_particleDecayCreator);
...
iFatras::G4ParticleDecayHelper::initialize()
{
  ...
  //Get the particle Decay Creator
  if (m_particleDecayCreator.retrieve().isFailure())
  {
    ATH_MSG_FATAL( "Could not retrieve " << m_particleDecayCreator );
    return StatusCode::FAILURE;
  }
  else
      ATH_MSG_DEBUG( "Retrieve " << m_particleDecayCreator );
...
void iFatras::G4ParticleDecayHelper::decay(const ISF::ISFParticle& particleToDecay) const
{
  ...
  // perform the decay via the decay createor
  const ISF::ISFParticleVector decayProducts
      = m_particleDecayCreator->decayParticle(particleToDecay,
                                              particleToDecay.position(),
                                              particleToDecay.momentum(),
                                              particleToDecay.pdgCode(),
                                              particleToDecay.timeStamp());
