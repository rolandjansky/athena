...
VolumeDebugger::VolumeDebugger(const std::string& type, const std::string& name, const IInterface* parent):
  ...
  m_res(1000000),m_verbose(true),m_tol(0.000001),m_volumeCheck(false), m_targetMaxCopiesToCheck(1.e6), m_dumpGDML(true){
  ...
  declareProperty("MaxCopiesToCheck", m_targetMaxCopiesToCheck);
...
void VolumeDebugger::EndOfRun(const G4Run* )
{
    ...
    if(m_targetVolume!=""){
      ...
      ATH_MSG_FATAL("Did not find the volume named " << m_targetVolume << ". Please set parameter TargetVolume to one of:\n\n");
...
bool VolumeDebugger::recursiveCheck(G4VPhysicalVolume *topPV) const {
  ...
  bool hasOverlaps = topPV->CheckOverlaps(m_res, m_tol, m_verbose);
  if (hasOverlaps && m_verbose) std::cerr << "Volume " << topPV->GetName() << " has overlaps.\n";    
  ...
  for (std::multimap<G4LogicalVolume *, G4VPhysicalVolume *>::iterator mapEl = lv2pvMap.begin(); mapEl != lv2pvMap.end(); ) {
    ...
    somethingOverlapped |= hasOverlaps;
    if (hasOverlaps && m_verbose) std::cerr << "Volume " << daughterPV->GetName() << " has overlaps.\n";    
    //
    ...
    int checkEveryNth = int(n / m_targetMaxCopiesToCheck + 0.5);
    if (checkEveryNth <= 0) checkEveryNth = 1;
    for (int i = 1; i < n; ++i) { // "i = 0" already done 
      if (i % checkEveryNth == 0) {
        hasOverlaps = mapEl->second->CheckOverlaps(m_res, m_tol, m_verbose);
        somethingOverlapped |= hasOverlaps;
        if (hasOverlaps && m_verbose) std::cerr << "Volume " << mapEl->second->GetLogicalVolume()->GetName() << " has overlaps.\n";
      }
      ++mapEl;
    }
}
