...
namespace G4UA
{
  ...
  /// But it only enters into each LogVol once (no point in rechecking inside a
  /// LogVol over and over again for each PhysVol that points to it), saving
  /// quite some time. Furthermore, the user can specify a new parameter,
  /// MaxCopiesToCheck, as a double. Setting this to say 1.e20 will check all
  /// PhysVols are inside their MotherVol, and do not collide with any sisters.
  /// Setting it to say 10. means max. 10 copies of a LogVol checked. The others are skipped.
  /// The 10 chosen are spread out through the list of PhysVols that use the LogVol in question.
  /// Setting this parameter to say 2.0 gives a very fast check, while still
  /// checking every single type of LogVol at least once. Finds most overlaps very fast.
  class VolumeDebugger : public AthMessaging, public G4UserRunAction
  {
