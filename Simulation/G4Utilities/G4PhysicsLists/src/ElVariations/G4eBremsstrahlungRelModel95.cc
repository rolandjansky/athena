...
G4eBremsstrahlungRelModel95::G4eBremsstrahlungRelModel95(const G4ParticleDefinition* p,
  ...
  : G4VEmModel(name),
    particle(0),
    bremFactor(CLHEP::fine_structure_const*CLHEP::classic_electr_radius*CLHEP::classic_electr_radius*16./3.),
    isElectron(true),
    fMigdalConstant(CLHEP::classic_electr_radius*CLHEP::electron_Compton_length*CLHEP::electron_Compton_length*4.0*CLHEP::pi),
    fLPMconstant(CLHEP::fine_structure_const*CLHEP::electron_mass_c2*CLHEP::electron_mass_c2/(4.*CLHEP::pi*CLHEP::hbarc)*0.5),
...
void  G4eBremsstrahlungRelModel95::CalcLPMFunctions(G4double k)
{
  ...
  if (s<0.1) {
    ...
    phiLPM = 1.-exp(-6.*s*(1.+(3.-CLHEP::pi)*s)
...
//  * no LPM effect
{
  ...
  if (use_completescreening|| currentZ<5) {
    ...
    G4double dd=100.*CLHEP::electron_mass_c2*y/(totalEnergy-gammaEnergy);
    G4double gg=dd*z13;
    G4double eps=dd*z23;
				      ...
				      G4double maxEnergy)
{
  ...
  G4double totMomentum = sqrt(kineticEnergy*(totalEnergy + CLHEP::electron_mass_c2));
