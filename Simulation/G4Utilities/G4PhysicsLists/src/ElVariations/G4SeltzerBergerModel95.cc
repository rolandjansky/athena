...
G4double G4SeltzerBergerModel95::ComputeDXSectionPerAtom(G4double gammaEnergy)
{
  ...
  G4double y = log(kinEnergy/CLHEP::MeV);
					...
					G4double maxEnergy)
{
  ...
  /*
  G4cout << "G4SeltzerBergerModel95::SampleSecondaries E(CLHEP::MeV)= " 
	 << kineticEnergy/CLHEP::MeV
	 << " Z= " << Z << " cut(CLHEP::MeV)= " << cut/CLHEP::MeV 
	 << " emax(CLHEP::MeV)= " << emax/CLHEP::MeV << " corr= " << densityCorr << G4endl;
  */
  G4double xmin = log(cut*cut + densityCorr);
  G4double xmax = log(emax*emax  + densityCorr);
  G4double y = log(kineticEnergy/CLHEP::MeV);

  G4double gammaEnergy, v; 

  // majoranta
  G4double vmax = dataSB[Z]->Value(cut/kineticEnergy, y);
  if(isElectron && Z > 12 && kineticEnergy < 100*CLHEP::keV) {
    if((Z < 41 && kineticEnergy < 10*CLHEP::keV) ||
       (Z >= 41 && Z < 61 && kineticEnergy < 50*CLHEP::keV) ||
       (Z >= 61) )
      {
