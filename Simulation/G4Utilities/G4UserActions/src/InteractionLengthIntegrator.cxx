...
void InteractionLengthIntegrator::EndOfEventAction(const G4Event*)
{
    ...
    if (!hSvc){
        ...
        if (svcLocator->service("THistSvc", hSvc).isFailure()){
            log() << MSG::ERROR << "Failed getting the hist svc!" << endreq;
        }
    }
	...
	for (it=detThick.begin();it!=detThick.end();it++)
	{
		...
		else
		{
			...
			if (hSvc){
                if (hSvc->regHist("/intLen/"+(*it).first,prof).isFailure()){
...
void InteractionLengthIntegrator::BeginOfRunAction(const G4Run*)
{
}
void InteractionLengthIntegrator::EndOfRunAction(const G4Run*)
{
}
...
//	G4StepPoint *preStep=aStep->GetPreStepPoint();
	...
	double radl=mat->GetNuclearInterLength();
	double stepl=aStep->GetStepLength();
	double thickstep= radl != 0 ? stepl/radl : DBL_MAX;
