...
#include "G4UserActions/HyperspaceCatcher.h"

static HyperspaceCatcher hsc("HyperspaceCatcher");
...
void HyperspaceCatcher::initAndSetup()
{
  // here we read the property map and set the needed variables
  if(theProperties.find("treatmentLevel")==theProperties.end()){
    ATH_MSG_DEBUG( "treatmentLevel not specified, setting to default (=0)" );
  } else {
    m_treatmentLevel=strtol(theProperties["treatmentLevel"].c_str(),0,0);
  }
  ...
  if(theProperties.find("killAfter")==theProperties.end()){
    ATH_MSG_DEBUG( "killAfter not specified, setting to default (=0)" );
  } else {
    m_killAfter=strtol(theProperties["killAfter"].c_str(),0,0);
  }
...
void HyperspaceCatcher::SteppingAction(const G4Step* aStep)
{
