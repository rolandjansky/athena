...
{
  ATH_MSG_INFO( name() << " GDMLDetectorTool::initialize(): Starting" );
  ...
  if(m_detectorName.empty())
    {
      ...
      if (ipos<length)
        {
          ATH_MSG_INFO( "m_detectorName: " << m_detectorName << " needs to be reset.");
  ...
  ATH_MSG_INFO( name() << "GDMLDetectorTool::initialize() : Detector name = " << m_detectorName<<" File name: "<<m_GDMLFileName );
  ...
  ATH_MSG_INFO( name() << "GDMLDetectorTool::initialize() : Geo Detector name = " << m_geoDetectorName<<" File name: "<<m_GDMLFileName );

  ATH_MSG_INFO( name() << " GDMLDetectorTool::initialize(): Finished" );
...
//  m_envelope.thePositionedVolume=parser.GetWorldVolume();
...
//void GDMLDetectorTool::PositionInParent()
//{
//  ATH_MSG_INFO( name() << " GDMLDetectorTool::PositionInParent(): Starting" );
  
  // Method is totally overridden   
//}
