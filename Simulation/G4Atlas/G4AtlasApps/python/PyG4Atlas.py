...
class G4AtlasEngine:
    ...
    def _init_Simulation(self):
        ...
        if not is_hive:
            if not self.useISF:
                ...
                G4AtlasEngine.log.debug('not initializing RecEnvelope in ' +
                                        'G4AtlasEngine because useISF=True')
    ...
    class menu_UserActions:
        ...
        def add_UserAction(cls, action_obj):
            raise Exception("Wrong user action configuration")
...
class UserAction:
    ...
    def __init__(self, userActionLib, actionName, listStatus):
        ...
        """
        G4AtlasEngine.log.error('You are trying to configure a user action using the old-style configuration')
        G4AtlasEngine.log.error('This does not work with the present simulation framework')
        G4AtlasEngine.log.error('You should use instead something like:')
        G4AtlasEngine.log.error('    from G4AtlasServices.G4AtlasUserActionConfig import UAStore')
        G4AtlasEngine.log.error('    UAStore.addAction(\''+actionName+'\','+str(listStatus)+')')
        raise Exception('Using wrong user action configuration')
        #self._Built = False
...
class SimSkeleton(object):
    ...
    def _do_All(cls):
        ...
        known_methods = ['do_GeoSD', 'do_MCtruth', 'do_Physics']
...
class _PyG4AtlasComp(PyG4Atlas_base):
    ...
    def __init__(self, name=PyG4Atlas_name, sim_module=None):
        ...
        if "ATLAS" in simFlags.SimLayout():
            ...
            AtlasG4Eng.G4Eng.log.error('No sim skeleton registered by time of' +
                                       ' %s construction: STOP!!' % self.name())
            raise ValueError('Unknown sim setup: STOP')

        ## Import sim module if requested
        # TODO: is this ever used?
        if self.sim_module:
            G4AtlasEngine.log.info("The kernel simulation Python module which" +
                                   " describes the simulation is: %s" % self.sim_module)
            try:
                __import__(self.sim_module, globals(), locals())
            except:
                G4AtlasEngine.log.fatal("The kernel simulation Python module" +
                                        " '%s' was not found!" % self.sim_module)
        ...
        ## Start profiler
        AtlasG4Eng.G4Eng._app_profiler('%s before initialize' % self.name())
    ...
    def initialize(self):
        ...
        if "atlas_flags" in simFlags.extra_flags:
            beamcondsvc = PyAthena.py_svc('BeamCondSvc/BeamCondSvc', createIf=True,
                                          iface=cppyy.gbl.IBeamCondSvc)
