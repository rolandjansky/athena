...
{
   bool committing = (action == Transaction::TRANSACT_COMMIT
                      || action == Transaction::TRANSACT_FLUSH);
   if( committing ) {
      // refresh the branch pointer beore writing
      m_indexBranch = nullptr;
   }
   DbStatus status = RootTreeContainer::transAct(action);
   if( committing ) {
      if( m_indexBranch ) {
         m_tree->SetEntries( m_indexBranch->GetEntries() );
         if( dynamic_cast<TMemFile*>(m_tree->GetCurrentFile()) == nullptr
             && m_tree->GetEntryNumberWithIndex(nextRecordId()) == -1 )
         {
