...
//$Id: PersistencySvc.cpp 601951 2014-06-16 14:37:00Z mnowak $
...
void*
pool::PersistencySvc::PersistencySvc::readObject( const Token& token )
{
  void* result( (void*)0 );
  if ( m_session->transaction().isActive() ) {
    ...
    result = db.databaseHandler().readObject( token );
  }
  return result;
}
