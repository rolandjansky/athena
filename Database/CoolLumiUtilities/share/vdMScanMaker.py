...
# Turn data reader
from CoolLumiUtilities.TurnDataReader import TurnDataReader
...
class OflLumiMaker:
    ...
    def parseOpts(self):
        ...
        parser.add_option("--turnFile",
                          dest="turnfile", metavar="FILE", default=self.turnFile,
                          help="specify input turn number file")
        ...
        self.turnFile = options.turnfile
    ...
    def findScanData(self):
        ...
        # Load the Turn counts from text files, if available
        if self.turnFile != None:
            self.turnData = TurnDataReader(self.turnFile)
            if not self.turnData.readData():
                print 'OflLumiMaker.findScanData - No turn data found in file', self.turnFile,'!'
