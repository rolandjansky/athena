...
hltca::HLTCounter::setContent(const TH2* h, int binx) {
   m_input    = round(h->GetBinContent(binx, g_inputbin));
   m_afterPS  = round(h->GetBinContent(binx, g_afterPSbin));
   m_raw      = round(h->GetBinContent(binx, g_rawbin));
   m_total    = round(h->GetBinContent(binx, g_totalbin));
}
