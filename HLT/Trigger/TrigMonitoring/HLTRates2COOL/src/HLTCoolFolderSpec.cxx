...
hltca::HLTCoolFolderSpec::createFolderStructure(cool::IDatabasePtr db)
{
   ...
   if( !db->existsFolder( "/TRIGGER/LUMI/EFCOUNTERS" ) ) {
      cool::IFolderPtr ptr = db->createFolder( "/TRIGGER/LUMI/EFCOUNTERS", getHLTCountersFolderSpecification());
      ptr->setDescription(multiChannelDesc);
   }

   if( !db->existsFolder( "/TRIGGER/LUMI/LVL2COUNTERS" ) ) {
      cool::IFolderPtr ptr = db->createFolder( "/TRIGGER/LUMI/LVL2COUNTERS", getHLTCountersFolderSpecification());
      ptr->setDescription(multiChannelDesc);
   }
