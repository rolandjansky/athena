...
std::ostream& operator<<(std::ostream& os, const hltinterface::HLTResult& rhs) {
  os << "[HLT Result]" ; 
  if ( (rhs.fragment_pointer != 0) && ( (*rhs.fragment_pointer) != 0 ) ) {
    eformat::ROBFragment<uint32_t*> hltrob(rhs.fragment_pointer); 
    os << "\n HLT ROB Fragment: ROB fragment size = " << hltrob.fragment_size_word() << ", max_result_size = " << rhs.max_result_size ;
    os << "\n [fragment_pointer[0]=" << (*rhs.fragment_pointer)
