...
namespace
{
  ...
  {
    string ret;
    BOOST_FOREACH(const ptree::value_type& item, level)
    {
      if(ret.size() != 0)
        ret.append(delim);
...
psc::Config::Config(const ptree& config)
  : m_nworker(0) // TODO: [2ndpass] remove this
  , m_config(config)
{
...
std::string psc::Config::dumpOptions() const
{
  ...
  ost << std::setw(6) << std::hex;
  bool first = true;
  for(auto rob : enabled_robs)
  {
    if(!first)
      ost << ", ";
    ost << " 0x" << std::setw(6) << std::hex << rob;
    first = false;
  }
  ost << "]";
...
{
  optmap["WORKERTHREADS"]     = "0"; // TODO: [2ndpass] remove this
...
{
  boost::optional<const ptree&> circbuf = hlt.get_child_optional("CircBuffer");
  if(circbuf)
  {
...
void psc::Config::fill_enabled_robs(const ptree& r2r)
{
  for(const auto& ros : r2r)
    for(const auto& rob: ros.second)
      enabled_robs.push_back(rob.second.get_value<uint32_t>());
}
