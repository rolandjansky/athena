...
psc::Psc::Psc () :
      ...
      m_interactive(false),
      m_eventProcessor(dumbDestructor),
...
//--------------------------------------------------------------------------------
std::string psc::Psc::getThreadSuffix(int id)
{
  if ( m_config && (m_config->nworker()>0) ) return getGaudiThreadIDfromID(id);
  else return "";
}

IEventProcessor* psc::Psc::eventProcessor ()
{
  if (m_eventProcessor.get_ptr()) return m_eventProcessor.get_ptr();

  if (m_availableEventProcessors.size() != 0) {
    m_eventProcessor = m_availableEventProcessors.begin()->second;
    m_availableEventProcessors.erase(m_availableEventProcessors.begin());
    SmartIF<IService> eventProcessor_s(m_eventProcessor.get_ptr());
    ThreadGaudi::instance()->setThreadID( getGaudiThreadIDfromName(eventProcessor_s->name()));
    return m_eventProcessor.get_ptr();
  }
  return 0;
}
...
bool psc::Psc::configure(const ptree& config)
{
  ...
  // Tell Gaudi how many threads will be created
  std::ostringstream oss;
  oss << m_config->nworker();
  sc = propMgr->setProperty( "NoOfThreads", oss.str() );
  if (sc.isFailure()) {                                                 \
    ERS_PSC_ERROR("Can not set Property = NoOfThreads");
  return false;
  }
  ...
  if ( m_config->getOption("JOBOPTIONSTYPE") == "NONE" ) {
    ...
    if ( m_config->getOption("LOGLEVEL").substr(0,4)!="INFO" ) {
      needPython = true;
    }
  }
  else {
...
bool psc::Psc::prepareForRun (const ptree& args)
{
  ...
  // find all available EventLoopMgr and call prepare_for_run 
  ...
  // In compatibility mode we use the EventLoopMgr of "thread 0"
  unsigned int nEventLoopMgr = m_config->nworker();
  if (nEventLoopMgr==0) nEventLoopMgr = 1;

  for (unsigned int i=0; i<nEventLoopMgr; ++i) {

    std::string threadStrID = m_nameEventLoopMgr + getThreadSuffix(i);
    //                                              ^^ empty string for compat. mode

    if ( m_config->nworker()>0 )
      ERS_DEBUG(1,"---> Thread " << i << "/" << m_config->nworker() <<
          " : Name for EventLoopManager = '" << threadStrID << "'" );

    // Create the EventLoopMgr
    IEventProcessor* processingMgr = 0 ;
    sc = svcLoc->service( threadStrID , processingMgr);
    if( !sc.isSuccess() )  {
      ERS_PSC_ERROR("Error retrieving EventLoopMgr = '" << threadStrID << "'" );
      return false;
    }
    m_availableEventProcessors[i] = processingMgr ;

    SmartIF<ITrigEventLoopMgr> trigEventLoopMgr( processingMgr );
    if ( !trigEventLoopMgr.isValid() ) {
      ERS_PSC_ERROR("Error retrieveing ITrigEventLoopMgr interface of " << threadStrID) ;
      return false;
    }

    sc = trigEventLoopMgr->prepareForRun(args) ;
    if( !sc.isSuccess() )  {
      ERS_PSC_ERROR("Error executing prepareForRun for EventLoopMgr = '" << threadStrID << "'" );
      return false;
    }
  }
  ...
  /* see JIRA ATR-8907
  IIOVCondDbSvc * iovdbsvc = 0;
  sc = svcLoc->service( "IOVDbSvc" , iovdbsvc);
  if( !sc.isSuccess() ) {
    ERS_PSC_ERROR("Could not load IOVDbSvc" );
  } else if( iovdbsvc == 0 ) {
    ERS_PSC_ERROR("IOVDbSvc is 0" );
  } else {
    iovdbsvc->getDatabase()->closeDatabase();
    ERS_LOG("Closed database from IOVDbSvc");
  }
  */
  ...
  ThreadGaudi::ThreadMap* p_threadMap = ThreadGaudi::instance()->getThreadMap();
  if ( p_threadMap ) {
    p_threadMap->clear();
    (*p_threadMap)[0]="";
  }
...
bool psc::Psc::stopRun (const ptree& /*args*/)
{
  ...
  StatusCode sc = foreachEventLoopMgr<IService>(&IService::sysStop, "sysStop");
  if( !sc.isSuccess() )  {
    ERS_PSC_ERROR("Error executing stop(void)");
    return false;
  }
...
void psc::Psc::timeOutReached (const ptree& /*args*/)
{
  ...
  /*
   * Call timeOutReached for each EventLoopMgr
   */
  foreachEventLoopMgr<ITrigEventLoopMgr>(&ITrigEventLoopMgr::timeOutReached,
      "timeOutReached").ignore();
}
...
bool psc::Psc::hltUserCommand(const ptree& args)
{
  ...
  //                usrParam.push_back(boost::bind(boost::lambda::_1.second.data()));
                       ...
                       const hltinterface::EventId& evId)
{
  ...
  //
  IEventProcessor* eventLoopMgr = eventProcessor() ;
  if ( 0 == eventLoopMgr ) {
    ERS_PSC_ERROR("psc::Psc::process executeEvent EventLoopMgr: no pointer to "
        "event loop manager");
    return false;
  }

#ifdef ATLAS_GAUDI_V21
  SmartIF<ITrigEventLoopMgr> processor(eventLoopMgr);
#else
  SmartIF<ITrigEventLoopMgr> processor(ITrigEventLoopMgr::interfaceID(), eventLoopMgr);
#endif

  if ( !processor.isValid() ) {
    ERS_PSC_ERROR("psc::Psc::process: executeEvent EventLoopMgr: no valid event processor ");
    return false;
  }

#ifdef ATLAS_GAUDI_V21
  SmartIF<IService> processor_s(eventLoopMgr);
#else
  SmartIF<IService> processor_s(IID_IService, eventLoopMgr);
#endif

  ERS_DEBUG(2,"psc::Psc::process: ---> Executing WorkerThread---> " << processor_s->name());


  StatusCode sc;
  try
  {
    sc = processor->processRoIs(l1r, hltr, evId);
  }
  ...
  if( sc.isSuccess() || sc.isRecoverable() ) {
    ...
    ERS_PSC_ERROR("psc::Psc::process: Error in processing RoIs for Level 1 ID =" << lvl1_id
        << " in the EventLoopMgr " << processor_s->name() <<".");
    return false;
  }
}
...
bool psc::Psc::prepareWorker (const boost::property_tree::ptree& args)
{
  ...
  m_config->updateUnique();
  m_config->optmap["DF_APPLICATIONNAME"] = args.get_child("appName").data();

  ServiceHandle<IJobOptionsSvc> p_jobOptionSvc("JobOptionsSvc","psc::Psc");
  return setDFProperties({{"DF_Pid", "DF_PID"},
                          {"DF_Ppid", "DF_PPID"},
                          {"DF_RandomSeed", "DF_RANDOM_SEED"},
                          {"DF_ApplicationName", "DF_APPLICATIONNAME"}});
}
...
//--------------------------------------------------------------------------------
// Helper to call a function on all EventLoopMgrs
//--------------------------------------------------------------------------------
template <class T>
StatusCode psc::Psc::foreachEventLoopMgr(boost::function<StatusCode (T*)> func,
    const std::string& name)
{
  ...
  for (unsigned int i=0; i<nEventLoopMgr; ++i) {
    ...
    T* processingMgr = 0;
    StatusCode sc = svcLoc->service( threadStrID , processingMgr);
    if( !sc.isSuccess() )  {
      ERS_PSC_ERROR("Error retrieving EventLoopMgr = '" << threadStrID << "'" );
      return sc;
    }
