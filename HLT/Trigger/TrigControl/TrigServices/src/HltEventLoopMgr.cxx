...
: MinimalEventLoopMgr(nam, svcLoc),
  ...
  m_l1_hltCondUpdateLB(0xffffffff),
  ...
  m_hist_l1_robs(0)
{
  ...
  declareProperty("setConfigFromOKS",         m_setConfigFromOKS=true);
...
StatusCode HltEventLoopMgr::initialize()
{
  ...
  bool subDetOKSconfigFound = false;
  // Try to get Application Node ID etc. from OKS when running in the online environment
  if ( m_setConfigFromOKS.value() ) {
    ServiceHandle<IJobOptionsSvc> p_jobOptionsSvc("JobOptionsSvc", name());
    if ((p_jobOptionsSvc.retrieve()).isFailure()) {
      logStream() << MSG::WARNING << "Could not find JobOptionsSvc to set DataFlow properties" << endreq;
    } else {
      // Application name
      const Property* pn=Gaudi::Utils::getProperty( p_jobOptionsSvc->getProperties("DataFlowConfig"), "DF_ApplicationName");
      if (pn != 0) {
        if (m_applicationName.assign(*pn)) {
          logStream() << MSG::DEBUG << " ---> Read from DataFlow configuration: " << m_applicationName << endreq;
        } else {
          logStream() << MSG::WARNING << "Could not set Property '" << pn->name() << "' from DataFlow." << endreq;
        }
      }
      // Partition name
      const Property* ppn=Gaudi::Utils::getProperty( p_jobOptionsSvc->getProperties("DataFlowConfig"), "DF_PartitionName");
      if (ppn != 0) {
        if (m_partitionName.assign(*ppn)) {
          logStream() << MSG::DEBUG << " ---> Read from DataFlow configuration: " << m_partitionName << endreq;
        } else {
          logStream() << MSG::WARNING << "Could not set Property '" << ppn->name() << "' from DataFlow." << endreq;
        }
      }
      ...
      if (pprob != 0) {
        if (m_enabledROBs.assign(*pprob)) {
          ...
          logStream() << MSG::DEBUG << " ---> Read from DataFlow configuration: " << m_enabledROBs.value().size()
		          << " enabled ROB IDs." << endreq;
        } else {
          logStream() << MSG::WARNING << "Could not set Property '" << pprob->name() << "' from DataFlow." << endreq;
        }
      }
      ...
      if (ppsd != 0) {
        if (m_enabledSubDetectors.assign(*ppsd)) {
          ...
          logStream() << MSG::DEBUG << " ---> Read from DataFlow configuration: " << m_enabledSubDetectors.value().size()
		          << " enabled Sub Detector IDs." << endreq;
        } else {
          logStream() << MSG::WARNING << "Could not set Property '" << ppsd->name() << "' from DataFlow." << endreq;
        }
      }
      // JobOptions type
  ...
  logStream() << MSG::INFO << " ---> setConfigFromOKS       = " << m_setConfigFromOKS << endreq ;
  ...
  if (m_enabledROBs.value().size() == 0) {
    ...
    if (m_setConfigFromOKS.value() && robOKSconfigFound) {
  ...
  if (m_enabledSubDetectors.value().size() == 0) {
    logStream() << MSG::INFO << " ---> The list of enabled Sub Detectors has size = 0. No check will be performed " << endreq;
  } else {
    if (m_setConfigFromOKS.value() && subDetOKSconfigFound) {
      logStream() << MSG::INFO << " ---> The list of enabled Sub Detectors has size = " << m_enabledSubDetectors.value().size()
		      << ". It was read from the partition database."  << endreq;
    } else {
      logStream() << MSG::INFO << " ---> The list of enabled Sub Detectors has size = " << m_enabledSubDetectors.value().size()
		      << ". It was read from job options."  << endreq;
...
StatusCode HltEventLoopMgr::executeEvent(void* par)
{
  ...
  // Retrieve the EventInfo object that was already created 
  ...
  */
  ...
  extraData.appName = applicationName()+std::string(":")+System::hostName();
  ...
  //-----------------------------------------------------------------------
  if (!b_invalidCTPRob) {
    sc = hltConditionsUpdate(l1_hltCounters);
    if(sc.isFailure()) {
      if (pHltResult) {
            ...
            const hltinterface::EventId& evId)
{
  ...
  m_l1_Trigger_Info.assign(8,0);  // triggerInfo
  ...
  if (m_hist_l1_robs) {
    ...
    for(std::vector<eformat::ROBFragment<const uint32_t*> >::const_iterator it = l1_result.begin();
  it != l1_result.end(); it++) {
      m_hist_l1_robs->Fill(eformat::helper::SourceIdentifier((*it).rob_source_id()).human_detector().c_str(),1.);
    }
  }
  ...
  m_time_stamp_ns_offset = 0; // time stamp ns - ns time offset for time_stamp,  32 bit unsigned
  m_lumi_block           = 0; // luminosity block identifier,                    32 bit unsigned
  m_l1_Trigger_Info.assign(8,0);  // triggerInfo can be obtained from CTP fragment
  ...
  if( ctpRobFragmentVec.size() == 1 ) {
    ...
    // no valid CTP fragment found
    m_invalid_lvl1_result++;
    if (m_hist_l1_robs) lock_histogram_operation<TH1F>(m_hist_l1_robs)->Fill("Missing_CTP_Fragment",1.);
    std::string issue_msg = std::string("No valid CTP fragment found.  ") ;
  ...
  // TODO uncomment when supported by xAOD::EventInfo
...
StatusCode HltEventLoopMgr::stop()
{
  ...
  // Print statistics of COOL updates
  if ( not m_condUpdateOnLB.empty() ) {
    std::ostringstream done, pending;
    CondUpdateMap::const_iterator upd = m_condUpdateOnLB.begin();
    for (; upd != m_condUpdateOnLB.end(); ++upd) {
      if (upd->second) pending << " " << upd->first;
      else             done    << " " << upd->first;
    }

    if (not done.str().empty())
      logStream() << MSG::INFO << "Lumiblocks with performed COOL updates:" << done.str() << endreq;
    if (not pending.str().empty())
      logStream() << MSG::INFO << "Lumiblocks with pending COOL updates:" << pending.str() << endreq;    
  }
 ...
 *
 */
StatusCode HltEventLoopMgr::timeOutReached()
{
  //  logStream() << MSG::INFO << " ---> HltEventLoopMgr = " << name() << " timeOutReached() " << endreq;

  // Set the timeout singleton (reset is done in executeEvent)
  setTimeout(Athena::Timeout::instance());

  // Print current algorithm and event ID to ERS
  IAlgorithm* alg = m_algContextSvc ? m_algContextSvc->currentAlg() : 0;
  std::ostringstream strEvent;
  if (m_currentEvent) strEvent << " on event " << *m_currentEvent->event_ID();
  ERS_HLT_WARNING("Timeout warning received"
...
//=========================================================================
// implementation of ITrigEventLoopMgr::hltConditionsUpdate() ---> for "online" running
//=========================================================================
StatusCode HltEventLoopMgr::hltConditionsUpdate(std::vector<uint32_t>& hltCondCounter)
{
  if (logLevel() <= MSG::DEBUG) {  
    logStream() << MSG::DEBUG << " ---> HltEventLoopMgr = " << name()
		    << " hltConditionsUpdate(std::vector<uint32_t>&): size of input = " << hltCondCounter.size() << ": ";
    for (size_t i=0; i<hltCondCounter.size(); ++i) logStream() << hltCondCounter[i] << " ";
    logStream() << endreq;
  }
  if ( hltCondCounter.empty() ) return StatusCode::SUCCESS;  

  // First element is LB
  uint32_t new_hltCondUpdateLB = hltCondCounter[0];

  // Only update if counter (lumiblock for update) changed (or first event)
  if ( m_l1_hltCondUpdateLB != new_hltCondUpdateLB ) {    
    m_l1_hltCondUpdateLB = new_hltCondUpdateLB;
    ...
    logStream() << MSG::INFO << "Prescale update requested for lumiblock "
        << m_l1_hltCondUpdateLB << ". Current event: " << *pEvent->event_ID() << endreq;

    // Perform prescale update right away (including on first event)
    sc = hltPrescaleUpdate(m_l1_hltCondUpdateLB);
    return sc;
  }
...
{
  bool b_enabled = true;
  eformat::helper::SourceIdentifier src(robid);

  // check if given ROB is actually enabled for readout
  // Consider some ROBs from SubDetectorGroup TDAQ as always enabled
  if ((m_enabledROBs.value().size() != 0) &&
      (src.subdetector_id() != eformat::TDAQ_CALO_CLUSTER_PROC_ROI) &&
      (src.subdetector_id() != eformat::TDAQ_CALO_JET_PROC_ROI) &&
      (src.subdetector_id() != eformat::TDAQ_HLT)
      )
  {
...
{
  bool b_enabled = true;
  auto src = eformat::helper::SourceIdentifier(
      static_cast<eformat::SubDetector>(subdetid),0);

  // check if given subdetector is actually enabled for readout
  if ((m_enabledSubDetectors.value().size() != 0) &&
      (src.subdetector_group() != eformat::TDAQ))
  {
...
void HltEventLoopMgr::HltBookHistograms()
{
  ...
  uint32_t n_bins_l1_robs = 10 ;
  m_hist_l1_robs = new TH1F ("HLTROBsReceivedFromL1",
      "HLTROBsReceivedFromL1;;entries",
      n_bins_l1_robs,0.,(float) n_bins_l1_robs);
  if (m_hist_l1_robs) {
    uint32_t n_tmp_bin = 1;
    for (uint16_t i=0x70; i<0x78; i++) {
      eformat::helper::SourceIdentifier tmpsrc( (eformat::SubDetector) i,0);
      if ((tmpsrc.human_detector() != "UNKNOWN") && (n_tmp_bin <= n_bins_l1_robs)) {
        m_hist_l1_robs->GetXaxis()->SetBinLabel( n_tmp_bin, tmpsrc.human_detector().c_str() );
        n_tmp_bin++;
      }
    }
    m_hist_l1_robs->GetXaxis()->SetBinLabel(n_bins_l1_robs, "Missing_CTP_Fragment");
    if( m_THistSvc->regHist(path + m_hist_l1_robs->GetName(), m_hist_l1_robs).isFailure() ) {
      logStream() << MSG::WARNING << "Can not register monitoring histogram: " << m_hist_l1_robs->GetName() << endreq;
    }
  }
...
void HltEventLoopMgr::fillHltResultHistograms(hltinterface::HLTResult& hlt_result)
{
  ...
  if (m_hist_streamTagNames) {
    if(hlt_result.stream_tag.empty()) {
      lock_histogram_operation<TH1F>(m_hist_streamTagNames)->Fill( (float) m_hist_streamTagNames->GetXaxis()->GetNbins() - 1. );
    } else {
      scoped_lock_histogram lock;
...
void HltEventLoopMgr::updMetadaStore(const coral::AttributeList & sor_attrlist)
{
  auto metadata = new ByteStreamMetadata(
    ...
    // TODO add also DetectorMaskFst once supported
    0,
    0,
    "",
    "",
...
namespace
{
                        ...
                        MsgStream& log, const MSG::Level& lvl)
  {
    ...
    log << "   FilenameTag      = "
        << atr["FilenameTag"].data<std::string>() << endreq;
                               ...
                               uint32_t bcid, uint32_t l1tt, uint32_t dett)
{
  ...
  for(const auto& modid : modids)
  {
    ...
    auto initial_robsize = robp->size_word();
