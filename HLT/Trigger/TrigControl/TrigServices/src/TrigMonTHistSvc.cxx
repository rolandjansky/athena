...
StatusCode TrigMonTHistSvc::initialize() {
  ...
  lock_histogram_mutex::set_histogram_mutex(
      hltinterface::ITHistRegister::instance()->getPublicationMutex());
...
StatusCode TrigMonTHistSvc::finalize() {
  ...
  std::stringstream f;
  hltinterface::ITHistRegister::THList thlist;
  hltinterface::ITHistRegister::instance()->get(".*", thlist); 
  hltinterface::ITHistRegister::THList::iterator lIter;
  // just listing
...
template <typename T> StatusCode TrigMonTHistSvc::regHist_i(T* hist, const std::string& id) {
    ...
    if (hist->Class()->InheritsFrom(TH1::Class())) {
        hltinterface::ITHistRegister::instance()->registerTObject(name(), regid, hist);
...
StatusCode TrigMonTHistSvc::deReg(TObject* optr) {
  ...
  hltinterface::ITHistRegister::instance()->getUnsummed(".+", allObjs); 
...
StatusCode TrigMonTHistSvc::deReg(const std::string& id ) {
  ...
  hltinterface::ITHistRegister::instance()->releaseTObject(name(), id);
