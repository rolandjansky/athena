...
private:
  ...
  /**
   * @brief Accessor method to signal if OKS is used for configuring properties.
   * @return value boolean = true if OKS is used
   */
  inline bool setConfigFromOKS() const { return m_setConfigFromOKS.value(); }
  ...
  // --------------------------- Properties -------------------------------------
  BooleanProperty           m_setConfigFromOKS;  //!< Allow reading of configuration data from OKS
