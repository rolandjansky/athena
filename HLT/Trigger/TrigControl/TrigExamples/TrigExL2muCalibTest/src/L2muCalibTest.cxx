...
StatusCode L2muCalibTest::initialize(){
  ...
  // Part 4: Locate the IncidentSvc and subscribe for UpdateAfterFork incident 
...
void L2muCalibTest::handle(const Incident& incident) {
  ...
  if (incident.type()==HLT::Incidents::UpdateAfterFork::type()) {
    MsgStream log(msgSvc(), name());
    log << MSG::INFO << "+-----------------------------------+" << endreq;
    log << MSG::INFO << "| handle for UpdateAfterFork called |" << endreq;
    log << MSG::INFO << "+-----------------------------------+" << endreq;
    ...
    log << MSG::INFO << " ---------------------------------------------------------- " << endreq;
    log << MSG::INFO << " --- Parameters from UpdateAfterFork incident           --- " << endreq;
    log << MSG::INFO << " ---------------------------------------------------------- " << endreq;
    log << MSG::INFO << " Worker ID             = " << updinc.workerID() << endreq;
    log << MSG::INFO << " Worker Process ID     = " << updinc.workerProcessID() << endreq;
    log << MSG::INFO << "=================================================" << endreq;
    ...
    // Print out the property values after update from catalogue
    log << MSG::INFO << " ---------------------------------------------------------- " << endreq;
    log << MSG::INFO << " --- Updated properties after forking the workers       --- " << endreq;
    log << MSG::INFO << " ---------------------------------------------------------- " << endreq;
    log << MSG::INFO << " AllowOksConfig        = " << m_allowOksConfig << endreq;
    log << MSG::INFO << " DumpPropertyCatalogue = " << m_dumpCatalogue << endreq;
    log << MSG::INFO << " MuonCalBufferName     = " << m_muonCalBufferName << endreq;
