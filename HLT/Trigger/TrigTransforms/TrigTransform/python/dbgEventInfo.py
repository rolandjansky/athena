...
class dbgEventInfo:
    ...
    def event_info(self, event, L1Chain_Names, HLTChain_Names):
        ...
        all_name_streamtags_of_an_event = ','.join([tag.name for tag in event.stream_tag()])
        self.Stream_Tag_Name = all_name_streamtags_of_an_event

        all_type_streamtags_of_an_event = ','.join([tag.type for tag in event.stream_tag()])
    ...
    def lvl1_info(self, event, L1Chain_Names):
        ...
        try:
            ...
            for j in range(24):
                k += 1
                if k == 8:
                    # Count the three different sets of words - that is counter l
                    k = 0
                    l += 1
                for i in range(32):
        ...
        if EF_Result:
            if EF_Result[0] == 0:
                ...
                try:
                    ...
                    msg.info('******** EF_Status_Word[EF_Result[2]]: {0}'.format(self.EF_Status_Word[EF_Result[2]]))
                except IndexError:
