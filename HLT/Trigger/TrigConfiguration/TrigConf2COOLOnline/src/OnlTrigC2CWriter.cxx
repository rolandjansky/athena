...
OnlTrigC2CWriter::OnlTrigC2CWriter(const std::string & partitionName, const std::string& cooldb, bool l1is, bool l1cool) :
   Controllable()
...
TrigConf::OnlTrigC2CWriter::readTriggerDb() {
   ...
   //std::unique_ptr< StorageMgr > sm( new StorageMgr("TRIGGERDBV2","","", cout) );
...
TrigConf::OnlTrigC2CWriter::user(const daq::rc::UserCmd& usrCmd) {
   ...
   if( commandName == "HLTPRESCALE2COOL" ) {
      ...
      try {
         hltps.checkin();
         ERS_INFO("Published currentl hlt pskey " << pskey << " to RunParams.TrigConfHltPsKey");
