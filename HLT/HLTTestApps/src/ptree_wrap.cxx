...
void HLTTestApps::wrap_ptree()
{
  ...
  class_<ptree> ("ptree") // default constructor taken care of

    // default policy
    .def(init<string>())
    .def("readxml", readxml)
    .def("__str__", to_string)
    .def("__len__", &ptree::size)
    .def("set_data", set_data)
    .def("keys", keys)
    .def("values", values)
    .def("values_data", values_data)
    .def("items", items)
    .def("__iter__", iter)
    .def("__contains__", in)

    // Strings are immutable => python does not accept refs => need to copy
    .def("data", const_data, return_value_policy<copy_const_reference>())
    .def("__getitem__", get, return_value_policy<copy_const_reference>())

    // Can return internal reference
    .def("__setitem__", put, return_internal_reference<>())
    ...
    .def("add_child", add_child, return_internal_reference<>())

    // if we need to add a sub-ptree (replacing existing path)
    .def("put_child", put_child, return_internal_reference<>())
    ...
    // can be modified.
    .def("get_child", get_child, return_internal_reference<>())
  ;
}
