...
from HLTTestApps import InfoService, ipc_init
...
class AutoFinInfoService(InfoService):
  """
  Auto finalize if, when deleting, somehow the last preRunHook call was still 
  not followed by a finalize call 
  """
  def __init__(self, finalize_ptree, *args):
    InfoService.__init__(self, *args)
    self.finalize_ptree = finalize_ptree
    self.needFinalize = False
  def __del__(self):
    ...
    finalize_THistSvc()
    logging.debug("Deleting AutoFinInfoService")
    if self.needFinalize:
      logging.debug("Finalizing the InfoService")
      self.finalize()
  def preRunHook(self, *args):
    ret = InfoService.preRunHook(self, *args)
    if ret:
      self.needFinalize = True
    return ret
  def finalize(self, *args):
    ret = InfoService.finalize(self, self.finalize_ptree, *args)
    if ret:
      self.needFinalize = False
    return ret
...
class online_infrastructure(infrastructure):
  ...
  def __init__(self, config):
    ...
    self.mon = AutoFinInfoService(self.config.get_mon_finalize_ptree(),
                                  config['info-service'])
  def __del__(self):
  ...
  def prepareForRun(self):
    ...
    return (self.mon.prepareForRun(self.config.get_mon_prepare_ptree()) and
            self.mon.preRunHook(self.config.get_mon_run_ptree()))
  def run(self):
    self.run_index += 1
    return infrastructure.run(self)
  def stopRun(self):
    infrastructure.stopRun(self)
    ret = self.mon.postRunHook(self.config.get_mon_stop_ptree())
    ret &= self.mon.finalize()
