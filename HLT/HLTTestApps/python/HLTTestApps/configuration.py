...
from HLTTestApps import set_ros2rob_map 
...
mon_config_rule_path       = ("HLTMonInfoImpl.ConfigurationRules."
                              "ConfigurationRuleBundle.ConfigurationRules."
                              "ConfigurationRule")
monparams_extension        = "Parameters.OHPublishingParameters"
oh_params_path             = mon_config_rule_path + "." + monparams_extension

# DB constants
prescales_db_str = 'COMP'
runparams_db_str = 'COOLONL_TDAQ/COMP200'
runparams_path   = '/TDAQ/RunCtrl/SOR_Params'
...
class configuration(dict):
  ...
  def get_prepare_ptree(self):
    ...
    innerpt['filename_tag']      = str(rparams['FilenameTag'])
  ...
  def get_mon_run_ptree(self):
    pt = ptree()
    if self['oh-monitoring']:
      pt['appName'] = 'athenaHLT'
    return pt
  ...
  def __digest_config(self):
    self.__setup_input()
    self.__setup_sor() # this throws if the format isn't good
  ...
  def __setup_input(self):
    ...
    # which is important for 'use-database' once it is implemented)
    self.stream = None
    if 'file' in self:
  ...
  def __get_dmask(self, rparams):
    dmask = hex(self['detector-mask'] if self['detector-mask']
                                      else rparams['DetectorMask'])
  ...
  def __get_cool_default_getter(self, param):
    ...
    Notice that no db connection is ever created until a getters returned by 
    ...
    def db():
      if not self.dbconn:
        self.dbconn = CoolUtil.GetConnection(prescales_db_str)
      return self.dbconn
  ...
  def __get_run_params(self):
    dbcon = AtlCoolLib.indirectOpen(runparams_db_str, True, True, False)
    folder = dbcon.getFolder(runparams_path)
  ...
  def __add_common_ptree(self, pt):
    ...
    pt["CircBuffer.CircName"] = self['muoncal-buffername']
    pt["CircBuffer.CircSize"] = self['muoncal-buffersize']
...
class configuration_tests(unittest.TestCase):
  ...
  def test_ptree_muoncal(self):
    ...
    buf_path = self.hltconf + '.CircBuffer'
    pt = self._gen_complete_config_ptree(['--muoncal-buffername', bufname,
                                          '--muoncal-buffersize', bufsize])
    self._test_ptree_value(pt, buf_path + ".CircName", bufname)
    self._test_ptree_value(pt, buf_path + ".CircSize", bufsize)
  ...
  def _test_ptree_common(self, pt):
    ...
    self._test_ptree_value(pt, self.hltconf + ".CircBuffer.CircName", '')
    self._test_ptree_value(pt, self.hltconf + ".CircBuffer.CircSize", '')
...
class pcommands_tests:
  ...
  def _test_ptree_pcommands(self, p):
    ...
    actual = filter(lambda c: 'include' not in c, actual)
