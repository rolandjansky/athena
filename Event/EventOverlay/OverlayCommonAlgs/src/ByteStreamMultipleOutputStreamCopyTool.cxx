...
void ByteStreamMultipleOutputStreamCopyTool::initlbnmap(){
  ...
  if (m_trigfile!=""){
    ...
    while (fgets(line,480,fp)) {
      if (line[0]=='-') continue;
      int s=sscanf(&line[0],"run_nbr=%d, evt_nbr=%d, time_stamp=%d, lbk_nbr=%d, noalg=%d, j40=%d, noalgps=%d, j40\
ps=%d",&run,&evt,&time,&lbn,&noalg,&j40,&psnoalg,&psj40);
      if (s>0) {
        ATH_MSG_INFO(evtinfile<<": "<<run<<" "<<lbn<<" "<<evt<<": passed noalg j40 "<<noalg<<" "<<j40<<", prescal\
e noalg j40 "<<psnoalg<<" "<<psj40);
