...
# endif
  ...
  tds.record(cont, name).ignore();
	...
	xAOD::Jet *jet = jetCont.back();
	jet->setJetP4( v ); // set the P4 of the last inserted jet
	for( const auto& attpair : jetAttMapF) { jet->setAttribute<float>(attpair.first, attpair.second[i]);}
	for( const auto& attpair : jetAttMapI) { jet->setAttribute<int>(attpair.first, attpair.second[i]);}
	for( const auto& attpair : jetAttMap4M) { jet->setAttribute<xAOD::JetFourMom_t>(attpair.first, attpair.second[i]);}
	i++;
      }  
      ...
      static const std::vector<xAOD::JetFourMom_t> jet4moms =
	{
	 ...
	 xAOD::JetFourMom_t(100000, 1.0, 2, 1000000.)
      ...
      fillJets(jetCont, jet4moms,
	       // pass some jet attributes :
	       { { "Width", std::vector<float>(jet4moms.size(), 0.123 )} },  // a map<string,vectot<float>
	       { {"NumTrkPt1000", std::vector<int>(jet4moms.size(), 5)} } ,  // a map<string,vectot<int>
	       {}
	       );

      // Set also some JetScale so they can be tested.
      //  We don't need these scales to have precise signification, just they are
      //  relatively close to the default scale, as what one could expect for a real jets (typically a calibrated LCTopo jets)
      for(Jet* jet: jetCont){
