...
namespace xAODMaker {
    ...
    : AthAlgorithm( name, svcLoc ), m_metaStore( "MetaDataStore", name ), inputMetaStore( "StoreGateSvc/InputMetaDataStore",name)
  {
    ...
    declareProperty("AODAdditionalContainerNames", m_aodAdditionalContainerNames,"List of additional input collections -eg pileup- to convert to xAOD");
  ...
  StatusCode xAODTruthCnvAlg::execute() {
    ...
    if (evtStore()->contains<McEventCollection>(m_aodContainerName)) {
      //Storing all input collection names into inputMcEventCollNames
      std::vector<std::string> inputMcEventCollNames;
      inputMcEventCollNames.push_back(m_aodContainerName);
      //Check the presence of other requested input McEventCollections
      for (auto collName : m_aodAdditionalContainerNames) {
        if (!evtStore()->contains<McEventCollection>(collName))
          ATH_MSG_WARNING("McEventCollection with name " << collName << " not found, will not dump it into " << m_xaodTruthPUEventContainerName);
        else
          inputMcEventCollNames.push_back(collName);
      }
      ...
      if (m_doAllPileUp || m_doInTimePileUp || inputMcEventCollNames.size()>1) {
      ...
      // This consists of three parts for each input McEventCollection:
      // (1)  For each Athena event, loop over the GenEvents and build TruthEvent collections
      //      There can be more than one GenEvent per event
      ...
      //Loop over all input McEventCollections
      for (auto inputCollName : inputMcEventCollNames) {
        // Retrieve the HepMC truth:
        const McEventCollection *mcColl(nullptr);
        CHECK( evtStore()->retrieve( mcColl, inputCollName ) );
        ATH_MSG_DEBUG( "Retrieved HepMC with key: " << inputCollName );

        EBC_EVCOLL hmplEnum = HepMcParticleLink::find_enumFromKey(inputCollName);
        ...
        for (unsigned int cntr = 0; cntr < mcColl->size(); ++cntr) {
          ...
          const unsigned int evNum = genEvt->event_number();
          bool isSignalProcess(false);
          if (hmplEnum==EBC_MAINEVCOLL && cntr==0) isSignalProcess=true;
          if (cntr>0) {
            ...
            if (inputMcEventCollNames.size()==1 && m_doInTimePileUp && pid==0 && eventNumber==-1) break; // stop at the separator in case we deal with only one McEventCollection (the 'HS' one or the old style)
          }
          //For the new PU scheme, if we want to keep only in-time events, checking the timing of the first vertex in the GenEvent
          if (hmplEnum!=EBC_MAINEVCOLL && m_doInTimePileUp && genEvt->vertices_size()) {
            PileUpTimeEventIndex::time_type vtxTime = (*genEvt->vertices_begin())->position().t();
            if (vtxTime<-24. || vtxTime>24.)
              continue;
          }
          ...
          for (HepMC::GenEvent::particle_const_iterator pitr=genEvt->particles_begin(); pitr!=genEvt->particles_end(); ++pitr) {
            ...
            if (isSignalProcess) truthLinkVec->push_back(new xAODTruthParticleLink(HepMcParticleLink((*pitr),evNum,hmplEnum), eltp));
            if (!isSignalProcess) truthLinkVec->push_back(new xAODTruthParticleLink(HepMcParticleLink((*pitr),evNum,hmplEnum), eltp));
          ...
          for (const HepMC::GenVertex* vertex : vertices) {
            ...
            if (isSignalProcess && vertex == signalProcessVtx) xTruthEvent->setSignalProcessVertexLink(eltv);
        ...
        } // end of loop over McEventCollection's GenEvents
      } // end of loop over input McEventCollections
    } else {
      ...
      for (auto evt : *xTruthEventContainer) {
        for (const auto& par : evt->truthParticleLinks()) {
          ...
          truthLinkVec->push_back(new xAODTruthParticleLink(HepMcParticleLink((*par)->barcode(), 0, EBC_MAINEVCOLL), par));
        }
      }
