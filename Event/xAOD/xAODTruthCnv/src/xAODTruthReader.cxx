...
namespace xAODReader {
  ...
  StatusCode xAODTruthReader::execute() {
    ...
    for (const xAOD::TruthEvent* evt : *xTruthEventContainer) {
      ...
      // Print number of truth events
      //cout << endl << "Number of signal truth events = " << xTruthPUEventContainer->size() << endl;
    }
  ...
  void xAODTruthReader::printEvent(const xAOD::TruthEvent* event) {
    cout << "--------------------------------------------------------------------------------\n";
    cout << "GenEvent: #" << "NNN" << "\n";
