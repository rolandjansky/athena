  ...
  for( ; it!= itE; ++it)
    {
      ...
      //    ATH_MSG_DEBUG( "trying to convert " << tau  << " with  pt="<< tau->pt() );    
      ...
      // xtaujet->setP4( tau->pt(), tau->eta(), tau->phi(), tau->m(), tau->e() );
     ...
     // xtaujet->setPtJetSeed(tau->getHLV(TauJetParameters::JetSeed).perp());
     // xtaujet->setEtaJetSeed(tau->getHLV(TauJetParameters::JetSeed).eta());
     // xtaujet->setPhiJetSeed(tau->getHLV(TauJetParameters::JetSeed).phi());
     // xtaujet->setEJetSeed(tau->getHLV(TauJetParameters::JetSeed).e());
     // xtaujet->setMJetSeed(tau->getHLV(TauJetParameters::JetSeed).m());
     ...
     // xtaujet->setPtDetectorAxis(tau->getHLV(TauJetParameters::DetectorAxis).perp());
     // xtaujet->setEtaDetectorAxis(tau->getHLV(TauJetParameters::DetectorAxis).eta());
     // xtaujet->setPhiDetectorAxis(tau->getHLV(TauJetParameters::DetectorAxis).phi());
     // xtaujet->setEDetectorAxis(tau->getHLV(TauJetParameters::DetectorAxis).e());
     // xtaujet->setMDetectorAxis(tau->getHLV(TauJetParameters::DetectorAxis).m());
     ...
     // xtaujet->setPtIntermediateAxis(tau->getHLV(TauJetParameters::IntermediateAxis).perp());
     // xtaujet->setEtaIntermediateAxis(tau->getHLV(TauJetParameters::IntermediateAxis).eta());
     // xtaujet->setPhiIntermediateAxis(tau->getHLV(TauJetParameters::IntermediateAxis).phi());
     // xtaujet->setEIntermediateAxis(tau->getHLV(TauJetParameters::IntermediateAxis).e());
     // xtaujet->setMIntermediateAxis(tau->getHLV(TauJetParameters::IntermediateAxis).m());
     ...
     // xtaujet->setPtTauEnergyScale(tau->getHLV(TauJetParameters::TauEnergyScale).perp());
     // xtaujet->setEtaTauEnergyScale(tau->getHLV(TauJetParameters::TauEnergyScale).eta());
     // xtaujet->setPhiTauEnergyScale(tau->getHLV(TauJetParameters::TauEnergyScale).phi());
     // xtaujet->setETauEnergyScale(tau->getHLV(TauJetParameters::TauEnergyScale).e());
     // xtaujet->setMTauEnergyScale(tau->getHLV(TauJetParameters::TauEnergyScale).m());
     ...
     // xtaujet->setPtTauEtaCalib(tau->getHLV(TauJetParameters::TauEtaCalib).perp());
     // xtaujet->setEtaTauEtaCalib(tau->getHLV(TauJetParameters::TauEtaCalib).eta());
     // xtaujet->setPhiTauEtaCalib(tau->getHLV(TauJetParameters::TauEtaCalib).phi());
     // xtaujet->setETauEtaCalib(tau->getHLV(TauJetParameters::TauEtaCalib).e());
     // xtaujet->setMTauEtaCalib(tau->getHLV(TauJetParameters::TauEtaCalib).m());
     ...
     for (unsigned int i = 0; i < commonDetails->TrackFilterPass().size(); ++i) 
    {
      ...
      // ATH_MSG_DEBUG( " after setting trackfilter pass on xtau" );

      // xtaujet->setTrackFilterPass(  i,     commonDetails->TrackFilterPass().at(i) );			  
      ATH_MSG_DEBUG( "converted xaod tau with trackFilterPass " << xtaujet->trackFilterPass(i)  << " for track " << i  );
...
void TauJetCnvAlg::setLinks(const Analysis::TauJet& aodtau, xAOD::TauJet& xaodtau) const {
...
ElementLink<xAOD::TrackParticleContainer> TauJetCnvAlg::getNewTrackLink(const ElementLink<Rec::TrackParticleContainer>& oldLink, const std::string& name) const{
  ElementLink<xAOD::TrackParticleContainer> newLink;
  newLink.resetWithKeyAndIndex( name, oldLink.index() );
  return newLink;
}

ElementLink<xAOD::JetContainer> TauJetCnvAlg::getNewJetLink(const ElementLink<JetCollection>& oldLink, const std::string& name) const{
  ElementLink<xAOD::JetContainer> newLink;
  newLink.resetWithKeyAndIndex( name, oldLink.index() );
  return newLink;
}

void TauJetCnvAlg::copyPanTauDetails(const Analysis::TauJet& aodtau, xAOD::TauJet& xaodtau) const {
