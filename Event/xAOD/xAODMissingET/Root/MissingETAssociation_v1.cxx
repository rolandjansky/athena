...
bool MissingETAssociation_v1::setCalKey(size_t objIdx,MissingETBase::Types::bitmask_t calkey)
{ if ( objIdx != MissingETBase::Numerical::invalidIndex() && objIdx < this->f_calkey().size() ) { this->f_calkey()[objIdx] = calkey; return true; } else { return false; } }
...
bool MissingETAssociation_v1::setTrkKey(size_t objIdx,MissingETBase::Types::bitmask_t trkkey)
{ if ( objIdx != MissingETBase::Numerical::invalidIndex() && objIdx < this->f_trkkey().size() ) { this->f_trkkey()[objIdx] = trkkey; return true; } else { return false; } }
...
{ return objIdx != MissingETBase::Numerical::invalidIndex() && objIdx < this->sizeCal() ? this->calpx()[objIdx] : MissingETBase::Numerical::cpxError(); }
double MissingETAssociation_v1::calpy(size_t objIdx) const
{ return objIdx != MissingETBase::Numerical::invalidIndex() && objIdx < this->sizeCal() ? this->calpy()[objIdx] : MissingETBase::Numerical::cpyError(); }
double MissingETAssociation_v1::calpz(size_t objIdx) const
{ return objIdx != MissingETBase::Numerical::invalidIndex() && objIdx < this->sizeCal() ? this->calpz()[objIdx] : MissingETBase::Numerical::cpzError(); }
double MissingETAssociation_v1::cale(size_t objIdx) const
{ return objIdx != MissingETBase::Numerical::invalidIndex() && objIdx < this->sizeCal() ? this->cale()[objIdx] : MissingETBase::Numerical::ceError(); }
double MissingETAssociation_v1::calsumpt(size_t objIdx) const
{ return objIdx != MissingETBase::Numerical::invalidIndex() && objIdx < this->sizeCal() ? this->calsumpt()[objIdx] : MissingETBase::Numerical::sumptError(); }
MissingETBase::Types::bitmask_t MissingETAssociation_v1::calkey(size_t objIdx) const
{ return objIdx != MissingETBase::Numerical::invalidIndex() && objIdx < this->sizeCal() ? this->calkey()[objIdx] : MissingETBase::Numerical::invalidIndex(); }

double MissingETAssociation_v1::trkpx(size_t objIdx) const
{ return objIdx != MissingETBase::Numerical::invalidIndex() && objIdx < this->sizeTrk() ? this->trkpx()[objIdx] : MissingETBase::Numerical::cpxError(); }
double MissingETAssociation_v1::trkpy(size_t objIdx) const
{ return objIdx != MissingETBase::Numerical::invalidIndex() && objIdx < this->sizeTrk() ? this->trkpy()[objIdx] : MissingETBase::Numerical::cpyError(); }
double MissingETAssociation_v1::trkpz(size_t objIdx) const
{ return objIdx != MissingETBase::Numerical::invalidIndex() && objIdx < this->sizeTrk() ? this->trkpz()[objIdx] : MissingETBase::Numerical::cpzError(); }
double MissingETAssociation_v1::trke(size_t objIdx) const
{ return objIdx != MissingETBase::Numerical::invalidIndex() && objIdx < this->sizeTrk() ? this->trke()[objIdx] : MissingETBase::Numerical::ceError(); }
double MissingETAssociation_v1::trksumpt(size_t objIdx) const
{ return objIdx != MissingETBase::Numerical::invalidIndex() && objIdx < this->sizeTrk() ? this->trksumpt()[objIdx] : MissingETBase::Numerical::sumptError(); }
MissingETBase::Types::bitmask_t MissingETAssociation_v1::trkkey(size_t objIdx) const
{ return objIdx != MissingETBase::Numerical::invalidIndex() && objIdx < this->sizeTrk() ? this->trkkey()[objIdx] : MissingETBase::Numerical::invalidIndex(); }
...
{ return objIdx != MissingETBase::Numerical::invalidIndex() && objIdx < this->sizeCal() 
...
{ return objIdx != MissingETBase::Numerical::invalidIndex() && objIdx < this->sizeTrk() 
...
{
  this->resetContrib();
  std::map<MissingETBase::Types::bitmask_t,xAOD::MissingETAssociation_v1::ConstVec> trkOverlaps;
  std::map<MissingETBase::Types::bitmask_t,xAOD::MissingETAssociation_v1::ConstVec> calOverlaps;
  ...
  for (std::map<ElementLink<IParticleContainer>,MissingETBase::Types::bitmask_t>::iterator it = objects.begin(); it != objects.end(); ++it) {
    if ((*it->first)->type()==xAOD::Type::TrackParticle) trkOverlaps[it->second] += MissingETBase::Types::constvec_t(**it->first);
    else if ((*it->first)->type()==xAOD::Type::ParticleFlow && m_override.find(*it->first)!=m_override.end()) calOverlaps[it->second] += m_override[*it->first];
    else if ((*it->first)->type()==xAOD::Type::ParticleFlow) {
      trkOverlaps[it->second] += MissingETBase::Types::constvec_t(**it->first);
      calOverlaps[it->second] += MissingETBase::Types::constvec_t(**it->first);
    }
    else calOverlaps[it->second] += MissingETBase::Types::constvec_t(**it->first);
  }
  for (std::map<MissingETBase::Types::bitmask_t,xAOD::MissingETAssociation_v1::ConstVec>::iterator it = calOverlaps.begin(); it != calOverlaps.end(); ++it) {
    this->f_calkey().push_back(it->first);
    this->f_calpx().push_back(it->second.cpx());
    this->f_calpy().push_back(it->second.cpy());
    this->f_calpz().push_back(it->second.cpz());
    this->f_cale().push_back(it->second.ce());
    this->f_calsumpt().push_back(it->second.sumpt());
  }
  for (std::map<MissingETBase::Types::bitmask_t,xAOD::MissingETAssociation_v1::ConstVec>::iterator it = trkOverlaps.begin(); it != trkOverlaps.end(); ++it) {
    this->f_trkkey().push_back(it->first);
    this->f_trkpx().push_back(it->second.cpx());
    this->f_trkpy().push_back(it->second.cpy());
    this->f_trkpz().push_back(it->second.cpz());
    this->f_trke().push_back(it->second.ce());
    this->f_trksumpt().push_back(it->second.sumpt());
  }
...
xAOD::JetFourMom_t MissingETAssociation_v1::getAlternateConstVec() const
{
    ...
    for (size_t iKey = 0; iKey < this->cale().size(); iKey++) if (mask & this->calkey()[iKey]) newpt += ConstVec(this->calpx()[iKey],this->calpy()[iKey],this->calpz()[iKey],this->cale()[iKey],this->calsumpt()[iKey]);
...
{
  constvec_t calvec;
  for (size_t iKey = 0; iKey < this->cale().size(); iKey++) {
    bool selector = (!this->isOldFormat() ? m_useObjectFlags & this->calkey()[iKey] : this->objSelected(iKey)) ? !this->isMisc() : this->isMisc();
    // printf("This is old format? %o. Selector? %o\n",this->isOldFormat(),selector);
    if (this->isOldFormat()) selector = selector && !this->hasOverlaps(iKey,MissingETBase::UsageHandler::OnlyCluster) && !this->hasOverlaps(iKey,MissingETBase::UsageHandler::ParticleFlow);
    if (selector) calvec+=ConstVec(this->calpx()[iKey],this->calpy()[iKey],this->calpz()[iKey],this->cale()[iKey],this->calsumpt()[iKey]);
    // printf("Updated selector? %o. Overlaps? %o\n",selector,this->hasOverlaps(iKey,MissingETBase::UsageHandler::OnlyCluster));
  }
  return calvec;
}
...
{
  constvec_t trkvec;
  for (size_t iKey = 0; iKey < this->trke().size(); iKey++) {
    bool selector = (!this->isOldFormat() ? m_useObjectFlags & this->trkkey()[iKey] : this->objSelected(iKey)) ? !this->isMisc() : this->isMisc();
    if (this->isOldFormat()) {
      selector = selector && !this->hasOverlaps(iKey,MissingETBase::UsageHandler::ParticleFlow);
      if (this->objectLinks()[iKey].isValid() && *(this->objectLinks()[iKey]) && (*this->objectLinks()[iKey])->type()==xAOD::Type::Muon) selector = selector && !this->hasOverlaps(iKey,MissingETBase::UsageHandler::OnlyTrack);
      else selector = selector && !this->hasOverlaps(iKey,MissingETBase::UsageHandler::TrackCluster);
    }
