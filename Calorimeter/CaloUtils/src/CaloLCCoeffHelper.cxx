...
const CaloLocalHadCoeff::LocalHadArea * CaloLCCoeffHelper::getAreaFromName(const CaloLocalHadCoeff * m_coeff, std::string sname, int &m_indx) const
{
...
**************************************************************************** */
void CaloLCCoeffHelper::PrintData(CaloLocalHadCoeff *m_data, const char *fname)
{
  std::ofstream fout;
  fout.open(fname);
...
**************************************************************************** */
void CaloLCCoeffHelper::PrintData(CaloLocalHadCoeff *m_data, std::ostream &fout)
{
...
/* **************************************************************************
   Interpolate m_data, area  in phase space point x, over dimensions dim (all if 0)
   for speed reason, no check on dim content !!!!!!!!
*************************************************************************** */
bool CaloLCCoeffHelper::Interpolate(const CaloLocalHadCoeff *m_data, const unsigned int n_area, 
                    std::vector<float> &x, CaloLocalHadCoeff::LocalHadCoeff &pars,  
                    const std::vector<int> &dim, double xfit) 
{
  // Sanity check
  if( n_area >= (unsigned int)m_data->getSizeAreaSet()) return false;
  // We are not extrapolator
  if(!m_data->isFilled(m_data->getBin(n_area,x))) {
    return false;
  }

  const CaloLocalHadCoeff::LocalHadArea *area = m_data->getArea(n_area);
  unsigned int ndim;
  if(!dim.size()) {
  ...
  switch (area->getType()) {
    case CaloLocalHadDefs::AREA_STD: case CaloLocalHadDefs::AREA_DMLOOKUP : case CaloLocalHadDefs::AREA_DMSMPW: case CaloLocalHadDefs::AREA_DMFIT: {
      nip = m_data->getCoeff(area->getOffset()+1)->size(); 
      ...
      for(unsigned int ip=0; ip<nip; ++ip) {
        bool isa =   m_data->getInterpArrays(n_area,dim,x,vXadj,vgbins);
        if(! isa) {
        ...
        for(unsigned int i_len=0; i_len<ncorners; ++i_len){
          ...
          if(m_data->isFilled(vgbins[i_len])) {
            const CaloLocalHadCoeff::LocalHadCoeff * lpars =
              m_data->getCoeff(vgbins[i_len]);
            if(area->getType() == CaloLocalHadDefs::AREA_DMFIT) {
            ...
            m_data->bin2indexes(vgbins[i_len],ebindx);
            ...
            for(unsigned int blen=0; blen<ncorners; ++blen){
              ...
              if(!m_data->isFilled(vgbins[blen])){
                m_data->bin2indexes(vgbins[i_len],bx);
                //for(unsigned int l=0; l<bx.size(); ++l) { std::cout<<bx[l]<<" "; }
                //std::cout<<" nf| ";
                continue;
              }
              if(m_data->isNeighbour(vgbins[blen], ebindx)) {
                const CaloLocalHadCoeff::LocalHadCoeff * lpars =
                  m_data->getCoeff(vgbins[blen]);
                if(area->getType() == CaloLocalHadDefs::AREA_DMFIT) {
                  vWeights[i_len] = (*lpars)[0] + (*lpars)[1]*pow(xfit,(*lpars)[2]);
                } else {
                  vWeights[i_len] += (*lpars)[ip];
