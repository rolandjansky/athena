...
MomentName moment_names[] = {
  { "ENERGY_Truth",         xAOD::CaloCluster::ENERGY_Truth },
  { "ETA_Truth",         xAOD::CaloCluster::ETA_Truth },
  { "PHI_Truth",         xAOD::CaloCluster::PHI_Truth },
  { "AVG_LAR_Q_Truth",         xAOD::CaloCluster::AVG_LAR_Q_Truth },
  { "AVG_TILE_Q_Truth",        xAOD::CaloCluster::AVG_TILE_Q_Truth },
  { "BADLARQ_FRAC_Truth",      xAOD::CaloCluster::BADLARQ_FRAC_Truth },
  { "BAD_CELLS_CORR_E_Truth",  xAOD::CaloCluster::BAD_CELLS_CORR_E_Truth },
  { "CELL_SIGNIFICANCE_Truth", xAOD::CaloCluster::CELL_SIGNIFICANCE_Truth },
  { "CELL_SIG_SAMPLING_Truth", xAOD::CaloCluster::CELL_SIG_SAMPLING_Truth },
  { "CENTER_LAMBDA_Truth",     xAOD::CaloCluster::CENTER_LAMBDA_Truth },
  { "CENTER_MAG_Truth",        xAOD::CaloCluster::CENTER_MAG_Truth },
  { "CENTER_X_Truth",          xAOD::CaloCluster::CENTER_X_Truth },
  { "CENTER_Y_Truth",          xAOD::CaloCluster::CENTER_Y_Truth },
  { "CENTER_Z_Truth",          xAOD::CaloCluster::CENTER_Z_Truth },
  { "DELTA_ALPHA_Truth",       xAOD::CaloCluster::DELTA_ALPHA_Truth },
  { "DELTA_PHI_Truth",         xAOD::CaloCluster::DELTA_PHI_Truth },
  { "DELTA_THETA_Truth",       xAOD::CaloCluster::DELTA_THETA_Truth },
  { "ENG_BAD_CELLS_Truth",     xAOD::CaloCluster::ENG_BAD_CELLS_Truth },
  { "ENG_BAD_HV_CELLS_Truth",  xAOD::CaloCluster::ENG_BAD_HV_CELLS_Truth },
  { "ENG_FRAC_CORE_Truth",     xAOD::CaloCluster::ENG_FRAC_CORE_Truth },
  { "ENG_FRAC_EM_Truth",       xAOD::CaloCluster::ENG_FRAC_EM_Truth },
  { "ENG_FRAC_MAX_Truth",      xAOD::CaloCluster::ENG_FRAC_MAX_Truth },
  { "ENG_POS_Truth",           xAOD::CaloCluster::ENG_POS_Truth },
  { "FIRST_ENG_DENS_Truth",    xAOD::CaloCluster::FIRST_ENG_DENS_Truth },
  { "FIRST_ETA_Truth",         xAOD::CaloCluster::FIRST_ETA_Truth },
  { "FIRST_PHI_Truth",         xAOD::CaloCluster::FIRST_PHI_Truth },
  { "ISOLATION_Truth",         xAOD::CaloCluster::ISOLATION_Truth },
  { "LATERAL_Truth",           xAOD::CaloCluster::LATERAL_Truth },
  { "LONGITUDINAL_Truth",      xAOD::CaloCluster::LONGITUDINAL_Truth },
  { "N_BAD_CELLS_Truth",       xAOD::CaloCluster::N_BAD_CELLS_Truth },
  { "N_BAD_HV_CELLS_Truth",    xAOD::CaloCluster::N_BAD_HV_CELLS_Truth },
  { "N_BAD_CELLS_CORR_Truth",  xAOD::CaloCluster::N_BAD_CELLS_CORR_Truth },
  { "SECOND_ENG_DENS_Truth",   xAOD::CaloCluster::SECOND_ENG_DENS_Truth },
  { "SECOND_LAMBDA_Truth",     xAOD::CaloCluster::SECOND_LAMBDA_Truth },
  { "SECOND_R_Truth",          xAOD::CaloCluster::SECOND_R_Truth },
  { "SIGNIFICANCE_Truth",      xAOD::CaloCluster::SIGNIFICANCE_Truth },
};
...
CaloClusterMomentsMaker_Truth::CaloClusterMomentsMaker_Truth(const std::string& type, 
...
StatusCode CaloClusterMomentsMaker_Truth::initialize()
{
  ...
  else
  {
    sc = detStore()->regFcn(&IGeoModelSvc::geoInit,
			  geoModel,
			  &CaloClusterMomentsMaker_Truth::geoInit,this);
    if(sc.isFailure())
    {
...
StatusCode
CaloClusterMomentsMaker_Truth::geoInit(IOVSVC_CALLBACK_ARGS)
{
  ...
  for(const auto& name: m_momentsNames) {
    ...
    if (it != moment_names_end) {
      ...
      switch (it->mom) {
      case xAOD::CaloCluster::SIGNIFICANCE_Truth:
      case xAOD::CaloCluster::CELL_SIGNIFICANCE_Truth:
        m_calculateSignificance = true;
        break;
      case xAOD::CaloCluster::ISOLATION_Truth:
        m_calculateIsolation = true;
        break;
      case xAOD::CaloCluster::ENG_BAD_HV_CELLS_Truth:
...
StatusCode CaloClusterMomentsMaker_Truth::execute(xAOD::CaloClusterContainer *theClusColl)
{
  ...
  if ( m_calculateIsolation ) {
    ...
    for (xAOD::CaloCluster* theCluster : *theClusColl) {
      ...
      for(; cellIter != cellIterEnd; cellIter++ ){
        ...
        cellPJ.SetPtEtaPhiM(myCell->pt(), myCell->eta(), myCell->phi(), myCell->m());
  ...
  for( ;clusIter!=clusIterEnd;clusIter++,iClus++) {
    ...
    if ( m_momentsNames.size() > 0 ) {
      ...
      for(; cellIter != cellIterEnd; cellIter++ ){
        ...
        if(hashid < 0 || hashid >= (signalCells)->size()) continue;
        pCell = (*signalCells)[hashid];
        if(!pCell) continue;
        pCell = *cellIter;

        TLorentzVector cellPJ(0,0,0,0);
        cellPJ.SetPtEtaPhiM(pCell->pt(), pCell->eta(), pCell->phi(), pCell->m());
        clusterPJ = clusterPJ + cellPJ;
	...
	for(i=0;i<ncell;i++) {
	  ...
	  for(size_t iMoment = 0, size = m_validMoments.size();
	    ...
	    case xAOD::CaloCluster::FIRST_PHI_Truth:
	      ...
	      break;
	    case xAOD::CaloCluster::SECOND_R_Truth:
	      myMoments[iMoment] += ci.energy*ci.r*ci.r;
	      break;
	    case xAOD::CaloCluster::SECOND_LAMBDA_Truth:
	      myMoments[iMoment] += ci.energy*ci.lambda*ci.lambda;
	      break;
	    case xAOD::CaloCluster::LATERAL_Truth:
		...
		myNorms[iMoment] += rm*rm*ci.energy;
	      ...
	      break;
	    case xAOD::CaloCluster::LONGITUDINAL_Truth:
	      if ( (int)i != iCellMax && (int)i != iCellScndMax ) {
		myMoments[iMoment] += ci.energy*ci.lambda*ci.lambda;
		...
		myMoments[iMoment] += ci.energy;
	      break;
	    case xAOD::CaloCluster::ENG_FRAC_MAX_Truth:
	      if ( (int)i == iCellMax ) 
		myMoments[iMoment] = ci.energy;
	      break;
	    default:
	...
	// assign moments which don't need the loop over the cells
	  ...
	  // now calculate the actual moments
          switch (m_validMoments[iMoment]) {
	  case xAOD::CaloCluster::FIRST_ETA_Truth:
	  case xAOD::CaloCluster::FIRST_PHI_Truth:
	  case xAOD::CaloCluster::SECOND_R_Truth:
	  case xAOD::CaloCluster::SECOND_LAMBDA_Truth:
	  case xAOD::CaloCluster::ENG_FRAC_EM_Truth:
	  case xAOD::CaloCluster::ENG_FRAC_MAX_Truth:
	    ...
	    break;
	  case xAOD::CaloCluster::DELTA_PHI_Truth:
	    myMoments[iMoment] = deltaPhi;
	    break;
	  case xAOD::CaloCluster::DELTA_THETA_Truth:
	    myMoments[iMoment] = deltaTheta;
	    break;
	  case xAOD::CaloCluster::DELTA_ALPHA_Truth:
	    myMoments[iMoment] = angle;
	    break;
	  case xAOD::CaloCluster::CENTER_X_Truth:
	    myMoments[iMoment] = showerCenter.x();
	    break;
	  case xAOD::CaloCluster::CENTER_Y_Truth:
	    myMoments[iMoment] = showerCenter.y();
	    break;
	  case xAOD::CaloCluster::CENTER_Z_Truth:
	    myMoments[iMoment] = showerCenter.z();
	    break;
	  case xAOD::CaloCluster::CENTER_MAG_Truth:
	    myMoments[iMoment] = showerCenter.mag();
	    break;
	  case xAOD::CaloCluster::CENTER_LAMBDA_Truth:
	    ...
	    break;
	  case xAOD::CaloCluster::ENG_FRAC_CORE_Truth:
	  ...
	  case xAOD::CaloCluster::ISOLATION_Truth:
	    ...
	    break;
	  case xAOD::CaloCluster::ENG_BAD_CELLS_Truth:
	    myMoments[iMoment] = eBad;
	    break;
	  case xAOD::CaloCluster::N_BAD_CELLS_Truth:
	    myMoments[iMoment] = nbad;
            break;
          case xAOD::CaloCluster::N_BAD_CELLS_CORR_Truth:
            myMoments[iMoment] = nbad_dac;
            break;
	  case xAOD::CaloCluster::BAD_CELLS_CORR_E_Truth:
	    myMoments[iMoment] = ebad_dac;
            break;
	  case xAOD::CaloCluster::BADLARQ_FRAC_Truth:
	    myMoments[iMoment] = eBadLArQ/(theCluster->e()!=0.?theCluster->e():1.);
            break;
	  case xAOD::CaloCluster::ENG_POS_Truth:
	  ...
	  case xAOD::CaloCluster::CELL_SIGNIFICANCE_Truth:
	    myMoments[iMoment] = maxAbsSig;
            break;
	  case xAOD::CaloCluster::CELL_SIG_SAMPLING_Truth:
	    myMoments[iMoment] = nSigSampl;
            break;
	  case xAOD::CaloCluster::AVG_LAR_Q_Truth:
	    myMoments[iMoment] = eLAr2Q/(eLAr2>0?eLAr2:1);
            break;
	  case xAOD::CaloCluster::AVG_TILE_Q_Truth:
	    myMoments[iMoment] = eTile2Q/(eTile2>0?eTile2:1);
            break;
	  case xAOD::CaloCluster::ENG_BAD_HV_CELLS_Truth:
	    myMoments[iMoment] = eBadLArHV;
	    break;
	  case xAOD::CaloCluster::N_BAD_HV_CELLS_Truth:
	    myMoments[iMoment] = nBadLArHV;
            break;
	  case xAOD::CaloCluster::ENERGY_Truth:
	    myMoments[iMoment] = clusterPJ.E();
            break;
	  case xAOD::CaloCluster::ETA_Truth:
	    myMoments[iMoment] = clusterPJ.Eta();
            break;
	  case xAOD::CaloCluster::PHI_Truth:
	    myMoments[iMoment] = clusterPJ.Phi();
            break;
	  default:
	    // nothing to be done for other moments
	    break;
	...
	if ( moment == xAOD::CaloCluster::FIRST_PHI_Truth ) 
