...
CaloSuperCellBuilderDriver::CaloSuperCellBuilderDriver (const std::string& name,
	...
	m_counter(0)
{
	...
	declareProperty("AddTileTriggerTowers",         m_addTileTriggerTowers=false);
	declareProperty("DefaultShiftTimeSample",       m_defaultShiftTimeSamples=0);
	declareProperty("BCID_Iter",                    m_bcs=2);
}

CaloSuperCellBuilderDriver::~CaloSuperCellBuilderDriver()
{ }

StatusCode CaloSuperCellBuilderDriver::initialize()
{
	...
	if (m_addTileTriggerTowers){
		if ( m_ttToSCTool.retrieve().isFailure() )
			{
				ATH_MSG_ERROR( "Unable to find TTToSCTool "  );
				return StatusCode::FAILURE; 
			}else{
			ATH_MSG_INFO( "Successfully retrieved TTToSCTool " );
		}
		ATH_CHECK( m_ttToSCTool->initialize() );
	}
	return StatusCode::SUCCESS;
}

void CaloSuperCellBuilderDriver::initEventTools(){
...
StatusCode CaloSuperCellBuilderDriver::execute() {
	...
	if ( m_addTileTriggerTowers )
		ATH_CHECK( m_ttToSCTool->buildTileSCs(caloCell));
	
	return StatusCode::SUCCESS;
}

bool CaloSuperCellBuilderDriver::buildLArCell(const LArDigit* digit,
                                             int&  energy ,
                                             int&  time ,
                                             CaloGain::CaloGain&  gain ,
                                             int& prov,
...
void CaloSuperCellBuilderDriver::finishEventTools(){
...
float CaloSuperCellBuilderDriver::pedestal(MsgStream* pLog)
{
...
void CaloSuperCellBuilderDriver::ADC2energy(MsgStream* pLog)
{
...
StatusCode CaloSuperCellBuilderDriver::finalize()
{
