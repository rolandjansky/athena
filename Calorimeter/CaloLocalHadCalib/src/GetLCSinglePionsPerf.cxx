...
{
  //std::cout << "CheckSinglePionsReco::execute() -> " << std::endl;
  MsgStream log(messageService(), name());
  StatusCode sc;
...
int GetLCSinglePionsPerf::fill_reco()
{
  ...
  ******************************************** */
  ...
  for(std::vector<std::string >::iterator it=m_clusterCollNames.begin(); it!=m_clusterCollNames.end(); it++){
    //std::cout << "CheckSinglePionsReco::execute() -> retrieving '" << (*it) << "'" << std::endl;
  ...
  for( ;clusIter!=clusIterEnd;clusIter++,iClus++) {
    ...
    if (!theCluster->retrieveMoment(xAOD::CaloCluster::FIRST_ENG_DENS, mx_dens))
      mx_dens = 0;
    bool clusIsGood(true);
    if(m_useGoodClus){
      if(theCluster->size() <= 1 || mx_dens <=0 || mx_center_lambda ==0) {
        clusIsGood = false;
      }
    }
...
//         + engCalibTotSmp[CaloSampling::TileGap3];
  ...
  ******************************************** */
  ...
  for( ;clusIter!=clusIterEnd;clusIter++,iClus++) {
    ...
    //std::cout << iClus << " " << theCluster->e() << std::endl;
    int iMoment=0;
