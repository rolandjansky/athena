...
void LArCellMonTool::sporadicNoiseCandidate(const CaloCell* cell, const LArCellMonTool::LayerEnum iLyr, const float threshold) {
  ...
  msg(MSG::INFO) << "Found sporadic noise candidate cell with id 0x" << std::hex << id.get_identifier32().get_compact() << std::dec << " in " << m_partitionNames[part] << endmsg;
...
StatusCode LArCellMonTool::procHistograms() {
  ...
  for (thresholdHist_t& thr : m_thresholdHists) {
    msg(MSG::INFO) << "processHist for threshold '" << thr.m_threshName << "', nb of events passed Trigger and Background removal: " << thr.m_eventsPassed << endmsg;
	...
	if (thr.m_h_averageEnergy_etaphi[iLyr]) {
	  ...
	  //thr.m_h_averageEnergy_etaphi[iLyr]->Divide(thr.m_h_totalEnergy_etaphi[iLyr],thr.m_h_occupancy_etaphi[iLyr]);
	...
	if (thr.m_h_averageTime_etaphi[iLyr]) {
	  ...
	  assert(thr.m_h_occupancy_etaphi[iLyr]);
	  //thr.m_h_averageTime_etaphi[iLyr]->Divide(thr.m_h_totalTime_etaphi[iLyr],thr.m_h_occupancy_etaphi[iLyr]);
	...
	if (thr.m_doEtaPhiFractionPastTth) {
	  ...
	  //thr.m_h_fractionPastTth_etaphi[iLyr]->Divide(thr.m_h_poorTimeOccupancy_etaphi[iLyr],thr.m_h_occupancy_etaphi[iLyr]);
	...
	if (thr.m_h_averageQuality_etaphi[iLyr]) {
	  ...
	  assert(thr.m_h_occupancy_etaphi[iLyr]);
	  //thr.m_h_averageQuality_etaphi[iLyr]->Divide(thr.m_h_totalQuality_etaphi[iLyr],thr.m_h_occupancy_etaphi[iLyr]);
	...
	if (thr.m_doEtaPhiFractionOverQth) {
	  ...
	  //thr.m_h_fractionOverQth_etaphi[iLyr]->Divide(thr.m_h_poorQualityOccupancy_etaphi[iLyr],thr.m_h_occupancy_etaphi[iLyr]);
...
StatusCode LArCellMonTool::createPerJobHistograms(const CaloCellContainer* cellCont) {
  ...
  for (const auto& thr :  m_thresholdHists) {  
	...
	//if (m_doSaveTempHists) {
	//  ATH_CHECK(mongroup.regHist(m_h_nActiveCellsFirstEvent_eta[iLyr]));
	//}
	//else {
	//  m_h_nActiveCellsFirstEvent_eta[iLyr]->SetDirectory(0); //We manage this histogram ourselves
	//}
      }//end loop over layers
  ...
  for (const auto& thr :  m_thresholdHists) {  
	...
	//if (m_doSaveTempHists) {
	//  ATH_CHECK(mongroup.regHist(m_h_nActiveCellsFirstEvent_phi[iLyr]));
	//}
	//else {
	//	  m_h_nActiveCellsFirstEvent_phi[iLyr]->SetDirectory(0); //We manage this histogram ourselves
	//}
      }//end loop over layers
      break; //break loop over thresholds, one such histo is enough
