...
void ref() {
	...
	TFile * mon = new TFile("reference.root", "RECREATE");
  ...
  /*
	...
	*/
	...
	TH1D * wrongBin = new TH1D("bad binning", "bad binning", 51, 0, 100);
	//wrongBin->Write();

	TH1D * wrongClass = new TH1D("diff class", "diff class", 100, 0, 100);
	//wrongClass->Write();
