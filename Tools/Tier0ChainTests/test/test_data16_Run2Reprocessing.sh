...
if [ ${rc1} -eq 0 ]
then
  art.py compare ref . /cvmfs/atlas-nightlies.cern.ch/repo/data/data-art/Tier0ChainTests/TCT_Run2Reprocessing_references_for_comparison/test_data16_2022-04-12T2101 --entries 30 --mode=semi-detailed --order-trees --ignore-exit-code diff-pool --ignore-leave '(.*)HLTResult_p1_HLTResult_HLT(.*)' --ignore-leave 'TrigNavigationAux(.*)'
  rc3=$?
fi
