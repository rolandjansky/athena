...
{
  auto cont = std::make_unique<Muon::CscPrepDataContainer>(5);

  for (int hash=2; hash <= 3; hash++) {
    auto coll = std::make_unique<Muon::CscPrepDataCollection>(IdentifierHash(hash));
    ...
    for (int i=0; i < 10; i++) {
      ...
      auto cl = std::make_unique<Muon::CscPrepData>
        (clusId,
         clusHash,
         locpos,
         rdoList,
