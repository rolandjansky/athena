...
{
  auto cont = std::make_unique<Muon::CscStripPrepDataContainer>(5);

  for (int hash=2; hash <= 3; hash++) {
    auto coll = std::make_unique<Muon::CscStripPrepDataCollection>(IdentifierHash(hash));
  ...
  auto cl = std::make_unique<Muon::CscStripPrepData>
        (clusId,
         clusHash,
         locpos,
         new Amg::MatrixX(cov),
