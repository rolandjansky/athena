...
// $Id$
/**
 * @file MuonEventTPCnv/test/CscStripPrepDataContainerCnv_p1_test.cxx
...
void compare (const Muon::CscStripPrepDataContainer& p1,
              const Muon::CscStripPrepDataContainer& p2)
{
  Muon::CscStripPrepDataContainer::const_iterator it1 = p1.begin();
  Muon::CscStripPrepDataContainer::const_iterator it1e = p1.end();
  Muon::CscStripPrepDataContainer::const_iterator it2 = p2.begin();
...
void testit (const Muon::CscStripPrepDataContainer& trans1)
{
  ...
  std::unique_ptr<Muon::CscStripPrepDataContainer> trans2
...
std::unique_ptr<const Muon::CscStripPrepDataContainer>
makeclusts (const MuonGM::MuonDetectorManager& muo_dd)
{
  auto cont = CxxUtils::make_unique<Muon::CscStripPrepDataContainer>(5);

  for (int hash=2; hash <= 3; hash++) {
    auto coll = CxxUtils::make_unique<Muon::CscStripPrepDataCollection>(IdentifierHash(hash));
    ...
    for (int i=0; i < 10; i++) {
      ...
      std::vector<float> sampleCharges {
        static_cast<float>(5.5+offs),
        static_cast<float>(6.5+offs),
        static_cast<float>(7.5+offs),
        static_cast<float>(8.5+offs) };

  auto cl = CxxUtils::make_unique<Muon::CscStripPrepData>
        (clusId,
         clusHash,
         locpos,
         new Amg::MatrixX(cov),
  ...
  return std::unique_ptr<const Muon::CscStripPrepDataContainer> (cont.release());
}
...
void test1 (const MuonGM::MuonDetectorManager& muo_dd)
{
  ...
  {
    std::unique_ptr<const Muon::CscStripPrepDataContainer> cont = makeclusts (muo_dd);
    testit (*cont);
  }
