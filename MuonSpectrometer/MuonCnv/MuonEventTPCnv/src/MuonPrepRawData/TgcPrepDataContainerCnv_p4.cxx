...
StatusCode Muon::TgcPrepDataContainerCnv_p4::initialize(MsgStream &log) {
...
void Muon::TgcPrepDataContainerCnv_p4::transToPers(const Muon::TgcPrepDataContainer* transCont,  Muon::TgcPrepDataContainer_p4* persCont, MsgStream &log) 
{

  if(log.level() <= MSG::DEBUG && !m_isInitialized) {
      if (this->initialize(log) != StatusCode::SUCCESS) {
          log << MSG::FATAL << "Could not initialize TgcPrepDataContainerCnv_p4 " << endmsg;
      } 
  }
...
//        std::cout<<"Coll Index: "<<pcollIndex<<"\tCollId: "<<collection.identify().get_compact()<<"\tCollHash: "<<collection.identifyHash()<<"\tpCollId: "<<pcollection.m_id<<"\tpCollHash: "<<std::endl;
        ...
        for (unsigned int i = 0; i < collection.size(); ++i) {
            ...
            TgcPrepData_p4*   pchan = &(persCont->m_prds[pchanIndex]); // persistent version to fill
            chanCnv.transToPers(chan, pchan, log); // convert from TgcPrepData to TgcPrepData_p4
...
void  Muon::TgcPrepDataContainerCnv_p4::persToTrans(const Muon::TgcPrepDataContainer_p4* persCont, Muon::TgcPrepDataContainer* transCont, MsgStream &log) 
{
...
//        std::cout<<"Coll Index: "<<pcollIndex<<"\tCollId: "<<collection.identify().get_compact()<<"\tCollHash: "<<collection.identifyHash()<<"\tpCollId: "<<pcollection.m_id<<"\tpCollHash: "<<std::endl;
        ...
        for (; pchanIndex < pchanEnd; ++ pchanIndex, ++chanIndex) {
            const TgcPrepData_p4* pchan = &(persCont->m_prds[pchanIndex]);
            ...
            if (result&&log.level() <= MSG::WARNING) 
              log << MSG::WARNING<< " Muon::TgcPrepDataContainerCnv_p4::persToTrans: problem converting Identifier to DE hash "<<endmsg;
...
//================================================================
Muon::TgcPrepDataContainer* Muon::TgcPrepDataContainerCnv_p4::createTransient(const Muon::TgcPrepDataContainer_p4* persObj, MsgStream& log) 
{
    if(!m_isInitialized) {
        if (this->initialize(log) != StatusCode::SUCCESS) {
            log << MSG::FATAL << "Could not initialize TgcPrepDataContainerCnv_p4 " << endmsg;
