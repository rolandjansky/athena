...
void Muon::TgcPrepDataContainerCnv_p1::transToPers(const Muon::TgcPrepDataContainer* transCont,  Muon::MuonPRD_Container_p1* persCont, MsgStream &log) 
{
    ...
    if (log.level() <= MSG::DEBUG) log << MSG::DEBUG  << " ***  Writing TgcPrepDataContainer ***" << endmsg;
}
...
// No hash based lookup for Tgcs?
        ...
        for (unsigned int ichan = 0; ichan < nchans; ++ ichan) {
            ...
            Muon::TgcPrepData* chan = dynamic_cast<Muon::TgcPrepData*>(createTransFromPStore((CONV**)nullptr, pchan, log ) );
            if(chan) {
              chan->m_detEl = de;
              (*coll)[ichan] = chan;
            }
    ...
    if (log.level() <= MSG::DEBUG) log << MSG::DEBUG  << " ***  Reading TgcPrepDataContainer" << endmsg;
}
...
Muon::TgcPrepDataContainer* Muon::TgcPrepDataContainerCnv_p1::createTransient(const Muon::MuonPRD_Container_p1* persObj, MsgStream& log) 
{
    if(!m_isInitialized) {
        if (this->initialize(log) != StatusCode::SUCCESS) {
            log << MSG::FATAL << "Could not initialize TgcPrepDataContainerCnv_p1 " << endmsg;
            return nullptr;
        } 
    }
