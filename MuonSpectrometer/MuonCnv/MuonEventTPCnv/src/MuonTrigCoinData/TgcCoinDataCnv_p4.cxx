...
void TgcCoinDataCnv_p4::
persToTrans( const Muon::TgcCoinData_p4 *persObj, Muon::TgcCoinData *transObj,MsgStream &log ) 
{
   log << MSG::DEBUG << "TgcCoinDataCnv_p4::persToTrans" << endmsg;
   ...
   if (dummy.values.size())
     EigenHelpers::vectorToEigenMatrix(dummy.values, *tempMat, "TgcCoinDataCnv_p4");
...
void TgcCoinDataCnv_p4::
transToPers( const Muon::TgcCoinData *transObj, Muon::TgcCoinData_p4 *persObj, MsgStream &log )
{
  log << MSG::DEBUG << "TgcCoinDataCnv_p4::transToPers" << endmsg;
  ...
  if((transObj->type()==Muon::TgcCoinData::TYPE_TRACKLET || 
    ...
    if(!transObj->channelIdIn().get_identifier32().is_valid()) {
      log << MSG::ERROR << "TgcCoinDataCnv_p4::transToPers - invalid trans channelIdIn!! " 
  ...
  if((transObj->type()==Muon::TgcCoinData::TYPE_TRACKLET ||
    ...
    if(!transObj->channelIdOut().get_identifier32().is_valid()) {
      log << MSG::ERROR << "TgcCoinDataCnv_p4::transToPers - invalid trans channelIdOut!! " 
          << (transObj->type()==Muon::TgcCoinData::TYPE_TRACKLET ? "(TRACKLET)" : 
  ...
  if (transObj->hasErrMat() )
  {
    if (! transObj->errMat().cols() || !transObj->errMat().rows()) {
      ...
      EigenHelpers::eigenMatrixToVector(pMat.values, transObj->errMat(), "TgcCoinDataCnv_p4");
