...
{
   log << MSG::DEBUG << "TgcCoinDataCnv_p2::persToTrans" << endreq;
...
{
  log << MSG::DEBUG << "TgcCoinDataCnv_p2::transToPers" << endreq;
   ...
   if (transObj->m_errMat ){
     if (! transObj->m_errMat->cols() || !transObj->m_errMat->rows()) {
       ...
       EigenHelpers::eigenMatrixToVector(pMat.values, *transObj->m_errMat, "TgcCoinDataCnv_p2");
