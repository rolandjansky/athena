...
uint32_t MdtAmtReadOut::makeTSM(uint16_t jt, uint16_t channel, bool leading, bool errflag, uint16_t coarse, uint16_t fine) {
    uint16_t lead = 1;
    uint16_t err = 0;
    if (leading) lead = 0;
    if (errflag) err = 1;
...
uint32_t MdtAmtReadOut::makeTCM(uint16_t jt, uint16_t channel, uint16_t width, uint16_t coarse, uint16_t fine) {
