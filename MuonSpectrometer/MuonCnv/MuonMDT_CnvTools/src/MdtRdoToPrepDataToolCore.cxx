...
StatusCode Muon::MdtRdoToPrepDataToolCore::decode(const std::vector<IdentifierHash>& chamberHashInRobs) const {
    ...
    if (!mdtPrepDataContainer) { return StatusCode::FAILURE; }
...
void Muon::MdtRdoToPrepDataToolCore::processPRDHashes(Muon::MdtPrepDataContainer* mdtPrepDataContainer,
    ...
    if (!rdoContainer || rdoContainer->size() == 0) { return; }

    for (auto it = chamberHashInRobs.begin(); it != chamberHashInRobs.end(); ++it) {
        if (!handlePRDHash(mdtPrepDataContainer, *it, *rdoContainer, idWithDataVect)) { ATH_MSG_DEBUG("Failed to process hash " << *it); }
...
bool Muon::MdtRdoToPrepDataToolCore::handlePRDHash(Muon::MdtPrepDataContainer* mdtPrepDataContainer, IdentifierHash hash,
                                                   const MdtCsmContainer& rdoContainer, std::vector<IdentifierHash>& idWithDataVect) const {
        ...
        if (m_idHelperSvc->mdtIdHelper().stationName(elementId) == m_BMEid) {
            ...
            if (rdoColl != nullptr && rdoColl2 != nullptr) {
                ...
                if (processCsm(mdtPrepDataContainer, rdoColl, idWithDataVect, muDetMgr, rdoColl2).isFailure()) {
                    ATH_MSG_WARNING("processCsm failed for RDO id " << (unsigned long long)(rdoColl->identify().get_compact()) << " and "
                ...
                ATH_MSG_DEBUG("handlePRDHash: hash id " << hash << " not found in RDO container, and is BME - Missing " << rdoHash << " "
                                                        << rdoHash2);
            }
        }  // End of m_BMEpresent
        else {
...
StatusCode Muon::MdtRdoToPrepDataToolCore::decode(std::vector<IdentifierHash>& idVect, std::vector<IdentifierHash>& idWithDataVect) const {
    ...
    // setup output container
    bool fullEventDone = false;
    Muon::MdtPrepDataContainer* mdtPrepDataContainer = setupMdtPrepDataContainer(sizeVectorRequested, fullEventDone);
    if (!mdtPrepDataContainer) { return StatusCode::FAILURE; }
...
void Muon::MdtRdoToPrepDataToolCore::printInputRdo() const {
    ...
    for (; it != rdoContainer->end(); ++it) {
        ...
        ATH_MSG_DEBUG("**** MdtCsm with online Id: subdetector: " << MSG::hex << subdet << MSG::dec << "  mrod: " << MSG::hex << mrodid
        ...
        for (; it_amt != mdtColl->end(); ++it_amt) {
            ...
            ATH_MSG_DEBUG(">> AmtHit in tdc: " << MSG::hex << tdcId << MSG::dec << "  channel: " << MSG::hex << channelId << MSG::dec
                                               << "  fine time: " << fine << "  coarse time: " << coarse << "  width: " << width);
        }
    }
...
StatusCode Muon::MdtRdoToPrepDataToolCore::processCsm(Muon::MdtPrepDataContainer* mdtPrepDataContainer, const MdtCsm* rdoColl,
                                                      std::vector<IdentifierHash>& idWithDataVect,
                                                      const MuonGM::MuonDetectorManager* muDetMgr, const MdtCsm* rdoColl2) const {
    // first handle the case of twin tubes
    if (m_useTwin) {
        // two chambers in ATLAS are installed with Twin Tubes; in detector coordinates BOL4A13 & BOL4C13; only INNER multilayer(=1) is with
        // twin tubes implement twin tube writing to prepData either for all BOL (m_useAllBOLTwin = true) _OR_ only for two chambers really
        // installed
        Identifier elementId = rdoColl->identify();
        MuonStationIndex::ChIndex chIndex = m_idHelperSvc->chamberIndex(elementId);
        if (chIndex == MuonStationIndex::BOL && (m_useAllBOLTwin || (std::abs(m_idHelperSvc->mdtIdHelper().stationEta(elementId)) == 4 &&
                                                                     m_idHelperSvc->mdtIdHelper().stationPhi(elementId) == 7))) {
    ...
    if (rdoColl2) {
        ...
        ATH_MSG_DEBUG("Special case with 2 RDO going into 1 PRD - ID1 " << elementId1 << " ID2 " << elementId2 << " hash 1 " << hash1
                                                                        << " hash 2 " << hash2);
    ...
    ATH_MSG_VERBOSE("Identifier = " << m_idHelperSvc->toString(elementId) << " subdetId/ mrodId/ csmId = " << subdetId << " / " << mrodId
                                    << " / " << csmId);
    ...
    for (; itD != itD_e;) {
        ...
        if (m_idHelperSvc->mdtIdHelper().stationName(parentId) == m_BMGid && m_BMGpresent) {
            std::map<Identifier, std::vector<Identifier> >::const_iterator myIt =
                m_DeadChannels.find(muDetMgr->getMdtReadoutElement(channelId)->identify());
            if (myIt != m_DeadChannels.end()) {
                if (std::find((myIt->second).begin(), (myIt->second).end(), channelId) != (myIt->second).end()) {
                    ATH_MSG_DEBUG("processCsm : Deleting BMG digit with identifier"
                                  << m_idHelperSvc->mdtIdHelper().show_to_string(channelId));
                    delete newDigit;
                    ++itD;
                    continue;
                }
        ...
        if (parentId != elementId) {
            ...
            if (stationName == m_idHelperSvc->mdtIdHelper().stationName(elementId)) {
                ...
                if (!doubleCsmFound) {
                    ...
                    ATH_MSG_DEBUG("First station: " << m_idHelperSvc->mdtIdHelper().stationNameString(stationName)
                                                    << " eta: " << m_idHelperSvc->mdtIdHelper().stationEta(elementId)
                                                    << " phi: " << m_idHelperSvc->mdtIdHelper().stationPhi(elementId));

                    ATH_MSG_DEBUG("Second station: " << m_idHelperSvc->mdtIdHelper().stationNameString(stationName)
                                                     << " eta: " << m_idHelperSvc->mdtIdHelper().stationEta(secondElementId)
                                                     << " phi: " << m_idHelperSvc->mdtIdHelper().stationPhi(secondElementId));
...
StatusCode Muon::MdtRdoToPrepDataToolCore::processCsmTwin(Muon::MdtPrepDataContainer* mdtPrepDataContainer, const MdtCsm* rdoColl,
                                                          std::vector<IdentifierHash>& idWithDataVect,
            ...
            if (!m_discardSecondaryHitTwin) {
                if (mdtDigitColl[m_secondaryHit_twin_chamber[multilayer - 1][layer - 1][twinPair - 1]].first == 0) {
                    ...
                    ATH_MSG_VERBOSE(" TWIN TUBES: found a tertiary hit in a twin tube in one RdoCollection for "
                                    ...
                                    << " tube = " << m_idHelperSvc->mdtIdHelper().tube(channelId) << " with adc  = " << newDigit->adc()
                                    << "  tdc = " << newDigit->tdc());
                }
    ...
    for (const std::pair<const int, std::pair<MdtDigit*, MdtDigit*> >& digitPair : mdtDigitColl) {
        ...
        if (multilayer == 1) {
            ...
            if (!second_digit) {
                ...
                ATH_MSG_DEBUG(" MADE ORIGINAL PREPDATA "
                              << m_idHelperSvc->mdtIdHelper().stationNameString(m_idHelperSvc->mdtIdHelper().stationName(channelId))
                              << "  eta = " << m_idHelperSvc->mdtIdHelper().stationEta(channelId)
                              << "  phi = " << m_idHelperSvc->mdtIdHelper().stationPhi(channelId)
                              << "  ml = " << m_idHelperSvc->mdtIdHelper().multilayer(channelId)
                              << "  layer = " << m_idHelperSvc->mdtIdHelper().tubeLayer(channelId) << "  tube = "
                              << m_idHelperSvc->mdtIdHelper().tube(channelId) << "  radius = " << radius << " +- " << errRadius);
                ...
                if (!m_use1DPrepDataTwin) {
                    ...
                    ATH_MSG_DEBUG(
                        ...
                        << "  layer = " << m_idHelperSvc->mdtIdHelper().tubeLayer(promptHit_channelId)
                        << " tube = " << m_idHelperSvc->mdtIdHelper().tube(promptHit_channelId) << "  zTwin = " << zTwin << " +- "
                    ...
                    ATH_MSG_DEBUG(" global pos center tube  x = " << gpos_centertube.x() << " y = " << gpos_centertube.y()
                                                                  << " z = " << gpos_centertube.z());
                    ATH_MSG_DEBUG(" local pos center tube w/ TWIN INFO  x = " << locpos_centertube.x() << " y = " << locpos_centertube.y()
                                                                              << " z = " << locpos_centertube.z());
            ...
            if (!second_digit) {
                ...
                ATH_MSG_DEBUG(" MADE ORIGINAL PREPDATA FOR SECOND DIGIT "
                              << m_idHelperSvc->mdtIdHelper().stationNameString(m_idHelperSvc->mdtIdHelper().stationName(second_channelId))
                              ...
                              << "  tube = " << m_idHelperSvc->mdtIdHelper().tube(second_channelId) << "  radius = " << second_radius
                              << " +- " << second_errRadius);
