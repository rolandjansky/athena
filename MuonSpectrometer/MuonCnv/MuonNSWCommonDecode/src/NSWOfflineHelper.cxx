...
uint16_t Muon::nsw::helper::NSWOfflineHelper::channel_number ()
{
  ...
  if (det_id == eformat::MUON_MMEGA_ENDCAP_A_SIDE || det_id == eformat::MUON_MMEGA_ENDCAP_C_SIDE)
  {
    ...
    chan_number = ((position < 10 ? position : position - 10) * Muon::nsw::VMM_per_MMFE8 + outw_vmm)
                * Muon::nsw::VMM_channels + outw_chan;
  }
  else if (det_id == eformat::MUON_STGC_ENDCAP_A_SIDE || det_id == eformat::MUON_STGC_ENDCAP_C_SIDE)
  {
    ...
    uint8_t quad_layer = m_elinkId->layer () % 4;
    ...
    switch (this->channel_type ())
    {
      case Muon::nsw::OFFLINE_CHANNEL_TYPE_STRIP:
        chan_number = m.channel_number (is_large, position, layer_type, m_vmm, m_chan);
        break;
