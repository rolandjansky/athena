...
public:
  ...
  virtual void getDigit(const CscRawData * rawData, Identifier& moduleId,
                Identifier& channelId, double& adc, double& time) =0;
  virtual Identifier stationIdentifier(const CscRawData* rawData) =0;
  virtual Identifier channelIdentifier(const CscRawData * rawData, int j) =0;
};
