...
StatusCode Muon::RpcROD_Decoder::fillCollectionsFromRob_v302(BS data, const uint32_t data_size, std::map<Identifier, RpcPad*>& vmap,
    ...
    if (size > 0 && msgLvl(MSG::VERBOSE)) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i > 1 && i == iSLh + 2 && (decoded != 0xf)) { sprintf(decoded_char, "SL hit ???"); }

            ATH_MSG_VERBOSE("word " << i << " = " << MSG::hex << p[i] << MSG::dec << " " << MSG::hex << decoded << MSG::dec << " "
                                    << decoded_char);
        }
    }
    int nwInCM = 0;
    ...
    if (msgLvl(MSG::VERBOSE)) {
        for (std::map<Identifier, RpcPad*>::const_iterator it = vmap.begin(); it != vmap.end(); ++it) {
            ...
            ATH_MSG_VERBOSE("The offline ID requested for conversion are " << m_idHelperSvc->rpcIdHelper().show_to_string(myPadOfflineId)
                                                                           << " hash version " << (int)myPadHashOfflineId);
        }
    ...
    if (msgLvl(MSG::DEBUG)) {
        ATH_MSG_DEBUG("subDetectorID = 0x" << MSG::hex << subDetectorID << MSG::dec);

        ATH_MSG_DEBUG("rodID = 0x" << MSG::hex << rodId << MSG::dec);
    ...
    for (uint16_t i = 0; i < size; ++i) {
        ...
        if (!skipSectorLogicDecoding) { SLROS.decodeFragment(currentWord, recFieldSL); }
        ...
        if (msgLvl(MSG::VERBOSE)) {
            ...
            ATH_MSG_VERBOSE(i << " -->current data word is " << MSG::hex << currentWord << MSG::dec << decoded_char
                              << " RecFieldROS,PAD,SL  " << recFieldROS << " " << recFieldPAD << " " << recFieldSL);
        }
        ...
        if (isRXHeader) {
            ...
            sector = side * 32 + sectorForCabling;  // range 0-63    //side = 1 for side A or 0 for side C // side 1 corresponds to
                                                    // subDetID=65 (side 0 for subDetID=66)
            if (msgLvl(MSG::VERBOSE))
                ATH_MSG_VERBOSE("RXid, sectorForCabling, sector = " << rxid << " " << sectorForCabling << " " << sector);
            ...
            if ((rodId == 3 || rodId == 1) && (m_specialROBNumber > 0)) {
                ...
                if (msgLvl(MSG::VERBOSE))
                    ATH_MSG_VERBOSE("Here a change !!! RXid, sectorForCabling, sector = " << rxid << " " << sectorForCabling << " "
                                                                                          << sector);
            ...
            // Decode the sector logic footer
                ...
                if (msgLvl(MSG::VERBOSE)) {
                    ...
                    for (unsigned short j = 0; j < SLBodyWords; j++) {
                        ATH_MSG_VERBOSE(" SL data word " << j << " : " << MSG::hex << SLBuff[j] << MSG::dec);
                    }
                }
                            ...
                            if (!outputHeaderFound) {
                                ...
                                if (SLROS.isOutputDecoded()) {
                                    ...
                                    for (int icand = 0; icand < SLROS.nTriggerCand(); ++icand) {
                                        ...
                                        RpcSLTriggerHit* slHit =
                                            new RpcSLTriggerHit(rowinBcid, 0, ptid, roi, outerPlane, overlapPhi, overlapEta, triggerBcid);
            ...
            if (recField == 'H') {
                ...
                if (msgLvl(MSG::VERBOSE))
                    ATH_MSG_VERBOSE("Decoding Pad Header - side,sectorLogic= " << side << " " << sectorLogic << "  PadID/Sector " << PadID
                                                                               << "/" << sector
                ...
                if (!rpcCabling->giveOfflineId(side, sectorLogic, PadID, padOfflineId)) {
                    if (msgLvl(MSG::VERBOSE))
                        ATH_MSG_VERBOSE("Cannot retrieve the OfflineID for the PAD n. " << PadID << " at side " << side << " and  sector "
                                                                                        << sectorLogic);
                } else {
                    if (msgLvl(MSG::VERBOSE))
                        ATH_MSG_VERBOSE("PadOfflineId " << m_idHelperSvc->rpcIdHelper().show_to_string(padOfflineId)
                                                        << " associated to PAD n. " << PadID << " at side " << side << " and  sectorLogic "
                                                        << sectorLogic);
                }
                ...
                if (myPadInList != vmap.end()) {
                    ...
                    if (msgLvl(MSG::VERBOSE))
                        ATH_MSG_VERBOSE("match found with ID " << m_idHelperSvc->rpcIdHelper().show_to_string(thisPadOfflineId)
                                                               << " requested for the conversion; return this collection with hash "
                                                               << thisPadHashOfflineId);
                    ...
                    if (msgLvl(MSG::VERBOSE))
                        ATH_MSG_VERBOSE("match NOT found with ID " << m_idHelperSvc->rpcIdHelper().show_to_string(thisPadOfflineId)
            ...
            if (recField == 'S') {
                ...
                if (currentWord & 0x0fff) {
                    if (msgLvl(MSG::VERBOSE))
                        ATH_MSG_VERBOSE("Pad Busy status not zero ! value: " << MSG::hex << (currentWord & 0x0fff) << MSG::dec);
                }
            }

            if (recField == 'F') {
                if (msgLvl(MSG::VERBOSE)) { ATH_MSG_VERBOSE(" Pad Footer "); }
                if (PDROS.errorCode() != 0) {
                    ATH_MSG_VERBOSE("Pad Error flag not zero ! value: " << MSG::hex << PDROS.errorCode() << MSG::dec);
                }
                if (foundPad) {
                    ...
                    if ((unsigned int)nDecodedPads == vmap.size()) {
                        ...
                        if (!decodeSL) {
                            ...
                            ATH_MSG_DEBUG("... keep going to allow full SL decoding");
                    }
                }
            }
            ...
            if (foundCM == 1) {  // corresponds to CM footer found - all CM words stored in myRPC
                ...
                if (msgLvl(MSG::VERBOSE))
                    ATH_MSG_VERBOSE(" in PadID/Sector " << PadID << "/" << sector << " CM footer found: n. of words in CM including Footer "
                                                        << nwInCM << myRPC.CMFragment());
                ...
                if (foundPad) {
                    ...
                    if ((sectorForCabling == 21 || sectorForCabling == 22 || sectorForCabling == 25 || sectorForCabling == 26) &&
                        (PadID == 2 || PadID == 4 || PadID == 5 || PadID == 7) && cmaId >= 4)
                        cmaId = cmaId - 4;
                    ...
                    if (msgLvl(MSG::VERBOSE))
                        ATH_MSG_VERBOSE("Creating a new CM, cmaId=" << cmaId << " fel1id=" << fel1id << " febcid=" << febcid
                                                                    << " in PadID/Sector " << PadID << "/" << sector);
