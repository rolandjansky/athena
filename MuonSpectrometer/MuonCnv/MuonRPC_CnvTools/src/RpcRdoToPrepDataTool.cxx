...
namespace {
    ...
    template <class CONT, class COLL> struct ATLAS_NOT_THREAD_SAFE GetCollection {
        GetCollection(const RpcIdHelper& idHelper, CONT* container, MsgStream& msg) :
            m_idHelper(idHelper), m_container(container), m_msg(msg) {}

        COLL* operator()(Identifier id) { return Muon::IDC_Helper::getCollection<CONT, RpcIdHelper>(id, m_container, m_idHelper, m_msg); }
...
Muon::RpcRdoToPrepDataTool::RpcRdoToPrepDataTool(const std::string& type, const std::string& name, const IInterface* parent) :
...
StatusCode Muon::RpcRdoToPrepDataTool::manageOutputContainers(bool& firstTimeInTheEvent) const {
    ...
    if (!rpcPrepDataHandle.isPresent()) {
        ...
        StatusCode status =
            rpcPrepDataHandle.record(std::make_unique<Muon::RpcPrepDataContainer>(m_idHelperSvc->rpcIdHelper().module_hash_max()));
...
StatusCode Muon::RpcRdoToPrepDataTool::decode(std::vector<IdentifierHash>& idVect, std::vector<IdentifierHash>& selectedIdVect) const {
    ...
    getPrepCollection_func getPrepCollection =
        GetCollection<RpcPrepDataContainer, RpcPrepDataCollection>(m_idHelperSvc->rpcIdHelper(), m_state.m_rpcPrepDataContainer, msg());
    getCoinCollection_func getCoinCollection =
        GetCollection<RpcCoinDataContainer, RpcCoinDataCollection>(m_idHelperSvc->rpcIdHelper(), m_state.m_rpcCoinDataContainer, msg());

    ATH_MSG_DEBUG("Calling Core decode function from Legacy decode function (hash vector)");
    StatusCode status = decodeImpl(m_state, getPrepCollection, getCoinCollection, idVect, selectedIdVect, firstTimeInTheEvent);
    if (status.isFailure()) {
...
StatusCode Muon::RpcRdoToPrepDataTool::decode(const std::vector<uint32_t>& robIds) const {
    ...
    StatusCode status = decodeImpl(m_state, getPrepCollection, getCoinCollection, robIds, firstTimeInTheEvent);
    if (status.isFailure()) {
...
void Muon::RpcRdoToPrepDataTool::printPrepData() const {
    printPrepDataImpl(*m_state.m_rpcPrepDataContainer, *m_state.m_rpcCoinDataContainer);
}
