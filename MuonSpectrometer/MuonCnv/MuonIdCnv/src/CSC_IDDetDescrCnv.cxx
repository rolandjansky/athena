...
CSC_IDDetDescrCnv::initialize()
{
    ...
    MsgStream log(messageService(), "CSC_IDDetDescrCnv");
...
//      if (sc.isFailure()) {
//  	log << MSG::FATAL << "Unable to add proxy for CscIdHelper to the Detector Store!" << endreq;
...
CSC_IDDetDescrCnv::createObj(IOpaqueAddress* pAddr, DataObject*& pObj) 
{
    ...
    MsgStream log(messageService(), "CSC_IDDetDescrCnv");
    log << MSG::INFO << "in createObj: creating a CcscIdHelper object in the detector store" << endreq;
	...
	log << MSG::ERROR << "Unable to initialize CscIdHelper" << endreq;
	return StatusCode::FAILURE;
    } 
