...
void CSCSimHitCollectionCnv_p2::transToPers(const CSCSimHitCollection* transCont, Muon::CSCSimHitCollection_p2* persCont, MsgStream &log)
{
...
CSCSimHitCollection* CSCSimHitCollectionCnv_p2::createTransient(const Muon::CSCSimHitCollection_p2* persObj, MsgStream &log) {
...
void CSCSimHitCollectionCnv_p2::persToTrans(const Muon::CSCSimHitCollection_p2* persCont, CSCSimHitCollection* transCont, MsgStream &log) 
{
    HepMcParticleLinkCnv_p1 hepMcPLCnv;
  ...
  for (unsigned int i = 0; i < persCont->m_CSCid.size(); i++) {
      ...
      CSCSimHit transHit(persCont->m_CSCid[i], persCont->m_globalTime[i], persCont->m_energyDeposit[i], start, end, persCont->m_particleID[i], 0, persCont->m_kineticEnergy[i]);
