...
namespace MuonGM {
  ...
  inline std::pair<int,int> MuonPadDesign::channelNumber( const Amg::Vector2D& pos) const {
    ...
    if ( pos.y() > (0.5*Length-ylFrame )|| pos.y() < (-0.5*Length+ysFrame ) ){
       std::cout << "MPD:ChannelNumber: pos.y() out of bounds"<<std::endl;
    ...
    if ( pos.y() <= (0.5*Length-ylFrame-yCutout)){
      if ( fabs(pos.x()) > (0.5*sPadWidth+0.5*(lPadWidth-sPadWidth)*((pos.y()+(0.5*Length-ysFrame))/(Length-ysFrame-ylFrame))) ) {
         std::cout << "MPD:ChannelNumber: pos.x() out of bounds"<<std::endl;
         return std::pair<int,int>(-1,-1);
      }
    }
    ...
    if (y1>ysFrame+firstRowPos) {
       ...
       std::cout <<"MPD:ChannelNumber: Hit the ysFrame" <<std::endl;
       return std::pair<int,int>(-1,-1);
    }
    else if (y1<0) {
       std::cout <<"MPD:ChannelNumber: Error - negative distance to hit" <<std::endl;
       return std::pair<int,int>(-1,-1);
    }
    else {
       std::cout <<"MPD:ChannelNumber: Error - undefined distance to hit" <<std::endl;
       return std::pair<int,int>(-1,-1);
    }
    ...
    if (padEta==nPadH+1){
       ...
       std::cout <<"MPD:ChannelNumber: Error: padEta too high" <<std::endl;
       return std::pair<int,int>(-1,-1);
    }
...
//    double maxlocPhi2 = 180*atan( 0.5*lPadWidth/( radialDistance + (0.5*Length-ylFrame) ))/M_PI;
   if (abs(locPhi)>maxlocPhi) {
      std::cout <<"MPD:ChannelNumber: Error locPhi too large"<<std::endl;
      return std::pair<int,int>(-1,-1);
   }
   ...
   if (abs(fuzziedlocPhi)>maxlocPhi) {
      std::cout <<"close to outer border"<<std::endl;
   ...
   if (padPhi == 0) {
      padPhi=1;
      std::cout << "MPD:ChannelNumber: adjusted rightmost. padPhi="<<padPhi<<std::endl;
   }
   else if(padPhi == nPadColumns+1){
      padPhi=nPadColumns;
      std::cout << "MPD:ChannelNumber: adjusted leftmost. padPhi="<<padPhi<<std::endl;
   }
   else if ( padPhi < 0 || padPhi > nPadColumns+1 ){
      std::cout << "MPD:ChannelNumber: Error: padPhi out of bounds" <<std::endl;
      return std::pair<int,int>(-1,-1);
   }
   std::cout <<"MPD:ChannelNumber:padEta,padPhi: " <<padEta<<" , "<<padPhi<<std::endl;
