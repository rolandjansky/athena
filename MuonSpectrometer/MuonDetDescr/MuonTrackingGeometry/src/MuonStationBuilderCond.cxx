  ...
  if (muonMgr) { 
    ...
    while (!vol.atEnd())
    {
      ...
      if (vname.substr(0,3) =="NSW" || vname.substr(0,8) =="NewSmall") {
        ...
        std::pair<const Trk::Layer*,const std::vector<const Trk::Layer*>*> layerReprL = m_muonStationTypeBuilder->createLayerRepresentation(newTypeL);
        // create prototype as detached tracking volume
        std::unique_ptr<const Trk::DetachedTrackingVolume> typeL{newTypeL ? new Trk::DetachedTrackingVolume("NSWL",newTypeL,layerReprL.first,layerReprL.second) : nullptr};
        ...
        std::unique_ptr<const Trk::DetachedTrackingVolume> typeS{newTypeS ? new Trk::DetachedTrackingVolume("NSWS",newTypeS,layerReprS.first,layerReprS.second) : nullptr};
...
void Muon::MuonStationBuilderCond::identifyLayers(const Trk::DetachedTrackingVolume* station, int eta, int phi, const MuonGM::MuonDetectorManager* muonMgr) const
{
...
void Muon::MuonStationBuilderCond::identifyPrototype(const Trk::TrackingVolume* station, int eta, int phi, Amg::Transform3D transf, const MuonGM::MuonDetectorManager* muonMgr) const
{
...
void Muon::MuonStationBuilderCond::getNSWStationsForTranslation(const GeoVPhysVol* pv, std::string name, Amg::Transform3D transform, std::vector<std::pair<std::pair<const GeoLogVol*,Trk::MaterialProperties*> , std::vector<Amg::Transform3D> > >& vols, std::vector< std::string >& volNames ) const
{
