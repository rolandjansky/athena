...
std::pair<EventIDRange, const Trk::TrackingGeometry*> Muon::MuonTrackingGeometryBuilderCond::trackingGeometry(const EventContext& ctx, std::pair<EventIDRange, const Trk::TrackingVolume*> tVolPair) const
{
  ...
  if (inertObjs && m_blendInertMaterial && m_removeBlended) {
    while ( m_inertPerm<inertObjs->size() 
        && (*inertObjs)[m_inertPerm]->name().substr((*inertObjs)[m_inertPerm]->name().size()-4,4)=="PERM") m_inertPerm++;
  }

  // find object's span with tolerance for the alignment 
  if (!aLVC.m_stationSpan) aLVC.m_stationSpan = findVolumesSpan(stations.get(), 100.*m_alignTolerance, m_alignTolerance*Gaudi::Units::deg,aLVC);
  ...
  // process volume
  // barrel
  if (aLVC.m_adjustStatic && aLVC.m_static3d) muonBarrel = processVolume( &barrelVol,0,"Muon::Detectors::Barrel",aLVC, hasStations); 
  else if (aLVC.m_adjustStatic) muonBarrel = processVolume( &barrelVol,-1,"Muon::Detectors::Barrel",aLVC, hasStations);
  else muonBarrel = processVolume( &barrelVol,m_barrelEtaPartition,m_phiPartition,"Muon::Detectors::Barrel",aLVC, hasStations); 
  // inner Endcap
  ...
  if (aLVC.m_adjustStatic && aLVC.m_static3d) negativeMuonSmallWheel = processVolume( &negSWVol,1,"Muon::Detectors::NegativeSmallWheel",aLVC , hasStations); 
  else if (aLVC.m_adjustStatic) negativeMuonSmallWheel = processVolume( &negSWVol,-1,"Muon::Detectors::NegativeSmallWheel" ,aLVC, hasStations); 
  ...
  if (aLVC.m_adjustStatic && aLVC.m_static3d) positiveMuonSmallWheel = processVolume( &posSWVol,1,"Muon::Detectors::PositiveSmallWheel" ,aLVC, hasStations); 
  else if (aLVC.m_adjustStatic) positiveMuonSmallWheel = processVolume( &posSWVol,-1,"Muon::Detectors::PositiveSmallWheel" ,aLVC, hasStations); 
  ...
  if (aLVC.m_adjustStatic && aLVC.m_static3d) negativeECT = processVolume( &negECTVol,2,"Muon::Detectors::NegativeECT" ,aLVC, hasStations); 
  else if (aLVC.m_adjustStatic) negativeECT = processVolume( &negECTVol,-1,"Muon::Detectors::NegativeECT" ,aLVC, hasStations); 
  ...
  if (aLVC.m_adjustStatic && m_static3d) positiveECT = processVolume( &posECTVol,2,"Muon::Detectors::PositiveECT" ,aLVC, hasStations); 
  else if (aLVC.m_adjustStatic) positiveECT = processVolume( &posECTVol,-1,"Muon::Detectors::PositiveECT" ,aLVC, hasStations); 
  ...
  if (aLVC.m_adjustStatic && aLVC.m_static3d) negativeMuonOuterWheel = processVolume( &negOWVol,3,"Muon::Detectors::NegativeOuterWheel" ,aLVC, hasStations); 
  else if (aLVC.m_adjustStatic) negativeMuonOuterWheel = processVolume( &negOWVol,-1,"Muon::Detectors::NegativeOuterWheel" ,aLVC, hasStations); 
  ...
  if (aLVC.m_adjustStatic && aLVC.m_static3d) positiveMuonOuterWheel = processVolume( &posOWVol,3,"Muon::Detectors::PositiveOuterWheel" ,aLVC, hasStations); 
  else if (aLVC.m_adjustStatic) positiveMuonOuterWheel = processVolume( &posOWVol,-1,"Muon::Detectors::PositiveOuterWheel" ,aLVC, hasStations); 
  ...
  if (aLVC.m_adjustStatic && aLVC.m_static3d) negativeMuonOuterBuffer = processVolume( &negBuffVol,3,"Muon::Detectors::NegativeOuterBuffer" ,aLVC, hasStations); 
  else if (aLVC.m_adjustStatic) negativeMuonOuterBuffer = processVolume( &negBuffVol,-1,"Muon::Detectors::NegativeOuterBuffer" ,aLVC, hasStations); 
  ...
  if (aLVC.m_adjustStatic && aLVC.m_static3d) positiveMuonOuterBuffer = processVolume( &posBuffVol,3,"Muon::Detectors::PositiveOuterBuffer" ,aLVC, hasStations); 
  else if (aLVC.m_adjustStatic) positiveMuonOuterBuffer = processVolume( &posBuffVol,-1,"Muon::Detectors::PositiveOuterBuffer" ,aLVC, hasStations); 
  ...
  if (aLVC.m_adjustStatic && aLVC.m_static3d) negativeMuonBigWheel = processVolume( &negBWVol,3,"Muon::Detectors::NegativeBigWheel" ,aLVC, hasStations); 
  else if (aLVC.m_adjustStatic) negativeMuonBigWheel = processVolume( &negBWVol,-1,"Muon::Detectors::NegativeBigWheel" ,aLVC, hasStations); 
  ...
  if (aLVC.m_adjustStatic && aLVC.m_static3d) positiveMuonBigWheel = processVolume( &posBWVol,3,"Muon::Detectors::PositiveBigWheel" ,aLVC, hasStations); 
  else if (aLVC.m_adjustStatic) positiveMuonBigWheel = processVolume( &posBWVol,-1,"Muon::Detectors::PositiveBigWheel" ,aLVC, hasStations); 
...
const std::vector<std::vector<std::pair<const Trk::DetachedTrackingVolume*,const Muon::Span*> >* >* Muon::MuonTrackingGeometryBuilderCond::findVolumesSpan(const std::vector<std::unique_ptr<const Trk::DetachedTrackingVolume> >* objs, double zTol, double phiTol, LocalVariablesContainer &aLVC) const
{
  ...
  for (unsigned int iobj=0; iobj<objs->size(); iobj++) {
    ...
    if ( (*span)[0] < -m_bigWheel ) {
      (*spans)[0]->push_back(std::pair<const Trk::DetachedTrackingVolume*,const Span*>((*objs)[iobj].get(),span));
      nspans++;
    } 
    // negative big wheel
    if ( (*span)[0] < -aLVC.m_innerEndcapZ && (*span)[1]>-m_bigWheel ) {
      (*spans)[1]->push_back(std::pair<const Trk::DetachedTrackingVolume*,const Span*>((*objs)[iobj].get(),span));
      nspans++;
    } 
    // neg.ect
    if ( (*span)[0] < -m_ectZ && (*span)[1]>-aLVC.m_innerEndcapZ ) {
      (*spans)[2]->push_back(std::pair<const Trk::DetachedTrackingVolume*,const Span*>((*objs)[iobj].get(),span));
      nspans++;
    } 
    // neg.small wheel
    if ( (*span)[0] < -m_diskShieldZ && (*span)[1]>-m_ectZ ) {
      (*spans)[3]->push_back(std::pair<const Trk::DetachedTrackingVolume*,const Span*>((*objs)[iobj].get(),span));
      nspans++;
    } 
    // barrel  
    if ( (*span)[0] <  m_diskShieldZ && (*span)[1]> -m_diskShieldZ ) {
      //	 && ((*span)[5]> m_innerBarrelRadius || (*span)[0]<-m_barrelZ || (*span)[1]>m_barrelZ)  ) {
      (*spans)[4]->push_back(std::pair<const Trk::DetachedTrackingVolume*,const Span*>((*objs)[iobj].get(),span));
      nspans++;
    } 
    // pos.small wheel
    if ( (*span)[0] < m_ectZ && (*span)[1]> m_diskShieldZ ) {
      (*spans)[5]->push_back(std::pair<const Trk::DetachedTrackingVolume*,const Span*>((*objs)[iobj].get(),span));
      nspans++;
    } 
    // pos.ect
    if ( (*span)[0] < aLVC.m_innerEndcapZ && (*span)[1]> m_ectZ ) {
      (*spans)[6]->push_back(std::pair<const Trk::DetachedTrackingVolume*,const Span*>((*objs)[iobj].get(),span));
      nspans++;
    } 
    // positive big wheel
    if ( (*span)[0] < m_bigWheel && (*span)[1]> aLVC.m_innerEndcapZ ) {
      (*spans)[7]->push_back(std::pair<const Trk::DetachedTrackingVolume*,const Span*>((*objs)[iobj].get(),span));
      nspans++;
    } 
    // positive outer wheel
    if ( (*span)[1] >  m_bigWheel ) {
      (*spans)[8]->push_back(std::pair<const Trk::DetachedTrackingVolume*,const Span*>((*objs)[iobj].get(),span));
      nspans++;
    } 
  ...
  const Trk::TrackingVolume* Muon::MuonTrackingGeometryBuilderCond::processVolume(const Trk::Volume* vol, int etaN , int phiN, std::string volumeName, LocalVariablesContainer& aLVC, bool hasStations) const
  {
    ...
    if ( etaN * phiN > 1 ) {  // partition
      ...
      for (int eta = 0; eta < etaN; eta++) {
        ...
        for (int phi = 0; phi < phiN; phi++) {
          ...
          blendVols.clear();
          std::vector<const Trk::DetachedTrackingVolume*>* detVols = nullptr;
          if(hasStations){
      ...
      // enclosed muon objects ? 
      blendVols.clear();
      std::vector<const Trk::DetachedTrackingVolume*>* muonObjs = nullptr;
      if(hasStations){
  ...
  const Trk::TrackingVolume* Muon::MuonTrackingGeometryBuilderCond::processVolume(const Trk::Volume* vol, int mode , std::string volumeName, LocalVariablesContainer& aLVC, bool hasStations) const
  {
  ...
  const Trk::TrackingVolume* Muon::MuonTrackingGeometryBuilderCond::processShield(const Trk::Volume* vol, int type,std::string volumeName, LocalVariablesContainer &aLVC, bool hasStations) const
  {
