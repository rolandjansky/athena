...
Muon::MuonInertMaterialBuilderCond::buildDetachedTrackingVolumes( const EventContext& ctx, bool blend) const
{
  ...
  std::pair<std::vector<std::unique_ptr<const Trk::DetachedTrackingVolume>>,std::vector<std::unique_ptr<const Trk::DetachedTrackingVolume> > > mInert;
  ...
  for (; msTypeIter != msTypes->end(); ++msTypeIter) {
    ...
    for (unsigned int it=0 ;it<(*msTypeIter).second.size(); it++)  { 
      ...
      if (perm) mInert.first.push_back(std::move(newStat));
      else mInert.second.push_back(std::move(newStat));
    }
  }
...
Muon::MuonInertMaterialBuilderCond::buildDetachedTrackingVolumeTypes(const EventContext& ctx, bool blend) const
{
  ...
  if (!readHandle.isValid() || *readHandle == nullptr) {
    ATH_MSG_ERROR(m_muonMgrReadKey.fullKey() << " is not available. Could not get MuonDetectorManager, no layers for muons will be built.");
  }
  ...
  const Trk::TrackingVolume* Muon::MuonInertMaterialBuilderCond::simplifyShape(const Trk::TrackingVolume* trVol, bool blend, std::vector<std::vector<std::pair<std::unique_ptr<const Trk::Volume>,float> > >* constituentsVector)const
  {
    // envelope
...
#undef double
  ...
  void Muon::MuonInertMaterialBuilderCond::getObjsForTranslation(const GeoVPhysVol* pv,Amg::Transform3D transform, std::vector<std::pair<const GeoLogVol*, std::vector<Amg::Transform3D> > >& vols ) const
  {
    // subcomponents 
    unsigned int nc = pv->getNChildVols();
  ...
  const Trk::Volume* Muon::MuonInertMaterialBuilderCond::createEnvelope(const Amg::Transform3D transf, std::vector<std::pair<const Trk::Volume*,std::pair<float,float> > > constituents ) const 
  {
...
#undef double
  ...
  std::vector<std::pair<const Trk::Volume*,std::pair<float,float> > > 
    Muon::MuonInertMaterialBuilderCond::splitComposedVolume(const Trk::Volume* trVol, bool estimateVol) const
    {
