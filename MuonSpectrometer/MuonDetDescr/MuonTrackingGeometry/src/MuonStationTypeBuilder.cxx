                                                          ...
                                                          Cache& cache) const
{
  ...
  for (const GeoVolumeVec_t::value_type& p : geoGetVolumes(mv)) {
    ...
    if (cname.substr(0, nameSize) == vname.substr(0, nameSize))
  ...
  */
  ...
  for (unsigned i = 0; i < compVol.size(); i++) {
    ...
    const Trk::CuboidVolumeBounds* compBounds =
      dynamic_cast<const Trk::CuboidVolumeBounds*>(&(compVol[i]->volumeBounds()));
    ...
    if (openRpc && compName[i].substr(0, 3) != "RPC" && compName[i].substr(0, 3) != "Ded") {
      ...
      if (Xcurr >= currX + rpclowXsize + rpcuppXsize) {
        ...
        Trk::Volume* rpcVol = new Trk::Volume(
    ...
    if (openSpacer && compName[i].substr(0, 1) != "C" && compName[i].substr(0, 2) != "LB") {
      ...
      if (Xcurr - currX - (spacerlowXsize + spaceruppXsize) >= -tolerance) {
        ...
        Trk::Volume spacerVol(new Amg::Transform3D(Amg::Translation3D(currX + spacerBounds->halflengthX(), 0., 0.)),
                              spacerBounds);
    ...
    if (compName[i].substr(0, 1) == "C" || compName[i].substr(0, 2) == "LB") {
      if (!openSpacer) {
        ...
        if (compVol[i]->center()[0] + compBounds->halflengthX() > currX + spacerlowXsize + spaceruppXsize)
          spaceruppXsize +=
            (compVol[i]->center()[0] + compBounds->halflengthX()) - (currX + spacerlowXsize + spaceruppXsize);
      }
      comp_processed = true;
    }
    if (compName[i].substr(0, 3) == "MDT") {
      ...
      if (lowX == currX) {
        ...
        mdtVol = new Trk::Volume(new Amg::Transform3D(Amg::Translation3D(0., 0., -zShift) * compVol[i]->transform()),
                                 mdtBounds);
      } else {
    ...
    if (!comp_processed)
      std::cout << "unknown technology:" << compName[i] << std::endl;
  ...
  for (size_t i = 0; i < compVol.size(); i++)
    delete compVol[i];
                                                          ...
                                                          Cache& cache) const
{
  ...
  for (const GeoVolumeVec_t::value_type& p : geoGetVolumes(mv)) {
    ...
    if (clv->getShape()->type() == "Trd") {
      ...
      if (halfX1 == halfX2 && halfY1 == halfY2)
        volBounds = new Trk::CuboidVolumeBounds(fmax(halfX1, halfX2), fmax(halfY1, halfY2), halfZ);
      if (halfX1 == halfX2 && halfY1 != halfY2) {
        transf *= Amg::AngleAxis3D(0.5 * M_PI, Amg::Vector3D(0., 1., 0.)) *
      ...
      if (!volBounds)
        std::cout << "volume shape for component not recognized" << std::endl;
    } else if (clv->getShape()->type() == "Box") {
      const GeoBox* box = dynamic_cast<const GeoBox*>(clv->getShape());
      halfX1 = box->getXHalfLength();
      ...
      volBounds = new Trk::TrapezoidVolumeBounds(
        envelope->minHalflengthX(), envelope->maxHalflengthX(), envelope->halflengthY(), xSize);
    }
    vol = new Trk::Volume(new Amg::Transform3D(transf), volBounds);
    std::string cname = clv->getName();
    std::string vname = mv->getLogVol()->getName();
  ...
  for (unsigned i = 0; i < compVol.size(); i++) {
    bool comp_processed = false;
    const Trk::CuboidVolumeBounds* compCubBounds =
      dynamic_cast<const Trk::CuboidVolumeBounds*>(&(compVol[i]->volumeBounds()));
    const Trk::TrapezoidVolumeBounds* compTrdBounds =
      dynamic_cast<const Trk::TrapezoidVolumeBounds*>(&(compVol[i]->volumeBounds()));
    if (compCubBounds) {
      lowX = compVol[i]->center()[0] - compCubBounds->halflengthX();
      uppX = compVol[i]->center()[0] + compCubBounds->halflengthX();
      if (lowX < currX)
        std::cout << "Warning: we have a clash between components here!" << std::endl;
      if (uppX > maxX)
        std::cout << "Warning: we have a clash between component and envelope!" << std::endl;
      // low edge of current volume
    ...
    if (openSpacer && compName[i].substr(0, 1) != "C" && compName[i].substr(0, 2) != "LB") {
      if (Xcurr - currX - (spacerlowXsize + spaceruppXsize) >= -tolerance) {
        Trk::TrapezoidVolumeBounds* spacerBounds =
          new Trk::TrapezoidVolumeBounds(envX1, envX2, envY, 0.5 * (Xcurr - currX));
    ...
    if (compName[i].substr(0, 3) == "MDT") {
      ...
      if (lowX == currX) {
        ...
        mdtVol = new Trk::Volume(
          new Amg::Transform3D(Amg::Translation3D(0., 0., 0.5 * (currX - lowX)) * compVol[i]->transform()), mdtBounds);
      }
  ...
  if (openSpacer) {
    if (maxX >= currX + spacerlowXsize + spaceruppXsize) {
      Trk::TrapezoidVolumeBounds* spacerBounds =
        new Trk::TrapezoidVolumeBounds(envX1, envX2, envY, 0.5 * (maxX - currX));
      ...
      std::cout << "currX,maxX,lowX,uppX:" << currX << "," << maxX << "," << spacerlowXsize << "," << spaceruppXsize
                << std::endl;
...
Muon::MuonStationTypeBuilder::processMdtBox(Trk::Volume*& vol,
                                            ...
                                            Cache& cache) const
{
    ...
    if ((clv->getName()) == "MultiLayerFoam") {
      ...
      if (!mdtMat) {
        ...
        if (!cub) {
          ...
          cache.m_mdtFoamMat.push_back(
            std::make_unique<Trk::MaterialProperties>(getAveragedLayerMaterial(cv, volume, 2 * xv)));
        }
        if (cache.m_mdtFoamMat.size())
  ...
  if (volBounds) {
    ...
    for (unsigned int iloop = 0; iloop < x_array.size(); iloop++) {
      ...
      Trk::MaterialProperties matLay =
        x_mat[iloop] ? *(x_mat[iloop]) : Trk::MaterialProperties(*m_muonMaterial, thickness);
  ...
  mdtLayerArray =
    new Trk::NavBinnedArray1D<Trk::Layer>(layerOrder, binUtility, new Amg::Transform3D(Trk::s_idTransform));
  std::string name = "MDT";
...
//
const Trk::TrackingVolume*
Muon::MuonStationTypeBuilder::processMdtTrd(Trk::Volume*& vol,
                                            const GeoVPhysVol*& gv,
                                            Amg::Transform3D* transf,
                                            Cache& cache) const
{
...
//
const Trk::TrackingVolume*
Muon::MuonStationTypeBuilder::processRpc(Trk::Volume*& vol,
                                         std::vector<const GeoVPhysVol*> gv,
                                         std::vector<Amg::Transform3D> transfc,
                                         Cache& cache) const
{
      ...
      if ((glv->getName()).substr(0, 3) == "Ded") {
        ...
        if (!found) {
          ...
          cache.m_rpcDed.push_back(
            std::make_unique<Trk::MaterialProperties>(getAveragedLayerMaterial(gv[ic], volc, 2 * xs)));
          ...
          for (unsigned int igc = 0; igc < ngc; igc++) {
            ...
            if ((gclv->getName()).substr(0, 6) == "RPC_AL") {
              ...
              layer = new Trk::PlaneLayer(
                Amg::Transform3D(Amg::Translation3D(trgc.translation()) * (cTr)), bounds, rpcMaterial, thickness, od);
              layers.push_back(layer);
              layer->setLayerType(0);
...
const Trk::TrackingVolume*
Muon::MuonStationTypeBuilder::processSpacer(Trk::Volume& vol,
                                            std::vector<const GeoVPhysVol*> gv,
                                            std::vector<Amg::Transform3D> transf) const
{
  ...
  for (unsigned int ic = 0; ic < gv.size(); ++ic) {
    ...
    ATH_MSG_VERBOSE(" spacer material all X0 " << cmat.X0 << " L0 " << cmat.L0 << " A " << cmat.A << " Z " << cmat.Z
                                               << " rho " << cmat.rho);
    Trk::OverlapDescriptor* od = 0;
    if (clv->getShape()->type() == "Box") {
      ...
      if (sub && sub->getOpA()->type() == "Box" && sub->getOpB()->type() == "Box") {
        ...
        if (thickness > 0.) {
          ...
          Trk::PlaneLayer* layx =
            new Trk::PlaneLayer(Amg::Transform3D(transf[ic] * Amg::Translation3D(shift, 0., 0.) *
                                                 Amg::AngleAxis3D(M_PI_2, Amg::Vector3D(0., 1., 0.)) *
                                                 Amg::AngleAxis3D(M_PI_2, Amg::Vector3D(0., 0., 1.))),
                                bounds,
                                spacerMaterial,
        ...
        if (thickness > 0.) {
          ...
          bounds = Trk::SharedObject<const Trk::SurfaceBounds>(rbounds);
          Trk::PlaneLayer* lay =
            new Trk::PlaneLayer(Amg::Transform3D(transf[ic] * Amg::Translation3D(0., shift, 0.) *
        ...
        if (thickness > 0.) {
          ...
          bounds = Trk::SharedObject<const Trk::SurfaceBounds>(rbounds);
          Trk::PlaneLayer* layz = new Trk::PlaneLayer(
            Amg::Transform3D(transf[ic] * Amg::Translation3D(0., 0., shift)), bounds, spacerMaterial, thickness, od, 0);
          layers.push_back(layz);
          Trk::SharedObject<const Trk::SurfaceBounds> bounds2(bounds);
          Trk::PlaneLayer* layzz =
            new Trk::PlaneLayer(Amg::Transform3D(transf[ic] * Amg::Translation3D(0., 0., -shift)),
                                bounds2,
                                spacerMaterial,
                                thickness,
                                od,
        ...
        if (shift)
          subVs.push_back(std::pair<const GeoShape*, Amg::Transform3D>(shift->getOp(), shift->getX()));
        ...
        if (box && subVs.size() == 4) {
          ...
          if (boxB && v1 && v2) {
            ...
            Trk::SubtractedPlaneLayer* layxx =
              new Trk::SubtractedPlaneLayer(subPlaneX.get(), spacerMaterial, thickness, od, 0);
            layers.push_back(layxx);
            delete subPlane;
...
const Trk::TrackingVolume*
Muon::MuonStationTypeBuilder::processNSW(std::vector<const Trk::Layer*> layers) const
{
  ...
  for (unsigned int il = 0; il < layers.size(); il++) {
    ...
    const Trk::TrapezoidBounds* trdBounds =
      dynamic_cast<const Trk::TrapezoidBounds*>(&(layers[il]->surfaceRepresentation().bounds()));
    ...
    if (trdBounds) {
      ...
      if (m_muonMgr->mmIdHelper()->is_mm(id)) {
        if (abs(m_muonMgr->mmIdHelper()->stationEta(id)) == 1)
          hMin = trdBounds->minHalflengthX();
        ...
        if (abs(m_muonMgr->mmIdHelper()->stationEta(id)) == 4)
          hMax = trdBounds->maxHalflengthX();
      }
    }
  }
  ...
  Trk::TrapezoidVolumeBounds* trdVolBounds =
    new Trk::TrapezoidVolumeBounds(hMin, hMax, 0.5 * (rMax - rMin), 0.5 * std::abs(zMax - zMin));
...
Trk::TrackingVolume*
Muon::MuonStationTypeBuilder::processCscStation(const GeoVPhysVol* mv, std::string name, Cache& cache) const
{
    ...
    if (clv->getShape()->type() == "Shift") {
      ...
      if (shift->getOp()->type() == "Union") {
        ...
        if (uni->getOpA()->type() == "Trd") {
          ...
          if (xSizes.size() > 0)
            xSizes.push_back(
              (std::abs(transform.translation()[0] - compTransf[ich - 1].translation()[0]) - xSizes.back()));
          else
            xSizes.push_back(xSize);
          double xpos = (transform * shift->getX()).translation()[0];
          if (xpos - xSize < xmn)
            xmn = xpos - xSizes.back();
          if (xpos + xSize > xmx)
            xmx = xpos + xSizes.back();
        }
      } // end Union
    }   // end Shift
    if (clv->getShape()->type() == "Trd") {
  ...
  double xTotal = 0;
  for (unsigned int i = 0; i < xSizes.size(); i++)
    xTotal += xSizes[i];
  ...
  if (!isDiamond) {
    ...
    for (unsigned int ic = 0; ic < xSizes.size(); ic++) {
      ...
      const Trk::TrackingVolume* compTV =
        new Trk::TrackingVolume(*compVol, *m_muonMaterial, cscLayerArray, 0, compName[ic]);
      delete compVol;
...
std::vector<const Trk::TrackingVolume*>
Muon::MuonStationTypeBuilder::processTgcStation(const GeoVPhysVol* mv, Cache& cache) const
{
  // TGC stations
  ...
  for (const GeoVolumeVec_t::value_type& p : geoGetVolumes(mv)) {
    ...
    if (baseShape->type() == "Trd") {
      ...
      // ready to build the station prototype
      const Trk::TrackingVolume* tgc_station =
        new Trk::TrackingVolume(*envelope, *m_muonMaterial, tgcLayerArray, 0, tgc_name);

      delete envelope;
      if (tgc_station)
        tgc_stations.push_back(tgc_station);
...
void
Muon::MuonStationTypeBuilder::printChildren(const GeoVPhysVol* pv) const
{
  // subcomponents
  for (const GeoVolumeVec_t::value_type& p : geoGetVolumes(pv)) {
    ...
    const Amg::Transform3D& transf = p.second;
    std::cout << "  ";
    std::cout << "subcomponent:" << clv->getName() << ", made of" << clv->getMaterial()->getName() << ","
              << clv->getShape()->type() << "," << transf.translation() << std::endl;

    if (clv->getShape()->type() == "Trd") {
      const GeoTrd* trd = dynamic_cast<const GeoTrd*>(clv->getShape());
      //
      std::cout << "dimensions:" << trd->getXHalfLength1() << "," << trd->getXHalfLength2() << ","
    ...
    if (clv->getShape()->type() == "Box") {
      const GeoBox* box = dynamic_cast<const GeoBox*>(clv->getShape());
      //
      std::cout << "dimensions:" << box->getXHalfLength() << "," << box->getYHalfLength() << ","
...
double
Muon::MuonStationTypeBuilder::get_x_size(const GeoVPhysVol* pv) const
{
  double xlow = 0;
  double xup = 0;
  // subcomponents
...
Muon::MuonStationTypeBuilder::getAveragedLayerMaterial(const GeoVPhysVol* pv, double volume, double thickness) const
{
  ...
  if (sumMat.thickness() != thickness && sumMat.thickness() > 0.) {
    ...
    ATH_MSG_VERBOSE("averaged material scale :" << sf << " sumMat.material().X0() " << sumMat.material().X0
                                                << " sumMat.material().L0 " << sumMat.material().L0
                                                << " sumMat.material().rho " << sumMat.material().rho
...
void
Muon::MuonStationTypeBuilder::collectMaterial(const GeoVPhysVol* pv, Trk::MaterialProperties& layMat, double sf) const
{
  ...
  if (lv->getMaterial()->getName() != "Air" && nm.substr(0, 1) != "T") {
    ...
    ATH_MSG_VERBOSE(" collectMaterial current material:" << lv->getMaterial()->getName() << " d " << d << " newMP.x0() "
                                                         << newMP.x0() << " L0 " << newMP.L0 << " A " << newMP.A
                                                         << " Z " << newMP.Z << " rho " << newMP.rho);

    // protect nan
    if (d > 0 && newMP.x0() > 0.) {
      layMat.addMaterial(newMP, d / newMP.x0());
      ATH_MSG_VERBOSE(" collectMaterial layMat thickness " << layMat.thickness() << " thickness in X0 "
                                                           << layMat.thicknessInX0());
    }
  }
  // subcomponents
...
double
Muon::MuonStationTypeBuilder::getVolume(const GeoShape* shape) const
{
  //
  double volume = 0.;
...
Muon::MuonStationTypeBuilder::processCSCTrdComponent(const GeoVPhysVol*& pv,
                                                     ...
                                                     Cache& cache) const
{
  ...
  if (name.substr(name.size() - 5, 5) == "CSC01") {
    ...
    if (x_array.size() == 0) {
      ...
      Trk::MaterialProperties xmatCSC(xthick,
                                      scale * matCSC.x0(),
                                      scale * matCSC.l0(),
                                      matCSC.averageA(),
                                      matCSC.averageZ(),
  ...
  Trk::SharedObject<const Trk::SurfaceBounds> bounds(tbounds);
  for (unsigned int iloop = 0; iloop < x_array.size(); iloop++) {
    Amg::Transform3D cTr((*transf) *
                         Amg::Translation3D(0., 0., x_array[iloop])); // this won't work for multiple layers !!! //
...
const Trk::LayerArray*
Muon::MuonStationTypeBuilder::processTGCComponent(const GeoVPhysVol*& pv,
                                                  Trk::TrapezoidVolumeBounds*& tgcBounds,
                                                  Amg::Transform3D*& transf,
                                                  Cache& cache) const
{
  ...
  matTGC = Trk::MaterialProperties(activeThick,
                                   scale * matTGC.x0(),
                                   scale * matTGC.l0(),
                                   matTGC.averageA(),
                                   matTGC.averageZ(),
  ...
  Trk::SharedObject<const Trk::SurfaceBounds> bounds(tbounds);
  for (unsigned int iloop = 0; iloop < x_array.size(); iloop++) {
    Amg::Transform3D cTr(Amg::Translation3D(x_array[iloop], 0., 0.) *
                         (*transf)); // this won't work for multiple layers !!! //
...
double
Muon::MuonStationTypeBuilder::decodeX(const GeoShape* sh) const
{
  double xHalf = 0;
  ...
  if (!trd && !box && !tub && !tubs && !shift && !uni && !sub && !spb) {
    ATH_MSG_WARNING("decodeX(GeoShape=" << sh->type() << "): shape type " << sh->type()
  ...
  if (trd)
    xHalf = fmax(trd->getXHalfLength1(), trd->getXHalfLength2());
  if (box)
...
std::pair<const Trk::Layer*, const std::vector<const Trk::Layer*>*>
Muon::MuonStationTypeBuilder::createLayerRepresentation(const Trk::TrackingVolume* trVol) const
{

  const Trk::Layer* layRepr = 0;
  if (!trVol)
    return std::pair<const Trk::Layer*, const std::vector<const Trk::Layer*>*>(layRepr, 0);
  ...
  const Trk::TrapezoidVolumeBounds* trdBounds =
    dynamic_cast<const Trk::TrapezoidVolumeBounds*>(&(trVol->volumeBounds()));
  const Trk::DoubleTrapezoidVolumeBounds* dtrdBounds =
    dynamic_cast<const Trk::DoubleTrapezoidVolumeBounds*>(&(trVol->volumeBounds()));

  Amg::Transform3D subt = Trk::s_idTransform;

  const Trk::SubtractedVolumeBounds* subBounds =
    dynamic_cast<const Trk::SubtractedVolumeBounds*>(&(trVol->volumeBounds()));
  if (subBounds) {
    subt *=
  ...
  if (cubBounds) {
    ...
    if (matProp.thickness() > thickness) {
      ...
      // matProp.scale(sf);
      matProp = Trk::MaterialProperties(thickness,
                                        sf * matProp.x0(),
                                        sf * matProp.l0(),
                                        matProp.averageA(),
                                        matProp.averageZ(),
    ...
    layer = new Trk::PlaneLayer(
      Amg::Transform3D(trVol->transform() * Amg::AngleAxis3D(0.5 * M_PI, Amg::Vector3D(0., 1., 0.)) *
                       Amg::AngleAxis3D(0.5 * M_PI, Amg::Vector3D(0., 0., 1.))),
      bounds,
      mat,
      thickness,
    ...
    if (m_multilayerRepresentation && trVol->confinedVolumes()) {
      ...
      if (vols.size() > 1) {
        for (unsigned int i = 0; i < vols.size(); i++) {
          ...
          multi->push_back(new Trk::PlaneLayer(Amg::Transform3D(vols[i]->transform()),
                                               bounds,
                                               Trk::HomogeneousLayerMaterial(matMulti, 0.),
                                               matMulti.thickness(),
                                               od,
...
Identifier
Muon::MuonStationTypeBuilder::identifyNSW(std::string vName, Amg::Transform3D transf) const
{
  Identifier id(0);
...
const Trk::Layer*
Muon::MuonStationTypeBuilder::createLayer(const Trk::TrackingVolume* trVol,
                                          Trk::MaterialProperties* matEx,
                                          Amg::Transform3D& transf) const
{
  ...
  const Trk::DoubleTrapezoidVolumeBounds* dtrdBounds =
    dynamic_cast<const Trk::DoubleTrapezoidVolumeBounds*>(&(trVol->volumeBounds()));
  const Trk::SimplePolygonBrepVolumeBounds* pbBounds =
    dynamic_cast<const Trk::SimplePolygonBrepVolumeBounds*>(&(trVol->volumeBounds()));
  ...
  if (subBounds) {
    subt *=
      Amg::AngleAxis3D(0.5 * M_PI, Amg::Vector3D(0., 1., 0.)) * Amg::AngleAxis3D(0.5 * M_PI, Amg::Vector3D(0., 0., 1.));
    while (subBounds) {
      ATH_MSG_VERBOSE("looping over subtracted volume bounds:outer,inner position:"
                      << subBounds->outer()->center() << "," << subBounds->inner()->center());
      const Trk::CuboidVolumeBounds* ocubBounds =
        dynamic_cast<const Trk::CuboidVolumeBounds*>(&(subBounds->outer()->volumeBounds()));
      ...
      ATH_MSG_VERBOSE("outer volume:box,trd,dtrd,spb,subtr:" << ocubBounds << "," << otrdBounds << "," << odtrdBounds
                                                             << "," << opbBounds << "," << subBounds);
      if (ocubBounds)
        cubBounds = ocubBounds;
      if (otrdBounds)
        trdBounds = otrdBounds;
      if (odtrdBounds)
        dtrdBounds = odtrdBounds;
      if (opbBounds)
        pbBounds = opbBounds;
      subBounds = dynamic_cast<const Trk::SubtractedVolumeBounds*>(&(subBounds->outer()->volumeBounds()));
    }
  ...
  if (cubBounds) {
    ...
    Trk::MaterialProperties matProp(thickness,
                                    matEx->x0() / scale,
                                    matEx->l0() / scale,
                                    matEx->averageA(),
                                    matEx->averageZ(),
    ...
    Trk::SharedObject<const Trk::SurfaceBounds> bounds(rbounds);
    layer =
      new Trk::PlaneLayer(Amg::Transform3D(trVol->transform() * Amg::AngleAxis3D(M_PI_2, Amg::Vector3D(0., 1., 0.)) *
                                           Amg::AngleAxis3D(M_PI_2, Amg::Vector3D(0., 0., 1.))),
                          bounds,
                          mat,
                          thickness,
    ...
    if (rtrd) {
      Trk::TrapezoidBounds* tbounds =
        new Trk::TrapezoidBounds(rtrd->halflengthX(), rtrd->minHalflengthY(), rtrd->maxHalflengthY());
      Trk::SharedObject<const Trk::SurfaceBounds> bounds(tbounds);
      layer = new Trk::PlaneLayer(Amg::Transform3D(subt * trVol->transform()), bounds, mat, thickness, od, 1);
      ...
      Trk::TrapezoidBounds* tbounds =
        new Trk::TrapezoidBounds(trd->minHalflengthX(), trd->maxHalflengthX(), trd->halflengthY());
      Trk::SharedObject<const Trk::SurfaceBounds> bounds(tbounds);
      layer = new Trk::PlaneLayer(Amg::Transform3D(subt * trVol->transform()), bounds, mat, thickness, od, 1);
      Amg::Vector3D mtg_pos = (transf * subt * trVol->transform()).translation();
      transf *= Amg::Translation3D(mrg_pos - mtg_pos);
...
Trk::MaterialProperties
Muon::MuonStationTypeBuilder::collectStationMaterial(const Trk::TrackingVolume* vol, double sf) const
{
