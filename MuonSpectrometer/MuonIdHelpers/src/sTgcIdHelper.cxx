...
int sTgcIdHelper::initialize_from_dictionary(const IdDictMgr& dict_mgr) {
  ...
  if(!m_dict) {
    if (m_msgSvc) {
      log << MSG::ERROR << " initialize_from_dict - cannot access MuonSpectrometer dictionary " << endmsg;
    }
    return 1;
  }
  ...
  if (index == -1) {
    if (m_msgSvc) {
      log << MSG::DEBUG << "initLevelsFromDict - there are no sTGC entries in the dictionary! "  << endmsg;
    }
    return 0;
  }
  ...
  if (field) {
    ...
    if (m_msgSvc) {
      log << MSG::ERROR << "initLevelsFromDict - unable to find 'stgcMultilayer' field "  << endmsg;
    }
    status = 1;
  }
  ...
  if (field) {
    ...
    if (m_msgSvc) {
      log << MSG::ERROR << "initLevelsFromDict - unable to find 'stgcGasGap' field " << endmsg;
    }
    status = 1;
  }
  ...
  if (field) {
    ...
    if (m_msgSvc) {
      log << MSG::ERROR << "initLevelsFromDict - unable to find 'channelType' field " 	<< endmsg;
    }
    status = 1;
  }
  ...
  if (field) {
    ...
    if (m_msgSvc) {
      log << MSG::ERROR << "initLevelsFromDict - unable to find 'channel' field " << endmsg;
    }
    status = 1;
  }
  ...
  if(!stgcGroup) {
    if (m_msgSvc) {
      log << MSG::ERROR << "Cannot find stgc group" << endmsg;
    }
  } else {
  ...
  if (atlasDict->get_label_value("subdet", "MuonSpectrometer", muonField)) {
    if (m_msgSvc) {
      log << MSG::ERROR << "Could not get value for label 'MuonSpectrometer' of field 'subdet' in dictionary " << atlasDict->m_name << endmsg;
    }
    return (1);
  }
  ...
  if (muon_range.size() > 0 ) {
    if (m_msgSvc) {
      log << MSG::INFO << "MultiRange built successfully to Technology: " << "MultiRange size is " << muon_range.size() << endmsg;
    }
  } else {
    if (m_msgSvc) {
      log << MSG::ERROR << "Muon sTGC detector element MultiRange is empty" << endmsg;
    }
  }
  ...
  if (muon_channel_range.size() > 0 ) {
    ...
    if (m_msgSvc) {
      log << MSG::ERROR << "Muon sTGC detector MultiRange is empty for channels" << endmsg;
    }
  }
  ...
  if (m_full_module_range.size() == 0) {
    if (m_msgSvc) {
      log << MSG::ERROR << "sTGC MultiRange ID is empty for modules" << endmsg;
    }
    status = 1;
  }
  ...
  if (m_full_detectorElement_range.size() == 0) {
    if (m_msgSvc) {
      log << MSG::ERROR << "sTGC MultiRange ID is empty for detector elements" << endmsg;
    }
    status = 1;
  }
  ...
  if (m_full_channel_range.size() == 0) {
    if (m_msgSvc) {
      log << MSG::ERROR << "sTGC MultiRange ID is empty for channels" << endmsg;
    }
    status = 1;
  }
  ...
  if (m_msgSvc) {
    log << MSG::INFO << "Initializing sTGC hash indices ... " << endmsg;
  }
  status = init_hashes();
  ...
  */
  ...
  if (m_msgSvc) {
    log << MSG::INFO << "Initializing sTGC hash indices for finding neighbors ... " << endmsg;
  }
