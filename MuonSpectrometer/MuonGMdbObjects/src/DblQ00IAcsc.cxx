 ...
 : m_nObj(0)
{
  ...
  if(m_iacsc) {
	...
	//std::cerr<<" do we ever come here ???"<<std::endl;
        ...
        m_d[i].version        = m_iacsc->data<int>("ISZT_DATA.VERS");    
        m_d[i].line           = i; 
        m_d[i].jff            = m_iacsc->data<int>("ISZT_DATA.JFF");
        m_d[i].jzz            = m_iacsc->data<int>("ISZT_DATA.JZZ");
        m_d[i].job            = m_iacsc->data<int>("ISZT_DATA.JOB");
        m_d[i].wireLayer      = m_iacsc->data<int>("ISZT_DATA.JLAY");
	//std::cerr<<" do we ever come here1 ???"<<std::endl;
	
        m_d[i].tras           = 10.*m_iacsc->data<float>("ISZT_DATA.TRAS"); // I lines in mm, but ISZT in cm
        m_d[i].traz           = 10.*m_iacsc->data<float>("ISZT_DATA.TRAZ"); // I lines in mm, but ISZT in cm
        m_d[i].trat           = 10.*m_iacsc->data<float>("ISZT_DATA.TRAT"); // I lines in mm, but ISZT in cm
        m_d[i].rots           = m_iacsc->data<float>("ISZT_DATA.ROTS");
        m_d[i].rotz           = m_iacsc->data<float>("ISZT_DATA.ROTZ");
        m_d[i].rott           = m_iacsc->data<float>("ISZT_DATA.ROTT");
        //m_d[i].i              = m_iacsc->data<int>("ISZT_DATA.I");
	//std::cerr<<" do we ever come here 2???"<<std::endl;

        sprintf(m_d[i].type,"%s",m_iacsc->data<std::string>("ISZT_DATA.TYP").c_str());
