...
class SgeJobList:
    ...
    def qsub(self,submitstring):
        ret, out = subprocess.getstatusoutput(submitstring)
        if not ret==0:
    ...
    def qstat(self,jobId):
        ...
        ret, out = subprocess.getstatusoutput(qstatstring)
        if ret!=0:
            print ("qstat failed:\n", out)
            return -1
    ...
    def wait(self,logfilename):
        ...
        for jobId in tmpJobList:
            ...
            #print ("qstatstring: ",qstatstring)
            #while  subprocess.getstatusoutput(qstatstring)[0] == 0:    
...
class LxbatchJobList:
    ...
    def bjobs(self,jobId):
        ...
        ret, out = subprocess.getstatusoutput(bjobsstring)
        if ret!=0:
            print ("bjobs failed:\n", out)
            return -1
        for line in out.split("\n"):
    ...
    def check(self,logfilename):
        ret, out=subprocess.getstatusoutput("ls %s.gz" % logfilename)
        if ret==0:
            os.system("gunzip %s.gz" % (logfilename))
        ret, out=subprocess.getstatusoutput("ls %s" % logfilename)
        if ret==0:
            grepstring = "grep \'leaving with code 0\' %s" % (logfilename)
            ret, out=subprocess.getstatusoutput(grepstring)
...
class PathenaJobList:
    ...
    def wait(self,logfilename):
        ...
        if self.jobId==-99:
            ...
            os.system(jobstring3)
            ret, out = subprocess.getstatusoutput("cat "+logfilename2)
            lines=out.split('\n')
            for line in lines:
                items=line.split()
