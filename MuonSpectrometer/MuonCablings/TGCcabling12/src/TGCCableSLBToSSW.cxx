...
TGCModuleMap* TGCCableSLBToSSW::getModuleIn (const TGCModuleId* ssw) const {
  ...
  for(int type=0; type<TGCIdBase::MaxRegionType* MaxModuleType; type++){
	...
	if( module[type]==TGCIdBase::WI   ) {
	  ...
	  //std::cerr << "TGCCableSLBToSSW::getModuleIn  " << type << ":" 
	  //	      << module[type] <<  ", " << region[type] 
	  //	      << " ssw sector= " << ssw->getReadoutSector()-1  << std::endl;
	  ...
	  for (int ip= 0; ip<3 ; ip++){ 
	    ...
	    //std::cerr << "SLB " << module[type] <<  ", " << region[type] 
	    //	      << ": sector=" << psector  << " id=" << id 
	    //	      <<std::endl;
	  }
