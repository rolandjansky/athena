...
SectorLogicSetup::SectorLogicSetup() : BaseObject(Logic,"Sector Logic Map"),
    ...
    m_layout(""),m_lowpt_1031(false)
{
}

SectorLogicSetup::SectorLogicSetup(int type,std::string layout,bool conf) : 
...
{
    //cout << "SectorLogicSetup::setup ==> Il settore tipo e` " << sector_type()
    //     << std::endl; 
        ...
        if(size)
	{
	    for (RPCmap::iterator rpc = lower; rpc != upper;++rpc)
	        RPCsetup &= (*rpc).second.setup(ijk_eta,ijk_phi);
	}
