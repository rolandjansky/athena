...
#include "RPCcablingSim/EvenPhiCMA.h"
...
bool
EvenPhiCMA::cable_CMA_channelsP03(void)
{
    if(m_pivot_station)  //Check and connect strips with Pivot matrix channels
    {
        WORlink::iterator found = m_pivot_WORs.find(this->pivot_start_ch());
...
EvenPhiCMA::get_confirm_strip_boundaries(int stat,int max)
{
	...
	if(this->id().Ixx_index() == 0)
	    ...
	    //m_highPt_stop_st = (confirm_channels<max)? confirm_channels : max;
	    //m_highPt_stop_st -= 40 - max/2; 
        }
        //else if (pivot_stop_st() == max_pivot_strips)
	else if (this->id().Ixx_index() == 1)
        {
...
void
EvenPhiCMA::get_confirm_strip_boundariesP03(int stat,int max)
{
    EvenPhiCMA::WORlink::const_iterator it = m_pivot_WORs.begin();
    int max_pivot_strips = (*it).second->give_max_phi_strips();
    if(stat == lowPt_station())
