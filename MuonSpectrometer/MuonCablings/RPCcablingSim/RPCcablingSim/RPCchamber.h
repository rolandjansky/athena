...
class RPCchamber : public CablingObject
{
    ...
    //int m_eta_splitters;     // Number of eta splitters
    ...
    //unsigned int m_eta_inv0; // bits to invert connectors in gas gap 0 (eta) 
    //unsigned int m_eta_inv1; // bits to invert connectors in gas gap 1 (eta)
    
    
    int m_phi_strips;        // Number of phi strips
    int m_phi_connectors;    // Number of phi connectors
    //int m_phi_splitters;     // Number of phi splitters
    int m_ijk_phiReadout;    // readout schema of the gas gaps in phi:
                             //     01 = gap 0 in layer 0, gap 1 in layer 1
			     //     10 = gap 0 in layer 1, gap 1 in layer 0
			     
    //unsigned int m_phi_inv0; // bits to invert connectors in gas gap 0 (phi) 
    //unsigned int m_phi_inv1; // bits to invert connectors in gas gap 1 (phi)
    ...
    //int eta_splitters(void)     const {return m_eta_splitters;}
    int phi_connectors(void)    const {return m_phi_connectors;}
    //int phi_splitters(void)     const {return m_phi_splitters;}
    
    int ijk_etaReadout(void)    const {return m_ijk_etaReadout;}
    int ijk_phiReadout(void)    const {return m_ijk_phiReadout;}
    //unsigned int eta_inv0(void) const {return m_eta_inv0;}
    //unsigned int eta_inv1(void) const {return m_eta_inv1;}
    //unsigned int phi_inv0(void) const {return m_phi_inv0;}
    //unsigned int phi_inv1(void) const {return m_phi_inv1;}
