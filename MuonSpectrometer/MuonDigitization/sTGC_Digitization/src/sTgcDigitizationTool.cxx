...
StatusCode sTgcDigitizationTool::processBunchXing(int bunchXing,
  ...
  for (; iEvt!=eSubEvents; ++iEvt) {
    ...
    const sTGCSimHitCollection* seHitColl(nullptr);
    ...
    //Copy hit Collection
    sTGCSimHitCollection* sTGCHitColl = new sTGCSimHitCollection("sTGCSensitiveDetector");
    sTGCSimHitCollection::const_iterator i = seHitColl->begin();
    sTGCSimHitCollection::const_iterator e = seHitColl->end();
  ...
  *************************************************/
  for(std::map< Identifier, std::map< Identifier, std::vector<sTgcDigit> > >::iterator it_coll = outputDigits.begin(); it_coll != outputDigits.end(); ++it_coll){
	...
	if ( acceptDigit ) { 
	  ...
	  if ( m_idHelper->channelType(it_digit->identify()) == 1 ) {
