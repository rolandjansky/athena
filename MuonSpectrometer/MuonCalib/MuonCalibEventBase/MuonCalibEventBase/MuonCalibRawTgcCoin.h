...
namespace MuonCalib {
  ...
  class MuonCalibRawTgcCoin {
    ...
    MuonCalibRawTgcCoin( const MuonFixedId& id, const Amg::Vector3D& gPosIn, const Amg::Vector3D& gPosOut, const int type, const int eta, const int phi,
			 ...
			 const float widthIn, const float widthOut, const float widthR, const float widthPhi, const int delta, const int roi,const int pt, 
			 const int sub, const int isPositiveDeltaR);  
    ...
    void setGlobalPositionOut(const Amg::Vector3D& gPosOut) { m_gPosOut = gPosOut;} //!< sets the position expressed in global coordinates
  ...
  inline MuonCalibRawTgcCoin::MuonCalibRawTgcCoin( const MuonFixedId& id, const Amg::Vector3D& gPosIn, const Amg::Vector3D& gPosOut, const int type, const int eta, const int phi,
						   const int sector, const int isForward, const int isStrip, const int trackletId, const int trackletIdStrip, const int bcTag,
						   const float widthIn, const float widthOut, const float widthR, const float widthPhi, const int delta, const int roi, const int pt, 
						   const int sub=0, const int isPositiveDeltaR=0)
    :  m_id(id), m_gPosIn(gPosIn), m_gPosOut(gPosOut), m_type(type), m_eta(eta), m_phi(phi), m_sector(sector), m_isForward(isForward), m_isStrip(isStrip), m_trackletId(trackletId),
    m_trackletIdStrip(trackletIdStrip), m_bcTag(bcTag), m_widthIn(widthIn), m_widthOut(widthOut), m_widthR(widthR), m_widthPhi(widthPhi), m_delta(delta), m_roi(roi), m_pt(pt),
    m_sub(sub), m_isPositiveDeltaR(isPositiveDeltaR)
{ }
