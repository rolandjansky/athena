...
def GetFitBy(region_string, joblist="joblist.txt", default="CHAMBER"):
	...
	if len(reg_items)!=3:
		...
		return default
	try:
		statnion_nm=reg_items[0].strip()
		station_phi=int(reg_items[1])
		station_eta=int(reg_items[2])
	except ValueError:
		print "WARNING Failed to interpret region string fro GetFitBy. Using default", default
		return default
	...
	for line in inf.readlines():
		...
		try:
			...
			continue
		if statnion_nm==ln_station and station_phi==ln_phi and station_eta==ln_eta:
			return items[3]
