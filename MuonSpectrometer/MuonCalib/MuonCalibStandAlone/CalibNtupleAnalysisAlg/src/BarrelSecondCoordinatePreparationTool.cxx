...
// detector store //
	...
	if (!sc.isSuccess()) {
		log << MSG::FATAL << "Cannot retrieve DetectorStore!" << endreq;
		return sc;
	}
...
// muon fixed id tool //
	sc = toolSvc()->retrieveTool("MuonCalib::IdToFixedIdTool",
					"MuonCalib_IdToFixedIdTool", m_id_tool);
	if(!sc.isSuccess()) {
		log << MSG::FATAL<< "Can't retrieve MuonCalib_IdToFixedIdTool!" << endreq;
		return sc;
	}
...
// COLLECT RAW RPC HITS
		...
		for (vector<MuonCalibRawRpcHit *>::const_iterator it1=raw_RPC_begin_it;
                                                    it1!=raw_RPC_end_it; it1++) {
...
int BarrelSecondCoordinatePreparationTool::rpcFit(vector<CLHEP::HepVector> & RPC_hits,
	...
	Gmm.invert(ierr);
	if(ierr) {
		cout<<"Gmm matrix inversion ERROR:"<<ierr<<" Can't invert matrix"<<endl;
		return -1;
	}
