...
SegmentRefitter :: SegmentRefitter(const std::string & t, const std::string & n, const IInterface *p): AlgTool(t, n, p)
	{
	m_curved = false;
...
inline bool SegmentRefitter :: refine_t0(MuonCalibSegment *& segment)
	...
	p_t0_refinement->getDeltaT0(segment, p_calib_input_svc->GetRtRelation(segment->mdtHOT()[0]->identify()), true, err, failed, m_curved);
	...
	for (unsigned int h=0; h<segment->mdtHitsOnTrack(); h++)
		{
		const IRtResolution *resol( p_calib_input_svc->GetResolution( segment->mdtHOT()[h]->identify()));
