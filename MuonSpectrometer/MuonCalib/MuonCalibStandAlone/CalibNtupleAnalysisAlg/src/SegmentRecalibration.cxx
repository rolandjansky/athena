...
SegmentRecalibration :: SegmentRecalibration(const std::string & t, const std::string & n, const IInterface *p): AlgTool(t, n, p)
	{
	m_t0_switch="LEAVE";
...
//				std::cout<<hit->slewingTime()<<std::endl;
		...
		//overwrite t0 correction
			...
			//get t0 if it was not already selected for this station
				if((t0=p_calib_input_svc->GetT0(id)) == NULL)
					{
					log << MSG::FATAL << "No t0 correction loaded for this chamber!" << endreq;
					event = NULL;
					return;
					}
		...
		//overwrite magnetic field correction of selected
			...
			if(m_B_corr_numswitch == 2)
				{
				if((B_corr = p_calib_input_svc->GetBCorr(id)) == NULL)
					{
					log << MSG::FATAL << "No rt relation loaded for this chamber!" << endreq;
					event=NULL;
					return;
					}
			...
			if(m_rt_scaling_numswitch==2)
					{
				if((rt_relation = p_calib_input_svc->GetRtRelation(id)) == NULL)
					{
						log << MSG::FATAL << "No rt relation loaded for this chamber! Needed for scaling!" << endreq;
						event=NULL;
						return;
					}
		...
		//overwrite drift radius if selected
			if(m_recalc_r_switch)
				...
				if((spat_res = p_calib_input_svc->GetResolution(id)) == NULL)
					{
					log << MSG::FATAL << "No rt relation loaded for this chamber!" << endreq;
					event=NULL;
					return;
...
inline bool SegmentRecalibration :: convert_switch(const std::string & s_switch, int & num_switch, const char * switchname) const
	...
	MsgStream log(msgSvc(), name());
	log << MSG::FATAL << "Invalid switch setting "<<s_switch<< " for "<< switchname<<"!"<<endreq;
	return false;
	}
