...
SegmentRawdataSelector :: SegmentRawdataSelector(const std::string & t, const std::string & n, const IInterface *p): AlgTool(t, n, p)
	{
	m_min_hits = 1;
...
// detector store //
	sc=service(m_detector_store, m_detStore);
	if (!sc.isSuccess()) {
...
// loop over the raw hits //
   ...
   // get the raw hit and check whether it is in the calibration region //
		...
		Identifier station_id = m_id_tool->fixedIdToId(
							hit->identify());
		const MuonGM::MdtReadoutElement *MdtRoEl =
		m_detMgr->getMdtReadoutElement(
	    	m_MdtIdHelper->channelID(station_id,
				hit->identify().mdtMultilayer(),
				hit->identify().mdtTubeLayer(),
				hit->identify().mdtTube()));
   ...
   // create a segment if necessary //
		...
		MDT_calib_hit->setDriftRadius(0.02*TDC_to_ns*hit->tdcCount(),
									0.2);
