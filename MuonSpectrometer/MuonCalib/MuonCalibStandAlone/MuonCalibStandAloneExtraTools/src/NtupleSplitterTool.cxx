...
NtupleSplitterTool::NtupleSplitterTool(const std::string& t, const std::string& n, const IInterface* p): AthAlgTool(t, n, p), m_split_level(1), m_ntuple_split_level(2), m_calib_tool_handle("MuonCalib::NtupleRunScanTool"), m_file_number(0), m_stored_patterns(0)
	{
...
//loop over al lnew segments
	...
	//get list of regions
		...
		//fil lsegment in patttern, if it is not already inserted
...
//			std::cout<<"."<<std::endl;
			...
			for(MuonCalibTrack_E :: HitVector::const_iterator h_it=track->beginHit(); h_it!=track->endHit(); h_it++)
				{
				const MuonCalibHit_E * hit(*h_it);
				if(!((*hb_it)->fillBranch(*hit, (*tb_it)->getBranchEntries() - 1)))
					{
