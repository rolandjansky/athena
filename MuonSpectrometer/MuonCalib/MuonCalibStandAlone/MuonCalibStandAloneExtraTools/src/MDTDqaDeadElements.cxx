...
//This method reads the reference histograms and builds the chamber with the correct geometry
  ...
  MDTDqaDeadElements::MDTDqaDeadElements()
  {
    ...
    dead_tubes_per_sector=0;
    dead_mezzanines_per_sector=0;
    dead_layers_per_sector=0;
    dead_multilayers_per_sector=0;
    dead_chambers_per_sector=0;
    lowstat_chambers_per_sector=0;
  }
    ...
    if(m_write_report)
      {
        ...
        file_report.open((const char*)filename);
      }
    if(m_write_compact_report)
      {
        ...
        comp_report.open((const char*)filename);
      }
    if(m_write_list_of_dead_tubes)
      {
        TString filename=f->GetName();
        filename+="_ListOfDeadTubes.txt";
    	filelistofdeads.open((const char*)filename);
      }
	...
	for(int isector=1; isector<=16; isector++)
	  {
		...
		delete chamber;	
            ...
            hsummary->SetBinContent(isector,1,lowstat_chambers_per_sector);
            hsummary->SetBinContent(isector,2,dead_chambers_per_sector);
            hsummary->SetBinContent(isector,3,dead_multilayers_per_sector);
            hsummary->SetBinContent(isector,4,dead_layers_per_sector);
            hsummary->SetBinContent(isector,5,dead_mezzanines_per_sector);
            hsummary->SetBinContent(isector,6,dead_tubes_per_sector);
  ...
  void MDTDqaDeadElements::doAnalysis(MDTChamber* chamber)
  {
    ...
    if(m_write_report) file_report<<endl<<"Analysing Chamber "<<chamber->getName()<<"------------------------"<<endl;

    if(chamber->getEntries()<=MINENTRIES)
      {
	chamber->setStatus(DEADCHAMBER);
        if(m_write_report) file_report<<chamber->getName()<<" Appears to be dead chamber as it has no entries"<<endl<<endl;
        if(m_write_compact_report) comp_report<<chamber->getName()<<" dead_CHAMBER"<<endl;
        m_deadchambers++;
        dead_chambers_per_sector++;
        return;
      }
    ...
    if(m_write_report) file_report<<"entries="<<chamber->getEntries();
    if(m_write_report) file_report<<" median= "<<median<<"; median_dev= "<<median_deviation<<"; ratio="<<ratio<<endl;

    if((median<=MINMEDIAN)&&(ratio<=MAXRATIO))
      {
	chamber->setStatus(LOWSTATISTICSCHAMBER);
        if(m_write_report) file_report<<chamber->getName()<<" Has Low Statistics, it won't be analysed"<<endl;
        if(m_write_compact_report) comp_report<<chamber->getName()<<" LOW STATISTICS"<<endl;
        m_lowstatisticschambers++;
        lowstat_chambers_per_sector++;
        return;
      }
     if(m_write_report) file_report<<chamber->getName()<<" Has enough statistics, starting other analysis"<<endl;
    ...
    if(m_write_report) file_report<<endl<<"END OF ANALYSIS "<<chamber->getName()<<"------------------------"<<endl;
    if(m_write_report) file_report<<endl<<endl;
  ...
  void MDTDqaDeadElements::analyseMultilayers(MDTChamber *chamber)
  {
    ...
    double maxMLmedian=0;
     ...
     if(m_write_report) file_report<<"MLs MAXMEDIAN="<<maxMLmedian<<endl;
     if(maxMLmedian<MIN_MAXMEDIAN_ML)
       {
          if(m_write_report) file_report<<"MLs have not enough statistics to be analized"<<endl;
          return;
       }
     if(chamber->getNmultilayers()==1)
       {
	 if(m_write_report) file_report<<"Only 1ML in chamber; ML analysis ok;"<< endl;
         return;
       }
     for(int i=1; i<=chamber->getNmultilayers(); i++)
       {
	  ...
	  double diffToOther=diff/meanMedDev;
          if(m_write_report) file_report<<"ML"<<i<<"entries="<<ml->getEntries();
	  if(m_write_report) file_report<<"; median="<<ml->getMedian()<<"; diffToOther="<<diffToOther<<"; ";
          if(m_write_report) file_report<<"maxMezzMed="<<maxMezzMed<<endl;  
	  ...
	  int contr=0;
	      ...
	      if(diffToOther<-SPREADDEAD)
		{
		  if(maxMezzMed<(ml2->getMedian()-SPREADDEAD*meanMedDev))
		    {
		      if(m_write_report) file_report<<"ML "<<i<<" seems to be dead ML"<<endl;
                      if(m_write_compact_report) comp_report<<chamber->getName()<<" dead_MULTILAYER "<<i<<endl;
                      ml->setStatus(DEADML);
                      m_deadmultilayers++;
                      dead_multilayers_per_sector++;
		      contr=1;
		    }
		}
	    }
	    ...
	    {
		      if(m_write_report) file_report<<"ML "<<i<<" seems to be inefficient ML"<<endl;
                      if(m_write_compact_report) comp_report<<chamber->getName()<<" inefficient_MULTILAYER "<<i<<endl;
	...
	if(contr==0) {if(m_write_report) file_report<<"ML "<<i<<" OK"<<endl;}
  ...
  void MDTDqaDeadElements::analyseLayers(MDTChamber *chamber)
  {
     ...
     if(maxMedianLayers<MIN_MAXLAYER_MED)
       {
	  if(m_write_report) file_report<<"Layers without enough statistics, not analysed"<<endl;
	  return;
       }	
		...
		double diff=l->getMedian()-medianOfOtherMedians;
 		...
 		if(diff==0) diffToOthers=0;
		else 
		   {
	        ...
	        if(m_write_report) file_report<<"Layer"<<j<<" entries= "<<l->getEntries()<<" median="<<l->getMedian()<<"; diffToOther="<<diffToOthers<<"; ";
                if(m_write_report) file_report<<"medianOfOthersMedian="<<medianOfOtherMedians<<"; ";
                if(m_write_report) file_report<<"medianOfOthersMedianDev="<<medianOfOtherMediansDevs<<"; ";
                if(m_write_report) file_report<<"maxMezzMedian="<<maxMezzMedian<<endl;

		if((diffToOthers>SPREADHOT)&&(l->getMedian()>MINMEDHOT))
	          {
                    if(!m_do_noisy) continue;

		    if(m_write_report) file_report<<"ML "<<i<<" Layer "<<j<<" seems to be noisy layer"<<endl;
                    if(m_write_compact_report) comp_report<<chamber->getName()<<" noisy_LAYER "<<i<<" "<<j<<endl;
		...
		if(l->getMedian()<MIN_LAYER_MED)
		  {
			...
			if((maxMezzMedian<(medianOfOtherMedians-SPREADDEAD*medianOfOtherMediansDevs))||(maxMezzMedian<=MIN_OTHERMEDIANS_DEAD))
			  {
			    if(m_write_report) file_report<<"ML "<<i<<" Layer "<<j<<" seems to be dead layer"<<endl;
                            if(m_write_compact_report) comp_report<<chamber->getName()<<" dead_LAYER "<<i<<" "<<j<<endl;
                            l->setStatus(DEADLAYER);
                            m_deadlayers++;
                            dead_layers_per_sector++;
			    contr=1;
			  }
		      }
		  }
		  ...
		  {
	  	    if(m_write_report) file_report<<"ML "<<i<<" Layer "<<j<<" seems to be inefficient layer"<<endl;
                           if(m_write_compact_report) comp_report<<chamber->getName()<<" inefficient_LAYER "<<i<<" "<<j<<endl;
                    l->setStatus(INEFFICIENTLAYER);
                    m_deadlayers++;
                    dead_layers_per_sector++;
	            contr=1;
		  }
		if(contr==0) {if(m_write_report) file_report<<"ML "<<i<<" Layer "<<j<<" ok"<<endl;}
	  ...
	  for(int j=0; j<ml->getNmezzanines(); j++)
	    {
		 ...
		 if(m_write_compact_report) comp_report<<chamber->getName()<<" dead_MEZZANINE_rean "<<i<<" "<<j<<endl;
   	         ...
   	         //dead_tubes_per_sector-=ntubes;
   	         dead_tubes_per_sector-=ndeadtubes;
                 // m_deadtubes-=ntubes;
                 m_deadtubes-=ndeadtubes;
                 dead_mezzanines_per_sector++;
                 m_deadmezzanines++;
...
//    const int SPREADHOT=10;          //The noisy mezzanine analysis needs to be tuned
    ...
    double medianDeviationOfMedians=0;
     ...
     if(maxMedianMezzanines<MIN_MAXMEDIAN)
       {
	  if(m_write_report) file_report<<"Mezzanines without enough statistics, not analysed"<<endl;
	  return;
       }
     if(m_write_report) file_report<<"ChamberFlatness="<<chamberFlatness<<endl;	
...
//			    double other_meddev=near_mezz[k]->getMedianDeviation();
			...
			if(m_write_report) file_report<<chamber->getName()<<" ML "<<i<<" Mezzanine "<<j<<" appears to be dead_mezzanine; low_limit="<<lower_limit<<" median="<<med<<endl;
			if(m_write_compact_report) comp_report<<chamber->getName()<<" dead_MEZZANINE "<<i<<" "<<j<<endl;
			mezz->setStatus(DEADMEZZANINE);
			m_deadmezzanines++;
			dead_mezzanines_per_sector++;
			//			contr=1;
		...
		if(mezz->getStatus()==10 && med<10) //med=10 is compatible with 0!
		  {
		    ...
		    continue;
                ...
                if(m_write_report) file_report<<"ML"<<i<<" mezz"<<j<<": med="<<mezz->getMedian()<<"; "<<" mad="<<mezz->getMedianDeviation();
                if(m_write_report) file_report<<"medev"<<medianOfOtherMediansDevs<<"; ";
                if(m_write_report) file_report<<"entries="<<mezz->getEntries()<<"; diffToOthers="<<diffToOthers<<"; ";
                if(m_write_report) file_report<<"minlargermed="<<minOtherLargerMedians<<endl;         
                if(m_write_report) file_report<<"inefficient if med below=";
		if(m_write_report) file_report<<(minOtherLargerMedians-5*medianOfOtherMediansDevs)<<endl;
  ...
  void MDTDqaDeadElements::analyseTubes(MDTChamber *chamber)
  {
    ...
    int ndeadtubes=0;
    int nnoisytubes=0;
    if(m_write_report) file_report<<"Starting Tube Analysis"<<endl;
    ...
    if((maxMezzMed>=50)&&(chamber->get70()==0))
      {
	chamber->setStatus(DEADCHAMBER);
        if(m_write_report) file_report<<chamber->getName()<<" Appears to be a chamber with only few live tubes"<<endl<<endl;
        if(m_write_compact_report) comp_report<<chamber->getName()<<" strange_CHAMBER"<<endl;
        m_deadchambers++;
        dead_chambers_per_sector++;
        return;
	...
	m_lowstatisticsfortubeschambers++;
        lowstat_chambers_per_sector++;
	if(m_write_report) file_report<<"This chamber has too low statistics to analyze single tubes"<<endl;
        if(m_write_compact_report) comp_report<<chamber->getName()<<" LOW STATISTICS FOR TUBES"<<endl;
        for(int i=1; i<=chamber->getNmultilayers(); i++)
         {
	    Multilayer *ml=chamber->getMultilayer(i);
            if(ml->getStatus()!=10) continue;
...
//    double marco_HOTlimit=10.*(chamber->get90max());
	...
	double limit=medML-SPREADDEAD*medDevML;
        ...
        if(m_write_report)
          {
             file_report<<"ML"<<i<<" Tube Limit="<<limit<<"; medML="<<medML<<" ;medDevML="<<medDevML<<"; medML/10="<<((double)medML/10.)<<"; tube noisy over="<<(10*standard_deviation+median)<<" marcolimit="<<marco_limit<<endl;
          }
		...
		for(int k=1; k<=mezz->getNtubes(); k++)
		  {
		    ...
		    if(value<marco_limitMezz)
		      {
			if(m_write_compact_report) comp_report<<chamber->getName()<<" dead_TUBE "<<t->getML()<<" "<<t->getLayer()<<" "<<t->getPosition()<<endl;
			//if(chamber->getName()=="BIL6A01")cout<<"DEAD!!!"<<endl;

			t->setStatus(DEADTUBE);
			ndeadtubes++;
			m_deadtubes++;
			dead_tubes_per_sector++;
		      }
		  }
	    }
// 	for(int j=1; j<=ml->getNlayers(); j++)
...
// 		  {
// 		    //file_report<<"ML "<<i<<" Layer "<<k<<" Tubo "<<k<<" seems to be dead"<<endl;
//                     if(m_write_compact_report) comp_report<<chamber->getName()<<" dead_TUBE "<<i<<" "<<j<<" "<<k<<endl;
//  		    t->setStatus(DEADTUBE);
//                     ndeadtubes++;
//                     m_deadtubes++;
//                     dead_tubes_per_sector++;
...
// 		  {
//                     if(!m_do_noisy) continue;
// 		    //file_report<<"ML "<<i<<" Layer "<<k<<" Tubo "<<k<<" seems to be noisy"<<endl;
//                     if(m_write_compact_report) comp_report<<chamber->getName()<<" noisy_TUBE "<<i<<" "<<j<<" "<<k<<endl;
...
//	    }
    ...
    if(m_write_report) file_report<<"End of tube analysis; "<<ndeadtubes<<"dead tubes found"<<nnoisytubes<<"noisy tubes found"<<endl;
	...
	for(int j=1; j<=ml->getNlayers(); j++)
	  {
		...
		if((t->getStatus()==10)||(t->getStatus()==0)) continue;
                filelistofdeads<<"Chamber "<<chamber->getName()<<" ML "<<i<<" L "<<j<<" Tube "<<k<<" DEAD"<<endl;
              }
          }
      }
    return;
