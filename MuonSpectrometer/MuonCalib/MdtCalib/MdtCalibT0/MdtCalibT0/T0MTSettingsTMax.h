...
namespace MuonCalib {
    ...
    public:
        ...
        which is likely to be adapted is TMaxMinBackgroundBins()
        */
        //@{
        ...
        /** For the  pattern-recognition only a part of the drift time spectrum is used for creating the VariableBinwidthHistogram. This
         * parameter gives the lower limit for this region minus t0 */
        inline const double &VBHLow() const { return m_vbh_low; }
        inline double &VBHLow() { return m_vbh_low; }
        /** The range of the falling edge of the spectrum is searched in a region after t0. This range is given here. The range defaults to
         * a verry large area which should be ok for all spectra
         */
        ...
        If the pattern recognition selects a smaller region, the upper boarder of the region will be moved to the last bin of the time
        spectrum.

        NOTE: This should be the approximate width of the trigger matching window after . If this parmeter is to big, the background may be
        underestimated (with verry little effect on t_max).
        */
