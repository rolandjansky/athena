...
///////////////////////
	...
	if (m_param.size()!=2) {
			log<<MSG::ERROR<< "Wron number of parameters!"<<endreq;
		m_Legendre=NULL;
		return;	
	}
...
/////////////////////
	...
	vector<SamplePoint> sample_points(nb_points);
	                                      // sample points for the integral
...
// set the sample points //
	...
	for (unsigned int k=0; k<nb_points; k++) {
		...
		sample_points[k].set_x1(2*(time-0.5*(m_t_min+m_t_max))/
						(m_t_max-m_t_min));
...
// perform the fit //
	if (fitter.fit_parameters(sample_points, 1, nb_points, &legendre)) {
		log<<MSG::WARNING<<"Unaible to fit the integral in the correction!"<<endreq;
		m_Legendre=NULL;
		return;	
	}
...
/////////////////
	...
	while (rp<radius) {
		...
		integ = integ +	1.0e-3*delta*
				std::pow(fabs(rt->driftvelocity(time))*1.0e6,
							1.0-m_param[1])/
...
/////////////////////////////////////////
	...
	while (t_max-t_min>precision) {
		...
		for (int k=0; k<m_alpha.rows(); k++) {
			integ = integ+m_alpha[k]*m_Legendre->value(k, 
				2*(0.5*(t_min+t_max)-tmean)/tlength);
		}		
