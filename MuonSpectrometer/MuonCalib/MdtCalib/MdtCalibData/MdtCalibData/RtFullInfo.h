...
namespace MuonCalib {
  ...
  public:
    ...
    {
    m_rmsPos=0.0; m_meanPos=0.0;
    }

    RtFullInfo( std::string impl, int numIt, int numSeg, double meanAng, double rmsAng, double convTest, double qualTest ) : m_implementation(impl), m_numIt(numIt), m_numSeg(numSeg), m_meanAng(meanAng), m_rmsAng(rmsAng), m_meanPos(0.0), m_rmsPos(0.0), m_convEst(convTest), m_qualEst(qualTest) {}
    ...
    std::istream& read( std::istream& is ) {
      is >> m_implementation >> m_numIt >> m_numSeg 
         >> m_meanAng >> m_rmsAng >> m_convEst >> m_qualEst;
      return is;
    }
    std::ostream& write( std::ostream& os ) const {
      os <<" "<< m_implementation <<" "<< m_numIt <<" "<< m_numSeg 
         <<" "<< m_meanAng <<" "<< m_rmsAng <<" "<< m_convEst <<" "<< m_qualEst;
      return os;
    }	
