	...
	RtResolutionChebyshev getRtResolutionChebyshev(
				const std::vector<SamplePoint> & sample_points,
						const unsigned int & order);
	                                ///< get an RtResolutionChebyshev 
					///< resembling the sigma(t)
	                                ///< function as described by the
	                                ///< sample points in the vector
	                                ///< "sample_points"; use Chebyshev
	                                ///< polynomials up to order "order";
	                                ///< x1 coordinate of the sample points
	                                ///< contains the drift time,
	                                ///< x2 the corresponding radius;
	                                ///< the method takes the minimum and
	                                ///< maximum x1 values in the set of
	                                ///< sample points a lower and upper
	                                ///< limits in RtResolutionChebyshev
	RtResolutionLookUp getRtResolutionLookUp(
				const std::vector<SamplePoint> & sample_points);
	                                ///< get an RtResolutionLookUp
	                                ///< resembling the sigma(t) function as 
					///< described by the sample points in 
	                                ///< the vector "sample_points";
					///< x1 coordinate of the sample points
	                                ///< contains the drift time,
	                                ///< x2 the corresponding radius;
	                                ///< the method takes the minimum and
...
private:
	void get_min_max(const std::vector<SamplePoint> & sample_points,
					double & x_min, double & x_max);
	                                // get the minimimum and maximum x1
	                                // coordinate of the given sample points
