...
namespace MuonCalib {
    ...
    SimpleMdtSegmentSelector::SimpleMdtSegmentSelector(double chi2_max, unsigned int hits_min) :
    ...
    bool SimpleMdtSegmentSelector::select(const MuonCalibSegment& seg) const {
        ...
        if (log.level() <= MSG::VERBOSE) {
            ...
            log << MSG::VERBOSE << "Segment: chi2 " << seg.chi2() << " mdtHits " << seg.mdtHitsOnTrack() << " tan "
                << seg.direction().y() / seg.direction().z() << endmsg;
        }
