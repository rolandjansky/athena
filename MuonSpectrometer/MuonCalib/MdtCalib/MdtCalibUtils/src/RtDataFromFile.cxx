...
namespace MuonCalib {
    ...
    void RtDataFromFile::write_forDB(FILE* frt, FILE* frtt, FILE* frtr, FILE* frts, int region) const {
        ...
        if (m_fullInfo[region]) { (m_fullInfo[region])->write_forDB(frt); }
        if (m_rtRelations[region]) { (m_rtRelations[region])->write_forDB(frtt, frtr, frts); }
        return;
    }
    ...
    void RtDataFromFile::write_forDB(FILE* frt, FILE* frtt, FILE* frtr, FILE* frts) const {
        ...
        for (unsigned int i = 0; i < m_rtRelations.size(); ++i) {
            if (m_rtRelations[i]) { (m_rtRelations[i])->write_forDB(frtt, frtr, frts); }
            if (m_fullInfo[i]) { (m_fullInfo[i])->write_forDB(frt); }
        }
        return;
    }
...
std::istream& operator>>(std::istream& is, MuonCalib::RtDataFromFile& data) { return data.read(is); }

std::ostream& operator<<(std::ostream& os, const MuonCalib::RtDataFromFile& data) { return data.write(os); }
