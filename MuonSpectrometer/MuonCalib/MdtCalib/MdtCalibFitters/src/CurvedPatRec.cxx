...
double CurvedPatRec::roadWidth(void) const { return m_road_width; }
...
unsigned int CurvedPatRec::numberOfTrackHits(void) const { return m_track_hits.size(); }
...
const std::vector<const MdtCalibHitBase *> &CurvedPatRec::trackHits(void) const { return m_track_hits; }
...
double CurvedPatRec::chi2(void) const { return m_chi2; }
...
double CurvedPatRec::chi2PerDegreesOfFreedom(void) const { return m_chi2 / static_cast<double>(m_track_hits.size() - 3); }
...
const CurvedLine &CurvedPatRec::curvedTrack(void) const { return m_curved_track; }
...
bool CurvedPatRec::fit(MuonCalibSegment &r_segment, HitSelection r_selection) const {
    ...
    if (m_sfitter.fit(r_segment, r_selection)) { est_dir = m_sfitter.track().directionVector(); }
    ...
    if (m_track_hits.size() < 4) { return false; }
    /////////////////////////
    // PATTERN RECOGNITION //
    /////////////////////////
        ...
        for (unsigned int cb = 0; cb < combination.numberOfCombinations(); cb++) {
            ...
            for (unsigned int cand = 0; cand < candidates.size(); cand++) {
                ...
                for (unsigned int k = 0; k < track_hits.size(); k++) {
                    MTStraightLine tang(m_curved_track.getTangent((track_hits[k]->localPosition()).z()));
    ...
    if (segment_found == false) { return false; }
    ...
    for (unsigned int k = 0; k < errors.size(); k++) {
        if (m_track_hits[k]->sigmaDriftRadius() > 0.0) {
            errors[k] = Amg::Vector3D(1.0, m_track_hits[k]->sigmaDriftRadius(), 0.0);
        } else {
            errors[k] = Amg::Vector3D(1.0, 1.0, 0.0);
        }
    }
    ...
    for (unsigned int k = 0; k < m_track_hits.size(); k++) {
        MTStraightLine tang(m_curved_track.getTangent((m_track_hits[k]->localPosition()).z()));
        MTStraightLine wire(Amg::Vector3D(0.0, m_track_hits[k]->localPosition().y(), m_track_hits[k]->localPosition().z()), xhat, null,
                            null);
        double d(std::abs(tang.signDistFrom(wire)));
        if (m_track_hits[k]->sigma2DriftRadius() != 0) {
            m_chi2 = m_chi2 + std::pow(d - m_track_hits[k]->driftRadius(), 2) / m_track_hits[k]->sigma2DriftRadius();
        } else {
    ...
    MTStraightLine tangent(m_curved_track.getTangent((r_segment.position()).z()));
    r_segment.set(m_chi2 / static_cast<double>(m_track_hits.size() - 3), tangent.positionVector(), tangent.directionVector());
...
std::vector<Amg::Vector3D> CurvedPatRec::getHitPoints(std::vector<const MdtCalibHitBase *> track_hits,
    ...
    for (unsigned int k = 0; k < track_hits.size(); k++) { hit_vec.push_back(getHitPoint(track_hits[k], straight_track)); }
...
std::vector<Amg::Vector3D> CurvedPatRec::getHitPoints(std::vector<const MdtCalibHitBase *> track_hits,
    ...
    for (unsigned int k = 0; k < track_hits.size(); k++) {
        hit_vec.push_back(getHitPoint(track_hits[k], curved_track.getTangent((track_hits[k]->localPosition()).z())));
    }
