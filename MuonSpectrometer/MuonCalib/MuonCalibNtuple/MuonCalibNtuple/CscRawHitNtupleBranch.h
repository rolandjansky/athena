...
#ifndef MUONCALIB_CSCRAWHITNTUPLEBRANCH_H
#define MUONCALIB_CSCRAWHITNTUPLEBRANCH_H
...
namespace MuonCalib {
  ...
  class MuonFixedId;
  /**@class CscRawHitNtupleBranch

     Class to create and fill a branch in a root tree which contains
     information of MuonCaliRawCscbHits. All members of the
     MuonCalibRawCscHit are stored on the branch.
  ...
  */
  ...
  class CscRawHitNtupleBranch {
  public:
    CscRawHitNtupleBranch(std::string branchName = "rawCsc_");    //!< default constructor 

    bool  fillBranch(const MuonCalibRawCscHit &hit );    //!< fill content of hit into branch 
  ...
  private:
    ...
    int   m_index;                                         //!< counter keeping track on the number of MuonCalibRawCscHits stored in the event

    int   m_occupancy[m_blockSize];
    int   m_id[m_blockSize];
    float m_width[m_blockSize];
    float m_t[m_blockSize];
    float m_charge[m_blockSize];
