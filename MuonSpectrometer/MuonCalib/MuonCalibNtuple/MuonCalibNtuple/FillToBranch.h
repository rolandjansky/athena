...
namespace MuonCalib {
/** @class FillToBranch
    Function object to be used in combination with std::for_each.
    The constructor gets a pointer to the branch to be filled and in addition
    to that the infomation linking it to the data.
...
*/
  ...
  template< class Branch, class Info, class Data >
    class FillToBranch { 
  public: 
    FillToBranch( Branch* branch, const Info& info )  : m_branch(branch), m_eventInfo(info) { }//!< constructor get pointer to the branch to be filled and the information from the events 
 
 
    void operator() ( const Data* data ){
      if( data && m_branch ) m_branch->fillBranch( *data, m_eventInfo );
      // if( data && m_branch ) m_branch->fillBranch( *data, m_index );
    }   //!< operator called by for_each, filles the event + EventInfo into the branch 
