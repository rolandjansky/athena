...
namespace MuonCalib {
    ...
    PatternNtupleMaker::PatternNtupleMaker(const std::string& t, const std::string& n, const IInterface* p) :
        AthAlgTool(t, n, p), m_ntupleName("PatternNtupleMaker"), m_printLevel(0), m_delayFinish(false), m_ntMaker(0), m_fillTruth(true) {
    ...
    bool PatternNtupleMaker::handleTruthCollection(const MuonCalibTruthCollection* truthCollection) {
        ...
        for (; truth_it != truth_it_end; ++truth_it) {
            if (!(*truth_it)) {
                ...
                if (((*truth_it)->PDGCode() == 22) || (std::abs((*truth_it)->PDGCode()) == 2112))
                    continue;
                else
                    m_ntMaker->fillTruthToTree(**truth_it);
            }
        }
    ...
    bool PatternNtupleMaker::handleEvent(const MuonCalibEvent* event) {
        ...
        for (; pat_it != pat_it_end; ++pat_it) { handlePattern(*pat_it); }
        handled = handleRawHits(event->rawHitCollection());

        if (event->rawTriggerHitCollection()) { m_ntMaker->handleRawTriggerHits(*event->rawTriggerHitCollection()); }

        if (event->rpcSectorLogicContainer()) { m_ntMaker->fillRpcSectorLogic(*event->rpcSectorLogicContainer()); }

        // When using the ExtraTreeAlg, the finishEvent causes the event to be written.
        if (m_delayFinish == false) m_ntMaker->finishEvent();
    ...
    bool PatternNtupleMaker::handleRawHits(const MuonCalibRawHitCollection* rawHits) {
        if (!rawHits)
            ...
            m_ntMaker->handleRawHits(*rawHits);
