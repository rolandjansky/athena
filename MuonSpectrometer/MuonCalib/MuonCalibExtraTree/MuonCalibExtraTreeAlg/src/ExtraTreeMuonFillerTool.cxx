...
namespace MuonCalib {
  ...
  StatusCode ExtraTreeMuonFillerTool::writeTracks(unsigned int &index) {
    const xAOD::MuonContainer *mounContainer;
    if(evtStore()->retrieve(mounContainer, m_muonContainer).isSuccess()) {
      for(auto muon: *mounContainer) {
	...
	if(prim) {
	  ...
	  if(part) {
	    if(!writeTrackPartice(part, part==prim, m_hitsForSA, index, 2).isSuccess()){
	      ATH_MSG_FATAL("Failed to write SA track!");
	    }
	  }
	  part=muon->trackParticle(xAOD::Muon::InnerDetectorTrackParticle);
	  if(part) {
	    if(!writeTrackPartice(part, part==prim, m_hitsForSA, index, 1).isSuccess()){
	      ATH_MSG_FATAL("Failed to write ID track!");
	    }
	  }
	  part=muon->trackParticle(xAOD::Muon::CombinedTrackParticle);
	  if(part){
	    if(!writeTrackPartice(part, part==prim, m_hitsForCombined, index, 0).isSuccess()){
  ...
  inline StatusCode ExtraTreeMuonFillerTool::writeTrackPartice(const xAOD::TrackParticle *& part, bool  isPrimaryAuthor, bool writeHits, unsigned int & index, int author) {
