...
MsgStream& MdtCalibHit::dump(MsgStream& sl) const {
    ...
    sl << "   Drift time " << driftTime() << " radius " << driftRadius() << " DistRO " << distanceToReadout() << std::endl;
    sl << "   Radial residual " << radialResidual() << std::endl;
    sl << "   Global position (" << globalPosition().x() << "," << globalPosition().y() << "," << globalPosition().z() << ") "
       << "   local position (" << localPosition().x() << "," << localPosition().y() << "," << localPosition().z() << ") " << std::endl;
    return sl;
}

std::ostream& operator<<(std::ostream& stream, const MdtCalibHit& hit) { return hit.dump(stream); }

MsgStream& operator<<(MsgStream& sl, const MdtCalibHit& hit) { return hit.dump(sl); }
