...
public:
    ...
    MdtCalibHit(
        ...
        const MuonGM::MdtReadoutElement *geo);  //!< constructor initializing with a MdtPrepData, global posiition and a MdtReadoutElement
    MdtCalibHit(const Muon::MdtPrepData *dig, const Amg::Vector3D &globalPos, const Amg::Vector3D &localPos,
                const MuonGM::MdtReadoutElement *geo);  //!< constructor initializing with a MdtPrepData, both local (station) and global
                                                        //!< position and a MdtReadoutElement

    MdtCalibHit(const Identifier &id, int tdc, int adc, const Amg::Vector3D &globalPos,
                const MuonGM::MdtReadoutElement
                    *geo);  //!< constructor initializing its Identifier, tdc/adc counts, global position and a mdtReadoutElement
    MdtCalibHit(const Identifier &id, int tdc, int adc, const Amg::Vector3D &globalPos, const Amg::Vector3D &localPos,
                const MuonGM::MdtReadoutElement *geo);  //!< constructor initializing its Identifier, tdc/adc counts, both local (station)
                                                        //!< and global position and a mdtReadoutElement
    ...
    MuonCalib::MdtCalibHitBase *hitBase(
        const MuonCalib::IIdToFixedIdTool &) const;  //!< return a pointer to the MdtCalibHit object to be used in the calibration framework
