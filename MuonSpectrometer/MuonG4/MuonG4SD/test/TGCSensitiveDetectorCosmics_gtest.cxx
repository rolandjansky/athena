...
#include "src/TGCSensitiveDetectorCosmics.h"
...
class TGCSensitiveDetectorCosmicstest : public ::testing::Test {	
  protected:
    virtual void SetUp() override {
    }
...
TEST_F( TGCSensitiveDetectorCosmicstest, Initialize )
{
  G4HCofThisEvent hce;
  TGCSensitiveDetectorCosmics sd1("name1", "name1" );
  sd1.Initialize( &hce );
  ASSERT_TRUE(sd1.m_myTGCHitColl.isValid()); //check if initialization of m_myTGCHitColl is successful
}

TEST_F( TGCSensitiveDetectorCosmicstest, ProcessHits )
{
  G4Step sp;
  ...
  G4String physicalname = "physical_TGCGas_Muon::STN[2]_av___impr_Name";//set a proper name for the physical volume, which will decide which block of the tested class would be executed 
  ...
  TGCSensitiveDetectorCosmics sd2("name2", "name2" );
  ...
  ASSERT_EQ(a->begin()->TGCid(), -656384); //test if the TGCid value of the Hit generated by this member function is right, the same below
