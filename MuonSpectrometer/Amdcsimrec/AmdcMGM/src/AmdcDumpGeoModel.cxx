    ...
    for (int sphi_index = 0; sphi_index<MuonDetectorManager::NRpcStatPhi; ++sphi_index)
    {
     for (int dbr_index = 0; dbr_index<MuonDetectorManager::NDoubletR; ++dbr_index)
     {
      for (int dbz_index = 0; dbz_index<MuonDetectorManager::NDoubletZ; ++dbz_index)
      {
       ...
       bool isValid=false;
       Identifier id = m_muIdHelper->rpcIdHelper().channelID(stationName, seta_index, sphi_index, dbr_index, dbz_index, 1, 1, 1, 1, true, &isValid); // last 6 arguments are: int doubletPhi, int gasGap, int measuresPhi, int strip, bool check, bool* isValid
       if (!isValid) continue;
