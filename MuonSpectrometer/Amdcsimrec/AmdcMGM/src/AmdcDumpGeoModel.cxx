...
AmdcDumpGeoModel::AmdcDumpGeoModel(const std::string& name, ISvcLocator* pSvcLocator) :
    AthAlgorithm(name, pSvcLocator), p_AmdcsimrecAthenaSvc("AmdcsimrecAthenaSvc", name) {
    m_KountCallsDoIt = 0;
...
StatusCode AmdcDumpGeoModel::initialize() {
    ...
    if (m_SwitchOff == 0) {
        ...
        if (m_ChamberSelection == 1) {
            ...
            if (StationNameSelectedSize < 1 || StationAbsAmdcJzzSelectedSize < 1 || StationAmdcJffSelectedSize < 1 ||
                StationNameSelectedSize != StationAbsAmdcJzzSelectedSize || StationAbsAmdcJzzSelectedSize != StationAmdcJffSelectedSize) {
                ATH_MSG_FATAL(" Badly set Option ");
...
int AmdcDumpGeoModel::SelectedChamber(const std::string& TheStationName, int AmdcJzz, int AmdcJff) {
    ...
    ToBeReturned = 0;
    for (int StationNameSelectedItem = 0; StationNameSelectedItem < StationNameSelectedSize; StationNameSelectedItem++) {
        if (TheStationName == m_StationNameSelected[StationNameSelectedItem] &&
...
void AmdcDumpGeoModel::LoopMdtElements(std::ofstream& OutFile) {
                ...
                for (int dbr_index = 0; dbr_index < MuonGM::MuonDetectorManager::NMdtMultilayer; ++dbr_index) {
                    ...
                    for (int tl = 1; tl <= pReadoutElement->getNLayers();) {
                        ...
                        for (int tube = 1; tube <= pReadoutElement->getNtubesperlayer();) {
                            Identifier channelId = m_idHelperSvc->mdtIdHelper().channelID(idp, pReadoutElement->getMultilayer(), tl, tube);
                            bool IsValid = m_idHelperSvc->mdtIdHelper().valid(channelId);
                            if (IsValid) {
                                MdtCompare(OutFile, channelId, pReadoutElement);
                            } else {
                                OutFile << "*sname_index/seta_index/sphi_index/dbr_index/tl/tube" << std::endl;
                                OutFile << " " << sname_index << " " << seta_index << " " << sphi_index << " " << dbr_index << " " << tl
                                        << " " << tube << std::endl;
                                OutFile << " Not Valid??????????" << std::endl;
                            }
                            if (turn == 1) { tube = pReadoutElement->getNtubesperlayer() / 2; }
    ...
    OutFile << "Max deviations " << std::setw(12) << std::setprecision(3) << m_Mdt_MaxDiffZ << std::setw(12) << std::setprecision(3)
            << m_Mdt_MaxDiffS << std::setw(12) << std::setprecision(3) << m_Mdt_MaxDiffT << std::setw(12) << std::setprecision(3)
            << m_Mdt_MaxDiffZs << std::setw(12) << std::setprecision(3) << m_Mdt_MaxDiffSs << std::setw(12) << std::setprecision(3)
            << m_Mdt_MaxDiffTs << std::endl;
}

void AmdcDumpGeoModel::MdtCompare(std::ofstream& OutFile, Identifier channelId, const MuonGM::MdtReadoutElement* pReadoutElement) {
    bool IsValid = m_idHelperSvc->mdtIdHelper().valid(channelId);
    if (IsValid) {
        ...
        MdtCompare(OutFile, m_EpsLoMdt, m_WarningLevel, SPstationName, SPstationNameString, SPstationEta, SPstationPhi, SPmultilayer,
                   SPtubeLayer, SPtube, GeoModX_RO, GeoModY_RO, GeoModZ_RO, GeoModX, GeoModY, GeoModZ, TubeLength);
    }
}

void AmdcDumpGeoModel::MdtCompare(std::ofstream& OutFile, double EpsLoMdt, int WarningLevel, int SPstationName,
                                  std::string SPstationNameString, int SPstationEta, int SPstationPhi, int SPmultilayer, int SPtubeLayer,
                                  int SPtube, double GeoModX_RO, double GeoModY_RO, double GeoModZ_RO, double GeoModX, double GeoModY,
    ...
    double MGMDistToRO = sqrt((GeoModX - GeoModX_RO) * (GeoModX - GeoModX_RO) + (GeoModY - GeoModY_RO) * (GeoModY - GeoModY_RO) +
    ...
    double GeoModDirNorm = sqrt(GeoModXDir * GeoModXDir + GeoModYDir * GeoModYDir + GeoModZDir * GeoModZDir);
    ...
    double SZT2[3];
    double Radius = 0.;
    pAmdcsimrec->GetTubePositionFromMDTindex(SPstationNameString, SPstationEta, SPstationPhi, SPmultilayer, SPtubeLayer, SPtube, XYZ1, XYZ2,
                                             SZT1, SZT2, Radius);
    ...
    double AmdcDirNorm = sqrt(AmdcXDir * AmdcXDir + AmdcYDir * AmdcYDir + AmdcZDir * AmdcZDir);
    AmdcXDir = AmdcXDir / AmdcDirNorm;
    AmdcYDir = AmdcYDir / AmdcDirNorm;
    AmdcZDir = AmdcZDir / AmdcDirNorm;

    double ScaDirGeoModAmdc = GeoModXDir * AmdcXDir + GeoModYDir * AmdcYDir + GeoModZDir * AmdcZDir;
    ...
    double AmdcTubeLength = sqrt((XYZ1[0] - XYZ2[0]) * (XYZ1[0] - XYZ2[0]) + (XYZ1[1] - XYZ2[1]) * (XYZ1[1] - XYZ2[1]) +
    ...
    if (Iwarning >= WarningLevel && SelectedStation(AmdcStationName) == 1 && AntiSelectedStation(AmdcStationName) == 1 &&
        ...
        OutFile << "*StNa/Name/Eta/Phi/MuL/TuL/Tube /" << std::setw(4) << SPstationName << std::setw(3) << SPstationNameString
                << std::setw(4) << SPstationEta << std::setw(4) << SPstationPhi << std::setw(4) << SPmultilayer << std::setw(4)
                << SPtubeLayer << std::setw(4) << SPtube << std::endl;
        OutFile << " Jtyp/Name/Jzz/Jff/Job/Jsl/Jtube/" << std::setw(4) << AmdcJtyp << std::setw(3) << AmdcStationName << std::setw(4)
                << AmdcJzz << std::setw(4) << AmdcJff << std::setw(4) << AmdcJob << std::setw(4) << AmdcJsl << std::setw(4) << AmdcJtube
                << std::endl;
        OutFile << " GM/Am/GM-Am/X " << std::setw(12) << std::setprecision(3) << GeoModX << std::setw(12) << std::setprecision(3) << AmdcX
                << std::setw(12) << std::setprecision(3) << DiffX << WarningX << std::endl;
        OutFile << "             Y " << std::setw(12) << std::setprecision(3) << GeoModY << std::setw(12) << std::setprecision(3) << AmdcY
                << std::setw(12) << std::setprecision(3) << DiffY << WarningY << std::endl;
        OutFile << "             Z " << std::setw(12) << std::setprecision(3) << GeoModZ << std::setw(12) << std::setprecision(3) << AmdcZ
                << std::setw(12) << std::setprecision(3) << DiffZ << WarningZ << std::endl;
        OutFile << "             S " << std::setw(12) << std::setprecision(3) << GeoModS << std::setw(12) << std::setprecision(3) << AmdcS
                << std::setw(12) << std::setprecision(3) << DiffS << WarningS << std::endl;
        OutFile << "             T " << std::setw(12) << std::setprecision(3) << GeoModT << std::setw(12) << std::setprecision(3) << AmdcT
                << std::setw(12) << std::setprecision(3) << DiffT << WarningT << std::endl;
        OutFile << "     (RO)    X " << std::setw(12) << std::setprecision(3) << GeoModX_RO << std::setw(12) << std::setprecision(3)
                << AmdcX_RO << std::setw(12) << std::setprecision(3) << DiffX_RO << WarningX_RO << std::endl;
        OutFile << "     (RO)    Y " << std::setw(12) << std::setprecision(3) << GeoModY_RO << std::setw(12) << std::setprecision(3)
                << AmdcY_RO << std::setw(12) << std::setprecision(3) << DiffY_RO << WarningY_RO << std::endl;
        OutFile << "     (RO)    Z " << std::setw(12) << std::setprecision(3) << GeoModZ_RO << std::setw(12) << std::setprecision(3)
                << AmdcZ_RO << std::setw(12) << std::setprecision(3) << DiffZ_RO << WarningZ_RO << std::endl;
        OutFile << "     (RO)    S " << std::setw(12) << std::setprecision(3) << GeoModS_RO << std::setw(12) << std::setprecision(3)
                << AmdcS_RO << std::setw(12) << std::setprecision(3) << DiffS_RO << WarningS_RO << std::endl;
        OutFile << "     (RO)    T " << std::setw(12) << std::setprecision(3) << GeoModT_RO << std::setw(12) << std::setprecision(3)
                << AmdcT_RO << std::setw(12) << std::setprecision(3) << DiffT_RO << WarningT_RO << std::endl;

        OutFile << "             Zs" << std::setw(12) << std::setprecision(3) << GeoModZs << std::setw(12) << std::setprecision(3) << AmdcZs
                << std::setw(12) << std::setprecision(3) << DiffZs << WarningZs << std::endl;
        OutFile << "             Ss" << std::setw(12) << std::setprecision(3) << GeoModSs << std::setw(12) << std::setprecision(3) << AmdcSs
                << std::setw(12) << std::setprecision(3) << DiffSs << WarningSs << std::endl;
        OutFile << "             Ts" << std::setw(12) << std::setprecision(3) << GeoModTs << std::setw(12) << std::setprecision(3) << AmdcTs
                << std::setw(12) << std::setprecision(3) << DiffTs << WarningTs << std::endl;
        OutFile << "     (RO)    Zs" << std::setw(12) << std::setprecision(3) << GeoModZs_RO << std::setw(12) << std::setprecision(3)
                << AmdcZs_RO << std::setw(12) << std::setprecision(3) << DiffZs_RO << WarningZs_RO << std::endl;
        OutFile << "     (RO)    Ss" << std::setw(12) << std::setprecision(3) << GeoModSs_RO << std::setw(12) << std::setprecision(3)
                << AmdcSs_RO << std::setw(12) << std::setprecision(3) << DiffSs_RO << WarningSs_RO << std::endl;
        OutFile << "     (RO)    Ts" << std::setw(12) << std::setprecision(3) << GeoModTs_RO << std::setw(12) << std::setprecision(3)
                << AmdcTs_RO << std::setw(12) << std::setprecision(3) << DiffTs_RO << WarningTs_RO << std::endl;

        OutFile << "   Tube Length " << std::setw(12) << std::setprecision(3) << TubeLength << std::setw(12) << std::setprecision(3)
                << AmdcTubeLength << std::setw(12) << std::setprecision(3) << DiffLtub << WarningTube << std::endl;
    }
}
...
void AmdcDumpGeoModel::LoopRpcElements(std::ofstream& OutFile) {
                        ...
                        for (int idbphi = 1; idbphi <= ndbphi; idbphi++) {
                            ...
                            for (int igg = 1; igg < 3; igg++) {
                                ...
                                for (int strip = 1; strip <= pReadoutElement->NphiStrips();) {
                                    Identifier channelId = m_idHelperSvc->rpcIdHelper().channelID(
                                    ...
                                    if (IsValid) {
                                        ...
                                        OutFile << "*sname_index/seta_index /sphi_index/dbr_index/dbz_index/dbp/igg/measphi/strip"
                                                << std::endl;
                                        OutFile << " " << sname_index << " " << seta_index << " " << sphi_index << " " << dbr_index << " "
                                                << dbz_index << " " << dbp << " " << igg << " " << measphi << " " << strip << std::endl;
                                        OutFile << " Not Valid??????????" << std::endl;
                                    }
    ...
    OutFile << "Max deviations " << std::setw(12) << std::setprecision(3) << m_Rpc_MaxDiffZ << std::setw(12) << std::setprecision(3)
            << m_Rpc_MaxDiffS << std::setw(12) << std::setprecision(3) << m_Rpc_MaxDiffT << std::endl;
}

void AmdcDumpGeoModel::RpcCompare(std::ofstream& OutFile, Identifier channelId, const MuonGM::RpcReadoutElement* pReadoutElement) {
    bool IsValid = m_idHelperSvc->rpcIdHelper().valid(channelId);
    if (IsValid) {
        ...
        RpcCompare(OutFile, m_EpsLoRpc, m_WarningLevel, SPstationName, SPstationNameString, SPstationEta, SPstationPhi, SPDoubletR,
                   SPDoubletZ, SPDoubletPhi, SPGasGap, SPMeasuresPhi, SPStrip, GeoModX, GeoModY, GeoModZ);
    }
}

void AmdcDumpGeoModel::RpcCompare(std::ofstream& OutFile, double EpsLoRpc, int WarningLevel, int SPstationName,
                                  std::string SPstationNameString, int SPstationEta, int SPstationPhi, int SPDoubletR, int SPDoubletZ,
                                  int SPDoubletPhi, int SPGasGap, int SPMeasuresPhi, int SPStrip, double GeoModX, double GeoModY,
                                  double GeoModZ) {
    Amdcsimrec* pAmdcsimrec = p_AmdcsimrecAthenaSvc->GetAmdcsimrec();
    ...
    double SZT3[3];
    double SZT4[3];
    pAmdcsimrec->GetStripPositionFromRPCindex(SPstationNameString, SPstationEta, SPstationPhi, SPDoubletR, SPDoubletZ, SPDoubletPhi,
                                              SPGasGap, SPMeasuresPhi, SPStrip, XYZ1, XYZ2, XYZ3, XYZ4, SZT1, SZT2, SZT3, SZT4);
    ...
    if (Iwarning >= WarningLevel && SelectedStation(AmdcStationName) == 1 && AntiSelectedStation(AmdcStationName) == 1 &&
        ...
        OutFile << "*StNa/Name/Eta/Phi/DbR/DbZ/DbP/GG /Mphi/Strip/" << std::setw(4) << SPstationName << std::setw(3) << SPstationNameString
                << std::setw(4) << SPstationEta << std::setw(4) << SPstationPhi << std::setw(4) << SPDoubletR << std::setw(4) << SPDoubletZ
                << std::setw(4) << SPDoubletPhi << std::setw(4) << SPGasGap << std::setw(4) << SPMeasuresPhi << std::setw(4) << SPStrip
                << std::endl;
        OutFile << " Jtyp/Name/Jzz/Jff/ Job/ Jspli/Jsl/Jsz /Jstri/" << std::setw(4) << AmdcJtyp << std::setw(3) << AmdcStationName
                << std::setw(4) << AmdcJzz << std::setw(4) << AmdcJff << std::setw(6) << AmdcJob << std::setw(6) << AmdcJspli
                << std::setw(4) << AmdcJsl << std::setw(4) << AmdcJsz << std::setw(4) << AmdcJstri << std::endl;
        OutFile << " GM/Am/GM-Am/X " << std::setw(12) << std::setprecision(3) << GeoModX << std::setw(12) << std::setprecision(3) << AmdcX
                << std::setw(12) << std::setprecision(3) << DiffX << WarningX << std::endl;
        OutFile << "             Y " << std::setw(12) << std::setprecision(3) << GeoModY << std::setw(12) << std::setprecision(3) << AmdcY
                << std::setw(12) << std::setprecision(3) << DiffY << WarningY << std::endl;
...
void AmdcDumpGeoModel::LoopTgcElements(std::ofstream& OutFile) {
    ...
    for (int sname_index = 0; sname_index < MuonGM::MuonDetectorManager::NTgcStatType; ++sname_index) {
        for (int seta_index = 5; seta_index < MuonGM::MuonDetectorManager::NTgcStatEta; ++seta_index) {
            for (int sphi_index = 0; sphi_index < MuonGM::MuonDetectorManager::NTgcStatPhi; ++sphi_index) {
                ...
                const MuonGM::TgcReadoutElement* pReadoutElement = p_MuonDetectorManager->getTgcReadoutElement(readout_id);
                if (!pReadoutElement) continue;
                Identifier idr = pReadoutElement->identify();
                ...
                const MuonGM::TgcReadoutElement* pReadoutElement1 = p_MuonDetectorManager->getTgcReadoutElement(idp1);
                ...
                for (int ngg = 0; ngg < pReadoutElement->NwirePlanes(); ++ngg) {
                    ...
                    for (int channel = 1; channel <= pReadoutElement->getNGangs(ngg + 1);) {
                        ...
                        if (IsValid) {
                            ...
                            OutFile << "*sname_index/seta_index/sphi_index/ngg/isStrip/channel" << std::endl;
                            OutFile << " idp "
                                    << " " << sname_index << " " << seta_index << " " << sphi_index << " " << ngg << " " << isStrip << " "
                                    << channel << std::endl;
                            OutFile << " Not Valid??????????" << std::endl;
                        }
                        ...
                        if (channel == 1) {
                            channel = 2;
                            OutFile << " getNGangs " << pReadoutElement->getNGangs(ngg + 1) << " Is this expected? "
                                    << " " << m_idHelperSvc->tgcIdHelper().show_to_string(channelId) << std::endl;
                        }
                    }
                    for (int channel = 1; channel <= pReadoutElement1->getNGangs(ngg + 1);) {
                        ...
                        if (channel == 1) {
                            channel = 2;
                            OutFile << " getNGangs " << pReadoutElement1->getNGangs(ngg + 1) << " Is this expected? "
                                    << " " << m_idHelperSvc->tgcIdHelper().show_to_string(channelId) << std::endl;
                        }
                    }
                    isStrip = 1;
    ...
    OutFile << "Max deviations " << std::setw(12) << std::setprecision(3) << m_Tgc_MaxDiffZ << std::setw(12) << std::setprecision(3)
            << m_Tgc_MaxDiffS << std::setw(12) << std::setprecision(3) << m_Tgc_MaxDiffT << std::endl;
}

void AmdcDumpGeoModel::TgcCompare(std::ofstream& OutFile, Identifier channelId, const MuonGM::TgcReadoutElement* pReadoutElement) {
    bool IsValid = m_idHelperSvc->tgcIdHelper().valid(channelId);
    if (IsValid) {
        ...
        TgcCompare(OutFile, m_EpsLoTgc, m_WarningLevel, SPstationName, SPstationNameString, SPstationEta, SPstationPhi, SPGasGap, SPIsStrip,
                   SPChannel, GeoModX, GeoModY, GeoModZ);
    }
}

void AmdcDumpGeoModel::TgcCompare(std::ofstream& OutFile, double EpsLoTgc, int WarningLevel, int SPstationName,
                                  std::string SPstationNameString, int SPstationEta, int SPstationPhi, int SPGasGap, int SPIsStrip,
    ...
    pAmdcsimrec->GetAMDCindexFromTGC(SPstationNameString, SPstationEta, SPstationPhi, SPGasGap, SPIsStrip, SPChannel, AmdcJff, AmdcJzz,
                                     AmdcJob, AmdcJspli, AmdcJsl, AmdcJsz, AmdcJstri);
    ...
    double SZT3[3];
    double SZT4[3];
    pAmdcsimrec->GetChannelPositionFromTGCindex(SPstationNameString, SPstationEta, SPstationPhi, SPGasGap, SPIsStrip, SPChannel, XYZ1, XYZ2,
                                                XYZ3, XYZ4, SZT1, SZT2, SZT3, SZT4);
    double AmdcX = (XYZ1[0] + XYZ2[0] + XYZ3[0] + XYZ4[0]) / 4.;
    double AmdcY = (XYZ1[1] + XYZ2[1] + XYZ3[1] + XYZ4[1]) / 4.;
    double AmdcZ = (XYZ1[2] + XYZ2[2] + XYZ3[2] + XYZ4[2]) / 4.;
    double AmdcS = DCF * AmdcY - DSF * AmdcX;
    ...
    if (Iwarning >= WarningLevel && SelectedStation(AmdcStationName) == 1 && AntiSelectedStation(AmdcStationName) == 1 &&
        ...
        OutFile << "*StNa/Name/Eta/Phi/GG             /IsSt/Chann/" << std::setw(4) << SPstationName << std::setw(3) << SPstationNameString
                << std::setw(4) << SPstationEta << std::setw(4) << SPstationPhi << std::setw(16) << SPGasGap << std::setw(4) << SPIsStrip
                << std::setw(4) << SPChannel << std::endl;
        OutFile << " Jtyp/Name/Jzz/Jff/ Job/ Jspli/Jsl/Jsz /Jstri/" << std::setw(4) << AmdcJtyp << std::setw(3) << AmdcStationName
                << std::setw(4) << AmdcJzz << std::setw(4) << AmdcJff << std::setw(6) << AmdcJob << std::setw(6) << AmdcJspli
                << std::setw(4) << AmdcJsl << std::setw(4) << AmdcJsz << std::setw(4) << AmdcJstri << std::endl;
        OutFile << " GM/Am/GM-Am/X " << std::setw(12) << std::setprecision(3) << GeoModX << std::setw(12) << std::setprecision(3) << AmdcX
...
void AmdcDumpGeoModel::LoopCscElements(std::ofstream& OutFile) {
        ...
        for (int seta_index = 1; seta_index < MuonGM::MuonDetectorManager::NCscStatEta; ++seta_index) {
            for (int sphi_index = 0; sphi_index < MuonGM::MuonDetectorManager::NCscStatPhi; ++sphi_index) {
                for (int ml = 0; ml < MuonGM::MuonDetectorManager::NCscChamberLayer; ++ml) {
                    ...
                    Identifier idp1ch = m_idHelperSvc->cscIdHelper().channelID(idp1, pReadoutElement->ChamberLayer(), 1, 0, 1);
                    const MuonGM::CscReadoutElement* pReadoutElement1 = p_MuonDetectorManager->getCscReadoutElement(idp1ch);
                    ...
                    for (int gg = 1; gg <= pReadoutElement->Ngasgaps(); ++gg) {
                        ...
                        for (int Strip = 1; Strip <= pReadoutElement->NetaStrips(gg);) {
                            ...
                            if (IsValid) {
                                ...
                                OutFile << "*sname_index/seta_index/sphi_index/ml/MeasuresPhi/gg/Strip" << std::endl;
                                OutFile << " idp "
                                        << " " << sname_index << " " << seta_index << " " << sphi_index << " " << ml << " " << MeasuresPhi
                                        << " " << gg << " " << Strip << std::endl;
                                OutFile << " Not Valid??????????" << std::endl;
                            }
                            ...
                            if (Strip == 1) {
                                Strip = 2;
                                OutFile << " NetaStrips " << pReadoutElement->NetaStrips(gg) << " Is this expected? "
                                        << " " << m_idHelperSvc->cscIdHelper().show_to_string(channelId) << std::endl;
                            }
                        }
                        for (int Strip = 1; Strip <= pReadoutElement1->NetaStrips(gg);) {
                            ...
                            if (Strip == 1) {
                                Strip = 2;
                                OutFile << " NetaStrips " << pReadoutElement1->NetaStrips(gg) << " Is this expected? "
                                        << " " << m_idHelperSvc->cscIdHelper().show_to_string(channelId) << std::endl;
                            }
                        }
                        MeasuresPhi = 1;
                        for (int Strip = 1; Strip <= pReadoutElement->NphiStrips(gg);) {
                            ...
                            if (pReadoutElement->NphiStrips(gg) == 1) {
                                Strip = 2;
                                OutFile << " NphiStrips " << pReadoutElement->NphiStrips(gg) << " Is this expected? "
                                        << " " << m_idHelperSvc->cscIdHelper().show_to_string(channelId) << std::endl;
                        ...
                        for (int Strip = 1; Strip <= pReadoutElement1->NphiStrips(gg);) {
                            ...
                            if (pReadoutElement->NphiStrips(gg) == 1) {
                                Strip = 2;
                                OutFile << " NphiStrips " << pReadoutElement1->NphiStrips(gg) << " Is this expected? "
                                        << " " << m_idHelperSvc->cscIdHelper().show_to_string(channelId) << std::endl;
    ...
    OutFile << "Max deviations " << std::setw(12) << std::setprecision(3) << m_Csc_MaxDiffss << std::setw(12) << std::setprecision(3)
            << m_Csc_MaxDiffzz << std::setw(12) << std::setprecision(3) << m_Csc_MaxDifftt << std::endl;
}

void AmdcDumpGeoModel::CscCompare(std::ofstream& OutFile, Identifier channelId, const MuonGM::CscReadoutElement* pReadoutElement) {
    bool IsValid = m_idHelperSvc->cscIdHelper().valid(channelId);
    if (IsValid) {
        ...
        CscCompare(OutFile, m_EpsLoCsc, m_WarningLevel, SPstationName, SPstationNameString, SPstationEta, SPstationPhi, SPChamberLayer,
                   SPWireLayer, SPMeasuresPhi, SPStrip, GeoModX, GeoModY, GeoModZ);
    }
}

void AmdcDumpGeoModel::CscCompare(std::ofstream& OutFile, double EpsLoCsc, int WarningLevel, int SPstationName,
                                  std::string SPstationNameString, int SPstationEta, int SPstationPhi, int SPChamberLayer, int SPWireLayer,
                                  int SPMeasuresPhi, int SPStrip, double GeoModX, double GeoModY, double GeoModZ) {
    Amdcsimrec* pAmdcsimrec = p_AmdcsimrecAthenaSvc->GetAmdcsimrec();
    ...
    double SZT4[3];
    double SZTCenter[3];
    pAmdcsimrec->GetStripPositionFromCSCindex(SPstationNameString, SPstationEta, SPstationPhi, SPChamberLayer, SPWireLayer, SPMeasuresPhi,
                                              SPStrip, XYZ1, XYZ2, XYZ3, XYZ4, XYZCenter, SZT1, SZT2, SZT3, SZT4, SZTCenter);
    ...
    if (Iwarning >= WarningLevel && SelectedStation(AmdcStationName) == 1 && AntiSelectedStation(AmdcStationName) == 1 &&
        ...
        OutFile << "*StNa/Name/Eta/Phi/chLa/wLa/Strip/meaPhi/" << std::setw(4) << SPstationName << std::setw(3) << SPstationNameString
                << std::setw(4) << SPstationEta << std::setw(4) << SPstationPhi << std::setw(4) << SPChamberLayer << std::setw(4)
                << SPWireLayer << std::setw(4) << SPStrip << std::setw(4) << SPMeasuresPhi << std::endl;
        OutFile << " Jtyp/Name/Jzz/Jff/ Job/Jsl/Jtube/SZflag/" << std::setw(4) << AmdcJtyp << std::setw(3) << AmdcStationName
                << std::setw(4) << AmdcJzz << std::setw(4) << AmdcJff << std::setw(4) << AmdcJob << std::setw(4) << AmdcJsl << std::setw(4)
                << AmdcJtube << std::setw(6) << std::setprecision(1) << AmdcSZflag << std::endl;
        OutFile << " GM/Am/GM-Am/X " << std::setw(12) << std::setprecision(3) << GeoModX << std::setw(12) << std::setprecision(3) << AmdcX
                << std::setw(12) << std::setprecision(3) << DiffX << WarningX << std::endl;
        OutFile << "             Y " << std::setw(12) << std::setprecision(3) << GeoModY << std::setw(12) << std::setprecision(3) << AmdcY
                << std::setw(12) << std::setprecision(3) << DiffY << WarningY << std::endl;
        ...
        OutFile << "             ss" << std::setw(12) << std::setprecision(3) << GeoModss << std::setw(12) << std::setprecision(3) << Amdcss
                << std::setw(12) << std::setprecision(3) << Diffss << Warningss << std::endl;
        OutFile << "             zz" << std::setw(12) << std::setprecision(3) << GeoModzz << std::setw(12) << std::setprecision(3) << Amdczz
                << std::setw(12) << std::setprecision(3) << Diffzz << Warningzz << std::endl;
        OutFile << "             tt" << std::setw(12) << std::setprecision(3) << GeoModtt << std::setw(12) << std::setprecision(3) << Amdctt
                << std::setw(12) << std::setprecision(3) << Difftt << Warningtt << std::endl;
    }
}
...
//  //Mismatch in Identifier scheme
//  if (Corrected_SPstationNameString[2] == 'F' || Corrected_SPstationNameString[1] == '4') Corrected_SPstationPhi = Corrected_SPstationPhi
//  - 1 ; if (Corrected_SPstationNameString[2] == 'E' && Corrected_SPstationNameString[1] != '4') Corrected_SPstationPhi =
//  Corrected_SPstationPhi - 2 ; if (Corrected_SPstationPhi <= 0) {
//   if (Corrected_SPstationNameString[2] == 'F' || Corrected_SPstationNameString[1] == '4') Corrected_SPstationPhi = Corrected_SPstationPhi
//   + 24 ; if (Corrected_SPstationNameString[2] == 'E' && Corrected_SPstationNameString[1] != '4') Corrected_SPstationPhi =
//   Corrected_SPstationPhi + 48 ;
//  }
