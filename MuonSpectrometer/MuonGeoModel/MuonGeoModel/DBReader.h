...
namespace MuonGM {
                         ...
                         std::string s)
  {
    ...
    if (RPCprint) {
      log << MSG::INFO
          ...
          << rpc->centralAlSupPanelThickness << endmsg;
    }
    ...
    if (RPCprint) {
        log << MSG::INFO
            << "ProcessRPC::WARNING redefining RPC::bakeliteThickness to include pet "
            << rpc->bakeliteThickness
            << endmsg;
    }
    ...
    if (RPCprint) {
        log << MSG::INFO
            << " ProcessRPC:: spacerDiam, pitch, MidChamberDeadRegion_in_s, MidChamberDeadRegion_in_z "
            << rpc->spacerDiameter << " " << rpc->spacerPitch << " "
            << rpc->MidChamberDeadRegion_in_s << " " << rpc->MidChamberDeadRegion_in_z
            << endmsg;
    }
    ...
    for (unsigned int i = 0; (i<dhwrpc->size() && !done); i++) {
      if( s.substr(3,s.size()-3) == MuonGM::buildString(wrpc[i].jsta,2) ) {
        if (RPCprint) {
          log << MSG::INFO
              <<" ProcessRPC " << s << " index " << i << " jsta ="
              << wrpc[i].jsta << " strings comparison:: <"
              << s.substr(3,s.size()-3) << ">=<"
              << MuonGM::buildString(wrpc[i].jsta,2) << ">"
              << endmsg;
        }
        ...
        if (RPCprint) {
          log << MSG::INFO
              << "ProcessRPC:: RPC external sup panel: tot & Al "
        ...
        if (RPCprint) {
          log << MSG::INFO
              << "ProcessRPC:: (computed) Total RPC thickness = "
              << rpc->TotalThickness
              << endmsg;
        }
        ...
        if (RPCprint) {
          log << MSG::INFO
              << "ProcessRPC:: s_pitch, z_pitch "
              << rpc->stripPitchS << " " << rpc->stripPitchZ
              << endmsg;
        }
        ...
        if (RPCprint) {
          log << MSG::INFO
              << "ProcessRPC:: ngg, nsp, nzp " << rpc->NGasGaps_in_s << " " << rpc->NstripPanels_in_s
      ...
      if (RPCprint) {
        log << MSG::INFO
            << " index = " << i << " nstruct " << nStruct << " " << dhwrpc->size()
            << endmsg;
      }
    }
  } // end of ProcessRPC
  ...
  static void ProcessPositions(const TYPEdnaptp dnaptp, const TYPEaptp * aptp)
  {
    ...
    for (unsigned int ipos = 0; ipos<dnaptp->size(); ipos++) {
      ...
      if (stat == NULL) {
        log << MSG::ERROR << " station "
            << name << " not found; no " << name << " element will be located at iz "
            << aptp[ipos].iz << endmsg;
        continue;
      }
      ...
      for (int phiindex = 0; phiindex<8; ++phiindex) {
        ...
        if (verbose_posmap) {
          log << MSG::VERBOSE
              << "p.zindex,p.phi " << p.zindex << " " << p.phiindex
              << " shift is " << p.shift
              << endmsg;
        }
    ...
    for (StationIterator  is = mysql->StationBegin(); is != mysql->StationEnd(); ++is) {
      ...
      if (verbose_posmap) {
        log << MSG::VERBOSE
            << " Symmetrizing position map for station " << name << " with " << np
      ...
      for (PositionIterator ip = stat->begin(); ip != stat->end(); ++ip) {
        ...
        if (iz >0 && phitype != 2) {
          ...
          if ( mysql->allocPosFind(skey) == mysql->allocPosEnd() ) {
            ...
            if (verbose_posmap) {
              log << MSG::VERBOSE
                  << " symmetric pos. created at iz,iphi, z "
                  << newp.zindex << " " << newp.phiindex << " " << newp.z
                  << endmsg;
            }
            mysql->addallocPos(name.substr(0,3), newp.phiindex, newp.zindex, newp.subtype, newp.icut);
          } else {
            if (verbose_posmap) {
              log << MSG::VERBOSE << " position already allocated by subtype/cutout "
  ...
  static void ProcessAlignements(const TYPEdnaszt dnaszt, const TYPEaszt * aszt)
  {
    ...
    if (verbose_alimap) {
      log << MSG::VERBOSE
          << " ProcessAlignements:: how many stations there are ? "
          << mysql->NStations()
          << endmsg;
    }
    int fi = 0;
    ...
    for (unsigned int ipos = 0; ipos<dnaszt->size(); ipos++) {
      ...
      if (subtype != 0) {
        if (verbose_alimap) {
          log << MSG::VERBOSE << " corresponding subtype/cutout are " << subtype << "/" << cutout
              << endmsg;
        }
          ...
          if ( pit != stat->end() ) {
            if ((*pit).second.zindex == zi && (*pit).second.phiindex== fi) {
              ...
              if (controlAlines >= 111111) {
                ...
                if (verbose_alimap) {
                  log << MSG::VERBOSE
                      << "A-line: " << ap.tras << " " << ap.traz << " " << ap.trat << " "
              ...
              if (verbose_alimap) {
                log << MSG::VERBOSE
                    << " Setting aligned position for station " << stname << " with key " << key
                    << endmsg;
              }
              stat->SetAlignPos(ap);
            }
    ...
    for (unsigned int icomp = 0; icomp<dnalmn->size(); ++icomp) {
      ...
      if (cartec == "CSC") {
        ...
        log << MSG::WARNING
            ...
            <<" will not be added to station " << stat->GetName() << endmsg;
      }
