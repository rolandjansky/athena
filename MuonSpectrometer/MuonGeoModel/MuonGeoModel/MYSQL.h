...
void  MYSQL::setControlAlines(int cA) {
  m_controlAlines = cA;
}

int  MYSQL::controlAlines() const {
  return m_controlAlines;
}
...
void MYSQL::setGeometryVersion(std::string s)
{
  ...
  if (m_geometry_version != "unknown") {
    ...
    log << MSG::WARNING
        << "GeometryVersion already set to  <" << m_geometry_version << ">" << " resetting to <" << s << ">"
        << endmsg;
  }
...
void MYSQL::setLayoutName(std::string s)
{
  ...
  if (m_layout_name != "unknown") {
    ...
    log << MSG::WARNING
        << "LayoutName already set to  <" << m_layout_name << ">" << " resetting to <" << s << ">"
        << endmsg;
  }
  m_layout_name = s;
  log << MSG::INFO
      << "LayoutName (from DBAM) set to <" << m_layout_name << ">  -- relevant for CTB2004"
      << endmsg;
}
