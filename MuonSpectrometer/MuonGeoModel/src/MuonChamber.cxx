...
GeoVPhysVol* MuonChamber::build(
  MuonDetectorManager* manager, int zi,
  int fi, bool is_mirrored, bool& isAssembly)
{
  ...
  if (length <= 0) {
    log << MSG::ERROR
        << " Invalid length " << length << " for m_station "
  ...
  if (verbose) {
    log << MSG::VERBOSE
        << " MuonChamber size thick,w,lw,l "
        << totthick << ", " << width << ", " << longWidth << ", " << length
        << endmsg;
  }
  ...
  if ( (extratop + extrabottom) != 0.) {
    ...
    if (verbose) {
      log << MSG::VERBOSE
          << " m_station name " << m_station->GetName() << " extra top, bottom, dx = "
  ...
  if (verbose) {
    log << MSG::VERBOSE
        << "amdb origine: in the length direction = "
        << amdbOrigine_along_length << " in the thickness direction = "
        << amdbOrigine_along_thickness
        << endmsg;
  }
  ...
  if (isAssembly) {
    if (debug) {
      log << MSG::DEBUG
          << "Station  " << stName << " at zi, fi "
          << zi << " " << fi+1 << " will be described as  Assembly"
          << endmsg;
    }
  }
  ...
  if (stName.substr(0,3) == "BOG" &&
    ...
    if (verbose) {
      log << MSG::VERBOSE << "amdb org: length= "
    ...
    if (m_station->GetNrOfCutouts() >0 ) {
      if (debug) {
        log << MSG::DEBUG
            << "Station  " << stName << " at zi, fi " << zi
            << " " << fi+1 << " has components with cutouts "
            << endmsg;
      }
      isAssembly = true;
  ...
  if (verbose) {
    log << MSG::VERBOSE
        << " Station Name = " << stName << " fi/zi " << fi << "/" << zi
  ...
  if (debug){
    ...
    log << MSG::DEBUG
        << " nMdt/Rpc/Tgc/Csc " << nMdt << "/" << nRpc << "/" << nTgc << "/" << nCsc
        << endmsg;
  }
  ...
  for (int i = 0; i < m_station->GetNrOfComponents(); i++) {
    ...
    if (verbose) {
      log << MSG::VERBOSE
          ...
          << endmsg;
      log << MSG::VERBOSE
          << " Component local (amdb) coords " << c->posx << " " << c->posy << " " << c->posz
          << endmsg;
    }
    ...
    for (int ii = 0; ii < m_station->GetNrOfCutouts(); ii++) {
      ...
      if ((cut->subtype ==
        ...
        if (verbose) {
          log << MSG::VERBOSE
              << "A new cutout for this component "
              << endmsg;
    ...
    if (type == "MDT") {
      ...
      if (fpv == 0) {
        ...
        if (verbose) {
          log << MSG::VERBOSE << " This MDT for station "
              << key << " component name is "
              << c->name << " already exists; clone it "
              << endmsg;
        }
        lvm = rfpv->clone();
      }
      ...
      if (fpv == 0 || (stName.substr(0,3) == "BOG" && type == "CMI")) {
        if (stName.substr(0,3)=="BOG") {
          if (verbose) {
            log << MSG::VERBOSE << " Building a SpacerBeam for station "
                << key << " component name is " << c->name
                << endmsg;
          }
        }
      ...
      if (ndivz != 1 || ndivy != 1) {
        log << MSG::ERROR << " RPC segmentation z,y "
            << ndivz << " " << ndivy << endmsg;
      }
      ...
      if (verbose) {
        log << MSG::VERBOSE << " In station " << stName
    ...
    if (lvm && manager->mdtIdHelper()) {
      ...
      if (debug) {
        log << MSG::DEBUG << std::string(stName+techname)
    ...
    if (lvt && manager->tgcIdHelper()) {
      ...
      if (debug) {
        log << MSG::DEBUG
            << std::string(stName+techname)
            << " trying to build a TGC Id from stType/eta/phi "
            << stationType << "/"
            << stationEta << "/"
            << stationPhi << "/ and /1/0/1"
            << endmsg
            << " Copy number is " << geoid << " tagName = " << stag
            << endmsg;
      }
    ...
    if (lvr && RPCON && manager->rpcIdHelper()) {
      ...
      if (xpos > 400.*Gaudi::Units::mm) dbphi = 2;
...
void MuonChamber::setTgcReadoutGeom(TgcReadoutElement* re, const TgcComponent* cc,
                                    const Position& ip, std::string /*gVersion*/,
                                    std::string stName)
{
  ...
  if (ip.isAssigned) {
    ...
    log << MSG::ERROR
        <<" MuonChamber::setTgcReadoutGeom position not found "
        << endmsg;
    assert(0);
  }
  ...
  if (re->m_readoutParams == 0) {
    log << MSG::WARNING
        << " MuonChamber::setTgcReadoutGeometry: no readoutParams found for key <"
        << re->m_readout_name << ">"
        << endmsg;
  } else {
