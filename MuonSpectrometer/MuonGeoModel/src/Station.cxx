...
void Station::SetAlignPos(const AlignPos& p)
{
  if (FindAlignPos(p.zindex, p.phiindex) != m_alignpositions.end() && p.jobindex==0) {
    ...
    log << MSG::WARNING
        << " this alignposition already exists !!!"
        << endmsg;
    log << MSG::WARNING
        << " for station named " << m_name << " setting alignposition at z,phi, key " <<p.zindex << " "
        << p.phiindex << " " << p.zindex*100+p.phiindex << " and jobIndex = 0"
        << endmsg;
    assert(0);
  }
  ...
  if (m_name[0]=='T') {
    ...
    for (unsigned int i=0;i<m_components.size();i++) {
      ...
      if (log.level()<=MSG::VERBOSE) {
        log << MSG::VERBOSE
            << "Station " << m_name << " calculating  Thinkness = " << thick << " and zstart = " << zstart
            << endmsg;
      }
    }
    ...
    if (fabs(zstart) > 0.001) {
      ...
      if (log.level()<=MSG::VERBOSE) {
        log << MSG::VERBOSE
            << "Station " << m_name << " redefining Thinkness = " << thick << " because zstart = " << zstart
...
double Station::GetLength() const
{
  ...
  if (m_name[0] == 'T') {
    ...
    for (unsigned int i=0;i<m_components.size();i++) {
      ...
      if (log.level()<=MSG::VERBOSE) {
        log << MSG::VERBOSE
            << "Station " << m_name << " *** comp " << i << " named " << sc->name
            << " posy " << sc->posy << " dy " << sc->dy << " len " << len << " ystart " << ystart
            << endmsg;
      }
    ...
    if (fabs(ystart) > 0.001) {
      ...
      if (log.level()<=MSG::VERBOSE) {
        log << MSG::VERBOSE
            << "Station " << m_name << " redefining len = " << len << " because ystart = " << ystart
            << endmsg;
      }
    }
  }
...
GeoTrf::Transform3D Station::native_to_tsz_frame( const Position & p ) const {
  ...
  if ( (m_name[0]=='B' || p.isBarrelLike) && p.zindex<0 && (!p.isMirrored) && hasMdts()) {
    ...
    AMDBorgTranslation =
      GeoTrf::Translate3D(
        GetThickness()/2.-getAmdbOrigine_along_thickness(),
        0.,
    ...
    log << MSG::VERBOSE
        ...
        << GetLength() << " "
        << getAmdbOrigine_along_length()+halfpitch
        << endmsg;
  } else {
    if (m_name[0]=='T') {
      AMDBorgTranslation =
...
GeoTrf::Transform3D Station::tsz_to_global_frame( const Position & p ) const {
  ...
  MsgStream log(Athena::getMessageSvc(),"MuonGeoModel.Station");
  log << MSG::VERBOSE
      << " translation according to " << vec.x() << " " << vec.y() << " " << vec.z()
      << endmsg;
...
//         }
    if ( p.z>=0 ){
        ...
        nominalTransf =  GeoTrf::Transform3D(
...
GeoTrf::Transform3D Station::getDeltaTransform_tszFrame(const AlignPos & ap) const
{
  ...
  if (ap.tras!=0 ||ap.trat!= 0 ||ap.traz!=0 ||
    ...
    log << MSG::VERBOSE << "For station " << m_name
        << " corrections sent are " << ap.tras << " " << ap.traz << " " << ap.trat << " " << ap.rots
  ...
  log << MSG::VERBOSE
      << " delta transform in the tsz frame --------------"<<endmsg
      << delta(0,0) << " "
      << delta(0,1) << " "
      << delta(0,2) << " "
      << delta(0,3) << " "
