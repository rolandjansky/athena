...
MB_SegmentAth::MB_SegmentAth():MB_Segment(){}
MB_SegmentAth::MB_SegmentAth(
                             const MB_Segment & oMB_Segment,
                             const MuonRecDigiStoreAth* pMuonRecDigiStoreAth)
  :MB_Segment(oMB_Segment){
...
/** Get  Surface */
const Trk::Surface* MB_SegmentAth::LiIdGetSurface        (int LinearNber) const{
  const Trk::PrepRawData* pPrepRawData = LiIdGetPrepRawData(LinearNber); if ( pPrepRawData ) return &(pPrepRawData->detectorElement()->surface(pPrepRawData->identify())) ;
  return 0;
}

/** For a Linear Nber LinearNber get a typed Pointer */
const Trk::PrepRawData* MB_SegmentAth::LiIdGetPrepRawData(int LinearNber) const{
...
const Muon::MdtPrepData* MB_SegmentAth::LiIdGetMdtPrepData(int LinearNber) const{
  if (LinearNber < 0 || LinearNber >= GetNberOfElements()) return 0;
  return  MdtPrepDataVector[LinearNber];
}
const Muon::RpcPrepData* MB_SegmentAth::LiIdGetRpcPrepData(int LinearNber) const{
  if (LinearNber < 0 || LinearNber >= GetNberOfElements()) return 0;
  return  RpcPrepDataVector[LinearNber];
}
const Muon::TgcPrepData* MB_SegmentAth::LiIdGetTgcPrepData(int LinearNber) const{
  if (LinearNber < 0 || LinearNber >= GetNberOfElements()) return 0;
  return  TgcPrepDataVector[LinearNber];
}
const Muon::CscStripPrepData* MB_SegmentAth::LiIdGetCscStripPrepData(int LinearNber) const{
  if (LinearNber < 0 || LinearNber >= GetNberOfElements()) return 0;
  return  CscStripPrepDataVector[LinearNber];
}
const Muon::CscPrepData* MB_SegmentAth::LiIdGetCscClusterPrepData(int LinearNber) const{
  if (LinearNber < 0 || LinearNber >= GetNberOfElements()) return 0;
