...
MboyMuonSegmentMakerTester::MboyMuonSegmentMakerTester(const std::string& name, ISvcLocator* pSvcLocator) :
  AthAlgorithm(name, pSvcLocator),
 p_OwnEDMHelper                  ( "MboyAthToolHelper" ),
 p_IMboyGetPrepDataFromEventTool ( "MboyGetPrepDataFromEventTool" ) ,
 p_IMboyMuonSegmentMaker ( "Muon::MboyMuonSegmentMaker" ) 
{
  ...
  declareProperty("MboyMuonSegmentMaker" , p_IMboyMuonSegmentMaker ) ;

//Declare the properties

  declareProperty( "SegmentCollectionLocation"                , m_SegmentCollectionLocation                = "MboyMuonSegmentMakerTester.ConvertedMBoySegments"                ) ;

  declareProperty( "DoDump"                                   , m_DoDump                                   = 0                                      ) ;      
  declareProperty( "MboyDumpSegmentCollectionOutputFile"      , m_MboyDumpSegmentCollectionOutputFile      = "Out.MboyMuonSegmentMakerTester"        ) ;      

}
MboyMuonSegmentMakerTester::~MboyMuonSegmentMakerTester(){}


// Initialize
StatusCode MboyMuonSegmentMakerTester::initialize(){
...
//Retrieve p_IMboyMuonSegmentMaker
  if ( p_IMboyMuonSegmentMaker.retrieve().isFailure() ) {
    ATH_MSG_FATAL( "Failed to retrieve tool " << p_IMboyMuonSegmentMaker );
    return StatusCode::FAILURE;
  }
  ATH_MSG_INFO( "Retrieved tool " << p_IMboyMuonSegmentMaker );
...
// Execute
StatusCode MboyMuonSegmentMakerTester::execute(){
...
//Find Segments
  std::vector<const Muon::MuonSegment*>* pVectorOfMuonSegment = p_IMboyMuonSegmentMaker->findFromPrep(VectorOfpPrepData) ;
...
// Finalize
StatusCode MboyMuonSegmentMakerTester::finalize(){
...
void MboyMuonSegmentMakerTester::Kount(){
...
void MboyMuonSegmentMakerTester::PrintKount(){
...
void MboyMuonSegmentMakerTester::DumpContainers(){
