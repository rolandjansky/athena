...
StatusCode MDTCablingDbTool::loadMDTMap(IOVSVC_CALLBACK_ARGS_P(/*I*/,/*keys*/)) 
{
  ...
  if(m_mdtIdHelper->stationNameIndex("BMG") != -1 && !BMGchamberadded) {
    ...
    for(int i=0; i<9; i++) // ML2
       m_cablingData->addMezzanine(         60, stationIndex,   2,   6,          2,     4,(i*6)+1,            97,   50,   1,   2*i,         0);
    // BMG3A12 ---------------- mezzanine_type, stationIndex, eta, phi, multilayer, layer,   tube, subdetectorId, mrod, csm, tdcId, channelId
    for(int i=0; i<9; i++) // ML1
       m_cablingData->addMezzanine(         60, stationIndex,   3,   6,          1,     4,(i*6)+1,            97,   50,   2, 2*i+1,         0);
    for(int i=0; i<9; i++) // ML2
       m_cablingData->addMezzanine(         60, stationIndex,   3,   6,          2,     4,(i*6)+1,            97,   50,   2,   2*i,         0);
    ...
    for(int i=0; i<9; i++) // ML1
       m_cablingData->addMezzanine(         60, stationIndex,  -1,   7,          1,     4,(i*6)+1,            98,   51,   0, 2*i+1,         0);
    for(int i=0; i<9; i++) // ML2
       m_cablingData->addMezzanine(         60, stationIndex,  -1,   7,          2,     4,(i*6)+1,            98,   51,   0,   2*i,         0);
    // BMG2C14 ---------------- mezzanine_type, stationIndex, eta, phi, multilayer, layer,   tube, subdetectorId, mrod, csm, tdcId, channelId
    for(int i=0; i<9; i++) // ML1
       m_cablingData->addMezzanine(         60, stationIndex,  -2,   7,          1,     4,(i*6)+1,            98,   51,   1, 2*i+1,         0);
    for(int i=0; i<9; i++) // ML2
       m_cablingData->addMezzanine(         60, stationIndex,  -2,   7,          2,     4,(i*6)+1,            98,   51,   1,   2*i,         0);
    // BMG3C14 ---------------- mezzanine_type, stationIndex, eta, phi, multilayer, layer,   tube, subdetectorId, mrod, csm, tdcId, channelId
    for(int i=0; i<9; i++) // ML1
       m_cablingData->addMezzanine(         60, stationIndex,  -3,   7,          1,     4,(i*6)+1,            98,   51,   2, 2*i+1,         0);
    for(int i=0; i<9; i++) // ML2
       m_cablingData->addMezzanine(         60, stationIndex,  -3,   7,          2,     4,(i*6)+1,            98,   51,   2,   2*i,         0);
  }
