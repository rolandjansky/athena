...
StatusCode RpcCablingCondAlg::setup(const CondAttrListCollection* readCdoMap, const CondAttrListCollection* readCdoCorr, const CondAttrListCollection* readCdoEta, const CondAttrListCollection* readCdoPhi, RpcCablingCondData* writeCdo) const {
  ...
  while (++data) {	
    ...
    if(stop == 63 || stop == 8)
    {   
      for(int i=0;i<64;++i) if(sectorMap[i] > maxType) maxType = sectorMap[i];
    ...
    if (sectorMap[sector]) {
      ...
      while(it != CMAs.end())
      {
        ...
        if( (*it).second.get_cabling(CMAinput::Pivot,0,0,0,cabling) ) {
          ...
          if(!ins.second) {
            ATH_MSG_ERROR("RpcCablingCondData::RDOmap is false for stationName="<<name<<", stationEta="<<sEta<<", stationPhi="<<sPhi<<", doubletR="<<dR<<", doubletZ="<<dZ<<", doubletPhi="<<dP);
            return StatusCode::FAILURE;
          }
        ...
        else if(!(*it).second.get_cabling(CMAinput::Pivot,0,0,0,cabling) && isFirst)
        {
          ...
          if(existLow) RPC_station = (*it).second.whichCMAstation(CMAinput::LowPt);
          else         RPC_station = (*it).second.whichCMAstation(CMAinput::HighPt);
          unsigned int lvl1_sector = sector;
  ...
  while (++data_corr) {
      ...
      if(data_corr("CABLE") >> data_corr.dbhex() >> SubId >> SecId >> PADId >> CMAId >> ijk >> data_corr.dbdec() >> type >> Channel1 >> Channel2 >> Number) {
          ...
          if(ok && ! correct (SubId,SecId,PADId,CMAId,static_cast<CMAinput>(inputType),layer,Channel1,Channel2,Number,static_cast<L1RPCcabCorrection>(type), sectorMap,sectorType)) {
              ATH_MSG_WARNING("Cannot apply correction: " << std::hex << std::setw(4) << std::showbase << SubId << " "
      ...
      if(data_corr("BOARD") >> data_corr.dbhex() >> SubId >> SecId >> PADId >> CMAId >> data_corr.dbdec() >> inputType  >> layer >> type >> Channel1 >> Channel2 >> Number) {
  ...
  for(int side=0;side<2;++side) {
    for(int rod=0;rod<16;++rod) {
      (writeCdo->m_rod2hash[side][rod]).clear();
    }
  }
  ...
  for (; pad_beg!=pad_end; ++pad_beg) {
    ...
    ATH_MSG_DEBUG(hashID<<"-th entry has sub_id_index="<<sub_id_index<<", ROBid="<<rob_id<<", RODid="<<rod_id<<", side="<<sub_id<<", SLid="<<sec_id<<", PADid="<<pad_id<<", pad_identifier="<<id.get_compact());
    ...
    ATH_MSG_DEBUG("OfflineOnlineMap new entry: Identifier with technology="<<m_idHelperSvc->rpcIdHelper().technology(id)<<", stationName="<<m_idHelperSvc->rpcIdHelper().stationName(id)<<", stationEta="<<m_idHelperSvc->rpcIdHelper().stationEta(id)<<", stationPhi="<<m_idHelperSvc->rpcIdHelper().stationPhi(id)<<", doubletR="<<m_idHelperSvc->rpcIdHelper().doubletR(id)<<", doubletZ="<<m_idHelperSvc->rpcIdHelper().doubletZ(id)<<", doubletPhi="<<m_idHelperSvc->rpcIdHelper().doubletPhi(id)<<" and hash of the RDOindex(key)= "<<pRDOindex->hash());
    if(!ins.second) {
      ATH_MSG_ERROR("RpcCablingCondData::OfflineOnlineMap is false for technology="<<m_idHelperSvc->rpcIdHelper().technology(id)<<", stationName="<<m_idHelperSvc->rpcIdHelper().stationName(id)<<", stationEta="<<m_idHelperSvc->rpcIdHelper().stationEta(id)<<", stationPhi="<<m_idHelperSvc->rpcIdHelper().stationPhi(id)<<", doubletR="<<m_idHelperSvc->rpcIdHelper().doubletR(id)<<", doubletZ="<<m_idHelperSvc->rpcIdHelper().doubletZ(id)<<", doubletPhi="<<m_idHelperSvc->rpcIdHelper().doubletPhi(id)<<" and hash of the RDOindex(key)= "<<pRDOindex->hash());
      return StatusCode::FAILURE;
    }
    ...
    ATH_MSG_DEBUG("RDO HashId = " << (int)rdoHashId << " RDO Id = " << id.get_compact()
    ...
    for (unsigned short int CMAId : {2, 3, 6 ,7}) { //loop over phi CMA IDs
      for (unsigned short int ijk : {1, 2, 3}) { //loop over IJK identifiers
        ...
        for (unsigned short int channel : {0, 31}) {//check for the first and the last channel
          strip_id_list.splice( strip_id_list.end(), give_strip_id(sub_id_index, sec_id, pad_id, CMAId, ijk, channel,sectorMap,sectorType) );
        }
  ...
  if (m_ApplyFeetPadThresholds) {
    ...
    ATH_MSG_INFO("Applying FeetPadThresholds : "
      <<  feedPadThresholds.at(0) << ","
      <<  feedPadThresholds.at(1) << ","
      <<  feedPadThresholds.at(2));
    ...
    for (unsigned int is=0; is<NumFeetSectors; is++) {
      for (unsigned int it=0; it<NumSpecialFeetPads; it++) {
        ...
        for (unsigned int th=0; th<3; th++) {
          writeCdo->m_RPCPadParameters_array[FeetSectors[is]][SpecialFeetPads[it]].set_feet_threshold(th,feedPadThresholds.at(th));
        }
      }
    }
  }
