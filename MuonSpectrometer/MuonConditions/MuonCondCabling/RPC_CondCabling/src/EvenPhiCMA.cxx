...
void
EvenPhiCMA::get_confirm_strip_boundaries(int stat,int max)
{
...
int
EvenPhiCMA::get_max_strip_readout(int stat)
{
    int max = 0;
...
bool
EvenPhiCMA::setup(SectorLogicSetup& setup)
{
    m_msgSvc = Athena::getMessageSvc();
    MsgStream log(m_msgSvc, name());
    m_debug = log.level() <= MSG::DEBUG;
...
bool
EvenPhiCMA::doInversion(SectorLogicSetup& setup)
{
    SectorLogicSetup::SECTORlist Sectors = setup.sectors();
    SectorLogicSetup::SECTORlist::const_iterator it = Sectors.begin();
    int sector = *it;
