...
MuonConditionsHistoSummary::MuonConditionsHistoSummary(const std::string& name, ISvcLocator* pSvcLocator) :
...
MuonConditionsHistoSummary::~MuonConditionsHistoSummary() = default;

// Initialize
StatusCode MuonConditionsHistoSummary::initialize() {
...
// Execute
StatusCode MuonConditionsHistoSummary::execute() {
    StatusCode sc(StatusCode::SUCCESS);
    ...
    if (m_pSummarySvc->deadStations().size() == m_pSummarySvc->deadStationsId().size()) {
        msg(MSG::INFO) << "id normalcomparionssuccessful" << endmsg;
    ...
    if (m_pSummarySvc->deadMultiLayers().size() == m_pSummarySvc->deadMultiLayersId().size()) {
        msg(MSG::INFO) << "id normalcomparionssuccessful" << endmsg;
    ...
    if (m_pSummarySvc->deadTubes().size() == m_pSummarySvc->deadTubesId().size()) {
        msg(MSG::INFO) << "id normalcomparionssuccessful" << endmsg;
