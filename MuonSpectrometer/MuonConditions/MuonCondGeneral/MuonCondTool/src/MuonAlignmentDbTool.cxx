...
StatusCode MuonAlignmentDbTool::loadAlignILines(std::string folderName) 
{
   ...
   for (itr = atrc->begin(); itr != atrc->end(); ++itr)
   {
	...
	if (type.find("Corr")==0) {
	  ...
	  int stationName = (&(m_idHelperTool->cscIdHelper())) ? m_idHelperTool->cscIdHelper().stationNameIndex(stationType) : 0;
	  Identifier id;
	  if( m_verbose ) log << MSG::VERBOSE<< "stationName  " << stationName << endmsg;
	  if ((&(m_idHelperTool->cscIdHelper())) && (stationType.substr(0,1)=="C"))
	  {
              // csc case
              int chamberLayer = 2;
