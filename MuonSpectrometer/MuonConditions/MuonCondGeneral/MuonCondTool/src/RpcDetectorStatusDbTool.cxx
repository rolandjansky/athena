...
StatusCode RpcDetectorStatusDbTool::updateAddress(StoreID::type /*storeID*/, SG::TransientAddress* /*tad*/, const EventContext& /*ctx*/) {
...
StatusCode RpcDetectorStatusDbTool::loadParameterStatus(IOVSVC_CALLBACK_ARGS_P(I, keys)) {
    ...
    for (itr = keys.begin(); itr != keys.end(); ++itr) {
        ...
        if (*itr == m_FolderName) { sc &= loadRpcDqStatus(I, keys); }
    }
...
StatusCode RpcDetectorStatusDbTool::loadRpcDqStatus(IOVSVC_CALLBACK_ARGS_P(I, keys)) {
    ...
    if (sc.isFailure()) {
        m_log << MSG::ERROR << "could not retreive the CondAttrListCollection from DB folder " << m_FolderName << endmsg;
        return sc;
    }
    ...
    for (itr = atrc->begin(); itr != atrc->end(); ++itr) {
        ...
        if (m_debug) {
            m_log << MSG::DEBUG << "-----------------------------entry #" << iEntry << " total # of entries in the folder =" << atrc->size()
                  << endmsg;
            m_log << MSG::DEBUG << "channel ID = Panel ID " << channum << " as identif. = " << m_rpcIdHelper->show_to_string(chamberId)
                  << endmsg;
        ...
        if (m_FirstCall)
            m_log << MSG::INFO << " DB version  " << info_panel[0] << " Size of info_panel array = " << info_panel.size() << endmsg;
        ...
        if (m_debug)
            m_log << MSG::DEBUG << "Size of  RPC_PanelProjectedTracksMap " << m_RPC_PanelProjectedTracksMap.size() << "; in panel "
                  << channum << " SProjectedTracks " << info_panel[1] << " ProjectedTracks " << ProjectedTracks << endmsg;
        ...
        if (m_debug)
            m_log << MSG::DEBUG << "Size of  RPC_PanelEfficiencyMap " << m_RPC_PanelEfficiencyMap.size() << "; in panel " << channum
                  << " SEfficiency " << info_panel[3] << " Efficiency " << Efficiency << endmsg;
        ...
        if (m_debug)
            m_log << MSG::DEBUG << "Size of  RPC_GapEfficiencyMap " << m_RPC_GapEfficiencyMap.size() << "; in panel " << channum
                  << " SGapEfficiency " << info_panel[5] << " GapEfficiency " << GapEfficiency << endmsg;
        ...
        if (m_debug)
            m_log << MSG::DEBUG << "Size of  RPC_PanelMeanClusterSizeMap " << m_RPC_PanelMeanClusterSizeMap.size() << "; in panel "
                  << channum << " SMeanClusterSizeMap " << info_panel[17] << " MeanClusterSizeMap " << MeanClusterSizeMap << endmsg;
        if (DBversion > 2) {
            double FracClusterSize1Map = MdtStringUtils::stof(info_panel[19]) + MdtStringUtils::stof(info_panel[20]) * 10000;
            ...
            if (m_debug)
                m_log << MSG::DEBUG << "Size of  RPC_PanelFracClusterSize1Map " << m_RPC_PanelFracClusterSize1Map.size() << "; in panel "
                      << channum << " SFracClusterSize1Map_a " << info_panel[19] << " SFracClusterSize1Map_b " << info_panel[20]
                      << " FracClusterSize1Map " << FracClusterSize1Map << endmsg;

            double FracClusterSize2Map = MdtStringUtils::stof(info_panel[21]) + MdtStringUtils::stof(info_panel[22]) * 10000;
            ...
            if (m_debug)
                m_log << MSG::DEBUG << "Size of  RPC_PanelFracClusterSize2Map " << m_RPC_PanelFracClusterSize2Map.size() << "; in panel "
                      << channum << " SFracClusterSize2Map_a " << info_panel[21] << " SFracClusterSize2Map_b " << info_panel[22]
                      << " FracClusterSize2Map " << FracClusterSize2Map << endmsg;

            double FracClusterSize3Map = MdtStringUtils::stof(info_panel[23]) + MdtStringUtils::stof(info_panel[24]) * 10000;
            ...
            if (m_debug)
                m_log << MSG::DEBUG << "Size of  RPC_PanelFracClusterSize3Map " << m_RPC_PanelFracClusterSize3Map.size() << "; in panel "
                      << channum << " SFracClusterSize3Map_a " << info_panel[23] << " SFracClusterSize3Map_b " << info_panel[24]
            ...
            if (info_panel.size() > 20) {
                ...
                if (m_debug)
                    m_log << MSG::DEBUG << "Size of  RPC_PanelFracClusterSize1Map " << m_RPC_PanelFracClusterSize1Map.size()
                          << "; in panel " << channum << " SFracClusterSize1Map " << info_panel[19] << " FracClusterSize1Map "
                          << FracClusterSize1Map << endmsg;
                ...
                if (m_debug)
                    m_log << MSG::DEBUG << "Size of  RPC_PanelFracClusterSize2Map " << m_RPC_PanelFracClusterSize2Map.size()
                          << "; in panel " << channum << " SFracClusterSize2Map " << info_panel[20] << " FracClusterSize2Map "
                          << FracClusterSize2Map << endmsg;
            } else {
                ...
                m_log << MSG::DEBUG << "Here a panel with incomplete info in the DB, size = " << info_panel.size()
                      << " instead of being>20 panel ID=" << channum << " = " << m_rpcIdHelper->show_to_string(chamberId)
        ...
        if (m_debug)
            m_log << MSG::DEBUG << "strip list " << striplist << " info list " << info_strip[0] << " " << info_strip.size() << " del "
        ...
        if (info_strip.size() > 1) {
            ...
            for (unsigned int i = 0; i < info_strip.size(); i++) {
                ...
                if (m_verbose)
                    m_log << MSG::VERBOSE << "strip # " << i << " after dec: Status+Timing INFO (time, error) " << strip_status << " "
                          << Time << "+/-" << SigmaTime << endmsg;
                ...
                if (m_verbose)
                    m_log << MSG::VERBOSE << "strip #" << i + 1 << " strip_id " << stripnum << " expanded "
                ...
                if (strip_status[0] == '0') {
                    if (m_debug && (!m_verbose))
                        m_log << MSG::DEBUG << "strip #" << i + 1 << " info_strip " << strip_status[0] << " strip_id " << stripnum
                              << " expanded " << m_rpcIdHelper->show_to_string(strip_id) << endmsg;
                    // m_RPC_PanelDeadStripList.insert(std::make_pair(strip_id,1));
                    m_RPC_PanelDeadStripList[strip_id] = 1;
                    countdeadstrip++;
                    if (m_debug) {
                        m_log << MSG::DEBUG << "Size of RPC_PanelDeadStripList (full det) " << m_RPC_PanelDeadStripList.size()
                              << " in this panel " << countdeadstrip << " for " << m_rpcIdHelper->show_to_string(strip_id) << endmsg;
                    }
                    if (i > 1 && i < info_strip.size() - 2) {
                        ...
                        if (m_debug) {
                            m_log << MSG::DEBUG << "dead strips n. " << i + 1 << " is at the border - not used for deadFrac of "
                                  << m_rpcIdHelper->show_to_string(strip_id) << endmsg;
                        }
                    }
                }
            ...
            for (unsigned int i = 0; i < striplist.size(); i++) {
                ...
                if (m_verbose)
                    m_log << MSG::VERBOSE << "strip #" << i + 1 << " info_strip " << part_strip << " strip_id " << stripnum << " expanded "
                          << m_rpcIdHelper->show_to_string(strip_id) << " panel = " << h_panel << endmsg;

                countpanelstrip++;

                if (part_strip == '0') {
                    if (m_debug && (!m_verbose))
                        m_log << MSG::DEBUG << "strip #" << i + 1 << " info_strip " << part_strip << " strip_id " << stripnum
                              << " expanded " << m_rpcIdHelper->show_to_string(strip_id) << endmsg;
                    countdeadstrip++;
                    // m_RPC_PanelDeadStripList.insert(std::make_pair(strip_id,1));
                    m_RPC_PanelDeadStripList[strip_id] = 1;
        ...
        if (countpanelstrip != npanelstrip)
            m_log << MSG::WARNING
                  ...
                  << countpanelstrip << " != " << npanelstrip << " Identifier: " << channum << endmsg;
        ...
        if (m_debug) {
            m_log << MSG::DEBUG << "Size of RPC_PanelFracDeadStripMap " << m_RPC_PanelFracDeadStripMap.size() << "; in panel " << channum
                  << " FracDeadStri(in fid.area) " << FracDeadStripMap;
            if (countpanelstrip == 0)
                m_log << " (incl. borders) DIVISION BY ZERO IMPOSSIBLE";
            else
                m_log << " (incl. borders) " << float(countdeadstrip) / float(countpanelstrip);
            ...
            if (Efficiency < 0.995 || FracDeadStripMap > 0.01) {
                if (Efficiency > 0.005 || FracDeadStripMap < 0.99) {
                    m_log << MSG::DEBUG << "WARNING: Inconsistent panel eff.=" << Efficiency
                          << " and 0.99-dead_frac=" << 0.99 - FracDeadStripMap << " nDeadStrips,InFidArea/nStrips " << countdeadstrip << ","
        ...
        if (Efficiency <= m_RPC_PanelEfficiency) { m_effPanelId.push_back(chamberId); }
