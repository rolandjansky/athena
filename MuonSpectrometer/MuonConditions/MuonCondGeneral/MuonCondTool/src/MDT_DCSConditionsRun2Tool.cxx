...
StatusCode MDT_DCSConditionsRun2Tool::updateAddress(StoreID::type /*storeID*/, SG::TransientAddress* /*tad*/, const EventContext& /*ctx*/) {
...
StatusCode MDT_DCSConditionsRun2Tool::loadHV(IOVSVC_CALLBACK_ARGS_P(I, keys)) {
    ...
    if (sc.isFailure()) {
        m_log << MSG::ERROR << "could not retreive the CondAttrListCollection from DB folder " << m_hvFolder << endmsg;
        return sc;
    }
    ...
    for (itr = atrc->begin(); itr != atrc->end(); ++itr) {
        ...
        if (atr.size()) {
            ...
            if (hv_name_ml1 != "ON" && hv_name_ml1 != "STANDBY" && hv_name_ml1 != "UNKNOWN") {
                if (m_verbose)
                    m_log << MSG::VERBOSE << "NOT ON and NOT STANDBY HV : " << hv_name_ml1 << " ChamberName : " << tokens2[0]
                          << "multilayer 1" << endmsg;
                //	m_log << MSG::INFO << "NOT ON and NOT STANDBY HV : " <<hv_name_ml1  << " ChamberName : "<<tokens2[0] << "multilayer
                // 1"  <<endmsg; 	m_cachedDeadMultiLayers.push_back(1);
                int multilayer = 1;
            ...
            if (hv_name_ml1 == "STANDBY" && hv_v0_ml1 != hv_v1_ml1) {
                if (m_verbose)
                    m_log << MSG::VERBOSE << "STANDBY HV : " << hv_name_ml1 << " ChamberName : " << tokens2[0] << "multilayer 1" << endmsg;
            ...
            if (hv_name_ml2 != "ON" && hv_name_ml2 != "STANDBY" && hv_name_ml2 != "UNKNOWN") {
                if (m_verbose)
                    m_log << MSG::VERBOSE << "NOT ON and NOT STANDBY HV : " << hv_name_ml2 << " ChamberName : " << tokens2[0]
                          << "multilayer 2" << endmsg;
                // m_log << MSG::INFO << "NOT ON and NOT STANDBY HV : " <<hv_name_ml2  << " ChamberName : "<<tokens2[0] << "multilayer 2"
                // <<endmsg; m_cachedDeadMultiLayers.push_back(2);
                int multilayer = 1;
                const std::string& chamber_name = tokens2[0];
                Identifier ChamberId = m_condMapTool->ConvertToOffline(chamber_name);
                Identifier MultiLayerId = m_mdtIdHelper->channelID(ChamberId, multilayer, 1, 1);
            ...
            if (hv_name_ml2 == "STANDBY" && hv_v0_ml2 != hv_v1_ml2) {
                if (m_verbose)
                    m_log << MSG::VERBOSE << "STANDBY HV : " << hv_name_ml2 << " ChamberName : " << tokens2[0] << "multilayer 2" << endmsg;
...
StatusCode MDT_DCSConditionsRun2Tool::loadLV(IOVSVC_CALLBACK_ARGS_P(I, keys)) {
    ...
    if (sc.isFailure()) {
        m_log << MSG::ERROR << "could not retreive the CondAttrListCollection from DB folder " << m_lvFolder << endmsg;
        return sc;
    }
