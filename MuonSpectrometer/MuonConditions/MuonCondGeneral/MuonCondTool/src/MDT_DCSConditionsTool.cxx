...
StatusCode MDT_DCSConditionsTool::initialize()
{
  ...
  ATH_MSG_INFO( "Initializing - folders names are: ChamberDropped "<<m_dropchamberFolder <<" / LV "<<m_lvFolder<< " / HV "<<m_hvFolder);
...
StatusCode MDT_DCSConditionsTool::loadHV(IOVSVC_CALLBACK_ARGS_P(I,keys))
{
  ...
  for (itr = atrc->begin(); itr != atrc->end(); ++itr){
	...
	ATH_MSG_VERBOSE( "NOT ON and NOT STANDBY HV : " << tokens[0]<< " ChamberName : "<<tokens2[2] << "multilayer" << tokens2[3]);	
  ...
  for (unsigned int vect=0; vect<m_cachedDeadMultiLayersId_standby.size(); vect++){
    ...
    if (m_ChamberML_V1.find(MultilayerId_ch)->second == m_ChamberML_V0.find(MultilayerId_ch)->second){
     ...
     ATH_MSG_DEBUG(" Chamber has  correct Voltage V1 = "<< m_ChamberML_V1.find(MultilayerId_ch)->second<<" V0=   " << m_ChamberML_V0.find(MultilayerId_ch)->second ); 
    }else{
     ATH_MSG_DEBUG(" Chamber has  wrong correct Voltage V1 = "<< m_ChamberML_V1.find(MultilayerId_ch)->second<<" V0=   " << m_ChamberML_V0.find(MultilayerId_ch)->second );
...
StatusCode MDT_DCSConditionsTool::loadLV(IOVSVC_CALLBACK_ARGS_P(I,keys))
{
  ...
  //finally remove duplicates
   ATH_MSG_VERBOSE( "Now merging the  DeadStations with DeadLVStations" <<  endmsg;
  ...
  //finally remove duplicates
   ...
   ATH_MSG_VERBOSE( "Now merging the  DeadStationsId with DeadLVStationsId" <<  endmsg;
...
StatusCode MDT_DCSConditionsTool::loadJTAG(IOVSVC_CALLBACK_ARGS_P(I,keys))
{
  ...
  ATH_MSG_DEBUG( "Level " << I << " Keys: "<<sstr);
