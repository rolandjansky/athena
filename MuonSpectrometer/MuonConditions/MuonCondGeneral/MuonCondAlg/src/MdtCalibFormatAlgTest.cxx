...
// destructor
MdtCalibFormatAlgTest::~MdtCalibFormatAlgTest() { ATH_MSG_VERBOSE("Calling destructor"); }

// initialize
StatusCode MdtCalibFormatAlgTest::initialize() {
...
// execute
StatusCode MdtCalibFormatAlgTest::execute(const EventContext& ctx) const {
    ...
    // postprocess
    ATH_MSG_INFO("Retrieving time for (RT, old) = "
                 << (std::chrono::duration_cast<std::chrono::microseconds>(retrieving_RT_old).count() * 1.0) << "s ");
    ATH_MSG_INFO("Retrieving time for (RT, new) = "
                 << (std::chrono::duration_cast<std::chrono::microseconds>(retrieving_RT_new).count() * 1.0) << "s ");
    ATH_MSG_INFO("Retrieving time for (T0, old) = "
                 << (std::chrono::duration_cast<std::chrono::microseconds>(retrieving_T0_old).count() * 1.0) << "s ");
    ATH_MSG_INFO("Retrieving time for (T0, new) = "
                 << (std::chrono::duration_cast<std::chrono::microseconds>(retrieving_T0_new).count() * 1.0) << "s ");
...
// finalize
StatusCode MdtCalibFormatAlgTest::finalize() {
...
StatusCode MdtCalibFormatAlgTest::retrieve(const EventContext& ctx, const std::string& folder, const std::string& setup,
    ...
    if (setup == "new" && folder == "T0")
        readKey = m_readKey_T0_new;
    else if (setup == "new" && folder == "RT")
        readKey = m_readKey_RT_new;
    else if (setup == "old" && folder == "T0")
        readKey = m_readKey_T0_old;
