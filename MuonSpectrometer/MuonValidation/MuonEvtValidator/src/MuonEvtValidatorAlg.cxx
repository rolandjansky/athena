...
void MuonEvtValidatorAlg::createSubAlg(std::string typeName, std::string assignedName)
{
  Algorithm* temp;
  StatusCode sc;
...
MuonEvtValidatorAlg::MuonEvtValidatorAlg(const std::string& name, ISvcLocator* pSvcLocator) :
  Algorithm(name, pSvcLocator), log( msgSvc(), name )
{
  log << MSG::INFO << "++++++++++++  MuonEvtValidatorAlg created  ++++++++++++"
      << endreq;
...
MuonEvtValidatorAlg::~MuonEvtValidatorAlg()
{
  log << MSG::INFO << " deleting MuonEvtValidatorAlg " << endreq;
}

StatusCode MuonEvtValidatorAlg::initialize()
{
...
StatusCode MuonEvtValidatorAlg::finalize()
{
  log << MSG::INFO << " finalize " << endreq;
  return StatusCode::SUCCESS;
}

StatusCode MuonEvtValidatorAlg::execute()
{
  log << MSG::INFO << " execute " << endreq;
	
  int nAlgs=algoSuite.size();
	
  for (int i=0;i<nAlgs;i++)	algoSuite[i]->execute();
