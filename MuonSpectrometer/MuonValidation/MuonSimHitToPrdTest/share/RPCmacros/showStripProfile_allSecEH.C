...
void showStripProfile_allSecEH(int file=999, int sector=999, int eta=999, int stName=999, int dbZ=999, int dbP=999)
{
  ...
  //  if (file==0)      _file = TFile::Open("trackChanges/RPCntuple_ConfFileName_type7_12_16_17_8_14_11_15_v004_corr009_newGEOreSim_Will.root");
  // if (file==0)        _file = TFile::Open("trackChanges/RPCntuple_ConfFileName_type7_12_16_17_8_14_11_15_v005GAB_corr009_newGEOreSim_Will.root");

  //  if (file==0)        _file = TFile::Open("trackChanges/RPCntuple_ConfFileName_type7_12_16_17_8_14_11_15_v007_corr009_newGEOreSim_Will.root");
  //  if (file==0)        _file = TFile::Open("trackChanges/RPCntuple_25kFeet_conf008corr010.root");
  if (file==0)        _file = TFile::Open("tmp/RPCntuple_v010.root");
  else if (file==1)   _file = TFile::Open("trackChanges/RPCntuple_25kFeet_conf007corr009.root");
  else if (file==10)  _file = TFile::Open("trackChanges/RPCntuple_100kBarrel_conf008corr010.root");
  else if (file==101) _file = TFile::Open("trackChanges/RPCntuple_100kBarrel_conf009corr010.root");
  else if (file==201) _file = TFile::Open("trackChanges/RPCntuple_100kBarrel_conf009corr010_sqlite.root");
  else if (file==11)  _file = TFile::Open("trackChanges/RPCntuple_100kBarrel_conf007corr009.root");
  else if (file==1)   _file = TFile::Open("trackChanges/RPCntuple_17tobereleased_006.root");
  else if (file==2)   _file = TFile::Open("trackChanges/RPCntuple_17tobereleased_000.root");
  ...
  else if (file==12)  _file = TFile::Open("dev/RPCntuple_run1_devrel1_29Jul.root");
  else if (file==100) _file = TFile::Open("dev/RPCntuple_run2_complete_devrel1_29Jul.root");
  else if (file==1001) _file = TFile::Open("/afs/cern.ch/work/s/stefspa/RPCntuple_GeantinoHits_test125k_condRunDESDM_MCP25ns_sqliteWithTimingInCOOL.root");
  ...
  t002->Project("phiViewD1","rpc_digit_strip",
	     cutDigitPhi1.c_str());
  t002->Project("phiViewP1","rpc_prd_strip",
	     cutPrdPhi1.c_str());
  t002->Project("phiViewD2","rpc_digit_strip",
	     cutDigitPhi2.c_str());
  t002->Project("phiViewP2","rpc_prd_strip",
	     cutPrdPhi2.c_str());
  ...
  t002->Project("etaViewD1","rpc_digit_strip",
	     cutDigitEta1.c_str());
  t002->Project("etaViewP1","rpc_prd_strip",
	     cutPrdEta1.c_str());
  t002->Project("etaViewD2","rpc_digit_strip",
	     cutDigitEta2.c_str());
  t002->Project("etaViewP2","rpc_prd_strip",
	     cutPrdEta2.c_str());
  ...
  t002->Project("XYviewDigits","rpc_digit_stripy:rpc_digit_stripx",
		cutDigitPhi.c_str());
  t002->Project("RZviewDigits","sqrt(rpc_digit_stripy*rpc_digit_stripy+rpc_digit_stripx*rpc_digit_stripx):rpc_digit_stripz",
		cutDigitEta.c_str());
  t002->Project("XYviewPrd","rpc_prd_stripy:rpc_prd_stripx",
		cutPrdPhi.c_str());
  t002->Project("RZviewPrd","sqrt(rpc_prd_stripy*rpc_prd_stripy+rpc_prd_stripx*rpc_prd_stripx):rpc_prd_stripz",
		cutPrdEta.c_str());
  t002->Project("XYviewHits","rpc_simhit_globaly:rpc_simhit_globalx",
		cutSimhitPhi.c_str());
