...
void plotRPClayer(Int_t inputLayer) {
    ...
    for (; it != myPanList->end(); ++it) {
        ...
        if (stName == "BOL" && layerRPC > 4) {
            if (stringId.substr(0, 7) == "[7.4.-8" || stringId.substr(0, 7) == "[7.4.8.") { t->SetLineColor(kGreen); }
        }
        if (stName == "BOF") t->SetLineColor(kRed);
        if (stName == "BME") t->SetLineColor(kGreen);

        std::cout << layerRPC << " " << stName << " " << viewString << " " << stringId << " " << pid << " " << etamin << " " << etamax
                  << " " << phimin << " " << phimax << " " << zmin << " " << zmax << std::endl;
    ...
    ifstream infileData;
    infileData.open(
        "ID_Ntrack_HitsOnTrack_GapOnTrack_MergedHistograms.data15_13TeV.00281411.physics_Main.DESDM_MCP.f629.FullRpcMonitoring_v10.txt");
    ifstream infileMC;
...
void plotRPClayer() { return plotRPClayer(0); }
