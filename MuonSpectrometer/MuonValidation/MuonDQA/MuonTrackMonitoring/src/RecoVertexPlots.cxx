...
void RecoVertexPlots::initializePlots(){
    ...
    m_nMDT = Book1D("m_MSVx_nMDT", "m MSVx nMDT: M_MSVx MDT Hits;;N_{MDT}", 300,  0.0,  3000.);
    m_nRPC = Book1D("m_MSVx_nRPC", "m MSVx nRPC: M_MSVx RPC Hits;;N_{RPC}", 100,  0.0,  1000.);
    m_nTGC = Book1D("m_MSVx_nTGC", "m MSVx nTGC: M_MSVx TGC Hits;;N_{TGC}", 100,  0.0,  1000.);

    m_nTracklets = Book1D("m_MSVx_nTracklets", "MSVx nTracklets: M_MSVx Number of Tracklets; N_{trackletS}", 20,  0.0,  20.);   
    m_VertexEtaPhi   = Book2D("m_VertexEtaPhi", "Vertex Eta Phi: M_MSVx #eta vs. #phi; #eta; #phi", 64, -3.2, 3.2, 64, -3.2, 3.2);    
