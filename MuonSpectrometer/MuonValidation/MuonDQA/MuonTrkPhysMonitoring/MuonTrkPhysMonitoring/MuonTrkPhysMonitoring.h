...
class MuonTrkPhysMonitoring : public ManagedMonitorToolBase
{
 ...
 protected:
  ...
  void fillHistograms_ZSignal(std::vector<std::pair<const xAOD::Muon*, const xAOD::Muon*> > mumucandidates);
  void fillTagAndProbe(const xAOD::Muon* tag, const xAOD::Muon* probe);
 ...
 private:
  ...
  std::vector<ZSignalPlots*> m_oZSignalPlots;
