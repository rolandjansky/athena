...
StatusCode MuonTrkPhysMonitoring::bookHistograms()
{
  ...
  if(m_environment == AthenaMonManager::tier0    || 
    ...
    if(newRun){
		...
		m_oZSignalPlots.push_back(new ZSignalPlots(0, muonMon_path));
		...
		if(endOfRun){
            ...
            if(m_environment != AthenaMonManager::online){
                ATH_MSG_DEBUG("Processing Histograms for procHistograms_ZSignal");
  				for (auto plots : m_oZSignalPlots){
                    delete plots;
                }
			}
		}
...
bool MuonTrkPhysMonitoring::handleMuonContainer(){
    ...
    // courtesy of MCP. where do we input a mass window?
    ...
    fillHistograms_ZSignal(Zresonances);
...
//---------------------------------------------------------------------------------------
void MuonTrkPhysMonitoring::fillHistograms_ZSignal(std::vector<std::pair<const xAOD::Muon*, const xAOD::Muon*> > mumucandidates){
    
  for (auto mumu: mumucandidates){

      // NB: ResonancePairingTool returns a pair of (plus, minus)-charged muons.
      fillTagAndProbe(mumu.first, mumu.second);
      
  }
}

//---------------------------------------------------------------------------------------
void MuonTrkPhysMonitoring::fillTagAndProbe(const xAOD::Muon* mu_plus, const xAOD::Muon* mu_minus){

    const float invariant_mass = (mu_plus->p4() + mu_minus->p4()).M();

    m_oZSignalPlots[0]->fill(mu_plus->eta(), mu_minus->eta(), invariant_mass);
