...
def MMMonitoringConfig(inputFlags):
    ...
    mmGroup.defineHistogram('numberofstrips_percluster;Number_of_strips_percluster',  type='TH1F', title='Number of strips per cluster; Number of strips; Number of Entries', path='Overview',   xbins=20, xmin=0., xmax=20., opt='kAlwaysCreate') 
    ...
    mmGroup.defineHistogram('time_all;Time',  type='TH1F', title='Time; Time[ns]; Number of Entries', path='Overview',   xbins=1000, xmin=0., xmax=10000., opt='kAlwaysCreate')
    mmGroup.defineHistogram('strip_times;time_strip',  type='TH1F', title='strip time; time[ns]; Number of Entries', path='Overview',   xbins=200, xmin=0., xmax=200., opt='kAlwaysCreate')
    mmGroup.defineHistogram('cluster_times;time_cluster',  type='TH1F', title='cluster time; time[ns]; Number of Entries', path='Overview',   xbins=200, xmin=0., xmax=200., opt='kAlwaysCreate')
    ...
    for iside in side:
        ...
        for eta in etasector:
            ...
            for multi in range(multipletmin, multipletmax+1):
                for gas_gap in range(1,5):
                    ...
                    mmSideGroup.defineHistogram(var_pcb_allphi, type='TEfficiency', title=title_allphi_eff+"; pcb ;Efficiency Eta1",path='Efficiency', xbins=maxpcb, xmin=0, xmax=maxpcb, opt='kAlwaysCreate') 
        for phi in range(1, phimax+1):
            ...
            mmSideGroup.defineHistogram(var, type='TH2F', title=title_MMSummary+"; ;Strip Number", path='Number_of_strips_percluster_perPhiSector', xbins=16, xmin=0, xmax=16, xlabels=thisLabelx11, ybins=5120, ymin=0., ymax=5120., opt='kAlwaysCreate')
            thisLabely=getMMLabelY("y_lab_occ_lb")
            mmSideGroup.defineHistogram('lb_mon,sector_lb_'+iside+'_phi'+str(phi)+';Occupancy_lb_'+iside+'_phi'+str(phi), type='TH2F', title="Occupancy wrt lb sector"+str(phi)+"; LB; PCB", path='Occupancy',  xbins=100, xmin=-0.5, xmax=99.5, opt='kAddBinsDynamically,kAlwaysCreate', ybins=64, ymin=0., ymax=64, ylabels=thisLabely)
            mmSideGroup.defineHistogram('lb_ontrack,sector_lb_'+iside+'_phi'+str(phi)+"_ontrack"+';Occupancy_lb_'+iside+'_phi'+str(phi)+"_ontrack", type='TH2F', title="Occupancy wrt lb sector"+str(phi)+" ontrack"+"; LB; PCB", path='Occupancy',  xbins=100, xmin=-0.5, xmax=99.5, opt='kAddBinsDynamically,kAlwaysCreate', ybins=64, ymin=0., ymax=64, ylabels=thisLabely)
            ...
            for eta in etasector:
                ...
                for multi in range(multipletmin, multipletmax+1):
                    for gas_gap in range(1,5):
                        ...
                        mmSideGroup.defineHistogram(var_pcb, type='TEfficiency', title=title_eff+"; pcb; Efficiency Eta1", path='Efficiency', xbins=maxpcb, xmin=0, xmax=maxpcb, opt='kAlwaysCreate')
                        ...
                        mmSideGroup.defineHistogram(var1, type='TH1F', title=title_MMSummary_charge+';Charge [fC]; Number of Entries', path='Charge_perLayer', xbins=120, xmin=0., xmax=1200., opt='kAlwaysCreate')
                        ...
                        mmSideGroup.defineHistogram(var3, type='TH1F', title=title_MMSummary_angle+"; #muTPC angle [degrees];Number of Entries", path='uTPC_angle_perLayer', xbins=2000, xmin=-100, xmax=100, opt='kAlwaysCreate')
                        ...
                        mmSideGroup.defineHistogram(var_residual, type='TH1F', title=title_residual+"; res [mm]; Number of Entries", path='Residuals', xbins=200, xmin=-10, xmax=10, opt='kAlwaysCreate')
                        ...
                        mmSideGroup.defineHistogram(var_clus_size, type='TH2F', title=title_cl_size + "; PCB; cluster size;", path="Cluster_size_perPCB", xbins=maxpcb, xmin=1, xmax=maxpcb+1, ybins=20, ymin=0, ymax=20, opt='kAlwaysCreate')
        ...
        for gas1 in range(1, 5):
            for multi1 in range(1, 3):
                ...
                mmSideGroup.defineHistogram(var_ontrack, type='TH2F', title=title_ontrack+"; MM-GlobalX [mm]; MM-GlobalY [mm];", path='PosY_vs_Posx_perLayer_ontrack',xbins=500, xmin=-5000, xmax=5000., ybins=500, ymin=-5000.,ymax=5000., opt='kAlwaysCreate')
...
if __name__=='__main__':
    ...
    ConfigFlags.Input.Files =[
        '/afs/cern.ch/user/b/bigliett/myeos/DQ/group.det-muon.DiMuonGenerator_EtaGtr_1p2_Pt10to100.ESD.Run3_2NSW_250322_reco_n270322_EXT0/group.det-muon.28531270.EXT0._000004.ESD.pool.root',
    ]
