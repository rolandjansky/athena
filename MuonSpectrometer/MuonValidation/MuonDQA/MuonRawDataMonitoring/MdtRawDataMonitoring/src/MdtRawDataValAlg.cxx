   ...
   m_hist_hash_list(0)
{
  ...
  declareProperty("MaxTDCCutForBackground",  m_TDCCut_Bkgrd=300.);
  ...
  declareProperty("Eff_segm_type",           m_segm_type="MooreSegments");
...
{  
  ...
  //   sc= StoreTriggerType();
  //   if (sc.isFailure()){
  //     ATH_MSG_ERROR(" Cannot StoreTriggerType " );
  //     return sc;
  //   }
  //   ATH_MSG_DEBUG("MdtRawDataValAlg::MDT RawData Monitoring Histograms being filled" );

  //
  // Retrieve the LVL1 RoIs:
  //
  StoreTriggerType(L1_UNKNOWN);
  StringProperty m_lvl1_roi_key("LVL1_ROI");
  const LVL1_ROI* lvl1_roi;
  sc = evtStore()->retrieve( lvl1_roi, m_lvl1_roi_key );
  if( sc.isFailure() ) {
    //     ATH_MSG_ERROR( "Failed to access LVL1_ROI in StoreGate with key: " << m_lvl1_roi_key );
    //     return StatusCode::FAILURE;
  } 
  else {
    ATH_MSG_VERBOSE( "Retrieved LVL1_ROI object from StoreGate with key: " << m_lvl1_roi_key );      
    for( LVL1_ROI::muons_type::const_iterator lvl1_muon = lvl1_roi->getMuonROIs().begin(); lvl1_muon != lvl1_roi->getMuonROIs().end(); ++lvl1_muon ) {
      // 	ATH_MSG_ERROR( "lvl1_muon->getSource(): " << lvl1_muon->getSource() << ", is Muon_ROI::Endcap: " << (lvl1_muon->getSource()==(Muon_ROI::Endcap)) << ", is Muon_ROI::Barrel: " << (lvl1_muon->getSource()==(Muon_ROI::Barrel)) );
      if( (lvl1_muon->getSource()==(Muon_ROI::Barrel)) ) StoreTriggerType(L1_BARREL);
      if( (lvl1_muon->getSource()==(Muon_ROI::Endcap)) ) StoreTriggerType(L1_ENDCAP);
    }
  }
...
// 	      {
	    ...
	    sc = fillMDTOverviewHistograms(*mdtCollection);
	    if(sc.isSuccess()) { 
	      ATH_MSG_DEBUG("Filled MDTOverviewHistograms" );
	    }
	    else {
	      ATH_MSG_ERROR("Failled to fill MDTOverviewHistograms" ); 
	      return sc;
	    }

	    sc = fillMDTSummaryHistograms(*mdtCollection);
	    if(sc.isSuccess())
	      {
...
StatusCode MdtRawDataValAlg::bookMDTHistograms( MDTChamber* chamber, Identifier digcoll_id ) {
  ...
  if(m_do_mdttdc) 
    sc = bookMDTHisto_chambers(chamber->mdttdc, hardware_name + "_MDT_Station_TDC", "[nsec]", "Number of Entries",
			       80, 0, 2000., *mongroup_chambers_expert);
  ...
  if(m_chamber_2D) {
    ...
    if(m_do_mdttdcadc) 
      sc = bookMDTHisto_chambers_2D(chamber->mdttdcadc, hardware_name + "_MDT_Station_TDCADC", "[nsec]", "[adc counts]",
				    80, 0, 2000., 40, 0., 300., *mongroup_chambers_expert);
...
StatusCode MdtRawDataValAlg::bookMDTSummaryHistograms(/* bool isNewEventsBlock, */bool newLumiBlock, bool newRun) {
  ...
  if( (newLowStatInterval && !m_isOnline) || (m_isOnline && newRun) ){
    ...
    //Book tdcadc per region per lowStat
    sc = bookMDTHisto_overview_2D(overalltdcadcPRLumi[enumBarrelA], "Overall_TDCADC_spectrum_BA", "[nsec]", "[adc counts]", 80, 0, 2000.,
				  40, 0., 300., mg->mongroup_brA_shiftLumi);        
    sc = bookMDTHisto_overview_2D(overalltdcadcPRLumi[enumBarrelC], "Overall_TDCADC_spectrum_BC", "[nsec]", "[adc counts]", 80, 0, 2000.,
				  40, 0., 300., mg->mongroup_brC_shiftLumi);        
    sc = bookMDTHisto_overview_2D(overalltdcadcPRLumi[enumEndCapA], "Overall_TDCADC_spectrum_EA", "[nsec]", "[adc counts]", 80, 0, 2000.,
				  40, 0., 300., mg->mongroup_ecA_shiftLumi);        
    sc = bookMDTHisto_overview_2D(overalltdcadcPRLumi[enumEndCapC], "Overall_TDCADC_spectrum_EC", "[nsec]", "[adc counts]", 80, 0, 2000.,
				  40, 0., 300., mg->mongroup_ecC_shiftLumi);        
...
StatusCode MdtRawDataValAlg::bookMDTOverviewHistograms(/* bool isNewEventsBlock,*/ bool newLumiBlock, bool newRun) {
  ...
  if( (newLowStatInterval && !m_isOnline) || (m_isOnline && newRun) ){
    ...
    sc = bookMDTHisto_overview(overalltdccutLumi, "Overall_TDC_ADCCut_spectrum", "[nsec]", "Number of Entries",
			       80, 0., 2000., mg->mongroup_overview_shiftLumi);
    ...
    sc = bookMDTHisto_overview(mdteventscutLumi_big, "TotalNumber_of_MDT_hits_per_event_big_ADCCut", "[counts]", "Number of Events",
			       200, 0., 50000., mg->mongroup_overview_shiftLumi);
    ////////////////////////////////////////////////////////////////////////////////////// 
    //histo path for TotalNumber_of_MDT_hits_per_event without a cut on ADC 
    ...
    ////////////////////////////////////////////////////////////////////////////////////// 
    //histo path for overall tdc vs adc spectrum 
    sc = bookMDTHisto_overview_2D(overalltdcadcLumi, "Overall_TDCADC_spectrum", "[nsec]", "[adc counts]", 80, 0, 2000.,
				  40, 0., 300., mg->mongroup_overview_shiftLumi);        
    ////////////////////////////////////////////////////////////////////////////////////// 
    //histo path for overall tdccut RPCtrig spectrum 
  ...
  if(newRun){
    ...
    sc = bookMDTHisto_overview(nummdtchamberswithhits_ADCCut, "number_of_Chambers_with_hits_per_event_ADCCut",
			       "[Number_of_MDT_chambers_with_hits]", "Number of Entries", 200, 0., 800., mg->mongroup_overview_shift);
...
StatusCode MdtRawDataValAlg::fillMDTHistograms( const Muon::MdtPrepData* mdtCollection ) {
  ...
  if (chamber->mdttdcadc) { chamber->mdttdcadc->Fill(tdc, adc); }
...
// 						 m_mdtIdHelper->stationPhi(digcoll_id),type);
  ...
  if(overalltdcadcLumi && !isNoisy) overalltdcadcLumi->Fill(tdc, adc);
