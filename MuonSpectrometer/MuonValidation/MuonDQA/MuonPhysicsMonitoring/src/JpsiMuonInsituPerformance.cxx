...
JpsiMuonInsituPerformance::JpsiMuonInsituPerformance( const std::string & type, const std::string & name, const IInterface* parent )
  :ManagedMonitorToolBase( type, name, parent ),
   ...
   m_V0Tools("Trk::V0Tools"),//@@@ disabled until fixed;
...
bool JpsiMuonInsituPerformance::doVertexing(const Rec::TrackParticle *theTag, const Rec::TrackParticle *theProbe, Trk::RecVertex &/*primaryVertex*/,double &vtxChi2, double &mass, double &tau)
{
  ...
  if( myVxCandidate ) {
    ...
    vtxChi2 = -1; //1.*m_V0Tools->chisq(&(*myExVxCandidate)) / m_V0Tools->ndof(&(*myExVxCandidate));//@@@ disabled until fixed;
    //Lxy = m_V0Tools->lxy(&(*myExVxCandidate),(*primaryVertex));
    const double mytau = -1; //m_V0Tools->tau(&(*myExVxCandidate),primaryVertex,3096.9);//@@@ disabled until fixed;
    const double mymass = -1;//m_V0Tools->invariantMass(&(*myExVxCandidate),myMuonMasses);//@@@ disabled until fixed;
    tau = mytau;
    mass = mymass;
