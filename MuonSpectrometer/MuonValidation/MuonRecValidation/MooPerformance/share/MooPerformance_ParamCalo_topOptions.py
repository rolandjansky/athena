...
muonCombinedRecFlags.doMuGirl                      = False # requires calo to produce track collection
muonCombinedRecFlags.doMuonPhysicsDressing         = False # requires calo
...
try:
    ...
    ### disable algs needing calo cells that don't switch off with DetFlags
    topSequence.CaloCellMaker.Enable               = False
    topSequence.LArNoisyROAlg.Enable               = False
    topSequence.MBTSTimeDiffEventInfoAlg.Enable    = False
    topSequence.InDetCaloClusterROISelector.Enable = False
    topSequence.BeamBackgroundFiller.Enable        = False
    topSequence.BackgroundWordFiller.Enable        = False
    #topSequence.JetGlobalEventSetup.Enable         = False
    #topSequence.CaloCellMakerFromCluster.Enable    = False
    ### suppress annoying WARNING (clogging up log file)
    ToolSvc.AtlasExtrapolator.OutputLevel          = ERROR  # negative path warnings
    ToolSvc.TrackDepositInCaloTool.OutputLevel     = ERROR
    #ToolSvc.TrackInCaloTools.OutputLevel           = ERROR

    ### force test extrapolate inwards fix from Sharka
    #ToolSvc.CombinedMuonTrackBuilder.ReallocateMaterial = True
    ...
    MuidStatistics.MuGirlMuonLocation   = ""
    MuidStatistics.MuGirlTrackLocation  = ""
