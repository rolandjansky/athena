...
public:
    ...
    struct TrackData {
        TrackData() :
            ...
            motherPdg(-1),
            chi2Ndof(0.),
            productionVertex(nullptr),
            momentumAtProduction(nullptr),
            truthTrajectory(nullptr),
            trackPars(nullptr),
            trackSummary(nullptr) {}
        ...
        bool hasMissingChambers() const { return !missingChambers.empty(); }

        bool hasWrongChambers() const { return !wrongChambers.empty(); }

        bool hasMissingLayers() const { return !missingLayers.empty(); }

        bool hasWrongLayers() const { return !wrongLayers.empty(); }

        bool hasMissingLayersTrigger() const { return !missingLayersTrigger.empty(); }
    ...
    struct EventData {
        ...
        bool goodEvent() const {
            return missingTruthTracks.empty() && missingTruthTracksOneStation.empty() && missingStationMomLoss.empty() &&
                   missingStationLayer.empty() && missingStation.empty() && wrongStationLayer.empty() && wrongStation.empty() &&
                   fakeTracks.empty() && fakeTracksLowPt.empty() && fakeTracksSL.empty();
        }
    };
...
private:
    ...
    bool insertChamber(const Identifier& chId, const std::set<Identifier>& hits, int minEtaHits, int minPhiHits,
                       ChamberData& chamberData) const;

    /** insert set of chambers into chamber data if hits counts pass cuts, returns true if anything was inserted */
    bool insertTechnology(const std::set<Identifier>& chIds, const std::set<Identifier>& hits, int minEtaHits, int minPhiHits,
                          std::vector<ChamberData>& chamberData) const;
    ...
    SG::ReadHandleKey<TrackCollection> m_trackKey{this, "TrackInputLocation", "MuonSpectrometerTracks",
                                                  "input tracks"};                                 //!< Location of the input tracks
