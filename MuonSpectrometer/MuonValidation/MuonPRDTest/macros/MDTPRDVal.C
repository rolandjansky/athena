...
{
  TFile* f_nom = new TFile("MDTTest.mag.rung.root","READ");
  f_nom->cd("MDTHitsValidation");
  ...
  TFile* f_nom100 = new TFile("MDTTest.mag.rung.pre.root","READ");
  f_nom100->cd("MDTHitsValidation");
  gStyle->SetMarkerColor(2);
  drawProp("BO","100");
  drawProp("EO","100");

  TFile* f_nom10000 = new TFile("MDTTest.mag.rung.pre.10000.root","READ");
  f_nom10000->cd("MDTHitsValidation");
...
void drawSag(){
  ...
  pad->SetLogy();
  MDTTestHits->Draw("MDT_ExNomWireR-MDT_SdoRadius");
  ...
  pad = sag_resi->cd(2);
  MDTTestHits->Draw("MDT_ExSagWireR-MDT_SdoRadius");
  ...
  pad = sag_resi->cd(3);
  MDTTestHits->Draw("MDT_LocalZ/MDT_WireLen",
		    "abs(MDT_ExSagWireR-MDT_SdoRadius) > 0.1" );
  hist = extractHist(pad,"htemp");
  if( hist ){
  ...
  pad = sag_resi->cd(4);
  MDTTestHits->Draw("MDT_SimRadius-abs(MDT_SdoRadius):MDT_LocalZ",
		    "" );
...
void drawSim(){
  ...
  pad->SetLogy();
  MDTTestHits->Draw("abs(MDT_ExNomWireR)-MDT_SimRadius");
  TH1* hist = extractHist(pad,"htemp");
  if( hist ){
  ...
  pad = sim_resi->cd(2);
  MDTTestHits->Draw("MDT_ExNomWireZ-MDT_LocalZ");
  hist = extractHist(pad,"htemp");
  if( hist ){
  ...
  pad = sim_resi->cd(3);
  MDTTestHits->Draw("MDT_LocalZ/MDT_WireLen",
		    "abs(abs(MDT_ExNomWireR)-MDT_SimRadius) > 0.1" );
  hist = extractHist(pad,"htemp");
  if( hist ){
  ...
  pad = sim_resi->cd(4);
  MDTTestHits->Draw("MDT_SimRadius:abs(MDT_ExNomWireR)",
		    "abs(MDT_LocalZ/MDT_WireLen) < 0.45 && abs(abs(MDT_ExNomWireR)-MDT_SimRadius) > 0.1" );
...
void drawROT(){
  ...
  pad->SetLogy();
  MDTTestHits->Draw("MDT_ExSagRotR-MDT_SdoRadius");
  TH1* hist = extractHist(pad,"htemp");
  if( hist ){
  ...
  pad = rot_resi->cd(3);
  MDTTestHits->Draw("abs(MDT_ExSagRotR)-MDT_RotRadius",
		    "abs( abs(MDT_ExSagRotR)-MDT_RotRadius ) < 0.5");
  hist = extractHist(pad,"htemp");
  if( hist ){
  ...
  pad = rot_resi->cd(4);
  MDTTestHits->Draw("abs(MDT_ExSagRotR)-MDT_RotRadius:MDT_LocalZ",
		    "abs( abs(MDT_ExSagRotR)-MDT_RotRadius ) < 0.5");
...
void drawProp(TString chname,TString suff = ""){
  
  TString cname = "prop_resi_";
  cname += chname + suff;
  TCanvas* prop_resi = new TCanvas(cname,cname,900,600);
  prop_resi->Divide(2,2);

  //TString radialResString("abs(MDT_ExNomWireR)-MDT_SimRadius");
  TString radialResString("abs(MDT_ExNomWireR)-MDT_SimRadius");
  TString radialResString_R( radialResString + ":MDT_HitR" );
  TString zResString("MDT_ExNomWireZ-MDT_LocalZ:MDT_HitR");

  // define cuts
  TString radialCut("abs(abs(MDT_ExNomWireR)-MDT_SimRadius)<0.5");
  TString zCut("abs(MDT_ExNomWireZ-MDT_LocalZ)<0.5");
  TString chamberCutS("MDT_StationName==\"");
  chamberCutS += chname + "S\""; 
  TString chamberCutL("MDT_StationName==\"");
  chamberCutL += chname + "L\""; 
  TString cutS = radialCut + "&&" + chamberCutS;
  TString cutL = radialCut + "&&" + chamberCutL;

  pad = prop_resi->cd(1);
  MDTTestHits->Draw(radialResString_R,cutS);
  hist = extractHist(pad,"htemp");
  if( hist ){
    ...
    hist->GetYaxis()->SetTitle("#Delta R");
  }
  ...
  pad = prop_resi->cd(3);
  MDTTestHits->Draw(radialResString_R,cutL);
  hist = extractHist(pad,"htemp");
  if( hist ){
    TString hname = "PropResi_R_";
    hname += chname + "L";
  ...
  prop_resi->Print(cname + ".eps","eps");
  prop_resi->Print(cname + ".gif","gif");
...
void drawProp(){
  ...
  MDTTestHits->Draw("abs(MDT_ExNomWireR)-MDT_SimRadius",
		    "abs(abs(MDT_ExNomWireR)-MDT_SimRadius)<0.5");
  TH1* hist = extractHist(pad,"htemp");
  if( hist ){
    hist->SetName("SimResi");
    hist->GetXaxis()->SetTitle("#Delta R_{sim-ex}");
  ...
  pad = prop_resi->cd(2);
  MDTTestHits->Draw("MDT_ExNomWireZ-MDT_LocalZ",
		    "abs(MDT_ExNomWireZ-MDT_LocalZ)<0.5");
  hist = extractHist(pad,"htemp");
  if( hist ){
    hist->SetName("SimZ_ExZ");
    hist->GetXaxis()->SetTitle("SimHit Z");
...
void draw()
{
 TFile* f = new TFile("MDTPRDTest.default_geom.defstep.root","READ");
