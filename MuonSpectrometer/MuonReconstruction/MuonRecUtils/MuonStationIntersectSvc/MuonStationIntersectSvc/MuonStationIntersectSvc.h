...
public:
    ...
    */
    const Muon::MuonStationIntersect tubesCrossedByTrack(const Identifier& id, const Amg::Vector3D& pos, const Amg::Vector3D& dir,
                                                         const MdtCondDbData* condData, const MuonGM::MuonDetectorManager* detMgr) const;

    /** @brief get geometry description of the given chamber + neighbouring chambers */
    const std::vector<std::unique_ptr<Muon::MdtIntersectGeometry> > getStationGeometry(const Identifier& id, const MdtCondDbData* dbData,
