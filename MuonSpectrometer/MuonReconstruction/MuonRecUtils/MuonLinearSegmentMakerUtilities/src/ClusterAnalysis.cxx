...
namespace ClusterSeg {
  ...
  bool sortfunction (Cluster* i,Cluster* j) { return (i->z() < j->z()); }
  ...
  void ClusterAnalysis::analyseWrite() {
    ...
    h_rz = new TH2F("rz_spacePoints","",1000,12000,16000,3000,1000,13000);
    ...
    for( Long64_t evt=0;evt<nentries;++evt ){
      ...
      std::sort (clust.begin(),clust.end(), sortfunction); 
     
      thePoints = createSpacePoints(clust);
    
      if(thePoints.empty()) continue;
      std::vector<std::vector<SpacePoint>> seeds = createSeeds(thePoints);
  ...
  std::vector<std::vector<SpacePoint>> ClusterAnalysis::createSeeds( std::vector<SpacePoint>& points ){
    ...
    for(auto &it: points){
      if (it.isTGC(1)) layer1Points.push_back(it);
      else if(it.isTGC(2)) layer2Points.push_back(it);
      else layer3Points.push_back(it);
    }
    if(writeOut){
    ...
    //look at events where one layer is missing spacepoints
    ...
    //First make the seeds from the two outer stations
    if(!layer1Points.empty() && !layer3Points.empty()){
      ...
      if (TMath::Sign(1.,it1.z()) != TMath::Sign(1.,it3.z())) continue;
        ...
        Cluster point = Cluster(it3.x()-it1.x(),it3.y()-it1.y(),it3.z()-it1.z(),false,false,0);
        ...
        if(angle < ang_cut) {
          //Now try to add the third point
          if(!layer2Points.empty()){
    ...
    //now add cases for which it is missing one layer
    if(seeds.size() < 25){
      if(!layer1Points.empty() && !layer2Points.empty()){
        for(auto &it1: layer1Points){
      ...
      } else if(!layer1Points.empty() && !layer3Points.empty()){
        for(auto &it1: layer1Points){
          for(auto &it3: layer3Points){
            if (TMath::Sign(1.,it1.z()) != TMath::Sign(1.,it3.z())) continue;
            std::vector<SpacePoint> theSeed;
            ...
            double dtheta = TMath::Abs(point.theta()-it1.theta());
            if (writeOut) {
              h_phi13->Fill(TMath::Abs(dphi));
              h_theta13->Fill(dtheta);
              h_angle13->Fill(angle);
            }
            if(writeOut && it3.isMatch() && it1.isMatch() && it3.barcode() == it1.barcode() && it1.barcode() != 0 ) {
              h_phi_t13->Fill(TMath::Abs(dphi));
              h_theta_t13->Fill(dtheta);
              h_angle_t13->Fill(angle);
            }
            if (angle < ang2_cut){
              theSeed.push_back(it1);
              theSeed.push_back(it3);
      ...
      } else if(!layer2Points.empty() && !layer3Points.empty()){
        for(auto &it1: layer2Points){
          for(auto &it3: layer3Points){
            if (TMath::Sign(1.,it1.z()) != TMath::Sign(1.,it3.z())) continue;
            std::vector<SpacePoint> theSeed;
            Cluster point = Cluster(it3.x()-it1.x(),it3.y()-it1.y(),it3.z()-it1.z(),false,false,0);
            ...
            double dtheta = TMath::Abs(point.theta()-it1.theta());
            if (writeOut) {
              h_phi23->Fill(TMath::Abs(dphi));
              h_theta23->Fill(dtheta);
              h_angle23->Fill(angle);
            }
            if(writeOut && it3.isMatch() && it1.isMatch() && it3.barcode() == it1.barcode() && it1.barcode() != 0 ) {
              h_phi_t23->Fill(TMath::Abs(dphi));
              h_theta_t23->Fill(dtheta);
              h_angle_t23->Fill(angle);
            }
            if (angle < ang2_cut){
              theSeed.push_back(it1);
              theSeed.push_back(it3);
  ...
  std::vector<SpacePoint> ClusterAnalysis::createSpacePoints( std::vector<Cluster*>& clust) {
    ...
    for(auto &eit: etaClusters){
      ...
      if(TMath::Sign(1.,pit.first->z()) == TMath::Sign(1.,eit.first->z()) && !pit.first->isTGC(4) && pit.first->chamberId() != "" && pit.first->chamberId() == eit.first->chamberId()) {
        SpacePoint thePoint = SpacePoint(eit.first->eta(),pit.first->phi(),pit.first->z(),tbool,eit.first->barcode(),eit.second,pit.second);
        spacePoints.push_back(thePoint);
      }
      if(writeOut && !pit.first->isTGC(4) && fabs(pit.first->z() - eit.first->z()) < 25.) {
        SpacePoint thePoint = SpacePoint(eit.first->eta(),pit.first->phi(),pit.first->z(),tbool,eit.first->barcode(),eit.second,pit.second);
        spacePoints.push_back(thePoint);
      }    
    }
