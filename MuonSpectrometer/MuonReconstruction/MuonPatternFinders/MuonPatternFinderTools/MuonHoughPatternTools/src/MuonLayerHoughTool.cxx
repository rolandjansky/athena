  ...
  StatusCode MuonLayerHoughTool::initialize() {
    ...
    if( m_truthNames.empty() && m_doTruth ){
      ...
      for( unsigned int tech=0; tech<m_ntechnologies;++tech ){
        ...
        if (tech==MuonStationIndex::CSC && !m_idHelper->HasCSC()) continue;
        else if (tech==MuonStationIndex::STGC && !m_idHelper->HasSTgc()) continue;
        else if (tech==MuonStationIndex::MM && !m_idHelper->HasMM()) continue;
      ...
      for( auto max : road.maxima ){
        ...
        for( auto ehit = maximum.hits.begin();ehit!=maximum.hits.end();++ehit ){
          ...
          if( etaHit.tgc ) {
            ...
            if( !m_idHelper->isRpc(etaHit.prd->identify()) && !m_idHelper->issTgc(etaHit.prd->identify()) ) continue;
    ...
    while( nmaxima < 5 ){
      ...
      if( hough.findMaximum( maximum, selectorLoose ) ) {
        ...
        if( nmdt > 0 || (nmm + nstgc) > 0) {
          ...
          // add to seed list if 
          if( maximum.max > selector.getCutValue(maximum.pos) ) m_seedMaxima.push_back(maxima.back());        
          ++nmaxima;
        }
  ...
  void MuonLayerHoughTool::fill( const MdtPrepDataCollection& mdts, MuonLayerHoughTool::HitVec& hits ){
    ...
    for( ;mit!=mit_end;++mit ){
      ...
      if (pos != m_techToTruthNameIdx.end()) {
        matchTruth(*m_truthCollections[pos->second],id,*debug);
      }
  ...
  void MuonLayerHoughTool::fill( const sTgcPrepDataCollection& stgcs, MuonLayerHoughTool::HitVec& hits, MuonLayerHoughTool::PhiHitVec& phiHits, int selectedSector ){
    ...
    for( ;mit!=mit_end;++mit ){
	  ...
	  // get the pad width from the detector design
          ...
          if(m_debugHough) std::cout << " Pad chWidth " << chWidth  << " phi global " << prd.globalPosition().phi() << std::endl;
        }
          ...
          if(m_debugHough) std::cout << " Wire Gang chWidth " << chWidth  << " phi global " << prd.globalPosition().phi() << std::endl;
        }
