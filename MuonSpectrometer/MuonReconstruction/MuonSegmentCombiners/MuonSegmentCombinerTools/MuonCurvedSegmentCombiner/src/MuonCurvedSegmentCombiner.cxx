...
std::unique_ptr<MuonSegmentCombinationCollection> Muon::MuonCurvedSegmentCombiner::combineSegments(
    ...
    if (msgLvl(MSG::DEBUG) || m_summary) {
        if (mdtCombiColl.empty())
            ...
            ATH_MSG_INFO(" summarizing input: Mdt MuonSegment combinations " << std::endl << m_printer->print(mdtCombiColl));
    }
    ...
    if (m_useCscSegments) {
        ...
        if (msgLvl(MSG::DEBUG) || m_summary) {
            if (csc4DCombiColl.empty())
                ...
                ATH_MSG_INFO(" summarizing input: Csc MuonSegment combinations " << std::endl << m_printer->print(csc4DCombiColl));
        }
        ...
        if (msgLvl(MSG::DEBUG) || m_summary) {
            if (csc2DCombiColl.empty())
                ...
                ATH_MSG_INFO(" summarizing input: Csc 2D MuonSegment combinations " << std::endl << m_printer->print(csc2DCombiColl));
        }
...
std::unique_ptr<MuonSegmentCombinationCollection> Muon::MuonCurvedSegmentCombiner::processCombinationCollection(
    ...
    for (; cit != cit_end; ++cit) {
        ...
        std::pair<MuonSegmentCombPatternCombAssociationMap::const_iterator, MuonSegmentCombPatternCombAssociationMap::const_iterator>
        ...
        if (segPattMap->count(combi) != 1) { ATH_MSG_INFO(" This MuonSegPatAssMap for MDTs should only have one entry!! "); }
        ...
        for (unsigned int i = 0; i < combi->numberOfStations(); ++i) {
            ...
            for (unsigned int si = 0; si < segs.size(); ++si) {
                ...
                std::map<Muon::MuonSegment*, const MuonPatternCombination*>::iterator it = segAssoMap.find(segs[si].get());
                nmdtsegments++;
    ...
    if (m_debug)
        std::cout << " First stage muonCurvedSegmentCombinations " << curvedCombiCol->size() << " Mdt segments " << nmdtsegments
                  << " Csc segments " << ncscsegments << std::endl;
    ...
    if (mergePatterns) {
        ...
        for (; cit != cit_end; ++cit) {
            ...
            for (; cit1 != cit_end; ++cit1) {
                ...
                if ((range1.first) == (range1.second)) { continue; }
                const MuonPatternCombination* pattern1 = (range1.first)->second;
                if (segPattMap->count(combi1) != 1) { ATH_MSG_INFO(" This MuonSegPatAssMap for MDTs should only have one entry!! "); }
                ...
                if ((cos(patternTheta) * cos(patternTheta1) + sin(patternTheta) * sin(patternTheta1)) < 0.955 && m_mergePatterns &&
                    !m_doCosmics)
                    continue;
                        ...
                        if (!segs.empty()) {
                            ...
                            for (unsigned int j = 0; j < combi1->numberOfStations(); ++j) {
                                ...
                                if (!segs1.empty()) {
                                    ...
                                    double dotprod1 =
                                        segpos1.x() * patternDir1.x() + segpos1.y() * patternDir1.y() + segpos1.z() * patternDir1.z();

                                    // if patterns are in opposite hemisphere (assuming directions are more or less
                                    // same)
                                    if ((dotprod < 0 && dotprod1 < 0) || (dotprod > 0 && dotprod1 > 0)) { compatible = true; }
                                }
    ...
    if (msgLvl(MSG::DEBUG) || m_summary) {
        if (curvedCombiCol->empty())
            ...
            ATH_MSG_INFO(" summarizing output: MuonCurvedSegment combinations " << std::endl << m_printer->print(*curvedCombiCol));
    }
        ...
        if (combi->numberOfStations() > 0) {
            ...
            for (unsigned int i = 0; i < combi->numberOfStations(); ++i) {
                ...
                for (unsigned int si = 0; si < segs.size(); ++si) {
                    ...
                    for (; ia_it != ia_it_end; ++ia_it) { info.patPointers.push_back((*ia_it).second); }
...
void Muon::MuonCurvedSegmentCombiner::muonCurvedSegmentCombinations(
    ...
    for (; seg_it != seg_it_end; ++seg_it) {
        ...
        if (5 * (missedh - miscorrect) > ntot * missedHitsCut) {
            if (m_debug)
                std::cout << " Drop this segment with missed hits " << missedh << " correction " << miscorrect << " phi pat hits "
                          << info.phiPatHits << " trigger layers " << info.nLayTrig << std::endl;
            continue;
        }
        int indexSegment = info.index;
        double key =
            10000 * (code + 1) + missedh * 100 + ntrigFrac * 10 + 10 * lchi2 - (indexSegment / 10000.) - patternHits - 100 * info.nHots;
        if (checkSegmentsIndex.count(indexSegment) == 1) {
            if (m_debug)
                std::cout << " PROBLEM with Segments Index is already in the checkSegmentsIndex collection " << indexSegment << std::endl;
        }
        ...
        if (checkSegments.count(key) == 1) {
            ...
            std::string st = info.station;
            if (m_debug)
                std::cout << " PROBLEM this key is already in the checkSegments collection " << key << " station " << st << " pointer1 "
    ...
    if (m_debug) {
        ...
        for (; ic_it != ic_it_end; ++ic_it) {
            ...
            if (m_debug)
                std::cout << " Check Station " << st << " code " << code << " theta " << theta << " key " << ic_it->first << std::endl;
        }
    }
    ...
    for (; ic_it != ic_it_end; ++ic_it) {
        ...
        int ntrigFrac = (int)layMissed * 10;
        if (m_debug)
            std::cout << " station " << info.station << " missed hits " << missedh << " nLayTrig " << nLayTrig << " layers missed "
                      << layMissed << " ntrigFrac " << ntrigFrac << std::endl;
        //        int ntot = nHots;
        //        if ( ntot < 5 ) ntot = 5;
        ...
        if (m_debug)
            std::cout << " segments station " << segmentsChamber[1000 * code + patIn] << " missed hits " << missedh << " ntrigFrac "
                      << ntrigFrac << " ip " << ip << " i " << i << " key " << key << std::endl;
        if (m_debug && info.nCsc == 1)
            std::cout << " Csc Segment " << info.nHots << " log 1 + chi2 dof " << log(1. + info.chi2 / nHots) << std::endl;
    ...
    for (; io_it != io_it_end; ++io_it) {
        ...
        if (m_debug) {
            ...
            std::cout << " Key station " << st << " theta " << seg->globalDirection().theta() << " key " << io_it->first << " patPhi "
                      << patPhi[i] << " patTheta " << patTheta[i] << std::endl;
        }
        drop[i] = 0;
        if (multi[i] > 9) {
            j0++;
        ...
        if (m_debug && drop[i] == 1)
            std::cout << "Drop this segment with multiplicity " << multi[i] << " drop  " << drop[i] << " station " << st << " theta "
        ...
        for (int j = i + 1; j < nSegments; ++j) {
            ...
            if (patPoint[i] != nullptr && patPoint[j] != nullptr) {
                ...
                if (patPoint[i] != patPoint[j]) {
                    ...
                    if (!selectPair) {
                        if (m_debug)
                            std::cout << "  Pair not selected i   " << i << " j " << j << " station 1 " << st1 << " theta " << theta1
                                      << " station 2 " << st2 << " theta " << theta2 << " point i " << patPoint[i] << "point j "
                                      << patPoint[j] << " phipat i " << iPhiPatHits[i] << " phipat j " << iPhiPatHits[j] << std::endl;
                        continue;
                    }
                }
                if (m_debug)
                    std::cout << " OK pattern pointer i " << patPoint[i] << " j " << patPoint[j] << " i " << i << " j " << j
                              << " station 1 " << st1 << " theta " << theta1 << " station 2 " << st2 << " theta " << theta2 << std::endl;
            } else {
                ...
                int sign = 1;
                if (hitz < 0) sign = -1;
                double rext = r0 + (hitz - z0) * std::tan(theta) -
            ...
            if (st1[0] != st2[0] || st1[1] != st2[1]) {
                ...
                if (st1[0] != st2[0] || st1[2] != st2[2]) {
                    ...
                    if (10 * (missedHits[i] + missedHits[j] + missedLay[i] + missedLay[j]) > ntot * (m_missedHitsCut + 1))
                        dropmissed = true;
                }
                if (dropmissed) {
                    if (m_debug)
                        std::cout << " DROP MISSED i " << i << st1 << " missed i " << missedHits[i] << " missedLay i " << missedLay[i]
                                  << " j " << j << st2 << " missed j " << missedHits[j] << " missedLay j " << missedLay[j] << " nHots i "
                                  << nHots[i] << " nHots j " << nHots[j] << std::endl;
                    continue;
                }
            }
            //
            ...
            if (m_debug) {
                ...
                std::cout << " Phi space 1 " << phisp << " Phi space 2 " << phispe << " Phi 1 " << segment[i]->globalDirection().phi()
                          << " Phi 2 " << segment[j]->globalDirection().phi() << " pattern phi 1 " << patPhi[i] << " pattern phi2 "
                          << patPhi[j] << std::endl;
            }
            ...
            if (selectPair) {
                ...
                if (m_debug)
                    std::cout << " End fits Pull " << Pull[0] << " station 1 " << st1 << " station 2 " << st2 << " theta " << theta1
                    ...
                    for (; it != it_end; ++it) {
                        ...
                        if (m_debug)
                            std::cout << " Combination length " << std::distance(it_end, it_begin) << " index "
                                      << std::distance(it, it_begin) << " length " << std::distance(combiSegmentList.end(), it) << " len2 "
                        ...
                        if ((iFound && !jFound) || (!iFound && jFound)) {
                            ...
                            if (reject && m_debug)
                                std::cout << " Try to add segment with different pattern pointer phi i " << patPhi[i] << " phi j "
                                          << patPhi[j] << " theta i " << patTheta[i] << " theta j " << patTheta[j] << std::endl;
        ...
        for (; it != it_end; ++it) {
            ...
            for (unsigned int k = 0; k < combi.size(); ++k) {
                ...
                if (m_debug) {
                    ...
                    double phisp = atan2(seg->globalPosition().y(), seg->globalPosition().x());
                    std::cout << " Segment position x " << seg->globalPosition().x() << " y " << seg->globalPosition().y() << " z "
                              << seg->globalPosition().z() << " phi " << seg->globalDirection().phi() << " phi space " << phisp << " theta "
                              << seg->globalDirection().theta() << std::endl;
            ...
            for (; cit != cit_end; ++cit) { combination->addSegments(std::move(cit->second)); }
...
unsigned int Muon::MuonCurvedSegmentCombiner::missedHits(Muon::MuonSegment* seg) const {
...
Muon::MCSCSegmentInfo Muon::MuonCurvedSegmentCombiner::segInfo(
    Muon::MuonSegment* seg, SegInfoMap segInfoMap, std::map<Muon::MuonSegment*, const MuonPatternCombination*> segAssoMap) const {
    ...
    for (; mit != mit_end; ++mit) {
        ...
        if (!rot) continue;
        if (m_idHelperSvc->isCsc(idr)) { id = idr; }
    ...
    if (ismdt) {
        ...
        double trigFrac = 0.;
        if (nLayTrig > nLayMax && nLayMax > 0) { nLayTrig = nLayMax; }
        if (nLayMax > 0) { trigFrac = (nLayMax - nLayTrig) / 2.; }
        ...
        for (; mit != mit_end; ++mit) {
            ...
            if (rot) idr = rot->identify();
            if (!rot) continue;
            if (m_idHelperSvc->isCsc(idr)) { nHots++; }
        }
    }
...
void Muon::MuonCurvedSegmentCombiner::trackParameters(Muon::MuonSegment& seg, double& theta, double& invcurvature, int& imeth,
    ...
    for (int i = 0; i < 2; ++i) { V(i, 0) = Model(0, i) * ym(0, 0) / ers2 + Model(1, i) * ym(1, 0) / ets2; }
    ...
    for (int i = 0; i < 2; ++i) { Res(i, 0) = Model(i, 0) * T(0, 0) + Model(i, 1) * T(1, 0) - ym(i, 0); }
    ...
    if (m_debug)
        std::cout << " covariance at measurement 00 " << CovP(0, 0) << " 01 " << CovP(0, 1) << " 10 " << CovP(1, 0) << " 11 " << CovP(1, 1)
                  << std::endl;
    if (m_debug)
        std::cout << " covariance at track 00 " << CovT(0, 0) << " 01 " << CovT(0, 1) << " 10 " << CovT(1, 0) << " 11 " << CovT(1, 1)
                  << std::endl;
    ...
    if (m_debug)
        std::cout << " Track parameters theta " << theta << " invcurvature " << invcurvature << " Momentum " << 0.1 / invcurvature
                  << std::endl;
}

void Muon::MuonCurvedSegmentCombiner::fit2SegmentsSL(Muon::MuonSegment& seg, Muon::MuonSegment& sege, Amg::VectorX& Res, Amg::VectorX& Pull,
                                                     SegInfoMap segInfoMap) const {
    ...
    if (m_doCosmics) {  // translate line to IP for cosmics
        ...
        if (m_debug)
            std::cout << " phiSegments " << phiSegments << " segment phi " << sege.globalDirection().phi() << " dist " << dist << " dphi "
                      << dphi << " dphie " << dphie << std::endl;
    ...
    for (int i = 0; i < 2; ++i) {
        V(i, 0) =
            Model(0, i) * ym(0, 0) / ers2 + Model(1, i) * ym(1, 0) / ets21 + Model(2, i) * ym(2, 0) / ers2 + Model(3, i) * ym(3, 0) / ets22;
    }
    ...
    // calculate residuals and chi2
    for (int i = 0; i < 4; ++i) { Resi(i, 0) = Model(i, 0) * T(0, 0) + Model(i, 1) * T(1, 0) - ym(i, 0); }
    double chi2 =
        Resi(0, 0) * Resi(0, 0) / ers2 + Resi(1, 0) * Resi(1, 0) / ets21 + Resi(2, 0) * Resi(2, 0) / ers2 + Resi(3, 0) * Resi(3, 0) / ets22;

    if (m_debug) std::cout << " resi 00 " << Resi(0, 0) << " chi2 " << chi2 << std::endl;

    Res = Amg::VectorX(4);
    Res.setZero();
    for (int i = 0; i < 4; ++i) { Res[i] = Resi(i, 0); }
    ...
    if (toobig && m_debug) {
        std::cout << " SL Pull too BIG "
                  << " rad pos1 " << rs << " rad pos2 " << rse << " ang1 " << thetas << " ang2 " << thetase << std::endl;
    ...
    if (m_debug)
        std::cout << " Fit2SegmentsSL: Residual 0 " << Res[0] << " pull 0 " << Pull[0] << " Residual 1 " << Res[1] << " pull 1 " << Pull[1]
                  << std::endl;
    if (m_debug)
        std::cout << " Fit2SegmentsSL: Residual 2 " << Res[2] << " pull 2 " << Pull[2] << " Residual 3 " << Res[3] << " pull 3 " << Pull[3]
                  << std::endl;
}

void Muon::MuonCurvedSegmentCombiner::fit2Segments(Muon::MuonSegment& seg, Muon::MuonSegment& sege, Amg::VectorX& Res, Amg::VectorX& Pull,
                                                   SegInfoMap segInfoMap) const {
    ...
    // calculate residuals and chi2
    for (int i = 0; i < 5; ++i) { Resi(i, 0) = Model(i, 0) * T(0, 0) + Model(i, 1) * T(1, 0) + Model(i, 2) * T(2, 0) - ym(i, 0); }
    ...
    Res = Amg::VectorX(4);
    Res.setZero();
    for (int i = 1; i < 5; ++i) { Res[i - 1] = Resi(i, 0); }
    ...
    if (m_debug)
        std::cout << " Fit2Segments: Residual 0 " << Res[0] << " pull 0 " << Pull[0] << " Residual 1 " << Res[1] << " pull 1 " << Pull[1]
                  << std::endl;
    if (m_debug)
        std::cout << " Fit2Segments: Residual 2 " << Res[2] << " pull 2 " << Pull[2] << " Residual 3 " << Res[3] << " pull 3 " << Pull[3]
                  << std::endl;
    if (m_debug)
        std::cout << " radius 1  " << ym(1, 0) << " cottan theta 1 " << ym(2, 0) << " radius 2  " << ym(3, 0) << " cottan theta 2 "
                  << ym(4, 0) << std::endl;
    if (m_debug)
        std::cout << " radius fit 1  " << -(Res[0] + ym(1, 0)) << " cottan theta fit 1 " << -(Res[1] + ym(2, 0)) << " radius 2 fit  "
                  << -(Res[2] + ym(3, 0)) << " cottan theta 2 fit " << -(Res[3] + ym(4, 0)) << std::endl;
}
...
void Muon::MuonCurvedSegmentCombiner::fit2SegmentsC(Muon::MuonSegment& seg, Muon::MuonSegment& sege, Amg::VectorX& Res, Amg::VectorX& Pull,
    ...
    for (int iter = 0; iter < 2; ++iter) {
        ...
        if (m_debug)
            std::cout << " Fit2Segments: Residual 0 " << Res[0] << " pull 0 " << Pull[0] << " Residual 1 " << Res[1] << " pull 1 "
                      << Pull[1] << std::endl;
        if (m_debug)
            std::cout << " Fit2Segments: Residual 2 " << Res[2] << " pull 2 " << Pull[2] << " Residual 3 " << Res[3] << " pull 3 "
                      << Pull[3] << std::endl;
    }
}

void Muon::MuonCurvedSegmentCombiner::extrapolateSegment(Muon::MuonSegment& seg, Muon::MuonSegment& sege, Amg::VectorX& Res,
    ...
    // calculate residuals and chi2
    for (int i = 0; i < 3; ++i) { Resi(i, 0) = Model(i, 0) * T(0, 0) + Model(i, 1) * T(1, 0) + Model(i, 2) * T(2, 0) - ym(i, 0); }
    ...
    Res = Amg::VectorX(2);
    Res.setZero();
    for (int i = 1; i < 3; ++i) { Res[i - 1] = ModelE(i, 0) * T(0, 0) + ModelE(i, 1) * T(1, 0) + ModelE(i, 2) * T(2, 0) - ymE(i, 0); }

    Amg::MatrixX ModelET(3, 3);  // Transposed
    ModelET.setZero();
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) { ModelET(i, j) = ModelE(j, i); }
    }
    ...
    if (m_debug)
        std::cout << " Extrapolation: Residual 0 " << Res[0] << " pull 0 " << Pull[0] << " Residual 1 " << Res[1] << " pull 1 " << Pull[1]
                  << std::endl;
}
