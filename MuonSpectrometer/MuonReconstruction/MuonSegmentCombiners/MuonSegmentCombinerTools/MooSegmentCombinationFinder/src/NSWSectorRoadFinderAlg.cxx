...
StatusCode NSWSectorRoadFinderAlg::execute() {
    ...
    if (!mmPrds.isValid()) { ATH_MSG_WARNING("Cannot retrieve MMPrepDataContainer " << m_keyMM.key() << " accessing via collections "); }
    ...
    for (int si = 1; si <= 16; ++si) {
        ...
        if (!chamberData.empty()) {
            patterns->push_back(new Muon::MuonPatternCombination(
                new Trk::Perigee(chamberData.front().intersectPosition(), chamberData.front().intersectDirection(), -1.,
