    ...
    m_mmPrdContainer(0)
{
  ...
  declareProperty("TgcPrepDataContainer", m_key_tgc = "TGC_Measurements");
    // New Small Wheel
  declareProperty("StgcPrepDataContainer", m_key_stgc = "STGC_Measurements");
  declareProperty("MmPrepDataContainer", m_key_mm = "MM_Measurements");
...
void MuonSeededSegmentFinder::extractsTgcPrdCols( const std::set<IdentifierHash>& chIdHs,
  ...
  if ( evtStore()->contains<Muon::sTgcPrepDataContainer>(m_key_stgc) ) {
    if (evtStore()->retrieve(m_stgcPrdContainer, m_key_stgc).isFailure()) {
      ATH_MSG_WARNING("Cannot retrieve sTgcPrepDataContainer " << m_key_stgc);
      return;
    }
  } else return;
  ...
  std::set<IdentifierHash>::const_iterator chit_end = chIdHs.end();
  for ( ; chit != chit_end; ++chit ) {
    sTgcPrepDataContainer::const_iterator colIt = m_stgcPrdContainer->indexFind(*chit);
    if ( colIt == m_stgcPrdContainer->end() || (*colIt)->empty() ) {
      //ATH_MSG_DEBUG(" TgcPrepDataCollection for:   "
      //       << m_idHelper->toStringChamber(*chit) << "  not found in container ");
      continue;
    }
...
// Temp fix because Region selector is not implemented  
     sTgcPrepDataContainer::const_iterator colIt     = m_stgcPrdContainer->begin();
     sTgcPrepDataContainer::const_iterator colIt_end = m_stgcPrdContainer->end();
...
void MuonSeededSegmentFinder::extractMMPrdCols( const std::set<IdentifierHash>& chIdHs,
  ...
  if ( evtStore()->contains<Muon::MMPrepDataContainer>(m_key_mm) ) {
    if (evtStore()->retrieve(m_mmPrdContainer, m_key_mm).isFailure()) {
      ATH_MSG_WARNING("Cannot retrieve mmPrepDataContainer " << m_key_mm);
      return;
    }
  } else return;
  ...
  std::set<IdentifierHash>::const_iterator chit_end = chIdHs.end();
  for ( ; chit != chit_end; ++chit ) {
    MMPrepDataContainer::const_iterator colIt = m_mmPrdContainer->indexFind(*chit);
    if ( colIt == m_mmPrdContainer->end() || (*colIt)->empty() ) {
      //ATH_MSG_DEBUG(" TgcPrepDataCollection for:   "
      //       << m_idHelper->toStringChamber(*chit) << "  not found in container ");
      continue;
    }
  ...
  if(chIdHs.size()==0) {
// Temp fix because Region selector is not implemented  
     MMPrepDataContainer::const_iterator colIt     = m_mmPrdContainer->begin();
     MMPrepDataContainer::const_iterator colIt_end = m_mmPrdContainer->end();
     for ( ; colIt != colIt_end; ++colIt ) {
       if((*colIt)->size()>0) {  
         ATH_MSG_DEBUG(" Adding ALL Prds for:   "
                  << m_idHelper->toStringChamber( (*colIt)->front()->identify() ) << "  size "
