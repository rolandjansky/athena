...
const sTgcPrepDataCollection* MuonChamberHoleRecoveryTool::findStgcPrdCollection( const Identifier& detElId ) const {
  ...
  // m_idHelperTool->stgcIdHelper().get_detectorElement_hash(detElId,hash_id );

  sTgcPrepDataContainer::const_iterator colIt = m_stgcPrdContainer->indexFind(hash_id);
  if ( colIt == m_stgcPrdContainer->end() ) {
...
const MMPrepDataCollection* MuonChamberHoleRecoveryTool::findMmPrdCollection( const Identifier& detElId ) const {
  ...
  // m_idHelperTool->mmIdHelper().get_detectorElement_hash(detElId,hash_id );

  MMPrepDataContainer::const_iterator colIt = m_mmPrdContainer->indexFind(hash_id);
  if ( colIt == m_mmPrdContainer->end() ) {
