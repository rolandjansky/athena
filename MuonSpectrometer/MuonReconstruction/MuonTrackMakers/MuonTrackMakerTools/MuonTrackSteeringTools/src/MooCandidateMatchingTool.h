...
class MooCandidateMatchingTool : virtual public IMuonTrackSegmentMatchingTool,
                                 public AthAlgTool {
 public:
  ...
  static const InterfaceID& interfaceID() {
    static const InterfaceID IID_MooCandidateMatchingTool(
        "Muon::MooCandidateMatchingTool", 1, 0);
    return IID_MooCandidateMatchingTool;
  }

  /** default AlgTool constructor */
  MooCandidateMatchingTool(const std::string&, const std::string&,
                           const IInterface*);
  ...
  bool match(const MuPatSegment& entry1, const MuPatSegment& entry2,
             bool useTightCuts = false) const;

  /** @brief match a segment entry with a track entry */
  bool match(const MuPatTrack& entry1, const MuPatSegment& entry2,
             bool useTightCuts = false) const;

  /** @brief match a segment entry with a base entry */
  bool match(const MuPatCandidateBase& entry1, const MuPatSegment& entry2,
             bool useTightCuts = false) const;

  /** @brief match a track with a segment */
  virtual bool match(const Trk::Track& track, const MuonSegment& segment,
                     bool useTightCuts) const override;

  /** @brief calculate the info needed for the matching decision */
  void calculateTrackSegmentMatchResult(const MuPatTrack& entry1,
                                        const MuPatSegment& entry2,
                                        MooTrackSegmentMatchResult& info) const;

  TrackSegmentMatchCuts getMatchingCuts(const MuPatTrack& entry1,
                                        const MuPatSegment& entry2,
                                        bool useTightCuts) const;

  bool applyTrackSegmentCuts(MooTrackSegmentMatchResult& info,
                             const TrackSegmentMatchCuts& cuts) const;
  ...
  /** @brief check whether two segments are on the same side of the point of
     ...
     calorimeter
  */
  bool sameSide(const MuPatSegment& entry1, const MuPatSegment& entry2,
  ...
  /** @brief Check whether the track and segment are on the same side of the
     ...
     calorimeter
  */
  bool sameSide(const MuPatTrack& entry1, const MuPatSegment& entry2,
                bool requireSameSideOfPerigee) const;
  ...
  /** check whether the two tracks are on the same side of the point of closest
     ...
     calorimeter
  */
  bool sameSide(const MuPatTrack& entry1, const MuPatTrack& entry2,
                bool requireSameSideOfPerigee) const;
  ...
  /** check whether two positions are on the same side of the point of closest
     ...
     calorimeter
  */
  bool sameSide(const Amg::Vector3D& pos1, const Amg::Vector3D& pos2,
                bool requireSameSideOfPerigee) const;
  ...
  /** check whether two positions are on the same side of the point of closest
     ...
     calorimeter
  */
  bool sameSide(const Amg::Vector3D& dir, const Amg::Vector3D& pos1,
                const Amg::Vector3D& pos2, bool requireSameSideOfPerigee) const;

  /** @brief extract Idenfitiers from a vector of measurements and copy them
   * into a set */
  void getIdentifierSet(
  ...
  bool checkPhiHitConsistency(const MuPatCandidateBase& entry1,
                              const MuPatCandidateBase& entry2) const;

  /** @brief evaluate distance between two segments, if too large return false
   * (cut at 3000.).  */
  bool checkSegmentDistance(const MuPatSegment& entry1,
                            const MuPatSegment& entry2) const;

  /** @brief return whether the 2 segments are in neighbouring chambers */
  bool areInNeighbouringChambers(const MuPatSegment& seg1,
                                 const MuPatSegment& seg2) const;
 ...
 private:
  ...
  ToolHandle<Trk::IExtrapolator> m_slExtrapolator{
      this, "SLExtrapolator", "Trk::Extrapolator/MuonStraightLineExtrapolator",
  ...
  Gaudi::Property<bool> m_doTrackSegmentMatching{
      this, "DoTrackSegmentMatching", false,
      "Apply dedicated track-segment matching"};
