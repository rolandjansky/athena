...
namespace {
    ...
    public:
        ...
        ROOT::Math::IBaseFunctionMultiDim* Clone() const { return new MyFCN(m_alpha, m_intercept); }
...
Muon::ConstraintAngleMMClusterBuilderTool::ConstraintAngleMMClusterBuilderTool(const std::string& t, const std::string& n,
                                                                               const IInterface* p) :
    AthAlgTool(t, n, p) {
...
StatusCode Muon::ConstraintAngleMMClusterBuilderTool::sortHitsToLayer(const std::vector<Muon::MMPrepData>& MMprds,
...
StatusCode Muon::ConstraintAngleMMClusterBuilderTool::scanLayer(const std::vector<Muon::MMPrepData>& mmPrdsPerLayer,
    ...
    for (const auto& prd : mmPrdsPerLayer) {
        ATH_MSG_DEBUG("Hit channel " << m_idHelperSvc->mmIdHelper().channel(prd.identify()) << " local positionX "
                                     << prd.localPosition().x() << " time " << prd.time() << " angle to IP"
    ...
    for (uint i_cluster = 0; i_cluster < idxClusters.size(); i_cluster++) {
        for (const auto& idx : idxClusters.at(i_cluster)) {
            double stripPointingAlpha =
                std::atan(mmPrdsPerLayer.at(idx).globalPosition().perp() / std::abs(mmPrdsPerLayer.at(idx).globalPosition().z()));
    ...
    for (const auto& theta : clusterTheta) { ATH_MSG_DEBUG("cluster theta " << theta << " " << theta / Gaudi::Units::degree); }
    ...
    for (uint i_cluster = 0; i_cluster < idxClusters.size(); i_cluster++) {
        ...
        for (double probeIntercept = idealIntercept - interceptWindow; probeIntercept <= idealIntercept + interceptWindow;
             probeIntercept += interceptSteps) {
            ...
            for (const auto& idx : idxClusters.at(i_cluster)) {
                ...
                ATH_MSG_VERBOSE("selection: x " << x << " y " << y << " fUpper(x) " << fUpper->Eval(x) << " fLower(x) " << fLower->Eval(x)
                                                << " ylower " << y - m_nSigmaSelection * sy << "yupper" << y + m_nSigmaSelection * sy);
                // select points which are within x sigma of the road
                if (x <= 0)
                    ATH_MSG_VERBOSE("x<=0: " << fUpper->Eval(x) - y << " " << y - fLower->Eval(x) << " x*sigma" << m_nSigmaSelection * sy);
                if (x > 0)
                    ATH_MSG_VERBOSE("x>0: " << y - fUpper->Eval(x) << " " << fLower->Eval(x) - y << " x*sigma " << m_nSigmaSelection * sy);
                if ((x <= 0 && fUpper->Eval(x) - y < m_nSigmaSelection * sy /*negativ diff means point is above line*/ &&
                     y - fLower->Eval(x) < m_nSigmaSelection * sy) ||
...
StatusCode Muon::ConstraintAngleMMClusterBuilderTool::fitCluster(const std::vector<Muon::MMPrepData>& prdPerLayer,
                                                                 const std::vector<uint>& idxCluster, const double& clusterTheta,
                                                                 std::vector<std::unique_ptr<Muon::MMPrepData>>& clustersVec) const {
    ...
    double meanX = 0;
    for (auto const& idx : idxCluster) { meanX += prdPerLayer.at(idx).localPosition().x(); }
    ...
    if (m_writeStripProperties) { stripsOfClusterChannels.reserve(nStrips); }
    ...
    if (std::abs(ffit->GetParameter(0) - clusterTheta) > m_fitAngleCut * Gaudi::Units::degree) {
        return StatusCode::FAILURE;
    };  // very loose cut for now
    ...
    Amg::Vector2D localClusterPositionV(clusterPos,
                                        prdPerLayer.at(idxCluster.at(0)).localPosition().y());  // y position is the same for all strips

    std::unique_ptr<MMPrepData> prdN =
        std::make_unique<MMPrepData>(prdPerLayer.at(idxCluster.at(0)).identify(), prdPerLayer.at(idxCluster.at(0)).collectionHash(),
                                     localClusterPositionV, stripsOfCluster, std::move(covN), prdPerLayer.at(0).detectorElement(),
                                     (short int)0, std::accumulate(stripsOfClusterCharges.begin(), stripsOfClusterCharges.end(), 0),
                                     driftDist, stripsOfClusterChannels, stripsOfClusterTimes, stripsOfClusterCharges);
...
StatusCode Muon::ConstraintAngleMMClusterBuilderTool::getCalibratedClusterPosition(const Muon::MMPrepData* , std::vector<NSWCalib::CalibratedStrip>&, const float ,
