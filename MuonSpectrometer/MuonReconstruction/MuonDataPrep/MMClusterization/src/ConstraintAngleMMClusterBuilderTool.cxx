...
const{
    ...
    int gasGap = m_muonIdHelperTool->mmIdHelper().gasGap(mmPrdsPerLayer.at(0).identify());

    ATH_MSG_DEBUG("Scanning gas gap "<< gasGap);
    for (const auto &prd:mmPrdsPerLayer){
        ATH_MSG_DEBUG("Hit channel "<< m_muonIdHelperTool->mmIdHelper().channel(prd.identify())
                       ...
                       << " stationEta " <<m_muonIdHelperTool->mmIdHelper().stationEta(prd.identify()) 
                       << " stationPhi "<< m_muonIdHelperTool->mmIdHelper().stationEta(prd.identify())
                       << " multilayer " <<m_muonIdHelperTool->mmIdHelper().multilayer(prd.identify()));
    }
    ...
    for(uint i_strip=1; i_strip<mmPrdsPerLayer.size();i_strip++){
        if(std::abs(m_muonIdHelperTool->mmIdHelper().channel(mmPrdsPerLayer.at(i_strip).identify()) -
                m_muonIdHelperTool->mmIdHelper().channel(mmPrdsPerLayer.at(i_strip-1).identify()))<nMaxMissingStrips){
