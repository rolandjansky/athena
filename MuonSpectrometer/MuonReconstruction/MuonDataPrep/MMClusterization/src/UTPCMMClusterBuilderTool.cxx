    ...
    m_mmIdHelper(nullptr)
{
    ...
    declareProperty("HoughSelectionCut",m_selectionCut=0.7);
...
StatusCode Muon::UTPCMMClusterBuilderTool::getClusters(std::vector<Muon::MMPrepData>& MMprds, std::vector<Muon::MMPrepData*>& clustersVect){
    ...
    for(const auto& MMprdsOfLayer:MMprdsPerLayer){
        ...
        for( auto MMprd: MMprdsOfLayer){
            ...
            stripsTime.push_back(MMprd.time()-tof); // use time minus tof minus vmm integration time as actual time
            ATH_MSG_DEBUG("Hit channel: "<< m_mmIdHelper->channel(id_prd) <<" time "<< MMprd.time() << " localPosX "<< MMprd.localPosition().x() << " tof "<<tof <<" angleToIp " << angleToIp<<" gas_gap "<< m_mmIdHelper->gasGap(id_prd) << " multiplet " << m_mmIdHelper->multilayer(id_prd) << " stationname " <<m_mmIdHelper->stationName(id_prd)  << " stationPhi " <<m_mmIdHelper->stationPhi(id_prd) << " stationEta "<<m_mmIdHelper->stationEta(id_prd));
        }
...
StatusCode Muon::UTPCMMClusterBuilderTool::finalFit(std::vector<double>& xpos, std::vector<double>& time, std::vector<int>& idxSelected, double& x0, double &sigmaX0 ,double &fitAngle, double &chiSqProb){
    ...
    for(int idx:idxSelected){
        ...
        fitGraph->SetPointError(fitGraph->GetN()-1,0.425/3.464101615 /*sqrt(12)*/,5*m_vDrift);
    }
    ...
    TFitResultPtr s=fitGraph->Fit("ffit","SM","",xmin-.5,xmax+.5);
    std::cout<< "First fit "<<s <<std::endl;
    s=fitGraph->Fit("ffit","SM"); //redo fit if first one failed
    std::cout<< "Second fit "<<s <<std::endl;
    ...
    if(s!=0){
        ...
        if(ffit->GetParameter(1)<=-11.5) return StatusCode::FAILURE;
        if(ffit->GetParameter(1)>=-0.15) return StatusCode::FAILURE;
    }
    return StatusCode::SUCCESS;
}
