...
StatusCode Muon::ProjectionMMClusterBuilderTool::getClusters(std::vector<Muon::MMPrepData>& MMprds,
    ...
    for (unsigned int i_layer = 0; i_layer < prdsPerLayer.size(); i_layer++) {
        std::sort(prdsPerLayer.at(i_layer).begin(), prdsPerLayer.at(i_layer).end(), [this](const MMPrepData& a, const MMPrepData& b) {
    ...
    for (const auto& prdsOfLayer : prdsPerLayer) {
        ...
        for (const auto& prd : prdsOfLayer) {
            ...
            ATH_MSG_DEBUG("Hit channel: " << m_idHelperSvc->mmIdHelper().channel(id_prd) << " time " << prd.time() << " drift dist "
                                          << prd.driftDist() << " localPosX " << prd.localPosition().x() << " angleToIp " << angleToIp
        ...
        while (true) {
            ...
            if (sc.isFailure()) {
                ATH_MSG_DEBUG("Fine scan failed");
                break;
            }  // returns FAILURE if less then three strips in a cluster
...
StatusCode Muon::ProjectionMMClusterBuilderTool::calculateCorrection(const std::vector<Muon::MMPrepData>& prdsOfLayer,
                                                                     std::vector<double>& v_posxc, std::vector<double>& v_cor) const {
    for (const auto& prd : prdsOfLayer) {
        ...
        cor *= (m_idHelperSvc->mmIdHelper().gasGap(prd.identify()) % 2 == 1
                    ? 1
                    : -1);  // correct for the local drift time directions. Gas gap 2 and 4 have "negativ" time, therefore correction is
                            // applied negative

        ATH_MSG_DEBUG("globalR: " << prd.globalPosition().perp() << " drift dist: " << tm - m_tmin - m_tOffset
                                  << "globalZ: " << prd.globalPosition().z()
                                  << " R/Z: " << 1.0 * prd.globalPosition().perp() / std::fabs(prd.globalPosition().z()));
...
StatusCode Muon::ProjectionMMClusterBuilderTool::doPositionCalculation(std::vector<double>& v_posxc, const std::vector<double>& v_cor,
                                                                       const std::vector<int>& idx_selected, double& xmean,
                                                                       double& xmeanErr, double& qtot,
...
StatusCode Muon::ProjectionMMClusterBuilderTool::writeNewPrd(std::vector<std::unique_ptr<Muon::MMPrepData>>& clustersVect, double xmean,
                                                             double xerr, double qtot, const std::vector<int>& idx_selected,
                                                             const std::vector<MMPrepData>& prdsOfLayer) const {
    ...
    Amg::Vector2D localClusterPositionV(xmean, prdsOfLayer.at(idx).localPosition().y());  // y position is the same
                                                                                          // for all strips
    ...
    std::unique_ptr<Muon::MMPrepData> prdN =
        std::make_unique<MMPrepData>(prdsOfLayer.at(idx).identify(), prdsOfLayer.at(idx).collectionHash(), localClusterPositionV,
                                     stripsOfCluster, std::move(covN), prdsOfLayer.at(idx).detectorElement(), (short int)int(meanTime),
                                     int(qtot), driftDist, stripsOfClusterStripNumber, stripsOfClusterDriftTime, stripsOfClusterCharge);
...
StatusCode Muon::ProjectionMMClusterBuilderTool::getCalibratedClusterPosition(const Muon::MMPrepData* , std::vector<NSWCalib::CalibratedStrip>&, const float ,
                                                Amg::Vector2D& , Amg::MatrixX& ) const {return StatusCode::SUCCESS;}