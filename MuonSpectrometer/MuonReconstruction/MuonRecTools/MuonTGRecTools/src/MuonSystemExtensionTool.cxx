...
bool
MuonSystemExtensionTool::initializeGeometry()
{
...
bool
MuonSystemExtensionTool::initializeGeometryEndcap(int sector, MuonStationIndex::DetectorRegionIndex regionIndex,
                                                  const Amg::AngleAxis3D& sectorRotation)
{
    ...
    for (std::vector<MuonStationIndex::StIndex>::const_iterator stLayer = layers.begin(); stLayer != layers.end();
         ++stLayer)
    {
        ...
        Amg::Transform3D* trans =
            new Amg::Transform3D(sectorRotation);  //*Amg::AngleAxis3D(xToZRotation,Amg::Vector3D(0.,1.,0.)
        ...
        if (msgLvl(MSG::VERBOSE)) {
            for (unsigned int test = 0; test < 2; ++test) {
                ...
                ATH_MSG_VERBOSE(" sector " << sector << " layer " << MuonStationIndex::layerName(layer) << " hit x "
                                           << globalPosition.x() << " hit z " << globalPosition.z() << " lpos3d "
                                           << lpos3d.x() << " " << lpos3d.y() << " " << lpos3d.z() << " lpos "
                                           << lpos[Trk::loc1] << " " << lpos[Trk::loc2] << " center "
                                           << surface->center().z() << " normal: phi " << surface->normal().phi()
                                           << " theta " << surface->normal().theta() << " normal: x "
                                           << surface->normal().x() << " y " << surface->normal().y() << " z "
                                           << surface->normal().z());
...
bool
MuonSystemExtensionTool::initializeGeometryBarrel(int sector, const Amg::AngleAxis3D& sectorRotation)
{
    ...
    for (unsigned int stationLayer = MuonStationIndex::BI; stationLayer <= MuonStationIndex::BE; ++stationLayer) {
        ...
        MuonChamberLayerDescriptor   layerDescriptor =
            chamberLayerDescription.getDescriptor(sector, MuonStationIndex::Barrel, layer);
        ...
        if (msgLvl(MSG::VERBOSE)) {
            ...
            ATH_MSG_VERBOSE(" sector " << sector << " layer " << MuonStationIndex::layerName(layer) << " phi "
                                       << sectorPhi << " ref theta " << globalPosition.theta() << " phi "
                                       << globalPosition.phi() << " r " << globalPosition.perp() << " pos "
                                       << globalPosition.x() << " " << globalPosition.y() << " " << globalPosition.z()
                                       << " lpos3d " << lpos3d.x() << " " << lpos3d.y() << " " << lpos3d.z()
                                       << " normal: x " << surface->normal().x() << " y " << surface->normal().y()
                                       << " z " << surface->normal().z());
...
bool
MuonSystemExtensionTool::muonSystemExtension(const xAOD::TrackParticle&  indetTrackParticle,
                                             const MuonSystemExtension*& muonSystemExtention) const
{
    ...
    for (; it != it_end; ++it) {
        ...
        if (msgLvl(MSG::VERBOSE)) {
            ATH_MSG_VERBOSE(" startPars: phi pos "
                            << currentPars->position().phi() << " direction phi " << currentPars->momentum().phi()
                            << " theta pos " << currentPars->position().theta() << " theta "
                            << currentPars->momentum().theta() << " r " << currentPars->position().perp() << " z "
                            << currentPars->position().z() << " momentum " << currentPars->momentum().mag() << " local "
                            << currentPars->parameters()[Trk::locX] << " " << currentPars->parameters()[Trk::locY]);
            ...
            ATH_MSG_VERBOSE(" destination: sector "
                            << it->sector << "  " << MuonStationIndex::regionName(it->regionIndex) << "  "
        ...
        const Trk::TrackParameters* exPars =
            m_extrapolator->extrapolate(*currentPars, surface, Trk::alongMomentum, false, Trk::muon);
        if (!exPars) {
    ...
    ATH_MSG_DEBUG(" completed extrapolation: destinations " << surfaces.size() << " intersections "
                                                            << intersections.size());

    if (intersections.empty()) return false;

    MuonSystemExtension* theExtension = new MuonSystemExtension(
        std::unique_ptr<const Trk::TrackParameters>(caloExtension->muonEntryLayerIntersection()->clone()),
...
MuonSystemExtensionTool::SurfaceVec
MuonSystemExtensionTool::getSurfacesForIntersection(const Trk::TrackParameters& muonEntryPars) const
{
    ...
    auto sortFunction =
        (regionIndex == MuonStationIndex::Barrel)
