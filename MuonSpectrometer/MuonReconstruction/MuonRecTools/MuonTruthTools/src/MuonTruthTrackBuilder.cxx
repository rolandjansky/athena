...
StatusCode
MuonTruthTrackBuilder::initialize()
{
...
StatusCode
MuonTruthTrackBuilder::finalize()
{
    ATH_MSG_INFO("Finalizing ...");
    double scaleT = m_truth != 0 ? 1. / m_truth : 1;
    ATH_MSG_INFO(" Tracks: " << m_truth << " fits " << m_trackFits << " good " << m_goodTrackFits << " cleaned "
                             << m_trackCleaned << " failed " << m_failedTrackFits << " cleaning "
                             << m_failedTrackCleaning << std::endl
                             << " fractions: fit " << scaleT * m_trackFits << " good " << scaleT * m_goodTrackFits
                             << " cleaned " << scaleT * m_trackCleaned << " failed " << scaleT * m_failedTrackFits
                             << " cleaning " << scaleT * m_failedTrackCleaning);
    scaleT = m_truthSL != 0 ? 1. / m_truthSL : 1;
    ATH_MSG_INFO(" SL Tracks: " << m_truthSL << " fits " << m_trackSLFits << " good " << m_goodTrackSLFits
                                << " cleaned " << m_trackCleanedSL << " failed " << m_failedTrackSLFits << " cleaning "
                                << m_failedTrackCleaningSL << std::endl
                                << " fractions: fit " << scaleT * m_trackSLFits << " good "
                                << scaleT * m_goodTrackSLFits << " cleaned " << scaleT * m_trackCleanedSL << " failed "
                                << scaleT * m_failedTrackSLFits << " cleaning " << scaleT * m_failedTrackCleaningSL);
    for (int i = 0; i < (int)MuonStationIndex::StIndexMax; ++i) {
        ...
        ATH_MSG_INFO(" Layer " << MuonStationIndex::stName(stIndex) << " truth: " << m_nallSegments[stIndex] << " fits "
                               << m_nsegmentFits[stIndex] << " good " << m_ngoodSegmentFits[stIndex] << " cleaned "
                               << m_nsegmentCleaned[stIndex] << " fractions: fit " << scaleT * m_nsegmentFits[stIndex]
                               << " good " << scaleT * m_ngoodSegmentFits[stIndex] << " failed "
                               << scaleT * m_nfailedSegmentFits[stIndex] << " cleaning "
                               << scaleT * m_nfailedSegmentCleaning[stIndex]);
    }
...
MuonTruthTrackBuilder::getEntryRecord(const HepMC::GenParticle& genPart) const
{
    ...
    for (; it != it_end; ++it) {
        ...
        if (barcode == genPart.barcode()) {
            ...
            ATH_MSG_VERBOSE("Found muon in track record: bc " << barcode << " pt " << (*it).GetMomentum().perp()
                                                              << " eta " << (*it).GetMomentum().eta());
        }
    }
...
Trk::Track*
MuonTruthTrackBuilder::createTrack(const Trk::PRD_TruthTrajectory& prdTraj, Trk::SegmentCollection* segs) const
{
    ...
    Amg::Vector3D startPos(trackRecord->GetPosition().x(), trackRecord->GetPosition().y(),
                           trackRecord->GetPosition().z());
    Amg::Vector3D startMom(trackRecord->GetMomentum().x(), trackRecord->GetMomentum().y(),
    ...
    std::map<const Trk::TrkDetElementBase*,
             std::pair<std::list<const Trk::PrepRawData*>, std::list<const Trk::PrepRawData*> > >
                 clustersPerDetEl;
    ...
    for (; pit != pit_end; ++pit) {
        ...
        if (m_idHelperSvc->isMdt(id)) {
            ...
            ATH_MSG_DEBUG("Adding r " << gpos->perp() << " z " << gpos->z() << "  " << m_idHelperSvc->toString(id)
                                      << " " << residual << " pull " << pull);
            ...
            if (m_buildCompRots) {
                if (m_idHelperSvc->measuresPhi(id))
                    clustersPerDetEl[cl->detectorElement()].first.push_back(cl);
                else
                    clustersPerDetEl[cl->detectorElement()].second.push_back(cl);
            } else {
                const MuonClusterOnTrack* rot =
                    m_muonClusterCreator->createRIO_OnTrack(*cl, prd.detectorElement()->surface(id).center());
                if (!rot) {
                    ATH_MSG_WARNING(" ROT creation failed " << m_idHelperSvc->toString(id));
                    continue;
                }
    ...
    if (m_buildCompRots) {
        std::map<const Trk::TrkDetElementBase*,
                 std::pair<std::list<const Trk::PrepRawData*>, std::list<const Trk::PrepRawData*> > >::const_iterator
        ...
        for (; cit != cit_end; ++cit) {
            ...
            DetectorLayer&            detLayer = hitsPerLayer[stIndex];
            detLayer.isEndcap                  = isEndcap;
            ATH_MSG_VERBOSE("  " << m_idHelperSvc->toString(id) << " phi " << cit->second.first.size() << " eta "
            ...
            if (!cit->second.first.empty()) {
                if (cit->second.first.size() == 1) {
                    ...
                    if (cl)
                        measPhi = m_muonClusterCreator->createRIO_OnTrack(
                            *cl, cl->detectorElement()->surface(cl->identify()).center());
                } else {
            ...
            if (!cit->second.second.empty()) {
                if (cit->second.second.size() == 1) {
                    ...
                    if (cl)
                        measEta = m_muonClusterCreator->createRIO_OnTrack(
                            *cl, cl->detectorElement()->surface(cl->identify()).center());
                } else
                    measEta = m_muonCompRotCreator->createBroadCluster(
                        std::list<const Trk::PrepRawData*>(cit->second.second), 0);
            }
            if (measPhi) {
            ...
            ATH_MSG_VERBOSE(" Adding compRot for  " << m_idHelperSvc->toStringDetEl(id) << " phi "
                                                    << cit->second.first.size() << " eta "
                                                    << cit->second.second.size());
        }
    }
    ...
    // insert the detector layers into a vector and sort them according to r or z depending whether we are in the barrel
    // or endcap
    ...
    if (measurements.size() < 6) {
        ...
        if (extractMeasurements(per, measurements, finalMeasurements, -1, -1, useEndcap, 400.)) {
            ...
            if ((nlayers == 1 && hasEM) || (useEndcap && !hasInner)) {
                ...
                std::map<MuonStationIndex::StIndex, DetectorLayer>::iterator pos =
                    hitsPerLayer.find(MuonStationIndex::EM);
...
void
MuonTruthTrackBuilder::sortLayer(MuonTruthTrackBuilder::DetectorLayer& layer) const
{
    ...
    if (msgLvl(MSG::VERBOSE)) {
        if (layer.minPos < 1e8)
            msg(MSG::VERBOSE) << " first " << layer.first3D.perp() << " z " << layer.first3D.z() << std::endl;
        if (layer.maxPos > -1e8)
            msg(MSG::VERBOSE) << " second " << layer.last3D.perp() << " z " << layer.last3D.z() << std::endl;
...
void
MuonTruthTrackBuilder::createSegments(MuonTruthTrackBuilder::DetectorLayer& layer, Trk::SegmentCollection& segs) const
{
    ...
    ATH_MSG_VERBOSE(" creating segments Layer: meas " << layer.meas.size() << " nphi " << layer.nphi << " nmdt "
                                                      << layer.nmdtL + layer.nmdtS << " nsw " << layer.nnsw << " "
                                                      << m_printer->print(per) << " trials " << nseg);
    ...
    for (int i = 0; i < nseg; ++i) {
        ...
        if (seg) {
            ...
            if (seg->fitQuality() && seg->fitQuality()->numberDoF() != 0
                && seg->fitQuality()->chiSquared() / seg->fitQuality()->numberDoF() < 3)
            {
...
MuonTruthTrackBuilder::createPseudo(const Trk::TrackParameters& pars, const Trk::MeasurementBase& meas) const
{
    double ly = 0.;
    if (meas.associatedSurface().isOnSurface(pars.position(), false)) {
        ...
        if (!measPars) {
            ATH_MSG_WARNING("Failed to reach measurement, cannot create PseudoMeasurement, z_meas "
                            << meas.globalPosition().z() << " z_pars " << pars.position().z() << " p "
                            << pars.momentum().mag());
            return 0;
        }
    ...
    ATH_MSG_VERBOSE("Creating pseudo: r " << meas.globalPosition().perp() << " z " << meas.globalPosition().z()
                                          << " local position " << ly);
    return new Trk::PseudoMeasurementOnTrack(Trk::LocalParameters(Trk::DefinedParameter(ly, Trk::locY)), cov,
                                             meas.associatedSurface());
}

MuonSegment*
MuonTruthTrackBuilder::createSegment(const Trk::TrackParameters& /*pars*/,
                                     std::vector<const Trk::MeasurementBase*>& measurements) const
{
...
MuonTruthTrackBuilder::retrieveTruthCollection(std::string colName) const
{
...
MuonTruthTrackBuilder::retrieveCscTruthCollection(std::string colName) const
{
    // Retrieve SDO map for this event
...
const MuonSimData::Deposit*
MuonTruthTrackBuilder::getDeposit(const MuonSimDataCollection& simCol, const HepMC::GenParticle& genPart,
...
bool
MuonTruthTrackBuilder::extractMeasurements(const Trk::TrackParameters&                     per,
                                           const std::vector<const Trk::MeasurementBase*>& hitsIn,
                                           std::vector<const Trk::MeasurementBase*>& hitsOut, int splitSL,
                                           int splitNSWEI, bool isEndcap, double phiDist) const
{
    int                         nprec     = 0;
    int                         ntrigEta  = 0;
    ...
    for (; it != it_end; ++it) {
        ...
        if (!m_usePadPhiHits && m_idHelperSvc->issTgc(id) && m_idHelperSvc->stgcIdHelper().channelType(id) == 0)
            measPhi = false;
    ...
    if (fabs(dist) < phiDist) {
        ...
        if (firstPhi) {
            ...
            double posLastPhi =
                lastPhi ? (isEndcap ? lastPhi->globalPosition().z() : lastPhi->globalPosition().perp()) : posFirstPhi;
            ...
            // std::cout << " pos phif " << posFirstPhi << " last " << posLastPhi << " measf " << posFirstMeas << " last
            // " << posLastMeas
