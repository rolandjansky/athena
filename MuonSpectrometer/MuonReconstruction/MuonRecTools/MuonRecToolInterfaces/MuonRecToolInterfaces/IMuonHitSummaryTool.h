...
namespace Muon {
  ...
  public:
    ...
    struct HitSummary {
      HitSummary() : nprecisionHits(0), nprecisionGoodHits(0), netaTriggerLayers(0), nphiLayers(0), netaPhiLayers(0),
	             nprecisionHoles(0), netaTriggerHoleLayers(0), nphiHoleLayers(0), nprecisionOutliers(0), nprecisionCloseHits(0), noutBoundsHits(0) {}
    ...
    struct CompactSummary {
      CompactSummary() : mainSector(-1),nprecisionLayers(0),nphiLayers(0),ntrigEtaLayers(0),
	                 nprecisionHoleLayers(0), ntrigEtaHoleLayers(0), nphiHoleLayers(0), nprecisionGoodLayers(0) {}
  ...
  inline const InterfaceID& IMuonHitSummaryTool::interfaceID()
  {
    return IID_IMuonHitSummaryTool;
  }
  ...
  inline std::string IMuonHitSummaryTool::CompactSummary::dump() const {
    ...
    for( std::set<MuonStationIndex::PhiIndex>::const_iterator it=phiLayers.begin();it!=phiLayers.end();++it ) sout << " " << MuonStationIndex::phiName(*it);
    return sout.str();
  }
