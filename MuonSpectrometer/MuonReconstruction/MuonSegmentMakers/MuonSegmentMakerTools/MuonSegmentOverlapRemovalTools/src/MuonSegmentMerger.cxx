...
namespace Muon {
    ...
    MuonSegmentMerger::MuonSegmentMerger(const std::string& ty, const std::string& na, const IInterface* pa) : AthAlgTool(ty, na, pa) {
    ...
    MuonSegmentMerger::SegVec MuonSegmentMerger::findDuplicates(const MuonSegmentMerger::SegVec& segments) const {
    ...
    std::vector<MuonSegmentMerger::SegVec> MuonSegmentMerger::resolveClusterOverlaps(const MuonSegmentMerger::SegVec& segments) const {
                ...
                if (clot) {
                    ...
                    if (cl) {
                        ...
                        if (rpc) {
                            ...
                            if (tgc) { tgcsPerCollection[tgc->collectionHash()].push_back(tgc); }
                        }
                    }
                }
        ...
        if (!rpcsPerCollection.empty()) {
            ...
            for (; rcit != rcit_end; ++rcit) {
                ...
                if (!rpcClustering.cluster(rcit->second)) { continue; }
        ...
        if (!tgcsPerCollection.empty()) {
            ...
            int offset = 0;
            for (; rcit != rcit_end; ++rcit) {
                if (!tgcClustering.cluster(rcit->second)) { continue; }
