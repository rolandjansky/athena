...
namespace Muon {
  ...
  MuonPatternCalibration::MuonPatternCalibration(const std::string& t,const std::string& n,const IInterface* p)  :  
    ...
    m_rpcPrdContainer(nullptr),
    m_tgcPrdContainer(nullptr)
  {
  ...
  for (; pit!=(*it).prepRawDataVec().end(); ++pit)
    {
      ...
      if( m_idHelperTool->isRpc(id) && m_idHelperTool->rpcIdHelper().measuresPhi(id) ) {hasPhiMeasurements = true; break;}
      else if( m_idHelperTool->isTgc(id) && m_idHelperTool->tgcIdHelper().isStrip(id) ) {hasPhiMeasurements = true; break;}
      else if( m_idHelperTool->isCsc(id) && m_idHelperTool->cscIdHelper().measuresPhi(id) ) {hasPhiMeasurements = true; break;}
    }
      }
      ...
      if( !etaPhiHitsPerChamber.empty() ){
       ...
       for( ;chit!=chit_end;++chit ){
         ...
         if( (hits.neta > 0 && hits.nphi == 0) || (hits.nphi > 0 && hits.neta == 0) ){
           if( m_idHelperTool->isRpc(id) && m_rpcPrdContainer ){
           ...
           }else if( m_idHelperTool->isTgc(id) && m_tgcPrdContainer ){
  ...
  if( measuresPhi){
    ...
    int wire = m_idHelperTool->tgcIdHelper().channel(id);
  ...
  ATH_MSG_VERBOSE(">>>> extrapolated position far outside volume, dropping hit " << m_idHelperTool->toString( id ) << ". dist along strip " << dif.mag() << " 1/2 strip len " << 0.5*striplen
  ...
  ATH_MSG_VERBOSE(">>>> extrapolated position outside volume, shifting position " << m_idHelperTool->toString( id ) << ". position along strip " << dif.mag() << " 1/2 tube len " << 0.5*striplen
  ...
  void MuonPatternCalibration::insertMdt( const MdtPrepData& mdt, RegionMap& regionMap, 
    ...
    if( hasPhiMeasurements ){
      ...
      Amg::Vector3D planenormal = m_idHelperTool->mdtIdHelper().isBarrel(id) ?
  amdbToGlobal.linear()*Amg::Vector3D(0.,0.,1.) : amdbToGlobal.linear()*Amg::Vector3D(0.,1.,0.);
      ...
      double tubelen = detEl->getActiveTubeLength( m_idHelperTool->mdtIdHelper().tubeLayer(id),m_idHelperTool->mdtIdHelper().tube(id) );
      
      if( dif.mag() > 0.5*tubelen ){
  Amg::Vector3D newpos = tubePos - dif*(0.5*tubelen/dif.mag());

  ATH_MSG_VERBOSE(">>>> extrapolated position outside volume, shifting position " << m_idHelperTool->toString( id ) << ". position along strip " << dif.mag() << " 1/2 tube len " << 0.5*tubelen
  ...
  intersect = newpos;
    ...
    if( m_doMultiAnalysis ){
      if( m_idHelperTool->isSmallChamber(id) ){
        ATH_MSG_VERBOSE("Small chamber " << m_idHelperTool->toString( elId ));
  chFlag = 0;
  if( chIndex == MuonStationIndex::BIS ){
    int eta = m_idHelperTool->stationEta(elId);
    if( abs(eta) == 8 ) {
      ATH_MSG_VERBOSE("BIS8 chamber " << m_idHelperTool->toString( elId ));
      chFlag = 3;
    }
  }
      }else{
        ATH_MSG_VERBOSE("Large chamber " << m_idHelperTool->toString( elId ));
  chFlag = 1;
  if( chIndex == MuonStationIndex::BIL ){
    std::string stName = m_idHelperTool->chamberNameString(id);
    if( stName[2] == 'R' ){
      ATH_MSG_VERBOSE("BIR chamber " << m_idHelperTool->toString( elId ));
      chFlag = 2;
    }
  }
      }
      int phi = m_idHelperTool->mdtIdHelper().stationPhi(id);
  ...
  void MuonPatternCalibration::clearRotsPerRegion( IMuonPatternCalibration::ROTsPerRegion& hitsPerRegion ) const 
  {
    ...
    for( ; rit!=rit_end;++rit ){
      ...
      ClusterIt cl_it = rit->clusters.begin();
      ClusterIt cl_it_end = rit->clusters.end();
  ...
  for( ;mdtit!=mdtit_end;++mdtit ){
    ...
    if( !mdt ) {
      ATH_MSG_VERBOSE("Failed to calibrate " << m_idHelperTool->toString(prd->identify()));
      continue;
    }
