...
public:  // Interface methods
    ...
    // Fit a RIO residual.
    virtual void fit_rio_residual(const Trk::PlaneSurface& ssrf, bool dump, const std::vector<const Trk::RIO_OnTrack*>& clus,
                                  unsigned int irclu, double& res, double& dres, double& rs, double& drs  // for 3pt-method
    ) const = 0;

    // Get 2d segments eta/phi seperately
    virtual Muon::MuonSegmentCombination* get2dMuonSegmentCombination(Identifier eta_id, Identifier phi_id,
                                                                      ICscSegmentFinder::ChamberTrkClusters& eta_clus,
                                                                      ICscSegmentFinder::ChamberTrkClusters& phi_clus,
                                                                      const Amg::Vector3D& lpos000, const EventContext& ctx,
                                                                      int etaStat = 0, int phiStat = 0) const = 0;

    virtual Muon::MuonSegmentCombination* get4dMuonSegmentCombination(Identifier eta_id, Identifier phi_id,
                                                                      ICscSegmentFinder::ChamberTrkClusters& eta_clus,
                                                                      ICscSegmentFinder::ChamberTrkClusters& phi_clus,
                                                                      const Amg::Vector3D& lpos000, const EventContext& ctx) const = 0;

    // Get 2d segments eta/phi seperately
    virtual Muon::MuonSegmentCombination* get4dMuonSegmentCombination(const Muon::MuonSegmentCombination* Muon2dSegComb,
                                                                      const EventContext& ctx) const = 0;

    virtual std::unique_ptr<std::vector<std::unique_ptr<Muon::MuonSegment> > > getMuonSegments(
        Identifier eta_id, Identifier phi_id, ICscSegmentFinder::ChamberTrkClusters& eta_clus,
        ICscSegmentFinder::ChamberTrkClusters& phi_clus, const Amg::Vector3D& lpos000, const EventContext& ctx) const = 0;
