...
public:  // Interface methods
    ...
    virtual std::unique_ptr<MuonSegmentCombinationCollection> find(const std::vector<const Muon::CscPrepDataCollection*>& pcols,
                                                                   const EventContext& ctx) const = 0;

    // for 4d SegmentMaker
    virtual std::unique_ptr<MuonSegmentCombinationCollection> find(const MuonSegmentCombinationCollection& segcols,
                                                                   const EventContext& ctx) const = 0;
    /*
...
public:
    ...
    struct sortByNunspoilAndChsq {
        bool operator()(const Segment& seg1, const Segment& seg2) {
            ...
            if (seg1.nunspoil > seg2.nunspoil)
                return true;
            else if (seg1.nunspoil < seg2.nunspoil)
                return false;
