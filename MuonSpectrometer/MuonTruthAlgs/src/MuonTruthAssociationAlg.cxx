...
MuonTruthAssociationAlg::MuonTruthAssociationAlg(const std::string& name, ISvcLocator* pSvcLocator) :
    AthAlgorithm(name, pSvcLocator), m_idHelper("Muon::MuonIdHelperTool/MuonIdHelperTool") {
...
StatusCode MuonTruthAssociationAlg::execute() {
    ...
    //     continue;
    //   }
    //   ATH_MSG_DEBUG("  truthParticleLink status " << (*truthLink)->status() << " barcode " << (*truthLink)->barcode() << " pt "<<
    //   (*truthLink)->pt() << " eta "<< (*truthLink)->eta());
    // }
    //////////////////////////
...
void MuonTruthAssociationAlg::addMuon(const xAOD::TruthParticleContainer& truthParticles, const xAOD::MuonContainer& muons) {
    ...
    for (const auto& muon : muons) {
        ...
        // Each track particle should carry a link to the corresponding truth particle. Then compare this truth particle link with the given
        // truth muon particle
        try {
            ...
            if (truthLink.isValid()) {
                ...
                for (const auto& truthParticle : truthParticles) {
                    ...
                    ATH_MSG_DEBUG("Adding recoMuonLink for truth muon with barcode " << truthParticle->barcode() << " pt "
                                                                                     << truthParticle->pt());
                    ...
                    if (((*truthLink)->barcode()) % m_barcodeOffset == truthParticle->barcode()) {
                        ...
                        ElementLink<xAOD::TruthParticleContainer> muonTruthLink =
                            ElementLink<xAOD::TruthParticleContainer>(truthParticle, truthParticles);
                        muonTruthLink.toPersistent();
                        const_cast<xAOD::Muon&>(*muon).auxdata<ElementLink<xAOD::TruthParticleContainer> >("truthParticleLink") =
                            muonTruthLink;
                        ...
                        if (muon->author() == 1 || muon->author() == 5 || muon->author() == 6) {  // only match hits for muons with MS
                                                                                                  // tracks
                            if (!truthParticle->isAvailable<std::vector<unsigned long long> >("truthMdtHits")) {
                                ATH_MSG_DEBUG("muon with author " << muon->author()
                                                                  << " has no truth hits vector in the truth association alg");
                            } else {
                                std::vector<unsigned long long> mdtTruth =
                                    truthParticle->auxdata<std::vector<unsigned long long> >("truthMdtHits");
                                std::vector<unsigned long long> cscTruth;
                                if (m_idHelper->HasCSC())
                                    cscTruth = truthParticle->auxdata<std::vector<unsigned long long> >("truthCscHits");
                                std::vector<unsigned long long> rpcTruth =
                                    truthParticle->auxdata<std::vector<unsigned long long> >("truthRpcHits");
                                std::vector<unsigned long long> tgcTruth =
                                    truthParticle->auxdata<std::vector<unsigned long long> >("truthTgcHits");
                                ...
                                for (; tsit != tsit_end; ++tsit) {
                                    ...
                                    if (rot)
                                        ...
                                        if (crot) {
                                            if (!crot->containedROTs().empty() && crot->containedROTs().front())
                                    ...
                                    Muon::MuonStationIndex::ChIndex chIndex =
                                        !isTgc ? m_idHelper->chamberIndex(id) : Muon::MuonStationIndex::ChUnknown;
                                    bool found = false;
                                    for (unsigned int i = 0; i < mdtTruth.size(); i++) {
                                        if (id == mdtTruth[i]) {
                                            if (nprecHitsPerChamberLayer[chIndex] == 999)
                                                nprecHitsPerChamberLayer[chIndex] = 1;
                                            else
                                        ...
                                        for (unsigned int i = 0; i < cscTruth.size(); i++) {
                                            if (id == cscTruth[i]) {
                                                if (measPhi) {
                                                    ...
                                                    if (nphiHitsPerChamberLayer[index] == 999)
                                                        nphiHitsPerChamberLayer[index] = 1;
                                                    else
                                    ...
                                    for (unsigned int i = 0; i < rpcTruth.size(); i++) {
                                        if (id == rpcTruth[i]) {
                                            ...
                                            if (measPhi) {
                                                ...
                                                if (ntrigEtaHitsPerChamberLayer[index] == 999)
                                                    ntrigEtaHitsPerChamberLayer[index] = 1;
                                                else
                                    ...
                                    if (nphiHitsPerChamberLayer[i] == 999) {
                                        ...
                                        if (m_idHelper->HasCSC()) {
                                            for (unsigned int j = 0; j < cscTruth.size(); j++) {
                                                Identifier id(cscTruth[j]);
                                                if (m_idHelper->measuresPhi(id)) {
                                                    if (m_idHelper->phiIndex(id) == (Muon::MuonStationIndex::PhiIndex)i) {
                                                        nphiHitsPerChamberLayer[i] = 0;
                                                        found = true;
                                                        break;
                                                    }
                                                }
                                ...
                                const_cast<xAOD::Muon&>(*muon).auxdata<std::vector<unsigned int> >("nprecMatchedHitsPerChamberLayer") =
                                    nprecHitsPerChamberLayer;
                                const_cast<xAOD::Muon&>(*muon).auxdata<std::vector<unsigned int> >("nphiMatchedHitsPerChamberLayer") =
                                    nphiHitsPerChamberLayer;
                                const_cast<xAOD::Muon&>(*muon).auxdata<std::vector<unsigned int> >("ntrigEtaMatchedHitsPerChamberLayer") =
                                    ntrigEtaHitsPerChamberLayer;
                            }
                        }
                        muonLink.toPersistent();
                        const_cast<xAOD::TruthParticle&>(*truthParticle).auxdata<ElementLink<xAOD::MuonContainer> >("recoMuonLink") =
                            muonLink;
                        break;
                    }
                }
...
// Finalize method:
StatusCode MuonTruthAssociationAlg::finalize() { return StatusCode::SUCCESS; }
