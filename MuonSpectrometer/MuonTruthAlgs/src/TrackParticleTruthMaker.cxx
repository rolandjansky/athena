...
namespace Muon {
    ...
    TrackParticleTruthMaker::TrackParticleTruthMaker(const std::string& name, ISvcLocator* pSvcLocator) :
    ...
    // Initialize method:
    StatusCode TrackParticleTruthMaker::initialize() { return StatusCode::SUCCESS; }
            ...
            if (tempTrackTruthItr != simTrackMap->end()) {
                for (unsigned int j = 0; j < (*tempTrackTruthItr).second.trajectory().size(); j++) {
                    ...
                    ATH_MSG_DEBUG("Barcode: " << (*tempTrackTruthItr).second.trajectory().at(j).cptr()->pdg_id()
                                              << "Status: " << (*tempTrackTruthItr).second.trajectory().at(j).cptr()->status()
                                              << "\t Probability: " << 1.);
                    ...
                    std::pair<ElementLink<Rec::TrackParticleContainer>, TrackParticleTruth> entry(
                        myLink, TrackParticleTruth((HepMcParticleLink)(*tempTrackTruthItr).second.trajectory().at(j), 1.));
    ...
    // Finalize method:
    StatusCode TrackParticleTruthMaker::finalize() { return StatusCode::SUCCESS; }
