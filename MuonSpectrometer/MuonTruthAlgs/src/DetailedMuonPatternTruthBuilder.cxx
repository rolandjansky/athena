...
namespace Trk {
    ...
    SubDetHitStatistics::SubDetType DetailedMuonPatternTruthBuilder::findSubDetType(Identifier id) {
        if (m_idHelper->is_pixel(id)) return SubDetHitStatistics::Pixel;
        if (m_idHelper->is_sct(id)) return SubDetHitStatistics::SCT;
        if (m_idHelper->is_trt(id)) return SubDetHitStatistics::TRT;
        if (m_idHelper->is_mdt(id)) return SubDetHitStatistics::MDT;
        if (m_idHelper->is_rpc(id)) return SubDetHitStatistics::RPC;
        if (m_idHelper->is_tgc(id)) return SubDetHitStatistics::TGC;
        if (m_idHelper->is_csc(id)) return SubDetHitStatistics::CSC;
        if (m_idHelper->is_mm(id)) return SubDetHitStatistics::MM;
        if (m_idHelper->is_stgc(id)) return SubDetHitStatistics::STGC;
    ...
    void DetailedMuonPatternTruthBuilder::addTrack(DetailedMuonPatternTruthCollection* output,
        ...
        for (SproutMap::iterator s = sprouts.begin(); s != sprouts.end(); ++s) {
            ...
            while ((current = m_truthTrajBuilder->getDaughter(current))) { s->second.push_front(current); }
    ...
    Amg::Vector3D DetailedMuonPatternTruthBuilder::getPRDTruthPosition(const Muon::MuonSegment& segment,
                                                                       std::list<const HepMC::GenParticle*> genPartList, int truthPos,
                                                                       set<Muon::MuonStationIndex::ChIndex> chIndices) {
        ...
        for (unsigned int i_cROTv = 0; i_cROTv < cROTv.size(); i_cROTv++) {
            ...
            if (m_idHelperTool->isMdt(id)) {
                ...
                for (std::list<const HepMC::GenParticle*>::const_iterator it = genPartList.begin(); it != genPartList.end() && !deposit;
                     ++it) {
                ...
                ATH_MSG_VERBOSE(" new MDT    " << m_idHelperTool->toString(id) << " radius " << mdt->driftRadius() << " true radius "
                ...
                ATH_MSG_DEBUG("Adding r " << gpos->perp() << " z " << gpos->z() << "  " << m_idHelperTool->toString(id) << " " << residual
                                          << " pull " << pull);
    ...
    const MuonSimData::Deposit* DetailedMuonPatternTruthBuilder::getDeposit(const MuonSimDataCollection& simCol,
                                                                            const HepMC::GenParticle& genPart, const Identifier& id) {
