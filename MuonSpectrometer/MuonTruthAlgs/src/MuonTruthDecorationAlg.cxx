...
namespace Muon {
    ...
    StatusCode MuonTruthDecorationAlg::execute() {
                ...
                if (evtStore()->contains<MuonSimDataCollection>(name)) {
                    ...
                    if (evtStore()->retrieve(col, name).isSuccess()) {
                        if (col && !col->empty()) {
                            ...
                            ATH_MSG_DEBUG("MuonSimDataCollection retrieved at " << name << " index " << index << " size " << col->size()
                                                                                << " tgcIndex "
                                                                                << m_idHelper->tgcIdHelper().technology(id));
        ...
        for (const auto& truth : *truthContainer) {
            ...
            // create segments
            if (m_createTruthSegment && goodMuon) { createSegments(truthLink, ids, *muonTruthSegments, sdoCollections, cscSdoCollection); }
        }
    ...
    void MuonTruthDecorationAlg::createSegments(const ElementLink<xAOD::TruthParticleContainer>& truthLink,
        ...
        for (const auto& lay : ids) {
            ...
            for (const auto& id : lay.second) {
                ...
                if (measPhi) {
                    if (isCsc)
                        phiLayers.insert(m_idHelper->gasGap(id));  // ++nphiLayers;
                    else
                ...
                if (useSDO) {
                    ...
                    if (!isCsc) {
                        ...
                        if (ok) {
                            ...
                            if (!firstPos)
                                firstPos = new Amg::Vector3D(gpos);
                            else if (!secondPos) {
    ...
    void MuonTruthDecorationAlg::addHitIDVectors(xAOD::TruthParticle& truthParticle,
        ...
        ATH_MSG_VERBOSE("Added " << mdtTruthHits.size() << " mdt truth hits, " << cscTruthHits.size() << " csc truth hits, "
                                 << rpcTruthHits.size() << " rpc truth hits, " << tgcTruthHits.size() << " tgc truth hits, "
                                 << stgcTruthHits.size() << " stgc truth hits, " << mmTruthHits.size() << " micromega truth hits");
    }

    // Finalize method:
    StatusCode MuonTruthDecorationAlg::finalize() { return StatusCode::SUCCESS; }
