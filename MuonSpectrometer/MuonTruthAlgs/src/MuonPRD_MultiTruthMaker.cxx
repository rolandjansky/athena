...
StatusCode MuonPRD_MultiTruthMaker::execute() {
    ...
    retvals.push_back(
        buildPRD_Truth<Muon::CscPrepDataContainer, CscSimDataCollection>(m_CSC_ContainerName, m_CSC_SimDataMapName, m_CSC_PRD_TruthName));
    retvals.push_back(
...
StatusCode MuonPRD_MultiTruthMaker::buildPRD_Truth(const std::string& prepDataKey, const std::string& sdoKey,
        ...
        if (!evtStore()->retrieve(prdContainer, prepDataKey).isSuccess()) {
            ...
            if (!evtStore()->retrieve(simDataMap, sdoKey).isSuccess()) {
                ...
                if (!evtStore()->record(output, outputKey, allow_modifications = false).isSuccess()) {
                    ATH_MSG_WARNING("PRD truth structure '" << outputKey << "' could not be registered in StoreGate !");
                    return StatusCode::SUCCESS;
                } else {
                    ATH_MSG_DEBUG("PRD truth structure '" << outputKey << "' is registered in StoreGate, size=" << output->size());
                }
            }
        }
    }
...
void MuonPRD_MultiTruthMaker::addPRDCollections(PRD_MultiTruthCollection* prdTruth, PRD_Container_Iterator collections_begin,
                                                PRD_Container_Iterator collections_end, const SIMDATACOLLECTION* simDataMap) {
...
void MuonPRD_MultiTruthMaker::addPRDRange(PRD_MultiTruthCollection* prdTruth, PRD_Collection_Iterator range_begin,
                                          PRD_Collection_Iterator range_end, const SIMDATACOLLECTION* simDataMap) {
...
void MuonPRD_MultiTruthMaker::addPrepRawDatum(PRD_MultiTruthCollection* prdTruth, const Trk::PrepRawData* prd,
                                              const SIMDATACOLLECTION* simDataMap) {
    if (!prdTruth || !prd || !simDataMap) { return; }
