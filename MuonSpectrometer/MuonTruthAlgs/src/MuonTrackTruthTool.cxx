...
namespace Muon {
    ...
    StatusCode MuonTrackTruthTool::finalize() { return StatusCode::SUCCESS; }
    ...
    const MuonTrackTruthTool::TruthTree& MuonTrackTruthTool::createTruthTree() const {
                ...
                if (genParticle) {
                    ...
                    if (!truthTrajectory->empty()) {
                        ...
                        if (msgLvl(MSG::VERBOSE)) {
                            msg(MSG::VERBOSE) << MSG::VERBOSE << " found GenParticle: size " << truthTrajectory->size() << " fs barcode "
                                              << barcode << " pdg " << truthTrajectory->front()->pdg_id() << " p "
                        ...
                        for (; pit != pit_end; ++pit) {
                            ...
                            if (msgLvl(MSG::VERBOSE) && code != barcode) {
                                msg(MSG::VERBOSE)
                                    << "  secondary barcode: " << code << " pdg " << (*pit)->pdg_id() << " p " << (*pit)->momentum().rho();
                                if ((*pit)->production_vertex())
                                    msg(MSG::VERBOSE) << " vertex: r  " << (*pit)->production_vertex()->point3d().perp() << " z "
                                                      << (*pit)->production_vertex()->point3d().z();
        ...
        for (const auto& name : m_simDataMapNames) { addSimDataToTree(name); }
            ...
            for (; dit != dit_end; ++dit) {
                ...
                if (bit == m_barcodeMap.end()) {
                    ATH_MSG_VERBOSE(" discarding "
                                    << "  " << m_idHelperTool->toString(id) << "   barcode " << barcodeIn);
                    continue;
                }
                ...
                if (eit == m_truthTree.end()) {
                    ATH_MSG_VERBOSE(" discarding "
                                    << "  " << m_idHelperTool->toString(id) << "   barcode " << barcode);
                    continue;
                }
    ...
    const TrackRecordCollection* MuonTrackTruthTool::getTruthTrackCollection() const {
        ...
        for (const auto& location : locations) {
            if (evtStore()->contains<TrackRecordCollection>(location) && evtStore()->retrieve(truthCollection, location).isSuccess() &&
                !truthCollection->empty()) {
    ...
    MuonTrackTruth MuonTrackTruthTool::getTruth(const std::vector<const Trk::MeasurementBase*>& measurements, bool restrictedTruth) const {
    ...
    MuonTrackTruth MuonTrackTruthTool::getTruth(const std::vector<const Trk::MeasurementBase*>& measurements, TruthTreeEntry& truthEntry,
    ...
    const std::pair<const HepMC::GenParticle*, unsigned int> MuonTrackTruthTool::getInitialPair(const TruthTrajectory& traj,
            ...
            if ((*pit)->barcode() == barcodeIn || foundBC) {
                if (!foundBC) {
                    ...
                    if ((*pit)->pdg_id() == pdgFinal) {
                        if ((theFirst != (*pit)) && ((*pit)->momentum().t() != ePrev))
