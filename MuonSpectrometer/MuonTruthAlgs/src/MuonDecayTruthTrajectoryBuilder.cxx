...
namespace Muon {
    ...
    //================================================================
    void MuonDecayTruthTrajectoryBuilder::buildTruthTrajectory(TruthTrajectory* result, const HepMC::GenParticle* input) {
        result->clear();
        if (input) {
            ...
            for (; pit != pit_end; ++pit) {
                ...
                if (msgLvl(MSG::DEBUG)) {
                    ...
                    if (par.production_vertex())
                        msg(MSG::DEBUG) << " vertices prod: r " << par.production_vertex()->point3d().perp() << " z "
                                        << par.production_vertex()->point3d().z();
                    if (par.end_vertex())
                        msg(MSG::DEBUG) << " end: r " << par.end_vertex()->point3d().perp() << " z " << par.end_vertex()->point3d().z();
