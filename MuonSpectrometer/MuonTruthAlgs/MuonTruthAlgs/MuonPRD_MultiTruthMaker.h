...
private:
    ...
    void addPRDCollections(PRD_MultiTruthCollection* prdTruth, PRD_Container_Iterator collections_begin,
    ...
    void addPRDRange(PRD_MultiTruthCollection* prdTruth, PRD_Collection_Iterator range_begin, PRD_Collection_Iterator range_end,
                     const SIMDATACOLLECTION* simDataMap);

    // An analogue of this comes from a tool in InDet
    template <class SIMDATACOLLECTION>
    void addPrepRawDatum(PRD_MultiTruthCollection* prdTruth, const Trk::PrepRawData* prd, const SIMDATACOLLECTION* simDataMap);
};
