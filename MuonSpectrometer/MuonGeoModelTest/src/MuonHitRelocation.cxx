...
MuonHitRelocation::MuonHitRelocation(const std::string& name, ISvcLocator* pSvcLocator) :
    ...
    m_c(new Clockwork()),
    m_rmuonHelper(nullptr),
    m_mmuonHelper(nullptr),
    m_tmuonHelper(nullptr),
    m_cmuonHelper(nullptr),
    m_stmuonHelper(nullptr),
    m_mmmuonHelper(nullptr) {
...
MuonHitRelocation::~MuonHitRelocation() { delete m_c; }
        ...
        if (evtStore()->retrieve(mdt_collection) == StatusCode::SUCCESS) {
            ...
            for (MDTSimHitConstIterator i_hit = mdt_collection->begin(); i_hit != mdt_collection->end(); ++i_hit) {
                ...
                if (fabs(m_c->zResid) > 0.001 || fabs(m_c->phiResid) > 0.001) {
                    ATH_MSG_WARNING("        MDT hit ---   zResid " << m_c->zResid << " >1micron or phiResid " << m_c->phiResid
                                                                    << " for stName/stEta/stphi " << stName << "/" << m_c->steta << "/"
                                                                    << m_c->stphi << " ml/l/t " << m_c->mlayer << "/" << m_c->layer << "/"
                                                                    << m_c->tube);
                }
    ...
    if (m_checkTgc) {
        ...
        if (evtStore()->retrieve(tgc_collection) == StatusCode::SUCCESS) {
            ...
            for (TGCSimHitConstIterator i_hit = tgc_collection->begin(); i_hit != tgc_collection->end(); ++i_hit) {
                ...
                m_c->tube = -999;
                if (fabs(m_c->zResid) > 0.001 || fabs(m_c->phiResid) > 0.001) {
                    ATH_MSG_WARNING("        TGC hit ---   zResid " << m_c->zResid << " >1micron or phiResid " << m_c->phiResid
                                                                    << " for stName/stEta/stphi " << stName << "/" << m_c->steta << "/"
                                                                    << m_c->stphi << " layer " << m_c->layer);
                }
    ...
    if (m_checkRpc) {
        ...
        if (evtStore()->retrieve(rpc_collection) == StatusCode::SUCCESS) {
            ...
            for (RPCSimHitConstIterator i_hit = rpc_collection->begin(); i_hit != rpc_collection->end(); ++i_hit) {
                ...
                m_c->tube = -999;
                if (fabs(m_c->zResid) > 0.001 || fabs(m_c->phiResid) > 0.001) {
                    ATH_MSG_WARNING("        RPC hit ---   zResid " << m_c->zResid << " >1micron or phiResid " << m_c->phiResid
                                                                    << " for stName/stEta/stphi " << stName << "/" << m_c->steta << "/"
                                                                    << m_c->stphi << " dbR/dbZ/dbP " << m_c->dbr << "/" << m_c->dbz << "/"
                                                                    << m_c->dbphi << " gg " << m_c->layer);
                }
    ...
    if (m_checkCsc) {
        ...
        if (evtStore()->retrieve(csc_collection) == StatusCode::SUCCESS) {
            ...
            for (CSCSimHitConstIterator i_hit = csc_collection->begin(); i_hit != csc_collection->end(); ++i_hit) {
                ...
                m_c->tube = -999;
                if (fabs(m_c->zResid) > 0.001 || fabs(m_c->phiResid) > 0.001) {
                    ATH_MSG_WARNING("        CSC hit ---   zResid " << m_c->zResid << " >1micron or phiResid " << m_c->phiResid
                                                                    << " for stName/stEta/stphi " << stName << "/" << m_c->steta << "/"
                                                                    << m_c->stphi << " clay/lay " << m_c->mlayer << "/" << m_c->layer);
                }
    ...
    if (m_checksTgc) {
        ...
        if (evtStore()->retrieve(stgc_collection, "sTGCSensitiveDetector") == StatusCode::SUCCESS) {
            ...
            for (sTGCSimHitConstIterator i_hit = stgc_collection->begin(); i_hit != stgc_collection->end(); ++i_hit) {
                ...
                m_c->tube = -999;
                if (fabs(m_c->zResid) > 0.001 || fabs(m_c->phiResid) > 0.001) {
                    ATH_MSG_WARNING("        sTGC hit ---   zResid " << m_c->zResid << " >1micron or phiResid " << m_c->phiResid
                                                                     << " for stName/stEta/stphi " << stName << "/" << m_c->steta << "/"
                                                                     << m_c->stphi << " layer " << m_c->layer);
                }
    ...
    if (m_checkMM) {
        ...
        if (evtStore()->retrieve(mm_collection, "MicromegasSensitiveDetector") == StatusCode::SUCCESS) {
            ...
            for (MMSimHitConstIterator i_hit = mm_collection->begin(); i_hit != mm_collection->end(); ++i_hit) {
                ...
                m_c->tube = -999;
                if (fabs(m_c->zResid) > 0.001 || fabs(m_c->phiResid) > 0.001) {
                    ATH_MSG_WARNING("        MM hit ---   zResid " << m_c->zResid << " >1micron or phiResid " << m_c->phiResid
                                                                   << " for stName/stEta/stphi " << stName << "/" << m_c->steta << "/"
                                                                   << m_c->stphi << " layer " << m_c->layer);
                }
