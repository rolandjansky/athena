...
private:
  ...
  struct Cache
  {
    ...
    EventIDRange m_condObjOutputRange{ EventIDRange(
      EventIDBase(0, 0),
      EventIDBase(EventIDBase::UNDEFNUM - 1, EventIDBase::UNDEFEVT - 1)) };
  };
  StatusCode updateCurrentFromConditions(const EventContext& ctx,
                                         Cache& cache) const;
  StatusCode updateCurrentFromParameters(const EventContext& ctx,
                                         Cache& cache) const;
