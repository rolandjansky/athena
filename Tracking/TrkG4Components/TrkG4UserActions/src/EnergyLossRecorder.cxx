  ...
  m_Zvec()
{
  ...
  declareProperty("cylinders",m_cylinders);
  declareProperty("minEta",m_minEta);
  declareProperty("maxEta",m_maxEta);
  declareProperty("momentum",m_momentum);
  declareProperty("pdg",m_pdg);
  declareProperty("usePt",m_usePt);
  declareProperty("useX0",m_useX0prop);
  declareProperty("onionMode",m_onionModeprop);
  declareProperty("keepAlive",m_keepAliveprop);
  declareProperty("R",m_Rvec);
  declareProperty("Z",m_Zvec);
...
StatusCode EnergyLossRecorder::initialize(){
  ...
  if(m_useX0prop!="")
    m_useX0 = TurnOffString(m_useX0prop);

  if(m_onionModeprop!="")
    m_onionMode = TurnOffString(m_onionModeprop);

  if(m_keepAliveprop!="")
    m_keepAlive = TurnOffString(m_keepAliveprop);

  ATH_MSG_INFO(" m_verboseLevel = " << m_verboseLevel);
  ATH_MSG_INFO(" m_cylinders    = " << m_cylinders   );
  ATH_MSG_INFO(" m_minEta       = " << m_minEta      );
  ATH_MSG_INFO(" m_maxEta       = " << m_maxEta      );
  ATH_MSG_INFO(" m_momentum     = " << m_momentum    );
  ATH_MSG_INFO(" m_pdg          = " << m_pdg         );
  ATH_MSG_INFO(" m_usePt        = " << m_usePt       );
  ATH_MSG_INFO(" m_useX0        = " << m_useX0       );
  ATH_MSG_INFO(" m_onionMode    = " << m_onionMode   );
  ATH_MSG_INFO(" m_keepAlive    = " << m_keepAlive   );
  ...
  for (size_t lay = 1; lay<m_cylinders; ++lay) {
    ...
    m_cylinderR[lay]=m_Rvec[lay];
    m_cylinderZ[lay]=m_Zvec[lay];
