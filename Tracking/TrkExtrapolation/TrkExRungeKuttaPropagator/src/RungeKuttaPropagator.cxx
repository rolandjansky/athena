 ...
 double                       & W ) const
{
  ...
  double D   = (A[0]*A[0]+A[1]*A[1])+(A[2]*A[2]-1.);
  double CBA = 1.-(.375*D)*((1./.75)-D);
...
(int kind,double* Su,const double* P,bool& Q) const
{
  ...
  double D     = (Ax*Ax+Ay*Ay)+(Az*Az-1.);
  double As    = 1.-(.375*D)*((1./.75)-D);
 ...
 std::pair<double,int>    & SN) const
{
  ...
  double D   = (As[0]*As[0]+As[1]*As[1])+(As[2]*As[2]-1.);
  double CBA = 1.-(.375*D)*((1./.75)-D);
