...
const Trk::TrackingVolume* Trk::Navigator::volume(const Amg::Vector3D& gp) const
{
    if (!m_trackingGeometry) updateTrackingGeometry();
    return(m_trackingGeometry->lowestTrackingVolume(gp));
}

const Trk::TrackingVolume* Trk::Navigator::highestVolume() const
{
    if (!m_trackingGeometry) updateTrackingGeometry();
    return (m_trackingGeometry->highestTrackingVolume());
}

const Trk::BoundarySurface<Trk::TrackingVolume>* Trk::Navigator::nextBoundarySurface(const Trk::IPropagator& prop,
                                                                                     const Trk::TrackParameters& parms,
                                                                                     Trk::PropDirection dir) const
{
  const Trk::TrackingVolume* trackingVolume = volume(parms.position());
  if (trackingVolume) return(nextBoundarySurface(prop, parms, dir, *trackingVolume));
  return 0;
}
                                                                ...
                                                                const Trk::TrackingVolume& vol) const
{
  ...
  const Trk::ObjectAccessor& surfAcc = vol.boundarySurfaceAccessor(parms.position(), dir * parms.momentum().normalized());
  ...
  if (!insideVolume) ++m_outsideVolumeCase;

  return 0;
}


const Trk::NavigationCell Trk::Navigator::nextTrackingVolume(const Trk::IPropagator& prop,
                                                        const Trk::TrackParameters& parms,
                                                        Trk::PropDirection dir,
                                                        const Trk::TrackingVolume& vol) const
{
  ...
  // ---------------------------------------------------
  if (dir == Trk::alongMomentum) ++m_forwardCalls;
  else ++m_backwardCalls;
  ...
  // debug version
  ATH_MSG_VERBOSE( "  [N] Starting parameters are : " << parms );
  ATH_MSG_VERBOSE( "  [N] This corresponds to [r,z] = [ " << parms.position().perp() << ", " << parms.position().z() << "]");
  ...
  for (surfAcc.begin(); surfAcc.end(); surfAcc.operator++()) {
    ...
    if (trackPar){
        ...
        if (msgLvl(MSG::VERBOSE)) {
           ATH_MSG_VERBOSE( "  [N] --> next BoundarySurface found with Parameters: " << *trackPar );
           ATH_MSG_VERBOSE( "  [N] This corresponds to [r,z] = [ " << trackPar->position().perp() << ", " << trackPar->position().z() << "]");
  ...
  // ---------------------------------------------------
    if (!first  && Trk::alongMomentum) 
     { ++m_forwardFirstBoundSwitch; first = true; }
    else if (!second && Trk::alongMomentum) 
     { ++m_forwardSecondBoundSwitch; second = true; }
    else if (Trk::alongMomentum)
     { ++m_forwardThirdBoundSwitch; }
    else if (!first  && Trk::oppositeMomentum) 
     { ++m_backwardFirstBoundSwitch; first = true; }
    else if (!second && Trk::oppositeMomentum) 
     { ++m_backwardSecondBoundSwitch; second = true; }
    else if (Trk::oppositeMomentum) 
...
const Trk::NavigationCell Trk::Navigator::nextDenseTrackingVolume(
                                                                ...
                                                                double& path) const
{
  ATH_MSG_DEBUG("  [N] nextDenseTrackingVolume() to volume '" << vol.volumeName() <<"', starting from "<< parms.position());
  ...
  if ( atVolumeBoundary(currPar,&vol,dir,nextVolume,tol) && nextVolume != (&vol) ) {
    if (!nextVolume) {
      ...
      if ( &vol != currStatic ) nextVolume = currStatic;
    } 
  ...
  if (destinationSurface) surfaces->push_back(std::pair<const Trk::Surface*,Trk::BoundaryCheck>(destinationSurface,false));
...
bool Trk::Navigator::atVolumeBoundary( const Trk::TrackParameters* parms, const Trk::TrackingVolume* vol,  Trk::PropDirection dir, const Trk::TrackingVolume*& nextVol, double tol) const
{
  ...
  for (unsigned int ib=0; ib< bounds.size(); ib++) {
    ...
    if ( surf.isOnSurface(parms->position(),true,tol,tol)  ) {
      //isAtBoundary = true;
      //const Trk::TrackingVolume* attachedVol =  (bounds[ib].getPtr())->attachedVolume(parms->position(),parms->momentum(),dir);
	...
	if (!nextVol && attachedVol ) nextVol = attachedVol;
	  ...
	  ATH_MSG_WARNING("navigator detects tangential intersection: switch of volumes reverted ");
          if (nextVol) ATH_MSG_WARNING(vol->volumeName()<<"->"<<nextVol->volumeName()<<"->"<<vol->volumeName()); 
	  isAtBoundary = false;
  ...
  /*  for debugging 
  ATH_MSG_DEBUG("navigator particle R,phi,z, momentum:"<< parms->position().perp()<<","<<parms->position().phi()<<","<<parms->position().z() <<","<<parms->momentum());
  ...
  for (unsigned int ib=0; ib< bounds.size(); ib++ ){
    ...
    Trk::DistanceSolution distSol = surf.straightLineDistanceEstimate(parms->position(),
								      dir*parms->momentum().unit());
    ATH_MSG_DEBUG("---> decomposed boundary surface position, normal, current distance:"<<ib<<","<<surf.center()<<","<<surf.normal()<<","<<distSol.currentDistance(false));
...
const Trk::TrackParameters* Trk::Navigator::closestParameters(const Trk::Track& trk,
                                                              ...
                                                              const Trk::IPropagator* propptr) const
{
 ...
 if (!m_trackingGeometry) updateTrackingGeometry();
  ...
  //-- corresponds to Extrapolator::m_searchLevel = 1 - search with dedicated algorithms for cylinder/sl/perigee surface
  const Trk::TrackParameters*  closestTrackParameters = 0;
  ...
  if (!measuredParameters.size()) return 0;
  ...
  if (m_searchWithDistance) {
      ...
      for ( ; tpIter != tpIterEnd; ++tpIter){
           ...
           if (currentDistance.numberOfSolutions() > 0) {
               ...
               double secondDistance = currentDistance.numberOfSolutions() > 1 ? fabs(currentDistance.second()) : firstDistance;
               // now do the check
  ...
  if (ccsf) {
   ...
   closestTrackParameters = *(std::min_element(measuredParameters.begin(), measuredParameters.end(), tParFinderCylinder));
...
void Trk::Navigator::validationAction() const
{
...
// finalize
StatusCode Trk::Navigator::finalize()
{
    m_trackingGeometry = 0;
...
const Trk::TrackingGeometry* Trk::Navigator::trackingGeometry() const
{
...
StatusCode Trk::Navigator::updateTrackingGeometry() const
{
