...
m_localCovXY(nullptr),
m_localCovYY(nullptr),
m_localCovYX(nullptr),
m_sizeX(nullptr),
m_sizeY(nullptr),
...
m_ntupleDirName("/SCT_ClusterAnalysis/"),
m_ntupleTreeName("SCT_ClusterAna"),
m_path("/SCT_ClusterAnalysis/"),
...
StatusCode SCT_ClusterAnalysis::initialize() {
    ...
    m_tree = new TTree(TString(m_ntupleTreeName), "SCT_ClusterAna");
...
StatusCode SCT_ClusterAnalysis::execute() {
    ...
    SG::ReadHandle<InDet::SCT_ClusterContainer> stripContainer (m_inputKey);
    if( stripContainer.isValid() ) {
        for( const InDet::SCT_ClusterCollection* stripCollection : *stripContainer ) {
            ...
            for( const InDet::SCT_Cluster* sCluster : *stripCollection ) {
                ...
                // needs some gymnastic to get the global position
                // for itk strip endcap, the one obtained from the cluster is on the beam-line
                // thus local position needs conversion to the local cell frame before converting to global
                const InDetDD::SiDetectorElement *detEl = m_SCT_manager->getDetectorElement(idColl);
                auto localPosForCell = detEl->localPositionOfCell(sCluster->identify());
                auto globalPos = detEl->surface().localToGlobal( localPosForCell );
