...
namespace TrkDriftCircleMath {
    ...
    struct SortSegByNumberOfMDTHitsAndChi2 {
        bool operator()(const Segment& seg1, const Segment& seg2) {
            ...
            if (seg1.hasT0Shift() && !seg2.hasT0Shift())
                return true;
            else if (!seg1.hasT0Shift() && seg2.hasT0Shift())
                return false;

            // sort by numbers of hits on track
            if (seg1.hitsOnTrack() > seg2.hitsOnTrack())
                return true;
            else if (seg1.hitsOnTrack() < seg2.hitsOnTrack())
                return false;

            // take the one with less empty crossed tubes
            if (seg1.emptyTubes().size() < seg2.emptyTubes().size())
                return true;
            else if (seg1.emptyTubes().size() > seg2.emptyTubes().size())
                return false;
            ...
            if (seg1.deltas() < seg2.deltas())
                return true;
            else if (seg1.deltas() > seg2.deltas())
                return false;
