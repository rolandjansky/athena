...
namespace TrkDriftCircleMath {
    ...
    std::ostream& operator<<(std::ostream& os, const TrkDriftCircleMath::DCOnTrack& dc) {
        ...
        if (dc.dr() != dc.drPrecise()) { os << " pull(2) " << dc.residual() / dc.drPrecise(); }
        os << " errTrk " << dc.errorTrack();
        if (dc.state() == TrkDriftCircleMath::DCOnTrack::OnTrack)
            os << " OnTrack ";
        else if (dc.state() == TrkDriftCircleMath::DCOnTrack::Delta)
            os << " Delta ";
        else if (dc.state() == TrkDriftCircleMath::DCOnTrack::OutOfTime)
            os << " OutOfTime ";
        else if (dc.state() == TrkDriftCircleMath::DCOnTrack::CloseDC)
            os << " CloseDC ";
        return os;
    }
    ...
    MsgStream& operator<<(MsgStream& os, const TrkDriftCircleMath::DCOnTrack& dc) {
        if (dc.DriftCircle::state() == TrkDriftCircleMath::DriftCircle::EarlyHit)
            ...
            os << " unknown state";
        os << std::setw(7) << " res " << dc.residual() << std::setw(7) << " pull " << dc.residual() / dc.dr();
        if (dc.state() == TrkDriftCircleMath::DCOnTrack::OnTrack)
            os << " OnTrack ";
        else if (dc.state() == TrkDriftCircleMath::DCOnTrack::Delta)
            os << " Delta ";
