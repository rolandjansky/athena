...
#ifdef _DEBUG
  if( i<0 )     { std::cerr << "AlSymMat::elem: Index 1 < zero! " << i << std::endl;  return *(m_ptr_data); };
  if( i>=size() ) { std::cerr << "AlSymMat::elem: Index 1 too large! " << i << std::endl;  return *(m_ptr_data); };
  if( j<0 )     { std::cerr << "AlSymMat::elem: Index 2 < zero! " << j << std::endl;  return *(m_ptr_data);  };
  if( j>=size() ) { std::cerr << "AlSymMat::elem: Index 2 too large! " << j << std::endl;  return *(m_ptr_data); };
#endif
  if( j<=i ) {
    return  ((i+1)*i/2+j);
  }
