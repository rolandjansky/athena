          ...
          const TrackParameters&  startParameters) const
{
    ...
    for ( ; m != measurements.end(); ++m)
    {
      ...
      if (m_indetVolume->inside((**m).position())){
        ...
        const bool surfaceIsPlaneOrDisc=((**m).surface()->type() == Surface::SurfaceType::Plane || (**m).surface()->type()  == Surface::SurfaceType::Disc );
        if (! endIndetMeasurement && (**m).hasIntersection(FittedTrajectory) && surfaceIsPlaneOrDisc){
          ...
          intersection    = m_intersector->intersectSurface(plane,intersection,qOverP);
          Amg::Vector2D localPos;
          if (intersection && plane.globalToLocal(intersection->position(),intersection->direction(),localPos))
          {
  ...
  while (distance > startDirection.dot((**m).intersection(FittedTrajectory).position() - startPosition))
  {
      ...
      {
    if (*m == measurements.back()) break;
    distance = 0.;  // just to be sure while loop is exited
      }
      ++m;
  }
