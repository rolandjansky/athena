                      ...
                      const Trk::CaloCluster_OnTrack* ccot) const
{
  ...
  if (!firstSmootherMeasurementOnTrack) {
    ATH_MSG_WARNING(
      "Initial state on surface in smoother does not have an associated MeasurementBase object... returning 0");
    if (!smootherPredictionMultiStateOnSurface)
  ...
  // TSOS that the cluster measuremenet will added on after .. can’t be teh last as the impact parameters will be
  // compromised
  ...
  for (; trackStateOnSurface != forwardTrajectory.rend(); ++trackStateOnSurface) {
    ...
    std::unique_ptr<const Trk::MultiComponentState> extrapolatedState =
      std::unique_ptr<const Trk::MultiComponentState>(m_extrapolator->extrapolate(
    ...
    if (!(*trackStateOnSurface)->type(TrackStateOnSurface::Measurement)) {
      ...
      const Trk::MultiComponentStateOnSurface* updatedStateOnSurface = new Trk::MultiComponentStateOnSurface(
        measurement.release(), updatedState->clone(), new FitQuality(1, 1), 0, type);
      smoothedTrajectory->push_back(updatedStateOnSurface);
      continue;
    }
    ...
    updatedState =
      std::unique_ptr<const Trk::MultiComponentState>(m_updator->update(*extrapolatedState, *measurement, fitQuality));
    ...
    if (m_combineWithFitter) {
      ...
      if (ccot && trackStateOnSurface == secondLastTrackStateOnSurface) {
        ...
        std::unique_ptr<const Trk::MultiComponentState> ccotState = std::unique_ptr<const Trk::MultiComponentState>(
                          ...
                          const Trk::MultiComponentState& smootherMultiState) const
{
  ...
  for (auto& forwardsComponent : forwardsMultiState) {
    ...
    for (auto& smootherComponent : smootherMultiState) {
      ...
      const Trk::TrackParameters* combinedTrackParameters = (forwardsComponent.first)
                                                              ->associatedSurface()
