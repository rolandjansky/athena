...
StatusCode Trk::GsfSmoother::configureTools(const ToolHandle<IMultiStateExtrapolator> &extrapolator, 
                                                ...
                                                const Trk::CaloCluster_OnTrack * ccot ) const
{
  ...
  const Trk::MultiComponentState* firstSmoothedState = m_updator->update(*smootherPredictionMultiState, 
  ...
  const Trk::MultiComponentStateOnSurface* updatedStateOnSurface
    = new MultiComponentStateOnSurface(firstSmootherMeasurementOnTrack, combinedFirstSmoothedState, 
  ...
  std::unique_ptr<const Trk::MultiComponentState> updatedState(
    m_updator->update(*smoothedStateWithScaledError, *firstSmootherMeasurementOnTrack));
    ...
    if( !(*trackStateOnSurface)->type(TrackStateOnSurface::Measurement) ){
      ...
      const Trk::MultiComponentStateOnSurface* updatedStateOnSurface =
        new Trk::MultiComponentStateOnSurface( measurement.release(), 
                                               updatedState->clone(), 
                                               new FitQuality(1,1), 0, type);
      smoothedTrajectory->push_back( updatedStateOnSurface );
      continue;
    }

    // Update newly extrapolated state with MeasurementBase measurement
    updatedState = std::unique_ptr<const Trk::MultiComponentState> (
      m_updator->update(*extrapolatedState, *measurement, fitQuality));
    ...
    if (m_combineWithFitter){
      ...
      if ( trackStateOnSurface == lasttrackStateOnSurface){
        ...
        if (combinedLastState)
          updatedStateOnSurface = new Trk::MultiComponentStateOnSurface(measurement.release(), 
                                                                        combinedLastState, 
                                                                        updatedState->clone(), 
                                                                        fitQuality.release());
        else
...
const Trk::MultiComponentState* Trk::GsfSmoother::combine (const Trk::MultiComponentState& forwardsMultiState, 
                                                           const Trk::MultiComponentState& smootherMultiState) const
{
  ...
  for ( auto& forwardsComponent : forwardsMultiState ) {
    ...
    for ( auto& smootherComponent: smootherMultiState ) {
      ...
      const Amg::VectorX newParameters = forwardsComponent.first->parameters() 
      ...
      const Trk::TrackParameters* combinedTrackParameters = 
        (forwardsComponent.first)->associatedSurface().createTrackParameters( newParameters[Trk::loc1],
                                                           ...
                                                           Trk::SmoothedTrajectory* smoothedTrajectory) const
{
  ...
  Trk::MultiComponentStateOnSurface* final = new Trk::MultiComponentStateOnSurface(pseudoMeasurement, 
                                                                                   combinedState, 
                                                                                   extrapolatedState, 
                                                                                   combinedFitQuality);
