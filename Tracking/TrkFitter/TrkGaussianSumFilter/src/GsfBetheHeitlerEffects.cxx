                                     ...
                                     Trk::ParticleHypothesis) const
{
  ...
  if (msgLvl(MSG::VERBOSE)) {
    ...
    msg(MSG::VERBOSE) << "Bethe-Heitler parameters and weights calculated as a function of X/X0 = " << pathlengthInX0
                      << endmsg;
  ...
  if (pathlengthInX0 > m_singleGaussianRange) {
    ...
    for (; componentIndex < m_numberOfComponents; ++componentIndex) {
      ...
      ATH_MSG_VERBOSE("Weight / deltaP / var (delta q/p) " << mixture[componentIndex].weight << "\t" << deltaP << "\t"
