...
Trk::PosteriorWeightsCalculator::weights ( const MultiComponentState& predictedState, const MeasurementBase& measurement ) const{
  ...
  for ( ; component != predictedState.end() ; ++component ){
    ...
    if(nLocCoord==1){
      result = calculateWeight_1D (componentTrackParameters,  predictedCov,
      ...
      if(measurementLocalParameters.parameterKey()==3){
        result = calculateWeight_2D_3 (componentTrackParameters,  predictedCov,
                             measurementLocalParameters.block<2,1>(0,0),
                             measurement.localCovariance().block<2,2>(0,0));
      } else {
        result = calculateWeight_T<2> (componentTrackParameters,  predictedCov,
                             measurementLocalParameters.block<2,1>(0,0),
                             measurement.localCovariance().block<2,2>(0,0),
                             measurementLocalParameters.parameterKey());
      }
    } else if(nLocCoord==3) {
      result = calculateWeight_T<3> (componentTrackParameters,  predictedCov,
                           measurementLocalParameters.block<3,1>(0,0),
                           measurement.localCovariance().block<3,3>(0,0),
                           measurementLocalParameters.parameterKey());
    } else if(nLocCoord==4) {
      result = calculateWeight_T<4> (componentTrackParameters,  predictedCov,
                           measurementLocalParameters.block<4,1>(0,0),
                           measurement.localCovariance().block<4,4>(0,0),
                           measurementLocalParameters.parameterKey());
    } else if(nLocCoord==5) {
      result = calculateWeight_T<5> (componentTrackParameters,  predictedCov,
...
std::pair<double, double> Trk::PosteriorWeightsCalculator::calculateWeight_1D(const TrackParameters* componentTrackParameters,
