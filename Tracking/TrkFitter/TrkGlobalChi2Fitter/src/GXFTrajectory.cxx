...
GXFTrajectory::GXFTrajectory(GXFTrajectory& rhs){
  ...
  if (rhs.m_caloelossstate) {
    for (int i=0;i<(int)m_states.size();i++){
      ...
      if (meff && par && meff->sigmaDeltaE()>0 && meff->sigmaDeltaPhi()==0 && (par->position().perp()>1400 || std::abs(par->position().z())>3700)) m_caloelossstate=m_states[i];
    }
  }
...
GXFTrajectory::~GXFTrajectory(){
  if (m_ownrefpar) delete m_refpar;
  for (int i=0;i<(int)m_states.size();i++){
    delete m_states[i];
  }
}
...
bool GXFTrajectory::addMeasurementState( GXFTrackState *state,int index){
  ...
  if (!m_states.empty() && m_states.back()->measurement()){
    ...
    if (&meas->associatedSurface() == &meas2->associatedSurface() && meas->localParameters().parameterKey()==meas2->localParameters().parameterKey()){
      delete state;
      return false;
    }
  }
  if (index==-1) m_states.push_back(state);
  else m_states.insert(m_states.begin()+index,state);
  int nmeas=0;
  double *errors=state->measurementErrors();
  for (int i=0;i<5;i++) if (errors[i]>0) nmeas++;
  if (state->trackStateType()==TrackState::Fittable) m_ndof+=nmeas;
  else {
    m_nmeasoutl+=nmeas;
    m_noutl++;
  }
  if (state->measurementType()!=TrackState::Pseudo) m_nhits++;
  if (state->measurementType()==TrackState::Pixel || state->measurementType()==TrackState::SCT) m_nsihits++;
  if (state->measurementType()==TrackState::TRT) m_ntrthits++;
  if (state->measurementType()==TrackState::Pseudo) m_npseudo++;
  return true;
}
...
void GXFTrajectory::addMaterialState(GXFTrackState *state,int index,bool owntp){
  ...
  if (owntp) state->setTrackParameters(par);
  if (index==-1) {
    m_states.push_back(state);
    if (meff->sigmaDeltaE()>0) m_brems.push_back(meff->delta_p());
  }
  else {
    m_states.insert(m_states.begin()+index,state);
    int previousscats=0,previousbrems=0;
    for (int i=0;i<index;i++){
      if (m_states[i]->trackStateType()==TrackState::Scatterer) previousscats++;
      if (m_states[i]->materialEffects() && m_states[i]->materialEffects()->sigmaDeltaE()>0) previousbrems++;
    } 
    if (meff->sigmaDeltaE()>0) m_brems.insert(m_brems.begin()+previousbrems,meff->delta_p());

  }
  if (state->trackStateType()==TrackState::Scatterer) {
    m_nscatterers++;
    if (meff->deltaE()==0) m_ncaloscatterers++;
  }
  if (meff->sigmaDeltaE()>0) {
    m_nbrems++;
    if (par && meff->sigmaDeltaPhi()==0 && (par->position().perp()>1400 || std::abs(par->position().z())>3700)) m_caloelossstate=state;
  }
  m_toteloss+=std::abs(meff->deltaE());
  m_totx0+=meff->x0();
  //std::cout << "toteloss: " << m_toteloss << std::endl;
  //std::cout << "nscat: " << m_nscatterers << " " << m_scatteringangles.size() << " nbrem: " << m_nbrems << " " << m_brems.size() << std::endl; 
}


void GXFTrajectory::setReferenceParameters(const TrackParameters *per){
  if (m_refpar){
    if (m_ownrefpar) delete m_refpar;
    m_refpar=per;
  }
    ...
    while (it2!=m_states.end()) {
      ...
      if ((**it2).trackParameters()) {
        ...
        if (distance>0) isdownstream=true;
      ...
      if ((**it2).trackStateType()==TrackState::Scatterer && (**it2).materialEffects()->sigmaDeltaTheta()!=0) {
        ...
        if ( (**it2).materialEffects()->deltaE()==0) m_nupstreamcaloscatterers++;
      }
      if ((**it2).materialEffects() && (**it2).materialEffects()->sigmaDeltaE()>0) m_nupstreambrems++;
    ...
    */
    ...
    /* for (std::vector<GXFTrackState*>::iterator it3=m_states.begin();it3!=m_states.end();it3++){
      if ((**it3).materialEffects() && (**it3).materialEffects()->sigmaDeltaE()>0) (**it3).materialEffects()->momentumJacobians().resize(m_nbrems+1);
    } */
  }
...
const TrackParameters *GXFTrajectory::referenceParameters(bool takeownership){
  ...
  if (takeownership) {
    if (!m_ownrefpar) returnpar=returnpar->clone();
    m_ownrefpar=false;
  }
  return returnpar;
}
...
void GXFTrajectory::setOutlier(int index,bool isoutlier){
  ...
  if (isoutlier && m_states[index]->trackStateType()==TrackState::GeneralOutlier) return;
  if (!isoutlier && m_states[index]->trackStateType()==TrackState::Fittable) return;
  int nmeas=0;
...
int GXFTrajectory::numberOfScatterers(){
  if (m_prefit) return m_ncaloscatterers;
...
int GXFTrajectory::numberOfUpstreamScatterers(){
  if (!m_prefit) return m_nupstreamscatterers;
...
int GXFTrajectory::numberOfFitParameters(){
  if (m_prefit==1) return m_nperpars+numberOfBrems()+numberOfScatterers();
...
std::vector<std::pair<double,double> > &GXFTrajectory::scatteringSigmas(){
  if (m_scatteringsigmas.empty() && numberOfScatterers()>0) scatteringAngles();
...
void GXFTrajectory::setScatteringAngles(std::vector<std::pair<double,double> > &scatteringangles){
...
void GXFTrajectory::setBrems(std::vector<double> &brems){
...
void GXFTrajectory::setTrackStates(std::vector<GXFTrackState*> &states){
  m_states=states;
}

std::vector<double> &GXFTrajectory::residuals(){
  if (m_res.empty()) m_res=std::vector<double>( /* 2*m_nscatterers+ */ numberOfBrems()+m_ndof+m_nperpars+m_nmeasoutl,0);
  return m_res;
}

std::vector<double> &GXFTrajectory::errors(){
  if (m_errors.empty()) m_errors=std::vector<double>( /* 2*m_nscatterers+ */ numberOfBrems()+m_ndof+m_nperpars+m_nmeasoutl,0);
  return m_errors;
}


std::vector<std::vector<double> > &GXFTrajectory::weightedResidualDerivatives(){
  if (m_weightresderiv.empty()) {
    m_weightresderiv.resize( numberOfBrems()+m_ndof+m_nperpars+m_nmeasoutl);
    //std::cout << "nmeas: " << 2*m_nscatterers+m_nbrems+m_ndof+m_nperpars+m_nmeasoutl << " capacity: " << m_resderiv.capacity() << " numberOfFitParameters: " << numberOfFitParameters() << std::endl;
    int nfitpar=numberOfFitParameters();
    //std::cout << "capacity: " << m_resderiv.capacity() << " nbrems: " << numberOfBrems() << " ndof: " << m_ndof << " perpars: " << m_nperpars << " noutl: " << m_nmeasoutl << std::endl;
...
double GXFTrajectory::totalX0(){
  return m_totx0;
}

double GXFTrajectory::totalEnergyLoss(){ 
  return m_toteloss;
}

double GXFTrajectory::mass(){
  return m_mass;
}

void GXFTrajectory::setMass(double mass){
  m_mass=mass;
}
