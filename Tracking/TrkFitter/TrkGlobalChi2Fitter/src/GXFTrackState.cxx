...
GXFTrackState::GXFTrackState(GXFTrackState& rhs):
  ...
  m_covariancematrix(rhs.m_covariancematrix ? (rhs.m_owncov ? new AmgSymMatrix(5)(*rhs.m_covariancematrix) : rhs.m_covariancematrix) : 0),  
...
{
  m_measerror[0]=m_measerror[1]=m_measerror[2]=m_measerror[3]=m_measerror[4]=-1;
  if (mef->sigmaDeltaTheta()==0) m_tsType=TrackState::Brem;
}


Trk::GXFTrackState::~GXFTrackState()
{
  if (m_ownmb) delete m_measurement;
  if (m_owntp) delete m_trackpar;
  delete m_materialEffects;
  if (m_owncov) delete m_covariancematrix;
  if (m_ownfq) delete m_fitqual;
}
...
{ 
  ...
  if (meas && !m_ownmb && takeownership) meas=meas->clone();
  if (takeownership) m_ownmb=false; 
  return meas; 
}


void GXFTrackState::setTrackParameters(const TrackParameters* par)
{
  if (m_trackpar && m_owntp) delete m_trackpar;
  m_trackpar = par;
  m_owntp=true;
}

void GXFTrackState::setMaterialEffects(GXFMaterialEffects* mef)
{
...
void GXFTrackState::setDerivatives(HepMatrix &deriv){
  m_derivs=&deriv;
}

void GXFTrackState::setTrackStateType(TrackState::TrackStateType tstype){
 m_tsType = tstype;
}
...
void GXFTrackState::setMeasurementErrors(double *measerror){
...
double GXFTrackState::sinStereo(){
  return m_sinstereo;
}

void GXFTrackState::setSinStereo(double sinstereo){
  m_sinstereo=sinstereo;
}

const Surface* GXFTrackState::surface() {
  if (m_measurement) return &m_measurement->associatedSurface();
  else if (m_trackpar) return &m_trackpar->associatedSurface();
  else if (m_materialEffects) return m_materialEffects->surface();
  else return 0;
}

void GXFTrackState::setTrackCovariance(AmgSymMatrix(5)* covmat){
  if (m_covariancematrix && m_owncov) delete m_covariancematrix;
  m_covariancematrix=covmat;
  if (covmat) m_owncov=true;
}

void GXFTrackState::setFitQuality(const FitQualityOnSurface *fitqual){
  if (m_fitqual && m_ownfq) delete m_fitqual;
  m_fitqual=fitqual;
  m_ownfq=true;
}

const FitQualityOnSurface *GXFTrackState::fitQuality(bool takeownership){
  if (takeownership) m_ownfq=false;
  return m_fitqual;
}

int GXFTrackState::numberOfMeasuredParameters(){
  int nmeas=0;
  if (m_tsType==TrackState::Fittable || m_tsType==TrackState::GeneralOutlier){
    for (int i=0;i<5;i++) if (m_measerror[i]>0) nmeas++;
  }
...
const Amg::Vector3D &GXFTrackState::position(){
  if (m_trackpar) return m_trackpar->position();
  else if (m_measurement) return m_measurement->globalPosition();
  //return surface()->center();  // Better than nothing...
  return m_globpos;
}

void GXFTrackState::setPosition(Amg::Vector3D &pos){
  m_globpos=pos;
}

bool GXFTrackState::measuresPhi(){
  return m_measphi;
}

void GXFTrackState::setMeasuresPhi(bool measphi){
  m_measphi=measphi;
}

void GXFTrackState::setRecalibrated(bool isrecal){
  m_recalib=isrecal;
}
