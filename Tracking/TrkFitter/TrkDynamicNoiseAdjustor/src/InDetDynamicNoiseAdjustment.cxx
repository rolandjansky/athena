...
// initialize
StatusCode
Trk::InDetDynamicNoiseAdjustment::initialize()
{
  msg(MSG::INFO) << "DNA tuning: yminmax <GEN> = " << m_yminmax_generic
                 ...
                 << "\nDNA tuning: lambdaqop <GEN> = " << m_lambdaqop_generic
                 << endmsg;
  msg(MSG::INFO) << "DNA tuning: yminmax <ELE> = " << m_yminmax_electron
                 << "(activity level of DNA, range 0.0 to 100)"
                 << "\nDNA tuning: signifmin <ELE> = " << m_signifmin_electron
                 << "\nDNA tuning: lambdaxmin <ELE> = " << m_lambdaxmin_electron
                 << "\n(par to tune the width of the pull, range -2.0 to 4.0)"
                 << "\nDNA tuning: lambdaqop <ELE> = " << m_lambdaqop_electron
                 << endmsg;
...
// finalize
StatusCode
Trk::InDetDynamicNoiseAdjustment::finalize()
{
...
////////////////////////////////////////////////////////////////////////////////
StatusCode
Trk::InDetDynamicNoiseAdjustment::configureWithTools(
  ...
  const Trk::DNA_MaterialEffects* /*forwardMEF*/) const
{
  ...
  double cut_yminmax =
    matEff.aggressiveDNA() ? m_yminmax_electron : m_yminmax_generic;
  double cut_signifmin =
    matEff.aggressiveDNA() ? m_signifmin_electron : m_signifmin_generic;
  double lambdaxmin =
    matEff.aggressiveDNA() ? m_lambdaxmin_electron : m_lambdaxmin_generic;
  double lambdaqop =
    matEff.aggressiveDNA() ? m_lambdaqop_electron : m_lambdaqop_generic;

  // Check if error matrix of start parameter and its associated layer material
  // exist
  const Trk::Layer* lay =
    updatedPar ? (updatedPar->associatedSurface()).associatedLayer() : nullptr;

  state.currentDnaStrategy = unidentifiedStrategy;

  if (!updatedPar->covariance())
    return nullptr;
  ...
  double rxy1 = std::sqrt(std::pow(updatedPar->position().x(), 2) +
                          std::pow(updatedPar->position().y(), 2));
  double rxy2 = std::sqrt(pow(predPar->position().x(), 2) +
                          std::pow(predPar->position().y(), 2));
  double z1 = updatedPar->position().z();
  double z2 = predPar->position().z();
  double cost12 = (rxy1 * rxy2 + z1 * z2) /
                  std::sqrt((rxy1 * rxy1 + z1 * z1) * (rxy2 * rxy2 + z2 * z2));
  ...
  double eta1 =
    -std::log(std::tan((updatedPar->parameters()[Trk::theta]) / 2.0));
  ...
  ATH_MSG_DEBUG("rxy1, z1, rxy2, z2, cost12, dsint, eta1, phi1   "
                << rxy1 << "   " << z1 << "   " << rxy2 << "   " << z2 << "   "
                << cost12 << "   " << dsint << "   " << eta1 << "   " << phi1
                << "   ");
  // Step zero parameters and errors before extrapolation and before measurement
  // update
  ...
  if (cost12 > 0.98 && std::fabs(dsint) < 0.003 && rxy1 < rxymax &&
      rxy1 > rxymin && sign * rxy1 < sign * rxy2 && sint1 > 0.0 &&
      std::fabs(eta1) < 2.7) {

    // pre-update the extrapolated predPar to new measurement, calculate
    // tentative Chi2
    auto fitQuality0 = std::unique_ptr<const FitQualityOnSurface>(
      m_updator->predictedStateFitQuality(
        *predPar,
        fittableMeasurement->localParameters(),
        fittableMeasurement->localCovariance()));
    if (!fitQuality0) {
      ATH_MSG_DEBUG("chi2 calculation failed for state with measurement "
                    << *fittableMeasurement);
      return nullptr;
    }
    ...
    if (qop0 == 0.0 || chi20 < 0.0) {
      ATH_MSG_WARNING((qop0 == 0.0 ? "FPE, bad fit (qop0="
                      ...
                      << (qop0 == 0.0 ? qop0 : chi20)
                      << "). Stop dna at this state");
      return nullptr;
    }
    double qoplimit,
      stepFactor = 1.0 + 3.0 * sqrt(chi20) * sigmaqop0 / std::fabs(qop0);
    if (direction == Trk::alongMomentum) {
      if (stepFactor > 1.6)
        stepFactor = 1.6; ///// needs tuning
      zinv1 = stepFactor;
      zinv2 = zinv1;
    ...
    if (chi20 > chi2threshold * dof0 && std::fabs(qop0) < qoplimit &&
        std::fabs(qop0) * 0.08 > sigmaqop0) // excludes undeveloped tracks
    {
      ...
      auto fitQuality1 = std::unique_ptr<const FitQualityOnSurface>(
        m_updator->predictedStateFitQuality(
          *testPredPars,
          fittableMeasurement->localParameters(),
          fittableMeasurement->localCovariance()));
      ...
      auto fitQuality2 = std::unique_ptr<const FitQualityOnSurface>(
        m_updator->predictedStateFitQuality(
          *testPredPars,
          fittableMeasurement->localParameters(),
          fittableMeasurement->localCovariance()));
      ...
      ATH_MSG_DEBUG("rxy1,dof0,x0,x1,x2,y0,y1,y2,xmin+/-dx,ymin:   "
                    << rxy1 << " " << dof0 << "  |  " << x0 << " " << x1 << " "
                    << x2 << "  |  " << y0 << " " << y1 << " " << y2 << "  |  "
                    << xmincan << "+/-" << dxcan << "  |  " << ymin);
      ...
      if (a > 0. && (xmincan - x0) * (xmincan - xth) < 0.0 &&
        ...
        state.currentDnaStrategy = noGoodMinimum;

    } else { // low chi2, low pt, or undeveloped track

      zinv1 = 1.0;
      zinv2 = 1.0;
      state.currentDnaStrategy = undevelopedTrack;

    }      // endif --- a brem is suspected
  } else { // geometric cuts are not satisfied
    state.currentDnaStrategy = geomCutsNotSatisfied;
  ...
  ATH_MSG_DEBUG("dir, rxy1, xmin, xminf: " << direction << "  " << rxy1
                                           << "     " << xmin << " +/- " << dx
                                           << ",   ");
  ...
  if (state.currentDnaStrategy !=
      unidentifiedStrategy) { // already known that no good minimum exists

    ATH_MSG_DEBUG("dir, Rxy, eta, phi, pT, dp/p, chi20, dof0, xmin+/-dx, "
                  << "ymin, m, dpdna/p: " << direction << "   " << rxy1 << "   "
                  << eta1 << "   " << phi1 << "   " << sint1 / qop0 << "   "
                  << sigmaqop0 / std::fabs(qop0) << "   " << chi20 << "   "
                  << dof0 << "   " << xmin << "   " << dx << "   " << ymin
                  << "   " << m << "   " << sigmaQoverP / std::fabs(qop0));

  } else if (xmin > 1.0) {
    // good minimum was found, determine DNA strategy further

    if (lay) {

      // bool doRefMat = true;

      // Estimate m (sometimes called c, pathlength in X0, corrected for theta,
      // divided by ln2) calculate the pathcorrection
      double pathcorrection =
        std::fabs(lay->surfaceRepresentation().pathCorrection(
          updatedPar->position(), updatedPar->momentum()));
      // prepare the material properties
      const Trk::MaterialProperties* mat =
        lay->fullUpdateMaterialProperties(*updatedPar);

      // Temporary fix to avoid crashes due to mat=0:
      double thick = 0.0;
      if (mat) {
        thick = mat->thicknessInX0();
      } else {
        ATH_MSG_DEBUG(
          "Problem with material: refMat=0, lay exists, but mat=0.    x,y,z = "
          << updatedPar->position().x() << "  " << updatedPar->position().y()
      ...
      // --- Calculate the median of B-H with that m: {approximation to
      // 1-(0.5)**(1/m)}
      if (m > 0.5)
        m = 0.5;
      if (m > 0.0 && m <= 0.5) {
        double temp = 45. * m * m, temp3 = temp * temp * temp,
               temp4 = temp * temp3;
      ...
      // dilerr can be used to dilute the argument of dna; dilcor weighs zest
      // with B-H mean
      if (rxy1 < 250.0 && sign == -1) { // special treatment for pixel hits
        state.currentDnaStrategy = backwardPixelTreatment;
      }
    } // end of if( lay )

    // Calculate dna
    if (state.currentDnaStrategy != backwardPixelTreatment) {
      state.currentDnaStrategy = regularSuccessfulDNA;
    }

  } // both dna and non-dna branches come here now
  else {
    ATH_MSG_INFO(
      "rupture in DNA logic, xmin doesn'f fullfil >1. xmin = " << xmin);
  }

  // --- now determine final noise adjustment depending on detected strategy

  switch (state.currentDnaStrategy) {
    case regularSuccessfulDNA: // regular successful DNA activity
      zest = 1.0;
      if (signif > cut_signifmin) {
        zest = 1.0 / denom;
      }
      dna = Trk::InDetDynamicNoiseAdjustment::calculateDna(xth, m, zest);
      break;

    case backwardPixelTreatment: // special treatment for regular DNA activity
                                 // in pixels in backward dir.
      zest = 1.0;
      if (matEff.aggressiveDNA()) { // now they are the same, near future test
                                    // different tunings here
        // double dilerr = 0.1, dilcor = 0.9;
        const double dilerr = 1.0;
        const double dilcor = 1.0;
        zcor =
          1.0 - dilcor * (1.0 - zcor) -
          (1.0 - dilcor) * (1.0 - std::pow(0.5, m)); // reweight z correction
        if (signif > cut_signifmin)
          zest = 1.0 - (1.0 - 1.0 / denom) * dilerr *
                         (1.0 - lambdaqop); // dilute dna argument
      } else {
        const double dilerr = 0.1;
        const double dilcor = 0.9;
        zcor =
          1.0 - dilcor * (1.0 - zcor) -
          (1.0 - dilcor) * (1.0 - std::pow(0.5, m)); // reweight z correction
        if (signif > cut_signifmin)
  ...
  ATH_MSG_DEBUG("chi20 dof0  pt  dpt eta  m *  dna zcor zinv1 "
                "zinv2  * Rxy lay casino : "
                << chi20 << "   " << dof0 << "  " << std::fabs(sint1 / qop0)
                << "   " << std::fabs(sigmaqop0 / qop0) << "   " << eta1
                << "   " << m << "   *   " << dna << "   " << zcor << "   "
                << zinv1 << "   " << zinv2 << "   *   " << rxy1 << "   " << lay
                << "   " << state.currentDnaStrategy << "      cas ");

  // --- only if a correction is there re-do the extrapolation, else leave
  // original parameters
  if (dna != 0.00) {
    // pick up values for dna, zcor, zinv1, zinv2; do corrections to momentum
    // and its error
    AmgVector(5) updatedParameters = (updatedPar->parameters());
    //       updatedParameters[Trk::qOverP] *= 1.0;    //should be z0 here!
    updatedParameters[Trk::qOverP] *= zcor / zinv1 / zinv2; // should be z0
                                                            // here!
    ...
    ATH_MSG_DEBUG("dir, Rxy, eta, phi, pT, dp/p, chi20, dof0, "
                  "xmin+/-dx, ymin, m, dpdna/p: "
                  << direction << "   " << rxy1 << "   " << eta1 << "   "
                  << phi1 << "   " << sint1 / qop0 << "   "
                  << sigmaqop0 / std::fabs(qop0) << "   " << chi20 << "   "
                  << dof0 << "   " << xmin << "   " << dx << "   " << ymin
                                               ...
                                               double zest) const
{
  ...
  if (zest < 1.0 && zest > 1.0 / xth && m > 0.0 && m <= 0.2) {
    // parameters for dna
    const double p1 = 0.23 + m * 1.5;
    const double p2 = 0.96 - m * 2.5;
    const double p3 = 0.325;
    const double p4 = 0.84 - m * 3.0;
    dna = (p1 * std::pow((std::log(1. / zest)), p2) * std::pow(zest, p3) *
           (1. + p4 * zest)); // dna parametrization
  }
  return dna;
}
