...
void DefParamPullPlots::initializePlots() {
    Pull_d0     = Book1D( "Pull"+m_sType+"_d0",     "d0 Pull "+m_sType+";d0 (track - truth) / error;Entries",       100, -5, 5 );
    Pull_z0     = Book1D( "Pull"+m_sType+"_z0",     "z0 Pull "+m_sType+";z0 (track - truth) / error;Entries",       100, -5, 5 );
    Pull_phi    = Book1D( "Pull"+m_sType+"_phi",    "phi Pull "+m_sType+";phi (track - truth) / error;Entries",     100, -5, 5 );
    Pull_theta  = Book1D( "Pull"+m_sType+"_theta",  "theta Pull "+m_sType+";theta (track - truth) / error;Entries", 100, -5, 5 );
    Pull_qOverP = Book1D( "Pull"+m_sType+"_qOverP", "qOverP Pull "+m_sType+";Q/P (track - truth) / error;Entries",  100, -5, 5 );

    Double_t Pull_ptBins[8] = {5, 10, 25, 40, 60, 100, 200, 1000};
    std::pair<Int_t,Double_t*> ptBins(7,Pull_ptBins);
    Pull_d0_vs_pt     = Book2D( "Pull"+m_sType+"_d0_vs_pt",     m_sType+" d0 Pull vs pt;pt;d0 Pull",       ptBins.first, ptBins.second, 160, -8, 8 );
    Pull_z0_vs_pt     = Book2D( "Pull"+m_sType+"_z0_vs_pt",     m_sType+" z0 Pull vs pt;pt;z0 Pull",       ptBins.first, ptBins.second, 160, -8, 8 );
    Pull_phi_vs_pt    = Book2D( "Pull"+m_sType+"_phi_vs_pt",    m_sType+" phi Pull vs pt;pt;phi Pull",     ptBins.first, ptBins.second, 160, -8, 8 );
    Pull_theta_vs_pt  = Book2D( "Pull"+m_sType+"_theta_vs_pt",  m_sType+" theta Pull vs pt;pt;theta Pull", ptBins.first, ptBins.second, 160, -8, 8 );
    Pull_qOverP_vs_pt = Book2D( "Pull"+m_sType+"_qOverP_vs_pt", m_sType+" qOverP Pull vs pt;pt;Q/P Pull",  ptBins.first, ptBins.second, 160, -8, 8 );
}
