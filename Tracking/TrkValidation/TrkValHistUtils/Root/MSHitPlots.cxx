...
void MSHitPlots::fill(const xAOD::TrackParticle& trkprt) {
...
void MSHitPlots::fill(const xAOD::TruthParticle& truthprt) {
...
void MSHitPlots::fillPlot(HitTypePlots& hitPlots, xAOD::SummaryType info, const xAOD::TrackParticle& trkprt){
  uint8_t hitval = 0;
  if (!trkprt.summaryValue(hitval,info)) return;
  hitPlots.fill(hitval, trkprt.eta(), trkprt.phi());
}

void MSHitPlots::fillPlot(HitTypePlots& hitPlots, const std::string& sInfo, const xAOD::TruthParticle& truthprt){
