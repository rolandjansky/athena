...
namespace Trk
{
  ...
  AdaptiveMultiVertexFitter::fit(std::vector<xAOD::Vertex*> & allVertices) const{
    ...
    do {
      for (const auto & pThisVertex : allVertices){
        ...
        if (isInitialized( *pThisVertex )) {
          ...
          if (MvfFitInfo( *pThisVertex)->seedVertex()==0) { // TODO: Is there a way of checking whether a decoration exists on an object?
            ATH_MSG_ERROR( "Candidate has no seed...CRASHING now!!!" );
          }
          oldpositions[pThisVertex] = *(MvfFitInfo( *pThisVertex)->seedVertex());
        }
        if (MvfFitInfo( *pThisVertex)->linearizationVertex()==0) { // TODO: Is there a way of checking whether a decoration exists on an object?
          ATH_MSG_ERROR( " Candidate has no linearization point...CRASHING now!!! ");
        }
        if ((oldpositions[pThisVertex] - *MvfFitInfo( *pThisVertex )->linearizationVertex()).perp()>m_maxDistToLinPoint) {
        ...
        pThisVertex->setPosition( MvfFitInfo( *pThisVertex )->constraintVertex()->position() );
        pThisVertex->setCovariancePosition( MvfFitInfo( *pThisVertex )->constraintVertex()->covariancePosition() );
        pThisVertex->setFitQuality( MvfFitInfo(*pThisVertex)->constraintVertex()->chiSquared(), MvfFitInfo( *pThisVertex)->constraintVertex()->numberDoF() );
        pThisVertex->setCovariancePosition( pThisVertex->covariancePosition() * 1./float(m_AnnealingMaker->getWeight(astate,1.)) );
        ...
        for (const auto & pThisTrack : theseTrackPointersAtVtx) {
          ATH_MSG_DEBUG( "Adding compatibility info to a track of " << theseTrackPointersAtVtx.size() );
          ...
          if ( not pThisTrack->ImpactPoint3dAtaPlane() ) {
            const bool success=m_ImpactPoint3dEstimator->addIP3dAtaPlane( *pThisTrack, *MvfFitInfo( *pThisVertex )->linearizationVertex() );
            if (!success){
          ...
          m_TrackCompatibilityEstimator->estimate( *pThisTrack, oldpositions[pThisVertex] );
      ...
      for (const auto & pThisVertex : allVertices){
        ...
        std::vector<Trk::VxTrackAtVertex>* tracksOfVertex = &( pThisVertex->vxTrackAtVertex() );
        tracksOfVertex->clear();
        //prepare the iterators for the tracks
        const auto & theseTrackPointersAtVtx = VTAV( *pThisVertex);
        ATH_MSG_VERBOSE( "Beginning lin&update of vertex with pointer: " << pThisVertex );
        for (const auto & pThisTrack : theseTrackPointersAtVtx){
          //set the weight according to all other track's weight
          ATH_MSG_DEBUG( "Calling collect weight for track " << pThisTrack );
          const std::vector<double> & allweights(collectWeights(*(static_cast<Trk::MVFVxTrackAtVertex*>(pThisTrack))->linkToVertices()));  
          ATH_MSG_DEBUG( "The vtxcompatibility for the track is: " << pThisTrack->vtxCompatibility() );
          pThisTrack->setWeight(m_AnnealingMaker->getWeight(astate,pThisTrack->vtxCompatibility(),allweights));
          ATH_MSG_DEBUG( "The resulting weight for the track is " << m_AnnealingMaker->getWeight(astate,pThisTrack->vtxCompatibility(),allweights));
          if (pThisTrack->weight()>m_minweight) {
            ...
            if (not pThisTrack->linState()) {
              ...
              m_LinearizedTrackFactory->linearize(*pThisTrack,oldpositions[pThisVertex]);
            } else if (relinearizations[pThisVertex]==true) {
              ATH_MSG_VERBOSE( "Relinearizing track " );
              m_LinearizedTrackFactory->linearize(*pThisTrack,oldpositions[pThisVertex]);
              MvfFitInfo( *pThisVertex )->setLinearizationVertex( new Amg::Vector3D(oldpositions[pThisVertex]) );
            }
            //now you can proceed with the update
            ATH_MSG_DEBUG( "Update of the track " << pThisTrack << " to the vertex " << pThisVertex );
        ...
        ATH_MSG_DEBUG( "Vertex pointer " << pThisVertex << 
          " New position x: " << pThisVertex->position().x() << " y: " << pThisVertex->position().y()
    ...
    if(m_doSmoothing) {
      ...
      auto trackAvailable = [](const xAOD::Vertex* pV){
        return pV->vxTrackAtVertexAvailable() and not( pV->vxTrackAtVertex()).empty();
      };
      for (const auto & pThisVertex : allVertices) {   
      ...
      for (const auto & pThisVertex : allVertices) {  
        ...
        for (const auto & pTrack :  theseTrackPointersAtVtx){
          if (pTrack->initialPerigee()) pTrack->setPerigeeAtVertex( ( pTrack->initialPerigee() )->clone() );
        }
      }
    }
  }
  ...
  std::vector<double>   
  AdaptiveMultiVertexFitter::collectWeights(const Trk::TrackToVtxLink & tracklink) const {
    ...
    for (const auto & pThisVertex : theseVertices){
      ...
      for (const auto & pThisTrack : trackPointersForThisVertex){
        if ((static_cast<Trk::MVFVxTrackAtVertex*>(pThisTrack))->linkToVertices()==&tracklink) {
          ATH_MSG_DEBUG( "found one weight: it's : " << pThisTrack->vtxCompatibility() << " for track " << pThisTrack );
  ...
  AdaptiveMultiVertexFitter::addVtxToFit(xAOD::Vertex* newvertex) const {
    ...
    std::vector<xAOD::Vertex*> allVerticesToFit;//how many vertices do you expect?
    ...
    do {
      for (const auto & vertexIterator: addedVerticesLastIteration){
        ...
        for (const auto & thisTrack : vertexTracksAtVertex){
          const auto & pTheseVertices = (static_cast<Trk::MVFVxTrackAtVertex*>(thisTrack))->linkToVertices()->vertices();
  ...
  AdaptiveMultiVertexFitter::prepareCompatibility(xAOD::Vertex* newvertex) const {
    ...
    auto addImpactPoint = [this, seedPoint] (const auto & thisVxTrack){
      return this->m_ImpactPoint3dEstimator->addIP3dAtaPlane( *thisVxTrack, *seedPoint );
    };
    const auto & vertexTracksAtVertex=VTAV(*newvertex);
    //std::reduce might be quicker, if compiler implemented it
    const bool success = std::all_of(vertexTracksAtVertex.begin(), vertexTracksAtVertex.end(), addImpactPoint );
    if (not success){
      ATH_MSG_WARNING( "Adding compatibility to vertex information failed. Newton distance finder didn't converge..." );
    }
  }
  ...
  bool 
  AdaptiveMultiVertexFitter::findAmongVertices(const xAOD::Vertex* vertex,const std::vector<xAOD::Vertex*> & previousVertices) const{
    return (std::find(previousVertices.begin(),previousVertices.end(),vertex) != previousVertices.end());
  }
}
