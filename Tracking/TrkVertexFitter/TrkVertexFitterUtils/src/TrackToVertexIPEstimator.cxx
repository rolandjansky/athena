...
//linearized track factory
 ...
 const  ImpactParametersAndSigma * TrackToVertexIPEstimator::estimate(const Trk::Track * track, const Trk::VxCandidate * vtx, bool doRemoval) const
 {
  if(track && vtx)
  {
    return estimate(track->perigeeParameters(),track->perigeeParameters(),vtx,doRemoval); 
  }else{
   msg(MSG::INFO) << "Empty Track or Vertex pointer passed. Returning zero " << endreq;
   return 0;
  }//end of track validity check 
 }//end of method using tracks
  
 const  ImpactParametersAndSigma * TrackToVertexIPEstimator::estimate(const Trk::Track * track, const Trk::Track * newtrack, const Trk::VxCandidate * vtx, bool doRemoval) const
 {
  if(track && vtx)
  {
 ...
 const  ImpactParametersAndSigma * TrackToVertexIPEstimator::estimate(const Trk::TrackParticleBase * track, const Trk::VxCandidate * vtx, bool doRemoval) const
 {
  if(track && vtx)
  {
   return estimate(&(track->definingParameters()),&(track->definingParameters()),vtx,doRemoval); 
  }else{
 ...
 const  ImpactParametersAndSigma * TrackToVertexIPEstimator::estimate(const Trk::TrackParticleBase * track, const Trk::TrackParticleBase * newtrack, const Trk::VxCandidate * vtx, bool doRemoval) const
 {
  if(track && vtx)
  {
    return estimate(&(track->definingParameters()),&(newtrack->definingParameters()),vtx,doRemoval); 
  }else{
   msg(MSG::INFO) << "Empty TrackParticle or Vertex pointer passed. Returning zero " << endreq;
   return 0;
  }//end of track particle validity check
 ...
 const  ImpactParametersAndSigma * TrackToVertexIPEstimator::estimate(const TrackParameters * track, const Trk::VxCandidate * vtx, bool doRemoval) const
 {
   if(track && vtx){
 ...
 const  ImpactParametersAndSigma * TrackToVertexIPEstimator::estimate(const TrackParameters * track, const TrackParameters * newtrack, const Trk::VxCandidate * vtx, bool doRemoval) const
 {
 
   if (vtx==0) 
   {
     msg(MSG::WARNING) << "Vertex is zero pointer. Will not estimate IP of track." << endreq;
     return 0;
   }
   ...
   if (doRemoval)
   {
     rvtx = getUnbiasedVertex(track,vtx);
     if (rvtx==0)
     {
       msg(MSG::WARNING) << "Unbiasing of vertex failed. Will not estimate IP of track." << endreq;
       return 0;
     }
   }

   const ImpactParametersAndSigma  *  IPandSigma=estimate(newtrack,rvtx);
...
//   std::cout<<"new sigma: "<<sqrt(s_d0_test)<<std::endl; 
 ...
 const  ImpactParametersAndSigma * TrackToVertexIPEstimator::calculate(const TrackParameters * track, const Trk::RecVertex& vtx) const
 { 
//estimating the d0 and its significance by propagating the trajectory state towards 
//the vertex position. By this time the vertex should NOT contain this trajectory anymore
...
//   std::cout<<"new sigma: "<<sqrt(s_d0_test)<<std::endl; 
                                                            ...
                                                            const Trk::RecVertex & primaryVertex) const
  {
    ...
    const  Trk::Perigee * extrapolatedParameters =
        dynamic_cast<const Trk::Perigee *>(m_extrapolator->extrapolate(track,perigeeSurface));
...
const Trk::RecVertex* TrackToVertexIPEstimator::getUnbiasedVertex(const Trk::Track * track, const Trk::VxCandidate * vtx) const
 {
  if(track && vtx)
  {
 ...
 const Trk::RecVertex* TrackToVertexIPEstimator::getUnbiasedVertex(const Trk::TrackParticleBase * track, const Trk::VxCandidate * vtx) const
 {
  if(track && vtx)
  {
   return getUnbiasedVertex(&(track->definingParameters()),vtx);
  }else{
   msg(MSG::INFO) << "Empty TrackParticle or Vertex pointer passed. Returning zero " << endreq;
   return 0;
  }//end of track particle validity check
...
const Trk::RecVertex * TrackToVertexIPEstimator::getUnbiasedVertex(const TrackParameters * track, const Trk::VxCandidate * vtx ) const
 {

  //checking whether the track is already fitted to the vertex 
  const std::vector <Trk::VxTrackAtVertex*>* fittedTracks=vtx->vxTrackAtVertex();

  const Trk::RecVertex & rvtx = vtx->recVertex();

  if(fittedTracks)
  {
    std::vector <Trk::VxTrackAtVertex*>::const_iterator tb = fittedTracks->begin();
    std::vector <Trk::VxTrackAtVertex*>::const_iterator te = fittedTracks->end();

    if (fittedTracks->size()==0)
    {
      msg(MSG::DEBUG) << "This vertex has no associated tracks. Normal if beam spot is used." << endreq;
    }

    //iterating until the track is found or the end of the vector is reached
    bool trackFound = false;

    //getting a position and momentum of  atrack in question.   
    const Amg::Vector3D & pos = track->position();
    const Amg::Vector3D & mom = track->momentum();
    Trk::RecVertex * reducedVertex=0;

    reducedVertex=new Trk::RecVertex();

    for (;tb!=te;++tb)
    {

      //getting the link to track or track particle  
      const  Trk::ITrackLink * trackLink = (*tb)->trackOrParticleLink();

      if(trackLink)
      {
        const  TrackParameters *  parameters = trackLink->parameters();
        if(parameters)
        {
          if((parameters->position() == pos)&&(parameters->momentum() == mom) )
          {

          //checking whether the linTrack is available 
          //if not (reading ESD or AOD without refit of the  vertex, for 
          //instance) creating one.
            if(!(*tb)->linState()) m_linFactory->linearize( **tb, rvtx);
            //this track was alredy used in the fit. removing it before 
            //proceeding with the actual estimate     
            *reducedVertex = m_Updator->positionUpdate(rvtx,(*tb)->linState(),(*tb)->weight(),-1);
             trackFound = true;
              break;
          }//end of track comparison
        }//end of parameters validity check      
      }//end of track link validity check
    }

    if(trackFound) return reducedVertex;
    else delete reducedVertex;
  }

  return new RecVertex(rvtx);
...
const xAOD::Vertex * TrackToVertexIPEstimator::getUnbiasedVertex(const TrackParameters * track, const xAOD::Vertex * vtx ) const 
 {
   ...
   for (unsigned int itrk=0; itrk < vtx->nTrackParticles(); ++itrk) {
	 ...
	 if (vtx->vxTrackAtVertex().size() > itrk) {
       ...
       if (!linTrack) {
	 linTrack = m_linFactory->linearizedTrack(track, Vertex(vtx->position()));
 ...
 const ImpactParametersAndSigma  * TrackToVertexIPEstimator::estimate(const Trk::Track * track,
                                                                      const Trk::RecVertex* vtx)const
{
 ...
 const ImpactParametersAndSigma  * TrackToVertexIPEstimator::estimate(const Trk::TrackParticleBase * track,
                                                                      const Trk::RecVertex* vtx)const
{

  if (track && vtx){
    return estimate( &(track->definingParameters()), vtx);
  }
  return 0;
