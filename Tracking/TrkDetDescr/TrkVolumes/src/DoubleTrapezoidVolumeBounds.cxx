...
Trk::DoubleTrapezoidVolumeBounds::DoubleTrapezoidVolumeBounds(double minhalex, double medhalex, double maxhalex, double haley1, double haley2, double halez) :
 VolumeBounds(),
 m_minHalfX(minhalex),
 m_medHalfX(medhalex),
 m_maxHalfX(maxhalex), 
...
Trk::DoubleTrapezoidVolumeBounds::DoubleTrapezoidVolumeBounds(const Trk::DoubleTrapezoidVolumeBounds& trabo) :
 VolumeBounds(),
...
Trk::DoubleTrapezoidVolumeBounds& Trk::DoubleTrapezoidVolumeBounds::operator=(const Trk::DoubleTrapezoidVolumeBounds& trabo)
{
  if (this!=&trabo){
  m_minHalfX         = trabo.m_minHalfX;
  m_medHalfX         = trabo.m_medHalfX;
...
const std::vector<const Trk::Surface*>* Trk::DoubleTrapezoidVolumeBounds::decomposeToSurfaces(const Amg::Transform3D& transform) const
{
    ...
    Amg::Vector3D A(diamondCenter - this->minHalflengthX()*diamondX - 2*this->halflengthY1()*diamondY );
    Amg::AngleAxis3D alpha1ZRotation(this->alpha1(), Amg::Vector3D(0.,0.,1.));
    Amg::RotationMatrix3D  alpha1Rotation(diamondRotation*alpha1ZRotation*Amg::AngleAxis3D(-90*Gaudi::Units::deg, Amg::Vector3D(0.,1.,0.))*Amg::AngleAxis3D(90*Gaudi::Units::deg, Amg::Vector3D(0.,0.,1.)));
    RectangleBounds* faceAlpha1Bounds = this->faceAlpha1RectangleBounds();
    Amg::Vector3D   faceAlpha1Position(A+alpha1Rotation.col(0)*faceAlpha1Bounds->halflengthX());
    retsf->push_back(new Trk::PlaneSurface(new Amg::Transform3D(alpha1Rotation * Amg::Translation3D(faceAlpha1Position)),faceAlpha1Bounds));
    //   (4) - at point B, attached to beta opening angle 
    Amg::Vector3D B(diamondCenter + this->minHalflengthX()*diamondX - 2*this->halflengthY1()*diamondY );
    Amg::AngleAxis3D beta1ZRotation(-this->alpha1(), Amg::Vector3D(0.,0.,1.));
    Amg::RotationMatrix3D  beta1Rotation(diamondRotation*beta1ZRotation*Amg::AngleAxis3D(90*Gaudi::Units::deg, Amg::Vector3D(0.,1.,0.))*Amg::AngleAxis3D(90*Gaudi::Units::deg, Amg::Vector3D(0.,0.,1.)));
    RectangleBounds* faceBeta1Bounds = this->faceBeta1RectangleBounds();
    Amg::Vector3D   faceBeta1Position(B+beta1Rotation.col(0)*faceBeta1Bounds->halflengthX());
    retsf->push_back(new Trk::PlaneSurface(new Amg::Transform3D(beta1Rotation * Amg::Translation3D(faceBeta1Position)),faceBeta1Bounds));
    // face surfaces yz
    // transmute cyclical
    //   (5) - at point A', attached to alpha opening angle 
    Amg::Vector3D AA(diamondCenter - this->maxHalflengthX()*diamondX + 2*this->halflengthY2()*diamondY );
    Amg::AngleAxis3D alpha2ZRotation(-this->alpha2(), Amg::Vector3D(0.,0.,1.));
    Amg::RotationMatrix3D  alpha2Rotation(diamondRotation*alpha2ZRotation*Amg::AngleAxis3D(-90*Gaudi::Units::deg, Amg::Vector3D(0.,1.,0.))*Amg::AngleAxis3D(-90*Gaudi::Units::deg, Amg::Vector3D(0.,0.,1.)));
    RectangleBounds* faceAlpha2Bounds = this->faceAlpha2RectangleBounds();
    Amg::Vector3D   faceAlpha2Position(AA+alpha2Rotation.col(0)*faceAlpha2Bounds->halflengthX());
    retsf->push_back(new Trk::PlaneSurface(new Amg::Transform3D(alpha2Rotation * Amg::Translation3D(faceAlpha2Position)),faceAlpha2Bounds));
    //   (6) - at point B', attached to beta opening angle 
    Amg::Vector3D BB(diamondCenter + this->maxHalflengthX()*diamondX + 2*this->halflengthY2()*diamondY );
    Amg::AngleAxis3D beta2ZRotation(this->alpha2(), Amg::Vector3D(0.,0.,1.));
    Amg::RotationMatrix3D  beta2Rotation(diamondRotation*beta2ZRotation*Amg::AngleAxis3D(90*Gaudi::Units::deg, Amg::Vector3D(0.,1.,0.))*Amg::AngleAxis3D(-90*Gaudi::Units::deg, Amg::Vector3D(0.,0.,1.)));
    RectangleBounds* faceBeta2Bounds = this->faceBeta2RectangleBounds();
    Amg::Vector3D   faceBeta2Position(BB+beta2Rotation.col(0)*faceBeta2Bounds->halflengthX());
    retsf->push_back(new Trk::PlaneSurface(new Amg::Transform3D(beta2Rotation * Amg::Translation3D(faceBeta2Position)),faceBeta2Bounds));
    // face surfaces zx
    ...
    retsf->push_back(new Trk::PlaneSurface( new Amg::Transform3D( transform *Amg::Translation3D(Amg::Vector3D(0.,this->halflengthY2(),0.))
								*Amg::AngleAxis3D(-90*Gaudi::Units::deg, Amg::Vector3D(0.,1.,0.))*Amg::AngleAxis3D(-90.*Gaudi::Units::deg, Amg::Vector3D(1.,0.,0.))  ),
...
{
    return new Trk::DiamondBounds(m_minHalfX,m_medHalfX,m_maxHalfX,m_halfY1,m_halfY2);
}
...
bool Trk::DoubleTrapezoidVolumeBounds::inside(const Amg::Vector3D& pos, double tol) const
{
    if (fabs(pos.z()) > m_halfZ + tol) return false;
    if (pos.y() < -2* m_halfY1 - tol) return false;
    if (pos.y() > 2* m_halfY2 - tol) return false;
