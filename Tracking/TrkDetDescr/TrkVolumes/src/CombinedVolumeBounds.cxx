...
Trk::CombinedVolumeBounds::CombinedVolumeBounds(Volume* vol1, Volume* vol2, bool intersection ) :
 VolumeBounds(),
 m_first(vol1),
 m_second(vol2),
...
Trk::CombinedVolumeBounds::CombinedVolumeBounds(const Trk::CombinedVolumeBounds& bobo) :
 VolumeBounds(),
...
Trk::CombinedVolumeBounds& Trk::CombinedVolumeBounds::operator=(const Trk::CombinedVolumeBounds& bobo)
{
  if (this!=&bobo){
    m_first          = bobo.m_first;
    m_second         = bobo.m_second;
...
const std::vector<const Trk::Surface*>* Trk::CombinedVolumeBounds::decomposeToSurfaces ATLAS_NOT_THREAD_SAFE (const Amg::Transform3D& transf) const
{
    ...
    const std::vector<const Trk::Surface*>* firstSurfaces = m_first->volumeBounds().decomposeToSurfaces(transf*m_first->transform());
    // get surfaces for second boundaries
    const std::vector<const Trk::Surface*>* secondSurfaces = m_second->volumeBounds().decomposeToSurfaces(transf*m_second->transform());
    ...
    for (unsigned int out=0; out < firstSurfaces->size(); out++) {
      ...
      Trk::Volume* secondSub = createSubtractedVolume((*firstSurfaces)[out]->transform().inverse()*transf, m_second);     
    ...
    cylVol = dynamic_cast<const Trk::CylinderVolumeBounds*> (&(m_second->volumeBounds()));
    spbVol = dynamic_cast<const Trk::SimplePolygonBrepVolumeBounds*> (&(m_second->volumeBounds()));
    comVol = dynamic_cast<const Trk::CombinedVolumeBounds*> (&(m_second->volumeBounds()));
    subVol = dynamic_cast<const Trk::SubtractedVolumeBounds*> (&(m_second->volumeBounds()));
    ...
    for (unsigned int in=0; in< secondSurfaces->size(); in++) {
      ...
      Trk::Volume* firstSub = createSubtractedVolume((*secondSurfaces)[in]->transform().inverse()*transf, m_first);
      if ( scli || spli ) {
        bool shared = false;
    ...
    //std::cout <<"combined volume has :" << retsf->size() << "boundary surfaces" << std::endl;      
