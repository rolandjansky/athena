...
Trk::CombinedVolumeBounds::CombinedVolumeBounds(Volume* vol1, Volume* vol2,
                                                bool intersection)
    : VolumeBounds(),
      m_first(vol1),
      m_second(vol2),
...
const std::vector<const Trk::Surface*>*
  ...
  const std::vector<const Trk::Surface*>* firstSurfaces =
      m_first->volumeBounds().decomposeToSurfaces(transf *
                                                  m_first->transform());
  // get surfaces for second boundaries
  const std::vector<const Trk::Surface*>* secondSurfaces =
      m_second->volumeBounds().decomposeToSurfaces(transf *
                                                   m_second->transform());
  ...
  for (unsigned int out = 0; out < firstSurfaces->size(); out++) {
    ...
    if (sclo || splo) {
      ...
      if (dlo) {
        ...
        EllipseBounds* eb =
            new EllipseBounds(db->rMin(), db->rMin(), db->rMax(), db->rMax(),
                              db->halfPhiSector());
  ...
  cylVol = dynamic_cast<const Trk::CylinderVolumeBounds*>(
      &(m_second->volumeBounds()));
  spbVol = dynamic_cast<const Trk::SimplePolygonBrepVolumeBounds*>(
      &(m_second->volumeBounds()));
  comVol = dynamic_cast<const Trk::CombinedVolumeBounds*>(
      &(m_second->volumeBounds()));
...
Trk::Volume* Trk::CombinedVolumeBounds::createSubtractedVolume(
    const Amg::Transform3D& transf, Trk::Volume* subtrVol) const {
