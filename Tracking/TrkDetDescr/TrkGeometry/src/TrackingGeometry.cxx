...
Trk::TrackingGeometry::~TrackingGeometry() {
  ...
  for (; bLayerIter != bLayerIterE; ++bLayerIter) delete bLayerIter->first;
}
...
void Trk::TrackingGeometry::registerTrackingVolumes ATLAS_NOT_THREAD_SAFE(
    const Trk::TrackingVolume& tvol, const Trk::TrackingVolume* mvol, int lvl) {
  int sublvl = lvl + 1;
  std::string indent = "";
  ...
  if (confinedDetachedVolumes) {
    for (const auto& volumesIter : (*confinedDetachedVolumes))
      if (volumesIter &&
        ...
        registerTrackingVolumes(*(volumesIter->trackingVolume()), &tvol,
                                sublvl);
  }
...
void Trk::TrackingGeometry::printVolumeInformation(
    MsgStream& msg, const Trk::TrackingVolume& tvol, int lvl) const {
  int sublevel = lvl + 1;

  for (int indent = 0; indent < sublevel; ++indent) msg << "  ";
