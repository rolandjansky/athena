...
Trk::ConeCylinderConeLayer::ConeCylinderConeLayer(
    Amg::Transform3D* htrans, double radius, double halfLength, double zStitch,
    double alpha, SurfaceArray* surfaceArray,
    const LayerMaterialProperties& laymatprop, double thickness,
    OverlapDescriptor* od, int laytyp)
    :  // set up the encompassing cylinder (and most of the layer properties)
      CylinderLayer(htrans, new CylinderBounds(radius, halfLength),
                    surfaceArray, laymatprop, thickness, od, nullptr, laytyp),
      m_transform(htrans),
      m_radius(radius),
