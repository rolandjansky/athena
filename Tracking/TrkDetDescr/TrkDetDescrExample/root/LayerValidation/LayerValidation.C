      ...
      if (oneFile){
       std::cout << " [m] File loading successful ! " << std::endl;
           ...
           while (key=(TKey*)next()){
              ...
              if (treeName.Contains("LayerValidation") && treeTitle.Contains(titleContains)){
                  // get the TTree
                  TTree* currentTree = (TTree*)oneFile->Get(treeName);
                  if (currentTree){
                    // now decide whether its a cylinder tree or not
                    bool isCylinder =  (  treeName.Contains("Validation_1")
                                       || treeName.Contains("Validation_-1"));

                    // construct the LayerValidationTree
                    LayerValidationTree* layValTree = new LayerValidationTree(currentTree);
                      layValTree->Loop(isCylinder, picExtension);
                    //delete layValTree;
                }
              }
            }
