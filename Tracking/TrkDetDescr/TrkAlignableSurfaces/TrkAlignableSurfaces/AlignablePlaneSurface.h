...
/**
 ...
 transform describing the local frame with respect to the global frame as @f$ T
 @f$, then the methods (1), (2) and (3) yield to :
 ...
 To avoid dependencies on GeoModel and/or InnerDetector pure Amg::Transform3D
 objects are used.
...
public:
  ...
  AlignablePlaneSurface(
    ...
    std::optional<Amg::Transform3D> alignedTransf = std::nullopt);
  ...
  /** return Identifier of the associated Detector Element - forwarded from
   * nominal Surface */
