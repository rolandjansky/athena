...
public:
  ...
  BinnedArray2DT(const std::vector<std::pair<SharedObject<T>, Amg::Vector3D>>& tclassvector, BinUtility* bingen)
    : BinnedArrayT<T>()
    , m_array(0)
    , m_arrayObjects(nullptr)
    , m_binUtility(bingen)
  {
    ...
    if (bingen) {
      m_array = new std::vector<std::vector<SharedObject<T>>*>(bingen->bins(1));
      for (size_t i = 0; i < bingen->bins(1); ++i)
        (*m_array)[i] = new std::vector<SharedObject<T>>(bingen->bins(0));
      ...
      for (size_t ivec = 0; ivec < vecsize; ++ivec) {
        ...
        if (bingen->inside(currentGlobal)) {
          std::vector<SharedObject<T>>* curVec = (*m_array)[bingen->bin(currentGlobal, 1)];
    ...
    , m_binUtility(0)
  {
    ...
    // copy over
    m_array = new std::vector<std::vector<SharedObject<T>>*>(barr.m_array->size());
    for (size_t ihl = 0; ihl < barr.m_array->size(); ++ihl) {
      (*m_array)[ihl] = new std::vector<SharedObject<T>>(((*barr.m_array)[0])->size());
