...
public:
  ...
  /** Returns the pointer to the templated class object from the BinnedArray
      it returns 0 if not defined;
   */
  const T* object(const Amg::Vector3D& gp) const { return ((*m_array)[m_binUtility->bin(gp, 0)]).get(); }

  /** Returns the pointer to the templated class object from the BinnedArray - entry point*/
  const T* entryObject(const Amg::Vector3D& gp) const { return ((*m_array)[m_binUtility->entry(gp, 0)]).get(); }
