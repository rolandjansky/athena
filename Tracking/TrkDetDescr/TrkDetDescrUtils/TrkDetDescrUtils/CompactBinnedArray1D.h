...
template<class T>
class CompactBinnedArray1D : public CompactBinnedArray<T>
{
...
public:
  ...
  CompactBinnedArray1D(const std::vector<const T*>& tclassvector,
    ...
    , m_binUtility(bingen)
  {
    ...
    if (indexvector.size() != bingen->bins())
      std::cout << " problem in construction of CompactBinnedArray1D: number of indexes not compatible with BinUtility:"
    ...
    if (iMax > tclassvector.size() - 1)
      std::cout << " problem in construction of CompactBinnedArray1D:runaway index:" << iMax << ","
