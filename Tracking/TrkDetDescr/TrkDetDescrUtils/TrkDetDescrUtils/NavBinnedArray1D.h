...
template<class T>
class NavBinnedArray1D : public BinnedArray<T>
{
...
public:
  ...
  NavBinnedArray1D(const std::vector<SharedObject<const T>>& tclassvector,
                   BinUtility* bingen,
  ...
  */
  ...
  const T* entryObject(const Amg::Vector3D&) const { return ((*m_array)[0]).get(); }
