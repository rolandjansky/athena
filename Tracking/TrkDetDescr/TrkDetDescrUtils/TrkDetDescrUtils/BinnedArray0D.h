...
public:
  ...
  BinnedArray0D* clone() const { return new BinnedArray0D(*this); }

  /**Virtual Destructor*/
  ~BinnedArray0D() { delete m_arrayObjects; }

  /** Returns the pointer to the templated class object from the BinnedArray,
      it returns 0 if not defined */
  const T* object(const Amg::Vector2D&) const { return m_object.getPtr(); }

  /** Returns the pointer to the templated class object from the BinnedArray
      it returns 0 if not defined */
  const T* object(const Amg::Vector3D& gp) const { return m_object.getPtr(); }

  /** Returns the pointer to the templated class object from the BinnedArray,
      it returns 0 if not defined */
  const T* entryObject(const Amg::Vector3&) const { return m_object.getPtr(); }
