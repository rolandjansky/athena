...
template<class T>
class BinnedArrayArray : public BinnedArray<T>
{
...
public:
  ...
  virtual BinnedArrayArray* clone() const override { return new BinnedArrayArray(*this); }
  ...
  virtual const T* entryObject(const Amg::Vector3D& gp) const override { return object(gp); }

  /** Returns the pointer to the templated class object from the BinnedArrayArray, takes 3D position & direction */
  virtual const T* nextObject(const Amg::Vector3D& gp, const Amg::Vector3D&, bool) const override { return object(gp); }

  /** Return all objects of the Array */
  virtual const std::vector<const T*>& arrayObjects() const override { return m_arrayObjects; }
