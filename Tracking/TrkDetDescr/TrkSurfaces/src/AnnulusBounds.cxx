...
Trk::AnnulusBounds::AnnulusBounds(double minR, double maxR, double R, double phi, double phiS) : m_boundValues(AnnulusBounds::bv_length, 0.) 
{
  ...
  m_k_L = tan( (M_PI+phi)/2.0 - phiS); // Gradiant of left edge
  m_k_R = tan( (M_PI-phi)/2.0 - phiS); // Gradiant of right edge
  
  m_d_L = R*sin(-phiS)*tan( (M_PI-phi)/2.0 + phiS) + R*(1.0-cos(-phiS));	// Intercept of left edge with y-axis
  m_d_R = R*sin(-phiS)*tan( (M_PI+phi)/2.0 + phiS) + R*(1.0-cos(-phiS)); // Intercept of right edge with y-axis
  
  

  
  // solving quadratic equation to find four corners of the AnnulusBounds
  m_solution_L_min = circleLineIntersection(minR, m_k_L, m_d_L);  //D
  m_solution_L_max = circleLineIntersection(maxR, m_k_L, m_d_L);  //C
  m_solution_R_min = circleLineIntersection(minR, m_k_R, m_d_R);  //A
  m_solution_R_max = circleLineIntersection(maxR, m_k_R, m_d_R);  //B
...
{
  if (this!=&annbo) m_boundValues = annbo.m_boundValues;
...
std::vector<double> Trk::AnnulusBounds::circleLineIntersection(double R, double k, double d) const
{
  ...
  if (delta < 0) // Does not intersect (imaginary solutions)
    return solution;
  else { // at least 1 intersect
