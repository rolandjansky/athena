...
#
#   Author: James Robinson <james.robinson@cern.ch> 31st October 2012

#   Added To: Daniel Hayden <danhayden0@gmail.com> 31st October 2012 : ZZ, WW, WZ Diboson Processes. 
#
#########################################################################################################################
...
class PowhegConfig_base:
    ...
    # These can be changed in the jobOptions - default values set here.
    # Put process-specific options in the appropriate class though
    nEvents = 10
    energy = 4000
    muF = 1.0
    muR = 1.0
    pdf = 10800 #(CT10) - POWHEG can set each proton with a different PDF, for now sticking to useing the same value for each one
    storePdfInfo = 1
    use_old_grid = 1
    use_old_ubound = 1
    ncall1 = 20000
    ...
    def generateRunCardSharedOptions(self):
        ...
        print "Writing POWHEG runcard to "+str(TestArea)+"/powheg.input"

        with open( str(TestArea)+'/powheg.input','w') as f:        
            f.write( "iseed 228096305                     ! Start the random number generator with seed iseed\n" )
...
class PowhegConfig_hvq(PowhegConfig_base):
    # These can be changed - put generic properties in PowhegConfig_base though
    bbscalevar = 1
    quarkMass = 4.95
    iymax = 1
    ixmax = 1
...
class PowhegConfig_Dijet(PowhegConfig_base):
    #POWHEG Dijet specific variables - can be chaged through JOs
    bornktmin = 1
    bornsuppfact = -1
    ...
    def generateRunCard(self):
        ...
        with open( str(TestArea)+'/powheg.input','a') as f:
            ...
            f.write( "bornsuppfact "+str(self.bornsuppfact)+"     ! (default 0d0) Mass parameter for Born suppression factor. If < 0 suppfact = 1\n" )
        # with open( str(TestArea)+'/powheg.input','r') as g:
        #     print "*************************"
        #     print g.read() 
        #     print "*************************"
