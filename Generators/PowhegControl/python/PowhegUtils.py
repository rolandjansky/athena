...
class PowhegConfig_base() :
  # As this is a dummy file now it might as well be generic
  __output_events_wrapper = 'PowhegOutput._1'
  __output_events_file_name = 'events.lhe'
  ...
  ncall1, ncall2 = 20000, 100000 # defaults are: 50000, 50000
  ...
  def __init__(self,runArgs=None) :
    ...
    if runArgs == None :
      ...
      if hasattr(runArgs,'inputGeneratorFile') :
        ...
        self.__output_events_file_name = genFileList[0]+'.'+genFileList[1]+'.events'
  ...
  def generateRunCardSharedOptions(self) :
    ...
    if self.withnegweights != 0 and self.foldx * self.foldy * self.foldphi == 1 :
      # Performing a folded integration is recommended if using negative weights: allowed values for folding parameters are 1, 2, 5, 10, 25, 50
      self.foldx, self.foldy, self.foldphi = 5, 5, 2
      logger.warning( 'Requesting negative weights without explicitly setting folding parameters! This is unlikely to work: setting folding parameters to foldx({0}) foldy({1}) foldphi({2})'.format(self.foldx,self.foldy,self.foldphi) )
  ...
  def generateFilteredEvents(self) :
    ...
    logger.info( str(filterPath) )
    LHEF_filter = subprocess.Popen( self.filterRunArgs )
    LHEF_filter.wait()
