...
class chi0chi0(PowhegV2):
    ...
    def validate_weakinos(self):
        ...
        for parameter in ["neutralino_A", "neutralino_B"]:
            weakino_name = list(self.parameters_by_name(parameter))[0].value
            self.check_decay_mode(weakino_name, self.allowed_weakinos.keys())
            weakinos.append(self.allowed_weakinos[weakino_name])
        list(self.parameters_by_name("neutralino_A"))[0].value = min(weakinos)
        list(self.parameters_by_name("neutralino_B"))[0].value = max(weakinos)
