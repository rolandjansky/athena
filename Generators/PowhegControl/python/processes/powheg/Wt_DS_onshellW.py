...
class Wt_DS_onshellW(PowhegV2):
    ...
    def __init__(self, base_directory, **kwargs):
        ...
        """
        super(Wt_DS_onshellW, self).__init__(base_directory, "ST_wtch_DS_onshellW", **kwargs)
        ...
        self.add_keyword("nubound", 900000)
        self.add_keyword("nwidthcutoff")
        ...
        self.add_keyword("withfluxfactor")
        self.add_keyword("withnegweights")
        self.add_keyword("withsubtr")
        self.add_keyword("withthetacut")
    ...
    def validate_decays(self):
        ...
        if self.decay_mode_top not in self.allowed_top_decay_modes:
            logger.warning("Decay mode {} not recognised!".format(self.decay_mode_top))
            logger.info("Allowed decay modes are:")
            [logger.info("... {}".format(decay_mode)) for decay_mode in self.allowed_top_decay_modes]
            raise ValueError("Decay mode {} not recognised!".format(self.decay_mode_top))
        if self.decay_mode_W not in self.allowed_W_decay_modes:
            logger.warning("Decay mode {} not recognised!".format(self.decay_mode_W))
            logger.info("Allowed decay modes are:")
            [logger.info("... {}".format(decay_mode)) for decay_mode in self.decay_mode_W]
            raise ValueError("Decay mode {} not recognised!".format(self.decay_mode_W))
