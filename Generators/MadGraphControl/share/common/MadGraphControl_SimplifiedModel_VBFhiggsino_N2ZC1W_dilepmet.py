...
decaytype='N2_ZN1' # Pure Higgsino
...
masses['1000022'] =  1.0 * MassToFloat(splitConfig[5]) # LSP mass
masses['1000023'] = -1.0 * MassToFloat(splitConfig[4]) # N2, make LSP higgsino like (positive for wino)
masses['1000024'] =  0.5 * (MassToFloat(splitConfig[4])+MassToFloat(splitConfig[5])) # Chargino is between N1 and N2

# interpret the generation type, so we know which processes to run.
gentype = splitConfig[3]
if not 'VBF' in gentype:
    raise RuntimeError("This control file is only for Higgsino VBF production")
...
elif 'N2N1' in gentype or 'N1N2' in gentype:
    mergeproc="Merging:Process = pp>{n2,1000023}{n1,1000022}"
    msdecaystring="decay n2 > l+ l-  n1\n" # N2 to leptons
    mgprocstring="p p > n2 n1 j j"
...
gentype = "C1N1" # Use given pure Higgsino param file
#//////////////////////////////////////////////////////////////////////////////
...
evgenLog.info('Registered generation of higgsino-like ~chi2/~chi1/~chi1+/~chi1- production via VBF, decay via SM W/Z to 2 leptons; grid point '+str(runArgs.runNumber)+' decoded into LSP mass point ' +str(masses['1000022'])+ ' with ' +str(dM)+ ' GeV splitting')
evgenConfig.contact  = [ "andrew.todd.aukerman@cern.ch" ]
evgenConfig.keywords += ['SUSY', 'VBF', 'MSSM', 'chargino', 'neutralino', 'Higgsino']
