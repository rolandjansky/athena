...
def parse_JO(input_jobconfig,processes):
    jobconfig_name=input_jobconfig.replace('.py','')

    # check if reweighting required
    reweight_name=None
    for s in jobconfig_name.split('_'):
        if 'rwgt' in s:
            reweight_name=s.replace('rwgt','')
            jobconfig_name=jobconfig_name.replace('_'+s,'')
            break
    ...
    if reweight_name==None and param_name==None:
        ...
        eft_params_list=re.findall(params_re,jobconfig_name)
        eft_params={}
        for e in eft_params_list:
            eft_params[e[0]]=e[1].replace('p','.')
        jobconfig_name=re.sub(params_re,"",jobconfig_name)
    ...
    jobconfig_tokens=jobconfig_name.split('_')
...
def create_SMEFT_cards(input_jobconfig,processes):
    arguments=parse_JO(input_jobconfig,processes)
    return get_SMEFT_cards(*arguments)

def get_SMEFT_cards(model_nick,process,param_name=None,reweight_name=None,eft_params=None):
           
    #determine model from nick and use naming conventions of model
    ...
    # get param card and set non-zero parameters
        ...
        param_card_loc='MadGraph_restrict_card_'+model_nick+'_'+param_name+'.dat'
        get_param_file = subprocess.Popen(['get_files','-data', param_card_loc])
        get_param_file.wait()
        if not os.path.exists(param_card_loc):
            raise RuntimeError("Cannot find "+param_card_loc)
    ...
    # dynamically create restricted model that only contains the relevant operators
        ...
        get_param_file = subprocess.Popen(['get_files','-data', param_card_default])
        get_param_file.wait()
        if not os.path.exists(param_card_default):
            raise RuntimeError("Cannot find "+param_card_default)
        param_card_loc=param_card_default.replace('.dat','_updated.dat')
        if len(eft_params)>0:
            build_param_card(param_card_default,param_card_loc,params={eft_parameter_block:eft_params})
        else:
            shutil.copy(param_card_default,param_card_loc)
        ...
        if len(eft_params_to_keep)>0:
            build_param_card(param_card_default,restricted_model+'/'+'restrict_'+restriction+'.dat',params={eft_parameter_block:eft_params_to_keep})
        else:
    ...
    proc_card+="\n"
    proc_card+=process+"\noutput -f\n"
    proc_card_loc='proc_card.dat'
    f_proc_card=open(proc_card_loc,'w')
    f_proc_card.write(proc_card)
    f_proc_card.close()


    return proc_card_loc,param_card_loc,reweight_card_loc
