...
def new_process(card_loc='proc_card_mg5.dat',grid_pack=None):
    ...
    thedir = None
    for adir in glob.glob( os.getcwd()+'/*PROC*~' ).sort(reverse=True):
    ...
    if thedir==None:
        raise RuntimeError('No diagrams for this process in dir='+str(thedir)+' from list: '+glob.glob( os.getcwd()+'/*PROC*~' ).sort(reverse=True))
