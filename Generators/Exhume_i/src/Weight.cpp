...
void Exhume::Weight::WeightInit(const double &min, const double &max){
  ...
  for(unsigned int jj = 1; jj10<NPoints;jj++){
    ...
    for(unsigned int kk=1; kk<jj10; kk++){
      ...
      UNLineShape.insert(std::pair<double, double>
			 ( UNLineShape.rbegin()->first + 
			   0.5 * (NewF + OldF) * (x - Oldx), x) );
    }
  }
  ...
  jj++;
  for(std::map<double, double>::iterator ii = FuncMap.begin();
      jj!=FuncMap.end(); ii++){
    UNLineShape.insert(std::pair<double, double> 
		       (UNLineShape.rbegin()->first + 
			0.5 * (ii->second + jj->second) * 
			(jj->first - ii->first), jj->first) );
    jj++;
  }
  ...
  std::cout<<std::endl<<"  "<<LineShape.size()<<
        " points calculated mapping {"<<LineShape.begin()->first<<","<<
        LineShape.rbegin()->first<<"} to {"<<
        (LineShape.begin())->second<<","<<(LineShape.rbegin())->second<<
        "}"<<std::endl<<std::endl;
...
void Exhume::Weight::AddPoint(const double &xx_, const double &f_){
  ...
  double OldSegment = 
    0.5 * (high_->second + low_->second) * (high_->first - low_->first);

  double NewSegment = 
    0.5 * ((low_->second + f_) * (xx_ - low_->first) + 
	   (high_->second + f_) * (high_->first - xx_));
