...
SUSYGGMHiggsinoPhotonFilter::SUSYGGMHiggsinoPhotonFilter(const std::string& name, ISvcLocator* pSvcLocator)
...
StatusCode SUSYGGMHiggsinoPhotonFilter::filterEvent() {
  ...
  for (itr = events_const()->begin(); itr!=events_const()->end(); ++itr) {
    ...
    ATH_MSG_DEBUG("----->>> Process : " << genEvt->signal_process_id());
     ...
     //
    int count_H(0), count_h(0), count_Z(0);
    for (HepMC::GenEvent::particle_const_iterator pitr=genEvt->particles_begin(); pitr!=genEvt->particles_end(); ++pitr) {

	if ((*pitr)->status() != 22) continue;
	...
	if (abs((*pitr)->pdg_id()) == 35 && pdgindex == 1000022) count_H++;
	if (abs((*pitr)->pdg_id()) == 25 && pdgindex == 1000022) count_h++;
	if (abs((*pitr)->pdg_id()) == 23 && pdgindex == 1000022) count_Z++;

	}
    }

    if(count_H==1 &&  count_h==1 && count_Z==0) {
	setFilterPassed(true);
    }
    else if(count_H==0 &&  count_h==1 && count_Z==1){
	setFilterPassed(true);
    }
    else {
