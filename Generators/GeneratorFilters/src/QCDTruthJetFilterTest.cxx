...
QCDTruthJetFilterTest::QCDTruthJetFilterTest(const std::string& name, ISvcLocator* pSvcLocator)
  ...
  , m_high(1.)
{
  ...
  declareProperty("NoFilter",m_noFilter = true);
}


StatusCode QCDTruthJetFilterTest::filterInitialize() {
...
StatusCode QCDTruthJetFilterTest::filterEvent() {
  ...
  CLHEP::HepRandomEngine* rndm = m_rand->GetEngine("QCDTruthJetFilterTest");
  if (!rndm) {
    ATH_MSG_WARNING("Failed to retrieve random number engine QCDTruthJetFilterTest");
    
    if (m_noFilter) {
      for (unsigned int i=0;i<mec->size();++i) {
	if ( !(*mec)[i] ) continue;
	if ((*mec)[i]->weights().size()>0) (*mec)[i]->weights()[0] = 0.;
	else (*mec)[i]->weights().push_back( 0. );
	ATH_MSG_INFO("Setting weight " << i << " to zero");
      }
...
//    if ((*it_truth)->eta()  > m_MaxEta || (*it_truth)->eta()  <= m_MinEta) continue;
  ...
  // Set weights of events that passed 
