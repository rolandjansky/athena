...
StatusCode MultiCjetFilter::filterFinalize() {
  ...
  msg(MSG::INFO) <<  m_SumOfWeights_Pass << " out of " << m_SumOfWeights_Evt << " SumOfWeights counter, passed/total" << endreq;
  return StatusCode::SUCCESS;
}
...
StatusCode MultiCjetFilter::filterEvent() {
  ...
  for(itr = events_const()->begin(); itr!=events_const()->end(); ++itr) {
    ...
    for(pitr=genEvt->particles_begin(); pitr!=genEvt->particles_end(); ++pitr ) {
      ...
      if( fabs( (*pitr)->momentum().pseudoRapidity() ) > m_charmEtaMax) continue;
