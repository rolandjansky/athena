...
# This is taken from upstream (Sherpa 2.2.9) https://gitlab.com/sherpa-team/sherpa/-/merge_requests/274
...
genSeq.Sherpa_i.Parameters += [ "MODEL=SM_sinThetaW_mZ",
                                "EW_SCHEME=22",
                                "SIN2THETAW=0.23113",
]
...
namespace MODEL {
  ...
  class SM_sinThetaW_mZ: public Model_Base {
  private:
  ...
  public :
    ...
    SM_sinThetaW_mZ(std::string,std::string);
...
DECLARE_GETTER(SM_sinThetaW_mZ,"SM_sinThetaW_mZ",Model_Base,Model_Arguments);

Model_Base *Getter<Model_Base,Model_Arguments,SM_sinThetaW_mZ>::
operator()(const Model_Arguments &args) const
{
  return new SM_sinThetaW_mZ(args.m_path,args.m_file);
}

void Getter<Model_Base,Model_Arguments,SM_sinThetaW_mZ>::
...
SM_sinThetaW_mZ::SM_sinThetaW_mZ(string _dir,string _file) :
  Model_Base(_dir,_file,1)
{
  m_name="SM_sinThetaW_mZ";
  ParticleInit();
  CustomContainerInit();
}

void SM_sinThetaW_mZ::ParticleInit()
{
...
bool SM_sinThetaW_mZ::ModelInit(const PDF::ISR_Handler_Map& isr)
{
  FixEWParameters();  
  FixCKM();
...
void SM_sinThetaW_mZ::FixEWParameters()
{
...
void SM_sinThetaW_mZ::InitVertices()
{
  InitQEDVertices();
  InitQCDVertices();
  InitEWVertices();
...
void SM_sinThetaW_mZ::InitQEDVertices()
{
...
void SM_sinThetaW_mZ::InitQCDVertices()
{
...
void SM_sinThetaW_mZ::InitEWVertices()
{
