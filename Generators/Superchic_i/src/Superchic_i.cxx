...
// to generate Superchic_v3.03 events and store the result in the
// Transient Store.
//
// AuthorList:
...
{}
  ...
  bool Superchic_i::prepare_params_file()
  {
    ...
    for(CommandVector::iterator i = m_InitializeVector.begin(); i != m_InitializeVector.end(); i++ )
    {
      ...
      else if (myparam == "s2int")
      {
        m_s2int  = mystring.numpiece(2);
      }
      ...
      {
        ATH_MSG_ERROR( " ERROR in Superchic3.03 INITIALIZATION PARAMETERS  "
    ...
    configFile << m_beam       << "                           ! [beam] : Beam type ('prot', 'ion', 'ionp' or 'electron') " << std::endl;
    ...
    configFile <<  m_nev  << "                                 ! [nev] : Number of events ( < 1000000 recommended) " << std::endl;
    ...
    configFile << m_gax << "                                   ! [gax] : ALP coupling (GeV^-2) " << std::endl;
