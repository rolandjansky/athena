...
namespace Pythia8 {
  ...
  public:
    ...
    virtual double multiplySigmaBy(const SigmaProcess* sigmaProcessPtr, 
      ...
      // event kinematics 
      double mHat = std::sqrt(phaseSpacePtr->sHat());
      ...
      if( mHat < mHatConstMin ){
        scale = std::pow(mHat/ecm, slope);
      }
      // const high-mass
      else {
        double xsec    = std::pow(mHat/ecm, p1) * std::pow(1 - mHat/ecm, p2);
        double xsecMin = std::pow(mHatConstMin/ecm, p1) * std::pow(1 - mHatConstMin/ecm, p2);
        scale = std::pow(mHatConstMin/ecm, slope) * xsecMin / xsec;
      }
      return scale; 
    }
