  ...
  struct WZPolarization : public UserHooks {
    ...
    bool doVetoResonanceDecays(Event& process) {
      ...
      for (int i = 1; i < process.size(); ++i) {
        ...
        // cout << v.id() << " -> " << d1.id() << " " << d2.id() << endl;

        // Get daughter momenta in the boson rest frame
        Vec4 pd1 = d1.p();
        Vec4 pd2 = d2.p();
        ///cout << "P1lab " << d1.p() << "P2lab " << d2.p() << endl;
        pd1.bstback(pv);
        pd2.bstback(pv);
        // cout << "P1    " << pd1 << "P2    " << pd2 << endl;
        ...
        while (true) {
          ...
          if (polmode == 0) { // Isotropic
            ...
            if (rand01() < sqr(sin(th))) break;
          } else if (polmode == 2) { // Trans-sum
            if (rand01() < (1 + sqr(cos(th)))/2) break;
          } else if (polmode == 3) { // Trans+
            if (rand01() < sqr(1 + cos(th))/4) break;
        ...
        pd1.bst(pv); d1.p(pd1);
        pd2.bst(pv); d2.p(pd2);
        ///cout << "P1'lab" << d1.p() << "P2'lab" << d2.p() << endl;
