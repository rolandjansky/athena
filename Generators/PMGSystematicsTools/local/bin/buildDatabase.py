...
Hessian = ['90900', '90700', '91900', '91700', '90400']
...
def nom(gen, dictionary, weight_dict):
  ...
  if nominal:
    ...
    # print n
    pdf = getPDF(n)  # next(re.finditer(r'\d+$',n)).group(0)
    # pdf = str(getPDF(n))
...
def alternate(gen, dictionary, weight_dict, p_nom):
  ...
  if alt:
      for element in alt.keys():
          pdf = getPDF(element)  # next(re.finditer(r'\d+$', element)).group(0)
          # print pdf
          if any(pdf == pdfset for pdfset in pdfsets):
              ...
              # print element
              pdf = getPDF(element)  # next(re.finditer(r'\d+$', element.split('_')[0])).group(0)
              w.append(pdf)
  lst = list(set(w))
  # print lst
  for p in lst:
      temp = dict((key, value) for (key, value) in alt.items() if str(p) in key)
      weight_dict.update({gen + '_ME_PDF' + str(p) + '_var': {'weights': sorted(temp.values()), 'combination': 'lhapdf', 'type': 'PDF_ME', 'nominal': sorted(temp.values())[0], 'nominal_pdf': p}})
  ...
  for key in weight_dict.keys():
      ...
      if any(key == datakey for datakey in data.keys()):
          if any(weight_dict.get(key) == data.get(k) for k in data.keys()):
              ...
              lenl = len(list(k for k in data.keys() if key in k))
              new_key = key + '_type' + str(lenl + 1)
...
def getMGkeys(dsid, dictionary):
  ...
  if nom_pdf:
    for key, val in PDFset.items():
      if val in nom_pdf:
        p_nom = key
        print("Nominal PDF =" + p_nom)
        flag2 = False
  if ((did >= 502198) and (did <= 502353)) or ((did > 367514) and (did < 367671)) or ((did > 502197) and (did < 502354)) or did == 412006:
    p_nom = '260000'
    flag2 = False
  elif ((did >= 302266) and (did <= 302290)) or ((did > 307730) and (did < 307749)) or (did == 309529) or ((did > 450764) and (did < 450775)):
    p_nom = '247000'
    print("got HVT" + p_nom)
    flag2 = False
  elif did == 312655:
...
def weightData(gen, dictionary):
  ...
  if gen == 'Sherpa' or gen == 'MGPy':
      ...
      other = dict((key, value) for (key, value) in dictionary.items() if key.startswith('other_'))
      dictionary = dict(set(dictionary.items()) - set(other.items()))
      dictionary, weight_dict = alternate('Sherpa', dictionary, weight_dict, nom_pdf)
      if dictionary or other:
          dictionary.update(other)
          weight_dict.update({'Sherpa_Other': {'weights': sorted(dictionary.values()), 'combination': 'none', 'type': 'unknown'}})
  elif gen == 'Powheg' or gen == 'aMcAtNlo' or gen == 'MadGraph':
      nom_pdf = dictionary.get('Nominal')
      print(nom_pdf)
...
def makePowhegDatabase(dsid, values):
  ...
  for value in values:
      ...
      if value.strip().lower() == 'nominal':
          ...
          if 'isr' in value:
              ph_name = 'other_' + value
          elif 'dyn_scale_choice' in value:
              ph_name = 'other_' + value
          else:
          ...
          if any(pdf == val for val in MGcentral_pdf):
              if p_var == '0':
                  ph_name = 'Nominal'
              else:
          ...
          if any(pdf == alpha for alpha in alpha_S.keys()):
              ...
              ph_name = 'PDF' + pdf
      elif (value.strip() != '9' or value.strip() == '') and (dsid == d for d in affectedDSID):
          print("nominal found: " + value)
          ph_name = 'Nominal'
      elif value.strip() == '110' and (dsid == d for d in affectedDSID):
          print("alt. PDF found")
          ph_name = 'PDF246800'
      else:
          if value.startswith('Var3c') or value.startswith('hard') or value.startswith('isr'):
              ...
              ph_name = value.strip()
      # if value.strip().isdigit() and dsid not in affectedDSID:
...
def getKey(filename):
  ...
  if file_is_empty(filename):
      ...
      with open(filename) as f:
          ...
          print(len(values))
          if len(values) == 1 and values[0].strip() == 'nominal':
              keys = ['nominal']
          # if 'MGPy' in filename:
          #    new_dict = makeSherpaDatabase(values)
          #    keys = weightData('MGPy', new_dict)
          elif 'MadGraph' in filename:
              new_dict = makePowhegDatabase(dsid, values)
