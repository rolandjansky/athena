 ...
 for dirName in os.listdir(testSampleDir):
  ...
  if identifier in dirName:
     ...
     for fileName in os.listdir(testSamplePath):
       ...
       if 'EVNT' not in fileName: continue
...
def main(argv):
  ...
  for line in f.readlines():
    ...
    pathenaCommandLine = r"pathena --nFilesPerJob = 100 --long --extOutFile \*.yoda --inDS=%s --outDS=%s --extFile = RivetAnalysis_%s.so,%s.yoda %s" % (thisSampleName, outputSampleName, opts.analysis, opts.analysis, thisSampleJobOption)
    analysis_files = []
    if isRivet:
    ...
    pathenaCommandLine = r"pathena --extOutFile \*.%s --inDS=%s --outDS=%s %s %s %s" % (rootOrYoda, thisSampleName, outputSampleName, analysis_files, thisSampleJobOption, opts.pathenaOptions)
    if opts.dryRun:
