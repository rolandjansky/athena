      ...
      if rootOrYoda == 'root':
        ...
        if structure == "AllVariationsInFile":
          if rivet_i_replacements(s) != "" and not opts.noSyst:
            command = "yodamerge_tmp.py --add -o %s %s -m '.*\[%s\].*' &> out.txt " % (mergedFile, " ".join(allFilesToMerge[safeFileName(s)]), rivet_i_replacements(s, escapePlus=True))
            if debug:
              print(command)
              print("sed -i 's/\[%s\]//g' %s" % (rivet_i_replacements(s), mergedFile))
            os.system(command)
