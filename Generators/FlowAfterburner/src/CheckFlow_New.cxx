  ...
  Algorithm(name, pSvcLocator)
{
  ...
  for ( int i = 0; i< 6; i++  ){
    ...
    hist_Psi_n_reco[i] = 0;
    hist_Psi_n_ebe[i] = 0;
    hist_Psi_n_ebe_pt[i] = 0;
    hist_vn_ebe[i] = 0;
  }

  for ( int i = 0; i< 36; i++  ){
    hist_psi_corr_true[i] = 0;
    hist_psi_corr_reco[i] = 0;
  }
...
StatusCode CheckFlow_New::initialize(){
  ...
  msglog << MSG::INFO << ">>> CheckFlow_New from Initialize" << endreq;
...
// /*
    ...
    if ( rootHistSvc->regHist(histPath+hist_Psi_n_true[ihar]->GetName(),hist_Psi_n_true[ihar]).isFailure() ){
      msglog << MSG::WARNING << "Can't book "<< histPath+hist_Psi_n_true[ihar]->GetName() << endreq;
    }
    ...
    if ( rootHistSvc->regHist(histPath+hist_Psi_n_reco[ihar]->GetName(),hist_Psi_n_reco[ihar]).isFailure() ){
      msglog << MSG::WARNING << "Can't book "<< histPath+hist_Psi_n_reco[ihar]->GetName() << endreq;
    }
    ...
    for (int ihar2=0;ihar2<6;ihar2++)
    {
      ...
      if ( rootHistSvc->regHist(histPath+hist_psi_corr_true[ihar_i]->GetName(),hist_psi_corr_true[ihar_i]).isFailure() ){
        msglog << MSG::WARNING << "Can't book "<< histPath+hist_psi_corr_true[ihar_i]->GetName() << endreq;
      }
      ...
      if ( rootHistSvc->regHist(histPath+hist_psi_corr_reco[ihar_i]->GetName(),hist_psi_corr_reco[ihar_i]).isFailure() ){
        msglog << MSG::WARNING << "Can't book "<< histPath+hist_psi_corr_reco[ihar_i]->GetName() << endreq;
      }
    ...
    if ( rootHistSvc->regHist(histPath+hist_vn_ebe[ihar]->GetName(),hist_vn_ebe[ihar]).isFailure() ){
      msglog << MSG::WARNING << "Can't book "<< histPath+hist_vn_ebe[ihar]->GetName() << endreq;
    }
    ...
    if ( rootHistSvc->regHist(histPath+hist_Psi_n_ebe[ihar]->GetName(),hist_Psi_n_ebe[ihar]).isFailure() ){
      msglog << MSG::WARNING << "Can't book "<< histPath+hist_Psi_n_ebe[ihar]->GetName() << endreq;
    }
    ...
    if ( rootHistSvc->regHist(histPath+hist_Psi_n_ebe_pt[ihar]->GetName(),hist_Psi_n_ebe_pt[ihar]).isFailure() ){
      msglog << MSG::WARNING << "Can't book "<< histPath+hist_Psi_n_ebe_pt[ihar]->GetName() << endreq;
    }
    ...
    for(int ieta=0;ieta<n_etabin;ieta++){
      ...
      if ( rootHistSvc->regHist(histPath+profile_pt_dep[ihar][ieta]->GetName(),profile_pt_dep[ihar][ieta]).isFailure() ){
        msglog << MSG::WARNING << "Can't book "<< histPath+profile_pt_dep[ihar][ieta]->GetName() << endreq;
      }
    ...
    for(int ipt=0;ipt<n_ptbin;ipt++){
      ...
      if ( rootHistSvc->regHist(histPath+profile_eta_dep[ihar][ipt]->GetName(),profile_eta_dep[ihar][ipt]).isFailure() ){
        msglog << MSG::WARNING << "Can't book "<< histPath+profile_eta_dep[ihar][ipt]->GetName() << endreq;
      }
    ...
    for(int ieta=0;ieta<n_etabin;ieta++){
      ...
      if ( rootHistSvc->regHist(histPath+profile_pt_dep_reco[ihar][ieta]->GetName(),profile_pt_dep_reco[ihar][ieta]).isFailure() ){
        msglog << MSG::WARNING << "Can't book "<< histPath+profile_pt_dep_reco[ihar][ieta]->GetName() << endreq;
      }
    ...
    for(int ipt=0;ipt<n_ptbin;ipt++){
      ...
      if ( rootHistSvc->regHist(histPath+profile_eta_dep_reco[ihar][ipt]->GetName(),profile_eta_dep_reco[ihar][ipt]).isFailure() ){
        msglog << MSG::WARNING << "Can't book "<< histPath+profile_eta_dep_reco[ihar][ipt]->GetName() << endreq;
      }
  ...
  if(rootHistSvc->regHist(histPath+profile_resolution->GetName(),profile_resolution).isFailure() ){
        msglog << MSG::WARNING << "Can't book "<< histPath+profile_resolution->GetName() << endreq;
  }
...
StatusCode CheckFlow_New::execute() {
  MsgStream msglog(messageService(), name());
  msglog << MSG::INFO << ">>> CheckFlow_New from execute" << endreq;
  ...
  for (std::vector<const HepMC::GenParticle*>::iterator pitr = particles.begin();pitr != particles.end(); pitr++) {
	...
	(fabs(pt) >= m_ptcut_min) && (fabs(pt) <= m_ptcut_max) ) {
      ...
      for(int ihar=0;ihar<6;ihar++){
        ...
        if(ieta>=0 && ieta<n_etabin) profile_pt_dep [ihar][ieta]->Fill(pt/1000,cos(temp));


        float temp_pt=pt/1000;
        for(int ipt=0;ipt<n_ptbin;ipt++){
          if(temp_pt<pt_binvals[ipt+1]){
            profile_eta_dep[ihar][ipt]->Fill(rapid  ,cos(temp));
            break;
          }
        }
...
// Also make correlation histos between Psi_n_truth and Psi_n_reco
  ...
  for(int ihar=0;ihar<6;ihar++){
    ...
    hist_Psi_n_ebe[ihar]->Fill( (ihar+1)*(psi_reco-Psi_n[ihar])  );
    hist_vn_ebe   [ihar]->Fill(sqrt(cos_n[ihar]*cos_n[ihar] +sin_n[ihar]*sin_n[ihar] ));
    ...
    hist_Psi_n_ebe_pt[ihar]->Fill( (ihar+1)*(psi_reco-Psi_n[ihar])  );
  }


// Make the plots for the correlation between Psi_n truth (for different n)  (same for Psi_n reco)
  for(int ihar=0;ihar<6;ihar++){
    hist_Psi_n_true[ihar]->Fill((ihar+1)*Psi_n[ihar]);
    hist_Psi_n_reco[ihar]->Fill((ihar+1)*Psi_n_reco[ihar]);

    float psi1,psi2;
    for(int ihar2=0;ihar2<6;ihar2++){
      psi1=(ihar+1)*Psi_n[ihar];psi2=(ihar2+1)*Psi_n[ihar2];
      hist_psi_corr_true[ihar*6+ihar2]->Fill(  atan2(  sin(psi1-psi2),cos(psi1-psi2) )  );

      psi1=(ihar+1)*Psi_n_reco[ihar];psi2=(ihar2+1)*Psi_n_reco[ihar2];
      hist_psi_corr_reco[ihar*6+ihar2]->Fill( atan2(  sin(psi1-psi2),cos(psi1-psi2) )  );
    }
  }
...
// calculate the pt and eta dependence using the Psi_reco angles also fill the resolution TProfile
  for(int ihar=0;ihar<6;ihar++){
    profile_resolution->Fill( ihar+1,  cos(  (ihar+1) * (Psi_n_reco_pos[ihar] - Psi_n_reco_neg[ihar]) ) );
  }
  for (std::vector<const HepMC::GenParticle*>::iterator pitr = particles.begin();pitr != particles.end(); pitr++) {
    double pt     = (*pitr)->momentum().perp();
    double rapid  = (*pitr)->momentum().pseudoRapidity();
	...
	(fabs(pt) >= m_ptcut_min) && (fabs(pt) <= m_ptcut_max) ) {
      ...
      for(int ihar=0;ihar<6;ihar++){
        ...
        int ieta= (int)(fabs(rapid)*n_etabin/eta_bin_max);
        if(ieta>=0 && ieta<n_etabin) profile_pt_dep_reco [ihar][ieta]->Fill(pt/1000,cos(temp));

        float temp_pt=pt/1000;
        for(int ipt=0;ipt<n_ptbin;ipt++){
          if(temp_pt<pt_binvals[ipt+1]){
            profile_eta_dep_reco[ihar][ipt]->Fill(rapid  ,cos(temp));
            break;
          }
        }
      }
...
/*
  for(int ihar=0;ihar<6;ihar++){
    ...
    for(int ieta=0;ieta<n_etabin;ieta++){
      profile_pt_dep_reco [ihar][ieta]->Scale(1.0/reso);
    }
    for(int ipt=0;ipt<n_ptbin;ipt++){
     profile_eta_dep_reco[ihar][ipt]->Scale(1.0/reso);
    }
  }
*/
  return StatusCode::SUCCESS;
