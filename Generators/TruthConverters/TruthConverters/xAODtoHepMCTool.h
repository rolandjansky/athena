...
class xAODtoHepMCTool: public asg::AsgTool, public virtual IxAODtoHepMCTool { 
 ...
 public:
  ...
  std::vector<HepMC::GenEvent> getHepMCEvents(const xAOD::TruthEventContainer* xTruthEventContainer, const unsigned long long evtNum) override;
 private:
  const HepMC::GenEvent createHepMCEvent(const xAOD::TruthEvent* xEvt, const unsigned long long evtNum);
  ...
  void printxAODEvent(const xAOD::TruthEvent* event, const unsigned long long evtNum) const;
   
 private:
  /// Input container key (job property)
  double m_momFac,m_lenFac;
  bool m_signalOnly;
  int m_maxCount;
  int m_evtCount;
  /// Counters
