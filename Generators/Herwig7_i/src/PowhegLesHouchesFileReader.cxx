...
//  int cwgtinfo_nn(0);
  ...
  while ( cfile.readline() ) {
	...
	optionalWeightsLabel[IdLabel]=name;	
  ...
  // std::cout<<"optional weights names"<<std::endl;
  //for(int i=0; i<optionalWeightsNames.size(); i++) std::cout<<optionalWeightsNames[i]+"\n";
}
...
bool powhegLesHouchesFileReader::doReadEvent() {
  ...
  //   cfile >> line;
  //   radtype=1;
  //   /* Store radtype: it may be useful */
  //   if(line.compare("#rwgt")==0){
  //     cfile>>radtype;
  //   }
      
  //   if(line.compare("<weights>")==0 || line.compare("<rwgt>")==0)
  //     {
  //       optionalWeights["central"]=hepeup.XWGTUP *upbfact[radtype-1];

  // 	int i=0;
  // 	while ( cfile.readline() && !cfile.find("</weights>") && !cfile.find("</rwgt>"))
  // 	  {
  // 	    double weight;
  // 	    cfile >> weight;
  //           string weightName=to_string(i+1);
  //           weightName='W'+weightName;
  // 	    optionalWeights[weightName] = weight*upbfact[radtype-1];
  //           i++;
  ...
  while ( cfile.readline() && !cfile.find("</event>")) {
    ...
    if(readingWeights) { 
      ...
      std::string hs=cfile.getline();
      hs.erase(std::remove(hs.begin(), hs.end(), ' '), hs.end());
      std::string IdLabel = hs.substr(0, hs.find(">", 0));
      IdLabel = std::regex_replace(IdLabel, std::regex(R"([\D])"), "");
      std::string weightName = optionalWeightsLabel[IdLabel];
      std::string value=hs;
