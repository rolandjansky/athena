	 ...
	 ///create and insert entries in SQLite DB 
	   ...
	   for(int ibin=1 ; ibin!=nbin+1 ; ibin++){
	    ...
	    if(PanelCode ==0)continue;
	      ...
	      sprintf(m_timephi  	,    "%f ", timephi	      ) ;   m_timephi	       [5]   =0;
              if ( h_Time	 )errtimephi	      = h_Time  	  ->GetBinError  (ibin) ;    
	      sprintf(m_errtimephi	,    "%f ", errtimephi        ) ;   m_errtimephi       [5]   =0;
	      ...
	      sprintf(m_timeeta 	 ,    "%f ", timeeta	       ) ;   m_timeeta  	[5]   =0;
	      if ( h_Time	)errtimeeta	     = h_Time		 ->GetBinError  (ibin) ;      
	      sprintf(m_errtimeeta	 ,    "%f ", errtimeeta        ) ;   m_errtimeeta	[5]   =0;
