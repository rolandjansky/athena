...
namespace dqutils {
  ...
  void MonitoringFile::HLTMuonHistogramDivision(std::string inFilename, TString& run_dir)
  {
	...
	iEFSA   = 1    // MSAlgo
      ...
      //  Standard Chains
      //TString m_alg[5] = {"_L2MuonSA", "_MuComb", "_MuonEFMS", "_MuonEFSA", "_MuonEFCB"};
      //TString m_wrtalg[5] = {"_L1", "_L2MuonSA", "_MuComb", "_MuComb", "_MuComb"};
	...
	double sumn = h1numb->Integral(m_iSTDL, m_iSTDH); // 60-100 GeV
	...
	sumn = h1num_mu0_15->Integral(m_iSTDL, m_iSTDH); // 25-100 GeV
	...
	mf.Write();
      ...
      // ******************************************************//
      TString m_alg2[3] = {"_L2MuonSA", "_MuonEFMS", "_MuonEFSA"};
      TString m_wrtalg2[3] = {"_L1", "_L2MuonSA", "_L2MuonSA"};
	...
	for( int alg = 0 ; alg < 3 ; alg++ ){
	  ...
	  for (int ies = 0; ies <= m_maxESbr; ies++) {
	    ...
	    if (0 == alg) {
	      ...
	      snum = nd_dir + chainName + m_triggerES[ies] + "_L2MuonSA" + "_Turn_On_Curve_wrt" + "_L1" + "_Denominator";
		...
		snum = nd_dir + chainName + m_triggerES[ies] + "_L2MuonSA" + "_Turn_On_Curve_wrt" + "_L1" + bestr[be] + "_Denominator";
	...
	for (int ialg = 0; ialg < MAXARR; ialg++) {
	  ...
	  */
	  ...
	  if (monL1[ialg]) { // MUST skip muGirl, muIso and MSonly as histograms are not defined!!!
	    ...
	    if(m_HI_pp_key)sumn = h1numb->Integral(13, 20); // 60-100 GeV
	    //if(m_HI_pp_key)sumn = h1numb->Integral(7, 10); // 30-50 GeV
	    double sumd = h1denb->Integral(13, 25);
	    if(m_HI_pp_key)sumd = h1denb->Integral(13, 20);
