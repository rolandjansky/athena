...
def handiWithComparisons(name, resultsFile, htmlDir, runlistLoc, compare, browserMenu, allDirsScriptDir, jsRoot=1):
    ...
    nSaved = saveAllHistograms(
        resultsFile, htmlDir, True, (name+LB_range), jsRoot)
    if nSaved == 0:
        ...
        d.write('<h1>Error:</h1>\n')
        d.write(
            'No histograms found for display.  Check the <tt>han</tt> configuration\n')
    ...
    if (browserMenu):
        ...
        dirlist, namelist = makeAllDirsBrowserFile(
            htmlDir, name, s, number, resultsFile, allDirsScriptDir)
    else:
        dirlist, namelist = makeAllDirsFile(
            htmlDir, name, s, number, resultsFile)

    for x in range(0, len(dirlist)):
        makeSubDirFile(htmlDir, name, s, number,
                       namelist[x], dirlist[x], runlistLoc, compare, allDirsScriptDir, jsRoot)
        makeColorFile(htmlDir, name, s, number,
                      namelist[x], dirlist[x], 'Red', runlistLoc, compare, allDirsScriptDir, jsRoot)
        makeColorFile(htmlDir, name, s, number,
                      namelist[x], dirlist[x], 'Yellow', runlistLoc, compare, allDirsScriptDir, jsRoot)
        makeColorFile(htmlDir, name, s, number,
                      namelist[x], dirlist[x], 'Green', runlistLoc, compare, allDirsScriptDir, jsRoot)
    ...
    for x in range(0, number):
        ...
        if sp[3] == 'dir':  # a directory
            ...
            if spaces_new > spaces:  # current dir is subdir of previous dir -> new item in new list
                if (spaces != -1):
                    sub += "sub"
                g.write('<dir tag=\"'+sub+'dir\" id=\"' + shortNameDir +
                        '\" status=\"'+sp[1]+'\" histo = \"' + histo + '\">\n')
                num_lists += 1
            ...
            if (histo == 'yes'):  # check that dir contains histos
                if namedir == '<top_level>':
                    namedir = '.'
                    g.write('<a href="'+namedir + '/toplevel.html">' +
                            shortNameDir + '</a>\n')
                else:
                    g.write('<a href="'+namedir + '/index.html" >' +
                            shortNameDir + '</a>\n')
            # g.write('<'+sp[1]+'></'+sp[1]+'>\n')
            spaces = spaces_new
    ...
    for x in range(0, number):
        ...
        if sp[3] == 'dir':    # a directory
            ...
            if spaces_new > spaces:  # current dir is subdir of previous dir -> new item in new list
                ...
                g.write('<ul%s>\n<li>' %
                        (' class="mktree"' if namedir == '<top_level>' else ''))
                num_lists += 1
            elif spaces_new == spaces:  # current en previous dir in same motherdir -> new item
                g.write('</li>\n<li>')
            else:  # close list and open new one
...
#                g.write('<img src="http://atlasdqm.web.cern.ch/atlasdqm/img/pixel.png" width="0" height="13" alt="" />')
            ...
            if ((x < number-1) and (s[x+1].rsplit())[3] == 'ass'):
                if namedir == '<top_level>':
                    namedir = '.'
                    g.write('<a href="'+namedir +
                            '/toplevel.html">'+shortNameDir + ':</a>')
                else:
                    g.write('<a href="'+namedir +
                            '/index.html" >'+shortNameDir + ':</a>')
                g.write('&nbsp;&nbsp;&nbsp;<font class="' +
                        sp[1]+'">' + sp[1] + '</font>\n')
                dirlist.append(x)
...
def makeAllDirsBrowserFile(htmlDir, name, s, number, resultsFile, AllDirsScriptDir):
    ...
    g.write('<title>' + name + '</title>\n')
    g.write('<link rel="stylesheet" href="AutomChecks.css" type="text/css" />\n')
    g.write('<link rel="stylesheet" href="'+AllDirsScriptDir +
            '/mktree/mktree.css" type="text/css" />')
    g.write('<script type="text/javascript" src="'+AllDirsScriptDir +
            '/AllDirs.js"><!-- dont contract--></script>\n')
    g.write('<script type="text/javascript" src="'+AllDirsScriptDir +
            '/mktree/mktree.js"><!-- dont contract--></script>\n')
    g.write('</head>\n')
...
def makeSubDirFile(htmlDir, name, s, number, subname, assessIndex, runlistLoc, compare, AllDirsScriptDir, jsRoot):
    ...
    h.write('<script type="text/javascript" src="'+AllDirsScriptDir +
            '/AllDirs.js"><!-- dont contract--></script>\n')
    h.write('</head>\n')
    h.write('<body>\n')
    h.write(
...
#    h.write('../index.html">Back</a>]</font>\n')
    h.write('<center>')
    h.write('<table>\n<tr valign="top">\n<td width="250"></td>\n')
    h.write('<td width="300" align="center"><h2>' +
            name+' '+subnameTitle+'</h2>\n')
    ...
    while(sp[3] == 'ass' and y < number):
        ...
        if col == 1:
            h.write('<tr>\n<td class="' + sp[1] + '" align="center"><a href="'+sp[0]+'.html" class="hintanchor" onmouseover="showhint(\'' + title+'\', this, event, \'400px\')"><img src="' +
                    sp[0] + '.png" height="200"  alt="' + name + ' ' + subname+'/'+sp[0]+'.png" /></a><br/><div style="text-overflow:ellipsis;overflow:hidden;max-width:240px">'+sp[0]+'</div></td>\n')
        elif col == 3:
            h.write('<td class="' + sp[1] + '" align="center"><a href="'+sp[0]+'.html" class="hintanchor" onmouseover="showhint(\'' + title+'\', this, event, \'500px\')"><img src="' + sp[0] +
                    '.png" height="200"  alt="' + name + ' ' + subname+'/'+sp[0]+'.png" /></a><br/><div style="text-overflow:ellipsis;overflow:hidden;max-width:240px">'+sp[0]+'</div></td>\n</tr>\n')
            col = 0
        else:
            h.write('<td class="' + sp[1] + '" align="center"><a href="'+sp[0]+'.html" class="hintanchor" onmouseover="showhint(\'' + title+'\', this, event, \'400px\')"><img src="' +
                    sp[0] + '.png" height="200" alt="' + name + ' ' + subname+'/'+sp[0]+'.png" /></a><br/><div style="text-overflow:ellipsis;overflow:hidden;max-width:240px">'+sp[0]+'</div></td>\n')
        temp = s[y].rsplit(" title ")
        sp = temp[0].split()
        makeOneHistFile(htmlDir, name, subname, sp,
                        runlistLoc, compare, jsRoot)
        y = y+1
        if y < number:
            sp = s[y].rsplit()
    if not (col == 3):
...
def makeColorFile(htmlDir, name, s, number, subname, assessIndex, color, runlistLoc, compare, AllDirsScriptDir, jsRoot):
    ...
    h.write('<center>')
    h.write('<table>\n<tr valign="top">\n<td width="250"></td>\n')
    h.write('<td width="300"><h2>'+name+' ' +
            subnameTitle+': ' + color + ' only</h2>\n')
    h.write('<a href="Red.html">[Only Red]</a>\n')
    h.write('<a href="Yellow.html">[Only Yellow]</a>\n')
    h.write('<a href="Green.html">[Only Green]</a></td>\n')
    h.write('<td width="250">\n<font class="Note">Click on images for details and full size.</font>\n')
        ...
        if sp[1] == color:
            ...
            if col == 1:
                h.write('<tr>\n<td class="' + sp[1] + '"><a href="'+sp[0]+'.html" class="hintanchor" onmouseover="showhint(\''+title +
                        '\', this, event, \'400px\')"><img src="' + sp[0] + '.png" height="200"  alt="' + name + ' ' + subname+'/'+sp[0]+'.png" /></a></td>\n')
            elif col == 3:
                h.write('<td class="' + sp[1] + '"><a href="'+sp[0]+'.html" class="hintanchor" onmouseover="showhint(\''+title +
                        '\', this, event, \'500px\')"><img src="' + sp[0] + '.png" height="200"  alt="' + name + ' ' + subname+'/'+sp[0]+'.png" /></a></td>\n</tr>\n')
                col = 0
            else:
                h.write('<td class="' + sp[1] + '"><a href="'+sp[0]+'.html" class="hintanchor" onmouseover="showhint(\'' + title +
                        '\', this, event, \'400px\')"><img src="' + sp[0] + '.png" height="200" alt="' + name + ' ' + subname+'/'+sp[0]+'.png" /></a></td>\n')
...
def makeOneHistFile(htmlDir, name, subname, sp, runlistLoc, compare, jsRoot):
    ...
    rm = runmatch.match(name)
    srm = subrunmatch.match(name)
    if rm is not None:
...
##     k.write('<script type=\"text/javascript\" src=\"'+javaScriptLoc +'\"><!-- dont contract-->\n</script>\n')
    ...
    k.write('<table>\n<tr valign="top">\n')
    k.write(
        '<td width="250"><font class="DQGroup">[<a href="#" onclick="history.go(-1);return false;">Back</a>]</font></td>\n')
    k.write('<td width="500"><h2>'+name + ' ' +
            subname+'/'+sp[0]+'</h2></td>\n')
    k.write('<td width="250"></td>\n</tr>\n')
    for x in range(4, len(sp)-1):
        if ('inputname' in sp[x]):
            inputname = sp[x+1]
            k.write('<tr><td width="250"></td><td width="500">Location in ROOT file: ' +
                    inputname + '</td>\n</tr>\n')
    k.write('</table>\n')
    k.write('<table cellpadding="10">\n<tr>\n')
    k.write('<td>\n<table width="400">\n')
    k.write(
        '<tr><td colspan="2"><font class="Heading">Assessment Details:</font></td></tr>\n')
    k.write('<tr><td>&nbsp;</td></tr>\n')
    ...
    while(extra > 0):
        if 'Green' in sp[cc]:
            ...
            if (cG > 0 and cR > 0):
                if(Green > Red):
                    writeLimitDiagram(k, limitName, "Red",
                                      "Green", sp[cR], sp[cG])
                else:
                    writeLimitDiagram(k, limitName, "Green",
                                      "Red", sp[cG], sp[cR])
                cG = -1
                cR = -1
        elif 'Red' in sp[cc]:
            ...
            k.write('<tr><td>&nbsp;</td></tr>\n')
            k.write(
                '<tr><td colspan="2"><font class="Heading">Results:</font></td></tr>\n')
            k.write('<tr><td>&nbsp;</td></tr>\n')
            cc += 2
            extra -= 2
        elif sp[cc] == "Config":
            ...
            k.write('<tr><td>&nbsp;</td></tr>\n')
            k.write(
                '<tr><td colspan="2"><font class="Heading">Configuration Parameters:</font></td></tr>\n')
            k.write('<tr><td>&nbsp;</td></tr>\n')
            cc += 2
            extra -= 2
        elif sp[cc] == "entries:":
            ...
            k.write(
                '<tr><td align="right"><font class="DatumName">Num. of Entries:</font> </td>')
            k.write('<td>'+sp[cc+1]+'</td></tr>\n')
            cc += 2
            extra -= 2
        elif sp[cc] == "Underflow:":
            k.write(
                '<tr><td align="right"><font class="DatumName">Underflow:</font> </td>')
            k.write('<td>'+sp[cc+1]+'</td></tr>\n')
            cc += 2
            extra -= 2
        elif sp[cc] == "Overflow:":
            k.write(
                '<tr><td align="right"><font class="DatumName">Overflow:</font> </td>')
            k.write('<td>'+sp[cc+1]+'</td></tr>\n')
            cc += 2
            extra -= 2
            ...
            if currentHeading == 'results':
                ...
                if compare and run is not None:
                    if period_type == 'run':
                        ...
                        k.write(
                            '<td>'+resultval+' (<a href="%s">History for this run</a>)</td></tr>\n' % queryurl2)
                else:
                    k.write('<tr><td align="right"><b> %s: </b></td>' %
                            (resultname,))
                    k.write('<td>'+resultval+'</td></tr>\n')
            else:
    ...
    if subname == '.':
        ...
        if((jsRoot & 2) and os.path.isfile(jsonPath)):
            ...
            jsonStr = jsonStr.replace('\n', '')
            k.write(
                '<td><div id="root_plot_2" style="width: 600px; height: 400px"></div></td>\n<script>\n requirejs.config( { paths: { \'JSRootCore\'    : \'https://root.cern.ch/js/dev//scripts/JSRootCore\', \'JSRootPainter\' : \'https://root.cern.ch/js/dev//scripts/JSRootPainter\', } });require([\'JSRootCore\', \'JSRootPainter\'], function(Core, Painter) {\n var obj = Core.parse(\''+jsonStr+'\');\nPainter.draw("root_plot_2", obj, ""); });</script>\n')
        else:
            k.write('<td><a href="index.html"><img src="' +
                    sp[0] + '.png" alt="' + name + ' ' + subname+'/'+sp[0]+'.png" /></a></td>\n')
    k.write('</tr></table>\n')
    k.write('</center>\n')
    now = time.localtime()
