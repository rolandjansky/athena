 ...
 : QObject(parent), d(new Imp)
{
  ...
  d->mainwindow = new VP1MainWindow(this,availEvents);//mainwindow takes ownership of availevents
						    ...
						    QStringList availableLocalInputDirectories )
{
  ...
  if (qApp) {
    VP1Msg::message("VP1ExecutionScheduler::init ERROR: QApplication already initialised. Expect problems!!!");
  } else {
...
bool VP1ExecutionScheduler::executeNewEvent(const int& runnumber, const int& eventnumber, const unsigned& triggerType, const unsigned& time)
{
  ...
  if(d->skipEvent) {
    ...
    qApp->exec();//We exit this when someone pushes the "next event" button.
