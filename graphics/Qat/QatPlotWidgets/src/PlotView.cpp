...
public:
  ...
  void mousePressEvent (QGraphicsSceneMouseEvent *e) {
    if (e->buttons()==Qt::RightButton) {
      ...
      double slX = (pvParent->rect()->right() - pvParent->rect()->left())/double(rect().width());
      double slY = (pvParent->rect()->top() - pvParent->rect()->bottom())/double(rect().height());
      double x = (e->pos().toPoint().x()-rect().left())   * slX + pvParent->rect()->left();
      double y = (e->pos().toPoint().y()-rect().bottom()) * slY + pvParent->rect()->top();
...
PlotView::PlotView(const QRectF * rect, Style xStyle, Style yStyle, bool createRangeDividers, QWidget *parent) : QGraphicsView(parent),c(new Clockwork)
{
  c->rect=*rect;
  create();
  setLogX(xStyle==LOG);
...
void PlotView::create() {
  ...
  if (c->created) {
    ...
    c->control=new PVControl(this,c->rect);
    setScene(&c->scene);
    c->g=new QGraphicsItemGroup();
    c->scene.addItem(c->g);
    c->gridLinesGroup=new QGraphicsItemGroup();
  ...
  // Label the x axes:
  if (c->xRangeDivider) c->xRangeDivider->setRange(c->rect.left(), c->rect.right());
  if (c->yRangeDivider) c->yRangeDivider->setRange(c->rect.top(), c->rect.bottom());
 
  LinToLog *toLogY= c->yAxisStyle==LOG ? new LinToLog (c->rect.top(),c->rect.bottom()) : NULL;
  LinToLog *toLogX= c->xAxisStyle==LOG ? new LinToLog (c->rect.left(),c->rect.right()) : NULL;
  // Master transformer:
  QMatrix m;
  m.scale(c->graphBoxSize.width()/c->rect.width(),-c->graphBoxSize.height()/c->rect.height());
  m.translate(-c->rect.bottomLeft().x(),  -c->rect.bottomLeft().y());
  ...
  if (c->xRangeDivider) {
    for (int ix=0;ix<c->xRangeDivider->getNumSubdivisions();ix++) {
      ...
      double yValue=c->rect.top();
      double wValue=c->rect.bottom();
      if (toLogX) xValue = (*toLogX)(xValue);
      
      

      QPointF X (xValue,yValue);
      QPointF XP(xValue,yValue+c->rect.height()/32.0);
	...
	QPointF XP(xValue,wValue-c->rect.height()/32.0);
	QGraphicsLineItem *line=new QGraphicsLineItem();
	line->setLine(QLineF(X,XP));
	c->scene.addItem(line);
	c->g->addToGroup(line);
	...
	for (int n=0;n<nPass;n++) {
	  ...
	  for (int i=0;i<subdivider->getNumSubdivisions();i++) {
	    ...
	    {
	      QPointF X (xValue,yValue);
	      QPointF XP(xValue,yValue+c->rect.height()/64.0);
	      QGraphicsLineItem *line=new QGraphicsLineItem();
	      line->setLine(QLineF(X,XP));
	      c->scene.addItem(line);
	      c->g->addToGroup(line);
	    ...
	    {
	      QPointF X (xValue,wValue);
	      QPointF XP(xValue,wValue-c->rect.height()/64.0);
	      QGraphicsLineItem *line=new QGraphicsLineItem();
	      line->setLine(QLineF(X,XP));
	      c->scene.addItem(line);
	      c->g->addToGroup(line);
	...
	for (int n=0;n<nPass;n++) {
      ...
      {
	QGraphicsLineItem *line=new QGraphicsLineItem(QLineF(QPointF(xValue,c->rect.top()),QPointF(xValue,c->rect.bottom())));
	QPen  pen=line->pen();
  ...
  if (c->yRangeDivider) {
    for (int iy=0;iy<c->yRangeDivider->getNumSubdivisions();iy++) {
      ...
      double xValue = c->rect.left();
      double wValue = c->rect.right();
      if (toLogY) yValue = (*toLogY)(yValue);
       
      
      QPointF  X(xValue,yValue);
      QPointF  XP(xValue+c->rect.width()/32.0,yValue);
      QGraphicsLineItem *line=new QGraphicsLineItem();
      line->setLine(QLineF(X,XP));
      c->scene.addItem(line);
      c->g->addToGroup(line);
	...
	QPointF  XP(wValue-c->rect.width()/32.0,yValue);
	QGraphicsLineItem *line=new QGraphicsLineItem();
	line->setLine(QLineF(X,XP));
	c->scene.addItem(line);
	c->g->addToGroup(line);
	...
	for (int n=0;n<nPass;n++) {
	  ...
	  for (int i=0;i<subdivider->getNumSubdivisions();i++) {
	    ...
	    {
	      QPointF X (xValue,yValue);
	      QPointF XP(xValue+c->rect.width()/64.0,yValue);
	      QGraphicsLineItem *line=new QGraphicsLineItem();
	      line->setLine(QLineF(X,XP));
	      c->scene.addItem(line);
	      c->g->addToGroup(line);
	    ...
	    {
	      QPointF X (wValue,yValue);
	      QPointF XP(wValue-c->rect.width()/64.0,yValue);
	      QGraphicsLineItem *line=new QGraphicsLineItem();
	      line->setLine(QLineF(X,XP));
	      c->scene.addItem(line);
	      c->g->addToGroup(line);
	...
	for (int n=0;n<nPass;n++) {
      ...
      {
	QGraphicsLineItem *line=new QGraphicsLineItem(QLineF(QPointF(c->rect.left(),yValue),QPointF(c->rect.right(),yValue)));
	QPen  pen=line->pen();
	pen.setStyle(Qt::DotLine);
	pen.setColor(QColor("lightGray"));
	line->setPen(pen);
  ...
  if (c->rect.contains(0,c->rect.center().y())) 
  {
    c->xZeroLine=new QGraphicsLineItem(QLineF(QPointF(0,c->rect.bottom()),QPointF(0,c->rect.top())));
  ...
  if (c->rect.contains(c->rect.center().x(), 0)) 
  {
    c->yZeroLine=new QGraphicsLineItem(QLineF(QPointF(c->rect.right(),0),QPointF(c->rect.left(),0)));
  ...
  QList<QGraphicsItem *> contentsList =c->contentsGroup->children();
  QList<QGraphicsItem *> gridLinesList=c->gridLinesGroup->children();
  QList<QGraphicsItem *> gList        =c->g->children();
