...
bool in_atlas_source (tree decl)
{
...
#else
  static std::unordered_map<std::string, bool> seen_files;
#endif
  ...
  std::unordered_map<std::string, bool>::iterator it = seen_files.find (sfile);
  if (it == seen_files.end()) {
    it = seen_files.insert (std::make_pair (sfile,
                                            in_atlas_source1 (file))) . first;
  }
  return it->second;
#endif
}
