...
#----------------------------------------------------------------------------------
  def addTemplateToName(self, attrs):
    if attrs['name'].find('>') == -1 and 'demangled' in attrs :
      ...
      # Don't do this for operators.
      if demangled.find ('::operator') >= 0:
        return
...
#----------------------------------------------------------------------------------
  def start_element(self, name, attrs):
    ...
    if name in ('EnumValue','Argument') :
      ...
      if 'demangled' not in attrs and 'name' in attrs:
        attrs['demangled'] = attrs['name']
...
#----------------------------------------------------------------------------------
  def tryCppSelections(self):
    for c in self.classes:
      id = self.cppClassSelect.get(self.genTypeName(c['id'],alltempl=True,_useCache=False))
      if (id != None) :
        selection = {'id' : id}
    ...
    for td in self.typedefs :
      ...
      if f != -1 :
        ...
        if (f == 0 or name[f-1] in notname) and (g == len(name) or name[g] in notname) :
          ...
          #if name.startswith ('const const '):
          #  name = name[6:]
    return name
#----------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------
 ...
 #----------------------------------------------------------------------------------
  def autosel_bases (self, classes, c):
    for b in c['bases'].split():
      batt = self.xref[b]['attrs']
      if batt not in classes:
        classes.append (batt)
        if not batt['name'].startswith ('_Hashtable'):
          self.autosel_bases (classes, batt)
    return
  def autosel(self, classes):
    ...
    for c in classes:
      ...
      #pos = c['name'].find('<')
      #sub = c['name'][:pos] if pos >= 0 else ''
      #if (sub in ['unordered_set',
      #            'unordered_multiset',
      #            'unordered_map',
      #            'unordered_multimap'] and
      #    self.xref[c['context']]['attrs']['name'] == 'std' and
      #    'bases' in c):
      #  self.autosel_bases(classes, c)
      ...
      for f in self.functions :
        ...
        if demangled and len(demangled) :
          if lenreturns and demangled[0:lenreturns] == returns:
            ...
            while demangled and demangled[0] == ' ': demangled = demangled[1:]
        else :
          demangled = ""
...
#----------------------------------------------------------------------------------
  def memberfilter( self, id ) :
    ...
    if elem == 'OperatorMethod' and attrs.get('name') == '=':
      if self.isAssignmentPrivate (attrs['context']):
        return 0


    # Special case for pair --- don't allow operator= if either of the
    # types are const.
    if ((elem == 'OperatorMethod' and attrs['name'] == '=') or
        (elem == 'Method' and attrs['name'] == 'swap')):
        context = self.genTypeName(attrs['context'])
        if context.startswith('std::pair<'):
          cattrs = self.xref[attrs['context']]['attrs']
          for m in cattrs['members'].split():
            if self.xref[m]['elem'] == 'Typedef':
              mattrs = self.xref[m]['attrs']
              if mattrs['name'] in ['first_type', 'second_type']:
                if mattrs['type'][-1] == 'c':
                  return 0

    if elem == 'Variable' and attrs.get('static') == '1' and attrs['type'][-1] == 'c':
      return 0

    for a in args:
      if a.get('type'):
        if self.xref[a['type']]['elem'] == 'RValueReferenceType':
          return 0

    for a in args:
      if a.get('type'):
        if self.xref[a['type']]['attrs'].get('name') == 'piecewise_construct_t':
          return 0
    ...
    if self.selector and 'name' in attrs and  elem in ('Constructor','Destructor','Method','OperatorMethod','Converter') :
      ...
      if demangledMethod.find ('(') < 0:
        # Fallback for the case where we don't have the demangled signature.
        demangledMethod = attrs['name'] + self.protoFor (self.xref[id])
...
#----------------------------------------------------------------------------------
  def isConstructorPrivate(self, id ) :
    ...
    return False
#----------------------------------------------------------------------------------
  def isAssignmentPrivate(self, id ) :
    attrs = self.xref[id]['attrs']
    if 'members' in attrs : 
       for m in attrs['members'].split() :
         elem = self.xref[m]['elem']
         ...
         # assignment op, or if the data member is const, then we can't
         # use this class's assignment op.
         if elem == 'Field':
           tid = attr['type']
           felem = self.xref[tid]['elem']
           while felem in ('CvQualifiedType','Typedef'):
             fattrs = self.xref[tid]['attrs']
             if felem == 'CvQualifiedType':
               if fattrs['const']: return True
               tid = fattrs['type']
               felem = self.xref[tid]['elem']
             elif felem == 'Typedef':
               tid = fattrs['type']
               felem = self.xref[tid]['elem']
           if felem in ('Class', 'Struct'):
             if self.isAssignmentPrivate(tid): return True

    if 'bases' in attrs :
       for b in attrs['bases'].split() :
         if b[:10] == 'protected:' : b = b[10:]
         if b[:8]  == 'private:'   : b = b[8:]
         if self.isAssignmentPrivate(b) : return True
    return False
#----------------------------------------------------------------------------------
...
#---------------------------------------------------------------------------------
  def genClassDict(self, attrs, ioReadRules, ioReadRawRules):
    ...
    if ioReadRawRules:
      self.removeBrokenIoRules( cls, ioReadRawRules, memberTypeMap );
...
#----------------------------------------------------------------------------------
  def genClassShadow(self, attrs, inner = 0 ) :
    ...
    if inner and attrs.has_key('demangled') and self.isUnnamedType(attrs['demangled']) :
      cls = attrs['demangled']
      clt = ''
    elif inner and attrs.get('isAnon'):
      cls = attrs.get('demangled', attrs.get('name'))
      clt = ''
    else:
    ...
    if typ == 'enumeration' :
      ...
      for m in memList :
        ...
        if member['elem'] in ('Class','Struct','Union','Enumeration') \
           ...
           and not member['attrs'].get('isAnon'):
          ...
          for bm in basememList:
            ...
            if basemember['elem'] in ('Method','OperatorMethod') \
              ...
              if found != 1:
                ...
                if found == 0:
                  allBasesMethods[demangledBaseMethod] = { 'bases': ( allbasebases ),
                                                           'returns': basemember['attrs'].get('returns'),
                                                           'mem' : basemember}
                else:
                  ...
                  allBasesMethods[demangledBaseMethod]['mem'] = basemember
        ...
        for demangledMethod in allBasesMethods.keys() :
          ...
          if len(member['bases']) > 1:
            ...
            if '(' not in ret:
              ...
              if demangledMethod in ['()', '+=', '-=', '*=', '/=']:
                demangledMethod = 'operator' + demangledMethod
              cmem = '  virtual %s %s' % (ret, demangledMethod)
              if (demangledMethod.find('(') < 0 or
                  demangledMethod == 'operator()'):
                cmem = cmem + self.protoFor (member['mem'])
              cmem = cmem + ' throw();'
              c += indent + cmem + '\n'
      # Data members.
      for m in memList :
        ...
        if member['elem'] in ('Field',) :
          ...
          if ( noPublicType and
               not self.isUnnamedType(axref['attrs'].get('demangled')) and
               not axref['attrs'].get('isAnon')):
          ...
          mType = axref
          if mType and (self.isUnnamedType(mType['attrs'].get('demangled')) or
                        mType['attrs'].get('isAnon')):
  ...
  def protoFor (self, member):
    args = []
    for a in member['subelems']:
      args.append (self.genTypeName (a['type']))
    ret = '(' + ','.join(args) + ')'
    if member['attrs'].get('const'):
      ret = ret + ' const'
    return ret
...
#----------------------------------------------------------------------------------
  def genEnumerationBuild(self, attrs, children):
    ...
    return s 
#----------------------------------------------------------------------------------
  def genScopeName(self, attrs, enum=False, const=False, colon=False, _useCache = True) :
    s = ''
    if 'context' in attrs :
      ctxt = attrs['context']
      while ctxt in self.unnamedNamespaces: ctxt = self.xref[ctxt]['attrs']['context']
      ns = self.genTypeName(ctxt, enum, const, colon, _useCache=_useCache)
      if ns : s = ns + '::'
      elif colon  : s = '::'
    return s
#----------------------------------------------------------------------------------
# const is CONST VETO!!!
  def genTypeName(self, id, enum=False, const=False, colon=False, alltempl=False, _useCache=True, _cache={}) :
    ...
    const = False
    s = self.genScopeName(attrs, enum, const, colon, _useCache=_useCache)
    if elem == 'Namespace' :
      ...
      if self.gccxmlvers == 'gccxml_plugin':
        version = 1.0
      else:
...
#----------------------------------------------------------------------------------
  def genMCODef(self, type, name, attrs, args):
    ...
    #returns = self.resolveTypedefName (returns)
...
#----------------------------------------------------------------------------------
  def genMCOArgs(self, args, narg, pad, head, body):
    ...
    for i in range(narg) :
      ...
      #if arg.find('ElemLinkVec') >= 0: print 'yyy1', arg
      ...
      if arg.find('[') != -1 and arg.count('<', 0, arg.find('[')) <= arg.count('>', 0, arg.find('[')):
        ...
        arg += argptr;
      #arg = self.resolveTypedefName(arg)
...
#----------------------------------------------------------------------------------
  def genConstructorDef(self, attrs, args):
    cl  = self.genTypeName(attrs['context'], colon=True)
    #cl_in = cl
    #if cl_in.find('ElemLinkVec')>=0: print 'xxx1', cl
    #cl = self.resolveTypedefName (cl)
    #if cl_in.find('ElemLinkVec')>=0: print 'xxx2', cl
    clt = string.translate(str(cl), self.transtable)
    id  = attrs['id']
    paramargs = ''
    ...
    if 'pseudo' in attrs :
      ...
      narg  = len(args)
      for n in range(narg-ndarg, narg+1) :
        #if cl.find('ElemLinkVec')>=0: print 'xxx3', cl
        if ndarg :
          if n == narg-ndarg :  body += '  if ( arg.size() == %d ) {\n  ' % n
          else               :  body += '  else if ( arg.size() == %d ) { \n  ' % n
...
#----------------------------------------------------------------------------------
  def completeClass(self, attrs):
    ...
    haveCtorCpy = 1
    haveDtor    = 1
    if 1:#self.gccxmlvers.find('0.7') == 0:
      haveCtor    = 0
      haveCtorCpy = 0
      haveDtor    = 0
      for m in members :
    ...
    if haveCtor == 0 :
      ...
      self.xref[id] = {'elem':'Constructor', 'attrs':new_attrs, 'subelems':[] }
      attrs['members'] += u' ' + id
    if haveCtor == 0 and not self.isConstructorPrivate(cid):
      ccid = cid + 'c'
      # const cid exists?
    ...
    if haveDtor == 0 :
      id = u'_x%d' % self.x_id.next()
      name = attrs.get('templateName', attrs['name'])
      new_attrs = { 'name':name, 'id':id, 'context':cid, 'artificial':'true', 'access':'public' }
...
stldeftab['unordered_set'] = '=','std::hash','std::equal_to','std::allocator'
stldeftab['unordered_multiset'] = '=','std::hash','std::equal_to','std::allocator'
stldeftab['unordered_map'] = '=','=','std::hash','std::equal_to','std::allocator'
stldeftab['unordered_multimap'] = '=','=','std::hash','std::equal_to','std::allocator'
...
def ClassDefImplementation(selclasses, self) :
  ...
  for attrs in selclasses :
    ...
    for ml in membersList:
      if ml[1].isdigit() :
        if self.xref[ml]['attrs'].get('tree_code_name') == 'using_decl':
          continue
