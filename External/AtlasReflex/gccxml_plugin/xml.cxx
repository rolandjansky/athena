...
#if GCC_VERSION < 4009
# define tree_fits_uhwi_p(T) (host_integerp(T, 1))
# define tree_to_uhwi(T) (tree_low_cst(T, 1))
# define tree_to_shwi(T) (tree_low_cst(T, 0))
...
# if GCC_VERSION >= 4008
static GTY(()) vec<tree/*, va_gc*/> xml_instantiations;
# else
static GTY(()) VEC(tree, heap) *xml_instantiations;
# endif
...
void do_xml_document (const char*, const char*);

static int xml_add_node (xml_dump_info_p, tree, int);
static void xml_dump (xml_dump_info_p);
static int xml_queue_incomplete_dump_nodes (splay_tree_node, void*);
static void xml_dump_tree_node (xml_dump_info_p, tree, xml_dump_node_p);
static void xml_dump_files (xml_dump_info_p);

static void xml_add_start_nodes (xml_dump_info_p, const char*);

static const char* xml_get_encoded_string (tree);
static const char* xml_get_encoded_string_from_string (const char*);
static tree xml_get_encoded_identifier_from_string (const char*);
static const char* xml_escape_string (const char* in_str);
...
# define xml_add_node(xdi, node, complete)                                    \
   (fprintf(stderr, "Adding node %i (%s) at line %d\n",_node_count++, get_tree_code_name(TREE_CODE(node)), __LINE__), \
...
static void
xml_print_name_attribute_from_string (xml_dump_info_p xdi, const char* s)
{
  const char* name = xml_get_encoded_string_from_string (s);
  fprintf (xdi->file, " name=\"%s\"", name);
}
...
xml_print_offset_attribute (xml_dump_info_p xdi, tree d)
{
  ...
  if (tree_byte_ofs && tree_fits_uhwi_p(tree_byte_ofs) &&
      tree_bit_ofs && tree_fits_uhwi_p(tree_bit_ofs))
    {
    unsigned HOST_WIDE_INT bit_ofs = tree_to_uhwi (tree_bit_ofs);
    unsigned HOST_WIDE_INT byte_ofs = tree_to_uhwi (tree_byte_ofs);
...
xml_print_bits_attribute (xml_dump_info_p xdi, tree d)
{
  if (DECL_BIT_FIELD_TYPE (d))
    ...
    if (size_tree && tree_fits_uhwi_p (size_tree))
      {
      unsigned HOST_WIDE_INT bits = tree_to_uhwi(size_tree);
                          ...
                          const char* where)
{
  ...
  fprintf (xdi->file, " tree_code=\"%d\" tree_code_name=\"%s\" node=\"%p\"",
           code, get_tree_code_name (code), (void*)t);
  if (where)
    {
...
#if GCC_VERSION >= 4009
  if (DECL_HAS_IN_CHARGE_PARM_P (fd))
    {
    /* Skip in-charge argument.  */
    if (arg) arg = TREE_CHAIN (arg);
    arg_type = TREE_CHAIN (arg_type);
    }
  if (DECL_HAS_VTT_PARM_P (fd))
    {
    /* Skip vtt argument.  */
    if (arg) arg = TREE_CHAIN (arg);
    arg_type = TREE_CHAIN (arg_type);
...
xml_output_record_type (xml_dump_info_p xdi, tree rt, xml_dump_node_p dn)
{
  ...
  if(!TYPE_ANONYMOUS_P (rt))
    {
      xml_print_name_attribute_from_string (xdi, type_as_string (rt, TFF_PLAIN_IDENTIFIER + TFF_UNQUALIFIED_NAME + TFF_NO_OMIT_DEFAULT_TEMPLATE_ARGUMENTS));
    }
/* START GCC 4.7.2 upgrade mods - From Andrej Mitrovic */
...
/* END GCC 4.7.2 upgrade mods */
  ...
  if (dn->complete && COMPLETE_TYPE_P (rt))
    ...
    // Make sure any implicit assignment operator is declared.
    if (CLASSTYPE_LAZY_COPY_ASSIGN (rt))
      lazily_declare_fn (sfk_copy_assignment, rt);
  ...
  if (TYPE_NAME (t))
    {
      if (TREE_CODE (TYPE_NAME (t)) == IDENTIFIER_NODE)
        {
          xml_print_name_attribute_from_string (xdi, IDENTIFIER_POINTER (TYPE_NAME (t)));
        }
      else
        {
...
xml_output_reference_type (xml_dump_info_p xdi, tree t, xml_dump_node_p dn)
{
  if (TYPE_REF_IS_RVALUE (t))
    fprintf (xdi->file, "  <RValueReferenceType");
  else
...
/* END GCC 4.7.2 upgrade mods */
  ...
  for (tv = TYPE_VALUES (t); tv ; tv = TREE_CHAIN (tv))
    ...
    if(TREE_CODE (TREE_VALUE (tv)) == CONST_DECL)
      {
        int value = TREE_INT_CST_LOW (DECL_INITIAL (TREE_VALUE (tv)));
        const char* s = xml_get_encoded_string (DECL_NAME (TREE_VALUE(tv)));
        fprintf (xdi->file,
                 "    <EnumValue name=\"%s\" init=\"%d\"/>\n",
                 s, value);
      }
...
xml_find_template_parm (tree t)
{
  ...
  switch (TREE_CODE (t))
    ...
    case TYPE_ARGUMENT_PACK: return 1;
    case NONTYPE_ARGUMENT_PACK: return 1;
    case TYPE_PACK_EXPANSION: return 1;
...
/* START GCC 4.7.2 upgrade changes */
    ...
    case DECLTYPE_TYPE:
      return 0;//xml_find_template_parm (DECLTYPE_TYPE_EXPR (t));
  ...
  for (tl = DECL_TEMPLATE_SPECIALIZATIONS (td);
    ...
    switch (TREE_CODE (ts))
      ...
      case TEMPLATE_DECL:
        break;
      default:
        //printf("\nUNIMPLEMENTED %i: %s", __LINE__, get_tree_code_name(TREE_CODE(ts)));
...
/* END GCC 4.7.2 upgrade mods */
    ...
    else if (TREE_CODE(ts) == RECORD_TYPE)
    {
      /* Add the instantiation only if it is real.  */
      if (!xml_find_template_parm (TYPE_TI_ARGS(ts)))
        xml_add_node (xdi, ts, complete);
    }
    else
    {
  ...
  {
# if GCC_VERSION >= 4008
    tree* t_vec = xml_instantiations.address();
    int len = xml_instantiations.length();
# else
    tree * t_vec = VEC_address(tree, xml_instantiations);
    int len = VEC_length (tree, xml_instantiations);
# endif
    for (int i=0; i < len; i++) {
      tree t = t_vec[i];
      if (!t)
        ;
          ...
          DECL_TEMPLATE_INFO (t) && DECL_TI_TEMPLATE(t) == td)
      {
        xml_add_node (xdi, t, complete);
      }
...
xml_dump_tree_node (xml_dump_info_p xdi, tree n, xml_dump_node_p dn)
{
  switch (TREE_CODE (n))
    ...
    //    printf("Translation Unit Declaration %i, %s\n", __LINE__, get_tree_code_name(TREE_CODE(n)));
    //    break;
    case NAMESPACE_DECL:
...
void do_pass(void* gcc_data)
{
  ...
  if (strcmp(pass->name, "lower") == 0 && current_function_decl)
  {
    if (DECL_TEMPLATE_INFO (current_function_decl))
    {
# if GCC_VERSION >= 4008
      xml_instantiations.safe_push (current_function_decl);
# else
      VEC_safe_push(tree, heap, xml_instantiations, current_function_decl);
# endif
    }
  }
}
...
void do_type(void* gcc_data)
{
  ...
  if (TREE_CODE(t) == FIELD_DECL || TREE_CODE(t) == VAR_DECL) {
    tree typ = TREE_TYPE(t);
    if (TREE_CODE(typ) == RECORD_TYPE && CLASSTYPE_USE_TEMPLATE(typ)) {
      tree decl = TREE_CHAIN(typ);

# if GCC_VERSION >= 4008
      xml_instantiations.safe_push (decl);
# else
      VEC_safe_push(tree, heap, xml_instantiations, decl);
# endif
    }
  }
}
